...
public interface DatasetIndexKeys {
  ...
  IndexKey DATASET_ID =
      IndexKey.newBuilder("id", "DATASET_ID", String.class)
  ...
  IndexKey UNQUOTED_NAME = IndexKey.newBuilder("uname", "SEARCH_NAME", String.class).build();
  IndexKey UNQUOTED_SCHEMA = IndexKey.newBuilder("uschm", "SEARCH_SCHEMA", String.class).build();

  // lower-cased paths for case insensitive matching.
  IndexKey UNQUOTED_LC_NAME =
      IndexKey.newBuilder(
              "ulname", UNQUOTED_NAME.getIndexFieldName() + LOWER_CASE_SUFFIX, String.class)
          .build();
  IndexKey UNQUOTED_LC_SCHEMA =
      IndexKey.newBuilder(
              "ulschm", UNQUOTED_SCHEMA.getIndexFieldName() + LOWER_CASE_SUFFIX, String.class)
          .build();
