...
/**
 * Representation for partition chunks that were created before the separation of dataset splits
 * into MultiSplit It will also be used, in the short term, while the code base is moving to a full
...
public class LegacyPartitionChunkMetadata extends AbstractPartitionChunkMetadata {
  ...
  public LegacyPartitionChunkMetadata(PartitionChunk partitionChunk) {
    ...
    Preconditions.checkArgument(
        !partitionChunk.hasSplitCount(), "Must be constructed with a legacy partitionChunk");
  }
  ...
  public Iterable<DatasetSplit> getDatasetSplits() {
    ...
    DatasetSplit split =
        DatasetSplit.newBuilder()
            .addAllAffinities(partitionChunk.getAffinitiesList())
            .setSize(partitionChunk.getSize())
            // N.B. "legacy" consumers of DatasetSplit expected extended property as part of split
            // extended info, but now
            // the "split" abstraction is called "partition chunk", so get the extended property
            // from partition chunk.
            // So "split" in this context, is different from split in multi-split. Or simply put,
