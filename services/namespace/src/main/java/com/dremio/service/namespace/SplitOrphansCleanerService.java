...
public class SplitOrphansCleanerService implements Service {
  ...
  public void start() throws Exception {
    ...
    cleanerTask = scheduler.get().schedule(Schedule.Builder.everyHours(splitOrphansCleanPeriodHour).build(),
        () -> {
          logger.info("Search for expired dataset splits");
          final int expired = namespaceService.deleteSplitOrphans(DatasetSplitId.SplitOrphansRetentionPolicy.KEEP_VALID_SPLITS);
