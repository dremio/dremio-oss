...
/**
 * Holds a collection of source capabilities. This is serializable and exposes a set of known
 * primitives. All classes used here are final to ensure that no references are connected back to
 * underlying sources from these objects. This exists to expose capabilities of the source that are
 * known based on either configuration, nature or as reported by connecting to the source.
 */
