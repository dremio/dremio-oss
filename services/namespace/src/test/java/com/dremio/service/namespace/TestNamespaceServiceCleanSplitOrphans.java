...
/**
 * Test for {@code
 * NamespaceServiceImpl#deleteSplitOrphans(com.dremio.service.namespace.PartitionChunkId.SplitOrphansRetentionPolicy)}
 */
public class TestNamespaceServiceCleanSplitOrphans {
  ...
  public void populate() throws Exception {
    ...
    addHome("foo");
    savePhysicalDataset(
        Arrays.asList("@foo", "uploadfile"),
        DatasetType.PHYSICAL_DATASET_HOME_FILE,
        now - TimeUnit.HOURS.toMillis(48),
        10);
    savePhysicalDataset(
        Arrays.asList("@foo", "uploadfolder"),
        DatasetType.PHYSICAL_DATASET_HOME_FOLDER,
    ...
    for (int i = 28; i >= 0; i--) {
      savePhysicalDataset(
          Arrays.asList("test", "dataset1"),
          DatasetType.PHYSICAL_DATASET,
          now - TimeUnit.HOURS.toMillis(i),
          100);
    }
    // Add a second dataset, refreshing once per day
    savePhysicalDataset(
        Arrays.asList("test", "dataset2"), DatasetType.PHYSICAL_DATASET_SOURCE_FOLDER, now, 1000);
    // Add a dataset which had splits but has been removed since
    int j = 0;
    for (int i = 16; i >= 5; i--) {
      j++;
      savePhysicalDataset(
          Arrays.asList("test", "dataset3"),
    ...
    assertThat(StreamSupport.stream(partitionChunksStore.find().spliterator(), false).count())
  ...
  public void testKeepCurrentVersion() throws Exception {
    ...
    generatePartitionChunks(now, 100)
        .forEach(
            split -> {
    ...
    DatasetConfig dataset2 =
        namespaceService.getDataset(new NamespaceKey(Arrays.asList("test", "dataset2")));
    generatePartitionChunks(now, 1000)
        .forEach(
            split -> {
    ...
    DatasetConfig dataset4 =
        namespaceService.getDataset(new NamespaceKey(Arrays.asList("other", "dataset4")));
    generatePartitionChunks(now, 100)
        .forEach(
            split -> {
  ...
  public void testKeepValidSplits() throws Exception {
    ...
    for (int i = 23; i >= 0; i--) {
      ...
      generatePartitionChunks(splitVersion, 100)
          .forEach(
              split -> {
  ...
  private void addHome(String name) throws Exception {
    final HomeConfig home =
        new HomeConfig().setId(new EntityId().setId(UUID.randomUUID().toString())).setOwner(name);
  ...
  public void savePhysicalDataset(List<String> path, DatasetType type, long splitVersion, int count)
      throws NamespaceException {
    final ReadDefinition readDefinition = new ReadDefinition().setSplitVersion(splitVersion);

    final DatasetConfig datasetConfig =
        saveDataset(path, type, config -> config.setReadDefinition(readDefinition));
    generatePartitionChunks(splitVersion, count)
        .forEach(
            split -> {
              partitionChunksStore.put(
                  PartitionChunkId.of(datasetConfig, split, splitVersion), split);
              multiSplitStore.put(
                  PartitionChunkId.of(datasetConfig, split, splitVersion),
                  createMultiSplit(split.getSplitKey()));
            });
  }

  private PartitionProtobuf.MultiSplit createMultiSplit(String splitKey) {
    return PartitionProtobuf.MultiSplit.newBuilder().setMultiSplitKey(splitKey).build();
  }
  ...
  public void saveVirtualDataset(List<String> path) throws NamespaceException {
    ...
    saveDataset(
        path, DatasetType.VIRTUAL_DATASET, config -> config.setVirtualDataset(virtualDataset));
  }

  private DatasetConfig saveDataset(
      List<String> path, DatasetType type, Function<DatasetConfig, DatasetConfig> transformer)
      throws NamespaceException {
    ...
    final DatasetConfig datasetConfig =
        transformer.apply(
            new DatasetConfig()
                .setId(
                    oldDataset
                        .map(DatasetConfig::getId)
                        .orElse(new EntityId().setId(UUID.randomUUID().toString())))
                ...
                PartitionChunk.newBuilder()
                    ...
                    .addPartitionValues(
                        PartitionValue.newBuilder()
                            .setColumn("column" + i)
                            .setIntValue(i)
                            .setType(PartitionValueType.IMPLICIT))
