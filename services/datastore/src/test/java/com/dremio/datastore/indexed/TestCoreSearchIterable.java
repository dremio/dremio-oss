...
public class TestCoreSearchIterable {
  ...
  /**
   * Test a possible scenario where index return documents which are not present in kvstore
   * Make sure the class guards against it by ignoring missing docs.
   */
  @Test
  public void testDeletedValue() {
    ...
    when(store.newKey()).then(new Answer<KVStoreTuple<String>>() {
      @Override
    ...
    when(store.newValue()).then(new Answer<KVStoreTuple<String>>() {
      @Override
      public KVStoreTuple<String> answer(InvocationOnMock invocation) throws Throwable {
        return new KVStoreTuple<>(StringSerializer.INSTANCE);
      }
    ...
    final com.dremio.datastore.api.Document<KVStoreTuple<String>,  KVStoreTuple<String>> foo1Doc = new ImmutableDocument.Builder<KVStoreTuple<String>,  KVStoreTuple<String>>()
        .setKey(foo1Key)
        .setValue(foo1Key)
        .setTag("test-tag")
        .build();
    ...
    final CoreSearchIterable<String, String> iterable = new CoreSearchIterable<>(store, index,
        LuceneQueryConverter.INSTANCE.toLuceneQuery(SearchQueryUtils.newTermQuery(queryKey, "bar")), new Sort(), 10, 0,
        10);
  ...
  /**
   * Tests a case where CoreSearchIterable thinks the search is not done (due to
   * deleted values), but LuceneSearchIndex thinks it's done.
   */
  @Test
  public void testIteratorWithDeletedEntries() {
    ...
    when(store.get(anyList())).thenAnswer(invocation -> {
      List<KVStoreTuple<String>> inputList =
        (List<KVStoreTuple<String>>)invocation.getArguments()[0];
      List<com.dremio.datastore.api.Document<KVStoreTuple<String>,
        KVStoreTuple<String>>> retDocs = new ArrayList<>();
      ...
      for (KVStoreTuple<String> entry : inputList) {
        ...
        retDocs.add(new ImmutableDocument.Builder<KVStoreTuple<String>,  KVStoreTuple<String>>()
          .setKey(entry)
          .setValue(entry)
          .setTag("test-tag")
          .build());
    ...
    final CoreSearchIterable<String, String> iterable = new CoreSearchIterable<>(store, index,
      LuceneQueryConverter.INSTANCE.toLuceneQuery(SearchQueryUtils.newTermQuery(queryKey, "bar")), new Sort(), totalEntries - nullValueEntries, 0,
