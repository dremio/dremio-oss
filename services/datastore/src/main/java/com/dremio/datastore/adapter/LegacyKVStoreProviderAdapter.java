...
public class LegacyKVStoreProviderAdapter implements LegacyKVStoreProvider {
  private static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(LegacyKVStoreProviderAdapter.class);
  ...
  public LegacyKVStoreProviderAdapter(KVStoreProvider underlyingProvider) {
    ...
    this.stores = CacheBuilder.newBuilder().build(CacheLoader.from(this::newStore));
  }
  ...
  public static LegacyKVStoreProvider inMemory(ScanResult scanResult) {
    final LocalKVStoreProvider kvStoreProvider =
        new LocalKVStoreProvider(scanResult, null, true, false);
  ...
  public <K, V, T extends LegacyKVStore<K, V>, U extends KVStore<K, V>> T getStore(
    ...
    return (T)
        Preconditions.checkNotNull(
            stores.getUnchecked(creator), "Unknown store creator %s", creator.getName());
  }
  ...
  private LegacyKVStore<?, ?> newStore(
      Class<? extends LegacyStoreCreationFunction<?, ?, ?, ?>> functionClass) {
    try {
      ...
      return functionClass.getDeclaredConstructor().newInstance().build(factory);
    } catch (Exception e) {
  ...
  private <K, V> LegacyStoreBuilder<K, V> newStoreBuilder(
      Class<? extends LegacyStoreCreationFunction<?, ?, ?, ?>> functionClass) {
    return new LegacyStoreBuilderAdapter<K, V>(() -> underlyingProvider.newStore()) {
      @Override
      protected KVStore<K, V> doBuild() {
        return underlyingProvider.getStore(
            (Class<? extends StoreCreationFunction<K, V, KVStore<K, V>>>) functionClass);
      }

      @Override
      protected IndexedStore<K, V> doBuildIndexed(DocumentConverter<K, V> documentConverter) {
        return underlyingProvider.getStore(
            (Class<? extends StoreCreationFunction<K, V, IndexedStore<K, V>>>) functionClass);
      }
    };
  }
  ...
  public static class LegacyStoreBuilderAdapter<K, V> extends LegacyAbstractStoreBuilder<K, V> {
    ...
    public LegacyStoreBuilderAdapter(
        Supplier<KVStoreProvider.StoreBuilder<K, V>> underlyingStoreBuilderFactory) {
    ...
    public LegacyKVStore<K, V> build() {
      return new LegacyKVStoreAdapter<>(
          doBuild(), getStoreBuilderHelper().tryGetVersionExtractor());
    }
    ...
    public LegacyIndexedStore<K, V> buildIndexed(DocumentConverter<K, V> documentConverter) {
      return new LegacyIndexedStoreAdapter<>(
          doBuildIndexed(documentConverter), getStoreBuilderHelper().tryGetVersionExtractor());
    }

    /**
     * Helper method to set configurations in StoreBuilder with settings provided by the this
     * LegacyStoreBuilder.
     *
    ...
    private void populateBuilder(
        KVStoreProvider.StoreBuilder<K, V> builder,
