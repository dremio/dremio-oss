...
public final class SearchFilterToQueryConverter {
  ...
  private SearchQuery buildLeafNodeQuery(final ComparisonNode node)
    ...
    if (node.getOperator().equals(RSQLOperators.EQUAL)) {
      ...
      return createRangeQuery(
          clazz, name, processedArgs.get(0), RSQLOperators.GREATER_THAN_OR_EQUAL);
      ...
      throw new IllegalArgumentException(
          format("%s: Invalid search operator %s", visitorName, node.toString()));
    }
  }

  // TODO case insensitive search?
  private SearchQuery createEqualsQuery(
      final Class<?> cls,
      final String name,
      final Object value,
  ...
  private SearchQuery createInQuery(
      final Class<?> cls,
      final String name,
  ...
  private SearchQuery createRangeQuery(
      Class<?> cls, String name, Object value, ComparisonOperator type) {

    boolean minInclusive =
        type == RSQLOperators.GREATER_THAN_OR_EQUAL || type == RSQLOperators.EQUAL;
    ...
    if (Number.class.isAssignableFrom(cls)) {
      ...
      throw new IllegalArgumentException(
          format(
              "%s: Can not do range query on field %s of type %s"
                  + "only long, int, double, float types are supported",
              visitorName, name, cls.getName()));
    }
  }

  private SearchQuery createIntRangeQuery(
      final String name,
      final Object value,
      final ComparisonOperator type,
      final boolean minInclusive,
      final boolean maxInclusive) {
    final Integer intValue = (Integer) value;

    final Integer minValue =
        (type != RSQLOperators.LESS_THAN && type != RSQLOperators.LESS_THAN_OR_EQUAL)
            ? intValue
            : null;
    final Integer maxValue =
        (type != RSQLOperators.GREATER_THAN && type != RSQLOperators.GREATER_THAN_OR_EQUAL)
            ? intValue
            : null;
  ...
  private SearchQuery createLongRangeQuery(
      final String name,
      final Object value,
      final ComparisonOperator type,
      final boolean minInclusive,
  ...
  private SearchQuery createDoubleRangeQuery(
      final String name,
      final Object value,
      final ComparisonOperator type,
      final boolean minInclusive,
  ...
  private SearchQuery createFloatRangeQuery(
      final String name,
      final Object value,
      final ComparisonOperator type,
      final boolean minInclusive,
  ...
  private SearchQuery createStringRangeQuery(
      final String name,
      final Object value,
      final ComparisonOperator type,
      final boolean minInclusive,
  ...
  private void checkAtleastOneArgument(final ComparisonNode node) {
    if (node.getArguments().size() < 1) {
      throw new IllegalArgumentException(
          format(
              "%s: Invalid search operator %s, needs at least one argument",
              visitorName, node.toString()));
    }
  }

  private List<Object> validate(final ComparisonNode node, final String name, final Class<?> clazz)
      throws IllegalArgumentException {
    ...
    if (name == null) {
      throw new IllegalArgumentException(
          format(
              "%s: Invalid search operator %s, unknown field %s",
              visitorName, node.toString(), node.getSelector()));
    }
    if (clazz == null) {
      throw new IllegalArgumentException(
          format(
              "%s: Invalid search operator %s, unknown type for field %s",
              visitorName, node.toString(), node.getSelector()));
    }
    ...
    if (Number.class.isAssignableFrom(clazz)) {
      ...
      throw new IllegalArgumentException(
          format(
              "%s: Invalid numeric field type %s for field %s in search operator %s, "
                  + "only long, int, double, float types are supported",
              visitorName, clazz.toString(), name, node.toString()));
      ...
      throw new IllegalArgumentException(
          format(
              "%s: Invalid field type %s for field %s in search operator %s, "
                  + "only numeric, string or boolean types are supported",
              visitorName, clazz.toString(), name, node.toString()));
    }
    return arguments;
  }
