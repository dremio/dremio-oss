...
public interface SearchTypeVisitor<R> {
  ...
  default R visit(SearchTypes.SearchQuery query) {
    switch (query.getType()) {
      ...
      case TERM_FLOAT:
      default:
        throw new UnsupportedOperationException(String.format("Unrecognized query type encountered: %s", query.getType().name()));
    }
  }
  ...
  /**
   * Method visited when a {@code Exists} is passed. When the flag
   * {@code doesItExist} is true, the field must exist to satisfy
   * the filter. When the flag is false, the field must not exist
