...
public class CoreIndexedStoreImpl<K, V> implements CoreIndexedStore<K, V> {
  private static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(CoreIndexedStoreImpl.class);
  ...
  public static final IndexKey ID_KEY =
      IndexKey.newBuilder(
              CoreIndexedStore.ID_FIELD_NAME, CoreIndexedStore.ID_FIELD_NAME, String.class)
          .setStored(true)
          .build();

  private class ReindexThread extends Thread {
    private final Iterator<com.dremio.datastore.api.Document<KVStoreTuple<K>, KVStoreTuple<V>>>
        iterator;
    ...
    public ReindexThread(
        Iterator<com.dremio.datastore.api.Document<KVStoreTuple<K>, KVStoreTuple<V>>> iterator,
        Object lock,
  ...
  public int reindex() {
    ...
    index.forReindexing(
        () -> {
          ...
          final Iterator<com.dremio.datastore.api.Document<KVStoreTuple<K>, KVStoreTuple<V>>>
  ...
  @Override
  public com.dremio.datastore.api.Document<KVStoreTuple<K>, KVStoreTuple<V>> get(
      KVStoreTuple<K> key, GetOption... options) {
    final com.dremio.datastore.api.Document<KVStoreTuple<K>, KVStoreTuple<V>> document =
        base.get(key, options);
  ...
  @Override
  public Iterable<com.dremio.datastore.api.Document<KVStoreTuple<K>, KVStoreTuple<V>>> find(
      FindOption... options) {
    final Iterable<com.dremio.datastore.api.Document<KVStoreTuple<K>, KVStoreTuple<V>>> documents =
        base.find(options);
  ...
  private void logKVPairsWithNullValues(
      com.dremio.datastore.api.Document<KVStoreTuple<K>, KVStoreTuple<V>> document) {
    if (document != null
  ...
  @Override
  public com.dremio.datastore.api.Document<KVStoreTuple<K>, KVStoreTuple<V>> put(
    ...
    final com.dremio.datastore.api.Document<KVStoreTuple<K>, KVStoreTuple<V>> doc =
        base.put(key, v, filteredOptions);
  ...
  private void index(KVStoreTuple<K> key, PutOption... options) {
    for (PutOption option : options) {
      switch (option.getPutOptionInfo().getType()) {
        ...
        default:
          logger.debug(
              "PutOption type {} ignored while performing remote index.",
              option.getPutOptionInfo().getType());
          break;
      }
    }
  }
  ...
  private Document toDoc(KVStoreTuple<K> key, PutOption option) {
    ...
    for (RemoteDataStoreProtobuf.PutRequestIndexField indexField :
        putOptionInfo.getIndexFieldsList()) {
      ...
      if (indexField.getValueDoubleCount() > 0) {
        ...
        for (int i = 0; i < indexField.getValueBytesList().size(); i++) {
          ...
          try {
            ...
            throw new IllegalStateException(
                String.format(
                    "Problem reading binary data from field: %s", indexKey.getIndexFieldName()),
                e);
          }
        ...
        documentWriter.write(
            ...
            indexField
                .getValueStringList()
                .toArray(new String[indexField.getValueStringList().size()]));
      } else {
        throw new IllegalStateException(
            String.format(
                "Unknown index field type for field name: %s",
                indexField.getKey().getIndexFieldName()));
      }
    }
  ...
  @Override
  public Iterable<com.dremio.datastore.api.Document<KVStoreTuple<K>, KVStoreTuple<V>>> get(
      List<KVStoreTuple<K>> keys, GetOption... options) {
    final Iterable<com.dremio.datastore.api.Document<KVStoreTuple<K>, KVStoreTuple<V>>> documents =
        base.get(keys, options);
  ...
  @Override
  public Iterable<com.dremio.datastore.api.Document<KVStoreTuple<K>, KVStoreTuple<V>>> find(
      FindByRange<KVStoreTuple<K>> find, FindOption... options) {
    final Iterable<com.dremio.datastore.api.Document<KVStoreTuple<K>, KVStoreTuple<V>>> documents =
        base.find(find, options);
  ...
  public void bulkDelete(List<KVStoreTuple<K>> keysToDelete, DeleteOption... deleteOptions) {
    base.bulkDelete(keysToDelete, deleteOptions);
  }
  ...
  public Iterable<com.dremio.datastore.api.Document<KVStoreTuple<K>, KVStoreTuple<V>>> find(
    ...
    // The FindOptions currently can't be propagated anywhere. This needs to be revisited when there
    // are FindOptions, since they could be
  ...
  private Sort toLuceneSort(List<SearchFieldSorting> orderings) {
    ...
    for (SearchFieldSorting ordering : orderings) {
      ...
      sortFields[i++] =
          new SortField(ordering.getField(), fieldType, ordering.getOrder() != SortOrder.ASCENDING);
    }
