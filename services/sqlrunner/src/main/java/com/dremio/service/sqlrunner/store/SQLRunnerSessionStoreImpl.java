...
public class SQLRunnerSessionStoreImpl implements SQLRunnerSessionStore {
  ...
  private static final IndexKey USER_ID = IndexKey.newBuilder("user_id", "USER_ID", String.class)
    .setSortedValueType(SearchTypes.SearchFieldSorting.FieldType.STRING)
    .build();
  private static final IndexKey EXPIRE_AT = IndexKey.newBuilder("expire_at", "EXPIRE_AT", Date.class)
      .setSortedValueType(SearchTypes.SearchFieldSorting.FieldType.LONG)
      .build();
  private static final IndexKey EXPIRE_AT_LONG = IndexKey.newBuilder("expire_at_long", "EXPIRE_AT_LONG", Long.class)
    .setSortedValueType(SearchTypes.SearchFieldSorting.FieldType.LONG)
    .build();
  ...
  public void start() throws Exception {
    store = Suppliers.memoize(() -> kvStoreProvider.get()
      .getStore(SQLRunnerSessionStoreImpl.StoreCreator.class));
  }
  ...
  public SQLRunnerSession update(SQLRunnerSession updatedSession) {
    Preconditions.checkArgument(StringUtils.isNotEmpty(updatedSession.getUserId()), "userId must not be empty.");
    Document<String, SQLRunnerSession> doc = store.get().put(updatedSession.getUserId(), updatedSession);
    return doc.getValue();
  }
  ...
  public int deleteExpired() {
    ...
    final SearchTypes.SearchQuery query = SearchQueryUtils.newRangeLong(EXPIRE_AT_LONG.getIndexFieldName(), 0L, now, true, false);
   ...
   * StoreCreator
   */
  public static final class StoreCreator implements IndexedStoreCreationFunction<String, SQLRunnerSession> {
    @Override
    public IndexedStore<String, SQLRunnerSession> build(StoreBuildingFactory factory) {
      return factory.<String, SQLRunnerSession>newStore()
        .name(STORE_NAME)
  ...
  static final class SQLRunnerSessionDocumentConverter implements DocumentConverter<String, SQLRunnerSession> {
