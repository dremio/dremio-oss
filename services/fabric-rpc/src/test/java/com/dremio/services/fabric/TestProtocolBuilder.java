...
public class TestProtocolBuilder extends BaseTestFabric {
  ...
  public void basicProtocolNoBuffers() throws Exception {
    ProtocolBuilder builder =
        ProtocolBuilder.builder().allocator(allocator).name("test-fabric").protocolId(3);

    SendEndpointCreator<NodeEndpoint, QueryId> ep1 =
        builder.register(
            1,
            new AbstractReceiveHandler<NodeEndpoint, QueryId>(
                NodeEndpoint.getDefaultInstance(), QueryId.getDefaultInstance()) {

              @Override
              public SentResponseMessage<QueryId> handle(NodeEndpoint request, ArrowBuf dBody)
                  throws RpcException {
    ...
    ReceivedResponseMessage<QueryId> resp1 =
        ep1.getEndpoint(address, fabric.getPort()).send(expectedD);
  ...
  public void protocolWithBuffers() throws Exception {
    try (ArrowBuf random1 = allocator.buffer(1024);
      ...
      SendEndpointCreator<NodeEndpoint, QueryId> ep2 =
          builder.register(
              2,
              new AbstractReceiveHandler<NodeEndpoint, QueryId>(
                  NodeEndpoint.getDefaultInstance(), QueryId.getDefaultInstance()) {
      ...
      ReceivedResponseMessage<QueryId> resp2 =
          ep2.getEndpoint(address, fabric.getPort()).send(expectedD, random1);
