...
public class OrphanageCleanerService implements Service {
  ...
  private static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(OrphanageCleanerService.class);
  ...
  public OrphanageCleanerService(
                        ...
                        Schedule.Builder.singleShotChain()
                            .startingAt(
                                Instant.ofEpochMilli(
                                    System.currentTimeMillis() + getCleanUpPeriodInMillis()))
                            .asClusteredSingleton(LOCAL_TASK_LEADER_NAME)
                            .build(),
                        this);
              }
  ...
  public int cleanup() {
    try {
      if (isDistributedMaster) {
        Iterator<Document<OrphanEntry.OrphanId, OrphanEntry.Orphan>> orphanEntryIterator =
            orphanage.getAllOrphans().iterator();
        if (!orphanEntryIterator.hasNext()) {
          logger.debug("No orphan dataset entry to clean up");
        }
        List<Document<OrphanEntry.OrphanId, OrphanEntry.Orphan>> orphanEntries =
            Lists.newArrayList(
  ...
  private long getCleanUpPeriodInMillis() {
    return TimeUnit.MINUTES.toMillis(
        optionManager.get().getOption(ExecConstants.ORPHANAGE_ENTRY_CLEAN_PERIOD_MINUTES));
  }
