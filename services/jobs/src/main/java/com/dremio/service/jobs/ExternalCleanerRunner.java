...
class ExternalCleanerRunner {
  ...
  public void run(JobResult jobResult) {
    if (jobResult == null || jobResult.getAttemptsList() == null || jobResult.getAttemptsList().isEmpty()) {
      return;
    }
  ...
  void printLastErrors() {
   ...
   String getReport() {
    ...
    sb.append(System.lineSeparator()).append("\tJobAttempts: ").append(jobAttemptExecution)
    ...
    for (ExternalCleaner externalCleaner : externalCleaners) {
      ...
      sb.append(System.lineSeparator()).append("\t").append(cleanerName).append(" executions: ")
        .append(total).append(", failures: ").append(failures);
    }
