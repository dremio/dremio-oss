...
public class JobsFlightProducer implements FlightProducer, AutoCloseable {
  ...
  public JobsFlightProducer(Provider<LocalJobsService> jobsService, BufferAllocator allocator) {
    ...
    this.allocator =
        checkNotNull(allocator).newChildAllocator("jobs-flight-producer", 0, Long.MAX_VALUE);
  }
  ...
  public void getStream(
     ...
     */
    try {
      final JobsFlightTicket jobsFlightTicket =
          CoordinatorFlightTicket.parseFrom(ticket.getBytes()).getJobsFlightTicket();
      final JobProtobuf.JobId jobId =
          JobProtobuf.JobId.newBuilder().setId(jobsFlightTicket.getJobId()).build();
      final int offset = jobsFlightTicket.getOffset();
      final int limit = jobsFlightTicket.getLimit();

      try (final JobDataFragment jobDataFragment =
          jobsService.get().getJobData(JobsProtoUtil.toStuff(jobId), offset, limit)) {
  ...
  @Override
  // TODO (DX-19234): Implement me. Standard Flight procedure requires clients call getFlightInfo
