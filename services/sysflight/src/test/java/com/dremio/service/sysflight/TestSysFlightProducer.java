...
public class TestSysFlightProducer extends BaseTestQuery {
  ...
  public static void setup() throws Exception {
    allocator =
        getDremioRootAllocator().newChildAllocator("sys-flight-allocator", 0, Long.MAX_VALUE);
    SystemTableManager tableManager =
        new SystemTableManagerImpl(allocator, SYS_FLIGHT_RESOURCE::getTablesProvider, null);
  ...
  public static void tearDown() throws Exception {
    ...
    await()
        ...
        .untilAsserted(
            () ->
                assertEquals(
                    "Not all child allocators were closed.",
                    0,
  ...
  public void testFlightInfoAndData() throws Exception {
    ...
    final CoordinatorFlightTicket ticket =
        CoordinatorFlightTicket.newBuilder()
            .setSyFlightTicket(
                SysFlightTicket.newBuilder().setDatasetName(TABLES.JOBS.getName()).build())
            .build();
    ...
    try (FlightStream stream = client.getStream(new Ticket(ticket.toByteArray()))) {
      while (stream.next()) {
        ...
        Assert.assertEquals(
            info.getSchema(), ProtobufRecordReader.getSchema(ActiveJobSummary.getDescriptor()));
      }
    }
  ...
  public void testFlightStreamBatchedMethod(int recordBatchSize) throws Exception {
    ...
    try (FlightStream stream = client.getStream(new Ticket(ticket.toByteArray()))) {
      while (stream.next()) {
        rowCount = rowCount + stream.getRoot().getRowCount();
        Assert.assertEquals(
            Math.min(expectedRowCount, recordBatchSize), stream.getRoot().getRowCount());
        batchCount++;
