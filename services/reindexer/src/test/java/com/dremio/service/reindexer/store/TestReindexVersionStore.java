...
public class TestReindexVersionStore extends DremioTest {
  ...
  private static final org.slf4j.Logger LOGGER =
      org.slf4j.LoggerFactory.getLogger(TestReindexVersionStore.class);
  ...
  public void setup() throws Exception {
    ...
    reindexVersionStore =
        new ReindexVersionStoreImpl(() -> kvStoreProvider, TestReindexVersionStoreCreator.class);
  ...
  private ReindexVersionInfo createReindexVersionInfo() {
    ...
    ReindexVersionInfo reindexVersionInfo =
        ReindexVersionInfo.newBuilder()
            .setVersion(VERSION)
  ...
  public void testVersionInfoCollection() {
    ImmutableMap<Class<? extends StoreCreationFunction<?, ?, ?>>, KVStore<?, ?>> stores =
        StoreLoader.buildStores(
            DremioTest.CLASSPATH_SCAN_RESULT, TestReindexVersionStore.this::newStore);
    assertTrue(stores.containsKey(TestReindexVersionStoreCreator.class));
    assertEquals(
        stores.get(TestReindexVersionStoreCreator.class).getName(), VERSION_INFO_COLLECTION);
  }
