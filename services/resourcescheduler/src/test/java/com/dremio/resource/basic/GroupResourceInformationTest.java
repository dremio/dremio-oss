...
public class GroupResourceInformationTest {
  private final CoordinationProtos.NodeEndpoint ep1 = CoordinationProtos.NodeEndpoint.newBuilder()
    .setAddress("172.21.1.1")
    .setFabricPort(30000)
    .setAvailableCores(32)
    .build();
  private final CoordinationProtos.NodeEndpoint ep2 = CoordinationProtos.NodeEndpoint.newBuilder(ep1)
    .setAddress("172.21.1.2")
    .build();
  ...
  public void configuredWidth() {
    ...
    when(optionManager.getOption(GroupResourceInformation.MAX_WIDTH_PER_NODE_FRACTION)).thenReturn(0.9);

    GroupResourceInformation groupInfo = new SelectedExecutorsResourceInformation(ImmutableList.of(ep1, ep2));
  ...
  public void configuredWidthFraction() {
    ...
    when(optionManager.getOption(GroupResourceInformation.MAX_WIDTH_PER_NODE_FRACTION)).thenReturn(0.25);

    GroupResourceInformation groupInfo = new SelectedExecutorsResourceInformation(ImmutableList.of(ep1, ep2));
    assertEquals(8, groupInfo.getAverageExecutorCores(optionManager));

    when(optionManager.getOption(GroupResourceInformation.MAX_WIDTH_PER_NODE_FRACTION)).thenReturn(0.75);
