...
public class StatisticsListManagerImpl implements StatisticsListManager {
  ...
  public StatisticsListManagerImpl(
    ...
    this.statisticsServiceProvider =
        Preconditions.checkNotNull(statisticsServiceProvider, "statistics service required");
  ...
  public Iterable<StatisticsListManager.StatisticsInfo> getStatisticsInfos() {
    ...
    if (!master.isPresent()) {
      throw UserException.connectionError()
          .message("Unable to get task leader while trying to get statistics information")
          .build(logger);
    }
    ...
    try {
      ...
      return statisticsInfoResp.getStatisticsInfoList().stream()
          .map(StatisticsInfo::fromProto)
          .collect(Collectors.toList());
    } catch (InterruptedException | ExecutionException | TimeoutException e) {
      throw UserException.connectionError(e)
          .message("Error while getting statistics information")
          .build(logger);
    }
  }
  ...
  public void start() throws Exception {
    final FabricRunnerFactory statisticsTunnelFactory =
        fabric
            .get()
            .registerProtocol(
                new StatisticsProtocol(
                    allocatorProvider.get(),
                    dremioConfigProvider.get().getSabotConfig(),
                    statisticsServiceProvider.get()));
  ...
  public static class StatisticsTunnelCreator {
    ...
    public StatisticsTunnel getTunnel(CoordinationProtos.NodeEndpoint ep) {
      return new StatisticsTunnel(
          ep, factory.getCommandRunner(ep.getAddress(), ep.getFabricPort()));
    }
  }
}
