...
public class TestTaskLeaderSchedulerService extends DremioTest {
  ...
  public void testTaskLeaderChangeListener() throws Exception {
    try (ClusterCoordinator coordinator = LocalClusterCoordinator.newRunningCoordinator()) {
      ...
      Cancellable cancellable1 = schedulerService.schedule(Schedule.Builder.everySeconds(3L).asClusteredSingleton("test").build(), new Runnable() {
        @Override
        public void run() {
          synchronized (this) {
  ...
  public void testAfterCancellation() throws Exception {
    try (ClusterCoordinator coordinator = LocalClusterCoordinator.newRunningCoordinator()) {
      ...
      AtomicBoolean checkGainedLeadership = new AtomicBoolean(false);
      AtomicInteger regularRuns = new AtomicInteger(0);
      Cancellable cancellable1 = schedulerService.schedule(Schedule.Builder.everySeconds(3L).asClusteredSingleton("abc").build(), new Runnable() {
        @Override
        public void run() {
          synchronized (this) {
            logger.info("Entering run");
  ...
  public void testLosingAndRegainingLeadershipAndTaskStillRunningInTheNode() throws Exception {
    try(ClusterCoordinator coordinator = LocalClusterCoordinator.newRunningCoordinator()) {
      ...
      CloseableSchedulerThreadPool schedulerPool = new CloseableSchedulerThreadPool("test-scheduler", 2);
      ...
      // simulate gain the leadership again. Previous task was not finished, we should continue having only 1 task
      taskLeaderElection.onElectedLeadership();
      Thread.sleep(1000);
      Assert.assertEquals(1, counter.get());
