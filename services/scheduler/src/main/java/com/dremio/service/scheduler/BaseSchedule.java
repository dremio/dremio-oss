...
class BaseSchedule implements Schedule {
    ...
    this.largePeriodicity =
        (singleShotType == null
            && ((amount == null && at.isAfter(Instant.now().plus(Duration.ofSeconds(61))))
                || (amount != null && moreThanOneMinute(amount))));
  }
  ...
  public boolean isToRunExactlyOnce() {
    // true if it is run once and only once per upgrade
    return SingleShotType.RUN_ONCE_EVERY_UPGRADE.equals(singleShotType)
        || SingleShotType.RUN_EXACTLY_ONCE.equals(singleShotType);
  }
  ...
  public Iterator<Instant> iterator() {
    ...
    final LocalDateTime start =
        adjustedAt.isBefore(atLocal) ? atLocal.plus(amount).with(adjuster) : adjustedAt;
    ...
    return new Iterator<Instant>() {
      ...
      public void remove() {
        throw new UnsupportedOperationException(
            "Schedule iterator does not support remove operation.");
      }
    };
  }
  ...
  public Long getScheduledOwnershipReleaseInMillis() {
    Preconditions.checkState(
        taskName != null, "Name of the task for which to release scheduled ownership is missing");
  ...
  private static boolean moreThanOneMinute(TemporalAmount amount) {
    if (amount instanceof Duration) {
      return (Duration.from(amount).toMinutes() > 1);
    }
    return true;
  }
}
