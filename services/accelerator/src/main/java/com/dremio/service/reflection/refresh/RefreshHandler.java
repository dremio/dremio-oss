...
public class RefreshHandler implements SqlToPlanHandler {
  ...
  public PhysicalPlan getPlan(SqlHandlerConfig config, String sql, SqlNode sqlNode) throws Exception {
    try{
      // Disable row level runtime filtering so the materialization can be properly marked if it can be used as starflake or not.
      config.getContext().getOptions().setOption(
      ...
      if (!isCreate && materialization.getIsIcebergDataset()) {
        Materialization lastDoneMaterialization = Preconditions.checkNotNull(materializationStore.getLastMaterializationDone(reflectionId),
        ...
        extendedByteString = oldReflection.getDatasetConfig().getReadDefinition().getExtendedProperty();
      }
      ...
      WriterOptions writerOptions = writerOptionManager.buildWriterOptionForReflectionGoal(
        0, goal, fields, materialization.getIsIcebergDataset(), isCreate, extendedByteString,snapshotDiffContextPointer.value);

      final ReflectionPartitionInfo reflectionPartitionInfo = buildReflectionPartitionInfo(materialization,config, goal, batchSchema);
      ...
      IcebergTableProps icebergTableProps = materialization.getIsIcebergDataset() ?
        getIcebergTableProps(materialization, refreshDecisions, attemptId, writerOptions.getPartitionColumns(),
          writerOptions, reflectionPartitionInfo,snapshotDiffContextPointer.value)
        : null;

      Rel writerDrel = null;
      if(snapshotDiffContextPointer.value != null &&
        snapshotDiffContextPointer.value.getFilterApplyOptions() == SnapshotDiffContext.FilterApplyOptions.FILTER_PARTITIONS){
        writerDrel = getIcebergIncrementalRefreshWriterRel(snapshotDiffContextPointer.value,
          materialize, sql, service, entry, config, tablePath, icebergTableProps, writerOptions, initial, oldReflection,
          materializationStore);
      }
      //either first republish, or we could not use incremental refresh
      if(writerDrel == null) {
        writerDrel = new WriterRel(
          drel.getCluster(),
  ...
  /**
   * Builds an IncrementalRefreshByPartitionWriterRel that is used for persisting the updated reflection
   * For now this class is only used when we have Incremental Refresh by Partition
   * We pass in some additional arguments that are needed to build a delete plan
   * which will remove any data from the old reflection belonging to the updated partitions.
   * The data in those partition is then recalculated, and inserted into the reflection
   *
   ...
   * @param service              the reflection service
   ...
   */
  private Rel getIcebergIncrementalRefreshWriterRel(final SnapshotDiffContext snapshotDiffContext,
                                                    final SqlRefreshReflection materialize, final String sql,
                                                    final ReflectionService service, final ReflectionEntry entry,
                                                    final SqlHandlerConfig config, final List<String> tablePath,
                                                    final IcebergTableProps icebergTableProps, final WriterOptions writerOptions,
                                                    final RelNode initial, final DremioTable oldReflection, MaterializationStore materializationStore)
  {
    //build currentReflection using OldReflection and the latest snapshot ID from the ReflectionManager
    final ReflectionManager reflectionManager = service.getReflectionManager();
    ...
    if(latestRefresh == null){
      throw SqlExceptionHelper.parseError(
        ...
        sql, materialize.getReflectionIdPos()).build(logger);
    }
    final NamespaceKey namespaceKey = oldReflection.getPath();
    String currentReflectionSnapshotID = null;
    if (latestRefresh.getIsIcebergRefresh() != null && latestRefresh.getIsIcebergRefresh()) {
      currentReflectionSnapshotID = reflectionManager.getIcebergSnapshot(latestRefresh, entry.getId()).toString();
    }
    if(currentReflectionSnapshotID == null){
      throw SqlExceptionHelper.parseError(
        "Could not find the Iceberg table and latest SnapshotID of a reflection to incrementally update.",
        sql, materialize.getReflectionIdPos()).build(logger);
    }

    final MetadataRequestOptions mdRequestOpt = MetadataRequestOptions.of(
      SchemaConfig.newBuilder(CatalogUser.from(SystemUser.SYSTEM_USERNAME)).build());
    DremioTable currentReflection = null;
    final String errorMessage = "Could not find reflection snapshot to perform incremental update on. Details: latestRefresh path="
      + latestRefresh.getPath() + ". SnapshotID=" + currentReflectionSnapshotID
      + ". oldReflection path=" + oldReflection.getPath().toString();
    try {
      currentReflection = config.getContext().getCatalogService().
        getCatalog(mdRequestOpt).getTableSnapshot(
          namespaceKey, new TableVersionContext(TableVersionType.SNAPSHOT_ID, currentReflectionSnapshotID));
    } catch(IllegalArgumentException e){
      SqlExceptionHelper.parseError(errorMessage, sql, materialize.getReflectionIdPos()).build(logger);
    }
    if(currentReflection == null) {
      SqlExceptionHelper.parseError(errorMessage, sql, materialize.getReflectionIdPos()).build(logger);
    }
    //build a dremioPrepareTable for currentReflection
    JavaTypeFactory javaTypeFactory = config.getConverter().getTypeFactory();
    final DremioCatalogReader dremioCatalogReader
      = new DremioCatalogReader(config.getConverter().getPlannerCatalog());
    DremioPrepareTable dremioPrepareTable = new DremioPrepareTable(dremioCatalogReader, javaTypeFactory, currentReflection);
  ...
  private String getInheritedReflectionRouting(boolean isQueue, DatasetConfig datasetConfig, SqlHandlerConfig config) throws Exception {
    ...
    while (!pathList.isEmpty()) {
      if (pathList.size() == 1) {
        try {
          SpaceConfig spaceConfig = config.getContext().getSystemNamespaceService().getSpace(new NamespaceKey(pathList));
          if (isQueue) {
        ...
        try {
          FolderConfig folderConfig = config.getContext().getSystemNamespaceService()
  ...
  private IcebergTableProps getIcebergTableProps(final Materialization materialization,
    ...
    if (isIcebergInsertRefresh(materialization, refreshDecisions[0])) {
      ...
      if(snapshotDiffContext != null &&
        snapshotDiffContext.getFilterApplyOptions() == SnapshotDiffContext.FilterApplyOptions.FILTER_PARTITIONS){
        icebergCommandType = IcebergCommandType.UPDATE;
      }
      icebergTableProps = new IcebergTableProps(null, attemptId.toString(),
        reflectionPartitionInfo.getBatchSchemaToUse(), partitionColumns,
        icebergCommandType, null, materialization.getBasePath(),
        null, null, reflectionPartitionInfo.getPartitionSpecByteString(), reflectionPartitionInfo.getSerializedIcebergSchemaAsJson(), null, null, Collections.emptyMap(), null);
    } else {
      icebergTableProps = new IcebergTableProps(null, attemptId.toString(),
        reflectionPartitionInfo.getBatchSchemaToUse(), partitionColumns,
        IcebergCommandType.CREATE, null, materialization.getId().getId() + "_" + attemptId.getAttemptNum(),
        null, null, reflectionPartitionInfo.getPartitionSpecByteString(), reflectionPartitionInfo.getSerializedIcebergSchemaAsJson(), null, null, Collections.emptyMap(), null);
    }

    if(reflectionPartitionInfo.getPartitionSpecByteString() != null) {
      final IcebergWriterOptions icebergOptions = new ImmutableIcebergWriterOptions.Builder()
        .from(writerOptions.getTableFormatOptions().getIcebergSpecificOptions())
        .setIcebergTableProps(icebergTableProps)
        .build();
      final TableFormatWriterOptions tableFormatWriterOptions = new ImmutableTableFormatWriterOptions.Builder()
      ...
      final BatchSchema writerSchema = getWriterSchema(reflectionPartitionInfo.batchSchemaToUse, writerOptions);
  ...
  private class ReflectionPartitionInfo{
    ...
    public ReflectionPartitionInfo(final BatchSchema batchSchemaToUse,final ByteString partitionSpecByteString,final String serializedIcebergSchemaAsJson){
  ...
  /**
   * Build Iceberg partition info for the reflection.
   * We need to build Iceberg Schema to carry the Iceberg partition Info.
   * Some Arrow types do NOT have corresponding Iceberg types, so building Iceberg Schema will fail
   * In that case we will set all the incoming pointers to null and fall back to using partition columns if possible
   * If not possible we will error out and fail
   * Example of problematic cases - CONVERT_FROM(ddd,'JSON') as col1, NULL as col_2
   * In both cases above the type is NULL, and we will fail when we try to build an Iceberg Schema from the batchSchema
   */
  private ReflectionPartitionInfo buildReflectionPartitionInfo(final Materialization materialization,
    ...
    final List<PartitionTransform> transforms = buildPartitionTransforms(reflectionDetails.getPartitionFieldList());

    Optional<PartitionTransform> firstNonIdentity = transforms.stream().filter(x-> !x.isIdentity()).findFirst();
    if(firstNonIdentity.isPresent()){
      ReflectionUtils.validateNonIdentityTransformAllowed(sqlHandlerConfig.getContext().getOptions(),firstNonIdentity.get().getType().getName());
      if(!materialization.getIsIcebergDataset()){
        throw new UnsupportedOperationException(String.format("[%s] partition transform is present, but the reflection uses a non-Iceberg materialization.",
    ...
    try {
      ...
      if(firstNonIdentity.isPresent()){
        //rethrow the error, we cannot support transformation functions and unsupported datatype together
        throw new UnsupportedOperationException(
          "A combination of Iceberg partition transform and non-Iceberg data types is not supported in the same reflection."
            + e.getMessage());
      }
      //if there are no transforms (all are identity) we can support this workflow by falling back to non-Iceberg workflow using partition
      //this will prevent regressions on cases like dx-59542 and dx-47572
      return new ReflectionPartitionInfo(null,null,null);
    }
    final PartitionSpec partitionSpec = getIcebergPartitionSpecFromTransforms(batchSchema,transforms,icebergSchema);
    return new ReflectionPartitionInfo(batchSchema,
  ...
  private RelNode determineMaterializationPlan(
    ...
    final Pointer<SnapshotDiffContext> snapshotDiffContextPointer) {
    ...
    // First, generate the plan with no DRRs to determine if the refresh method is incremental or full.
    sqlHandlerConfig.getConverter().getSubstitutionProvider().disableDefaultRawReflection();
    HandlerToPreparePlanBase.RecordingObserver recordingObserver = new HandlerToPreparePlanBase.RecordingObserver();
    SqlHandlerConfig recordingConfig = sqlHandlerConfig.cloneWithNewObserver(recordingObserver);
    ReflectionPlanGenerator planGenerator = new ReflectionPlanGenerator(recordingConfig,
      catalogService, config, goal, entry, materialization,
      reflectionSettings, materializationStore, getForceFullRefresh(materialization), StrippingFactory.LATEST_STRIP_VERSION, false);
    RelNode normalizedPlan = planGenerator.generateNormalizedPlan();
    RefreshDecision noDefaultReflectionDecision = planGenerator.getRefreshDecision();

    // Default raw reflections can be used to accelerate refresh jobs if the support key for it is enabled AND we are doing a full refresh.
    ...
    if (sqlHandlerConfig.getContext().getOptions().getOption(ReflectionOptions.ACCELERATION_ENABLE_DEFAULT_RAW_REFRESH)
      && (planGenerator.getRefreshDecision().getAccelerationSettings().getMethod().equals(RefreshMethod.FULL))) {
      planGenerator = new ReflectionPlanGenerator(sqlHandlerConfig,
        catalogService, config, goal, entry, materialization,
        reflectionSettings, materializationStore, getForceFullRefresh(materialization), StrippingFactory.LATEST_STRIP_VERSION, false);
    ...
    refreshDecisions[0] = noDefaultReflectionDecision;

    if (noDefaultReflectionDecision.getAccelerationSettings().getMethod() == RefreshMethod.INCREMENTAL &&
    ...
    sqlHandlerConfig.getConverter().getObserver().recordExtraInfo(DECISION_NAME, ABSTRACT_SERIALIZER.serialize(noDefaultReflectionDecision));
    ...
    snapshotDiffContextPointer.value = planGenerator.getSnapshotDiffContext();
