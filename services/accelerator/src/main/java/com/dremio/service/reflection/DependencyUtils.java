...
public class DependencyUtils {
  ...
  private static final Predicate<DependencyEntry> IS_DATASET =
      new Predicate<DependencyEntry>() {
        @Override
        public boolean apply(DependencyEntry entry) {
  ...
  private static final Predicate<DependencyEntry> IS_TABLEFUNCTION =
      new Predicate<DependencyEntry>() {
        @Override
        public boolean apply(DependencyEntry entry) {
  ...
  public static String describeDependencies(
      ReflectionId reflectionId, Iterable<DependencyEntry> dependencyEntries) {
  ...
  static FluentIterable<ReflectionDependency> filterReflectionDependencies(
      Iterable<DependencyEntry> dependencyEntries) {
    return FluentIterable.from(dependencyEntries)
        .filter(IS_REFLECTION)
        .transform(
            new Function<DependencyEntry, ReflectionDependency>() {
              @Override
  ...
  static FluentIterable<DatasetDependency> filterDatasetDependencies(
      Iterable<DependencyEntry> dependencyEntries) {
    return FluentIterable.from(dependencyEntries)
        .filter(IS_DATASET)
        .transform(
            new Function<DependencyEntry, DatasetDependency>() {
              @Override
  ...
  static FluentIterable<TableFunctionDependency> filterTableFunctionDependencies(
      Iterable<DependencyEntry> dependencyEntries) {
    return FluentIterable.from(dependencyEntries)
        .filter(IS_TABLEFUNCTION)
        .transform(
            new Function<DependencyEntry, TableFunctionDependency>() {
              @Override
   ...
   */
  public static ExtractedDependencies extractDependencies(
      final JobInfo jobInfo, final RefreshDecision decision, CatalogService catalogService)
    ...
    if (jobScanPaths != null) {
      for (ScanPath scanPath : jobScanPaths) {
        ...
        DremioTable table =
            catalog.getTable(
                CatalogEntityKey.newBuilder()
                    .keyComponents(scanPath.getPathList())
                    .tableVersionContext(versionContext)
                    .build());
        if (table == null) {
          throw new NamespaceNotFoundException(
              new NamespaceKey(scanPath.getPathList()),
              "Dataset not found in catalog " + scanPath.getVersionContext());
        }
        long snapshotId = 0L;
        ...
        if (scanPath.getPathList().get(0).equals(ACCELERATOR_STORAGEPLUGIN_NAME)) {
          plandDependencies.add(
              DependencyEntry.of(
                  new ReflectionId(
                      scanPath.getPathList().get(TABLE_SCHEMA_PATH_REFLECTION_ID_COMPONENT)),
                  snapshotId));
        } else {
          plandDependencies.add(
              DependencyEntry.of(
                  table.getDatasetConfig().getId().getId(),
                  scanPath.getPathList(),
                  snapshotId,
                  versionContext));
        }
      }
      ...
      for (int i = 0; i < externalQuerySourceInfo.size(); i += 2) {
        plandDependencies.add(
            DependencyEntry.of(
                ...
                externalQuerySourceInfo.get(i),
                externalQuerySourceInfo.get(i + 1)));
      }
    }
