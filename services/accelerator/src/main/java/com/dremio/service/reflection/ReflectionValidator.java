...
public class ReflectionValidator {
  ...
  public ReflectionValidator(final Provider<CatalogService> catalogService, final Provider<OptionManager> optionManager) {
  ...
  public void validate(ReflectionGoal goal, DremioTable table) {
    ...
    validatePartitionFields(goal.getDetails().getPartitionFieldList(), schemaMap, "Partition", true);
  ...
  /**
   * Make sure that the partition transforms used are data type compatible with the columns they are used on
   * @param partitionFieldList partition fields to validate
   * @param table table/view the reflection is based on
   */
  private void validatePartitionTransformsWithSchema(final List<ReflectionPartitionField> partitionFieldList, final DremioTable table) {
    ...
    for(final PartitionTransform transform:transforms){
      if(!transform.isIdentity()){
        ...
        //here we will try to convert a field to batchSchema with a single field and set its Iceberg partition spec
        //this will throw an error if the transform is not supported on the field type
        //all unsupported transforms will fail here. Examples - Month(integer), Year(string), truncate(5, timestamp)
        final BatchSchema batchSchemaForSingleField = BatchSchema.newBuilder().addField(field).build();
        final Schema icebergSchema = schemaConverter.toIcebergSchema(batchSchemaForSingleField);
        /*final PartitionSpec partitionSpec = */
          getIcebergPartitionSpecFromTransforms(batchSchemaForSingleField,ImmutableList.of(transform),icebergSchema);
      }
    }
  }
  ...
  private void validatePartitionFields(List<ReflectionPartitionField> fieldList, Map<String, ViewFieldType> schemaMap, String fieldName, Boolean mustBePrimitive) {
    if (fieldList == null) {
      return;
    }
