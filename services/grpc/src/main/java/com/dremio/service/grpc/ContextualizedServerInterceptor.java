...
public class ContextualizedServerInterceptor implements ServerInterceptor {
  /**
   * Used to copy ExecutorToken so that it can be used for other gRPC calls in the context of the
   * original API.
   */
  public static ContextualizedServerInterceptor buildExecutorTokenCopier() {
    return new ContextualizedServerInterceptor(ImmutableList.of(new ExecutorToken.Transformer()));
  }

  public static ContextualizedServerInterceptor buildBasicContextualizedServerInterceptor() {
    return new ContextualizedServerInterceptor(
        ImmutableList.of(
  ...
  public ContextualizedServerInterceptor(
      ImmutableList<SerializableContextTransformer> transformers) {
  ...
  public <ReqT, RespT> ServerCall.Listener<ReqT> interceptCall(
    ...
    try {
      ...
      return new ForwardingServerCallListener.SimpleForwardingServerCallListener<ReqT>(
          context.call(() -> next.startCall(call, requestHeaders))) {
        @Override
