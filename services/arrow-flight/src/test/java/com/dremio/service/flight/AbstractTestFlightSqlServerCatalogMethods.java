...
public abstract class AbstractTestFlightSqlServerCatalogMethods extends BaseFlightQueryTest {
  ...
  public void testGetCatalogs() throws Exception {
    ...
    try (FlightStream stream =
        flightSqlClient.getStream(flightInfo.getEndpoints().get(0).getTicket(), callOptions)) {
  ...
  public void testGetTablesWithoutFiltering() throws Exception {
    final FlightInfo flightInfo =
        flightSqlClient.getTables(null, null, null, null, false, getCallOptions());
  ...
  public void testGetTablesFilteringByTableAndIncludingSchema() throws Exception {
    final FlightInfo flightInfo =
        flightSqlClient.getTables(null, null, "columns", null, true, getCallOptions());
    try (final FlightStream stream =
        flightSqlClient.getStream(flightInfo.getEndpoints().get(0).getTicket(), getCallOptions())) {
      Assert.assertTrue(stream.next());
      final VectorSchemaRoot root = stream.getRoot();
  ...
  public void testGetTablesFilteringByCatalogPattern() throws Exception {
    final FlightInfo flightInfo =
        flightSqlClient.getTables("DREMIO", null, null, null, false, getCallOptions());
    try (FlightStream stream =
        flightSqlClient.getStream(flightInfo.getEndpoints().get(0).getTicket(), getCallOptions())) {
      Assert.assertTrue(stream.next());
      final VectorSchemaRoot root = stream.getRoot();
  ...
  public void testGetTablesFilteringBySchemaPattern() throws Exception {
    final FlightInfo flightInfo =
        flightSqlClient.getTables(null, "INFORMATION_SCHEMA", null, null, false, getCallOptions());
    try (FlightStream stream =
        flightSqlClient.getStream(flightInfo.getEndpoints().get(0).getTicket(), getCallOptions())) {
      Assert.assertTrue(stream.next());
      final VectorSchemaRoot root = stream.getRoot();
  ...
  public void testGetTablesFilteringByTablePattern() throws Exception {
    final FlightInfo flightInfo =
        flightSqlClient.getTables(null, null, "COLUMNS", null, false, getCallOptions());
    try (FlightStream stream =
        flightSqlClient.getStream(flightInfo.getEndpoints().get(0).getTicket(), getCallOptions())) {
      Assert.assertTrue(stream.next());
      final VectorSchemaRoot root = stream.getRoot();
  ...
  public void testGetTablesFilteringByTableTypePattern() throws Exception {
    final FlightInfo flightInfo =
        flightSqlClient.getTables(
            null, null, null, Collections.singletonList("SYSTEM_TABLE"), false, getCallOptions());
    try (FlightStream stream =
        flightSqlClient.getStream(flightInfo.getEndpoints().get(0).getTicket(), getCallOptions())) {
      Assert.assertTrue(stream.next());
      final VectorSchemaRoot root = stream.getRoot();
  ...
  public void testGetTablesFilteringByMultiTableTypes() throws Exception {
    final FlightInfo flightInfo =
        flightSqlClient.getTables(
            null, null, null, ImmutableList.of("TABLE", "VIEW"), false, getCallOptions());
    try (FlightStream stream =
        flightSqlClient.getStream(flightInfo.getEndpoints().get(0).getTicket(), getCallOptions())) {
      Assert.assertTrue(stream.next());
      final VectorSchemaRoot root = stream.getRoot();
  ...
  public void testGetTablesTypes() throws Exception {
    ...
    try (final FlightStream stream =
      ...
      range.forEach(
          i -> {
            Assert.assertEquals(
                root.getVector(0).getObject(i).toString(), tableTypes.get(i).toString());
          });
    }
  }

  private void testGetSchemas(String catalog, String schemaPattern, boolean expectNonEmptyResult)
      throws Exception {
    final FlightInfo flightInfo =
        flightSqlClient.getSchemas(catalog, schemaPattern, getCallOptions());
    try (final FlightStream stream =
        flightSqlClient.getStream(flightInfo.getEndpoints().get(0).getTicket(), getCallOptions())) {
      Assert.assertTrue(stream.next());
      final VectorSchemaRoot root = stream.getRoot();
      ...
      final Pattern schemaFilterPattern =
          schemaPattern != null
              ? Pattern.compile(RegexpUtil.sqlToRegexLike(schemaPattern))
              : Pattern.compile(".*");
  ...
  private FlightStream getSchemasFlightStream() {
    ...
    return flightSqlClient.getStream(
        flightInfo.getEndpoints().get(0).getTicket(), getCallOptions());
  }
  ...
  private FlightStream getSqlInfoFlightStream() {
    final FlightInfo flightInfo =
        flightSqlClient.getSqlInfo(new FlightSql.SqlInfo[] {}, getCallOptions());
    return flightSqlClient.getStream(
        flightInfo.getEndpoints().get(0).getTicket(), getCallOptions());
  }
