...
public class TestFlightPreparedStatement {
  ...
  private static final Exception UNKNOWN =
      Status.UNKNOWN
  ...
  private static final Exception CANCELLED =
      Status.CANCELLED
  ...
  private static final Schema schema =
      new Schema(Collections.singletonList(Field.nullable("test1", ArrowType.Bool.INSTANCE)));
  private static final PreparedStatementHandle preparedStatementHandle =
      PreparedStatementHandle.newBuilder()
          .setServerInfo(ByteString.copyFrom(new byte[] {1, 2, 3}))
          .build();

  private static final UserProtos.CreatePreparedStatementArrowResp response =
      UserProtos.CreatePreparedStatementArrowResp.newBuilder()
  ...
  private static CancellableUserResponseHandler<UserProtos.CreatePreparedStatementArrowResp>
      mockHandler;
  ...
  public void testGetSchemaCancelled() {
    ...
    final FlightPreparedStatement flightPreparedStatement =
        new FlightPreparedStatement(mockHandler);
  ...
  public void testGetFlightInfoCancelled() {
    ...
    // Act
    assertThatThrownBy(() -> flightPreparedStatement.getFlightInfo(Optional.of(mockLocation)))
        .isInstanceOf(StatusRuntimeException.class)
        .hasCauseInstanceOf(UserException.class)
        .hasMessage("CANCELLED: Client cancelled.");
  }
  ...
  private void testGetFlightInfoSuccessfulBase(Optional<Location> mockLocation) {
    // Arrange
    when(mockHandler.get()).thenReturn(response);
    final FlightPreparedStatement flightPreparedStatement =
        new FlightPreparedStatement(mockHandler);
    ...
    final FlightEndpoint flightEndpoint =
        mockLocation
            .map(location -> new FlightEndpoint(ticket, location))
            .orElseGet(() -> new FlightEndpoint(ticket));

    final FlightInfo expected =
        new FlightInfo(schema, flightDescriptor, ImmutableList.of(flightEndpoint), -1, -1);
  ...
  public void testGetFlightInfoSuccessful() {
    testGetFlightInfoSuccessfulBase(Optional.of(mockLocation));
  }

  @Test
  public void testGetFlightInfoSuccessfulDefaultLocation() {
    testGetFlightInfoSuccessfulBase(Optional.empty());
  }

  private void testGetFlightInfoLegacySuccessfulBase(Optional<Location> mockLocation) {
    // Arrange
    when(mockHandler.get()).thenReturn(response);
    final FlightPreparedStatement flightPreparedStatement =
        new FlightPreparedStatement(mockHandler);
    ...
    final TicketContent.PreparedStatementTicket ticketContent =
        TicketContent.PreparedStatementTicket.newBuilder()
            .setHandle(response.getPreparedStatement().getServerHandle())
            .setQuery(dummyQuery)
            .build();
    final FlightDescriptor flightDescriptor =
        FlightDescriptor.command(dummyQuery.getBytes(StandardCharsets.UTF_8));
    ...
    // Act
    final FlightInfo actual =
        flightPreparedStatement.getFlightInfoLegacy(mockLocation, flightDescriptor);
  ...
  public void testGetFlightInfoLegacySuccessful() {
    testGetFlightInfoLegacySuccessfulBase(Optional.of(mockLocation));
  }

  @Test
  public void testGetFlightInfoLegacySuccessfulDefaultLocation() {
    testGetFlightInfoLegacySuccessfulBase(Optional.empty());
  }
