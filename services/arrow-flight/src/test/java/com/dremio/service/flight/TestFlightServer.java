 ...
 */
public class TestFlightServer extends BaseFlightQueryTest {
  ...
  public void testConnection() throws Exception {
    final FlightClient client = openFlightClient(DUMMY_USER, DUMMY_PASSWORD);
    client.close();
  }

  @Test(expected = FlightRuntimeException.class)
  public void testFlightClientEncryptedServerUnencrypted() throws Exception {
    openEncryptedFlightClient(DUMMY_USER, DUMMY_PASSWORD, null);
  }

  @Test(expected = FlightRuntimeException.class)
  public void testBadCredentials() throws Exception {
    openFlightClient(DUMMY_USER, "bad password");
  }
  ...
  private FlightInfo getFlightInfo(String query) {
    return getFlightClient().getInfo(toFlightDescriptor(query));
  }

  private FlightStream executeQuery(FlightClient client, String query) {
    // Assumption is that we have exactly one endpoint returned.
    return client.getStream(getFlightInfo(query).getEndpoints().get(0).getTicket());
  }

  private FlightStream executeQuery(String query) {
    // Assumption is that we have exactly one endpoint returned.
    return executeQuery(getFlightClient(), query);
  }
