...
/**
 * This test measures to determine that signals from a Flight Client which is not consuming data
 * fast enough will be recognized by the server, causing the server stop adding data to the stream.
 * {@link DelegatingWaitCountingResponseHandlerFactory} creates a {@link
 * DelegatingWaitCountingResponseHandlerFactory.DelegatingWaitCountingResponseHandler} which extends
 * {@link RunQueryResponseHandler} to add instrumentation. The test executes the query with {@link
 * FlightClient#getInfo} and {@link FlightClient#getStream} then waits on a latch. When the server
 * calls {@link UserResponseHandler#sendData}, the overridden method in
 * DelegatingSleepCountingResponseHandler will check if the {@link ServerStreamListener#isReady()}
 * is false (which it won't be initially). After several buffers of data, {@link
 * ServerStreamListener#isReady()} is expected to report as false. The test will then wait, which
 * should cause the server to also wait. The waits are captured by
 * DelegatingSleepCountingResponseHandler. After the wait expires, consumption resumes. Finally the
 * test validates itself by ensuring an appropriate amount of waiting took place, and that the
 * server did see a false value from {@link ServerStreamListener#isReady()}
 */
public class ITBackPressure extends BaseFlightQueryTest {
  ...
  private static final DelegatingWaitCountingResponseHandlerFactory
  ...
  public void ensureWaitUntilProceed() throws Exception {
    ...
    try (FlightStream flightStream =
      ...
      assertTrue(
          "The query above may have two few results so that the client never "
      ...
      assertTrue(
          String.format(
              "Expected a wait of at least %dms but only waited for %d", expectedMS, actualSleepMS),
   ...
   */
  private static final class DelegatingWaitCountingResponseHandlerFactory
    ...
    public UserResponseHandler getHandler(
      ...
      if (!hasOneHandlerBeenCreated) {
        ...
        return new DelegatingWaitCountingResponseHandler(
            runExternalId,
            userSession,
            workerProvider,
            clientListener,
            ...
            waitMSCounter,
            releasesWhenClientNotReadyOrQueryComplete,
            wasClientNotReady);
      } else {
    ...
    private static final class DelegatingWaitCountingResponseHandler
        extends BackpressureHandlingResponseHandler {
      ...
      DelegatingWaitCountingResponseHandler(
          UserBitShared.ExternalId runExternalId,
          UserSession userSession,
          Provider<UserWorker> workerProvider,
          ServerStreamListener clientListener,
