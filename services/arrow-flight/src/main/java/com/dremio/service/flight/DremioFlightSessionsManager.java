...
public interface DremioFlightSessionsManager extends AutoCloseable {
  ...
  /**
   * Resolves an existing UserSession for the given token. Null if the UserSession has expired and
   ...
   */
  UserSessionService.UserSessionData getUserSession(
      String peerIdentity, CallHeaders incomingHeaders);

  /**
   * Creates a UserSession object and store it in the local cache, assuming that the token was
   * already validated.
   *
   * @param peerIdentity identity after authorization
   * @param incomingHeaders The CallHeaders to parse client properties from.
   ...
   *
   * @param updatedSession the updated session data, including the session id, the previous version,
