...
public final class DremioFlightAuthUtils {
   ...
   *     - If the number of UserSession has reached its maximum. - If unable to authenticate against
   *     Dremio with the provided credentials.
   */
  public static String authenticateAndCreateToken(
    ...
    return createToken(tokenManagerProvider, authResult.getUserName());
  }
   ...
   */
  public static AuthResult authenticateCredentials(
      Provider<UserService> userServiceProvider, String username, String password, Logger logger) {
    try {
      return userServiceProvider.get().authenticate(username, password);
    } catch (Exception e) {
      logger.error("Unable to authenticate user {}", username, e);
      final String errorMessage =
          "Unable to authenticate user " + username + ", exception: " + e.getMessage();
   ...
   * @return session token associated with the provided user.
   */
  public static String createUserSessionWithTokenAndProperties(
      Provider<TokenManager> tokenManagerProvider, String username) {
    return createToken(tokenManagerProvider, username);
  }
   ...
   */
  private static String createToken(Provider<TokenManager> tokenManagerProvider, String username) {
    // TODO: DX-25278: Add ClientAddress information while creating a Token in
