...
public abstract class BasicClient<
        T extends EnumLite,
        R extends RemoteConnection,
        HS extends MessageLite,
        HR extends MessageLite>
    extends AbstractClient<T, R, HS> {
  private static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(BasicClient.class);
  ...
  private static final OutboundRpcMessage PING_MESSAGE =
      new OutboundRpcMessage(RpcMode.PING, 0, 0, Acks.OK);

  // The percentage of time that should pass before sending a ping message to ensure server doesn't
  // time us out. For
  // example, if timeout is set to 30 seconds and we set percentage to 0.5, then if no write has
  // happened within 15
  ...
  /** Delegates to {@link RpcBus#send} for this client. */
  public <SEND extends MessageLite, RECEIVE extends MessageLite> RpcFuture<RECEIVE> send(
  ...
  /**
   * Establish a connection to the server running on {@param host} and {@param port}. After
   * successfully connecting to the server, which may include setting up SSL, send the {@param
   * handshakeValue handshake message} to the server. If handshake is successful, invoke {@link
   * RpcConnectionHandler#connectionSucceeded} on the given {@param connectionHandler}. In case of
   * any failure, invoke {@link RpcConnectionHandler#connectionFailed}, with the reason, on the
  ...
  protected void connectAsClient(
      RpcConnectionHandler<R> connectionHandler, HS handshakeValue, String host, int port) {
    ConnectionMultiListener cml =
        new ConnectionMultiListener(connectionHandler, handshakeValue, host, port);
  ...
  /**
   * Holder class for listeners needed to establish a connection to a server, and eventually convey
   * to the requester.
   *
   * <p>A {@link ConnectionEstablishmentListener} is added as listener on the {@link
   * Bootstrap#connect} future. On successfully connecting to the server (i.e. the future is
   * successful), a negotiator is added to the pipeline.
   *
   * <p>1. If SSL is disabled, the negotiator is a {@link HandshakeRequester}. When the channel
   * becomes active, a handshake message is sent to the server. A {@link HandshakeSendListener} is
   * registered to listen to the server's response to that handshake request. The handshake response
   * from the server is decoded by {@link AbstractHandshakeHandler}, and the response is passed
   * along to the registered {@link HandshakeSendListener}. A {@link HandshakeSendListener#success
   * successful} response from the server is validated and the connection is {@link
   * #finalizeConnection finalized}. And finally {@link RpcConnectionHandler#connectionSucceeded} is
   * invoked on {@param connectionHandler}.
   *
   * <p>2. If SSL is enabled, the negotiator is a {@link SslHandler} which is configured based on
   * the {@link RpcConfig#sslConfig} for a client. The {@link SslHandler} is added as the first
   * handler in the pipeline. A listener is registered on the {@link SslHandler#handshakeFuture SSL
   * handshake future}, which adds a {@link HandshakeRequester} on success. And then the logic
   * mentioned in (1) applies.
   *
   * <p>On any connection-related, handshake-related or SSL-related failure, the {@param
  ...
  private class ConnectionMultiListener {
    ...
    private final ConnectionEstablishmentListener establishmentListener =
        new ConnectionEstablishmentListener();
    ...
    ConnectionMultiListener(
        RpcConnectionHandler<R> connectionHandler, HS handshakeValue, String hostName, int port) {
    ...
    private final class ConnectionEstablishmentListener
      ...
      public void operationComplete(ChannelFuture connectionFuture) throws Exception {
        ...
        // We want to wait for at least 120 secs when interrupts occur. Establishing a connection
        // fails/succeeds
        ...
        while (true) {
          try {
            ...
            logger.trace(
                "Connection establishment to '{}' completed with state '{}'",
      ...
      void addNegotiator(ChannelFuture connectionFuture) throws Exception {
        ...
        final SSLEngine clientEngine =
            engineFactory.get().newClientEngine(connectionFuture.channel().alloc(), hostName, port);
        ...
        sslHandler
            ...
            .addListener(
                future -> {
                  logger.debug(
                      "SSL client state '{}' on connection '{}'",
                      future.isSuccess(),
                      connectionFuture.channel());

                  if (future.isSuccess()) {
                    logger.trace(
                        "Adding handshake negotiator on '{}', after SSL succeeded",
                        connectionFuture.channel());
                    addHandshakeRequester(connectionFuture);
                  } else {
                    connectionHandler.connectionFailed(
        ...
        // TODO(DX-12921):
        // sslHandler.setHandshakeTimeoutMillis(sslConfig.getHandshakeTimeoutMillis());
    ...
    void sendHandshake() {
      ...
      // send a handshake on the current thread. This is the only time we will send from within the
      // event thread.
    ...
    /**
     * Sends a handshake request to the server when the channel becomes active, or if it is already
     * active. After sending, the handler removes itself from the pipeline.
     */
    private final class HandshakeRequester extends ChannelInboundHandlerAdapter {
      ...
      public void handlerAdded(ChannelHandlerContext ctx) throws Exception {
        ...
        if (ctx.channel().isActive()) {
          if (logger.isDebugEnabled()) {
            logger.debug(
                "sendHandshakeAndRemoveSelf - handler added: L {} -> R {}",
      ...
      public void channelActive(ChannelHandlerContext ctx) throws Exception {
        if (logger.isDebugEnabled()) {
          logger.debug(
              "sendHandshakeAndRemoveSelf - channel active: L {} -> R {}",
              ctx.channel().localAddress(),
              ctx.channel().remoteAddress());
        }
        sendHandshakeAndRemoveSelf(ctx);
    ...
    /**
     * Listens to handshake response from the server, and conveys the outcome to the {@link
     * #connectionHandler}.
     */
    private final class HandshakeSendListener implements RpcOutcomeListener<HR> {
      ...
      public void failed(RpcException ex) {
        logger.debug(
            "Failure while initiating handshake for connection {}",
      ...
      public void success(HR value, ByteBuf buffer) {
        ...
        try {
          ...
          logger.info(
              "Failure while validating handshake for connection {}",
              getConnectionName(connection),
              ex);
  ...
  /**
   * Handler to invoke the {@link ConnectionMultiListener.HandshakeSendListener handshake listener},
   * but generic enough to support listeners on handshake response.
   */
  private class ClientHandshakeHandler extends AbstractHandshakeHandler<HR> {
    ...
    protected final void consumeHandshake(ChannelHandlerContext ctx, HR msg) throws Exception {
      ...
      final RpcOutcome<HR> response =
          connection.getAndRemoveRpcOutcome(
              handshakeType.getNumber(), coordinationId, responseClass);
      response.set(msg, null);
    }
  }

  /**
   * Handler that watches for situations where we have not written to the socket in a certain
   * timeout. If we exceed this timeout, we send a PING message to the server to convey that we are
   * still alive.
   */
  private class IdlePingHandler extends IdleStateHandler {
    ...
    private final ChannelFutureListener pingFailedListener =
        future -> {
          if (!future.isSuccess()) {
            logger.error(
                "Unable to maintain connection {}. Closing connection.", connection.getName());
            connection.close();
          }
        };
    ...
    protected void channelIdle(ChannelHandlerContext ctx, IdleStateEvent evt) throws Exception {
      if (evt.state() == IdleState.WRITER_IDLE) {
        ctx.writeAndFlush(PING_MESSAGE).addListener(pingFailedListener);
      }
    }
  }
}
