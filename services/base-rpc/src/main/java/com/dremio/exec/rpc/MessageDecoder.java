...
/**
 * Frame decoder that decodes the rpc header of each message as it is available. Expected to only
 * allocate on heap for message only items. Can hit OOM for message + trailing bytes messages but
 * simply responds to sender that message cannot be accepted due to out of memory. Should be
 * resilient to all off-heap OOM situations.
 */
public class MessageDecoder extends ByteToMessageDecoder {
  ...
  public static int decodeLengthFromMessage(ChannelHandlerContext ctx, ByteBuf in)
      throws Exception {
    int length = 0;
    ...
    if (!ctx.channel().isOpen()) {
      if (in.readableBytes() > 0) {
        logger.info(
            "Channel is closed, discarding remaining {} byte(s) in buffer.", in.readableBytes());
      }
    ...
    /**
     * a variable-width message length can be up to five bytes in length. read bytes until we have a
     * length.
     */
  ...
  /**
   * We decode the message in the same context as the length decoding to better manage running out
   * of memory. We decode the rpc header using heap memory (expected to always be available) so that
   * we can propagate a error message with correct coordination id to sender rather than failing the
   * channel.
   *
   * @param ctx The channel context.
   * @param frame The Frame of the message we're processing.
   * @param length The length of the frame.
   * @throws Exception Code should only throw corrupt channel messages, causing the channel to
   *     close.
   */
  private InboundRpcMessage decodeMessage(
      final ChannelHandlerContext ctx, final ByteBuf frame, final int length) throws Exception {
    // now, we know the entire message is in the buffer and the buffer is constrained to this
    // message. Additionally,
    // this process should avoid reading beyond the end of this buffer so we inform the
    // ByteBufInputStream to throw an
      ...
      if (frame.readableBytes() != dBodyLength) {
        throw new CorruptedFrameException(
            String.format(
                ...
                dBodyLength, frame.readableBytes()));
      }

      try {
        dBody = NettyArrowBuf.unwrapBuffer(allocator.buffer(dBodyLength));
        // need to make buffer copy, otherwise netty will try to refill this buffer if we move the
        // readerIndex forward...
    ...
    InboundRpcMessage m =
        new InboundRpcMessage(
            header.getMode(), header.getRpcType(), header.getCoordinationId(), pBody, dBody);
    return m;
  }
  ...
  private void sendOutOfMemory(Throwable e, final ChannelHandlerContext ctx, int coordinationId) {
    ...
    // if we were unable to report back the failure make sure we close the channel otherwise we may
    // cause the sender
  ...
  private void checkTag(ByteBufInputStream is, int expectedTag) throws IOException {
    ...
    if (actualTag != expectedTag) {
      throw new CorruptedFrameException(
          String.format(
              "Expected to read a tag of %d but actually received a value of %d.  Happened after reading %d message.",
              expectedTag, actualTag, messageCounter.get()));
    }
  }
