...
/**
 * Helper class to create {@link SSLConfig} instances for the specified communication path, based on
 * config.
 */
public class SSLConfigurator {
  private static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(SSLConfigurator.class);
   ...
   *
   * <p>{@param hostName} and {@param alternativeHostNames} are required for auto-generating
   * certificates. {@param disablePeerVerification} is ignored if auto-generating certificates,
   * since the peer would have to as well.
   *
  ...
  public Optional<SSLConfig> getSSLConfig(
      boolean disablePeerVerification, String hostName, String... alternativeHostNames)
    ...
    if (autoGenerated) {
      logger.warn(
          "*** Using generated self-signed SSL settings for server ('{}' component) ***\n"
      ...
      final boolean configured =
          configureUsingPreviouslyGeneratedStores(builder, securityFolder, localWritePathString);
      if (!configured) {
        logger.info(
            "No previous keystore detected, creating certificate. This operation might take time...");
        generateCertificatesAndConfigure(
            builder, securityFolder, localWritePathString, hostName, alternativeHostNames);
      }
  ...
  private boolean configureUsingPreviouslyGeneratedStores(
      SSLConfig.Builder builder, SecurityFolder securityFolder, String localWritePathString)
      throws GeneralSecurityException, IOException {
    if (securityFolder.exists(KEY_STORE_FILE)) {
      ...
      Preconditions.checkState(
          Files.exists(trustStorePath), "auto-generated trust store is missing");
  ...
  private void moveTrustStoreIfNecessary(
      SecurityFolder securityFolder, String localWritePathString) {
    ...
    try {
      ...
      final String message =
          String.format(
              "Failed to move trust store from '%s' to '%s'. "
    ...
    try {
      ...
      final String message =
          String.format(
              "Failed to set 644 permissions on trust store at '%s'. Please do so manually.",
              toPath);
  ...
  private void generateCertificatesAndConfigure(
    ...
    trustStore.setEntry(
        "DremioAutoGeneratedCert" + RandomStringUtils.randomNumeric(5),
    ...
    try (OutputStream stream =
        securityFolder.newSecureOutputStream(
            KEY_STORE_FILE, SecurityFolder.OpenOption.CREATE_ONLY)) {
      keyStore.store(stream, UNSECURE_PASSWORD_CHAR_ARRAY);
    }

    final Path trustStorePath = Paths.get(localWritePathString, TRUST_STORE_FILE);
    try (OutputStream stream =
        Files.newOutputStream(trustStorePath, StandardOpenOption.CREATE_NEW)) {
   ...
   *
   * @return a password looked up by {@link CredentialsService} or {@param passwordPattern} itself
   *     if {@param passwordPattern} is not a URI.
   * @throws RuntimeException if {@param passwordPattern} has a valid format but {@link
   *     CredentialsService} cannot resolve the password.
   */
  private String lookupPassword(
      String passwordPattern, Provider<CredentialsService> credentialsServiceProvider) {
    try {
  ...
  private void configureUsingConfFile(SSLConfig.Builder builder) {
    ...
    logger.info(
        "Using configured keystore for '{}' component at '{}'", communicationPath, keyStorePath);
    keyStorePath.ifPresent(builder::setKeyStorePath);
    getStringConfig(DremioConfig.SSL_KEY_STORE_PASSWORD)
        .ifPresent(
            jksPwdUri ->
                builder.setKeyStorePassword(lookupPassword(jksPwdUri, credentialsServiceProvider)));
    getStringConfig(DremioConfig.SSL_KEY_PASSWORD)
        .ifPresent(
            keyPwdUri ->
                builder.setKeyPassword(lookupPassword(keyPwdUri, credentialsServiceProvider)));
    getStringConfig(DremioConfig.SSL_KEY_STORE_TYPE).ifPresent(builder::setKeyStoreType);

    getStringConfig(DremioConfig.SSL_TRUST_STORE_TYPE).ifPresent(builder::setTrustStoreType);
    getStringConfig(DremioConfig.SSL_TRUST_STORE_PATH).ifPresent(builder::setTrustStorePath);
    getStringConfig(DremioConfig.SSL_TRUST_STORE_PASSWORD)
        .ifPresent(
            tsPwdUri ->
                builder.setTrustStorePassword(
                    lookupPassword(tsPwdUri, credentialsServiceProvider)));
  }

  /**
   * Loads and returns the trust store as defined in the configuration. If not explicitly configured
   * or auto-generated, this returns {@code null}.
   *
   * @return trust store
   */
  public Optional<KeyStore> getTrustStore() throws GeneralSecurityException, IOException {
    ...
    if (configuredPath.isPresent()) {
      ...
      final Path path =
          Paths.get(config.getString(DremioConfig.LOCAL_WRITE_PATH_STRING), TRUST_STORE_FILE);
