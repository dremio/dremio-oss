...
public final class SqlQueryTokenizer {
  ...
  /**
   * Need to see if the bel character is attached to the previous token. If it is, then we make cursor to be part
   * of the previous identifier.
   */
  private static void parseCursor(SimpleCharStream stream, List<DremioToken> tokens, Token currentToken) {
  ...
  /**
   * Input is ALWAYS at the Double quote character.
   * Resulting stream will either be after EOF or right after the closing double quote.
   * If cursor is in the middle of the identifier, then
   * - The processing stops at the cursor and what came before is counted as identifier AND
   * - Anything that comes after the cursor BUT before the closing quote is discarded.
   * NOTE: We might want to do some postfix matching in the future on the rest of the identifier, but might be too much.
   */
  ...
  /**
   * Input is ALWAYS at the Single quote character.
   * Output will always contain BOTH beginning and end single quotes in the resulting image even if the input didn't.
   * NOTE: This is a simple implementation which greedily takes all input until the end of the input if quote is unclosed.
   * This is fine for a single quote, since we are guaranteed to never have cursor inside a single quote.
   * This is because AutocompleteEngine will not try to autocomplete these cases.
   * NOTE: In reality, parser will get here only if it finds unmatched quote,
   * so very likely we will always go until the end of the stream. It also likely means that the user input is incorrect.
   */
