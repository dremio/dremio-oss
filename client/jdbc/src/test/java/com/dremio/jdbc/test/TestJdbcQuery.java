...
public class TestJdbcQuery extends JdbcTestQueryBase {
  @Rule public final TestRule timeoutRule = TestTools.getTimeoutRule(70, TimeUnit.SECONDS);
  ...
  public void testFix3635() throws Exception {
    // When we run a CTAS query, executeQuery() should return after the table has been flushed to
    // disk even though we
    // didn't yet receive a terminal message. To test this, we run CTAS then immediately run a query
    // on the newly
    // created table.
  ...
  public void testCast() throws Exception {
    testQuery(
        String.format(
            "select R_REGIONKEY, cast(R_NAME as varchar(15)) as region, cast(R_COMMENT as varchar(255)) as comment from dfs.\"%s/region.parquet\"",
            SAMPLE_DATA_PATH));
  }
  ...
  public void testDateLiteral() throws Exception {
    ...
    JdbcAssert.withNoDefaultSchema(getJDBCURL())
        ...
        .returns(
            "EXPR$0=2009-07-06; EXPR$1=2017-12-16; EXPR$2=2000-01-21; EXPR$3=1999-12-25; EXPR$4=2012-12-23");
  }
  ...
  public void testDateWithTimezoneChange() throws Exception {
    ...
    try {
      ...
      // The underlying Connection is cached unless we reset, which is needed to pick up the new
      // timezone.
      reset();
  ...
  public void testTimeLiteral() throws Exception {
    ...
    JdbcAssert.withNoDefaultSchema(getJDBCURL())
        ...
        .returns(
            "EXPR$0=09:15:00; EXPR$1=16:20:00; EXPR$2=16:30:59;"
  ...
  public void testLogicalExplain() throws Exception {
    testQuery(
        String.format(
            "EXPLAIN PLAN WITHOUT IMPLEMENTATION FOR select * from dfs_test.\"%s/region.parquet\"",
            SAMPLE_DATA_PATH));
  }
  ...
  public void testPhysicalExplain() throws Exception {
    testQuery(
        String.format(
            "EXPLAIN PLAN FOR select * from dfs_test.\"%s/region.parquet\"", SAMPLE_DATA_PATH));
  }
  ...
  public void checkUnknownColumn() throws Exception {
    testQuery(
        String.format(
            "SELECT unknownColumn FROM dfs_test.\"%s/region.parquet\"", SAMPLE_DATA_PATH));
  }
  ...
  public void testCaseWithNoElse() throws Exception {
    JdbcAssert.withNoDefaultSchema(getJDBCURL())
        ...
        .returns("employee_id=99; EXPR$1=Elizabeth\n" + "employee_id=100; EXPR$1=null\n");
  }
    ...
    JdbcAssert.withNoDefaultSchema(getJDBCURL())
        .sql(
            "SELECT \"left\"('abcdef', 2) as LEFT_STR, \"right\"('abcdef', 2) as RIGHT_STR, \"replace\"('abcdef', 'ab', 'zz') as REPLACE_STR "
                + "from cp.\"employee.json\" limit 1")
        .returns("LEFT_STR=ab; " + "RIGHT_STR=ef; " + "REPLACE_STR=zzcdef\n");
  }
