...
public class JdbcDataTest extends JdbcWithServerTestBase {
   ...
   *
   * <p>The forwarding method ensures that the IDE calls this method with the right classpath.
   */
  ...
  public void testPrepare() throws Exception {
    JdbcAssert.withModel(sabotNode.getJDBCConnectionString(), MODEL, "DONUTS")
        .withConnection(
  ...
  public void testSelectEmployees() throws Exception {
    JdbcAssert.withModel(sabotNode.getJDBCConnectionString(), MODEL, "HR")
  ...
  /**
   * Query with subquery, filter, and projection of one real and one nonexistent field from a map
   * field.
   */
  @Test
  ...
  private static <T extends LogicalOperator> Iterable<T> findOperator(
      LogicalPlan plan, final Class<T> operatorClazz) {
    return (Iterable<T>)
        Iterables.filter(
            plan.getSortedOperators(),
  ...
  private static <T extends LogicalOperator> T findOnlyOperator(
      LogicalPlan plan, final Class<T> operatorClazz) {
  ...
  //    JdbcAssert.withModel(sabotNode.getJDBCConnectionString(), MODEL, "HR").sql("select distinct
  // deptId from emp")
  ...
  //    JdbcAssert.withModel(sabotNode.getJDBCConnectionString(), MODEL, "HR").sql("select
  // count(deptId) as cd, count(*) as c from emp").returns("CD=5; C=6\n")
  //        .planContains(CollapsingAggregate.class);
  //  }
  //
  //  @Test
  ...
  //    JdbcAssert.withModel(sabotNode.getJDBCConnectionString(), MODEL, "HR").sql("select
  // count(distinct deptId) as c from emp").returns("C=3\n")
  //        .planContains(CollapsingAggregate.class);
  //  }
  //
  //  @Test
  ...
  //    JdbcAssert.withModel(sabotNode.getJDBCConnectionString(), MODEL, "HR").sql("select
  // count(distinct deptId) as c from emp group by ()").returns("C=3\n")
  //        .planContains(CollapsingAggregate.class);
  //  }
  //
  //  @Test
  ...
  //    JdbcAssert.withModel(MODEL, "HR").sql("select deptId, count(*) as c from emp group by
  // deptId")
  //        .returnsUnordered("DEPTID=31; C=1", "DEPTID=33; C=2", "DEPTID=34; C=2", "DEPTID=null;
  // C=1")
  //        .planContains(CollapsingAggregate.class); // make sure using Dremio
  //  }
  ...
  /**
   * Right join is tricky because Dremio's "join" operator only supports "left", so we have to flip
   * inputs.
   */
  @Test
  @Ignore
  ...
  /**
   * Join on subquery; also tests that if a field of the same name exists in both inputs, both
   * fields make it through the join.
   */
  @Test
  ...
  public void testFoodMart() throws Exception {
    JdbcAssert.withModel(sabotNode.getJDBCConnectionString(), MODEL, "FOODMART")
