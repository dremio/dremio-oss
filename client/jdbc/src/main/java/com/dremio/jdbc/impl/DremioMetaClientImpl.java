...
public class DremioMetaClientImpl implements DremioMeta {
  private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(DremioMetaClientImpl.class);
  ...
  private MetaResultSet s(String s) throws SQLException {
    try {
      ...
      return MetaResultSet.create(connection.id, statement.getId(), true,
          DremioMetaImpl.newSignature(s), null);
    } catch (Exception e) {
  ...
  public MetaResultSet getSchemas(String catalog, Pat schemaPattern) throws SQLException {
    ...
    if (schemaPattern.s != null) {
      sb.append(" AND SCHEMA_NAME like '" + DremioStringUtils.escapeSql(schemaPattern.s) + "' escape '" + searchStringEscape + "'");
    }
  ...
  public MetaResultSet getTables(String catalog, Pat schemaPattern, Pat tableNamePattern, List<String> typeList)
    ...
    sb.append("select "
        ...
        + "'' as REF_GENERATION "
        + "FROM INFORMATION_SCHEMA." + identifierQuote + "TABLES" + identifierQuote +" WHERE 1=1 ");
    ...
    if (schemaPattern.s != null) {
      sb.append(" AND TABLE_SCHEMA like '" + DremioStringUtils.escapeSql(schemaPattern.s) + "' escape '" + searchStringEscape + "'");
    }

    if (tableNamePattern.s != null) {
      sb.append(" AND TABLE_NAME like '" + DremioStringUtils.escapeSql(tableNamePattern.s) + "' escape '" + searchStringEscape + "'");
    }
    ...
    if (typeList != null && typeList.size() > 0) {
      ...
      for (int t = 0; t < typeList.size(); t++) {
        ...
        sb.append(" TABLE_TYPE LIKE '" + DremioStringUtils.escapeSql(typeList.get(t)) + "' escape '" + searchStringEscape + "' ");
      }
      sb.append(")");
    }
  ...
  public MetaResultSet getColumns(String catalog, Pat schemaPattern, Pat tableNamePattern, Pat columnNamePattern)
      ...
      if (schemaPattern.s != null) {
        sb.append("\n  AND TABLE_SCHEMA like '" + DremioStringUtils.escapeSql(schemaPattern.s) + "' escape '" + searchStringEscape + "'");
      }
      if (tableNamePattern.s != null) {
        sb.append("\n  AND TABLE_NAME like '" + DremioStringUtils.escapeSql(tableNamePattern.s) + "' escape '" + searchStringEscape + "'");
      }
      if (columnNamePattern.s != null) {
        sb.append("\n  AND COLUMN_NAME like '" + DremioStringUtils.escapeSql(columnNamePattern.s) + "' escape '" + searchStringEscape + "'");
      }
