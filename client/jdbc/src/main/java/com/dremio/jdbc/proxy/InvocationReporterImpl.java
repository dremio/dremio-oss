 ...
 *
 * <p>Currently, just writes to System.err.
 */
class InvocationReporterImpl implements InvocationReporter {
  ...
  static {
    ...
    set.add(java.sql.Connection.class.getPackage());
    set.add(javax.sql.PooledConnection.class.getPackage());
    set.add(javax.sql.rowset.BaseRowSet.class.getPackage());
    set.add(javax.sql.rowset.serial.SerialJavaObject.class.getPackage());
    set.add(javax.sql.rowset.spi.SyncFactory.class.getPackage());
    for (Package p : set) {
  ...
  static {
    ...
    set.add(java.util.Collection.class.getPackage());
    set.add(java.lang.Class.class.getPackage());
  ...
  void reportAbbreviatedPackages() {
    ...
    setupMessage(
        "Abbreviating (unique) class names in packages " + StringUtils.join(names, ", ") + ".");
  }
   ...
   *
   * <p>Is it intended that all tracing output goes through this method.
   */
  ...
  /**
   * For warnings such as warning about encountering a type that for which rendering isn't known to
   * show values well.
   */
  ...
  /**
   * Renders a type name. Uses simple names for common types (JDBC interfaces and {code
   * java.lang.*}).
   */
  ...
  private String formatDriverPropertyInfo(final DriverPropertyInfo info) {
    return "[ "
        + "name = "
        + formatValue(info.name)
        + ", value = "
        + formatValue(info.value)
        + ", required = "
        + info.required
        + ", choices = "
        + formatValue(info.choices)
        + ", description = "
  ...
  private String formatValue(final Object value) {
    ...
    if (null == value) {
      ...
      // seen so far.
      rawActualType == java.util.Properties.class || rawActualType.isEnum()) {
        result = value.toString();
      } else if (
      ...
      rawActualType.getName().equals("org.apache.hadoop.io.Text")
          ...
          || rawActualType == com.dremio.exec.vector.accessor.sql.TimePrintMillis.class) {
        ...
        // (No handled yet: byte[].)
        printWarningLine(
            "Unnoted type encountered in formatting (value might"
                + " not render well): "
                + rawActualType
                + ".");
        result = value.toString();
   ...
   *
   * @param declaredType the corresponding declared method parameter or return type
  ...
  /** Renders given exception. Includes test of chained exceptions. */
  ...
  /**
   * Renders corresponding given sequence of declared types and given sequence of values from method
   * call.
   */
  ...
  /**
   * Renders the call part for a method call, method return, or exception-thrown event.
   *
   * @param target the target (receiver) of the method call
   * @param targetType the interface containing called method
   * @param method the called method
   * @param argValues the argument values (represented as for {@link Method#invoke})
   */
  private String formatCallPart(
      final Object target,
  ...
  public void methodCalled(
      final Object target, final Class<?> targetType, final Method method, final Object[] args) {
  ...
  public void methodReturned(
    ...
    if (void.class == method.getReturnType()) {
      assert null == result
          : "unexpected non-null result value "
              + result
              + " for method returning "
              + method.getReturnType();
      printTraceLine(callPart + ", RESULT: (none--void) ");
    } else {
      printTraceLine(
          callPart + ", RESULT: " + formatReturnTypeAndValue(method.getReturnType(), result));
    }
  }

  @Override
  public void methodThrew(
      final Object target,
      final Class<?> targetType,
      final Method method,
      final Object[] args,
