...
/**
 * Proxy driver for tracing calls to a JDBC driver. Reports calls and parameter values to, and
 * return values and exceptions from, methods defined by JDBC interfaces.
 *
 * <p><strong>Invocation:</strong>
 *
 * <p>To set up a tracing version of a JDBC connection:
 *
 * <ul>
 *   <li>Construct the proxying URL corresponding to the original URL.
 *       <p>The proxying URL corresponding to an original URL is "<code>
 *       jdbc:proxy:<i>original.Driver</i>:<i>original_URL</i></code>", where:
 *       <ul>
 *         <li><code><i>original.Driver</i></code> is the fully qualified name of the driver class
 *             to proxy and trace (used to load the class to get it registered with JDBC's {@link
 *             DriverManager}, when or in case it's not already loaded) and can be blank if the
 *             driver class will already be loaded, and
 *         <li><code><i>original_URL</i></code> is the original URL for the JDBC data source to now
 *             be traced.
 *       </ul>
 *       <p>For example, for an original URL of "{@code jdbc:dremio:zk=localhost:2181}", the tracing
 *       URL is "{@code jdbc:proxy:com.dremio.jdbc.Driver:jdbc:dremio:zk=localhost:2181}".
 *   <li>In the JDBC-client code or tool, replace the occurrence of the original URL with the
 *       corresponding proxying URL.
 *   <li>Make sure that class {@link TracingProxyDriver} will be loaded (e.g., configure the client
 *       to use it as the driver class).
 * </ul>
 *
 * <p><strong>Output:</strong>
 *
 * <p>Currently, the tracing output lines are simply written to {@link System#out} ({@code stdout}
 * or "standard output").
 */
public class TracingProxyDriver implements java.sql.Driver {
  ...
  /** Most recent (and usually only) proxyDriver created by this (usually singleton) instance. */
    ...
    UrlHandler(ProxiesManager proxiesManager, String url) throws ProxySetupSQLException {
      ...
      if (-1 == classEndColonPos) {
        throw new ProxySetupSQLException(
            "Connection URL syntax error: no third colon in proxy URL \""
                + url
                + "\"; ("
                + SYNTAX_TEXT
                + ")");
      ...
      try {
        reporter.setupMessage(
            "Proxy calling DriverManager.getDriver(...) for"
                + " proxied URL \""
                + proxiedURL
                + "\".");
      ...
      proxyDriver =
          proxiesManager.getProxyInstanceForOriginal(proxiedDriverForProxiedUrl, Driver.class);
    }
  ...
  private void setProxyDriver(final Driver newProxyDriver, final Driver proxiedDriver) {
  ...
  public boolean acceptsURL(String url) throws SQLException {
    reporter.setupMessage(
        "Proxy's acceptsURL(...) called with " + (null == url ? "null" : "\"" + url + "\"."));
  ...
  public Connection connect(String url, Properties info) throws ProxySetupSQLException {
    ...
    reporter.setupMessage(
        "Proxy's connect(...) called with URL " + (null == url ? "null" : "\"" + url + "\"") + ".");
