...
class DremioMetaServerImpl implements DremioMeta {
    ...
    MetaResultSet getMeta(RpcFuture<Response> future) throws SQLException {
      ...
      try {
        List<Object> rows =
            Lists.transform(
                getResult(response),
                new Function<ResponseValue, Object>() {
                  @Override
        ...
        Meta.Signature signature =
            Meta.Signature.create(
                ...
                CursorFactory.record(
                    ...
                    fieldMetaData.columns.stream()
                        .map(column -> column.columnName)
                        .collect(Collectors.toList())),
                StatementType.SELECT);

        AvaticaStatement statement = connection.createStatement();
        return MetaResultSet.create(connection.id, statement.getId(), true, signature, frame);
  ...
  public MetaResultSet getCatalogs() throws SQLException {
    return new MetadataAdapter<MetaImpl.MetaCatalog, GetCatalogsResp, CatalogMetadata>(
  ...
  public MetaResultSet getSchemas(String catalog, Pat schemaPattern) throws SQLException {
    ...
    return new MetadataAdapter<MetaImpl.MetaSchema, GetSchemasResp, SchemaMetadata>(
  ...
  public MetaResultSet getTables(
    ...
    return new MetadataAdapter<MetaImpl.MetaTable, GetTablesResp, TableMetadata>(MetaTable.class) {
      ...
      protected MetaImpl.MetaTable adapt(TableMetadata protoValue) {
        return new MetaImpl.MetaTable(
            protoValue.getCatalogName(),
            protoValue.getSchemaName(),
            protoValue.getTableName(),
            protoValue.getType());
      }
      ;
    }.getMeta(
        connection
            .getClient()
            .getTables(catalogNameFilter, schemaNameFilter, tableNameFilter, typeList));
  }
  ...
  public MetaResultSet getColumns(
    ...
    return new MetadataAdapter<MetaColumn, GetColumnsResp, ColumnMetadata>(MetaColumn.class) {
      ...
      protected MetaColumn adapt(ColumnMetadata value) {
        ...
        connection
            .getClient()
            .getColumns(catalogNameFilter, schemaNameFilter, tableNameFilter, columnNameFilter));
  }
}
