...
public class TestBackwardsCompatibilityHandler extends DremioTest {
  ...
  public void before() {
    this.allocator =
        allocatorRule.newAllocator("test-backwards-compatibility-handler", 0, Long.MAX_VALUE);
  }
  ...
  private SerializedField.Builder[] fields(int... size) {
    ...
    for (int i = 0; i < size.length; i++) {
      result[i] = SerializedField.newBuilder().setBufferLength(size[i]).setValueCount(1);
    }
    return result;
  }
  ...
  public void testConvertBitsToBytes() {
    try (BitVector bits = new BitVector("$bits$", allocator);
      ...
      ArrowBuf newBuf =
          convertBitsToBytes(allocator, fieldBuilder, NettyArrowBuf.unwrapBuffer(oldBuf))
              .arrowBuf();
  ...
  public void testPatchDecimal() {
    ...
    ByteBuf newBuffer =
        patchDecimal(
            allocator,
            NettyArrowBuf.unwrapBuffer(decimalVector.getDataBuffer()),
            decimalField,
            childDecimalField);

    int startIndex = 0;
    BigDecimal bd =
        DecimalHelper.getBigDecimalFromSparse(
            newBuffer,
            startIndex,
            DrillBackwardsCompatibilityHandler.NUMBER_DECIMAL_DIGITS,
  ...
  public void testPadValues() {
    ...
    try (UInt1Vector bytes = new UInt1Vector("$bits$", allocator); ) {
      ...
      ArrowBuf newBuf =
          padValues(
                  allocator,
                  fieldBuilder,
                  NettyArrowBuf.unwrapBuffer(oldBuf),
