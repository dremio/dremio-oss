...
class BackwardsCompatibilityDecoder extends MessageToMessageDecoder<InboundRpcMessage> {
  ...
  BackwardsCompatibilityDecoder(
      final BufferAllocator allocator, BaseBackwardsCompatibilityHandler handler) {
  ...
  @Override
  protected void decode(ChannelHandlerContext ctx, InboundRpcMessage msg, List<Object> out)
      throws Exception {
    if (msg.mode == RpcMode.REQUEST && msg.rpcType == RpcType.QUERY_DATA.getNumber()) {
      ...
      out.add(
          new InboundRpcMessage(
              ...
              msg.rpcType,
              msg.coordinationId,
              newBatch.getHeader().toByteArray(),
              newBuffer));
    } else {
      out.add(msg);
    }
