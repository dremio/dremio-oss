...
public class UserClient
    extends BasicClientWithConnection<RpcType, UserToBitHandshake, BitToUserHandshake> {
  private static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(UserClient.class);
  ...
  private static Optional<SSLEngineFactory> newSSLEngineFactory(Optional<SSLConfig> sslConfig)
      throws RpcException {
    try {
  ...
  public void submitQuery(UserResultsListener resultsListener, RunQuery query) {
    send(
        queryResultHandler.getWrappedListener(resultsListener),
        RpcType.RUN_QUERY,
        query,
        QueryId.class);
  }
  ...
  public void connect(
      RpcConnectionHandler<ServerConnection> handler,
      NodeEndpoint endpoint,
      UserProperties props,
      UserBitShared.UserCredentials credentials) {
    UserToBitHandshake.Builder hsBuilder =
    ...
    this.connectAsClient(
        queryResultHandler.getWrappedConnectionHandler(handler),
        hsBuilder.build(),
        endpoint.getAddress(),
  ...
  @Override
  protected Response handleReponse(
  ...
  protected void validateHandshake(BitToUserHandshake inbound) throws RpcException {
    ...
    if (inbound.getStatus() != HandshakeStatus.SUCCESS) {
      final String errMsg =
          String.format(
              "Status: %s, Error Id: %s, Error message: %s",
    ...
    RecordBatchFormat recordBatchFormat =
        inbound.hasRecordBatchFormat()
            ? inbound.getRecordBatchFormat()
      ...
      case DREMIO_0_9:
        {
          ...
          final BufferAllocator bcAllocator =
              connection.getAllocator().newChildAllocator("dremio09-backward", 0, Long.MAX_VALUE);
          ...
          connection
              ...
              .addAfter(
                  ...
                  new BackwardsCompatibilityDecoder(
                      bcAllocator, new Dremio09BackwardCompatibilityHandler(bcAllocator)));
        }
        break;
  ...
  protected void finalizeConnection(
      BitToUserHandshake handshake, BasicClientWithConnection.ServerConnection connection) {}
   ...
   */
  public RpcFuture<QueryPlanFragments> planQuery(GetQueryPlanFragments req) {
