...
public class UserRpcConfig {
  ...
  public static RpcConfig getMapping(
      SabotConfig config, Executor executor, Optional<SSLConfig> sslConfig) {
    return RpcConfig.newBuilder()
        .name("USER")
        .timeout(config.getInt(USER_RPC_TIMEOUT))
        .executor(executor)
        .sslConfig(sslConfig)
        .add(
            RpcType.HANDSHAKE,
            UserToBitHandshake.class,
            RpcType.HANDSHAKE,
            BitToUserHandshake.class) // user to bit
        ...
        .add(
            ...
            GetQueryPlanFragments.class,
            RpcType.QUERY_PLAN_FRAGMENTS,
            QueryPlanFragments.class) // user to bit
        .add(
            RpcType.GET_CATALOGS,
            GetCatalogsReq.class,
            RpcType.CATALOGS,
            GetCatalogsResp.class) // user to bit
        .add(
            RpcType.GET_SCHEMAS,
            GetSchemasReq.class,
            RpcType.SCHEMAS,
            GetSchemasResp.class) // user to bit
        .add(
            RpcType.GET_TABLES,
            GetTablesReq.class,
            RpcType.TABLES,
            GetTablesResp.class) // user to bit
        .add(
            RpcType.GET_COLUMNS,
            GetColumnsReq.class,
            RpcType.COLUMNS,
            GetColumnsResp.class) // user to bit
        .add(
            RpcType.CREATE_PREPARED_STATEMENT,
            CreatePreparedStatementReq.class,
            RpcType.PREPARED_STATEMENT,
            CreatePreparedStatementResp.class) // user to bit
        .add(
            RpcType.CREATE_PREPARED_STATEMENT_ARROW,
            CreatePreparedStatementArrowReq.class,
            RpcType.PREPARED_STATEMENT_ARROW,
            CreatePreparedStatementArrowResp.class) // user to bit
        .add(
            RpcType.GET_SERVER_META,
            GetServerMetaReq.class,
            RpcType.SERVER_META,
            GetServerMetaResp.class) // user to bit
        .build();
  }
  ...
  public static final Set<RpcType> SUPPORTED_SERVER_METHODS =
      Sets.immutableEnumSet(
          ImmutableSet.<RpcType>builder()
              .add(
                  RpcType.RUN_QUERY,
                  RpcType.CANCEL_QUERY,
                  RpcType.GET_QUERY_PLAN_FRAGMENTS,
                  RpcType.RESUME_PAUSED_QUERY,
                  RpcType.GET_CATALOGS,
                  RpcType.GET_SCHEMAS,
                  RpcType.GET_TABLES,
                  RpcType.GET_COLUMNS,
