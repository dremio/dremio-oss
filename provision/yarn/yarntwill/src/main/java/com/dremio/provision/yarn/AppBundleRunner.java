 ...
 *
 * <p>1. Loads the bundled jar and its dependencies into a class loader
 *
 * <p>2. Instantiates an instance of the class {#mainClassName} and calls main({#args}) on it.
 */
public class AppBundleRunner implements AutoCloseable {
  ...
  public AppBundleRunner(File jarFile, Arguments arguments) {
    ...
    checkArgument(
        arguments.getMainClassName() != null,
        "Main class name cannot be null",
  ...
  public URLClassLoader load() throws IOException, ReflectiveOperationException {
    ...
    checkArgument(
        Files.exists(manifestPath) && Files.isReadable(manifestPath),
        "Jar file %s must contain a valid manifest file",
        jarFile.getAbsolutePath());
    ...
    final List<Path> nativeLibraryPaths =
        Arrays.stream(nativeLibraryPath.split(" "))
            .map(
                s -> {
                  final URI uri = URI.create(s);
    ...
    final ClassLoader parentClassLoader =
        ClassLoader.getSystemClassLoader() != null
            ...
            : null;
    bundleJarClassLoader =
        new BundledDaemonClassLoader(classPathUrlArray, parentClassLoader, nativeLibraryPaths);
    ...
    checkArgument(
        ...
        "{} does not implement `java.lang.Runnable` interface");
    Constructor<? extends Runnable> constructor =
        cls.asSubclass(Runnable.class).getConstructor(String[].class);
  ...
  public void run() throws Exception {
    ...
    try {
      ...
      logger.error(
          "Error while trying to run {} within {}", mainClassName, jarFile.getAbsolutePath(), t);
      throw t;
    }
  }
