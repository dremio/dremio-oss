...
public class TestAppBundleGenerator {
  ...
  public void testSkipJDKClassLoaders() {
    ...
    // This test is only meaningful if parent classloader exists (most likely the extension
    // classloader
    ...
    assertThat(AppBundleGenerator.toPathStream(classLoader))
        .noneMatch(
            path -> {
              try {
  ...
  public void testToPathStreamFromClassPath() throws IOException {
    ...
    try (Stream<Path> stream =
        AppBundleGenerator.toPathStream(
            Arrays.asList(
  ...
  public void testJarBundle() throws IOException {
    ...
    Files.write(
        outsideFolder.resolve("linktofoo.jar"), Arrays.asList("some random content"), UTF_8);
    ...
    Files.write(
        mainFolder.resolve("plugins/connectors/test-pf4j.jar"),
        Arrays.asList("fake pf4j stuff"),
        UTF_8);
    ...
    try (URLClassLoader classLoader =
        new URLClassLoader(
            ...
            null)) {
      AppBundleGenerator generator =
          new AppBundleGenerator(
              classLoader,
    ...
    try (JarFile jarFile = new JarFile(jarPath.toFile())) {
      ...
      assertThat(mf.getMainAttributes().get(Attributes.Name.CLASS_PATH))
          .isEqualTo(
              Stream.of("prefix.jar", "foo.jar", "bar.jar", "dir", "suffix.jar")
      ...
      assertThat(
          ...
          .isEqualTo(
              "dremio.app".concat(mainFolder.resolve("plugins").toAbsolutePath().toString()));
