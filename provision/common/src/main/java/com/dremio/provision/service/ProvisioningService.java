...
/**
 * Interface for provisioning services implementations We will start with YARN, but could/will
 * extend to Mesos, AWS, Kubernetes APIs here should be async. and return data that is available at
 * the time of API call subsequent calls to the "getter" APIs will return more info as available
 */
public interface ProvisioningService extends Service {
   ...
   * @throws ProvisioningHandlingException
   */
  Iterable<ClusterEnriched> getClusterInfoByTypeByState(ClusterType type, ClusterState state)
      throws ProvisioningHandlingException;
   ...
   * @return A future that notifies when the cluster is up or throws ActionDisallowed,
   *     NoClusterException as appropriate.
   */
  ...
  /**
   * Stop the collection of clusterIds if possible. No exceptions are thrown if the stop is
   * disallowed.
   */
