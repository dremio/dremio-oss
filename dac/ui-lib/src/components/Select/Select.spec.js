...
describe('Radio', () => {
  ...
  it('has the required components with props', () => {
    ...
    expect(wrapper.find(MuiSelect).props().IconComponent).toEqual(ExpandMoreIcon);
    expect(wrapper.find(MuiSelect).props().value).toEqual(defaultProps.value);
    expect(wrapper.find(MenuItem).at(0).text()).toEqual(defaultProps.options[0].label);
    expect(wrapper.find(MenuItem).at(0).props().value).toEqual(defaultProps.options[0].value);
  });

  it('adds the classes passed as props to respective elements', () => {
    expect(wrapper.find('div.selectRoot').props().className).toEqual(expect.stringContaining('sample-root-class'));
