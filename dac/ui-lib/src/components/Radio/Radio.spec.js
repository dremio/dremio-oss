...
describe('Radio', () => {
  ...
  it('has the required components with props', () => {
    ...
    expect(wrapper.find(FormControlLabel).length).toBe(defaultProps.options.length);
    expect(wrapper.find(FormControlLabel).at(0).props()).toEqual(expect.objectContaining({
  ...
  it('adds the classes passed as props to respective elements', () => {
    expect(wrapper.find('div.radio-root').props().className).toEqual(expect.stringContaining('sample-root-class'));
    expect(wrapper.find(Label).props().className).toEqual(expect.stringContaining('sample-label-class'));
    expect(wrapper.find(FormControl).props().classes.root).toEqual(expect.stringContaining('sample-options-container-class'));
  });

  it('aligns according to prop', () => {
    expect(wrapper.find(RadioGroup).props().row).toBe(false);
    const rowAlignWrapper = getShallowWrapper({ ...defaultProps, align: 'row' });
