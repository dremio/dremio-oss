...
/**
 * Upgrade task that deletes all system tables.<br>
 * max_version should be updated to the next release version whenever we make a change to an existing system table schema.
 */
public class DeleteSysTablesMetadata extends UpgradeTask {

  public DeleteSysTablesMetadata() {
    super("Deleting System table metadata", VERSION_150, VERSION_300, NORMAL_ORDER + 14);
  }
  ...
  public void upgrade(UpgradeContext context) {
    ...
    try {
      ...
      namespaceService.deleteDataset(key, dataset.getVersion());
