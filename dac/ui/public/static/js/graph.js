...
$(window).on("load", function () {
  globalconfig.majorcolorscale = d3.scale
    .category20()
    .domain([
      0,
      d3.max(globalconfig.fragmentProfileSize, accessor("majorFragmentId")),
    ]);
  ...
  function extractminortimes(profile) {
    ...
    var timetable = $.map(t2, function (record, index) {
      var newrec = {
        name: record.ma.majorFragmentId + "-" + record.mi.minorFragmentId,
  ...
  function parseplan(planstring) {
    ...
    //
    // CHANGED to ignore non-matching lines
    //
    var ps = planstring
      .trim()
      .split("\n")
      .reduce(function (lines, line) {
        var matched = /^([0-9-]+)( *)([a-zA-Z]*)/.exec(line);
        if (matched) {
          return lines.concat([matched.slice(1)]);
        }
        return lines;
      }, []);
    // end change
    ...
    for (var i = 1; i < ps.length; i++) {
      var top = st.pop();
      while (top[1].length >= ps[i][1].length) top = st.pop();

      g.addEdge(null, ps[i][0], top[0]);

      if (ps[i][1].length != top[1].length) st.push(top);
      if (ps[i][1].length >= top[1].length) st.push(ps[i]);
    }
    return g;
  }
  ...
  function buildplangraph(svg, planstring) {
    ...
    svg
      .attr("width", result.graph().width + 2 * padding)
  ...
  function buildtimingchart(svgdest, timetable) {
    ...
    chartprops.h =
      timetable.length * (chartprops.bheight + chartprops.bpad * 2);
    ...
    chartprops.scaler = d3.scale
      .linear()
      .domain([
        d3.min(timetable, accessor("start")),
    ...
    chartprops.svg
      ...
      .attr("x", 0)
      .attr("y", function (d, i) {
        return i * (chartprops.bheight + 2 * chartprops.bpad);
      })
    ...
    chartprops.svg
      ...
      //.attr("ry", 3)
      .attr("x", function (d) {
        return chartprops.scaler(d.start) + chartprops.bpad;
      })
      .attr("y", function (d, i) {
        return i * (chartprops.bheight + 2 * chartprops.bpad) + chartprops.bpad;
      })
      .attr("width", function (d) {
        return chartprops.scaler(d.end) - chartprops.scaler(d.start);
      })
