...
describe("FormUtils", () => {
  ...
  describe("getFieldByComplexPropName", () => {
    it("should not find field", () => {
      ...
      expect(FormUtils.getFieldByComplexPropName({}, "a")).to.be.undefined;
      expect(FormUtils.getFieldByComplexPropName({ b: "b" }, "a")).to.be
        .undefined;
      expect(FormUtils.getFieldByComplexPropName({ a: { b: "c" } }, "a.c")).to
        .be.undefined;
    });
  ...
  describe("tabFieldsIncludeErrorFields", () => {
    ...
    it("should not find entry in non matching array", () => {
      expect(FormUtils.tabFieldsIncludeErrorFields(["a", "b"], ["c", "d"])).to
        .be.false;
    });

    it("should find entry in matching array", () => {
      expect(FormUtils.tabFieldsIncludeErrorFields(["a", "b"], ["a", "d"])).to
        .be.true;
    });

    it("should find entry in array with dot delimetered name", () => {
      expect(FormUtils.tabFieldsIncludeErrorFields(["a.e", "b"], ["a.e", "d"]))
        .to.be.true;
    });
  });
  ...
  describe("findFieldsWithError", () => {
    ...
    it("should return array of simple paths", () => {
      ...
      expect(FormUtils.findFieldsWithError({ a: "1", b: "2" })).to.eql([
        "a",
        "b",
      ]);
      expect(FormUtils.findFieldsWithError({ a: "1", bbb: "2" })).to.eql([
        "a",
        "bbb",
      ]);
    });
    ...
    it("should apply prefix", () => {
      expect(FormUtils.findFieldsWithError({ a: "1", b: "2" }, "p")).to.eql([
        "p.a",
        "p.b",
      ]);
      expect(
  ...
  describe("findTabWithError", () => {
    ...
    it("should return undefined if selected name is not found", () => {
      expect(FormUtils.findTabWithError(formconfig, [], "tabQ")).to.be
        .undefined;
    });
  ...
  describe("addInitValue", () => {
    ...
    it("should add nested value", () => {
      ...
      expect(FormUtils.addInitValue({}, "a.b.c", "a")).to.eql({
        a: { b: { c: "a" } },
      });
    });

    it("should add nested value to existing accumulator", () => {
      expect(FormUtils.addInitValue({ d: "d" }, "a.b", "a")).to.eql({
        d: "d",
        a: { b: "a" },
      });
      expect(FormUtils.addInitValue({ a: { d: "d" } }, "a.b.c", "a")).to.eql({
        a: { d: "d", b: { c: "a" } },
      });
    });
  });
  ...
  describe("addValidator", () => {
    ...
    it("should add validators", () => {
      ...
      const elementConfig = {
        propName: "a",
        label: "L",
        validate: { isRequired: true, isNumber: true, isWholeNumber: true },
      };
      const result = FormUtils.addValidators(accumulator, elementConfig);
