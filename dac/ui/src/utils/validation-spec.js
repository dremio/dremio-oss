...
describe("validation", () => {
  describe("isRequired", () => {
    it("should fail if null or empty string", () => {
      ...
      expect(isRequired("foo")({ foo: "" })).to.eql({
        foo: "Foo is required.",
      });
    });
  ...
  describe("confirmPassword", () => {
    it("should not fail when both password and confirmation password are empty", () => {
      ...
      expect(confirmPassword("password", "passwordVerify")(values)).to.be
        .undefined;
    });
    ...
    it("should fail when password and confirmation password don't match", () => {
      ...
      expect(validator({ password: "p", passwordVerify: "" })).to.be.eql({
        passwordVerify: "Passwords don't match.",
      });
      expect(validator({ password: "", passwordVerify: "p" })).to.be.eql({
        passwordVerify: "Passwords don't match.",
      });
    });
  });
  ...
  describe("noDoubleQuotes", () => {
    it("should prevent double quotes", () => {
      expect(noDoubleQuotes("name")({ name: 'a"b' })).to.eql({
        name: "Double quotes are not allowed.",
      });
      expect(noDoubleQuotes("name")({ name: 'a""b' })).to.eql({
        name: "Double quotes are not allowed.",
      });
  ...
  describe("noSpaces", () => {
    it("should prevent spaces", () => {
      expect(noSpaces("name")({ name: "a b" })).to.eql({
        name: "Spaces are not allowed.",
      });
      expect(noSpaces("name")({ name: "a b   c" })).to.eql({
        name: "Spaces are not allowed.",
      });
      expect(noSpaces("name")({ name: " ab" })).to.eql({
        name: "Spaces are not allowed.",
      });
      expect(noSpaces("name")({ name: "ab " })).to.eql({
        name: "Spaces are not allowed.",
  ...
  describe("isIntegerWithLimits", () => {
    ...
    describe("topLimit only", () => {
      const testCase = (value, limit, isInvalidValue = true) => {
        it(`'${value}' does${
          isInvalidValue ? " not" : ""
        } meet '${limit}' top limit`, () => {
          const fieldName = "f1";
          const data = {
            [fieldName]: value,
          };
          const validator = isIntegerWithLimits(
            fieldName,
            fieldName,
            null,
            limit,
    ...
    describe("both limits", () => {
      const testCase = (value, lowLimit, topLimit, isInvalidValue = true) => {
        it(`'${value}' does${
          isInvalidValue ? " not" : ""
        } lie between '${lowLimit}' and '${topLimit}'`, () => {
          const fieldName = "f1";
          const data = {
            [fieldName]: value,
          };
...
function itShouldWorkForNestedKeys(validator, goodValue, badValue) {
  it("should work for nested keys", () => {
    ...
    expect(validator("foo.bar")({ foo: { bar: badValue } }).foo.bar).to.not.be
      .undefined;
  });
}
