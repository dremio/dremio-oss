...
class Transforms {
  ...
  dropColumn(table, item) {
    ...
    const columns = table
      .get("columns")
      .filter((col) => col.get("name") !== colName);
  ...
  _getColumnsWithoutPreviewColumns(columns) {
    return columns
      .filter((col) => col.get("status") !== "HIGHLIGHTED")
  ...
  previewTransforms(table, item) {
    ...
    if (
      item.fieldTransformation &&
      item.fieldTransformation.type.indexOf("Convert") !== -1
    ) {
