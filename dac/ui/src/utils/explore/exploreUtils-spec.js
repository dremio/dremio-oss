...
describe("exploreUtils", () => {
  ...
  describe("updateSelectionToSingleCell", () => {
    ...
    it("should return null if no selection or ranges or more than one range", () => {
      ...
      expect(exploreUtils.updateSelectionToSingleCell({ rangeCount: 0 })).to.be
        .null;
      expect(exploreUtils.updateSelectionToSingleCell({ rangeCount: 2 })).to.be
        .null;
    });

    it("returns selection if start and end cells are the same ", () => {
      sinon
        .stub(exploreUtils, "getTableCellElement")
        .returns(centerCell)
        .onSecondCall()
        .returns(centerCell);
  ...
  describe("calculateListSelectionOffsets", () => {
    ...
    it("should return null if no item is selected", () => {
      expect(exploreUtils.calculateListSelectionOffsets(0, 1, '["a"]')).to.be
        .null;
      expect(exploreUtils.calculateListSelectionOffsets(4, 5, '["a"]')).to.be
        .null; // ["a"|]|
      expect(exploreUtils.calculateListSelectionOffsets(4, 5, '["a","b"]')).to
        .be.null; // ["a"|,|"b"]
    });
    ...
    it("should complete a partial item", () => {
      expect(selectionResult('["a|b|c","def","ghi"]', listText)).to.eql([
        '"abc"',
        0,
        1,
      ]);
    ...
    it("should complete multiple items", () => {
      expect(selectionResult('["a|bc","def","ghi|"]', listText)).to.eql([
        '"abc","def","ghi"',
        0,
        3,
      ]);
    ...
    it("should select the included but not excluded item when selecting at a boundary", () => {
      expect(selectionResult('["abc"|,"def",|"ghi"]', listText)).to.eql([
        '"def"',
        1,
        2,
      ]);
    ...
    it("should not include square brackets", () => {
      expect(selectionResult('|["ab|c","def","ghi"]', listText)).to.eql([
        '"abc"',
        0,
        1,
      ]);
  ...
  describe("method getLocationToGoToTransformWizard", () => {
    it("should return pathname/details", () => {
      ...
      const result = exploreUtils.getLocationToGoToTransformWizard({
        detailType,
        column,
        toType,
        location,
  ...
  describe("method getPreviewTransformationLink", () => {
    ...
    it("should return link for preview", () => {
      ...
      expect(exploreUtils.getPreviewTransformationLink(dataset, "345")).to.eql(
        `${dataset.getIn([
          "apiLinks",
          "self",
