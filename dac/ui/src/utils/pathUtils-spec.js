...
describe("pathUtils", () => {
  describe("splitFullPath", () => {
    it('should remove ""', () => {
      ...
      expect(splitFullPath('"my-space"."my-folder"')).to.eql([
        "my-space",
        "my-folder",
      ]);
      expect(splitFullPath('"my-space".my-folder')).to.eql([
  ...
  describe("parseResourceId", () => {
    ...
    it("should return username for home path with folder", () => {
      expect(parseResourceId("/home/@test_user/folder/ff", "test_user")).to.eql(
        '"@test_user"."ff"'
      );
    });
  ...
  describe("getRouteParamsFromLocation", () => {
    ...
    it("should decode pathname", () => {
      let routParams = getRouteParamsFromLocation({
        pathname: 'a/b/"c"/%22d%22',
      });
      expect(routParams.resourceId).to.equal('"c"');
      expect(routParams.tableId).to.equal('"d"');
      ...
      const pathname = ["a", "b", spaceName, datasetName]
