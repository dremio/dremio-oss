...
export function isRequiredIfAnotherPropertyEqual(
  key,
  dependetKey,
  dependetValue,
  label
...
export function isWholeNumber(key, label = key) {
  return function (values) {
    ...
    if (
      !isEmptyValue(value) &&
      (isNaN(Number(value)) || Number(value) % 1 || Number(value) < 0)
    ) {
      return set(
        {},
        key,
...
export const isIntegerWithLimits = (
  key,
  label = key,
  lowLimit = null,
  topLimit = null
  ...
  return (values) => {
    ...
    const value = result(values, key);
    if (
      (!isEmptyValue(value) && lowLimit !== null && value < lowLimit) ||
