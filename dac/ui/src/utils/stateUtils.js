...
class StateUtils {
  ...
  request(state, propertyPath) {
    ...
    return state
      .setIn([...activePath, "isInProgress"], true)
  ...
  success(state, propertyPath, payload, mapFunction) {
    ...
    const activePath = propertyPath.slice(0, -1);
    return state
      .setIn(propertyPath, Immutable.fromJS(mapFunction(payload)))
  ...
  failed(state, propertyPath) {
    ...
    const activePath = propertyPath.slice(0, -1);
    return state
      .set([...activePath, "isInProgress"], false)
      .set([...activePath, "isFailed"], true);
  }
}
