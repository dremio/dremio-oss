...
export const INVISIBLE_CHARS = new Set(
  Object.keys(INVISIBLES).map((k) => INVISIBLES[k])
);
...
class DataFormatUtils {
  ...
  formatValue(value, columnType, row, withInvisibles = false) {
    ...
    for (const char of str) {
      ...
      if (!INVISIBLE_CHARS.has(char) || charName === "OGHAM_SPACE_MARK") {
        // OGHAM_SPACE_MARK already shows something in Menlo
        ...
        out.push(
          <span
            ...
            title={charName.replace(/_/g, " ")}
            data-char={charName}
          >
            <span className="special-char-inner" data-char={charName}>
              {char}
            </span>
          </span>
        );
