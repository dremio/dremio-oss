...
export const useVersionedPageCommits = ({
  filter,
  branch,
  hash,
  pageSize = 50,
  ...
  const { reservedNamespace } = useVersionedPageContext();
  ...
  const VersionedPageCommitsResource = useRef(
  ...
  useEffect(() => {
    if (branch) {
      VersionedPageCommitsResource.current.fetch({
        name: branch,
        search: filter,
        path: extractedPath,
        hash,
  ...
  const [next, error] = useResourceSnapshot(
    VersionedPageCommitsResource.current,
  );
  ...
  return [
    ...
    error,
    useResourceStatus(VersionedPageCommitsResource.current),
  ] as const;
};
