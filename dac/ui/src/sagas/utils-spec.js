...
describe("saga utils", () => {
  describe("getLocationChangePredicate", () => {
    it("should return predicate that tests action type and that location has changed", () => {
      ...
      expect(predicate({ type: "foo", payload: { pathname: "foo" } })).to.be
        .false;
      expect(predicate({ type: LOCATION_CHANGE, payload: oldLocation })).to.be
        .false;
      expect(
        predicate({
  ...
  describe("getExplorePageLocationChangePredicate", () => {
    const generateRouteState = ({ path, pageType, query, state }) => ({
      location: {
  ...
  describe("getActionPredicate", () => {
    ...
    describe("with entity", () => {
      it("should return true only if entity === action entity, when action type matches", () => {
        const entity = "entity";
        expect(getActionPredicate("SUCCESS", entity)({ type: "SUCCESS" })).to.be
          .false;
        expect(
          getActionPredicate(
            "SUCCESS",
