...
export default function table(state, action) {
  switch (action.type) {
    ...
    case LOAD_NEXT_ROWS_SUCCESS: {
      ...
      const outputRecords = state.getIn([
        "tableData",
        datasetVersion,
        "outputRecords",
      ]);
      return state.mergeIn(["tableData", datasetVersion], {
    ...
    case FAILED_EXPLORE_JOB_PROGRESS: {
      const jobProgress = state.getIn([
        "tableData",
        action.datasetVersion,
        "jobProgress",
      ]);
    ...
    case UPDATE_EXPLORE_JOB_PROGRESS: {
      ...
      const newState = haveChange
        ? state.setIn(["tableData", jobUpdate.datasetVersion, "jobProgress"], {
            ...jobProgress,
            jobId: jobUpdate.id,
    ...
    case SET_EXPLORE_JOBID_IN_PROGRESS: {
      const previousJobStatus = state.getIn([
        "tableData",
        action.datasetVersion,
        "jobProgress",
      ]);
      const jobProgress =
        previousJobStatus || state.getIn(["tableData", "jobProgress"]);
      return state.setIn(["tableData", action.datasetVersion, "jobProgress"], {
        ...jobProgress,
        jobId: action.jobId,
