...
export default function grid(oldState, action) {
  ...
  switch (action.type) {
    ...
    case ActionTypes.UPDATE_TRANSFORM_CARD: {
      ...
      const nextCards =
        state.getIn(["transform", transformType, method, "cards"]) ||
        Immutable.List();
      const card = (nextCards.get(index) || Immutable.Map()).merge({
        description,
        examplesList,
        unmatchedCount,
        matchedCount,
