...
import { DEFAULT_FORMAT_WITH_TIME, formatDate, formatDateSince } from '@app/utils/date';
  ...
  return (
    <div className={classNames('commitEntry', { isSelected, disabled })}>
      ...
      <div className='commitEntry-content' ref={ref}>
        ...
        {commit.hash && <CommitHash disabled={disabled} hash={commit.hash} branch={branch} />}
        <span className='commitEntry-time text-ellipsis'>{formatDateSince(commit.commitTime + '')}</span>
      </div>
    </div>
  );
}
