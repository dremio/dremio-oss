...
export default class AboutModal extends Component {
  ...
  tabDetails = Immutable.fromJS([
    {
      name: "build",
      label: this.props.intl.formatMessage({ id: "Common.BuildInfo" }),
    },
    {
      name: "metrics",
      label: this.props.intl.formatMessage({ id: "Common.ClusterData" }),
    },
  ]);
      ...
      async () => {
        ...
        for (let i = 0; i < dates.length; i++) {
          if (userData.userStatsByDate) {
            const userList = userData.userStatsByDate.filter(
          ...
          if (jobData.jobStats) {
            const jobList = jobData.jobStats.filter(
              (item) => item.date === dates[i]
            );
            if (jobList.length !== 0) {
  ...
  renderTabs() {
    ...
    return (
      <div className="tabs-holder" style={styles.base}>
  ...
  renderTabsContent = (activeTab) => {
    ...
    return activeTab === "build"
      ? this.renderVersion()
      : this.renderCluterData();
  };

  renderVersion() {
    const { versionInfo, inProgress } = this.state;
    ...
    return inProgress ? (
      ...
      <dl className="normalFontSize">
        ...
        <dt style={styles.dtStyle}>
          <FormattedMessage id="App.BuildTime" />
        </dt>
        <dd>{buildTime}</dd>

        <dt style={styles.dtStyle}>
          <FormattedMessage id="App.ChangeHash" />
        </dt>
      ...
      >
        <div style={styles.container}>
          ...
          <div style={styles.pane}>
            <div style={{ flex: 1, marginBottom: 21 }}>
              <div style={{ fontSize: "2em", marginBottom: 10 }}>
                {intl.formatMessage({
                  id: isBeta ? "App.DremioBeta" : "App.Dremio",
                })}
              </div>
              {this.renderTabs()}
              <div>{this.renderTabsContent(activeTab)}</div>
            </div>
            <div style={formDescription}>
              {intl.formatMessage({ id: "App.Copyright" })}
            </div>
            <div style={formDescription}>
              {isBeta && intl.formatMessage({ id: "App.ProductionWarning" })}
            </div>
          </div>
        </div>
      </Modal>
