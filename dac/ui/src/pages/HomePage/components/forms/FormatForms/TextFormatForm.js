...
export default class TextFormatForm extends Component {
  ...
  onDelimiterChange = (field) => (evtOrValue) => {
    const value =
      typeof evtOrValue === "object" ? evtOrValue.target.value : evtOrValue;
    let newValue = value;
    ...
    try {
      ...
      this.setState((state) => ({
        invalidJSONFields: { ...state.invalidJSONFields, [field.name]: false },
      }));
    } catch (e) {
      this.setState((state) => ({
        invalidJSONFields: { ...state.invalidJSONFields, [field.name]: true },
      }));
    }
  ...
  getDelimiterValue = (field) => {
    const { value } = field;
    if (value.match(/^\\+$/)) {
      // treating as invalid, see note above
      return value;
    }
  ...
  render() {
    ...
    const fieldDelimiterOptions = [
      { option: ",", label: intl.formatMessage({ id: "File.Comma" }) },
      { option: "\\t", label: intl.formatMessage({ id: "File.Tab" }) },
      { option: "|", label: intl.formatMessage({ id: "File.Pipe" }) },
    ];
    const quoteOptions = [
      { option: '"', label: intl.formatMessage({ id: "File.DoubleQuote" }) },
      { option: "'", label: intl.formatMessage({ id: "File.SingleQuote" }) },
    ];
    const commentOptions = [
      { option: "#", label: intl.formatMessage({ id: "File.NumberSign" }) },
      { option: "//", label: intl.formatMessage({ id: "File.DoubleSlash" }) },
    ];
    const lineDelimiterOptions = [
      {
        option: "\\r\\n",
        label: intl.formatMessage({ id: "File.WindowsDelimiter" }),
      },
      {
        option: "\\n",
          ...
          <div style={styles.lastField}>
            ...
            <div style={styles.options}>
              ...
              <Checkbox
                disabled={disabled}
                style={styles.checkbox}
                dataQa="skip-first-line"
                label={intl.formatMessage({ id: "File.SkipFirstLine" })}
                {...skipFirstLine}
              />
            </div>
            <div style={styles.options}>
