...
class JobTr extends PureComponent {
  static propTypes = {
  ...
  renderSqlBlock(queryType) {
    ...
    if (queryType !== "run" && queryType !== "preview") {
      return (
        <div
          className="sql"
          style={{ ...fixedWidthSmall, ...styles.description }}
        >
          <TextHighlight
  ...
  render() {
    const { even, active, onClick, containsTextValue, job, jobDataset } =
      this.props;
    ...
    let trStyles = { ...styles.base };
    if (even) {
      trStyles = { ...trStyles, ...styles.even };
    }
    if (active) {
      trStyles = { ...trStyles, ...styles.active };
    }
      ...
      >
        <div style={styles.headerRow}>
          ...
          <div style={styles.columns}>
            <div style={{ width: 100 }}>{jobDataset}</div>
            <div className="user">
              <TextHighlight
                text={job.get("user")}
                inputValue={containsTextValue}
