...
export class JobTable extends PureComponent {
  static propTypes = {
  ...
  renderHeaderBlock() {
    return (
      <div
        className="job-table-header"
        style={{ ...style.jobTableHeader, ...formLabel }}
      >
        {this.state.columnsHeader.keySeq().map((key) => (
          ...
          >
            <span style={h5}>
              {this.state.columnsHeader.getIn([key, "label"])}
            </span>
  ...
  renderIconForColumnHeader(colName) {
    ...
    if (direction && colName === this.context.location.query.sort) {
      const type = direction === "DESCENDING" ? "fa-caret-down" : "fa-caret-up";
  ...
  renderJobTr({ index, key, style }) {
    if (
      index + JOBS_OFFSET > this.props.jobs.size &&
      this.props.next &&
      !this.props.isNextJobsInProgress &&
    ...
    const showOverlay =
      !jobsUtils.isMetadataJob(item.get("requestType")) && !!datasetType;
    const datasetLabel = (
      <DatasetItemLabel
        name={dataset}
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <div style={style.jobsWrap}>{this.renderJobs()}</div>
      </div>
    );
  }
}
