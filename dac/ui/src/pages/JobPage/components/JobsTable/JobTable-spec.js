...
describe("JobsTable-spec", () => {
      ...
      jobs: Immutable.fromJS([
        {
          ...
          state: "COMPLETED",
          sql:
            'WITH dremio_limited_preview AS (SELECT *\nFROM "Prod-Sample"."ds2") ' +
          ...
          parentsList: [
            { datasetPathList: ["Tpch-Sample.tpch03", "Tpch-Sample.tpch04"] },
          ],
  ...
  it("render Jobs Table", () => {
    const wrapper = shallow(<JobTable {...commonProps} />, {
  ...
  describe("renderHeaderBlock", () => {
    ...
    beforeEach(() => {
      ...
      wrapper = shallow(<JobTable {...commonProps} />, {
        context: { location: { query: {} } },
      });
