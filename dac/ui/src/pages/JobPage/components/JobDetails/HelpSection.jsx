...
export class HelpSection extends PureComponent {
  ...
  async componentDidMount() {
    try {
      if (
        getPrivilegeContext().isAdmin() &&
        (await getSupportFlag("support.admins.download")).value
      ) {
        this.setState({ profileDownloadEnabled: true });
        return;
      }

      if ((await getSupportFlag("support.users.download")).value) {
        this.setState({ profileDownloadEnabled: true });
      }
    } catch (e) {
