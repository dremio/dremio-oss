...
import {
  getQueueName,
  getRouteLabel,
} from "@inject/pages/JobPage/components/JobDetails/OverviewContentUtil";
...
class OverviewContent extends PureComponent {
  static checkResultOfProfile = (attemptDetails, reason = "") => {
    ...
    return attemptDetails.filter(
      (profile) => profile.get("reason").indexOf(reason) !== -1
    ).size;
  };
  ...
  isSingleReflectionBlockToBeShown() {
    ...
    return (
      ...
      materializationFor &&
      materializationFor.has("reflectionType")
    );
  }
  ...
  isParentsBlockToBeShown() {
    ...
    return !!(
      listItem &&
      (listItem.get("datasetPathList") || listItem.get("type"))
    );
  }

  renderJobSummary(jobDetails, intl) {
    const endTime = jobsUtils.getFinishTime(
      jobDetails.get("state"),
      jobDetails.get("endTime")
    );
    const jobId = jobDetails.get("jobId").get("id");
    const queueName = getQueueName(jobDetails);
    const jobIdUrl = jobsUtils.navigationURLForJobId({
      id: jobId,
      createFullUrl: true,
      ...
      <div className="detail-row">
        ...
        <ul>
          <ListItem label={intl.formatMessage({ id: "Job.QueryType" })}>
            <span>
              {intl.formatMessage({
                id: this.getFormatMessageIdForQueryType(),
              })}
            </span>
          </ListItem>
          {haveMultipleAttempts && (
            <ListItem
              label={intl.formatMessage({ id: "Job.LastAttemptDuration" })}
            >
          ...
          {queueName && (
            <ListItem label={intl.formatMessage({ id: routeLabel })}>
              <span>{queueName}</span>
            </ListItem>
          )}
          <ListItem
  ...
  renderCancellationLog() {
    ...
    return cancellationInfo ? (
      <JobErrorLog failureInfo={cancellationInfo} />
    ) : null;
  }
  ...
  renderDatasetBlock() {
    ...
    const dataset =
      jobDetails.getIn(["datasetPathList", -1]) ||
      intl.formatMessage({ id: "File.Unknown" });
    ...
    return (
      <div>
        <h4 style={{ marginBottom: 10 }}>
          {intl.formatMessage({ id: "Job.Reflection" })}
        </h4>
        <ReflectionBlock
          datasetFullPath={datasetPathList}
  ...
  renderParentsBlock() {
    ...
    return (
      <div>
        <h5>
          <FormattedMessage id="Job.Parents" />
        </h5>
        <ul style={styles.parentList}>
          {jobDetails.get("parentsList").map((item, key) => {
            const dataset =
              item.get("datasetPathList") && item.get("datasetPathList").last();
            return (
              <li key={key} style={styles.parentItem}>
                ...
                <div style={{ display: "flex" }}>
                  {false && (
                    <SettingsBtn
                      style={{
                        backgroundColor: "rgba(0, 0, 0, 0.0392157)",
                        height: 27,
                      }}
                      hasDropdown={false}
                    />
  ...
  renderReflectionListBlock() {
    const byRelationship = jobsUtils.getReflectionsByRelationship(
  ...
  renderInfoAboutProfile() {
    ...
    const isHaveSchemas = OverviewContent.checkResultOfProfile(
      attemptDetails,
      "schema"
    );
    const isHaveMemory = OverviewContent.checkResultOfProfile(
      attemptDetails,
      "memory"
    );
    ...
    return (
      <div>
        ...
        {!this.isMetadataJob() && (
          ...
          >
            {(this.isDatasetBlockToBeShown() ||
              this.isParentsBlockToBeShown()) && (
              <h4>{intl.formatMessage({ id: "Job.Query" })}</h4>
            )}
            {this.isDatasetBlockToBeShown() && this.renderDatasetBlock()}
            {this.isSingleReflectionBlockToBeShown() &&
              this.renderSingleReflectionBlock()}
            {this.isParentsBlockToBeShown() && this.renderParentsBlock()}
            {this.renderReflectionListBlock()}
            {jobDetails.get("stats") && (
              <Quote jobIOData={jobDetails.get("stats")} />
            )}
            {this.renderSqlBlock()}
          </div>
        )}
