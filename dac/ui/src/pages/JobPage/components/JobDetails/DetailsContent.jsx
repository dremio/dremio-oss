...
class DetailsContent extends PureComponent {
      ...
      tableDatasetList.map((item, index) => {
        ...
        const title = (
          <span style={{ ...styles.flexAlign, paddingRight: 7, flexGrow: 1 }}>
            ...
            {item.get("accelerated") && (
              <dremio-icon name="interface/flame"></dremio-icon>
            )}
          </span>
        );
            ...
            >
              <ul>
                ...
                {datasetProfile.get("pushdownQuery")
                  ? this.getCodeMirrorItem(datasetProfile.get("pushdownQuery"))
                  : null}
                {datasetProfile.get("dataVolumeInBytes")
                  ? this.getFsDatasetData(item)
                  : null}
              </ul>
            </div>
          </VisibilityToggler>
  ...
  getTopOperations(topOperations) {
    ...
    return (
      <ul style={styles.topOperations}>
        {topOperations.map &&
          topOperations.map((item, index) => {
            const cpuUsage = DetailsContent.getCpuUsage(
              item.get("timeConsumed"),
            );
            return (
              <ListItem key={index}>
                <div style={styles.operationName}>
                  {DetailsContent.getProcessType(item.get("type"))}
                </div>
  ...
  render() {
    ...
    const spilledDetails =
      jobDetails.get("spilled") && jobDetails.get("spillDetails");
