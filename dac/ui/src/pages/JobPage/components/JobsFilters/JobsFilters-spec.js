...
describe("JobsFilters", () => {
  ...
  it("should render ShowHideColumn component", () => {
    const wrapper = shallow(<JobsFilters {...minimalProps} isQVJobs />, {
      context,
    });
  ...
  describe("handlers -", () => {
    ...
    describe("addInfoToFilter", () => {
      it("should update the queryState.filter for type", () => {
        ...
        wrapper.instance().addInfoToFilter("foo", "bar");
        expect(commonProps.onUpdateQueryState.getCall(1).args[0].equals(result))
          .to.be.true;
      });
    });
    ...
    describe("changeSortItem", () => {
      it("should update queryState with sort", () => {
        ...
        const result = commonProps.queryState
          .set("sort", "id")
    ...
    describe("handleEnterText", () => {
      it("should update contains with text, and remove it if no text", () => {
        ...
        const result2 = commonProps.queryState.deleteIn([
          "filters",
          "contains",
        ]);
        expect(commonProps.onUpdateQueryState.getCall(1).args[0].toJS()).to.eql(
