...
describe("YarnForm", () => {
  ...
  describe("#componentWillReceiveProps", () => {
    ...
    it("should not update spillDirectories on distroType change if user changed its value", () => {
      wrapper.setProps({
        values: { ...commonProps.values, spillDirectories: ["/custom/dir"] },
      });
      wrapper.setProps({
        values: {
          ...commonProps.values,
          distroType: "OTHER",
          spillDirectories: ["/custom/dir"],
        },
      });
    ...
    it("should not update namenodeHost field when user changed its value", () => {
      wrapper.setProps({
        values: { ...commonProps.values, namenodeHost: "custom_host" },
      });
      wrapper.setProps({
        values: {
          ...commonProps.values,
          distroType: "OTHER",
          namenodeHost: "custom_host",
        },
      });
  ...
  describe("#prepareValuesForSave", () => {
    ...
    it("should map values with propertyList into entity", () => {
      ...
      const result = instance.prepareValuesForSave({
        ...values,
        propertyList: [propListEntry],
      });
      const subPropertyList = result.yarnProps.subPropertyList;
      expect(subPropertyList[subPropertyList.length - 1]).to.eql({
        key: "a",
        value: "v",
        type: "JAVA_PROP",
      });
