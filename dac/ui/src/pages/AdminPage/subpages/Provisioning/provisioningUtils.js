...
export function getYarnSubProperty(entity, propName) {
  const subProperty = entity
    .getIn(["yarnProps", "subPropertyList"])
...
export function makeContainerPropertyRow(listItem) {
  ...
  return containerPropertyList.reduce((prev, property) => {
    return {
      ...prev,
      [property.get("key")]: { node: () => property.get("value") },
    };
  }, {});
}
...
export function getEngineStatusCounts(engine) {
  if (!engine)
    return { active: 0, pending: 0, disconnected: 0, decommissioning: 0 };

  const workersSummary = engine.get("workersSummary");
  const { active, pending, disconnected, decommissioning } =
...
export function getIsInReadOnlyState(engine) {
  ...
  const currentState = engine.get("state");
  return (
    currentState === CLUSTER_STATE.starting ||
