...
import {
  ExploreTableView as ExploreTable,
  RIGHT_TREE_OFFSET,
} from "./ExploreTable";
...
describe("ExploreTable", () => {
  ...
  describe("needUpdateColumns", () => {
    it("should return false when there are no columns at all", () => {
      expect(instance.needUpdateColumns({ columns: Immutable.fromJS([]) })).to
        .be.false;
    });
    ...
    it("should return true when hidden value changes", () => {
      instance.setState({
        columns: commonProps.columns.setIn([0, "hidden"], true),
      });
      expect(instance.needUpdateColumns(commonProps)).to.be.true;
    });
  ...
  describe("shouldShowNoData", () => {
    it("should return false if viewState.isInProgress", () => {
      expect(instance.shouldShowNoData(Immutable.Map({ isInProgress: true })))
        .to.be.false;
    });

    it("should return false if viewState.isFailed", () => {
      expect(instance.shouldShowNoData(Immutable.Map({ isInFailed: true }))).to
        .be.false;
    });

    it("should return false if dataset has no version", () => {
      wrapper.setProps({
        dataset: commonProps.dataset.set("datasetVersion", null),
      });
