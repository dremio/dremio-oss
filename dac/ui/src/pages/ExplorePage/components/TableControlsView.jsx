...
export class TableControlsView extends PureComponent {
  ...
  async componentDidMount() {
    try {
      ...
      const asyncRes = await getSupportFlag(ALLOW_ASYNC_DOWNLOAD);
      this.setState({
        allowAsyncDownload: asyncRes?.value,
      });
    } catch (e) {
      //
    }
  ...
  renderSavedButton = (datasetColumns) => {
    const { dataset } = this.props;
    const isActionDisabled =
  ...
  renderDownloadingMessage() {
    ...
    if (!jobSummary || !isDownloading) {
      return;
    }

    return (
      <Message
        message={intl.formatMessage({ id: "Download.Notification.New" })}
        messageType="info"
        isDismissable
      />
    );
  ...
  render() {
    ...
    return (
      <div className="table-controls-wrapper">
            ...
            {!showJobsTable && isQuerySuccess && (
              <div className="table-controls__right" style={styles.right}>
                ...
                {this.state.allowDownload &&
                  (this.state.allowAsyncDownload ? (
                    <DownloadButton
                      dataset={dataset}
                      datasetColumns={datasetColumns}
                      isDownloading={isDownloading}
                      disableOptions={disableButtons}
                      setIsDownloading={this.setIsDownloading}
                    />
                  ) : (
                    this.renderSavedButton(datasetColumns)
                  ))}
        ...
        {this.state.allowAsyncDownload && this.renderDownloadingMessage()}
...
function mapStateToProps(state) {
  ...
  return {
    jobSummary: getJobSummaries(state)[jobId],
  };
}
