...
export class TransformView extends Component {
  ...
  renderSubHeadersTitle(subtitle) {
    if (
      !this.props.cardsViewState.get("isInProgress") &&
      subtitle.name === this.props.transform.get("method")
    ) {
      return (
        <div
          style={{
    ...
    return (
      <Link
        ...
        to={{
          ...location,
          state: { ...location.state, method: subtitle.name },
        }}
      >
        {subtitle.name}
      </Link>
  ...
  renderReplace() {
    ...
    const formKey = `${transform.get("transformType")}.${transform.get(
      "method",
    )}`;

    return (
      <Tabs activeTab={method}>
        <ReplacePatternForm {...formProps} tabId="Pattern" formKey={formKey} />
        <ReplaceValuesForm
          tabId="Values"
          {...formProps}
        ...
        <ReplaceExactForm
          ...
          formKey={formKey}
        />
        <ReplaceRangeForm tabId="Range" {...formProps} formKey={formKey} />
      </Tabs>
    );
  }
    ...
    return (
      <Tabs activeTab={transform.get("transformType")}>
        ...
        <Tabs tabId="split" activeTab="split">
          <SplitForm tabId="split" {...formProps} />
        </Tabs>
        <div tabId="keeponly">{this.renderReplace()}</div>
        <div tabId="exclude">{this.renderReplace()}</div>
      </Tabs>
    );
  }
  ...
  render() {
    ...
    const subtitles =
      ["replace", "keeponly", "exclude"].indexOf(transformType) !== -1
