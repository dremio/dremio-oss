...
describe("ReplaceRangeForm", () => {
  ...
  describe("combineBins", () => {
    it("should combine bins and decrease data length by 2", () => {
      ...
      const originalBinLength =
        chartData[0].range.upperLimit - chartData[0].range.lowerLimit;
      const result = instance.combineBins(chartData);
      const resultBinLength =
        result[0].range.upperLimit - result[0].range.lowerLimit;
