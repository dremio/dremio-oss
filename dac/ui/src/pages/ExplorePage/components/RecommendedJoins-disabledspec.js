...
describe("RecommendedJoins", () => {
  ...
  describe("#componentDidMount", () => {
    ...
    it("should set active recommended join as first item from recommended joins list", () => {
      shallow(<RecommendedJoins {...commonProps} />, { context })
        .instance()
        .componentDidMount();
  ...
  describe("#updateFormFields", () => {
    it("should trigger the appropriate onChange calls", () => {
      ...
      expect(commonProps.fields.activeDataset.onChange).to.be.calledWith([
        "Prod-Sample",
        "ds1",
      ]);
      expect(commonProps.fields.columns.onChange).to.be.calledWith([
        { joinedColumn: "age", joinedTableKeyColumnName: "user" },
      ]);
