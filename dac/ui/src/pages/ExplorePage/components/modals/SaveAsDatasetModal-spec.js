...
describe("SaveAsDatasetModal", () => {
  ...
  beforeEach(() => {
    minimalProps = {
      ...
      submitReapplyAndSaveAsDataset: sinon
        .stub()
        .returns(Promise.resolve({ payload: Immutable.Map() })),
      submitSaveAsDataset: sinon
        .stub()
        .returns(Promise.resolve({ payload: Immutable.Map() })),
      navigateToNextDataset: sinon
        .stub()
        .returns(Promise.resolve("navigateToNextDataset")),
      afterSaveDataset: sinon
        .stub()
        .returns(Promise.resolve("afterSaveDataset")),
    };
