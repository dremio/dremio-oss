...
export class TableControls extends PureComponent {
  ...
  navigateToTransformWizard(wizardParams) {
    ...
    const { dataset, exploreViewState, queryStatuses, queryTabNumber } =
      this.props;
    // Lazily fetch currentSql from imperative store (rather than relying on connect) to avoid updating this function on keypress
    const explorePageState = getExploreState(store.getState());
    const currentSql = explorePageState?.view?.currentSql;
    const queryContext = explorePageState?.view?.queryContext;
