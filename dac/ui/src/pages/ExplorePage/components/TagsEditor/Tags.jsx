...
export class TagsView extends Component {
  ...
  onKeyDown = (e) => {
    ...
    const { selectedTagIndex, value } = this.state;
  ...
  moveCursorToTags(positionOrOffset, { isRelative = false } = {}) {
    let focusInput = false;
    this.setState(
      (/* state */ { selectedTagIndex }, /* props */ { tags }) => {
        const cnt = tags.size;
        ...
        focusInput =
          this.showInputField() && (cnt === 0 || cnt <= cursorPosition); // indidcates if an input should be selected after cursor position move. This may happen if position > number of tags. Or if there are no tags at all.

        const newState = {
          selectedTagIndex: focusInput
            ? -1
            : Math.min(Math.max(cursorPosition, 0), cnt - 1),
        };
  ...
  removeTag = async (index, selectNext) => {
    const { onRemoveTag, tags } = this.props;
  ...
  render() {
    const { tags, placeholder, className } = this.props;
    const { value, selectedTagIndex } = this.state;
    ...
    tags.forEach((t, i) => {
      ...
      tagElements.push(
        <span key={`w_${i}`} className={tagWrapper}>
          <div key={`cursor_${i}`} className={cursorPlaceholder}></div>
          <Tag
            key={`tag_${i}`}
            ...
            daqa={`tag-pill-${t}`}
            deleteHandler={
              this.props.onRemoveTag ? () => this.removeTag(i) : null
            }
