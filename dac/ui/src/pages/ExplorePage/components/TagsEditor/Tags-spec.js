...
describe("Tags", () => {
  ...
  describe("cursor move", () => {
    // here we have to use mount, as we need access to dom element to handle focus

    const homeTest = (keyName) =>
      it(`Cursor is moved to a first tag, when ${keyName} is pressed.`, () => {
    ...
    const endTest = (keyName) =>
      it(`Cursor is moved to a last character, when ${keyName} is pressed.`, () => {
        const wrapper = mount(<TagsView {...commonProps} />);
        const instance = wrapper.instance();
  ...
  describe("tags deletion", () => {
    ...
    const deleteTest = (isBACKSPACE) =>
      it(`tag is removed using ${
