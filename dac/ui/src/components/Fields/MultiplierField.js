...
export default class MultiplierField extends Component {
  ...
  handleSelectChange = (unit) => {
    ...
    let valueWithMultipliers =
      this.getConvertedNumberForDisplay() * unitMultipliers.get(unit);
    if (valueMultiplier) {
      valueWithMultipliers /= valueMultiplier;
    }
    onChange(valueWithMultipliers);
  ...
  getUnit() {
    ...
    for (const [key, multiplier] of this.getFilteredUnitMultipliers()) {
      ...
      if (stringifyWithoutExponent(value / multiplier).match(/\.[0-9]{3}/))
        break;
      unit = key;
    }
    return unit;
  ...
  getConvertedNumberForDisplay() {
    ...
    const valueWithMultiplier = valueMultiplier
      ? value * valueMultiplier
      : value;
    let convertedValue =
      valueWithMultiplier / unitMultipliers.get(this.getUnit());
