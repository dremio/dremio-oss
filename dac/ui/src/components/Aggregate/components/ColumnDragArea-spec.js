...
describe("ColumnDragArea", () => {
  ...
  it(
    "should render ExploreDragArea with isDragged true only if type and draItem.type is different," +
      " isDragInProgress is true, canSelectColumn returns true",
    () => {
      const wrapper = shallow(<ColumnDragArea {...commonProps} />);
      const instance = wrapper.instance();
      const canSelectColumnStub = sinon
        .stub(instance, "canSelectColumn")
        .returns(true);
