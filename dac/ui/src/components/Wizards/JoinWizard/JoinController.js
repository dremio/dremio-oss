...
export class JoinController extends Component {
  ...
  tryToPreviewRecommendedJoin(nextProps) {
    if (
      nextProps.joinTab !== RECOMMENDED_JOIN ||
      !nextProps.recommendedJoins.size
    ) {
      return;
    }
    const oldActiveRecommendedJoin =
      this.props.activeRecommendedJoin || Immutable.Map();
    const isActiveRecommendedJoinChanged =
      nextProps.activeRecommendedJoin.size &&
  ...
  renderTab() {
    ...
    return (
      <ViewStateWrapper
        style={{ ...styles.viewStateWrapper, ...viewStateWrapperStyle }}
  ...
  render() {
    ...
    const footer =
      this.props.joinStep === 2 || isRecommendedTabActive ? (
        <DefaultWizardFooter
          ...
          style={{ width: "100%", borderTop: "none", margin: 0 }}
        />
      ) : (
        <StepWizard
    ...
    return (
      <div className="join">
        ...
        >
          <div style={FLEX_WRAPPER}>{this.renderTab()}</div>
          {footer}
        </InnerComplexForm>
      </div>
    );
...
function mapStateToProps(state, ownProps) {
  ...
  const joinDatasetPathList = explorePageState.join.getIn([
    "custom",
    "joinDatasetPathList",
  ]);
  return {
