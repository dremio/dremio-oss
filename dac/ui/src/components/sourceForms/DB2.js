...
export class DB2 extends Component {
  ...
  render() {
    ...
    const {config: {databaseName}} = fields;
    return (
      <ModalForm {...modalFormProps(this.props)} onSubmit={handleSubmit(onFormSubmit)}>
        <FormBody style={formBodyStyle}>
          <General fields={fields} editing={editing}>
            ...
            <div style={{...section, marginTop: -15}}>
              <FieldWithError label='Database' {...databaseName}>
...
export default connectComplexForm({
  ...
  validate: (values) => applyValidators(values, [isRequired('config.databaseName', 'Database Name')])
}, SECTIONS, mapStateToProps, null)(DB2);
