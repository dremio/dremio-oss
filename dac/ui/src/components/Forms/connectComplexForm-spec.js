...
const generateSections = (...funcList) =>
  funcList.map((fn) => ({ [testFnPropName]: fn }));
...
describe("sectionsContainer", () => {
  it("merges 'getFields' methods correctly", () => {
    ...
    expect(sectionsContainer(section1, section2)(comp).getFields()).to.be.eql([
      "1",
      "2",
      "3",
    ]);
  ...
  const testApiKey = (methodName) =>
    it(`merges '${methodName}' methods correctly`, () => {
      const comp = {
        [methodName]: () => ({ a: 1 }),
      };
