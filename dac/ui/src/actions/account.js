...
export const setUserState = (userObject) => {
  return {
    type: SET_USER_STATE,
    payload: userObject,
  };
...
export const editAccount = (accountData, oldName) => (dispatch) =>
  dispatch(fetchEditAccount(accountData, oldName));
...
export const checkUser = () => {
  const apiCall = new APIV2Call().path("login").uncachable();

  return {
    [RSAA]: {
      types: [CHECK_USER_START, CHECK_USER_SUCCESS, CHECK_USER_FAILURE],
      method: "GET",
      endpoint: apiCall,
    },
  };
