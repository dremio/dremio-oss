  ...
  return {
    [RSAA]: {
      types: [
        ...
        {
          type: LOAD_PROVISIONING_SUCCESS,
          meta: { ...meta, entityClears: ["provision"] },
        },
...
export function changeWorkersSize(form, provisionId, viewId) {
  // todo: rename to ~'putDynamicConfig'
  return (dispatch) => {
...
function fetchEditProvision(data, viewId) {
  ...
  const apiCall = new APIV2Call().paths("provision/cluster").path(data.id);
