...
export const uploadFileToPath =
  (file, fileConfig, extension, viewId) => (dispatch, getState) => {
    ...
    const apiCall = new APIV2Call().paths(parentPath).params({ extension });
...
export function loadFileFormat(formatUrl, viewId) {
  ...
  const abortableMeta = {
    ...meta,
    abortInfo: { ...abortInfo, abortController },
  };
