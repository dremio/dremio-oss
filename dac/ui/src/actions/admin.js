...
function fetchNodeCredentials(viewId, skipStartAction) {
  const meta = !skipStartAction ? { viewId } : {};
...
export function loadNodeCredentials(viewId, skipStartAction = false) {
  return (dispatch) => {
    return dispatch(fetchNodeCredentials(viewId, skipStartAction));
  };
}
...
function fetchFilteredUsers(value = "") {
  ...
  if (value) {
    apiCall.paths("users/search").params({ filter: value });
  } else {
    apiCall.paths("users/all").uncachable();
  }
...
export function createFirstUser(form, meta) {
  ...
  const apiCall = new APIV2Call().path("bootstrap").path("firstuser");
