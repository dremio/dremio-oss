...
public class SnapshotBasedIncrementalRefreshTestUtils {
   ...
   */
  public static void validateFullRefresh(final String planReflection, final String tablePath, final Pair<String, String> ... snapshotIdPairs) {
    //we expect only the latest snapshot to be used
    for (Pair<String, String> snapshotIdPair : snapshotIdPairs) {
      testPlanSubstrPatternsInOrderPlanInput(planReflection,
        new String[]{String.format("IcebergManifestList(table=[%s], snapshot=[%s]", tablePath, snapshotIdPair.right)},
        new String[]{String.format("IcebergManifestList(table=[%s], snapshot=[%s]", tablePath, snapshotIdPair.left)});
    }
  }

  /**
   * Validate that we did NOT perform any incremental refresh
   * The plan contains an Empty operator.
   ...
   * @throws JobNotFoundException
   */
  public static void validateNoRefresh(final String planReflection, final String tablePath, final Pair<String, String> ... snapshotIdPairs) {
  ...
  /**
   * Validate that we did perform a Snapshot Based Incremental Refresh
   * Also validate that the plan for that Snapshot Based Incremental Refresh is correct
   * @param planReflection  reflection refresh plan to check for
   * @param tablePath iceberg table being scanned in plan
   ...
   */
  public static void validateSnapshotBasedIncrementalRefresh(final String planReflection, final String tablePath, final Pair<String, String>... snapshotIdPairs) throws Exception {
    for (final Pair<String, String> snapshotIdPair : snapshotIdPairs) {
      ...
      testPlanSubstrPatternsInOrderPlanInput(planReflection,
        new String[]{
          ...
          "HashJoin",
          "IcebergManifestScan",
          String.format("IcebergManifestList(table=[%s], snapshot=[%s]", tablePath, snapshotIdPair.right),
          "Project",
          "IcebergManifestScan",
          String.format("IcebergManifestList(table=[%s], snapshot=[%s]", tablePath, snapshotIdPair.left)
        }, null);
      ...
      testPlanWithAttributesMatchingPatternsPlanInput(planReflection,
        new String[]{
          ".*WriterCommitter(?s:(?!\\R).)*iceberg_operation=\\[INSERT\\].*",
          String.format(".*TableFunction(?s:(?!\\R).)*Table Function Type=\\[DATA_FILE_SCAN\\].*table=\\[%s\\]*",tablePath),
          ...
          String.format("IcebergManifestList\\(table=\\[%s\\], snapshot=\\[%s\\](?s:(?!\\R).)*manifestContent=\\[DATA\\].*", tablePath, snapshotIdPair.right),
          String.format("IcebergManifestList\\(table=\\[%s\\], snapshot=\\[%s\\](?s:(?!\\R).)*manifestContent=\\[DATA\\].*", tablePath, snapshotIdPair.left)
        }, null);
    }
  ...
  /**
   * Validate that we did perform a Snapshot Based Incremental Refresh by Partition
   * Also validate that the plan for that Snapshot Based Incremental Refresh is correct
   */
  public static void validatePartitionBasedIncrementalRefresh(final String planReflection,
    ...
    try {
      ...
      final String[] commonPartPlan = new String[]{
        "WriterCommitter", "iceberg_operation=[UPDATE]", "\n", //commits the changes, adding some files, deleting others
      ...
      final String[] selectDataFromMainDatasetPlan = new String[]{
        ...
        "TableFunction", String.format("Table Function Type=[DATA_FILE_SCAN], table=[%s]",baseTableName), "\n",
        //split gen for the base dataset
        "IcebergSplitGen", "Table Function Type=[ICEBERG_SPLIT_GEN]", "\n",
        //main join on ICEBERG_INCREMENTAL_REFRESH_JOIN_KEY to filter the data to only include partitions
        //that have been modified since last incremental update
        "HashJoin(condition=[=", "joinType=[inner]", "\n",
        //verify the main scan from the base table
        //calculate the affected partitions for the base dataset
        "IcebergIncrementalRefreshJoinKey", String.format("join key=[%s])", joinKeyTransform), "\n",
        //scan data files from the base dataset
        "IcebergManifestScan", "manifestContent=[DATA]", "\n",
        //scan manifest files from the end snapshot
        String.format("IcebergManifestList(table=[%s], snapshot=[%s]", baseTableName, endSnapshotID), "\n"};

      //verify the plan to generate the list of partitions with any files added or deleted
      final String[] selectModifiedPartitionsPlan = new String[]{
        "Project(incrementalRefreshJoinKey0=[$0])", "\n",
        //remove duplicates, we could have duplicates if multiple files in the same partition are added or deleted
        "HashAgg(group=[{0}])", "\n",
        //calculate the affected partitions for the reflection
        "IcebergIncrementalRefreshJoinKey", String.format("join key=[%s])", joinKeyTransform), "\n",
        //project, so that the columns from old and new snapshot align
        "Project(partitionSpecId=[CASE(IS NULL(", "partitionKey=[CASE(IS NULL(", "partitionInfo=[CASE(IS NULL(", "\n",
        "SelectionVectorRemover",  "\n",
        //only keep files that are in the old snapshot or the new snapshot, but not both
        "Filter(condition=[OR(IS NULL", "IS NULL(", "\n",
        //join of file path
        "HashJoin(condition=[=", "joinType=[full]", "\n",
        //get all files (DATA or DELETE) from the end snapshot
        "IcebergManifestScan", "manifestContent=[ALL]", "\n",
        ...
        String.format("IcebergManifestList(table=[%s], snapshot=[%s]", baseTableName, beginSnapshotID), "\n"};

      //verify the plan to delete from the reflection
      final String[] deleteFromReflectionPlan = new String[]{
        //build metadata for deleting files from the reflection
        "TableFunction", "Table Function Type=[DELETED_FILES_METADATA]", "\n",
        "Project(", "\n",
        //filter the files on ICEBERG_INCREMENTAL_REFRESH_JOIN_KEY to only files belonging to affected partitions
        "HashJoin(condition=[=", "joinType=[inner]", "\n",
        ...
        String.format("IcebergManifestList(table=[\"%s\".\"%s\".", ACCELERATOR_STORAGEPLUGIN_NAME,
          m.getReflectionId().getId()), "\n"};
      ...
      final String[] plan = Stream.of(
        commonPartPlan,
        selectDataFromMainDatasetPlan,
        selectModifiedPartitionsPlan,
        deleteFromReflectionPlan,
        selectModifiedPartitionsPlan).flatMap(Stream::of).toArray(String[]::new);
