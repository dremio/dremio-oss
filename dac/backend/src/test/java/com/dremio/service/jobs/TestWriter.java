...
public class TestWriter extends BaseTestServer {
  ...
  public void testDistributionInPlan() throws Exception {
    String query =
        String.format(
            "create table %s.\"%s\" hash partition by (\"position\") as select \"position\", isFte, "
                ...
                + "group by \"position\", isFte",
            DFS_TEST_PLUGIN_NAME, "hashDistribute");
              ...
              for (RelTrait trait : other.getTraitSet()) {
                if (trait instanceof DistributionTrait) {
                  List<DistributionField> distributionFieldList =
                      ((DistributionTrait) trait).getFields();
                  if (distributionFieldList.size() != 1) {
                    continue;
                  }
                  int fieldId = distributionFieldList.get(0).getFieldId();
                  String fieldName =
                      ((WriterPrel) other).getInput().getRowType().getFieldNames().get(fieldId);
  ...
  public void testWriterCommitterInPlan() throws Exception {
    String query =
        String.format(
            "create table %s.\"%s\" as select * from cp.acceleration.\"employees.json\"",
            DFS_TEST_PLUGIN_NAME, "xyz");
    final RelNode physical = getPlan(query);
          ...
          public RelNode visit(final RelNode other) {
            if (other instanceof WriterCommitterPrel) {
              ...
              if (haveWriterCommiterAvailable) {
                ...
                Assert.fail(
                    "WriterPrel must have a ancestor of WriterCommitterPrel. \n"
  ...
  private RelNode getPlan(final String queryString) {
    ...
    final PlanTransformationListener planTransformationListener =
        new PlanTransformationListener() {
          @Override
    ...
    JobsServiceTestUtils.submitJobAndWaitUntilCompletion(
        l(LocalJobsService.class), request, planTransformationListener);
    return physical.get();
  }
}
