...
public class TestReflectionJob extends BaseTestReflection {
  ...
  private static final String DATA1 = "{ \"key\" : \"A\", \"value\" : 0 }\n" +
    "{ \"key\" : \"A\", \"value\" : 1 }\n" +
    "{ \"key\" : \"B\", \"value\" : 2 }\n" +
    "{ \"key\" : \"B\", \"value\" : 3 }\n" +
    "{ \"key\" : \"C\", \"value\" : 0 }\n" +
  ...
  private java.io.File createDataFile(String data) throws Exception {
    final File file = temp.newFile(String.format("my_dataset_%d_%d.json", DATA_ID.getAndIncrement(), System.currentTimeMillis()));
  ...
  private ReflectionId createAgg() throws Exception {
    ...
    return getReflectionService().create(new ReflectionGoal()
      ...
      .setName("test-agg")
      .setDetails(new ReflectionDetails()
        .setDimensionFieldList(Collections.singletonList(new ReflectionDimensionField("key")))
        .setMeasureFieldList(ImmutableList.of(new ReflectionMeasureField("value").setMeasureTypeList(ImmutableList.of(MeasureType.COUNT))))
      )
    );
  }
  ...
  private ReflectionEntry validateAndGetReflectionEntry(ReflectionId id) throws Exception {
    ...
    assertTrue("raw reflection not found",!isEmpty(refls) && Iterables.any(refls, new Predicate<ReflectionGoal>() {
      @Override
    ...
    final DatasetConfig dataset = getNamespaceService().getDataset(datasetKey);
    assertTrue("no reflection found for dataset",
  ...
  private JobDetails getJobDetails(JobId jobId, String reflectionId, String userName) throws Exception {
    JobDetailsRequest jobDetailsRequest = JobDetailsRequest.newBuilder().setJobId(JobsProtoUtil.toBuf(jobId))
      .setFromStore(true)
      .setUserName(userName).build();
    ReflectionJobDetailsRequest.Builder builder = ReflectionJobDetailsRequest.newBuilder()
  ...
  private Iterable<JobSummary> searchJobs(String reflectionId, String userName) {
    SearchReflectionJobsRequest.Builder builder1 = SearchReflectionJobsRequest.newBuilder()
      .setLimit(100).setOffset(0).setReflectionId(reflectionId).setUserName(userName);
    return getJobsService().searchReflectionJobs(builder1.build());
  }

  private UserBitShared.QueryProfile getQueryProfile(JobId jobId, String reflectionId, String userName) throws Exception {
    QueryProfileRequest queryProfileRequest = QueryProfileRequest.newBuilder().setAttempt(0)
      .setJobId(JobsProtoUtil.toBuf(jobId))
      .setUserName(userName)
      .build();

    ReflectionJobProfileRequest reflectionJobProfileRequest = ReflectionJobProfileRequest.newBuilder()
  ...
  public void testJoProfileWithAdminUser() throws Exception {
    ...
    UserBitShared.QueryProfile queryProfile = getQueryProfile(reflectionEntry.getRefreshJobId(), reflectionId.getId(), DEFAULT_USERNAME);
  ...
  public void testSocketListenProgress() throws Exception {
    ...
    List<SocketMessage.Payload> payloads = socket.awaitCompletion(2, 10);
    SocketMessage.JobProgressUpdate progressUpdate = (SocketMessage.JobProgressUpdate) payloads.get(payloads.size() - 1);
  ...
  public void testDetailsAPI() throws Exception {
    ...
    expectSuccess(getBuilder(getAPIv2().path(JOB_PATH).path(reflectionEntry.getRefreshJobId().getId())
      .path("reflection")
      .path(reflectionId.getId())
      .path("details")).buildGet());
  }
  ...
  public void testDetailsAPINeg() throws Exception {
    ...
    expectError(FamilyExpectation.CLIENT_ERROR, getBuilder(getAPIv2().path(JOB_PATH).path(reflectionEntry.getRefreshJobId().getId())
      .path("reflection")
      .path(reflectionId1.getId())
      .path("details")).buildGet(), GenericErrorMessage.class);
  }
  ...
  public void testSearchJobsAPI() throws Exception {
    ...
    expectSuccess(getBuilder(getAPIv2().path("/jobs/")
      .path("reflection")
      .path(reflectionId.getId()))
      .buildGet());
  }
  ...
  public void testProfilesAPI() throws Exception {
    ...
    expectSuccess(getBuilder(getAPIv2().path("/profiles/")
      .path("reflection")
      .path(reflectionId.getId())
  ...
  public void testProfilesAPINeg() throws Exception {
    ...
    expectError(FamilyExpectation.CLIENT_ERROR, getBuilder(getAPIv2().path("/profiles/")
      .path("reflection")
      .path(reflectionId1.getId())
      .path(reflectionEntry.getRefreshJobId().getId() + ".json")
      .queryParam("attempt", "0")).buildGet(),
