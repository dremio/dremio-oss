...
/**
 * Tests used to search the space of valid queries given a certain prefix. This helps us determine
 * which partial queries have a identifier completion and thus need intellisense support.
 */
public final class PrefixedTokensSearchTests {
  private static final DremioToken IDENTIFIER_TOKEN =
      new DremioToken(ParserImplConstants.IDENTIFIER, "myIdentifier");

  /**
   * This test documents what are the possible top level statements and what work will need to be
   * done to add intellisense support for them.
   */
  @Test
  ...
  public static Output executeTest(Input input) {
    ...
    ImmutableList<DremioToken> prefix =
        ImmutableList.copyOf(SqlQueryTokenizer.tokenize(seedPrefix));
    List<List<DremioToken>> suffixes = new ArrayList<>();
    scanInputTree(suffixes, prefix, ImmutableList.of(), maxDepth);
  ...
  private static void scanInputTree(
    ...
    ImmutableList<DremioToken> query =
        ImmutableList.<DremioToken>builder().addAll(prefix).addAll(suffix).build();
    ...
    if (predictions.isIdentifierPossible()) {
      ImmutableList<DremioToken> extendedSuffix =
          new ImmutableList.Builder<DremioToken>().addAll(suffix).add(IDENTIFIER_TOKEN).build();
      scanInputTree(suffixes, prefix, extendedSuffix, maxDepth);
    }

    for (Integer keywordKind : predictions.getKeywords()) {
      if (keywordKind != ParserImplConstants.SEMICOLON) {
        ImmutableList<DremioToken> extendedSuffix =
            new ImmutableList.Builder<DremioToken>()
                .addAll(suffix)
