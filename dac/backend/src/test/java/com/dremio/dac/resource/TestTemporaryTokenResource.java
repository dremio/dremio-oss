...
public class TestTemporaryTokenResource extends BaseTestServer {
  ...
  public void testTempTokenValidation() {
        ...
        getBuilder(
                getAPIv2()
                    ...
                    .queryParam(
                        "request",
                        String.format(
                            "/apiv2/testjob/%s/download/?downloadFormat=%s",
                            jobId.getId(), "JSON")))
            .buildPost(null)
            .invoke();
    ...
    response = invocation.invoke();
    assertEquals(
        "Temporary access resource should fail with regular token in query param",
        401,
        response.getStatus());
        ...
        getBuilder(
                getAPIv2()
                    ...
                    .queryParam(
                        "request",
                        String.format(
                            "/apiv2/testjob/%s/download/?downloadFormat=%s&a=1&b=2&a=3",
                            jobId.getId(), "JSON")))
            .buildPost(null)
            .invoke();
        ...
        getBuilder(
                getAPIv2()
                    ...
                    .queryParam(
                        "request",
                        String.format(
                            "/apiv2/testjob/%s/download?downloadFormat=%s", jobId.getId(), "JSON")))
            .buildPost(null)
            .invoke();
    final String token = tokenResponse.readEntity(TempTokenResponse.class).getToken();
    ...
    final Response tokenResponse2 =
        getBuilder(
                getAPIv2()
                    ...
                    .queryParam("durationSeconds", 30)
                    .queryParam(
                        "request", String.format("/apiv2/testjob/%s/download", jobId.getId())))
            .buildPost(null)
            .invoke();
    final String token2 = tokenResponse2.readEntity(TempTokenResponse.class).getToken();
  ...
  public void testRegularAPIUsingTempToken() {
    ...
    Response response = invocation.invoke();
    assertEquals(
        "Not allowed to use a temporary token for a non temporary access method.",
        401,
        response.getStatus());
    ...
    response = invocation.invoke();
    assertEquals(
        "Not allowed to provide a regular token as a query param for a non temporary access method or resource.",
        401,
        response.getStatus());
