...
public class TestSourceResource extends BaseTestServer {
  ...
  public void testSourceHasDefaultRefreshPolicy() throws Exception {
    ...
    assertEquals(
        CatalogService.DEFAULT_AUTHTTLS_MILLIS, result.getMetadataPolicy().getAuthTTLMillis());
    assertEquals(
        CatalogService.DEFAULT_REFRESH_MILLIS, result.getMetadataPolicy().getNamesRefreshMillis());
    assertEquals(
        CatalogService.DEFAULT_REFRESH_MILLIS,
        result.getMetadataPolicy().getDatasetDefinitionRefreshAfterMillis());
    assertEquals(
