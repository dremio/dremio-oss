...
public class TestQueryParser extends BaseTestServer {
  ...
  public void testFields() {
    validateAncestors(
        "select s_suppkey, s_name, s_address from " + table, "cp.tpch/supplier.parquet");
  }

  @Test
  public void testFieldsAs() {
    validateAncestors(
        "select s_suppkey, s_name as mycol from " + table, "cp.tpch/supplier.parquet");
  }

  @Test
  public void testFlatten() {
    validateAncestors(
        "select flatten(b), a as mycol from cp.\"json/nested.json\"", "cp.json/nested.json");
  }
  ...
  public void testMultipleOrder() {
    validateAncestors(
        "select a, b from cp.\"json/nested.json\" order by b desc, a asc", "cp.json/nested.json");
  }

  @Test
  public void testOrderAlias() {
    validateAncestors(
        "select a as b from cp.\"json/nested.json\" order by b", "cp.json/nested.json");
  }

  @Test
  public void testJoin() {
    validateAncestors(
        "select foo.a from cp.\"json/nested.json\" foo, \"cp\".\"tpch/supplier.parquet\" bar where foo.a = bar.s_suppkey",
        "cp.json/nested.json",
        "cp.tpch/supplier.parquet");
    validateAncestors(
        "select foo.a from cp.\"json/nested.json\" foo, \"cp\".\"tpch/supplier.parquet\" bar, \"cp\".\"tpch/customer.parquet\" baz where foo.a = bar.s_suppkey and bar.s_suppkey = baz.c_custkey",
        "cp.json/nested.json",
        "cp.tpch/supplier.parquet",
  ...
  public void testsubQuery() {
    validateAncestors(
        "select foo.a from (select * from cp.\"json/nested.json\") foo, \"cp\".\"tpch/supplier.parquet\" bar where foo.a = bar.s_suppkey",
        "cp.json/nested.json",
        "cp.tpch/supplier.parquet");
  }
  ...
  public void testUnicodeCharacters() {
    // Unicode Equality
    validateAncestors(
        "select * from " + table + " where s_name = 'インターコンチネンタル'", "cp.tpch/supplier.parquet");

    // Unicode LIKE
    validateAncestors(
        "select * from " + table + " where s_name LIKE 'インターコンチネンタル'", "cp.tpch/supplier.parquet");
    ...
    String[] unicodeStrings =
        new String[] {
          ...
          // Strings which contain "corrupted" text. The corruption will not appear in non-HTML
    ...
    for (String unicodeString : unicodeStrings) {
      validateAncestors(
          "select * from " + table + " where s_name = '" + unicodeString + "'",
          "cp.tpch/supplier.parquet");
    }
  }
}
