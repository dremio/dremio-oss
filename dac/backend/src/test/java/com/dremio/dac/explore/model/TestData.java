...
/**
 * Unittests for {@link com.dremio.dac.model.job.JobDataFragmentWrapper}. Currently it only tests
 * the serialization aspects such as
 *
 * <ul>
 *   <li>able to serialize all types of ValueVectors
 *   <li>truncate large cell values
 *   <li>providing URL to fetch the complete cell value for truncated cell values in results
 * </ul>
 */
...
public class TestData extends DremioTest {
  ...
  public TestData(Boolean convertNumbersToStrings) {
    this.convertNumbersToStrings =
        convertNumbersToStrings == null ? false : convertNumbersToStrings.booleanValue();
    if (convertNumbersToStrings == null) {
      ...
      OBJECT_MAPPER.setConfig(
          OBJECT_MAPPER
              .getSerializationConfig()
              .withAttribute(
                  DataJsonOutput.DREMIO_JOB_DATA_NUMBERS_AS_STRINGS_ATTRIBUTE,
                  convertNumbersToStrings.booleanValue()));
    }
  }
  ...
  public void serializeAllTypesAtOnce() throws Exception {
    ...
    List<ValueVector> vvs =
        Lists.newArrayList(
            Iterables.transform(
                pairs,
                new Function<Pair<? extends ValueVector, ResultVerifier>, ValueVector>() {
                  @Nullable
                  @Override
                  public ValueVector apply(
                      @Nullable Pair<? extends ValueVector, ResultVerifier> input) {
                    return input.getKey();
                  }
                }));

    try (com.dremio.dac.model.job.JobDataFragment dataInput =
        createDataObject(vvs.toArray(new ValueVector[pairs.size()]))) {
      DataPOJO dataOutput =
          OBJECT_MAPPER.readValue(OBJECT_MAPPER.writeValueAsString(dataInput), DataPOJO.class);
  ...
  private static void helper(
      Pair<? extends ValueVector, ResultVerifier> testPair,
      String colName,
      boolean potentialTruncation,
      boolean isUnion)
      throws Exception {
  ...
  private static com.dremio.dac.model.job.JobDataFragment createDataObject(ValueVector... vv) {
    ...
    return new JobDataFragmentWrapper(
        0,
        new JobDataFragmentImpl(
            new RecordBatches(asList(newRecordBatchHolder(batch, 0, batch.getRecordCount()))),
            0,
            TEST_JOB_ID,
            TEST_SESSION_ID));
  }
  ...
  private static Pair<BitVector, ResultVerifier> testBitVector(
      final int startIndexInCurrentOutput, final int startIndexInJob) {
  ...
  private <T extends Number> void verifyIntValues(List<T> values, DataPOJO output, String colName) {
    for (int i = 0; i < values.size(); i++) {
      ...
      if (value == null) {
        ...
        // always compare as strings, as there could be type mismatches. For example, if we send
        // (byte)0 or 0L, returned
  ...
  private <T extends Number> void verifyDoubleValues(
      List<T> values, DataPOJO output, String colName, double accuracy) {
    for (int i = 0; i < values.size(); i++) {
      T value = values.get(i);
  ...
  private <T extends Number> void verifyDoubleValue(
      T expectedValue, DataPOJO output, String colName, int index, double accuracy) {
  ...
  private Pair<Float8Vector, ResultVerifier> testFloat8Vector() {
    ...
    final List<Double> values =
        asList(20.2345234d, 503453.023d, -238423.3453453d, 3273452.345324563245d, null);
  ...
  private static Pair<DateMilliVector, ResultVerifier> testDateMilliVector(
      final int startIndexInCurrentOutput, final int startIndexInJob) {
  ...
  private static Pair<TimeMilliVector, ResultVerifier> testTimeVector(
      final int startIndexInCurrentOutput, final int startIndexInJob) {
  ...
  private static Pair<TimeStampMilliVector, ResultVerifier> testTimeStampVector(
      final int startIndexInCurrentOutput, final int startIndexInJob) {
    ...
    ResultVerifier verifier =
        new ResultVerifier() {
          ...
          public void verify(DataPOJO output) {
            ...
            assertEquals(
                "7426303-09-23 10:54:54.234", output.extractValue("colTimeStamp", index++));
          }
        };
  ...
  private static Pair<IntervalYearVector, ResultVerifier> testIntervalYearVector(
      final int startIndexInCurrentOutput, final int startIndexInJob) {
  ...
  private static Pair<IntervalDayVector, ResultVerifier> testIntervalDayVector(
      final int startIndexInCurrentOutput, final int startIndexInJob) {
  ...
  private Pair<DecimalVector, ResultVerifier> testDecimalVector() {
    ...
    final List<BigDecimal> values =
        asList(
            ...
            .map(
                decimalString ->
                    decimalString == null ? null : new BigDecimal(decimalString).setScale(scale))
    ...
    for (int i = 0; i < values.size(); i++) {
      if (values.get(i) == null) {
        ...
        DecimalUtility.writeBigDecimalToArrowBuf(
            values.get(i), tempBuf, 0, DecimalVector.TYPE_WIDTH);
            ...
            for (int i = 0; i < values.size(); i++) {
              ...
              if (value == null) {
                ...
                if (convertNumbersToStrings) {
                  ...
                  assertEquals(
                      value.doubleValue(),
                      ((Double) output.extractValue(colName, i)).doubleValue(),
                      0.000f);
                }
              }
            }
  ...
  private static Pair<VarCharVector, ResultVerifier> testVarCharVector(
      final int startIndexInCurrentOutput, final int startIndexInJob) {
    ...
    ResultVerifier verifier =
        new ResultVerifier() {
          ...
          public void verify(DataPOJO output) {
            ...
            assertEquals(
                "long long long long long long ", output.extractValue("colVarChar", index));
  ...
  private static Pair<VarBinaryVector, ResultVerifier> testVarBinaryVector(
      final int startIndexInCurrentOutput, final int startIndexInJob) {
    ...
    colVarBinaryV.set(
        1,
        "long long long long long long long long long long long long long long long value"
            .getBytes());
    ...
    ResultVerifier verifier =
        new ResultVerifier() {
          ...
          public void verify(DataPOJO output) {
            ...
            assertEquals(
                "bG9uZyBsb25nIGxvbmcgbG9uZyBsb25nIGxvbmcg",
                output.extractValue("colVarBinary", index));
            assertEquals(
                cellUrl(uIndex++, "colVarBinary"), output.extractUrl("colVarBinary", index++));

            assertEquals(
                "bG9uZyBsb25nIGxvbmcgbG9uZyB2YWx1ZQ==", output.extractValue("colVarBinary", index));
            assertNull(output.extractUrl("colVarBinary", index++));
            uIndex++;
  ...
  private static Pair<NonNullableStructVector, ResultVerifier> testMapVector(
      final int startIndexInCurrentOutput, final int startIndexInJob) {
    NonNullableStructVector colStructV =
        new NonNullableStructVector(
            "colMap", allocator, new FieldType(false, ArrowType.Struct.INSTANCE, null), null);
    ...
    ResultVerifier verifier =
        new ResultVerifier() {
          ...
          public void verify(DataPOJO output) {
            ...
            int uIndex = startIndexInJob;
            assertEquals(
                "{colMap={nBigIntCol=23, nVarCharCol=value, nListCol=[1970-01-01, 1970-01-03, 1969-12-31]}}",
                output.extractValue("colMap", index).toString());
            assertEquals(cellUrl(uIndex++, "colMap"), output.extractUrl("colMap", index++));

            assertEquals(
                "{colMap={nBigIntCol=223, nVarCharCol=long long value, nListCol=[1969-12-29]}}",
                output.extractValue("colMap", index).toString());
            assertEquals(cellUrl(uIndex++, "colMap"), output.extractUrl("colMap", index++));

            assertEquals(
                "{colMap={nBigIntCol=54645, nUnionCol=null, nMapCol={a=1}}}",
                output.extractValue("colMap", index).toString());
            ...
            assertEquals(
                "{colMap={nBigIntCol=234543, nUnionCol=null}}",
                output.extractValue("colMap", index).toString());
            assertNull(output.extractUrl("colMap", index++));
          }
        };
  ...
  private static Pair<ListVector, ResultVerifier> testListVector(
      final int startIndexInCurrentOutput, final int startIndexInJob) {
    ListVector colListV =
        new ListVector("colList", allocator, FieldType.nullable(ArrowType.List.INSTANCE), null);
    ...
    ResultVerifier verifier =
        new ResultVerifier() {
          ...
          public void verify(DataPOJO output) {
            ...
            int uIndex = startIndexInJob;
            assertEquals(
                "[item 0-0, item 0-1, item 0-2, item 0]",
                output.extractValue("colList", index).toString());
            assertEquals(cellUrl(uIndex++, "colList"), output.extractUrl("colList", index++));

            assertEquals(
                "[item 1-0, item 1-1, item 1-2, item 1]",
                output.extractValue("colList", index).toString());
            assertEquals(cellUrl(uIndex++, "colList"), output.extractUrl("colList", index++));

            assertEquals(
                "[item 2-0, item 2-1, item 2-2, item 2]",
                output.extractValue("colList", index).toString());
            assertEquals(cellUrl(uIndex++, "colList"), output.extractUrl("colList", index++));

            assertEquals(
                "[item 3-0, item 3-1, item 3-2, item 3]",
                output.extractValue("colList", index).toString());
            assertEquals(cellUrl(uIndex++, "colList"), output.extractUrl("colList", index++));

            assertEquals(
                "[item 4-0, item 4-1, item 4-2, item 4]",
                output.extractValue("colList", index).toString());
            assertEquals(cellUrl(uIndex++, "colList"), output.extractUrl("colList", index));
          }
        };
  ...
  private Pair<UnionVector, ResultVerifier> testUnionVector(
      final int startIndexInCurrentOutput, final int startIndexInJob) {
  ...
  public void testDataSerialization() throws Exception {
    ...
    try (JobDataFragment dataInput =
        new JobDataFragmentWrapper(
            0,
            new JobDataFragmentImpl(
                new RecordBatches(
                    asList(
    ...
    try (JobDataFragment jdf =
      ...
      try (JobDataFragment jdf2 =
        ...
        value =
            jdf2.extractString(
                "colVarChar",
                3); // will not fit first batch - will be 1st element of the 2nd batch which element
        // 1
        assertEquals(
            "long long long long long long long long long long long long long long long long value",
            value);
