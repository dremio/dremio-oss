...
public class ITTestVersionedViews extends ITBaseTestVersioned {
  ...
  public void testSaveAsInSpaceWithUnspecifiedVersion() throws Exception {
    ...
    UserExceptionMapper.ErrorMessageWithContext errorMessage = createViewInSpaceFromQueryExpectError(viewQuery, DEFAULT_SPACE_NAME, newViewName);
    assertContains("Validation of view sql failed. Version context for table "+ fullyQualifiedTableName(DATAPLANE_PLUGIN_NAME,tablePath)+" must be specified using AT SQL syntax", errorMessage.getErrorMessage());
  }
  ...
  public void testSaveAsInSpaceWithUnspecifiedVersionInJoin() throws Exception {
    ...
    createFolders(table1Path, VersionContext.ofBranch(DEFAULT_BRANCH_NAME), null);
    runQuery(createEmptyTableQuery(table1Path));
    runQuery(insertTableQuery(table1Path));

    createFolders(table2Path, VersionContext.ofBranch(DEFAULT_BRANCH_NAME), null);
    runQuery(createEmptyTableQuery(table2Path));
    runQuery(insertTableQuery(table2Path));

    String viewQuery = String.format("Select * from %s.%s AT BRANCH \"main\" JOIN %s.%s  ON TRUE" ,
      DATAPLANE_PLUGIN_NAME, joinedTableKey(table1Path),
      DATAPLANE_PLUGIN_NAME, joinedTableKey(table2Path));

    UserExceptionMapper.ErrorMessageWithContext errorMessage = createViewInSpaceFromQueryExpectError(viewQuery, DEFAULT_SPACE_NAME, newViewName);
    assertContains("Validation of view sql failed. Version context for table "+ fullyQualifiedTableName(DATAPLANE_PLUGIN_NAME,table2Path)+" must be specified using AT SQL syntax", errorMessage.getErrorMessage());
  }
  ...
  public void testCreateViewFromPublicAPIWithUnspecifiedVersionInJoin() throws Exception {
    ...
    com.dremio.dac.api.Dataset dataset = new com.dremio.dac.api.Dataset(
      null,
      VIRTUAL_DATASET,
    ...
    UserExceptionMapper.ErrorMessageWithContext errorMessage = createViewFromPublicAPIExpectError(dataset);
    assertContains("Validation of view sql failed. Version context for table "+ fullyQualifiedTableName(DATAPLANE_PLUGIN_NAME,table2Path)+" must be specified using AT SQL syntax", errorMessage.getErrorMessage());
  }
  ...
  public void testUpdateViewFromPublicAPIWithUnspecifiedVersionInJoin() throws Exception {
    ...
    final String viewName = generateUniqueViewName();
    String viewQuery = String.format("Select * from %s.%s AT BRANCH \"main\"" , DATAPLANE_PLUGIN_NAME, joinedTableKey(table1Path));
    ...
    String updatedViewQuery = String.format("Select * from %s.%s AT BRANCH \"main\" JOIN %s.%s  ON TRUE" ,
      DATAPLANE_PLUGIN_NAME, joinedTableKey(table1Path),
      DATAPLANE_PLUGIN_NAME, joinedTableKey(table2Path));

    com.dremio.dac.api.Dataset updatedDataset = new com.dremio.dac.api.Dataset(
      catalogEntity.getId(),
      VIRTUAL_DATASET,
      ImmutableList.of(DATAPLANE_PLUGIN_NAME_FOR_REFLECTION_TEST, viewName),
      null,
    ...
    UserExceptionMapper.ErrorMessageWithContext errorMessage = updateViewFromPublicAPIExpectError(updatedDataset);
    assertContains("Validation of view sql failed. Version context for table "+ fullyQualifiedTableName(DATAPLANE_PLUGIN_NAME,table2Path)+" must be specified using AT SQL syntax", errorMessage.getErrorMessage());
  }
  ...
  protected void createSpace(String name) {
    expectSuccess(getBuilder(getPublicAPI(3).path("/catalog/")).buildPost(Entity.json(new com.dremio.dac.api.Space(null, name, null, null, null))), new GenericType<Space>() {});
  }

  protected void dropSpace(String name) {
    com.dremio.dac.api.Space s = expectSuccess(getBuilder(getPublicAPI(3).path("/catalog/").path("by-path").path(name)).buildGet(), new GenericType<com.dremio.dac.api.Space>() {});
  ...
  protected UserExceptionMapper.ErrorMessageWithContext createViewInSpaceFromQueryExpectError(String query, String space, String viewName) {
    final DatasetPath datasetPath = new DatasetPath(ImmutableList.of(space, viewName));
    return createDatasetFromSQLAndSaveExpectError(datasetPath, query, Collections.emptyList());
  }

  private UserExceptionMapper.ErrorMessageWithContext createViewFromPublicAPIExpectError(com.dremio.dac.api.Dataset dataset) {
  ...
  private UserExceptionMapper.ErrorMessageWithContext updateViewFromPublicAPIExpectError(com.dremio.dac.api.Dataset dataset) {
    try {
      final Invocation invocation =
        getBuilder(
          getPublicAPI(3)
