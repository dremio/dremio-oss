...
import static com.dremio.service.namespace.source.proto.SourceType.CLASSPATH;
...
/**
 * generates a StoragePluginConfig for ClassPath Sources
 *
 */
public class ClassPathSourceConfigurator extends SingleSourceToStoragePluginConfig<ClassPathConfig> {

  public ClassPathSourceConfigurator() {
    super(CLASSPATH);
  }
  ...
  @Override
  public StoragePluginConfig configureSingle(ClassPathConfig cp) {
    String path = checkNotNull(cp.getPath(), "missing path");
    String connection = "classpath:///";
    FileSystemConfig config = new FileSystemConfig(connection, path, null, getDefaultFormats(),
        /*impersonationEnabled=*/false /* impersonation is not supported in ClassPath */,
