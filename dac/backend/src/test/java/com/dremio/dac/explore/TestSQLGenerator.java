...
public class TestSQLGenerator {
  ...
  public void testGenSQL() {
    ...
    VirtualDatasetState state = new VirtualDatasetState().setFrom(nameDSRef);
  ...
  public void testStar() {
    ...
    validate("SELECT *\nFROM myspace.parentDS", new VirtualDatasetState().setFrom(nameDSRef));

    validate(sqlDSRef.getSql().getSql(), new VirtualDatasetState().setFrom(sqlDSRef));
  }
  ...
  public void selectConstant() {
    ...
    validate(query, new VirtualDatasetState().setFrom(sqlDSRef));
  ...
  public void testJoin() {
    ...
    validateForTranformRelatedCalls(
        ...
        state.setJoinsList(
            asList(
                new Join(JoinType.Inner, "\"space\".foo", "join_1")
  ...
  public void testNumberToDate() {
    ...
    validate(
        "SELECT TO_TIMESTAMP((bar - 25569) * 86400) AS foo\n" + "FROM myspace.parentDS",
        state.setColumnsList(asList(new Column("foo", exp1))));

    FieldTransformationBase transf2 =
        new FieldConvertNumberToDate().setDesiredType(DATE).setFormat(JULIAN);
    Expression exp2 = new ExpFieldTransformation(transf2.wrap(), exp0).wrap();
    validate(
        "SELECT TO_DATE((bar - 2440587.5) * 86400) AS foo\n" + "FROM myspace.parentDS",
        state.setColumnsList(asList(new Column("foo", exp2))));

    FieldTransformationBase transf3 =
        new FieldConvertNumberToDate().setDesiredType(TIME).setFormat(EPOCH);
    Expression exp3 = new ExpFieldTransformation(transf3.wrap(), exp0).wrap();
    validate(
        "SELECT TO_TIME(bar) AS foo\n" + "FROM myspace.parentDS",
        state.setColumnsList(asList(new Column("foo", exp3))));
  }
  ...
  public void testFromJSON() {
    ...
    Expression exp1 = new ExpFieldTransformation(transf1.wrap(), exp0).wrap();
    validate(
        "SELECT convert_from(bar, 'JSON') AS foo\n" + "FROM myspace.parentDS",
        state.setColumnsList(asList(new Column("foo", exp1))));
  }
  ...
  public void testTableNameEnquoting() {
    VirtualDatasetState state =
        new VirtualDatasetState().setFrom(new FromTable("myhome.tables.1234.test").wrap());
  ...
  public void testSubQuery() {
    VirtualDatasetState state =
        new VirtualDatasetState().setFrom(new FromSQL("SELECT * FROM cp.\"region.json\"").wrap());
  ...
  public void testMultipleOrderBys() {
    VirtualDatasetState state =
        new VirtualDatasetState()
            .setFrom(new FromTable("myhome.tables.1234.test").wrap())
            // home.dir contains a special identifier character, make sure it is escaped in
            // generated SQL
            .setOrdersList(
                asList(
                    new Order("home.dir", OrderDirection.DESC),
                    new Order("join_date", OrderDirection.ASC)));
    validateForTranformRelatedCalls(
        "SELECT *\nFROM myhome.\"tables\".\"1234\".test\nORDER BY \"home.dir\" DESC, join_date ASC",
        state);
  }
  ...
  public void testReplaceInvalidReplacedValues() {
    ...
    try {
      ...
      SQLGenerator.generateSQL(
          state.setColumnsList(asList(new Column("foo", exp))), true, catalogService);
  ...
  public void testFilterSummed() {
    ...
    String expSumQuery = "SELECT a, SUM(b) AS Sum_b\n" + "FROM myspace.parentDS\n" + "GROUP BY a";
                ...
                asList(
                    new Filter(
                            ...
                            new FilterDefinition(FilterType.Value)
                                .setValue(
                                    new FilterValue(DataType.DATE)
                                        .setValuesList(asList(null, "foo", "bar"))))
                        .setExclude(true))));
                ...
                asList(
                    new Filter(
                            ...
                            new FilterDefinition(FilterType.Value)
                                .setValue(
                                    new FilterValue(DataType.DATE)
                                        .setValuesList(asList((String) null))))
                        .setExclude(true))));
  ...
  public void testJoinSelectionCollision() {
    VirtualDatasetState state =
        new VirtualDatasetState().setFrom(new FromTable("\"space\".\"t1\"").setAlias("t1").wrap());
    ...
    validateForTranformRelatedCalls(
        ...
        state
            .setJoinsList(asList(makeJoin(JoinType.Inner, "\"space\".t2", "t2", "id")))
            .setColumnsList(
                asList(
  ...
  private static class ColumnSpec {
    ...
    public ColumnSpec(
        String table, String colName, String desiredColAlias, String expectedColAlias) {
      this.table = table;
  ...
  private void validateJoinColumnCollisionTest(
      String t1, String t2, String t3, List<ColumnSpec> selections) {
    VirtualDatasetState state =
        new VirtualDatasetState()
  ...
  public void testColumnCollisionDoesntAlterAliasBaseWhenItEndsInNumbers() {
    validateJoinColumnCollisionTest(
        "t1",
        "t2",
        "t3",
        asList(
