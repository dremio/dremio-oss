...
public class ITNessieSourceApi extends BaseTestServerJunit5 {
  ...
  private static String createNessieURIString() {
    String baseUrl = Preconditions.checkNotNull(System.getProperty("nessie.server.url"),
  ...
  class NessieRestApi extends BaseTestNessieRest {
    ...
    @Disabled // Disabled because NaaS proxy only proxies /trees endpoints, so /config is not available
    ...
    private static final class ClientFactory implements NessieClientFactory, Serializable {
      ...
      public NessieApiV2 make(NessieClientCustomizer customizer) {
        return HttpClientBuilder
          .builder()
          .withUri(nessieUri)
          .withApiCompatibilityCheck(false) // Nessie API proxy in the source does not have the /config endpoint
          .build(NessieApiV2.class);
      }
    }
  }
