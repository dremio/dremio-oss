...
public class TestUserStatsResource {
  ...
  public void testActiveUserStats() {
    ...
    testJobResults.add(newJob("testuser1", QueryType.ODBC, DateUtils.getLastSundayDate(now))); // repeat user should get ignored
    ...
    testJobResults.add(newJob("testuser1", QueryType.ODBC, DateUtils.getMonthStartDate(now))); // repeat user should get ignored
    ...
    testJobResults.add(newJob("testuser1", QueryType.ODBC, DateUtils.getMonthStartDate(now.minusMonths(1))));
    testJobResults.add(newJob("testuser1", QueryType.ODBC, DateUtils.getMonthStartDate(now.minusMonths(1)))); // repeat user should get ignored
    testJobResults.add(newJob("testuser1", QueryType.UI_PREVIEW, DateUtils.getMonthStartDate(now.minusMonths(1))));
    testJobResults.add(newJob("testuser2", QueryType.ODBC, DateUtils.getMonthStartDate(now.minusMonths(1))));
    ...
    Map<String, Object> secondDateEntry = fetchDateEntry("date", DateUtils.getLastSundayDate(now).toString(), statsByDate);
    ...
    Map<String, Object> weekDateEntry = fetchDateEntry("week", DateUtils.getLastSundayDate(now).toString(), stats.getUserStatsByWeek());
    assertEquals(1, weekDateEntry.get("UI_PREVIEW"));
    assertEquals(2, weekDateEntry.get("ODBC"));
    assertEquals(2, weekDateEntry.get("total"));

    Map<String, Object> monthDateEntry = fetchDateEntry("month", DateUtils.getMonthStartDate(now).toString(), stats.getUserStatsByMonth());
    ...
    Map<String, Object> lastMonthEntry = fetchDateEntry("month", DateUtils.getMonthStartDate(now.minusMonths(1)).toString(), stats.getUserStatsByMonth());
  ...
  private JobSummary newJob(String user, QueryType queryType, LocalDate startDate) {
    return JobSummary.newBuilder().setUser(user).setQueryType(queryType).setStartTime(toEpochMillis(startDate)).build();
  }
}
