...
public class ITDataplanePluginMerge extends ITDataplanePluginTestSetup {
  ...
  public void mergeAll() throws Exception {
    ...
    long mtime1 = getMtimeForTable(targetTablePath, new TableVersionContext(TableVersionType.BRANCH, DEFAULT_BRANCH_NAME), this);
    // Add an extra row in the source
    runSQL(insertTableWithValuesQuery(sourceTablePath,
    ...
    long mtime2 = getMtimeForTable(targetTablePath, new TableVersionContext(TableVersionType.BRANCH, devBranch), this);
    // Assert
    ...
    runSQL(useBranchQuery(DEFAULT_BRANCH_NAME));
    long mtime3 = getMtimeForTable(targetTablePath, new TableVersionContext(TableVersionType.BRANCH, DEFAULT_BRANCH_NAME), this);
    // Assert
    assertTableHasExpectedNumRows(targetTablePath, 3);
    assertTableHasExpectedNumRows(sourceTablePath, 4);
  ...
  public void mergeWithTagSet() throws Exception {
    ...
    assertQueryThrowsExpectedError(mergeByIdQuery(targetTablePath, sourceTablePath),
  ...
  public void testDmlMergeWithAtTargetTable() throws Exception {
    ...
    assertThatThrownBy(() -> runSQL(
      String.format("MERGE INTO %s.%s AT BRANCH %s AS t USING %s.%s AS s ON (t.EXPR$0 = s.EXPR$0)\n" +
        "  WHEN MATCHED THEN UPDATE SET EXPR$1 = s.EXPR$2", DATAPLANE_PLUGIN_NAME, tableName, branchName, DATAPLANE_PLUGIN_NAME, tableName2)))
  ...
  public void testDmlMergeWithAtTargetTableNotMatched() throws Exception {
    ...
    assertThatThrownBy(() -> runSQL(
      String.format("MERGE INTO %s.%s AT BRANCH %s AS t USING %s.%s AS s ON (t.EXPR$0 = s.EXPR$0)\n" +
        "  WHEN NOT MATCHED THEN INSERT (EXPR$0, EXPR$1, EXPR$2) VALUES (9,9,9))", DATAPLANE_PLUGIN_NAME, tableName, branchName, DATAPLANE_PLUGIN_NAME, tableName2)))
      .isInstanceOf(UserRemoteException.class)
      .hasMessageContaining("When specifying the version of the table to be modified, you must also specify the version of the source table")
      .hasMessageContaining("using AT SQL syntax");
  }
  ...
  public void testDmlMergeWithAtWrongReferenceType() throws Exception {
    ...
    assertThatThrownBy(() -> runSQL(String.format("MERGE INTO %s.%s AT TAG %s AS t USING %s.%s AS s ON (t.EXPR$0 = s.EXPR$0)\n" +
      "  WHEN MATCHED THEN UPDATE SET EXPR$1 = s.EXPR$2", DATAPLANE_PLUGIN_NAME, tableName, tagName, DATAPLANE_PLUGIN_NAME, tableName)))
      .isInstanceOf(UserRemoteException.class);
  }
  ...
  public void testDmlMerge() throws Exception {
    ...
    runSQL(String.format("MERGE INTO %s AS t USING %s.%s AS s ON (t.EXPR$0 = s.EXPR$0)\n" +
      "  WHEN MATCHED THEN UPDATE SET EXPR$1 = s.EXPR$2", tableName, DATAPLANE_PLUGIN_NAME, tableName2));
  ...
  public void testDmlMergeWithContextWithDifferentAtStatement() throws Exception {
    ...
    assertThatThrownBy(() -> runSQL(
      String.format("MERGE INTO %s at branch %s AS t USING %s.%s AS s ON (t.EXPR$0 = s.EXPR$0)\n" +
        "  WHEN MATCHED THEN UPDATE SET EXPR$1 = s.EXPR$2", tableName, branchName, DATAPLANE_PLUGIN_NAME, tableName2)))
      .isInstanceOf(UserRemoteException.class)
      .hasMessageContaining("When specifying the version of the table to be modified, you must also specify the version of the source table")
      .hasMessageContaining("using AT SQL syntax");
  }
  ...
  public void testDmlMergeWithContextWithDifferentAtStatementToNonExistentTable() throws Exception {
    ...
    assertThatThrownBy(() -> runSQL(String.format("MERGE INTO %s.%s at branch %s AS t USING %s.%s AS s ON (t.EXPR$0 = s.EXPR$0)\n" +
      "  WHEN MATCHED THEN UPDATE SET EXPR$1 = s.EXPR$2", DATAPLANE_PLUGIN_NAME, tableName, DEFAULT_BRANCH_NAME ,DATAPLANE_PLUGIN_NAME, tableName)))
      .isInstanceOf(UserRemoteException.class);
  }
  ...
  public void testMergeDifferentVersionContext() throws Exception {
    ...
    runSQL(String.format("CREATE table %s.%s at branch %s as select 1,2,3",DATAPLANE_PLUGIN_NAME, mainTable, DEFAULT_BRANCH_NAME));

    runSQL(
      String.format("MERGE INTO %s.%s AT BRANCH %s as t using %s.%s at branch %s as s on t.EXPR$0 = s.EXPR$0\n" +
        "when matched then update set EXPR$0 = 888", DATAPLANE_PLUGIN_NAME, devTable, devBranch, DATAPLANE_PLUGIN_NAME, mainTable, DEFAULT_BRANCH_NAME)
    );
    Assertions.assertThat(runSqlWithResults(String.format("select * from %s.%s at branch %s",DATAPLANE_PLUGIN_NAME, mainTable, DEFAULT_BRANCH_NAME)))
      .isEqualTo(Collections.singletonList(Arrays.asList("1", "2", "3")));
    Assertions.assertThat(runSqlWithResults(String.format("select * from %s.%s at branch %s",DATAPLANE_PLUGIN_NAME, devTable, devBranch)))
  ...
  public void testMergeUseSelectWithAt() throws Exception {
    ...
    runSQL(
      String.format("MERGE INTO %s.%s at branch %s as t\n" +
        "USING (SELECT * FROM %s.%s at branch %s ) AS s\n" +
        "ON (t.EXPR$0 = s.EXPR$0) WHEN MATCHED THEN UPDATE SET EXPR$2 = 999"
      ,DATAPLANE_PLUGIN_NAME, devTable, devBranch, DATAPLANE_PLUGIN_NAME, mainTable, DEFAULT_BRANCH_NAME)
    );
    Assertions.assertThat(runSqlWithResults(String.format("select * from %s.%s at branch %s",DATAPLANE_PLUGIN_NAME, mainTable, DEFAULT_BRANCH_NAME)))
      .isEqualTo(Collections.singletonList(Arrays.asList("1", "2", "3")));
    Assertions.assertThat(runSqlWithResults(String.format("select * from %s.%s at branch %s",DATAPLANE_PLUGIN_NAME, devTable, devBranch)))
  ...
  public void testMergeUseSelectWithAtRef() throws Exception {
    ...
    runSQL(
      String.format("MERGE INTO %s.%s at REF %s as t\n" +
          "USING (SELECT * FROM %s.%s at branch %s ) AS s\n" +
          "ON (t.EXPR$0 = s.EXPR$0) WHEN MATCHED THEN UPDATE SET EXPR$2 = 999"
        ,DATAPLANE_PLUGIN_NAME, devTable, devBranch, DATAPLANE_PLUGIN_NAME, mainTable, DEFAULT_BRANCH_NAME)
    );
  ...
  public void testDmlMergeOnlySourceVersionSpecified() throws Exception {
    ...
    runSQL(
      String.format("MERGE INTO %s.%s as t using %s.%s at branch %s as s on t.EXPR$0 = s.EXPR$0\n" +
        "when matched then update set EXPR$0 = 12345", DATAPLANE_PLUGIN_NAME, mainTable, DATAPLANE_PLUGIN_NAME, devTable, devBranchName)
    );
  ...
  public void testDmlUpdateBothSourceVersionSpecified() throws Exception {
    ...
    runSQL(
      String.format("MERGE INTO %s.%s at branch %s as t using %s.%s at branch %s as s on t.EXPR$0 = s.EXPR$0\n" +
        "when matched then update set EXPR$0 = 321", DATAPLANE_PLUGIN_NAME, mainTable, DEFAULT_BRANCH_NAME, DATAPLANE_PLUGIN_NAME, devTable, devBranchName)
    );
    ...
    // update mainTable using source in dev
    runSQL(
      String.format("MERGE INTO %s.%s at branch %s as t using %s.%s at branch %s as s on t.EXPR$1 = s.EXPR$1\n" +
        "when matched then update set EXPR$1 = 54321", DATAPLANE_PLUGIN_NAME, mainTable, DEFAULT_BRANCH_NAME, DATAPLANE_PLUGIN_NAME, devTable, devBranchName)
    );
  ...
  public void testDmlMergeOnlyTargetVersionSpecified() throws Exception {
    ...
    assertThatThrownBy(() -> runSQL(
      String.format("MERGE INTO %s.%s at branch %s as t using %s.%s as s on t.EXPR$0 = s.EXPR$0\n" +
        "when matched then update set EXPR$0 = 12345", DATAPLANE_PLUGIN_NAME, devTable1, devBranchName, DATAPLANE_PLUGIN_NAME, devTable2)
    ))
      .isInstanceOf(UserRemoteException.class)
      .hasMessageContaining("When specifying the version of the table to be modified, you must also specify the version of the source table")
      .hasMessageContaining("using AT SQL syntax");
    ...
    assertThatThrownBy(() -> runSQL(
      String.format("MERGE INTO %s.%s at branch %s as t using %s.%s as s on t.EXPR$0 = s.EXPR$0\n" +
        "when matched then update set EXPR$0 = 12345", DATAPLANE_PLUGIN_NAME, devTable1, devBranchName, DATAPLANE_PLUGIN_NAME, mainTable)
      ))
      .isInstanceOf(UserRemoteException.class);
    ...
    // update devTable1 using source in dev
    assertThatThrownBy(() -> runSQL(
      String.format("MERGE INTO %s.%s at branch %s as t using %s.%s as s on t.EXPR$1 = s.EXPR$1\n" +
        "when matched then update set EXPR$1 = 67890", DATAPLANE_PLUGIN_NAME, devTable1, devBranchName, DATAPLANE_PLUGIN_NAME, devTable2)
    ))
      .isInstanceOf(UserRemoteException.class)
      .hasMessageContaining("When specifying the version of the table to be modified, you must also specify the version of the source table")
      .hasMessageContaining("using AT SQL syntax");
    ...
    assertThatThrownBy(() -> runSQL(
      String.format("MERGE INTO %s.%s at branch %s as t using %s.%s as s on t.EXPR$0 = s.EXPR$0\n" +
        "when matched then update set EXPR$1 = 67890", DATAPLANE_PLUGIN_NAME, devTable1, devBranchName, DATAPLANE_PLUGIN_NAME, mainTable)
    ))
      .isInstanceOf(UserRemoteException.class);
  }
}
