...
import static com.dremio.exec.catalog.dataplane.DataplaneTestDefines.alterTableAddColumnsQueryWithAtSyntax;
import static com.dremio.exec.catalog.dataplane.DataplaneTestDefines.alterTableAddPartitionQuery;
import static com.dremio.exec.catalog.dataplane.DataplaneTestDefines.alterTableAddPartitionQueryAt;
import static com.dremio.exec.catalog.dataplane.DataplaneTestDefines.alterTableAddPrimaryKeyQuery;
import static com.dremio.exec.catalog.dataplane.DataplaneTestDefines.alterTableAddPrimaryKeyQueryWithAtSyntax;
import static com.dremio.exec.catalog.dataplane.DataplaneTestDefines.alterTableChangeColumnQuery;
import static com.dremio.exec.catalog.dataplane.DataplaneTestDefines.alterTableChangeColumnQueryWithAtSyntax;
import static com.dremio.exec.catalog.dataplane.DataplaneTestDefines.alterTableDropColumnQuery;
import static com.dremio.exec.catalog.dataplane.DataplaneTestDefines.alterTableDropColumnQueryWithAtSyntax;
import static com.dremio.exec.catalog.dataplane.DataplaneTestDefines.alterTableDropPrimaryKeyQuery;
import static com.dremio.exec.catalog.dataplane.DataplaneTestDefines.alterTableDropPrimaryKeyQueryWithAtSyntax;
import static com.dremio.exec.catalog.dataplane.DataplaneTestDefines.alterTableReplaceSortOrder;
import static com.dremio.exec.catalog.dataplane.DataplaneTestDefines.alterTableSetTablePropertiesQuery;
import static com.dremio.exec.catalog.dataplane.DataplaneTestDefines.alterTableUnsetTablePropertiesQuery;
...
public class ITDataplanePluginAlter extends ITDataplanePluginTestSetup {
  ...
  void alterTableWithInvalidReference() throws Exception {
    ...
    final String query = String.format("ALTER TABLE %s.%s AT COMMIT %s add columns %s",
      DATAPLANE_PLUGIN_NAME,
      String.join(".", tablePath),
      quoted(commitHashTemporaryBranch),
      "("+ String.join(",", addedColDef) +")");

    assertQueryThrowsExpectedError(query, "DDL and DML operations are only supported for branches - not on tags or commits. DETACHED is not a branch. ");
  ...
  public void testAddAndDropPrimaryKeyWithAtSyntax() throws Exception {
    ...
    try {
      ...
      runSQL(alterTableAddPrimaryKeyQueryWithAtSyntax(tablePath, Collections.singletonList("id"), devBranch));
  ...
  public void testAddInvalidPrimaryKey() throws Exception {
    ...
    try {
      ...
      assertThatThrownBy(() -> runSQL(alterTableAddPrimaryKeyQuery(tablePath, Collections.singletonList(primaryKey))))
  ...
  public void addColumnWithTagSet() throws Exception {
    ...
    // Act and Assert
    assertQueryThrowsExpectedError(createTableAsQuery(tablePath, 5),
  ...
  void alterTableAddPartitionInDevWithAtSyntaxContextMain() throws Exception {
    ...
    assertCommitLogTail(VersionContext.ofBranch(DEFAULT_BRANCH_NAME),
      String.format("CREATE TABLE %s", joinedTableKey(tablePath))
    );
  ...
  void alterTableSortOrder() throws Exception {
    ...
    final List<String> columnDefinition = Arrays.asList("a BOOLEAN", "b BOOLEAN", "c BOOLEAN", "d BOOLEAN");
    ...
    //bundle values
    final List<String> insertValues = Collections.singletonList(
      "(true, true, true, true)," +
    ...
    //Will be adjusted ASAP. We need to perform a file scan of parquet files. Sort Order occurs at the data-file level.
    testBuilder()
      .sqlQuery(selectStarQuery(tablePath))
      .ordered()
  ...
  public void testSetUnsetTableProperties() throws Exception {
    ...
    queryDataBatches = testRunAndReturn(UserBitShared.QueryType.SQL, showTablePropertiesQuery(tablePath));
    resultString = getResultString(queryDataBatches, ",", false);
    Assert.assertTrue(resultString == null || resultString.isEmpty() || !resultString.contains("property_name"));
    runSQL(dropTableQuery(tablePath));
  }
}
