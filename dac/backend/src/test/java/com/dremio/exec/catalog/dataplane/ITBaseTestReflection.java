...
public abstract class ITBaseTestReflection extends ITBaseTestVersioned {
  ...
  protected void createSpace(String name) {
    expectSuccess(getBuilder(getPublicAPI(3).path("/catalog/")).buildPost(Entity.json(new com.dremio.dac.api.Space(null, name, null, null, null))), new GenericType<com.dremio.dac.api.Space>() {});
  }
  ...
  protected JobDataFragment getQueryDataInSession(
    JobsService jobsService, String query, String sessionId, int maxRows, BufferAllocator allocator)
    throws JobNotFoundException {
    return submitJobAndGetData(
      jobsService,
      createJobRequestFromSqlAndSessionId(query, sessionId),
      0,
      maxRows,
      allocator);
  }
