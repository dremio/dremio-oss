...
import static com.dremio.exec.catalog.dataplane.DataplaneTestDefines.selectStarQueryWithoutSpecifyingSource;
...
/**
 * Integration tests for the /sql REST API endpoints.
 * <br><br>
 * Tests are done using SELECT because it is read-only, other queries would be
 * valid too. Validation is done using row counts just to make things simple.
 */
...
public class ITSqlRestApi extends ITBaseTestVersioned {
  ...
  public static void setup() throws Exception {
    ...
    createFolders(TABLE_PATH, VersionContext.ofBranch(DEFAULT_BRANCH_NAME), null);
    runQuery(createTableAsQuery(TABLE_PATH, TABLE_ROWS));

    // We re-use the same name as table #1, but with different row count
    createFolders(TABLE_PATH, VersionContext.ofBranch(ALTERNATE_BRANCH_NAME), null);
    runQuery(new SqlQuery(
      createTableAsQuery(TABLE_PATH, TABLE_IN_BRANCH_ROWS),
      Collections.emptyList(),
      DEFAULT_USERNAME,
      null,
      null,
      ImmutableMap.of(DATAPLANE_PLUGIN_NAME, new JobsVersionContext(JobsVersionContext.VersionContextType.BRANCH, ALTERNATE_BRANCH_NAME))));
  }
  ...
  public void selectFromFullyQualifiedNameAndNullContext(EndpointVersion endpointVersion) {
    // Act
    final JobDataFragment tableResults = runSqlApiExpectSuccess(
        endpointVersion,
        new CreateFromSQL(
  ...
  public void selectWithReferenceOverriddenByAtSyntax(EndpointVersion endpointVersion) {
    ...
    // Assert
    assertThat(tableResults.getReturnedRowCount()).isEqualTo(TABLE_ROWS + rowsInsertedIntoThirdBranch);
  }
  ...
  public void selectWithMultipleReferences(EndpointVersion endpointVersion) {
    // Arrange
    Map<String, VersionContextReq> references = twoBranchReferences(
    ...
    final List<String> tableInSource2Path = Stream.concat(
        Stream.of(DATAPLANE_PLUGIN_NAME_FOR_REFLECTION_TEST),
        TABLE_PATH.stream())
  ...
  public void insertIntoBranches(EndpointVersion endpointVersion) {
    ...
    final List<String> tablePath = generateFolderPath(tableName);
    ...
    // Assert
    final JobDataFragment tableInDefaultBranchResults = runSqlApiExpectSuccess(
      endpointVersion,
      new CreateFromSQL(
        selectStarQuery(tablePath),
        null));
    assertThat(tableInDefaultBranchResults.getReturnedRowCount()).isEqualTo(initialRowsInTable + rowsInsertedIntoDefaultBranch);

    final JobDataFragment tableInAlternateBranchResults = runSqlApiExpectSuccess(
      endpointVersion,
      new CreateFromSQL(
        selectStarQuery(tablePath),
        null,
        oneBranchReference(DATAPLANE_PLUGIN_NAME, otherBranchName)));
    assertThat(tableInAlternateBranchResults.getReturnedRowCount()).isEqualTo(initialRowsInTable + rowsInsertedIntoOtherBranch);
  }
  ...
  private JobDataFragment runSqlApiExpectSuccess(EndpointVersion endpointVersion, CreateFromSQL createFromSQL) {
    return expectSuccess(sqlApiInvocation(endpointVersion, createFromSQL), JobDataFragment.class);
  }

  private void runSqlApiExpectFailure(EndpointVersion endpointVersion, CreateFromSQL createFromSQL) {
    expectStatus(
      Response.Status.BAD_REQUEST,
      sqlApiInvocation(endpointVersion, createFromSQL));
  }

  private Invocation sqlApiInvocation(EndpointVersion endpointVersion, CreateFromSQL createFromSQL) {
    switch (endpointVersion) {
      case API_V2:
        // The /apiv2 implementation returns (up to 500) rows directly
        return getBuilder(getApiForVersion(endpointVersion)
            .path("sql"))
            .buildPost(Entity.json(createFromSQL));
      case API_V3:
        // The /api/v3 implementation only returns a Job ID, if we want results, we need to fetch them
        final SQLResource.QueryDetails queryDetails = expectSuccess(
          getBuilder(getApiForVersion(endpointVersion) // Always v3 inside this case
            .path("sql"))
            .buildPost(Entity.json(createFromSQL)),
        ...
        return getBuilder(getApiForVersion(EndpointVersion.API_V2)
            .path("job")
  ...
  private static Map<String, VersionContextReq> oneBranchReference(String sourceName, String branchName) {
    return Collections.singletonMap(
      sourceName,
      new VersionContextReq(
        VersionContextReq.VersionContextType.BRANCH,
        branchName));
  }

  @SuppressWarnings("SameParameterValue") // Only used once, but worth it for the clarity
  private static Map<String, VersionContextReq> twoBranchReferences(String sourceName1, String branchName1, String sourceName2, String branchName2) {
  ...
  private void insertRowsIntoTableInBranch(List<String> tablePath, String branchName, int numberOfRows) {
    final String sessionId = runQueryAndGetSessionId(useBranchQuery(branchName));
    List<String> valuesToInsert = IntStream.rangeClosed(1, numberOfRows)
