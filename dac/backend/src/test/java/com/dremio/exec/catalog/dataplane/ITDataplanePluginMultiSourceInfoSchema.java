...
/**
 *
 * Entities created under a Nessie source will be shared by other Nessie sources too as underneath they are pointing to same Nessie instance
 * Testing is around multiple sources, nested folders, tables, views
 * <p>
 * Dataplane sources in this framework: dataplane_plugin, dataplane%plugin, dataplaneASDFplugin, dataplaneA1S2DFplugin
 * <p>
 * dataplane_plugin   (same hierarchy is being used by other dataplane sources)
 *  - tableAFirst
 *  - tableBSecond
 *  - viewCThird
 *  - viewDFourth
...
public class ITDataplanePluginMultiSourceInfoSchema extends ITDataplanePluginTestSetup {
  ...
  private static final String DATAPLANE_SOURCE_NAME_WITH_UNDERSCORE = DATAPLANE_SOURCE_PREFIX + "_plugin";
  private static final String DATAPLANE_SOURCE_NAME_WITH_PERCENT = DATAPLANE_SOURCE_PREFIX + "%plugin";
  private static final String DATAPLANE_SOURCE_NAME_WITH_CHARS = DATAPLANE_SOURCE_PREFIX + "ASDFplugin";
  private static final String DATAPLANE_SOURCE_NAME_WITH_ALPHANUMERIC = DATAPLANE_SOURCE_PREFIX + "A1S2DFplugin";
  private static final String NAS_SOURCE_NAME = "nas_plugin";
  private static final String EMPTY_STRING = "";

  private static final List<String> DATAPLANE_SOURCE_NAMES = ImmutableList.of(
  ...
  private static final Map<String, AbstractConnectionConf> sourceNamesWithConnectionConf = new HashMap<>();
  ...
  public static void setupContainers() throws Exception {
    ...
    //As this is multi source tests, so we have to create all sources and one external source to test across platform
  ...
  private static void initializeDataplanePluginContainers() {
    DATAPLANE_SOURCE_NAMES.forEach(
      sourceName -> DATAPLANE_SOURCE_CONTAINERS.add(new ContainerEntity(
        sourceName,
        ContainerEntity.Type.SOURCE,
    ...
    DATAPLANE_SOURCE_CONTAINERS.forEach(
      containerEntity -> DATAPLANE_FOLDER_CONTAINERS.add(new ContainerEntity(
        "Folder1",
        ContainerEntity.Type.EXPLICIT_FOLDER,
        ContainerEntity.Contains.FOLDERS_TABLES_AND_VIEWS,
    ...
    DATAPLANE_FOLDER_CONTAINERS.forEach(
      containerEntity -> DATAPLANE_SUB_FOLDER_CONTAINERS.add(new ContainerEntity(
        "subFolderInFolder1",
        ContainerEntity.Type.EXPLICIT_FOLDER,
        ContainerEntity.Contains.FOLDERS_TABLES_AND_VIEWS,
        containerEntity.getFullPath()))
    ...
    DATAPLANE_SOURCE_CONTAINERS.forEach(
      containerEntity -> DATAPLANE_EMPTY_FOLDER_CONTAINERS.add(new ContainerEntity(
        "emptyFolder2",
        ContainerEntity.Type.EXPLICIT_FOLDER,
  ...
  //Had to initiate the Nessie entities here as Nessie instance removes entities before each test case.
  ...
  private void createEntitiesForDataplanePluginContainer(ContainerEntity container) throws Exception {
    switch(container.getType()) {
      case SOURCE:
        //Source creation is already done so intentional break through
        break;
      case EXPLICIT_FOLDER:
        runSQL(createFolderQuery(container.getFullPath()));
        break;
      default:
  ...
  private static void createEntitiesForNASPluginContainer(ContainerEntity container) throws Exception {
    switch(container.getContains()) {
  ...
  private static void createTablesAndViewsInDataplanePluginContainer(ContainerEntity container) throws Exception {
    ...
    String sourceName = container.getParentPath().isEmpty() ? container.getName(): container.getParentPath().get(0);
    runSQL(createViewSelectQuery(
      sourceName,
  ...
  private static Stream<Arguments> schemataQueryWithEscapeKeywordArguments() {
    return Stream.of(
      ...
      Arguments.of(
        ...
        1,
        Collections.singletonList(
          NAS_SOURCE_CONTAINER.getExpectedSchemata())
      ),
      Arguments.of(
        "dataplane_plugin",
        "_",
    ...
    return Stream.of(
      Arguments.of(
        ...
        Streams.concat(
          ...
          DATAPLANE_SOURCE_CONTAINERS.get(1).getExpectedTablesIncludingViews().stream()).collect(Collectors.toList())
      ),
      Arguments.of(
        "nas_%plugin",
        "%",
  ...
  private static Stream<Arguments> tablesQueryArguments() {
    List<List<String>> expectedResult3 = new ArrayList<>();
    DATAPLANE_FOLDER_CONTAINERS.forEach(
      containerEntity -> expectedResult3.addAll(containerEntity.getExpectedTablesIncludingViews())
    );
    DATAPLANE_SUB_FOLDER_CONTAINERS.forEach(
      containerEntity -> expectedResult3.addAll(containerEntity.getExpectedTablesIncludingViews())
    );
    ...
    List<List<String>> expectedResult5 = new ArrayList<>();
    DATAPLANE_SOURCE_CONTAINERS.forEach(
      containerEntity -> expectedResult5.addAll(containerEntity.getExpectedTablesIncludingViews())
    );
    ...
    return Stream.of(
      Arguments.of(
        "dataplane\\%plugin",
        4,
        DATAPLANE_SOURCE_CONTAINERS.get(1).getExpectedTablesIncludingViews()
      ),
      Arguments.of(
        "dataplane\\_plugin",
        4,
      ...
      Arguments.of(
        "dataplane\\_plugin%subFolderInFolder_",
        4,
        DATAPLANE_SUB_FOLDER_CONTAINERS.get(0).getExpectedTablesIncludingViews()
      ),
      Arguments.of(
        "nas%plugin%Folder%",
        4,
        Streams.concat(
          ...
          NAS_SUB_FOLDER_CONTAINER.getExpectedTablesForNonNessieContainers().stream()).collect(Collectors.toList())
      ),
      Arguments.of(
        "nas_plugin.Folder_",
        2,
  ...
  private static Stream<Arguments> viewsQueryArguments() {
    List<List<String>> expectedResult3 = new ArrayList<>();
    DATAPLANE_FOLDER_CONTAINERS.forEach(
      containerEntity -> expectedResult3.addAll(containerEntity.getExpectedViewsForSelect1Query())
    );
    DATAPLANE_SUB_FOLDER_CONTAINERS.forEach(
      containerEntity -> expectedResult3.addAll(containerEntity.getExpectedViewsForSelect1Query())
    );
    ...
    return Stream.of(
      Arguments.of(
        "dataplane\\%plugin",
        2,
        DATAPLANE_SOURCE_CONTAINERS.get(1).getExpectedViewsForSelect1Query()
      ),
      Arguments.of(
        "dataplane\\_plugin",
        2,
      ...
      Arguments.of(
        "dataplane\\_plugin%subFolderInFolder_",
        2,
        DATAPLANE_SUB_FOLDER_CONTAINERS.get(0).getExpectedViewsForSelect1Query()
      ));
  }
  ...
  public void testSchemataQuery(String regex, int expectedRows, List<List<String>> expectedSchemata) throws Exception {
    //Test SQL: SELECT * from INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME LIKE 'dataplane%plugin'
    List<List<String>> rows = runSqlWithResults(String.format("SELECT * FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME LIKE '%s'", regex));
  ...
  public void testSchemataQueryWithEscape(String regex, String escapeKeyword, int expectedRows, List<List<String>> expectedSchemata) throws Exception {
    //Test SQL: SELECT * from INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME LIKE 'dataplane_%plugin' ESCAPE '%'
    List<List<String>> rows = runSqlWithResults(String.format("SELECT * FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME LIKE '%s' ESCAPE '%s'", regex, escapeKeyword));
    assertThat(rows.size()).isEqualTo(expectedRows);
    assertThat(rows)
      .containsExactlyInAnyOrderElementsOf(expectedSchemata);
  }
  ...
  public void testTablesQueryWithEscape(String regex, String escapeKeyword, int expectedRows, List<List<String>> expectedSchemata) throws Exception {
    //Test SQL: SELECT * from INFORMATION_SCHEMA."TABLES" WHERE TABLE_SCHEMA LIKE 'dataplane_%plugin' ESCAPE '%'
    List<List<String>> rows = runSqlWithResults(String.format("SELECT * FROM INFORMATION_SCHEMA.\"TABLES\" WHERE TABLE_SCHEMA LIKE '%s' ESCAPE '%s'", regex, escapeKeyword));
    assertThat(rows.size()).isEqualTo(expectedRows);
    assertThat(rows)
      .containsExactlyInAnyOrderElementsOf(expectedSchemata);
  }
  ...
  public void testTablesQuery(String regex, int expectedRows, List<List<String>> expectedSchemata) throws Exception {
    //Test SQL: SELECT * from INFORMATION_SCHEMA."TABLES" WHERE TABLE_SCHEMA LIKE 'dataplane\%plugin'
    List<List<String>> rows = runSqlWithResults(String.format("SELECT * FROM INFORMATION_SCHEMA.\"TABLES\" WHERE TABLE_SCHEMA LIKE '%s'", regex));
    assertThat(rows.size()).isEqualTo(expectedRows);
    assertThat(rows)
      .containsExactlyInAnyOrderElementsOf(expectedSchemata);
  }
  ...
  public void testColumnsQuery(String regex, int expectedRows, String expectedSourceName) throws Exception {
    //Test SQL: SELECT * from INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA LIKE 'dataplane\%plugin'
    List<List<String>> rows = runSqlWithResults(String.format("SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA LIKE '%s'", regex));
    assertThat(rows.size()).isEqualTo(expectedRows);
    assertThat(rows).doesNotHaveDuplicates();
    if (!EMPTY_STRING.equals(expectedSourceName)) {
      assertThat(rows)
        .anyMatch(row -> row.get(1).startsWith(expectedSourceName));
    }
  }

  @ParameterizedTest(name = "{index} {0} {1}")
  @MethodSource("viewsQueryArguments")
  public void testViewsQuery(String regex, int expectedRows, List<List<String>> expectedSchemata) throws Exception {
    //Test SQL: SELECT * from INFORMATION_SCHEMA.VIEWS WHERE TABLE_SCHEMA LIKE 'dataplane\%plugin'
    List<List<String>> rows = runSqlWithResults(String.format("SELECT * FROM INFORMATION_SCHEMA.VIEWS WHERE TABLE_SCHEMA LIKE '%s'", regex));
    assertThat(rows.size()).isEqualTo(expectedRows);
    assertThat(rows)
      .containsExactlyInAnyOrderElementsOf(expectedSchemata);
  }
  ...
  public void testTablesQueryForTableTypeCondition(String regex, int expectedRows, List<List<String>> expectedSchemata) throws Exception {
    //Test SQL: SELECT * from INFORMATION_SCHEMA."TABLES" WHERE TABLE_TYPE = 'TABLE'
    List<List<String>> rows = runSqlWithResults(String.format("SELECT * FROM INFORMATION_SCHEMA.\"TABLES\" WHERE " +
      "TABLE_TYPE = 'TABLE' AND TABLE_SCHEMA LIKE '%s'", regex));
    assertThat(rows.size()).isEqualTo(expectedRows);
    assertThat(rows)
      .containsExactlyInAnyOrderElementsOf(expectedSchemata);
  }
