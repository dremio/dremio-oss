...
/**
 * External reflection tests on versioned datasets.  Currently, external reflections on versioned datasets is not supported
 * so just verify that DDL query fails.  In the future, when we do support this feature, add tests here.
 */
public class ITTestExternalReflections extends ITBaseTestReflection {
  ...
  private static final List<String> tablePath = tablePathWithSource(DATAPLANE_PLUGIN_NAME, Collections.singletonList(tableName));

  @BeforeAll
  public static void testSetup() {
    JobsServiceTestUtils.submitJobAndWaitUntilCompletion(
      l(JobsService.class), createNewJobRequestFromSql(createTableAsQuery(Collections.singletonList(tableName), 1000)), JobStatusListener.NO_OP);
  }
  ...
  public void testExternalReflectionFails() {
    assertThatThrownBy(() -> JobsServiceTestUtils.submitJobAndWaitUntilCompletion(
      l(JobsService.class), createNewJobRequestFromSql(createExternalReflection(tablePath, tablePath, "ext_ref")), JobStatusListener.NO_OP))
      .isInstanceOf(UserRemoteException.class)
      .hasMessageContaining("External reflections are not supported on versioned source dataPlane_Test");
  }

  public static String createExternalReflection(final List<String> query, final List<String> target, final String name) {
    return String.format(
      "ALTER DATASET %s CREATE EXTERNAL REFLECTION %s USING %s",
      joinedTableKey(query),
      name,
      joinedTableKey(target));
  }
}
