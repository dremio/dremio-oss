...
public class ITDataplanePluginInfoSchema extends ITDataplanePluginTestSetup {
  ...
  public void selectInformationSchemaColumns() throws Exception {
    ...
    assertThat(runSqlWithResults("select TABLE_CATALOG, TABLE_SCHEMA, TABLE_NAME, COLUMN_NAME, ORDINAL_POSITION from INFORMATION_SCHEMA.COLUMNS"))
    ...
    assertThat(runSqlWithResults(String.format("select TABLE_CATALOG, TABLE_SCHEMA, TABLE_NAME, COLUMN_NAME, ORDINAL_POSITION from INFORMATION_SCHEMA.COLUMNS where table_schema = '%s.%s.%s'", DATAPLANE_PLUGIN_NAME, tablePath.get(0), tablePath.get(1))))
      .containsAll(expected);
  }
  @Test
  public void selectInformationSchemaTableWithSchemaFilter() throws Exception {
    ...
    assertThat(runSqlWithResults(String.format("select * from INFORMATION_SCHEMA.\"TABLES\" WHERE TABLE_SCHEMA = '%s.%s.%s'",DATAPLANE_PLUGIN_NAME, tablePath1.get(0), tablePath1.get(1))))
      .contains(tableRow)
  ...
  public void selectInformationSchemaTableWithNameFilter() throws Exception {
    ...
    assertThat(runSqlWithResults(String.format("select * from INFORMATION_SCHEMA.\"TABLES\" WHERE TABLE_NAME = '%s'",tableName1)))
      .contains(tableRow)
      .allMatch(row -> !row.get(2).contains(tableName2));
  }
  @Test
  public void selectInformationSchemaViewWithSchemaFilter() throws Exception {
    ...
    //Test with schema
    assertThat(runSqlWithResults(String.format("select * from INFORMATION_SCHEMA.views where table_schema = '%s.%s.%s'", DATAPLANE_PLUGIN_NAME, viewKey.get(0), viewKey.get(1))))
      .contains(viewRow)
      .allMatch(row -> !row.get(2).contains(tableName));
  }
  ...
  public void selectInformationSchemaViewWithNameFilter() throws Exception {
    ...
    //Test with name
    assertThat(runSqlWithResults(String.format("select * from INFORMATION_SCHEMA.views where table_name = '%s'", viewName)))
      .contains(viewRow)
      .allMatch(row -> !row.get(2).contains(tableName));
  }
  ...
  public void selectInformationSchemaSchemataWithFilter() throws Exception {
    ...
    assertThat(runSqlWithResults(String.format("select * from INFORMATION_SCHEMA.SCHEMATA where schema_name = '%s.%s.%s'",DATAPLANE_PLUGIN_NAME, tablePath.get(0), tablePath.get(1))))
      .contains(row1)
  ...
  public void selectInformationSchemaColumnsWithNameFilter() throws Exception {
    ...
    expected.add(row2);
    //Test with name
    assertThat(runSqlWithResults(String.format("select TABLE_CATALOG, TABLE_SCHEMA, TABLE_NAME, COLUMN_NAME, ORDINAL_POSITION from INFORMATION_SCHEMA.COLUMNS where table_name = '%s'", tableName)))
      .containsAll(expected);
  }
  ...
  public void testMultipleBackSlash() throws Exception {
    ...
    String query1 = String.format("select * from information_schema.\"tables\" where table_name = '%s'", resultEscapedTableName);
    assertThat(runSqlWithResults(query1))
      .contains(table_row)
      .allMatch(row -> !row.get(2).equals(tableName));

    String query2 = String.format("select * from information_schema.\"tables\" where table_name like '%s'", resultEscapedTableNamePattern);
  ...
  public void testNestedFolders() throws Exception {
    ...
    //Value of tablePath(1) which is a folder name will be get created under the root folder (Value of tablePath(0)) in the source
    getNessieClient().createNamespace()
      .namespace(Namespace.of(tablePath.subList(0, 2)))
      .refName("main")
      .create();
     ...
     * - folder2
     */
    String sameFolderNameLikeQuery = String.format("select * from information_schema.SCHEMATA where schema_name like '%s.%s'", DATAPLANE_PLUGIN_NAME, tablePath.get(1));
    List<List<String>> rows = runSqlWithResults(sameFolderNameLikeQuery);
    assertThat(rows.size()).isEqualTo(1);
    assertThat(rows)
      .allMatch(row -> row.get(1).equals(String.format("%s.%s", DATAPLANE_PLUGIN_NAME, tablePath.get(1))));

    sameFolderNameLikeQuery = String.format("select * from information_schema.SCHEMATA where schema_name like '%s.%s.%s'", DATAPLANE_PLUGIN_NAME, tablePath.get(0), tablePath.get(1));
  ...
  public void testSelectFromNonVersionedSources() throws Exception {
    ...
    createFolders(tablePath, VersionContext.ofBranch(DEFAULT_BRANCH_NAME));
    runSQL(createEmptyTableQuery(tablePath));
    String selectFromNonVersionedSource = "SELECT * FROM INFORMATION_SCHEMA.\"TABLES\" WHERE TABLE_SCHEMA = 'cp'";
  ...
  public void testNamespaceLinksToCorrectNamespace() throws Exception {
    ...
    runSqlWithResults(createFolderQuery(folderPath1));

    //create folder B
    List<String> folderPath2 = Arrays.asList(DATAPLANE_PLUGIN_NAME, folderA, folderB);
    runSqlWithResults(createFolderQuery(folderPath2));
    ...
    runSqlWithResults(createFolderQuery(folderPath3));
