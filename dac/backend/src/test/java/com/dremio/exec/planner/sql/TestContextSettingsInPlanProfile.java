...
public class TestContextSettingsInPlanProfile extends ITBaseTestVersioned {
  ...
  public void testVersionContextSetToMainBranch() throws Exception {
    ...
    final String query = String.format("Select * from %s.%s AT BRANCH \"main\"" , DATAPLANE_PLUGIN_NAME, joinedTableKey(table1Path));
    final UserBitShared.QueryProfile profile = getQueryProfile(createJobRequestFromSqlAndSessionId(query, sessionId));

    assertEquals(DATAPLANE_PLUGIN_NAME,  profile.getContextInfo().getSchemaPathContext());
    assertEquals(VersionContext.ofBranch(DEFAULT_BRANCH_NAME).toString(), profile.getContextInfo().getSourceVersionSettingList().get(0).getVersionContext());
  }
  ...
  public void testPathContextSetToOneFolder() throws Exception {
    ...
    runQuery(insertTableQuery(table1Path));
    runQueryInSession(useBranchQuery(DEFAULT_BRANCH_NAME), sessionId);
    runQueryInSession(useContextQuery(Arrays.asList(DATAPLANE_PLUGIN_NAME, table1Path.get(0))), sessionId);
  ...
  public void testVersionContextSetToDifferentBranch() throws Exception {
    ...
    final String sessionId = runQueryAndGetSessionId(createBranchAtBranchQuery(branchName, DEFAULT_BRANCH_NAME));
    createFolders(table1Path, VersionContext.ofBranch(DEFAULT_BRANCH_NAME), null);
    ...
    assertEquals(DATAPLANE_PLUGIN_NAME,  profile.getContextInfo().getSchemaPathContext());
    assertEquals(VersionContext.ofBranch(branchName).toString(), profile.getContextInfo().getSourceVersionSettingList().get(0).getVersionContext());
  }
  ...
  public void testViewQuery() throws Exception {
    ...
    final String query = String.format("Select * from %s.%s AT BRANCH \"main\"" , DATAPLANE_PLUGIN_NAME, joinedTableKey(viewPath));
    final UserBitShared.QueryProfile profile = getQueryProfile(createJobRequestFromSqlAndSessionId(query, sessionId));
  ...
  public void testViewQueryWithAT() throws Exception {
    ...
    final String viewName = generateUniqueViewName();
    List<String> viewPath = tablePathWithFolders(viewName);
    createFolders(viewPath, VersionContext.ofBranch(branchName), null);
    String createViewQuery = String.format("CREATE VIEW %s.%s AT BRANCH %s AS SELECT * FROM %s.%s AT BRANCH %s",
    ...
    final String query = String.format("Select * from %s.%s AT BRANCH %s" , DATAPLANE_PLUGIN_NAME, joinedTableKey(viewPath), branchName);
    final UserBitShared.QueryProfile profile = getQueryProfile(createJobRequestFromSqlAndSessionId(query, sessionId));
