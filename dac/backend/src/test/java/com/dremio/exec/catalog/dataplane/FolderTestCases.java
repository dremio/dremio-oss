...
/**
 *
 * Add tests around Dataplane listEntries and listEntriesWithNested. In Nessie API V2, there is no implicit namespaces
 *
 * dataplane_test
 *  - tableAFirst
 *  - tableBSecond
...
public class FolderTestCases {
  ...
  FolderTestCases(ITDataplanePluginTestSetup base) {
    this.base = base;
  }
  ...
  private Stream<List<String>> listEntries(ContainerEntity container) {
    return base.getDataplanePlugin().
  ...
  @Test
  public void testNamespaceLinksToCorrectNamespace() {
    ...
    assertThat(base.getDataplanePlugin().listEntries(incorrectFullPath, MAIN))
      .map(ExternalNamespaceEntry::getNameElements)
      .isEmpty();
  }

  public Stream<List<String>> getFullPath(ContainerEntity parent) {
    return Collections.singletonList(parent.getPathWithoutRoot()).stream();
  }
  public Stream<List<String>>  getFullPath(ContainerEntity parent, String nameElement) {
    if (parent.getType() == ContainerEntity.Type.SOURCE) {
      return Collections.singletonList(Arrays.asList(nameElement)).stream();
    }
    return Collections.singletonList(Streams.concat(parent.getPathWithoutRoot().stream(), Stream.of(nameElement))
