...
public class ITVersionedSourceCollaborationResource extends ITBaseTestVersioned {
  ...
  public void testVersionedTableIdFromSummaryEndpoint() throws Exception {
    ...
    //Act
    DatasetSummary summary = expectSuccess(getBuilder(getAPIv2().path("/datasets/summary" + PathUtils.toFSPath(tableFullPath))).buildGet(), DatasetSummary.class);
    //Assert
  ...
  public void testVersionedViewIdFromSummaryEndpoint() throws Exception {
    ...
    DatasetSummary summary = expectSuccess(getBuilder(getAPIv2().path("/datasets/summary" + PathUtils.toFSPath(viewFullPath))).buildGet(), DatasetSummary.class);
    //Assert
    assertThat(summary).isNotNull();
    assertThat(VersionedDatasetId.isVersionedDatasetId(summary.getEntityId())).isTrue();
  ...
  public void testVersionedSetWikiWithoutFeatureFlag() throws Exception {
    ...
    //Assert
    UserExceptionMapper.ErrorMessageWithContext error = expectError(FamilyExpectation.CLIENT_ERROR,
      getBuilder(getPublicAPI(3)
        .path("catalog")
        .path(PathUtils.encodeURIComponent(versionedDatasetIdAsString))
        .path("collaboration").path("wiki")).buildPost(Entity.json(newWiki)),
      UserExceptionMapper.ErrorMessageWithContext.class);
    assertTrue(error.getErrorMessage().contains("Wiki and Label not supported on entities in source"));
  ...
  public void testVersionedGetWikiOnTableAfterUpdate() throws Exception {
    ...
    //Act 2
    summary = expectSuccess(getBuilder(getAPIv2().path("/datasets/summary" + PathUtils.toFSPath(tableFullPath))).buildGet(), DatasetSummary.class);
  ...
  public void testVersionedSetWikiOnBranch() throws Exception {
    ...
    String sessionId = runQueryAndGetSessionId(createBranchAtBranchQuery(devBranchName, DEFAULT_BRANCH_NAME));
    // create table at dev
    ...
    final List<String> testTableFullPathInBranch = tablePathWithSource(DATAPLANE_PLUGIN_NAME, testTablePathInBranch);
    createFolders(testTablePathInBranch, VersionContext.ofBranch(devBranchName), sessionId);
    runQueryInSession(createTableAsQuery(testTablePathInBranch, 500), sessionId);

    // Act
    DatasetSummary summary = expectSuccess(getBuilder(getAPIv2()
      ...
      .buildGet(), DatasetSummary.class);
    String versionedDatasetIdAsString = summary.getEntityId();
    // create wiki
    Wiki newWiki = new Wiki("sample wiki text", null);

    //Assert
    UserExceptionMapper.ErrorMessageWithContext error = expectError(FamilyExpectation.CLIENT_ERROR, getBuilder(getPublicAPI(3).path("catalog").path(PathUtils.encodeURIComponent(versionedDatasetIdAsString)).path("collaboration").path("wiki")).buildPost(Entity.json(newWiki)), UserExceptionMapper.ErrorMessageWithContext.class);
    assertTrue(error.getErrorMessage().contains("Wiki and Label can only be set on the default branch"));
  ...
  public void testNoTags() throws Exception {
    ...
    String versionedDatasetIdAsString = summary.getEntityId();
    // Test no tags
    Tags noLabels = expectSuccess(getBuilder(getPublicAPI(3).path("catalog").path(PathUtils.encodeURIComponent(versionedDatasetIdAsString)).path("collaboration").path("tag")).buildGet(), Tags.class);
  ...
  public void testVersionedSetlabelWithoutFeatureFlag() throws Exception {
    ...
    UserExceptionMapper.ErrorMessageWithContext error = expectError(FamilyExpectation.CLIENT_ERROR,
      getBuilder(getPublicAPI(3)
        ...
        .path(PathUtils.encodeURIComponent(versionedDatasetIdAsString))
        .path("collaboration")
        .path("tag")).buildPost(Entity.json(newlabel)),
      UserExceptionMapper.ErrorMessageWithContext.class);
    assertTrue(error.getErrorMessage().contains("Wiki and Label not supported on entities in source"));
  ...
  public void testOrphanPruningWithVersioned() throws Exception {
    ...
    final List<String> vdsInFolder = Arrays.asList(spacePath.getRoot(), "testFolder", "vdsInFolder");
    ...
    addWikiToNamespaceEntity(Arrays.asList(spacePath.getRoot(), "testFolder"), "wiki for testfolder");
  ...
  public void testVersionedGetFolderId() throws Exception {
    ...
    final List<String> folderPath = generateFolderPath(folderName);
    createFolder(folderPath, VersionContext.ofBranch(DEFAULT_BRANCH_NAME), null);

    //Act
    Folder folder = sourceService.getFolder(new SourceName(DATAPLANE_PLUGIN_NAME), new SourceFolderPath( joinedTableKey(folderPath)), false,"dremio", "BRANCH", "main");
  ...
  public void testVersionedSetFolderWiki() throws Exception {
    ...
    //Act
    Wiki newWiki = new Wiki("sample wiki text", null);
    Wiki setWiki = setWikiForVersionedFolder(folderPath, newWiki);
  ...
  public void testVersionedGetFolderWiki() throws Exception {
    ...
    //Assert
    Wiki readWiki = getWikiForVersionedFolder(folderPath);
  ...
  public void testVersionedSetFolderLabel() throws Exception {
    ...
    Tags newLabels = new Tags(labelList, null);
    Tags setTags = setLabelsForVersionedFolder(folderPath, newLabels);
  ...
  public void testVersionedGetFolderLabel() throws Exception {
    ...
    //Assert
    Tags tags = getLabelsForVersionedFolder(folderPath);
  ...
  public void testVersionedSetFolderTagNoFeatureFlag() throws Exception {
    ...
    UserExceptionMapper.ErrorMessageWithContext error = expectError(FamilyExpectation.CLIENT_ERROR,
      getBuilder(getPublicAPI(3).path("catalog")
        .path(PathUtils.encodeURIComponent(versionedDatasetIdAsString))
        .path("collaboration")
        .path("tag")).buildPost(Entity.json(newTags)),
      UserExceptionMapper.ErrorMessageWithContext.class);
    assertTrue(error.getErrorMessage().contains("Wiki and Label not supported on entities in source"));
  }
  ...
  public void testVersionedUpdateLabelOnFolder() throws Exception {
    ...
    List<String> labelList = Arrays.asList("label1", "label2");
    Tags newLabels = new Tags(labelList, null);
    setLabelsForVersionedFolder(folderPath, newLabels);

    Tags readTags = getLabelsForVersionedFolder(folderPath);
   ...
   Tags setLabels = setLabelsForVersionedFolder(folderPath, updatedLabels);
  ...
  public void testVersionedUpdateWikiOnFolder() throws Exception {
    ...
    setWikiForVersionedFolder(folderPath, newWiki);
    Wiki readWiki = getWikiForVersionedFolder(folderPath);
    assertEquals(readWiki.getText(), newWiki.getText());
    // Update wiki
    Wiki updatedWiki = new Wiki("updated wiki text", readWiki.getVersion());
    readWiki = setWikiForVersionedFolder(folderPath, updatedWiki);
  ...
  public void testVersionedSetFolderWikiOnBranch() throws Exception {
    ...
    runQueryInSession(useBranchQuery(devBranchName), sessionId);
    List<String> folderPathInBranch = generateFolderPath(generateUniqueFolderName());
    createFolder(folderPathInBranch, VersionContext.ofBranch(devBranchName), sessionId);

    // Act
    Folder folderInBranch = sourceService.getFolder(new SourceName(DATAPLANE_PLUGIN_NAME), new SourceFolderPath( joinedTableKey(folderPathInBranch)), false,"dremio", "BRANCH", devBranchName);
  ...
  protected static AutoCloseable enableWikiLabel() {
    setSystemOption(CatalogOptions.WIKILABEL_ENABLED_FOR_VERSIONED_SOURCE_DEFAULT_BRANCH.getOptionName(), "true");
    return () ->
      setSystemOption(
        CatalogOptions.WIKILABEL_ENABLED_FOR_VERSIONED_SOURCE_DEFAULT_BRANCH.getOptionName(),
        CatalogOptions.WIKILABEL_ENABLED_FOR_VERSIONED_SOURCE_DEFAULT_BRANCH.getDefault().getBoolVal().toString());

  }

  protected static AutoCloseable disableWikiLabel() {
    setSystemOption(CatalogOptions.WIKILABEL_ENABLED_FOR_VERSIONED_SOURCE_DEFAULT_BRANCH.getOptionName(), "false");
    return () ->
      setSystemOption(
        CatalogOptions.WIKILABEL_ENABLED_FOR_VERSIONED_SOURCE_DEFAULT_BRANCH.getOptionName(),
        CatalogOptions.WIKILABEL_ENABLED_FOR_VERSIONED_SOURCE_DEFAULT_BRANCH.getDefault().getBoolVal().toString());
  ...
  private Wiki getWikiForVersionedEntity(List<String> fullPath) {
    DatasetSummary summary = expectSuccess(getBuilder(getAPIv2().path("/datasets/summary" + PathUtils.toFSPath(fullPath))).buildGet(), DatasetSummary.class);
    String versionedDatasetIdAsString = summary.getEntityId();
    Wiki returnedWiki = expectSuccess(getBuilder(getPublicAPI(3).path("catalog").path(PathUtils.encodeURIComponent(versionedDatasetIdAsString)).path("collaboration").path("wiki")).buildGet(), Wiki.class);
    return returnedWiki;
  }
  private Wiki setWikiForVersionedEntity(List<String> fullPath, Wiki wiki) {
    DatasetSummary summary = expectSuccess(getBuilder(getAPIv2().path("/datasets/summary" + PathUtils.toFSPath(fullPath))).buildGet(), DatasetSummary.class);
    String versionedDatasetIdAsString = summary.getEntityId();
    Wiki createdWiki = expectSuccess(getBuilder(getPublicAPI(3)
      .path("catalog")
      .path(PathUtils.encodeURIComponent(versionedDatasetIdAsString))
      .path("collaboration")
      .path("wiki"))
      .buildPost(Entity.json(wiki)), Wiki.class);
    return createdWiki;
  }
  ...
  private Tags setLabelsForVersionedEntity(List<String> fullPath, Tags labelList) {
    ...
    // create new labels
    Tags createdlabels = expectSuccess(getBuilder(getPublicAPI(3)
      .path("catalog")
      .path(PathUtils.encodeURIComponent(versionedDatasetIdAsString))
      .path("collaboration")
      .path("tag"))
      .buildPost(Entity.json(labelList)), Tags.class);
  ...
  private Tags getLabelsForVersionedEntity(List<String> fullPath) {
    ...
    //Get labels
    Tags readlabels = expectSuccess(getBuilder(getPublicAPI(3)
      .path("catalog")
      .path(PathUtils.encodeURIComponent(versionedDatasetIdAsString))
      .path("collaboration")
      .path("tag"))
  ...
  private Wiki getWikiForVersionedFolder(List<String> folderPath) throws NamespaceException, IOException {
    createFolder(folderPath, VersionContext.ofBranch(DEFAULT_BRANCH_NAME), null);
    SourceService sourceService = l(SourceService.class);
    Folder folder = sourceService.getFolder(new SourceName(DATAPLANE_PLUGIN_NAME), new SourceFolderPath( joinedTableKey(folderPath)), false,"dremio", "BRANCH", "main");
    String versionedDatasetIdAsString = folder.getId();
  ...
  private Wiki setWikiForVersionedFolder(List<String> folderPath, Wiki wiki) throws NamespaceException, IOException {
    createFolder(folderPath, VersionContext.ofBranch(DEFAULT_BRANCH_NAME), null);
    SourceService sourceService = l(SourceService.class);
    Folder folder = sourceService.getFolder(new SourceName(DATAPLANE_PLUGIN_NAME), new SourceFolderPath( joinedTableKey(folderPath)), false,"dremio", "BRANCH", "main");
    String versionedDatasetIdAsString = folder.getId();
  ...
  private Tags setLabelsForVersionedFolder(List<String> folderPath, Tags labelList) throws NamespaceException, IOException {
    createFolder(folderPath, VersionContext.ofBranch(DEFAULT_BRANCH_NAME), null);
    SourceService sourceService = l(SourceService.class);
    Folder folder = sourceService.getFolder(new SourceName(DATAPLANE_PLUGIN_NAME), new SourceFolderPath( joinedTableKey(folderPath)), false,"dremio", "BRANCH", "main");
    String versionedDatasetIdAsString = folder.getId();
  ...
  private Tags getLabelsForVersionedFolder(List<String> folderPath) throws NamespaceException, IOException {
    createFolder(folderPath, VersionContext.ofBranch(DEFAULT_BRANCH_NAME), null);
    SourceService sourceService = l(SourceService.class);
    Folder folder = sourceService.getFolder(new SourceName(DATAPLANE_PLUGIN_NAME), new SourceFolderPath( joinedTableKey(folderPath)), false,"dremio", "BRANCH", "main");
    String versionedDatasetIdAsString = folder.getId();
