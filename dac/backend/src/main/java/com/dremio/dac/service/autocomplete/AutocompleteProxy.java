...
public final class AutocompleteProxy {
  ...
  private static AutocompleteResponse getColumnSuggestions(
    ...
    final Map<List<String>, List<Field>> matchingColumnsMap = getMatchingColumnsMap(catalogServiceHelper, catalogEntityKeys, queryContext, prefix, refType, refValue);
  ...
  private static AutocompleteResponse getReferenceSuggestions(
    ...
    final List<ReferenceInfo> matchingReferences = getMatchingReferences(catalogServiceHelper, catalogEntityKeys.get(0).get(0), prefix, refType);
  ...
  private static List<CatalogItem> getMatchingContainers(
    ...
    // When it's not null nor empty, it's assuming that the queryContext is a list of non-empty strings.
    if (queryContext != null && !queryContext.isEmpty()) {
      List<String> catalogEntityKeyWithQueryContext =
        Stream.concat(queryContext.stream(), catalogEntityKey.stream()).collect(Collectors.toList());
      matchingContainers.addAll(getMatchingContainersFromPath(catalogServiceHelper, catalogEntityKeyWithQueryContext, prefix, refType, refValue));
    }
  ...
  private static Map<List<String>, List<Field>> getMatchingColumnsMap(
    ...
    for (List<String> catalogEntityKey : dedupedCatalogEntityKeys) {
      // The implementation here is to match the query planning implementation which will search the entity key IN the
      ...
      if (queryContext != null && !queryContext.isEmpty()) {
        List<String> catalogEntityKeyWithQueryContext =
          Stream.concat(queryContext.stream(), catalogEntityKey.stream()).collect(Collectors.toList());
        List<Field> matchedColumns = getMatchingColumnsFromPath(catalogServiceHelper, catalogEntityKeyWithQueryContext, prefix, refType, refValue);
        if (matchedColumns != null) {
          matchingColumnsMap.put(catalogEntityKeyWithQueryContext, matchedColumns);
          continue;
        }
      }
      List<Field> matchedColumns = getMatchingColumnsFromPath(catalogServiceHelper, catalogEntityKey, prefix, refType, refValue);
      if (matchedColumns != null) {
  ...
  private static List<ReferenceInfo> getMatchingReferences(
    ...
    SuggestionsType refType) {
    try {
      Stream<ReferenceInfo> matchingReferences = catalogServiceHelper.getReferencesForVersionedSource(sourceName, refType);
