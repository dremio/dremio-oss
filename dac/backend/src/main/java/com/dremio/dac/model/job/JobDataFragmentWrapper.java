...
public class JobDataFragmentWrapper implements JobDataFragment {
  ...
  public JobDataFragmentWrapper(
      int offsetInJobResults, com.dremio.service.jobs.JobDataFragment delegate) {
  ...
  public DataType extractType(String column, int index) {
    ...
    if (columnDef.getType() == DataType.MIXED) {
      ...
      return DataTypeUtil.getDataType(
          getMinorTypeFromArrowMinorType(MinorType.values()[typeValue]));
    }
  ...
  public static class JobDataFragmentSerializer extends JsonSerializer<JobDataFragmentWrapper> {
    ...
    private static final org.slf4j.Logger logger =
        org.slf4j.LoggerFactory.getLogger(JobDataFragmentSerializer.class);

    @Override
    public void serialize(
        final JobDataFragmentWrapper data, JsonGenerator generator, SerializerProvider provider)
        throws IOException {
      try (TimedBlock b = time("serialize job results VV -> JSON")) {
        /**
         * Example output:
         *
         * <p>{ "returnedRowCount" : 5, "columns": [ { "name": "colBit", "type": "BOOLEAN", "index":
         * 0, "status": null }, { "name": "colTinyInt", "type": "INTEGER", "index": 1, "status":
         * null } ], "rows" : [ ... one object for each row written by {@link
         * DACJobResultsSerializer} ... ... See {@link DACJobResultsSerializer} for details of
         * format ... ] }
         */
        ...
        DACJobResultsSerializer jsonWriter =
            new DACJobResultsSerializer(
                generator,
                context,
                Integer.getInteger(MAX_CELL_SIZE_KEY, 100),
