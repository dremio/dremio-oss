...
public class MatchPattern {
  ...
  public static ReplaceSelectionType initSelectionType(NullableVarCharHolder value) {
    return ReplaceSelectionType.valueOf(
        StringFunctionHelpers.toStringFromUTF8(value.start, value.end, value.buffer));
  }

  public static ReplaceMatcher initMatcher(
      NullableVarCharHolder patternStr, ReplaceSelectionType selectionType, Boolean ignoreCase) {
  ...
  public static class MatchExample implements SimpleFunction {
    ...
    @Param(constant = true)
    private NullableVarCharHolder selectionTypeStr;

    @Param(constant = true)
    private NullableVarCharHolder patternStr;

    @Param(constant = true)
    private NullableBitHolder ignoreCaseInt;
    ...
    public void setup() {
      ...
      matcher =
          com.dremio.dac.explore.udfs.MatchPattern.initMatcher(
              patternStr, selectionType, ignoreCaseInt.value == 1);
    }
