...
public class QlikAppMessageBodyGenerator implements MessageBodyWriter<DatasetConfig> {
  ...
  private static final Function<ViewFieldType, String> FIELD_TO_NAME =
      new Function<ViewFieldType, String>() {
        @Override
        public String apply(ViewFieldType input) {
          String name = input.getName();

          // Qlik needs field names that contain spaces to be surrounded by " so we surround all
          // field names.
          // If the field name contains a ", we escape it with "" (per
          // http://help.qlik.com/en-US/sense/3.1/Subsystems/Hub/Content/Scripting/use-quotes-in-script.htm)
          return quoteString(name);
        }
      };

  private static final Function<ViewFieldType, QlikFieldType> FIELD_TO_QLIKTYPE =
      new Function<ViewFieldType, QlikFieldType>() {
        @Override
  ...
  public void writeTo(
    ...
    ImmutableListMultimap<QlikFieldType, ViewFieldType> mapping =
        FluentIterable.from(viewFieldTypes).index(FIELD_TO_QLIKTYPE);
    ...
    String sanitizedDatasetName =
        dataset.getName().replaceAll("[" + Pattern.quote("/!@ *-=+{}<>,~") + "]+", "_");
    ...
    try (PrintWriter pw = new PrintWriter(entityStream)) {
      ...
      for (final String component : dataset.getFullPathList()) {
        ...
        if (!quoted.startsWith(String.valueOf(SqlUtils.QUOTE))
            || !quoted.endsWith(String.valueOf(SqlUtils.QUOTE))) {
  ...
  private void writeFields(PrintWriter pw, String fieldType, Collection<ViewFieldType> fields)
      throws IOException {
    if (!fields.isEmpty()) {
