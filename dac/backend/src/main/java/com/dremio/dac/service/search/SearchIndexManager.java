...
public class SearchIndexManager implements Runnable {
  ...
  static final IndexKey PATH_UNQUOTED_LC =
      IndexKey.newBuilder("pathlc", "PATH_LC", String.class)
          .setCanContainMultipleValues(true)
          .build();
  static final IndexKey NAME_LC = IndexKey.newBuilder("namelc", "NAME_LC", String.class).build();
  static final IndexKey TAGS_LC =
      IndexKey.newBuilder("tagslc", "TAGS_LC", String.class)
          .setCanContainMultipleValues(true)
          .build();
  static final IndexKey DATASET_COLUMNS_LC =
      IndexKey.newBuilder("dscolumnslc", "DATASET_COLUMNS_LC", String.class)
          .setCanContainMultipleValues(true)
          .build();
   ...
   *
   * <p>This is a join of two stores - tags and the namespace. First get all tags that have been
   * modified since the previous wakeup. After that, search the namespace for any newly modified
   * entities as well as the entities that have modified tags. Finally we fetch all tags for
   * namespace entities for whom we don't have tags from the first step.
   *
  ...
  private void handleChanges(long previousWakeupTime) {
    ...
    StreamSupport.stream(namespaceEntries.spliterator(), false)
        .filter(
            input -> {
              ...
              return !"sys".equals(connectorName)
                  ...
                  && (dataset.getOwner() == null
                      || !dataset.getOwner().equals(SystemUser.SYSTEM_USERNAME));
            })
        .forEach(
            input -> {
              final NameSpaceContainer nameSpaceContainer = input.getValue();
              ...
              if (collaborationTagMap.containsKey(datasetId)) {
                ...
                // Check if there are tags for the container.  However, if the previous wakeup is <=
                // 0, that means that we are
                // doing a first time index and therefore we have all possible tags stored in
                // collaborationTagMap so we can
                // skip this lookup.
                Optional<CollaborationTag> tag =
                    collaborationTagStore.getTagsForEntityId(datasetId);
                if (tag.isPresent()) {
                  collaborationTag = tag.get();
                }
              }

              final SearchContainer searchEntity =
                  new SearchContainer(nameSpaceContainer, collaborationTag);
              final NamespaceKey namespaceKey =
                  new NamespaceKey(nameSpaceContainer.getFullPathList());
  ...
  private Iterable<Map.Entry<NamespaceKey, NameSpaceContainer>> getModifiedNamespaceContainers(
    ...
    queries.add(
        newRangeLong(
            NamespaceIndexKeys.LAST_MODIFIED.getIndexFieldName(),
            previousWakeupTime,
            Long.MAX_VALUE,
            true,
            false));
    ...
    queries.addAll(
        collaborationTagMap.keySet().stream()
  ...
  private Map<String, CollaborationTag> getModifiedTags(long previousWakeupTime) {
    ...
    final SearchQuery modifiedTagsQuery =
        newRangeLong(
            CollaborationTagStore.LAST_MODIFIED.getIndexFieldName(),
            previousWakeupTime,
            Long.MAX_VALUE,
            true,
            false);
    final LegacyFindByCondition newTags =
        new LegacyFindByCondition().setCondition(modifiedTagsQuery);

    final Iterable<Map.Entry<String, CollaborationTag>> changedTags =
        collaborationTagStore.find(newTags);
    StreamSupport.stream(changedTags.spliterator(), false)
        .forEach(
            input -> {
  ...
  public static final class NamespaceSearchConverter
      implements DocumentConverter<String, SearchContainer> {
    ...
    public void convert(DocumentWriter writer, String id, SearchContainer record) {
      ...
      writer.write(
          PATH_UNQUOTED_LC, fullPathList.stream().map(String::toLowerCase).toArray(String[]::new));
      ...
      if (collaborationTag != null && collaborationTag.getTagsList() != null) {
        ...
        writer.write(
            TAGS_LC,
            collaborationTag.getTagsList().stream()
  ...
  SearchConfiguration getSearchConfig() {
    ...
    if (configurationEntry != null) {
      ProtostuffIOUtil.mergeFrom(
          configurationEntry.getValue().toByteArray(),
          searchConfiguration,
          SearchConfiguration.getSchema());
    } else {
  ...
  private static ByteString convertSearchConfigurationToByteString(
      SearchConfiguration configuration) {
    final LinkedBuffer buffer = LinkedBuffer.allocate();
    byte[] bytes =
        ProtostuffIOUtil.toByteArray(configuration, SearchConfiguration.getSchema(), buffer);
