...
public class LogInLogOutResource {
  private static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(LogInLogOutResource.class);
  ...
  public Response login(UserLogin userLogin, @Context HttpServletRequest request) {
    try {
      if (userLogin == null
          || Strings.isNullOrEmpty(userLogin.getUserName())
          || Strings.isNullOrEmpty(userLogin.getPassword())) {
      ...
      final TokenDetails tokenDetails =
          tokenManager.createToken(userLogin.getUserName(), clientAddress);
      ...
      try {
        ...
        CatalogServiceHelper.ensureUserHasHomespace(
            ns, userConfig.getUserName(), dContext.getOptionManager());
      } catch (NamespaceException ex) {
        logger.error("Failed to make sure the user has home space setup.", ex);
        return Response.status(Status.INTERNAL_SERVER_ERROR)
            .entity(new GenericErrorMessage(ex.getMessage()))
            .build();
      }
  ...
  @Secured
  /**
   * This method serves for following purposes: 1) Check if current user is still authenticated 2)
   * If any user exists. See {@link com.dremio.dac.server.NoUserFilter}
   */
