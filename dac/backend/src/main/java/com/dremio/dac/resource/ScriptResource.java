...
public class ScriptResource {
  ...
  public ScriptData postScripts(ScriptData scriptData) {
    try {
      return fromScript(scriptService.createScript(ScriptData.toScriptRequest(scriptData, false)));
  ...
  public ScriptData updateScript(@PathParam("id") String scriptId, ScriptData scriptData) {
    ...
    try {
      // update the script
      return fromScript(
          scriptService.updateScript(scriptId, ScriptData.toScriptRequest(scriptData, true)));
    } catch (ScriptNotFoundException exception) {
      logger.error(exception.getMessage(), exception);
      throw new NotFoundException(exception.getMessage());
    } catch (UserNotFoundException | DuplicateScriptNameException exception) {
      logger.error(exception.getMessage(), exception);
      throw new BadRequestException(exception.getMessage());
    } catch (ScriptNotAccessible exception) {
      logger.error(exception.getMessage(), exception);
