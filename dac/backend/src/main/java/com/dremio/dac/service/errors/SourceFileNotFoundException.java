...
public class SourceFileNotFoundException extends NotFoundException {
  ...
  public SourceFileNotFoundException(SourceName soureName, SourceFilePath path, Exception error) {
    super(new SourceFileResourcePath(soureName, path), "file " + path.toPathString(), error);
    this.path = path;
