 ...
 *
 * <p>This daemon currently this manages the Dremio Web UI, launching an embedded Zookeeper instance
 * and launching a SabotNode.
 */
public final class DACDaemon implements AutoCloseable {
  ...
  private DACDaemon(
    ...
    final DremioConfig config =
        incomingConfig
            .withSabotValue(
                ExecConstants.ZK_CONNECTION,
                incomingConfig.getString(DremioConfig.ZOOKEEPER_QUORUM))
            .withSabotValue(
                DremioClient.INITIAL_USER_PORT,
                incomingConfig.getString(DremioConfig.CLIENT_PORT_INT))
            .withSabotValue(
                ExecConstants.SPILL_DIRS, incomingConfig.getList(DremioConfig.SPILLING_PATH_STRING))
            .withSabotValue(
                ExecConstants.REGISTRATION_ADDRESS,
                incomingConfig.getString(DremioConfig.REGISTRATION_ADDRESS))
            .withSabotValue(
                ExecConstants.ZK_SESSION_TIMEOUT,
                incomingConfig.getString(DremioConfig.ZK_CLIENT_SESSION_TIMEOUT))
            .withSabotValue(
                ExecConstants.ZK_RETRY_UNLIMITED,
                incomingConfig.getString(DremioConfig.ZK_CLIENT_RETRY_UNLIMITED))
            .withSabotValue(
                ExecConstants.ZK_CONNECTION_HANDLE_ENABLED,
                incomingConfig.getString(DremioConfig.ZK_CONNECTION_HANDLE_ENABLED))
            .withSabotValue(
                ExecConstants.ZK_RETRY_LIMIT,
                incomingConfig.getString(DremioConfig.ZK_CLIENT_RETRY_LIMIT))
            .withSabotValue(
                ExecConstants.ZK_INITIAL_TIMEOUT_MS,
                incomingConfig.getString(DremioConfig.ZK_CLIENT_INITIAL_TIMEOUT_MS))
            .withSabotValue(
                ExecConstants.ZK_SUPERVISOR_INTERVAL_MS,
                incomingConfig.getString(DremioConfig.ZK_SUPERVISOR_INTERVAL_MS))
            .withSabotValue(
                ExecConstants.ZK_SUPERVISOR_READ_TIMEOUT_MS,
                incomingConfig.getString(DremioConfig.ZK_SUPERVISOR_READ_TIMEOUT_MS))
            .withSabotValue(
                ExecConstants.ZK_SUPERVISOR_MAX_FAILURES,
                incomingConfig.getString(DremioConfig.ZK_SUPERVISOR_MAX_FAILURES));
    ...
    logger.info(
        "Dremio daemon write path: " + config.getString(DremioConfig.LOCAL_WRITE_PATH_STRING));
    ...
    if (isMaster || isMasterless) {
      ...
      registry =
          new NonMasterSingletonRegistry(bootstrapRegistry.provider(MasterStatusListener.class));
    }
  ...
  /**
   * Closes this daemon on JVM shutdown. By invoking this method, the caller is delegating the
   * responsibility of invoking {@link #close} to the JVM. Although this method is idempotent, the
  ...
  /** Notify that daemon is being shut down. Code waiting on awaitClose() will be released */
  ...
  public static DACDaemon newDremioDaemon(DACConfig dacConfig, ScanResult scanResult)
      throws IOException {
    return newDremioDaemon(dacConfig, scanResult, new DACDaemonModule());
  }

  public static DACDaemon newDremioDaemon(
      DACConfig dacConfig, ScanResult scanResult, DACModule dacModule, DremioServer server)
      throws IOException {
  ...
  public static DACDaemon newDremioDaemon(
      DACConfig dacConfig, ScanResult scanResult, DACModule dacModule) throws IOException {
  ...
  public static void main(final String[] args) throws Exception {
    try (TimedBlock b = Timer.time("main")) {
      ...
      DACDaemon daemon =
          newDremioDaemon(
              config, ClassPathScanner.fromPrescan(config.getConfig().getSabotConfig()));
      daemon.init();
