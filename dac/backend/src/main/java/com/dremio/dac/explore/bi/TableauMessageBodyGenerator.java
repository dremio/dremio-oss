...
public class TableauMessageBodyGenerator extends BaseBIToolMessageBodyGenerator {
  ...
  @VisibleForTesting static final String TABLEAU_VERSION = "18.1";

  private static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(TableauMessageBodyGenerator.class);
  private static final Map<String, String> CUSTOMIZATIONS =
      ImmutableMap.<String, String>builder()
          // Keep this map in sync with the TDC files included with the macOS and Windows
          // installers.
          // macOS:
          // https://github.com/dremio/odbc/blob/master/osx/DremioODBC-pkgbuild/Library/Dremio/ODBC/Resources/DremioConnector.tdc
          // Windows x86:
          // https://github.com/dremio/odbc/blob/master/windows/x86/deploy/Resources/DremioConnector.tdc
          // Windows x64:
          // https://github.com/dremio/odbc/blob/master/windows/x64/deploy/Resources/DremioConnector.tdc
          ...
          // TODO: DX-25509 - Investigate the additional customizations in the
          // TableauMessageBodyGenerator
  ...
  /**
   * Option to add extra connection properties to ODBC string. Should follow ODBC connection string
   * format.
   */
  public static final StringValidator EXTRA_CONNECTION_PROPERTIES =
      new StringValidator("export.tableau.extra-odbc-connection-properties", "");

  /**
   * Option to add extra connection properties to NATIVE connection. Should follow JDBC connection
   * string format.
   */
  public static final StringValidator EXTRA_NATIVE_CONNECTION_PROPERTIES =
      new StringValidator("export.tableau.extra-native-connection-properties", "");

  /**
   * Option to add extra connection properties to FLIGHT connection. Should follow JDBC connection
   * string format.
   */
  public static final StringValidator EXTRA_FLIGHT_CONNECTION_PROPERTIES =
      new StringValidator("export.tableau.extra-flight-connection-properties", "");
  ...
  public static final EnumValidator<TableauExportType> TABLEAU_EXPORT_TYPE =
      new EnumValidator<>(
          "export.tableau.export-type", TableauExportType.class, TableauExportType.NATIVE);
  ...
  @Inject
  public TableauMessageBodyGenerator(
    ...
    this.customizationEnabled =
        MoreObjects.firstNonNull((Boolean) configuration.getProperty(CUSTOMIZATION_ENABLED), false);
  }
  ...
  public void writeTo(
    ...
    try {
      final XMLStreamWriter xmlStreamWriter =
          xmlOutputFactory.createXMLStreamWriter(entityStream, "UTF-8");
  ...
  private void writeDatasource(
    ...
    if (WebServer.MediaType.APPLICATION_TDS_TYPE.equals(mediaType)) {
      // The EnumValidator lower-cases the enum for some reason, so we upper case it to match our
      // enum values again.
      if (TableauExportType.NATIVE
          == TableauExportType.valueOf(
              getOptionManager().getOption(TABLEAU_EXPORT_TYPE).toUpperCase(Locale.ROOT))) {
        writeSdkConnection(xmlStreamWriter, datasetConfig, hostname, TableauExportType.NATIVE);
      } else if (TableauExportType.FLIGHT
          == TableauExportType.valueOf(
              getOptionManager().getOption(TABLEAU_EXPORT_TYPE).toUpperCase(Locale.ROOT))) {
  ...
  private void writeColumnAliases(XMLStreamWriter xmlStreamWriter, DatasetConfig datasetConfig)
      throws XMLStreamException {
    ...
    for (final Field field : fields) {
      ...
      final TableauColumnMetadata tableauColumnMetadata =
          new TableauColumnMetadata(field.getType().getTypeID(), fieldName);
  ...
  private void writeSdkConnection(
      XMLStreamWriter xmlStreamWriter,
      DatasetConfig datasetConfig,
      String hostname,
      TableauExportType tableauExportType)
      throws XMLStreamException {
    ...
    String port =
        tableauExportType.equals(TableauExportType.FLIGHT)
  ...
  private void writeOdbcConnection(
      XMLStreamWriter xmlStreamWriter, DatasetConfig datasetConfig, String hostname)
      throws XMLStreamException {
  ...
  private void writeNativeDrillConnection(
      XMLStreamWriter xmlStreamWriter, DatasetConfig datasetConfig, String hostname)
      throws XMLStreamException {
    DatasetPath dataset = new DatasetPath(datasetConfig.getFullPathList());
  ...
  private void writeRelation(XMLStreamWriter xmlStreamWriter, DatasetConfig datasetConfig)
      throws XMLStreamException {
    DatasetPath dataset = new DatasetPath(datasetConfig.getFullPathList());
    ...
    xmlStreamWriter.writeAttribute(
        "table", format("[%s].[%s]", tableauEncode(schema), tableauEncode(name)));
    xmlStreamWriter.writeEndElement();
  }

  private void writeConnectionCustomization(XMLStreamWriter xmlStreamWriter)
      throws XMLStreamException {
  ...
  private void writeTableauMetadata(XMLStreamWriter xmlStreamWriter, DatasetConfig datasetConfig)
      throws XMLStreamException {
    final List<Field> fields = getFieldsFromDatasetConfig(datasetConfig);
    if (fields == null) {
      return;
    ...
    for (final Field field : fields) {
      ...
      writeElement(
          xmlStreamWriter, "local-type", TableauColumnMetadata.getTableauDataType(fieldId));
  ...
  private void writeElement(XMLStreamWriter xmlStreamWriter, String name, String contents)
      throws XMLStreamException {
  ...
  static class TableauColumnMetadata {
    @VisibleForTesting static final String TABLEAU_TYPE_ORDINAL = "ordinal";
    @VisibleForTesting static final String TABLEAU_TYPE_QUANTITATIVE = "quantitative";
    @VisibleForTesting static final String TABLEAU_TYPE_NOMINAL = "nominal";
    // Splits the string based on delimiter underscore or whitespace, while including the delimiter
    // in the tokens.
    private static final Pattern COLUMN_NAME_SPLIT_REGEX_PATTERN =
        Pattern.compile("((?<=_)|(?=_)|(?<=\\s)|(?=\\s))");
    ...
    static final Map<ArrowTypeID, String> ARROW_TABLEAU_DATATYPES_MAPPINGS =
        ImmutableMap.<ArrowTypeID, String>builder()
    ...
    static final List<String> TABLEAU_ROLE_DIMENSION_STARTS_WITH_KEYWORDS =
    ...
    static final List<String> TABLEAU_ROLE_DIMENSION_ENDS_WITH_KEYWORDS =
        new ArrayList<String>() {
          {
            add("code");
            add("id");
    ...
    /**
     * Converts the column name to a pretty column name with the following rules: Non-trailing and
     * non-leading underscore are replaced with whitespace. Leading, trailing and multiple
     * underscore remain as is. Leading and trailing whitespace are removed. Character after a
     * whitespace or underscore is capitalized. Converts everything to CamelCase. Ignoring rules
     * about all CAPS as separate word. Adds a space for CamelCase separation E.g. ThisCase becomes
     * “This Case”. All other rules are ignored.
    ...
    static String getPrettyColumnName(String columnName) {
      ...
      // COLUMN_NAME_SPLIT_REGEX_PATTERN Splits the trimmed column name into tokens based on
      // delimiters
    ...
    /**
     * Processes spaces and underscores based on their position in the column name and on the basis
     * of the rules defined below: Non-trailing and non-leading underscore are replaced with
     * whitespace. Leading, trailing and multiple underscore remain as is. The character after a
     * whitespace or underscore is capitalized.
     *
     * @param tokens The column name split into tokens, based on the delimiters
     * @return The column where the spaces and the underscores have been processed as per the rules.
     */
    private static String processWhitespaceAndUnderscore(String[] tokens) {
      ...
      for (int i = 0; i < tokens.length; i++) {
        ...
        if ("_".equals(currentToken)) {
          ...
          prettyColumnName
              .append(currentToken.substring(0, 1).toUpperCase())
    ...
    /**
     * Adds spaces to the column name if it contains CamelCase, otherwise converts the column name
     * to CamelCase as per the rules below: Converts everything to CamelCase. Ignoring rules about
     * all CAPS as separate word. Adds a space for CamelCase separation E.g. ThisCase becomes “This
     * Case”.
     *
      ...
      for (int i = 0; i < columnName.length(); i++) {
        ...
        if (i == 0) {
          ...
          if (isLowerCase(columnName.charAt(i - 1)) && isUpperCase(currentChar)) {
            stringBuilder.append(lowercaseBuffer.toString()).append(" ").append(currentChar);
            // Clear the builder.
    ...
    /**
     * Returns the Tableau Role for a particular column with the given ArrowType. Dates and text
     * values are dimensions, and numbers are measures.
    ...
    static TableauRole getTableauRole(ArrowTypeID fieldID, String fieldName) {
      if ((fieldID == ArrowTypeID.Int
              || fieldID == ArrowTypeID.Decimal
              || fieldID == ArrowTypeID.FloatingPoint)
    ...
    /**
     * Determines if the the field should be treated as a Tableau Role Dimension. based on if the
     * column name starts or ends with certain keyword.
     *
    ...
    private static boolean treatFieldAsDimension(String columnName) {
      return (TABLEAU_ROLE_DIMENSION_STARTS_WITH_KEYWORDS.stream()
              .anyMatch(prefix -> columnName.regionMatches(true, 0, prefix, 0, prefix.length()))
          || TABLEAU_ROLE_DIMENSION_ENDS_WITH_KEYWORDS.stream()
              .anyMatch(
                  suffix ->
                      columnName.regionMatches(
                          true,
                          columnName.length() - suffix.length(),
                          suffix,
                          0,
                          suffix.length())));
    }
