...
public class ExtractPosition {
  ...
  /**
   * Generates a position (start and offset) of substring that is matching the given positions in
   * input. Used for generating card examples.
   *
   * <p>Parameters: 1. in : input column 2. start : 1-based start index. If negative, count from the
   * end. 3. length : length of the string.
   */
  @FunctionTemplate(
      name = GEN_EXAMPLE,
    ...
    public void eval() {
      ...
      if (s >= 0 && e >= s) {
        com.dremio.dac.explore.udfs.DremioUDFUtils.writeCardExample(
            out, new com.dremio.dac.proto.model.dataset.CardExamplePosition(s, e - s));
      } else {
        out.rootAsList(); // calling this sets the ComplexWriter as a list type.
      }
    }
