...
public class FragmentWrapper {
  private static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(FragmentWrapper.class);

  private interface FieldAccessor {
    long getValue(MinorFragmentProfile object);
  }

  private static final FieldAccessor firstRunAccessor =
      new FieldAccessor() {
        @Override
  ...
  private static final FieldAccessor wallClockAccessor =
      new FieldAccessor() {
        @Override
        public long getValue(MinorFragmentProfile p) {
  ...
  private static final FieldAccessor sleepAccessor =
      new FieldAccessor() {
        @Override
        public long getValue(MinorFragmentProfile p) {
  ...
  private static final FieldAccessor blockedAccessor =
      new FieldAccessor() {
        @Override
        public long getValue(MinorFragmentProfile p) {
  ...
  public static final String[] FRAGMENT_OVERVIEW_COLUMNS = {
    "Phase",
    "Weight",
    "Thread Reporting",
    "First Start",
  ...
  public void addSummary(TableBuilder tb) {
    try {
      ...
      // Complete iff the fragment profile has at least one operator profile, and start and end
      // times.
      ...
      tb.appendCell(
          complete.size() + " / " + major.getMinorFragmentProfileCount()); // Thread Reporting
      ...
      // TODO(DRILL-3494): Names (maxMem, getMaxMemoryUsed) are misleading; the value is peak memory
      // allocated to fragment
  ...
  private void addFieldStats(
      final TableBuilder tb,
      final Collection<MinorFragmentProfile> fragments,
  ...
  public static final String[] FRAGMENT_COLUMNS_NO_BLOCKED_SPLITS = {
    "Thread ID",
    "Host Name",
    "Start",
    "End",
    ...
    "Last Progress",
    "Peak Memory",
    "Peak Incoming Memory",
    "State",
    "Recent Slice Start Time",
    "Cancel Start Time"
  };

  // same as above but with extra debug columns: "Diff w OPs"
  public static final String[] FRAGMENT_COLUMNS_DEBUG_NO_BLOCKED_SPLITS = {
    "Thread ID",
    "Host Name",
    "Start",
    "End",
  ...
  // same as above but with extra debug columns: "Diff w OPs"
  public static final String[] FRAGMENT_COLUMNS_DEBUG = {
    "Thread ID",
    "Host Name",
    "Start",
    "End",
  ...
  public static final String[] PHASE_METRICS_COLUMNS = {
    "Host Name",
    "Peak Memory",
    "Num Threads",
    "Total Max Records",
  ...
  private void addFragmentInfo(JsonGenerator generator) throws IOException {
    ...
    if (includeDebugColumns) {
      columnHeaders =
          withBlockedSplits ? FRAGMENT_COLUMNS_DEBUG : FRAGMENT_COLUMNS_DEBUG_NO_BLOCKED_SPLITS;
    } else {
    ...
    for (final MinorFragmentProfile minor : complete) {
      ...
      builder.appendString(
          new OperatorPathBuilder().setMajor(major).setMinor(minor).build()); // Thread ID
      ...
      // prior to 1.0.4 runtime was not stored in the profile but rather computed from the other
      // stats
      final long runtime =
          minor.hasRunDuration()
              ? minor.getRunDuration()
              : wallClockTime - waitDuration - blockedDuration;
      ...
      builder.appendFormattedInteger(
          (minor.hasNumSlices() ? minor.getNumSlices() : -1)); // Num Slices
      builder.appendFormattedInteger(
          (minor.hasNumLongSlices() ? minor.getNumLongSlices() : -1)); // Num long slices
      builder.appendFormattedInteger(
          (minor.hasNumShortSlices() ? minor.getNumShortSlices() : -1)); // Num short slices
      ...
      builder.appendBytes(minor.getMaxIncomingMemoryUsed());
      // Peak memory for incoming buffers
      builder.appendString(minor.getState().name());
      if (minor.hasRecentSliceStartTime()) {
        builder.appendTime(minor.getRecentSliceStartTime());
      } else {
        builder.appendString("");
      }
      if (minor.hasCancelStartTime()) {
        builder.appendTime(minor.getCancelStartTime());
      } else {
        builder.appendString("");
      }
      builder.endEntry();
  ...
  private void addMetrics(JsonGenerator generator) throws IOException {
    ...
    final List<NodePhaseProfile> nodePhaseProfiles =
        new ArrayList<>(major.getNodePhaseProfileList());
  ...
  public Map<String, NodePhaseProfile> getHostToNodePhaseProfileMap(
      List<NodePhaseProfile> nodePhaseProfiles) {
