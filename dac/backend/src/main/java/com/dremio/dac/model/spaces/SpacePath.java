...
/** Path to space. */
public class SpacePath extends NamespacePath {

  private static final Pattern PARSER = Pattern.compile("/space/([^/]+)");

  public static SpacePath parseUrlPath(String urlPath) {
    Matcher m = PARSER.matcher(urlPath);
    if (m.matches()) {
      return new SpacePath(m.group(0));
    }
    throw new IllegalArgumentException("Not a valid spacePath: " + urlPath);
  }

  public SpacePath(SpaceName spaceName) {
    super(spaceName, Collections.<FolderName>emptyList(), null);
  }

  @JsonCreator
  public SpacePath(String path) {
    super(path);
  }
