...
class TestCloudStoragePathValidator {
  ...
  private static final List<Arguments> folderOrDirectorySuffixArguments = Arrays.asList(
    // Valid
  ...
  private static Stream<Arguments> awsS3BucketNameValidity() {
    return Stream.of(
      ...
      Arguments.of("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa63", true), // Maximum number of characters
  ...
  private static Stream<Arguments> testIsValidAwsS3RootPathParams() {
    ...
    return awsS3BucketNameValidity().flatMap(storageContainerNameArgument ->
      folderOrDirectorySuffixArguments.stream().flatMap(directorySuffixArgument -> {
        final String storageContainerName = (String) storageContainerNameArgument.get()[0];
        final boolean storageContainerNameIsValid = (boolean) storageContainerNameArgument.get()[1];
        final String directorySuffix = (String) directorySuffixArgument.get()[0];
        final boolean directorySuffixIsValid = (boolean) directorySuffixArgument.get()[1];

        final String rootPath = String.format("/%s/%s", storageContainerName, directorySuffix);
        final boolean isValid = storageContainerNameIsValid && directorySuffixIsValid;
  ...
  void testIsValidAzureStorageAccountName(String storageAccountName, boolean isValid) {
    assertThat(CloudStoragePathValidator.isValidAzureStorageAccountName(storageAccountName)).isEqualTo(isValid);
  }
  ...
  void testIsValidAzureStorageContainerName(String storageContainerName, boolean isValid) {
    assertThat(CloudStoragePathValidator.isValidAzureStorageContainerName(storageContainerName)).isEqualTo(isValid);
  }
  ...
  private static Stream<Arguments> testIsValidAzureStorageRootPathParams() {
    ...
    return azureStorageContainerNameValidity().flatMap(storageContainerNameArgument ->
      folderOrDirectorySuffixArguments.stream().flatMap(directorySuffixArgument -> {
            final String storageContainerName = (String) storageContainerNameArgument.get()[0];
            final boolean storageContainerNameIsValid = (boolean) storageContainerNameArgument.get()[1];
            final String directorySuffix = (String) directorySuffixArgument.get()[0];
