...
/**
 * This class converts SearchQuery into INFORMATION_SCHEMA SPECIFIC CEL filter that is being used by
 * nessie if searchQuery has the structure something like : Where table_name = "table" and
 * table_schema like '%nessie.folder%" then it will return "(entry.name == "table") && ((name +
 * entry.namespace + entry.name).matches(^.*nessie_folder,*$))"
 */
public final class InformationSchemaCelFilter {
  ...
  private static String getEqualSearchSchema(
      boolean isSchemata, String equalsPattern, String path, String sourceName) {
    ...
    return String.format(
        "(%s) == (%s)",
        path, SearchQueryToCelConversionUtilities.convertToRawCelStringLiteral(equalsPattern));
  }

  private static String getLikeSearchSchema(
      boolean isSchemata, String regexPattern, String sourceName) {
    String regexPatternAsRawCelStringLiteral =
        SearchQueryToCelConversionUtilities.convertToRawCelStringLiteral(regexPattern);
    String sourceNameAsCelRawStringLiteral =
        SearchQueryToCelConversionUtilities.convertToRawCelStringLiteral(sourceName);
    String nameElements =
        sourceNameAsCelRawStringLiteral + " + '.' + entry.namespace + '.' + entry.name";
   ...
   */
  private static String leafSearchQueryToCelFiler(
      SearchQuery searchQuery, boolean isSchemata, String sourceName) {
    ...
    switch (searchQuery.getQueryCase()) {
      case EQUALS:
        ...
        if (fieldName.equals(SEARCH_NAME)) {
          ...
          throw new IllegalStateException(
              String.format(
                  "Field should be SEARCH_NAME or SEARCH_SCHEMA. Provided = %s", fieldName));
        }
      case LIKE:
        String regexPattern =
            SearchQueryToCelConversionUtilities.likeQueryToRe2Regex(searchQuery.getLike());
        fieldName = searchQuery.getLike().getField();
        if (fieldName.equals(SEARCH_NAME)) {
          return String.format(
              "entry.name.matches(%s)",
              SearchQueryToCelConversionUtilities.convertToRawCelStringLiteral(regexPattern));
        } else if (fieldName.equals(SEARCH_SCHEMA)) {
      ...
      default:
        throw new IllegalStateException(
            String.format(
                "SearchQuery should be EQUALS or LIKE. Provided = %s", searchQuery.getQueryCase()));
    }
  }
   ...
   */
  public static String getInformationSchemaFilter(
      SearchQuery searchQuery, boolean includeTableNameInQuery, String sourceName) {
    if (searchQuery == null) {
      return null;
    }
    ...
    switch (searchQuery.getQueryCase()) {
      ...
      default:
        throw new IllegalStateException(
            String.format(
                "SearchQuery should be one of the following to get celFilter: EQUALS, LIKE, AND, OR. Provided = %s",
  ...
  /**
   * @return checking if we need to add entry.namespace in our cel filter. In schemata, we should
   *     include entry.namespace in our celFilter if and only if we have three elements in our
   *     value. for example, if we have path that is source.folder1.folder2, which has three
   *     elements, the celFilter should be source.<entry.namespace><entry.name>, while if we have
   *     path that is source.folder1, which has two elements, the celFilter should be
   *     source.folder1, which is source.<entry.name>
   */
   ...
   * There are several cases we should handle: if we are not looking for a source and if we need to
   * include name of the entry,
   */
