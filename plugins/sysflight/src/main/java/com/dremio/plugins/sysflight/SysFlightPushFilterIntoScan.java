...
/**
 * Implements filter pushdown (if filters are present) of all the filters and a fall back mechanism
 * in case the required columns are not indexed in the service exposing the System Table
 */
public abstract class SysFlightPushFilterIntoScan extends RelOptRule {
  private static final org.slf4j.Logger LOGGER =
      org.slf4j.LoggerFactory.getLogger(SysFlightPushFilterIntoScan.class);

  public static final RelOptRule IS_FILTER_ON_PROJECT =
      new SysFlightPushFilterIntoScan(
          RelOptHelper.some(
              FilterPrel.class,
              RelOptHelper.some(ProjectPrel.class, RelOptHelper.any(SysFlightScanPrel.class))),
  ...
  public static final RelOptRule IS_FILTER_ON_SCAN =
      new SysFlightPushFilterIntoScan(
          RelOptHelper.some(FilterPrel.class, RelOptHelper.any(SysFlightScanPrel.class)),
  ...
  protected void doMatch(
      RelOptRuleCall call, SysFlightScanPrel scan, ProjectPrel project, FilterPrel filter) {
    if (scan.hasFilter()) {
      return;
    }

    ExpressionConverter.PushdownResult result =
