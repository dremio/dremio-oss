...
public abstract class SysFlightPushFilterIntoScan extends RelOptRule {
  ...
  protected void doMatch(RelOptRuleCall call, SysFlightScanPrel scan, ProjectPrel project, FilterPrel filter) {
    ...
    RelNode input = new SysFlightScanPrel(scan.getCluster(), scan.getTraitSet(), scan.getTable(), scan.getTableMetadata(), result.getQuery(), scan.getProjectedColumns(), scan.getObservedRowcountAdjustment(), scan.getHints(), scan.getPlugin(), scan.getRuntimeFilters());
