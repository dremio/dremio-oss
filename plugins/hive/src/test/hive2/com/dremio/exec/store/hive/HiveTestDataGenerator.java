...
public final class HiveTestDataGenerator {
  ...
  public static final String HIVE_TEST_PLUGIN_NAME_WITH_WHITESPACE =
      "hive plugin name with whitespace";
  ...
  private static synchronized HiveTestDataGenerator getInstance(Map<String, String> cfg)
      throws Exception {
  ...
  public static synchronized HiveTestDataGenerator newInstanceWithoutTestData(
      String warehouseLocation, String metastoreLocation, Map<String, String> cfg)
      throws Exception {
    return new HiveTestDataGenerator(warehouseLocation, metastoreLocation, cfg);
  }
  ...
  private HiveTestDataGenerator(Map<String, String> cfg) throws Exception {
    ...
    this(getTempDir("warehouse"), HiveTestBase.createDerbyDB(getTempDir("metastore_db")), cfg);
  }

  private HiveTestDataGenerator(
      String warehouseLocation, String metastoreLocation, Map<String, String> cfg)
      throws Exception {
    this.whDir = warehouseLocation;

    final HiveConf conf = new HiveConf();
    this.dbDir = metastoreLocation;
   ...
   * @throws Exception if fails to update or no Hive plugin currently exists in given plugin
   *     registry.
   */
  public void updatePluginConfig(
    ...
    Hive2StoragePluginConfig conf =
        (Hive2StoragePluginConfig)
            msp.getId().<Hive2StoragePluginConfig>getConnectionConf().clone();
  ...
  public void generateTestData(java.util.function.Function<Driver, Void> generatorFunction)
      throws Exception {
    try (DriverState driverState = new DriverState(newHiveConf())) {
  ...
  private void generateTestData() throws Exception {
    try (DriverState driverState = new DriverState(newHiveConf())) {
      ...
      // Create a (key, value) schema table in non-default database with RegexSerDe which is
      // available in hive-contrib.jar
      ...
      executeQuery(
          hiveDriver,
          "CREATE TABLE IF NOT EXISTS partitioned_empty_table(a INT, b STRING) PARTITIONED BY (p INT)");
      ...
      final String parquetUpperSchemaTable =
          "create external table parquet_region(R_REGIONKEY bigint, R_NAME string, R_COMMENT string) "
              + "stored as parquet location '"
              + file.getParent()
              + "'";
      executeQuery(hiveDriver, parquetUpperSchemaTable);

      final String parquetschemalearntest =
          "create external table parquetschemalearntest(R_REGIONKEY bigint) "
              + "stored as parquet location '"
              + file.getParent()
              + "'";
      ...
      final String parquetWithTwoFilesTable =
          "create external table parquet_with_two_files(R_REGIONKEY bigint, R_NAME string, R_COMMENT string) "
              + "stored as parquet location '"
              + file1Of2.getParent()
              + "'";
      executeQuery(hiveDriver, parquetWithTwoFilesTable);

      final String orcRegionTable =
          "create table orc_region stored as orc as SELECT * FROM parquet_region";
      ...
      final String orcWithTwoFilesTable =
          "create external table orc_with_two_files(R_REGIONKEY bigint, R_NAME string, R_COMMENT string) "
              + "stored as orc location '"
              + fileOrc1Of2.getParent()
              + "'";
      executeQuery(hiveDriver, orcWithTwoFilesTable);

      final String orcStringsTable =
          "create table orc_strings (key int, country_char25 CHAR(25), "
              + "country_string string, country_varchar VARCHAR(1000), continent_char25 CHAR(25)) stored as orc";
      final String insert1 =
          "insert into orc_strings values (1, 'INDIA', 'CHINA', 'NEPAL', 'ASIA')";
      final String insert2 =
          "insert into orc_strings values (2, 'INDONESIA', 'THAILAND', 'SINGAPORE', 'ASIA')";
      final String insert3 =
          "insert into orc_strings values (3, 'FRANCE', 'ITALY', 'ROMANIA', 'EUROPE')";
      ...
      executeQuery(
          hiveDriver,
          "CREATE TABLE IF NOT EXISTS default.dummy(key INT, value STRING) "
              + "ROW FORMAT DELIMITED FIELDS TERMINATED BY ',' STORED AS TEXTFILE");
      executeQuery(
          hiveDriver, "LOAD DATA LOCAL INPATH '" + dummy + "' OVERWRITE INTO TABLE default.dummy");
      executeQuery(
          hiveDriver,
          "CREATE TABLE parquet_timestamp_nulls(a TIMESTAMP, b STRING) stored as parquet");
      executeQuery(
          hiveDriver,
          "INSERT INTO TABLE parquet_timestamp_nulls SELECT null, 'a' FROM default.dummy");
      ...
      executeQuery(
          hiveDriver,
          "INSERT OVERWRITE TABLE partition_with_few_schemas PARTITION(c, d, e) "
              + "SELECT a, b, c, d, e FROM partition_pruning_test_loadtable");
      executeQuery(
          hiveDriver,
          "alter table partition_with_few_schemas partition(c=1, d=1, e=1) change a a1 INT");
      executeQuery(
          hiveDriver,
          "alter table partition_with_few_schemas partition(c=1, d=1, e=2) change a a1 INT");
      executeQuery(
          hiveDriver,
          "alter table partition_with_few_schemas partition(c=2, d=2, e=2) change a a1 INT");
      ...
      executeQuery(
          hiveDriver,
          "INSERT INTO TABLE kv_mixedschema PARTITION(part=1) SELECT key, value FROM default.kv LIMIT 2");
      executeQuery(hiveDriver, "ALTER TABLE kv_mixedschema change column key key string");
      executeQuery(
          hiveDriver,
          "INSERT INTO TABLE kv_mixedschema PARTITION(part=2) "
              + "SELECT key, value FROM default.kv ORDER BY key DESC LIMIT 2");

      executeQuery(
          hiveDriver,
          "CREATE TABLE sorted_parquet(id int, key int) clustered by (id) sorted by (key) into 10 buckets stored as Parquet");

      executeQuery(
          hiveDriver,
          "INSERT INTO TABLE sorted_parquet select key as id, key as key from kv_parquet distribute by id sort by key");

      // Create a StorageHandler based table (DRILL-3739)
      executeQuery(
          hiveDriver,
          "CREATE TABLE kv_sh(key INT, value STRING) STORED BY "
      ...
      executeQuery(
          hiveDriver, generateTestDataWithHeadersAndFooters("skipper.kv_text_small", 5, 1, 1));

      executeQuery(
          hiveDriver,
          createTableWithHeaderFooterProperties("skipper.kv_text_large", "textfile", "2", "2"));
      executeQuery(
          hiveDriver, generateTestDataWithHeadersAndFooters("skipper.kv_text_large", 5000, 2, 2));

      executeQuery(
          hiveDriver,
          createTableWithHeaderFooterProperties(
              "skipper.kv_incorrect_skip_header", "textfile", "A", "1"));
      executeQuery(
          hiveDriver,
      ...
      executeQuery(
          hiveDriver,
          createTableWithHeaderFooterProperties(
              "skipper.kv_incorrect_skip_footer", "textfile", "1", "A"));
      executeQuery(
          hiveDriver,
      ...
      executeQuery(
          hiveDriver,
          "insert into table skipper.kv_rcfile_large select * from skipper.kv_text_large");
      ...
      executeQuery(
          hiveDriver,
          "insert into table skipper.kv_parquet_large select * from skipper.kv_text_large");

      // Create sequencefile table with skip header and footer table property
      executeQuery(
          hiveDriver,
          createTableWithHeaderFooterProperties(
              "skipper.kv_sequencefile_large", "sequencefile", "1", "1"));
      executeQuery(
          hiveDriver,
      ...
      executeQuery(
          hiveDriver, "load data local inpath '" + simpleJson + "' into table default.simple_json");
      ...
      createDecimalParquetTableWithDecimalColumnMismatch(
          hiveDriver, "parquet_varchar_to_decimal_with_filter");
      ...
      // This test requires a systemop alteration. Refresh metadata on hive seems to timeout the
      // test preventing re-use of an existing table. Hence, creating a new table.
      createParquetDecimalSchemaChangeFilterTestTable(
          hiveDriver, "test_nonvc_parqdecimalschemachange_table");
    }
  }

  private void createTableWithMapOfIntKey(Driver hiveDriver, String format) {
    String createTable =
        "create table map_of_int_" + format + " (col1 map<int,string>) stored as " + format;
  ...
  private void createTableWithMapOfBigIntKey(Driver hiveDriver, String format) {
    String createTable =
        "create table map_of_bigint_" + format + " (col1 map<bigint,string>) stored as " + format;
    String insert =
        "insert into map_of_bigint_"
            + format
            + " select map(9223372036854775800, 'value1',9223372036854775801,'value2') ";
  ...
  private void createTableWithMapOfBooleanKey(Driver hiveDriver, String format) {
    String createTable =
        "create table map_of_boolean_" + format + " (col1 map<boolean,string>) stored as " + format;
    String insert =
        "insert into map_of_boolean_" + format + " select map(true, 'value1',false,'value2') ";
  ...
  private void createTableWithMapOfDateKey(Driver hiveDriver, String format) {
    String createTable =
        "create table map_of_date_" + format + " (col1 map<date,string>) stored as " + format;
    String insert =
        "insert into map_of_date_"
            + format
            + " select map(cast('1993-05-26' as date), 'value1',cast('1993-05-27' as date),'value2') ";
  ...
  private void createTableWithMapOfDecimalKey(Driver hiveDriver, String format) {
    String createTable =
        "create table map_of_decimal_"
            + format
            + " (col1 map<decimal(3,2),string>) stored as "
            + format;
    String insert =
        "insert into map_of_decimal_"
            + format
            + " select map(cast(1.1 as decimal(3,2)), 'value1', cast(1.2 as decimal(3,2)),'value2') ";
  ...
  private void createTableWithMapOfDoubleKey(Driver hiveDriver, String format) {
    String createTable =
        "create table map_of_double_" + format + " (col1 map<double,string>) stored as " + format;
    String insert =
        "insert into map_of_double_"
            + format
            + " select map(cast(1.10 as double), 'value1',cast(1.20 as double),'value2') ";
  ...
  private void createTableWithMapOfFloatKey(Driver hiveDriver, String format) {
    String createTable =
        "create table map_of_float_" + format + " (col1 map<float,string>) stored as " + format;
    String insert =
        "insert into map_of_float_"
            + format
            + " select map(cast(1.1 as float), 'value1',cast(1.2 as float),'value2') ";
  ...
  private void createTableWithMapOfStringKey(Driver hiveDriver, String format) {
    String createTable =
        "create table map_of_string_" + format + " (col1 map<string,string>) stored as " + format;
    String insert =
        "insert into map_of_string_" + format + " select map('key1', 'value1','key2','value2') ";
  ...
  private void createTableWithMapOfTimestampKey(Driver hiveDriver, String format) {
    String createTable =
        "create table map_of_timestamp_"
            + format
            + " (col1 map<timestamp,string>) stored as "
            + format;
    String insert =
        "insert into map_of_timestamp_"
            + format
            + " select map(cast('1993-05-26 11:12:33' as timestamp), 'value1',cast('1993-05-26 11:12:35' as timestamp),'value2') ";
  ...
  private void createTableWithMapOfVarbinaryKey(Driver hiveDriver, String format) {
    String createTable =
        "create table map_of_varbinary_"
            + format
            + " (col1 map<binary,string>) stored as "
            + format;
    String insert =
        "insert into map_of_varbinary_" + format + " select map(cast('1234' as binary), 'value1') ";
  ...
  private void createTableWithMapOfNullValues(Driver hiveDriver, String format) {
    String createTable =
        "create table map_of_null_values_"
            + format
            + " (col1 map<string,string>) stored as "
            + format;
  ...
  private void createTableWithMapOfList(Driver hiveDriver, String format) {
    String createTable =
        "create table map_of_list_values_"
            + format
            + " (col1 map<string,array<int>>) stored as "
            + format;
  ...
  private void createTableWithMapOfStruct(Driver hiveDriver, String format) {
    String createTable =
        "create table map_of_struct_values_"
            + format
            + " (col1 map<string,struct<f1:int>>) stored as "
            + format;
    String insert =
        "insert into map_of_struct_values_" + format + " select map('key1',named_struct('f1',1)) ";
  ...
  private void createTableWithMapOfMap(Driver hiveDriver, String format) {
    String createTable =
        "create table map_of_map_values_"
            + format
            + " (col1 map<string,map <string, int>>) stored as "
            + format;
    String insert =
        "insert into map_of_map_values_" + format + " select map('key1',map('innerKey1',1)) ";
  ...
  private void createOrcStringTableWithComplexTypes(final Driver hiveDriver) {
    final String orcStringsTable =
        "create table orc_strings_complex (list_col array<int>, "
            + "struct_col struct<f1: int>, map_col map<int, int>, key int, country_char25 CHAR(25), "
            + "country_string string, country_varchar VARCHAR(1000), continent_char25 CHAR(25)) stored as orc";
    final String insert1 =
        "insert into orc_strings_complex select array(1), named_struct('f1', 1), map(1,1), 1, 'INDIA', 'CHINA', 'NEPAL', 'ASIA'";
    final String insert2 =
        "insert into orc_strings_complex select array(1), named_struct('f1', 1), map(1,1), 2, 'INDONESIA', 'THAILAND', 'SINGAPORE', 'ASIA'";
    final String insert3 =
        "insert into orc_strings_complex select array(1), named_struct('f1', 1), map(1,1), 3, 'FRANCE', 'ITALY', 'ROMANIA', 'EUROPE'";
    executeQuery(hiveDriver, orcStringsTable);
    executeQuery(hiveDriver, insert1);
    executeQuery(hiveDriver, insert2);
    executeQuery(hiveDriver, insert3);
  ...
  private void createParquetDecimalSchemaChangeTestTable(
      final Driver hiveDriver, final String table) throws Exception {
    String createParqetTableCmd =
        "CREATE TABLE " + table + " (col1 decimal(5,2)) STORED AS " + "PARQUET";
  ...
  private void createParquetDecimalSchemaChangeFilterTestTable(
      final Driver hiveDriver, final String table) {
    String createParqetTableCmd =
        "CREATE TABLE "
            + table
            + " (schema_mismatch_col decimal(8,5), value_in_parquet decimal(8,5)) STORED AS "
            + "PARQUET";
    ...
    String alterTableCmd =
        "ALTER TABLE " + table + " CHANGE schema_mismatch_col schema_mismatch_col decimal(5,2)";
    executeQuery(hiveDriver, createParqetTableCmd);
    executeQuery(hiveDriver, insertDataCmd);
  ...
  private void createPartitionDecimalOverflow(final Driver hiveDriver, final String table)
      throws Exception {
    String createParqetTableCmd =
        "CREATE TABLE "
            + table
            + " (col1 int) partitioned by (col2 decimal(20, 2)) STORED AS PARQUET";
    String insertData =
        "INSERT INTO TABLE " + table + " PARTITION(col2=123456789101214161.12) VALUES(202)";

    String createParquetExtTable =
        "CREATE EXTERNAL TABLE "
            + table
            + "_ext"
            + " (col1 int) partitioned by (col2 decimal(15,3))"
            ...
            + "'";
    String insertData1 =
        "INSERT INTO TABLE " + table + "_ext" + " PARTITION(col2=123456789.120) VALUES(2202)";
    String insertData2 =
        "INSERT INTO TABLE " + table + "_ext" + " PARTITION(col2=123456789101.123) VALUES(234)";
    String insertData3 =
        "INSERT INTO TABLE " + table + "_ext" + " PARTITION(col2=123456789101.123) VALUES(154)";
    String insertData4 =
        "INSERT INTO TABLE " + table + "_ext" + " PARTITION(col2=123456789102.123) VALUES(184)";
    String insertData5 =
        "INSERT INTO TABLE " + table + "_ext" + " PARTITION(col2=15.300) VALUES(153)";
  ...
  private void createParquetVarcharTable(
      final Driver hiveDriver, final String table, final URL resourceUrl) throws Exception {
    ...
    String fixedLenVarchar =
        "create external table "
            + table
            + " (R_NAME varchar(6)) stored as parquet location '"
            + parquetFile.getParent()
            + "'";
    executeQuery(hiveDriver, fixedLenVarchar);
  }

  private void createParquetVarcharTableNoTruncationReqd(
      final Driver hiveDriver, final String table, final URL resourceUrl) throws Exception {
    File parquetDir = new File(getWhDir(), "parq_varchar");
    parquetDir.mkdirs();
    File parquetFile = new File(parquetDir, "region_varchar.parquet");
    ...
    String fixedLenVarchar =
        "create external table "
            + table
            + " (R_NAME varchar(50)) stored as parquet location '"
            + parquetFile.getParent()
            + "'";
    executeQuery(hiveDriver, fixedLenVarchar);
  }

  private void createParquetCharTable(
      final Driver hiveDriver, final String table, final URL resourceUrl) throws Exception {
    ...
    String fixedLenChar =
        "create external table "
            + table
            + " (R_NAME char(6)) stored as parquet location '"
            + parquetFile.getParent()
            + "'";
    executeQuery(hiveDriver, fixedLenChar);
  }

  private void createParquetVarcharWithMoreTypesTable(final Driver hiveDriver, final String table)
      throws Exception {
    String createParqetTableCmd =
        "CREATE TABLE "
            + table
  ...
  private void createParquetVarcharWithComplexTypeTable(final Driver hiveDriver, final String table)
      throws Exception {
    String createParqetTableCmd =
        "CREATE TABLE "
            + table
  ...
  private void createPartitionTruncatedVarchar(final Driver hiveDriver, final String table)
      throws Exception {
    String createParqetTableCmd =
        "CREATE TABLE " + table + " (col1 int) partitioned by (col2 varchar(8)) STORED AS PARQUET";
    ...
    String createParquetExtTable =
        "CREATE EXTERNAL TABLE "
            + table
            + "_ext"
            + " (col1 int) partitioned by (col2 varchar(4))"
            + " STORED AS PARQUET LOCATION 'file://"
            + this.getWhDir()
            + "/"
            + table
  ...
  private void createPartitionTruncatedChar(final Driver hiveDriver, final String table)
      throws Exception {
    String createParqetTableCmd =
        "CREATE TABLE " + table + " (col1 int) partitioned by (col2 char(8)) STORED AS PARQUET";
    String insertData = "INSERT INTO TABLE " + table + " PARTITION(col2='abcdefgh') VALUES(100)";
    ...
    String createParquetExtTable =
        "CREATE EXTERNAL TABLE "
            + table
            + "_ext"
            + " (col1 int) partitioned by (col2 char(4))"
            + " STORED AS PARQUET LOCATION 'file://"
            + this.getWhDir()
            + "/"
            + table
  ...
  private void createSimpleListWithNullsHiveTables(final Driver hiveDriver) throws Exception {
    final File simpleListWithNullsParquetDir =
        new File(BaseTestQuery.getTempDir("simplelistwithnullsparquet"));
    simpleListWithNullsParquetDir.mkdirs();
    final URL simpleListWithNullsParquetUrl =
        Resources.getResource("simple_list_with_nulls_test.parquet");
    if (simpleListWithNullsParquetUrl == null) {
      throw new IOException(
          String.format("Unable to find path %s.", "simple_list_with_nulls_test.parquet"));
    }
    ...
    final File simpleListWithNullsParquetFile =
        new File(simpleListWithNullsParquetDir, "simple_list_with_nulls_test.parquet");
    simpleListWithNullsParquetFile.deleteOnExit();
    simpleListWithNullsParquetDir.deleteOnExit();
    Files.write(
        Paths.get(simpleListWithNullsParquetFile.toURI()),
        Resources.toByteArray(simpleListWithNullsParquetUrl));

    // create good table with matching names, types and positions
    final String basicAllMatchingTest =
        "create external table array_simple_with_nulls_test_ext1("
            + "tinyintcol array<tinyint>, "
            + "smallintcol array<smallint>, "
            + "intcol array<int>, "
            + "bigintcol array<bigint>, "
            ...
            + "timestampcol array<timestamp> ) "
            + "stored as parquet location '"
            + simpleListWithNullsParquetFile.getParent()
            + "'";
    executeQuery(hiveDriver, basicAllMatchingTest);

    // create good table with matching names, types and positions
    final String upPromoteWithNulls =
        "create external table array_simple_with_nulls_test_ext2("
  ...
  private void createNestedListWithNullsHiveTables(final Driver hiveDriver) throws Exception {
    final File nestedListWithNullsParquetDir =
        new File(BaseTestQuery.getTempDir("nestedlistwithnullsparquet"));
    ...
    if (nestedListWithNullsParquetUrl == null) {
      throw new IOException(
          String.format("Unable to find path %s.", "list_list_null_test.parquet"));
    }

    // parquet file has following columns with one valid row
    // col1 array<array<array<string> with value [[['a', null], null], null]
    final File nestedListWithNullsParquetFile =
        new File(nestedListWithNullsParquetDir, "list_list_null_test.parquet");
    nestedListWithNullsParquetFile.deleteOnExit();
    nestedListWithNullsParquetDir.deleteOnExit();
    Files.write(
        Paths.get(nestedListWithNullsParquetFile.toURI()),
        Resources.toByteArray(nestedListWithNullsParquetUrl));

    final String nestedListTest =
        "create external table array_nested_with_nulls_test_ext1("
            + "col1 array<array<array<string>>>)"
            + "stored as parquet location '"
            + nestedListWithNullsParquetFile.getParent()
            + "'";
  ...
  private void createNestedStructWithNullsHiveTables(final Driver hiveDriver) throws Exception {
    final File nestedStructWithNullsParquetDir =
        new File(BaseTestQuery.getTempDir("nestedstructwithnullsparquet"));
    nestedStructWithNullsParquetDir.mkdirs();
    final URL nestedStructWithNullsParquetUrl =
        Resources.getResource("list_struct_null_test.parquet");
    if (nestedStructWithNullsParquetUrl == null) {
      throw new IOException(
          String.format("Unable to find path %s.", "list_struct_null_test.parquet"));
    }

    // parquet file has following columns with one valid row
    // col1 array<struct<f1:array<string> with value [{['a', null]}, null]
    final File nestedStructWithNullsParquetFile =
        new File(nestedStructWithNullsParquetDir, "list_struct_null_test.parquet");
    nestedStructWithNullsParquetFile.deleteOnExit();
    nestedStructWithNullsParquetDir.deleteOnExit();
    Files.write(
        Paths.get(nestedStructWithNullsParquetFile.toURI()),
        Resources.toByteArray(nestedStructWithNullsParquetUrl));

    final String nestedStructTest =
        "create external table array_struct_with_nulls_test_ext1("
            + "col1 array<struct<f1:array<string>>>)"
            + "stored as parquet location '"
            + nestedStructWithNullsParquetFile.getParent()
            + "'";
  ...
  private void createParuqetComplexFilterTestTable(final Driver hiveDriver) throws Exception {
    ...
    Files.write(
        Paths.get(complexFilterTestParquetFile.toURI()),
        Resources.toByteArray(complexFilterTestParquetUrl));
    ...
    final String basicAllMatchingTest =
        "create external table filter_simple_test_ext1("
            + "intcol int,"
            + "floatcol float,"
            + "decimalcol decimal(10,5),"
            + "varcharcol varchar(16),"
            ...
            + "structcol struct<f1:int>) "
            + "stored as parquet location '"
            + complexFilterTestParquetFile.getParent()
            + "'";
    executeQuery(hiveDriver, basicAllMatchingTest);

    // create table with type coercions
    final String tableWithCoercions =
        "create external table filter_simple_test_ext2("
            + "intcol bigint,"
            + "floatcol double,"
  ...
  private void createParquetTableWithDeeplyNestedColumns(final Driver hiveDriver) throws Exception {
    final File complexNestedLevels =
        new File(BaseTestQuery.getTempDir("parquetcomplexnestedlevels"));
    complexNestedLevels.mkdirs();
    final URL complexNestedLevelsTestParquetUrl =
        Resources.getResource("deeply_nested_list.parquet");
    ...
    final File complexNestedLevelsTestParquetFile =
        new File(complexNestedLevels, "deeply_nested_list.parquet");
    complexNestedLevelsTestParquetFile.deleteOnExit();
    complexNestedLevels.deleteOnExit();
    Files.write(
        Paths.get(complexNestedLevelsTestParquetFile.toURI()),
        Resources.toByteArray(complexNestedLevelsTestParquetUrl));
    ...
    final String nestedLevelsTest =
        "create external table deeply_nested_list_test("
            + "col1 int,"
            ...
            + "col4 array<string>) "
            + "stored as parquet location '"
            + complexNestedLevelsTestParquetFile.getParent()
            + "'";
    executeQuery(hiveDriver, nestedLevelsTest);

    // create an empty table with one column using struct and arrays with depth 20
    final String nestedLevelsStructTest =
        "create table deeply_nested_struct_test("
  ...
  private void createComplexVarcharHiveTables(final Driver hiveDriver) throws Exception {
    final File complexVarcharParquetDir =
        new File(BaseTestQuery.getTempDir("complexvarcharparquet"));
    ...
    // parquet file has following columns with one valid row, all varchars have 'abcdefghijklmno' as
    // value
    // col1 varchar(15)
    ...
    final File complexVarcharParquetFile =
        new File(complexVarcharParquetDir, "varchar_complex.parquet");
    complexVarcharParquetFile.deleteOnExit();
    complexVarcharParquetDir.deleteOnExit();
    Files.write(
        Paths.get(complexVarcharParquetFile.toURI()),
        Resources.toByteArray(complexVarcharParquetUrl));
  ...
  private void createComplexTypesTextTable(final Driver hiveDriver, final String table)
      throws Exception {
  ...
  private void createComplexTypesTable(
      final Driver hiveDriver, final String format, final String table) throws Exception {
    executeQuery(
        hiveDriver,
        "CREATE TABLE "
            + table
            ...
            + " map_struct_field map<string, struct<type:string>> "
            + ") ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t' COLLECTION ITEMS TERMINATED BY ','"
            + " MAP KEYS TERMINATED BY ':' STORED AS "
            + format);
    executeQuery(
        hiveDriver, "INSERT OVERWRITE TABLE " + table + format + " SELECT * FROM " + table);
  }

  private void createListTypesTextTable(final Driver hiveDriver, final String table)
      throws Exception {
  ...
  private void createListTypesTable(
      final Driver hiveDriver, final String format, final String table) throws Exception {
    executeQuery(
        hiveDriver,
        "CREATE TABLE "
  ...
  private void createMapTypesTextTable(final Driver hiveDriver, final String table)
      throws Exception {
  ...
  private void createMapTypesTable(final Driver hiveDriver, final String format, final String table)
      throws Exception {
    executeQuery(
        hiveDriver,
        "CREATE TABLE "
  ...
  private void createORCDecimalCompareTestTable(final Driver hiveDriver, final String table)
      throws Exception {
    String createTableCmd =
        "CREATE TABLE " + table + " (col1 float, col2 double, col3 decimal(1,1)) stored as orc";
  ...
  private void createStructTypesTextTable(final Driver hiveDriver, final String table)
      throws Exception {
  ...
  private void createStructTypesTable(
      final Driver hiveDriver, final String format, final String table) throws Exception {
    executeQuery(
        hiveDriver,
        "CREATE TABLE "
  ...
  private void createUnionTypesTextTable(final Driver hiveDriver, final String table)
      throws Exception {
    ...
    executeQuery(
        hiveDriver,
        String.format(
            "LOAD DATA LOCAL INPATH '%s' INTO TABLE default.%s_int_input", testIntDataFile, table));
    executeQuery(
        hiveDriver,
        String.format(
            "LOAD DATA LOCAL INPATH '%s' INTO TABLE default.%s_double_input",
            testDoubleDataFile, table));
    executeQuery(
        hiveDriver,
        String.format(
            "LOAD DATA LOCAL INPATH '%s' INTO TABLE default.%s_string_input",
            testStringDataFile, table));
    executeQuery(
        hiveDriver,
        "INSERT INTO TABLE "
            + table
            + " SELECT rownum, CREATE_UNION(0, int_field, cast(0.0 as double), \"\") FROM "
            + table
            + "_int_input");
    executeQuery(
        hiveDriver,
        "INSERT INTO TABLE "
            + table
            + " SELECT rownum, CREATE_UNION(1, 0, double_field, \"\") FROM "
            + table
            + "_double_input");
    executeQuery(
        hiveDriver,
        "INSERT INTO TABLE "
            + table
            + " SELECT rownum, CREATE_UNION(2, 0, cast(0.0 as double), string_field) FROM "
            + table
            + "_string_input");
  }

  private void createUnionTypesTable(
      final Driver hiveDriver, final String format, final String table) throws Exception {
    executeQuery(
        hiveDriver,
        "CREATE TABLE "
  ...
  private void createAllTypesTextTable(final Driver hiveDriver, final String table)
      throws Exception {
    ...
    executeQuery(
        ...
        "CREATE TABLE IF NOT EXISTS "
            ...
            + ") PARTITIONED BY ("
            +
            // There is a regression in Hive 1.2.1 in binary type partition columns. Disable for
            // now.
            // "  binary_part BINARY," +
    ...
    executeQuery(
        hiveDriver,
        String.format(
            "LOAD DATA LOCAL INPATH '%s' INTO TABLE default."
                + table
                + " PARTITION ("
                +
                // There is a regression in Hive 1.2.1 in binary type partition columns. Disable for
  ...
  private void createDecimalConversionTable(Driver hiveDriver, String table, String tableFormat)
      throws Exception {
    String datatable =
        "CREATE TABLE IF NOT EXISTS "
            + table
            + " (col1 decimal(30, 9), col2 decimal(23, 6), col3 decimal(16, 2)) STORED AS "
            + tableFormat;
    executeQuery(hiveDriver, datatable);
    String intsert_datatable =
        "INSERT INTO "
            + table
            + " VALUES (111111111111111111111.111111111, 22222222222222222.222222, 333.0)";
    ...
    String ext_table =
        "CREATE EXTERNAL TABLE IF NOT EXISTS "
            + exttable
            + " (col1 decimal(25,2), col2 string, col3 varchar(32))"
            + "STORED AS "
            + tableFormat
            + " LOCATION 'file://"
            + this.getWhDir()
    ...
    String ext_table2 =
        "CREATE EXTERNAL TABLE IF NOT EXISTS "
            + exttable2
            + " (col1 decimal(2,2), col2 decimal(16,7), col3 decimal(4,1))"
            + "STORED AS "
            + tableFormat
            + " LOCATION 'file://"
            + this.getWhDir()
    ...
    String datatablerev =
        "CREATE TABLE IF NOT EXISTS "
            + tablerev
            + " (col1 int, col2 string, col3 double) STORED AS "
            + tableFormat;
    executeQuery(hiveDriver, datatablerev);
    String intsert_datatable_rev =
        "INSERT INTO " + tablerev + " VALUES (1234, 1234567, 1234567.123)";
    executeQuery(hiveDriver, intsert_datatable_rev);

    // string to decimal
    String exttable_rev = tablerev + "_ext";
    String ext_table_rev =
        "CREATE EXTERNAL TABLE IF NOT EXISTS "
            + exttable_rev
            + " (col1 decimal(3,2), col2 decimal(5,2), col3 decimal(5,2))"
            + "STORED AS "
            + tableFormat
            + " LOCATION 'file://"
            + this.getWhDir()
    ...
    String ext_drop_col3 =
        "CREATE EXTERNAL TABLE IF NOT EXISTS "
            + table
            + "_no_col3_ext"
            + " (col1 decimal(30, 9), col2 decimal(23, 6))"
            + " STORED AS "
            + tableFormat
            + " LOCATION 'file://"
            + this.getWhDir()
  ...
  private void createTimestampToStringTable(Driver hiveDriver, String table) throws Exception {
    ...
    String datatableorc =
        "CREATE TABLE IF NOT EXISTS " + table + "_orc" + " (col1 timestamp) STORED AS ORC";
    executeQuery(hiveDriver, datatabletxt);
    executeQuery(hiveDriver, datatableorc);

    String insert_datatable =
        String.format("LOAD DATA LOCAL INPATH '%s' INTO TABLE default." + table, testDataFile);
    executeQuery(hiveDriver, insert_datatable);
    String insert_datatableorc =
        "INSERT OVERWRITE TABLE " + table + "_orc" + " SELECT * FROM " + table;
  ...
  private void createDoubleToStringTable(Driver hiveDriver, String table) throws Exception {
    ...
    String datatableorc =
        "CREATE TABLE IF NOT EXISTS " + table + "_orc" + " (col1 double) STORED AS ORC";
    executeQuery(hiveDriver, datatabletxt);
    executeQuery(hiveDriver, datatableorc);
  ...
  private void createExtTableWithMoreColumnsThanOriginal(Driver hiveDriver, String table)
      throws Exception {
    String datatable =
        "CREATE TABLE IF NOT EXISTS " + table + " (col1 int, col2 int) STORED AS ORC";
    executeQuery(hiveDriver, datatable);
  ...
  private void createAllTypesTable(Driver hiveDriver, String format, String source)
      throws Exception {
    executeQuery(
        hiveDriver,
        "CREATE TABLE readtest_"
            + format
            + "("
            + "  binary_field BINARY, "
            + "  boolean_field BOOLEAN, "
    ...
    executeQuery(
        hiveDriver,
        "INSERT OVERWRITE TABLE readtest_"
            + format
            + " PARTITION ("
            +
            // There is a regression in Hive 1.2.1 in binary type partition columns. Disable for
  ...
  private String generateAllTypesDataFile() throws Exception {
    ...
    PrintWriter printWriter = new PrintWriter(file);
    printWriter.println(
        "YmluYXJ5ZmllbGQ=,false,34,65.99,2347.923,2758725827.9999,29375892739852.7689,"
    ...
    for (int row = 0; row < ROW_COUNT; ++row) {
      ...
      String mapValue =
          "name"
              ...
              + Integer.toString(row + 1)
              + ((row % 2 == 0)
                  ? ("," + "name" + Integer.toString(row + 2) + ":" + Integer.toString(row + 2))
                  : "");
      String mapStructValue =
          "key" + Integer.toString(row) + ":" + "struct" + Integer.toString(row);
    ...
    for (int row = 0; row < ROW_COUNT; ++row) {
      ...
      String tinyint_field =
          Integer.toString(1)
              ...
              + ","
              + Integer.toString(5);
      String smallint_field =
          Integer.toString(1024) + "," + Integer.toString(1025) + "," + Integer.toString(1026);
      String int_field =
          Integer.toString(row) + "," + Integer.toString(row + 1) + "," + Integer.toString(row + 2);
      String bigint_field =
          Long.toString(90000000000L)
              + ","
              + Long.toString(90000000001L)
              + ","
              + Long.toString(90000000002L);
      String float_field =
          Float.toString(row) + "," + Float.toString(row + 1) + "," + Float.toString(row + 2);
      String double_field =
          Double.toString(row)
              + ","
              + Double.toString(row + 1)
              + ","
              + Double.toString(row + 2)
              + ","
              + Double.toString(row + 3)
              + ","
              + Double.toString(row + 4);
  ...
  private String createTableWithHeaderFooterProperties(
      String tableName, String format, String headerValue, String footerValue) {
    return String.format(
        "create table %s (key int, value string) stored as %s tblproperties('%s'='%s', '%s'='%s')",
        tableName,
        format,
        serdeConstants.HEADER_COUNT,
        headerValue,
        serdeConstants.FOOTER_COUNT,
        footerValue);
  }

  private String generateTestDataWithHeadersAndFooters(
      String tableName, int rowCount, int headerLines, int footerLines) {
  ...
  private void createMixedPartitionTypeTable(Driver hiveDriver, String table) throws Exception {
    ...
    createTable =
        "CREATE TABLE "
            + mixTableWithDecimalField
            + " (col1 decimal(5,1)) PARTITIONED BY (col2 int)";
    insertTextRow1 =
        "insert into " + mixTableWithDecimalField + " partition (col2=1) values(1234.5)";
    ...
    alterPartitionType =
        "alter table "
            ...
            + "serde 'org.apache.hadoop.hive.ql.io.parquet.serde.ParquetHiveSerDe'";
    insertParquetRow1 =
        "insert into " + mixTableWithDecimalField + " partition (col2=3) values(1234.5)";
    String alterColumn =
        "alter table " + mixTableWithDecimalField + " change column col1 col1 decimal(3,2)";
    String insertParquetRow2 =
        "insert into " + mixTableWithDecimalField + " partition (col2=4) values(3.4)";
  ...
  private void createDecimalParquetTableWithDecimalColumnMismatch(Driver hiveDriver, String table)
      throws Exception {
    String createTable =
        "CREATE TABLE " + table + " (int_col int, name varchar(4)) stored as parquet";
    ...
    String ext_table =
        "CREATE EXTERNAL TABLE IF NOT EXISTS "
            + table
            + "_ext (int_col int, name decimal(3,2)) STORED AS PARQUET LOCATION 'file://"
            + this.getWhDir()
            + "/"
            + table
            + "'";
  ...
  private void createTableWithList(Driver hiveDriver, String table) {
    String createTable =
        "create table "
            + table
            + " (str_list array<string>, int_list array<int>, bool_list array<boolean>) stored as parquet";
    String insert =
        "INSERT INTO table "
            + table
            + " SELECT array('str1', 'str2', 'str3', 'str4') as str_list, array(1,2,3,4) as int_list, array(true, false) as bool_list";
    executeQuery(hiveDriver, createTable);
    executeQuery(hiveDriver, insert);
  }
  ...
  private void createTableWithStruct(Driver hiveDriver, String table) {
    String createTable =
        "create table "
            + table
            + " (mixed_struct struct<salary:double, country:string, citizen:boolean, phone:int>) stored as parquet";
    String insert =
        "insert into "
            + table
            + " select named_struct(\"salary\",cast(100.1 as double), \"country\",\"IN\", \"citizen\",false , \"phone\", 12341234) as mixed_struct";
    executeQuery(hiveDriver, createTable);
    executeQuery(hiveDriver, insert);
  }
  ...
  private void createTableNestedList(Driver hiveDriver, String table) {
    String createTable =
        "create table complex_types_nested_list "
  ...
  private void createTableWithUnsupportedComplexTypes(Driver hiveDriver, String table) {
    String createTable =
        "create table "
            + table
            + " (name_col string, map_col map<string, string>) stored as parquet";
  ...
  private void createTableWithMapColumn(Driver hiveDriver, String table) {
    ...
    String insert =
        "insert into "
            + table
            + " select 12, map(\"abc\", 123) as stringKey, map(11, \"xyz\") as stringValue, map(\"aa\",\"bb\") as bothString, "
  ...
  private void createTableMixedCaseColumnsWithComplexTypes(Driver hiveDriver, String table)
      throws IOException {
  ...
  private void createTableFlagTestColumns(Driver hiveDriver, String table) {
    String createTable =
        "create table "
            + table
            + " (primary_col string, complex_col struct<subcol:string>) stored as parquet";
    String insertTable =
        "insert into " + table + " select 'primary_col_val', named_struct('subcol', 'subcol_val')";
    executeQuery(hiveDriver, createTable);
    executeQuery(hiveDriver, insertTable);
  }

  private void createTableForPartitionValueFormatException(Driver hiveDriver, String table)
      throws IOException {
    ...
    String createOrcTable =
        "create table "
            + table
            + "_orc"
            + " (col1 int)"
            + " partitioned by (col2 bigint) stored as orc";
    String insertOrcTable =
        "insert into " + table + "_orc" + " partition(col2)  select * from " + table + "_text";
  ...
  private void createParquetStructExtraField(final Driver hiveDriver) throws Exception {
    ...
    final File structWithExtraFieldFile =
        new File(structWithExtraField, "struct_extra_test.parquet");
    structWithExtraFieldFile.deleteOnExit();
    structWithExtraField.deleteOnExit();
    Files.write(
        Paths.get(structWithExtraFieldFile.toURI()),
        Resources.toByteArray(structWithExtraFieldParquetUrl));

    final String structExtraFieldTest =
        "create external table struct_extra_test_ext("
            + "col1 int, col2 array<struct<f1:int, f2:struct<f_f_1:int, f_f_2:int>>>)"
            + "stored as parquet location '"
            + structWithExtraFieldFile.getParent()
            + "'";
  ...
  private void createEmptyFloatFieldORCTable(final Driver hiveDriver) throws Exception {
    ...
    Files.write(
        Paths.get(emptyFloatFieldOrcFile.toURI()), Resources.toByteArray(emptyFloatFieldOrcURL));
    ...
    executeQuery(
        ...
        "CREATE EXTERNAL TABLE empty_float_field ("
            ...
            + ") STORED AS ORC location '"
            + emptyFloatFieldOrcFile.getParent()
            + "'");
  }

  private void createNullORCStructTable(final Driver hiveDriver) throws Exception {
    final File structWithNullsOrc =
        new File(BaseTestQuery.getTempDir("struct_with_nulls_orc_test"));
    ...
    Files.write(
        Paths.get(structWithNullsOrcFile.toURI()), Resources.toByteArray(structWithNullsOrcUrl));
    ...
    executeQuery(
        ...
        "CREATE EXTERNAL TABLE orcnullstruct ("
            ...
            + ") STORED AS ORC location '"
            + structWithNullsOrcFile.getParent()
            + "'");
  }
  ...
  private void createORCPartitionSchemaTestTable(final Driver hiveDriver) {
    executeQuery(
        hiveDriver,
        "create table orc_part_test (col1 decimal(5,3)) partitioned by (partcol1 int) stored as orc");
  ...
  private void createFlattenOrcHiveTable(final Driver hiveDriver) throws Exception {
    ...
    // column ooa :
    // array<struct<in:bigint,fl:struct<f1:double,f2:double>,a:struct<aa:struct<aaa:string>>,b:struct<bb:struct<bbb:string>>,c:struct<cc:struct<ccc:string>>>>
    // with values [{in: 1},{in: 1,fl:{f1: 1.6789,f2: 54331}},{a:{aa:{aaa: "aaa
    // 1"}},b:{bb:{}},c:{cc:{ccc: "ccc 1"}}}]
    ...
    final String flattenTest =
        "create external table flatten_parquet("
            ...
            + " stored as parquet location '"
            + flattenOrcFile.getParent()
            + "'";
    executeQuery(hiveDriver, flattenTest);

    final String orcRegionTable =
        "create table flatten_orc stored as orc as SELECT * FROM flatten_parquet";
    executeQuery(hiveDriver, orcRegionTable);
  }
}
