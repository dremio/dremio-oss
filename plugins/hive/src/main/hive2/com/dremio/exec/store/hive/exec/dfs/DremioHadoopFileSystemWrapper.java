 ...
 *
 * <p>If {@link OperatorStats} are provided it returns an instrumented FSDataInputStream to measure
 * IO wait time and tracking file open/close operations.
 */
public class DremioHadoopFileSystemWrapper
    implements com.dremio.io.file.FileSystem, OpenFileTracker {
  static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(DremioHadoopFileSystemWrapper.class);
  ...
  private final boolean mustSwapContextClassLoader;

  public DremioHadoopFileSystemWrapper(
      ...
      throws IOException {
    this(fsConf, fileSystem, operatorStats, enableAsync);
  }

  public DremioHadoopFileSystemWrapper(
      Configuration fsConf, FileSystem fs, OperatorStats operatorStats, boolean enableAsync) {
    ...
    this.mustSwapContextClassLoader = !(fs instanceof ContextClassLoaderAware);
  }
  ...
  private void openNonExistentFileInPath(org.apache.hadoop.fs.Path f) throws IOException {
    org.apache.hadoop.fs.Path nonExistentFile =
        f.suffix(NON_EXISTENT_FILE_SUFFIX + NON_EXISTENT_FILE_COUNTER++);
  ...
  public FSInputStream open(Path f) throws IOException {
    try (Closeable closeable = swapClassLoader();
        WaitRecorder recorder = OperatorStats.getMetadataWaitRecorder(operatorStats, f)) {
  ...
  public FSOutputStream create(Path f) throws IOException {
    try (Closeable closeable = swapClassLoader();
        WaitRecorder recorder = OperatorStats.getWaitRecorder(operatorStats)) {
  ...
  public void setPermission(Path p, Set<PosixFilePermission> permissions) throws IOException {
    try (Closeable closeable = swapClassLoader();
        WaitRecorder recorder = OperatorStats.getMetadataWaitRecorder(operatorStats, p)) {
  ...
  public boolean mkdirs(Path folderPath) throws IOException {
    try (Closeable closeable = swapClassLoader();
        WaitRecorder recorder = OperatorStats.getMetadataWaitRecorder(operatorStats, folderPath)) {
  ...
  public DirectoryStream<FileAttributes> glob(Path pattern, Predicate<Path> filter)
      throws FileNotFoundException, IOException {
    try (Closeable closeable = swapClassLoader();
        WaitRecorder recorder = OperatorStats.getMetadataWaitRecorder(operatorStats, pattern)) {
      return new ArrayDirectoryStream(
  ...
  public boolean rename(Path src, Path dst) throws IOException {
    try (Closeable closeable = swapClassLoader();
        WaitRecorder recorder = OperatorStats.getMetadataWaitRecorder(operatorStats, dst)) {
  ...
  public void access(final Path path, final Set<AccessMode> mode)
    ...
    try (Closeable closeable = swapClassLoader();
        WaitRecorder recorder = OperatorStats.getMetadataWaitRecorder(operatorStats, path)) {
  ...
  public void fileOpened(Path path, FSInputStream fsDataInputStream) {
    openedFiles.put(
        fsDataInputStream, new DebugStackTrace(path, Thread.currentThread().getStackTrace()));
  }
  ...
  public boolean supportsAsync() {
    ...
    if (underlyingFs instanceof MayProvideAsyncStream) {
      ...
      // have to look for specific dremio file system for hive
      // mayprovideasync breaks interface compatibility for path
      ...
      delegate =
          statuses != null
              ? ImmutableList.copyOf(
                  Iterables.transform(Arrays.asList(statuses), HadoopFileStatusWrapper::new))
  ...
  private final class FetchOnDemandDirectoryStream implements DirectoryStream<FileAttributes> {
      ...
      convertedIterator =
          new Iterator<FileAttributes>() {
            ...
            public boolean hasNext() {
              try (Closeable closeable = swapClassLoader();
                  WaitRecorder metaRecorder = OperatorStats.getMetadataWaitRecorder(stats, p)) {
            ...
            public FileAttributes next() {
              try (Closeable closeable = swapClassLoader();
                  WaitRecorder metaRecorder = OperatorStats.getMetadataWaitRecorder(stats, p)) {
                return new HadoopFileStatusWrapper(statuses.next());
              } catch (IOException e) {
                logger.error(
  ...
  com.dremio.io.FSInputStream newFSDataInputStreamWrapper(Path f, final FSDataInputStream is)
      throws IOException {
    try {
      return (operatorStats != null)
          ? com.dremio.exec.store.hive.exec.dfs.FSDataInputStreamWithStatsWrapper.of(
              is, operatorStats, true, f.toString())
  ...
  com.dremio.io.FSOutputStream newFSDataOutputStreamWrapper(
      final FSDataOutputStream os, String filePath) throws IOException {
    try {
      return (operatorStats != null)
          ? new com.dremio.exec.store.hive.exec.dfs.FSDataOutputStreamWithStatsWrapper(
              os, operatorStats, filePath)
  ...
  FSInputStream newFSDataInputStreamWrapper(
      Path f, final FSDataInputStream is, boolean recordWaitTime) throws IOException {
    FSInputStream result;
    if (operatorStats != null) {
      result =
          com.dremio.exec.store.hive.exec.dfs.FSDataInputStreamWithStatsWrapper.of(
              is, operatorStats, recordWaitTime, f.toString());
    } else {
  ...
  /**
   * Swaps the current thread's context class loader to the plugin class loader, if necessary. If
   * the underlying filesystem implements ContextClassLoaderAware, the responsibility to swap to the
   * correct class loader is delegated to that filesystem.
   */
  private Closeable swapClassLoader() {
    return mustSwapContextClassLoader
        ? ContextClassLoaderSwapper.swapClassLoader(DremioHadoopFileSystemWrapper.class)
        : () -> {};
  }
}
