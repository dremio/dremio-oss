...
public interface HivePluginOptions {
  ...
  BooleanValidator HIVE_ORC_READER_VECTORIZE =
      new BooleanValidator("store.hive.orc.vectorize", true);
  ...
  TypeValidators.LongValidator RESERVE =
      new TypeValidators.PositiveLongValidator(
          "planner.op.scan.hive.reserve_bytes", Long.MAX_VALUE, Prel.DEFAULT_RESERVE);

  TypeValidators.LongValidator LIMIT =
      new TypeValidators.PositiveLongValidator(
          "planner.op.scan.hive.limit_bytes", Long.MAX_VALUE, Prel.DEFAULT_LIMIT);

  /** Option to use bytebuffers using direct memory while reading ORC files */
  BooleanValidator HIVE_ORC_READER_USE_DIRECT_MEMORY =
      new BooleanValidator("store.hive.orc.use_direct_memory", true);

  /**
   * Option to exclude Hive table or partition properties from loading into KV store. If no
   * exclusion is intended set to (?!)
   */
  TypeValidators.RegexStringValidator HIVE_PROPERTY_EXCLUSION_REGEX =
      new TypeValidators.RegexStringValidator(
          "store.hive.property_exclusion_regex",
