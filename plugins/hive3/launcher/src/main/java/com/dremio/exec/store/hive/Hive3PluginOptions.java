...
@Options
public interface Hive3PluginOptions {

  /** Options to enable vectorized ORC reader and filter pushdown into vectorized ORC reader */
  BooleanValidator HIVE_ORC_READER_VECTORIZE =
      new BooleanValidator("store.hive3.orc.vectorize", true);

  BooleanValidator ENABLE_FILTER_PUSHDOWN_HIVE_ORC =
      new BooleanValidator("store.hive3.orc.vectorize.enable_filter_pushdown", true);
   ...
   */
  String HIVE_USE_STATS_IN_METASTORE_KEY = "store.hive3.use_stats_in_metastore";

  BooleanValidator HIVE_USE_STATS_IN_METASTORE =
      new BooleanValidator(HIVE_USE_STATS_IN_METASTORE_KEY, false);
  ...
  TypeValidators.DoubleValidator HIVE_PARQUET_COMPRESSION_FACTOR_VALIDATOR =
      new TypeValidators.RangeDoubleValidator(
          HIVE_PARQUET_COMPRESSION_FACTOR_KEY, 0.01d, 100.00d, 30.00d);
  ...
  String HIVE_MAX_INPUTSPLITS_PER_PARTITION_KEY = "store.hive3.max_inputsplits_per_partition";
  ...
  TypeValidators.LongValidator RESERVE =
      new TypeValidators.PositiveLongValidator(
          "planner.op.scan.hive3.reserve_bytes", Long.MAX_VALUE, Prel.DEFAULT_RESERVE);

  TypeValidators.LongValidator LIMIT =
      new TypeValidators.PositiveLongValidator(
          "planner.op.scan.hive3.limit_bytes", Long.MAX_VALUE, Prel.DEFAULT_LIMIT);

  /** Option to use bytebuffers using direct memory while reading ORC files */
  BooleanValidator HIVE_ORC_READER_USE_DIRECT_MEMORY =
      new BooleanValidator("store.hive3.orc.use_direct_memory", true);

  /**
   * Option to exclude Hive table or partition properties from loading into KV store. If no
   * exclusion is intended set to (?!)
   */
  RegexStringValidator HIVE_PROPERTY_EXCLUSION_REGEX =
      new RegexStringValidator(
          "store.hive3.property_exclusion_regex",
