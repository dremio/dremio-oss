...
public class TestHive3StoragePlugin {
  ...
  public Hive3StoragePlugin testPlugin(long parallelism, long timeoutMS) throws Exception {
    ...
    when(optionManager.getOption(ExecConstants.HIVE_SIGNATURE_VALIDATION_PARALLELISM)).thenReturn(parallelism);
    when(optionManager.getOption(ExecConstants.HIVE_SIGNATURE_VALIDATION_TIMEOUT_MS)).thenReturn(timeoutMS);
