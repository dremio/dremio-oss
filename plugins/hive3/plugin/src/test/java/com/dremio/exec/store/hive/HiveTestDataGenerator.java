...
public final class HiveTestDataGenerator {
  ...
  private static synchronized HiveTestDataGenerator getInstance(Map<String, String> config)
      throws Exception {
  ...
  /**
   * Update the current Hive3StoragePlugin in given plugin registry with given
   * <i>configOverride</i>.
   *
  ...
  public void updatePluginConfig(
    ...
    Hive3StoragePluginConfig conf =
        (Hive3StoragePluginConfig)
            msp.getId().<Hive3StoragePluginConfig>getConnectionConf().clone();

    List<Property> updated = new ArrayList<>();
    List<Property> updatedSecretPropertyList = new ArrayList<>();
  ...
  private void createTableWithStruct(Driver hiveDriver, String table) {
    String createTable =
        "create table "
            + table
            + " (mixed_struct struct<salary:decimal(4,1), country:string, citizen:boolean, phone:int>) stored as parquet";
    String insert =
        "insert into "
            + table
            + " select named_struct(\"salary\",100.1, \"country\",\"IN\", \"citizen\",false , \"phone\", 12341234) as mixed_struct";
    executeQuery(hiveDriver, createTable);
    executeQuery(hiveDriver, insert);
  }
