...
public class ITHiveStorage extends HiveTestBase {
  ...
  public void testNativeScanWhenNoColumnIsRead() throws Exception {
    try {
      test(String.format("alter session set \"%s\" = true", Hive3PluginOptions.HIVE_OPTIMIZE_SCAN_WITH_NATIVE_READERS));
      ...
      test(String.format("alter session set \"%s\" = %s",
          Hive3PluginOptions.HIVE_OPTIMIZE_SCAN_WITH_NATIVE_READERS,
              Hive3PluginOptions.HIVE_OPTIMIZE_SCAN_WITH_NATIVE_READERS_VALIDATOR.getDefault().getBoolVal() ? "true" : "false"));
    }
  }
  ...
  public void testHiveParquetRefreshOnMissingFile() throws Exception {
    ...
    setSessionOption(Hive3PluginOptions.HIVE_OPTIMIZE_SCAN_WITH_NATIVE_READERS, "true");
    try {
      ...
      setSessionOption(Hive3PluginOptions.HIVE_OPTIMIZE_SCAN_WITH_NATIVE_READERS, "false");
    }
  }
  ...
  public void testCheckHasPermission() throws Exception {
    ...
    assertTrue(getSabotContext().getCatalogService().getSource("hive").hasAccessPermission(HiveImpersonationUtil.getProcessUserName(), dataset, datasetConfig));
    ...
    try {
      ...
      assertFalse(getSabotContext().getCatalogService().getSource("hive").hasAccessPermission(HiveImpersonationUtil.getProcessUserName(), dataset, datasetConfig));
    } finally {
