...
public class Hive3StoragePlugin extends BaseHiveStoragePlugin implements StoragePluginCreator.PF4JStoragePlugin,
  ...
  public Hive3StoragePlugin(HiveConf hiveConf, PluginManager pf4jManager, SabotContext context, String name, Provider<StoragePluginId> pluginIdProvider) {
    super(context, name);
    ...
    this.hiveSettings = new HiveSettings(context.getOptionManager(), HiveConfFactory.isHive2SourceType(hiveConf));
    ...
    // This is required for the HBase client that Hive 3 plugin uses irrespective of HBase server side.
  ...
  public Writer getWriter(PhysicalOperator child, String location, WriterOptions options, OpProps props)
    throws IOException {
    throw new UnsupportedOperationException("Hive3 plugin doesn't support table creation via CTAS.");
  }
  ...
  MetadataValidity checkHiveMetadata(HiveTableXattr tableXattr, EntityPath datasetPath, BatchSchema tableSchema, final HiveReadSignature readSignature) throws TException {
    ...
    BatchSchema hiveSchema = HiveMetadataUtils.getBatchSchema(table, hiveConf, new HiveSchemaTypeOptions(optionManager), this);
