...
class HiveClientImpl implements HiveClient, AutoCloseable {
  ...
  private static final ImmutableList<String> SYSTEM_DB =
      ImmutableList.of("sys", "information_schema");
  ...
  static void getAndSetDelegationToken(
    ...
    try {
      ...
      SecurityUtils.setTokenStr(
          proxyUGI, delegationToken, "DremioDelegationTokenForHiveMetaStoreServer");
  ...
  public List<String> getDatabases(boolean ignoreAuthzErrors) throws TException {
    return doCommand(
        (RetryableClientCommand<List<String>>)
            client -> {
  ...
  public boolean databaseExists(final String dbName) {
    try {
      return doCommand(client -> !SYSTEM_DB.contains(dbName) && client.getDatabase(dbName) != null);
    ...
    return doCommand(
        (RetryableClientCommand<List<String>>)
            client -> {
              ...
              try {
                tableNames = client.getAllTables(dbName);
              } catch (NoSuchObjectException e) {
                logger.info("Database not found for db {}", dbName);
                tableNames = null;
              }
