...
public class TestRemoteNodeFileSystem extends ExecTest {
  ...
  private static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(TestRemoteNodeFileSystem.class);
  ...
  private static final List<NodeEndpoint> ENDPOINTS_LIST =
      Arrays.asList(LOCAL_ENDPOINT, REMOTE_ENDPOINT);
  ...
  private RemoteNodeFileSystem newRemoteNodeFileSystem() throws IOException {
    ...
    configuration.setTimeDuration(
        RemoteNodeFileSystem.RPC_TIMEOUT_KEY, TEST_RPC_TIMEOUT_MS, TimeUnit.MILLISECONDS);
    return newRemoteNodeFileSystem(configuration);
  }

  private RemoteNodeFileSystem newRemoteNodeFileSystem(Configuration configuration)
      throws IOException {
    RemoteNodeFileSystem fs = new RemoteNodeFileSystem(runner, allocator);
    fs.initialize(
        URI.create(
            String.format(
                "sabot://%s:%d/", REMOTE_ENDPOINT.getAddress(), REMOTE_ENDPOINT.getFabricPort())),
        configuration);
    return fs;
  }
  ...
  private void setupRPC(
      final EnumLite requestType,
      final MessageLite request,
      final EnumLite responseType,
      final MessageLite response)
      throws Exception {
  ...
  private void setupRPC(
      final EnumLite requestType,
      final List<MessageLite> requests,
      final EnumLite responseType,
      final List<MessageLite> responses,
            ...
            new Answer<Void>() {
              ...
              public Void answer(InvocationOnMock invocation) throws Throwable {
                throw new AssertionError(
                    format(
                        "Unexpected call to ProxyConnection#send with parameters: %s",
                        Arrays.toString(invocation.getArguments())));
              }
            })
        .when(proxyConnection)
        .send(
            any(RpcOutcomeListener.class),
            any(EnumLite.class),
      ...
      doAnswer(
              new Answer<Void>() {
                ...
                public Void answer(final InvocationOnMock invocation) throws Throwable {
                  final RpcOutcomeListener<MessageLite> listener =
                      invocation.getArgument(0, RpcOutcomeListener.class);
          ...
          .when(proxyConnection)
          .send(
              any(RpcOutcomeListener.class), eq(requestType), eq(request), eq(response.getClass()));
    }
    ...
    doAnswer(
            new Answer<Void>() {
              ...
              public Void answer(InvocationOnMock invocation) throws Throwable {
                throw new AssertionError(
                    format(
                        "Unexpected call to FabricCommandRunner#runCommand with parameters: %s",
                        Arrays.toString(invocation.getArguments())));
              }
            })
        .when(runner)
        .runCommand(any(RpcCommand.class));

    doAnswer(
            new Answer<Void>() {
              @Override
              public Void answer(final InvocationOnMock invocation) throws Throwable {
                final RpcCommand<?, ProxyConnection> rpcCommand =
                    invocation.getArgument(0, RpcCommand.class);
  ...
  private void setupRPC(
      final EnumLite requestType,
      final MessageLite request,
      Class<? extends MessageLite> responseClazz,
    ...
    doAnswer(
        ...
        .when(proxyConnection)
        .send(any(RpcOutcomeListener.class), eq(requestType), eq(request), eq(responseClazz));
  ...
  public void testGetFileStatusRoot() throws Exception {
    {
      DFS.FileStatus status =
          newFileStatus(1024L, true, 0, 4096L, 37L, 42L, 0755, "root", "wheel", "/");
      setupRPC(
          DFS.RpcType.GET_FILE_STATUS_REQUEST,
              DFS.GetFileStatusRequest.newBuilder().setPath("/").build(),
          DFS.RpcType.GET_FILE_STATUS_RESPONSE,
              DFS.GetFileStatusResponse.newBuilder().setStatus(status).build());
    }
  ...
  public void testGetFileStatusWithInvalidPath() throws Exception {
    {
      setupRPC(
          DFS.RpcType.GET_FILE_STATUS_REQUEST,
              DFS.GetFileStatusRequest.newBuilder().setPath("/foo/bar").build(),
          DFS.GetFileStatusResponse.class,
              newRPCException(LOCAL_ENDPOINT, new FileNotFoundException("File not found")));
    }
  ...
  private static DFS.FileStatus newFileStatus(
      long length,
      boolean isDir,
  ...
  public void testGetFileStatusWithValidPath() throws Exception {
    {
      DFS.FileStatus status =
          newFileStatus(1024, false, 1, 4096, 42, 37, 0644, "root", "wheel", "/foo/bar");
      setupRPC(
          DFS.RpcType.GET_FILE_STATUS_REQUEST,
              DFS.GetFileStatusRequest.newBuilder().setPath("/foo/bar").build(),
          DFS.RpcType.GET_FILE_STATUS_RESPONSE,
  ...
  public void testListStatusRoot() throws Exception {
    {
      DFS.ListStatusResponse listStatusResponse =
          DFS.ListStatusResponse.newBuilder()
              .addStatuses(newFileStatus(42, true, 0, 0, 456, 879, 0755, "root", "wheel", "/foo"))
              .addStatuses(
                  newFileStatus(1024, false, 1, 4096, 354, 435, 0644, "admin", "admin", "/bar"))
              .build();

      setupRPC(
          DFS.RpcType.LIST_STATUS_REQUEST,
          DFS.ListStatusRequest.newBuilder()
              .setPath("/")
              .setLimit(RemoteNodeFileSystem.LIST_STATUS_BATCH_SIZE_DEFAULT)
              .build(),
  ...
  public void testListStatusRootOneByOne() throws Exception {
    {
      DFS.ListStatusContinuationHandle handle =
          DFS.ListStatusContinuationHandle.newBuilder().setId("test-handle").build();

      DFS.ListStatusResponse listStatusInitialResponse =
          DFS.ListStatusResponse.newBuilder()
              .addStatuses(newFileStatus(42, true, 0, 0, 456, 879, 0755, "root", "wheel", "/foo"))
              .setHandle(handle)
              .build();
      DFS.ListStatusResponse listStatusLastResponse =
          DFS.ListStatusResponse.newBuilder()
              .addStatuses(
                  newFileStatus(1024, false, 1, 4096, 354, 435, 0644, "admin", "admin", "/bar"))
              .build();
  ...
  public void testListStatusWithValidPath() throws Exception {
    {
      DFS.ListStatusResponse listStatusResponse =
          DFS.ListStatusResponse.newBuilder()
              .addStatuses(
                  newFileStatus(42, true, 0, 0, 456, 879, 0755, "root", "wheel", "/foo/bar/dir"))
              .addStatuses(
                  newFileStatus(
                      1024, false, 1, 4096, 354, 435, 0644, "admin", "admin", "/foo/bar/file"))
              .build();
  ...
  public void testDeleteWithValidPath() throws Exception {
    setupRPC(
        DFS.RpcType.DELETE_REQUEST,
            DFS.DeleteRequest.newBuilder().setPath("/foo/bar").setRecursive(true).build(),
  ...
  private static final RpcException newRPCException(NodeEndpoint endpoint, IOException ioe) {
    UserRemoteException ure =
        UserRemoteException.create(
            UserException.ioExceptionError(ioe)
  ...
  public void testMkdirsWithValidPath() throws Exception {
    setupRPC(
        DFS.RpcType.MKDIRS_REQUEST,
            DFS.MkdirsRequest.newBuilder().setPath("/foo/bar").setPermission(0755).build(),
  ...
  public void testRenameWithValidPath() throws Exception {
    setupRPC(
        DFS.RpcType.RENAME_REQUEST,
            DFS.RenameRequest.newBuilder().setOldpath("/foo/bar").setNewpath("/foo/bar2").build(),
  ...
  public void testInputStream() throws Exception {
    ...
    setupRPC(
        ...
        DFS.GetFileDataRequest.newBuilder()
            ...
            .setLength(100)
            .build(),
        DFS.RpcType.GET_FILE_DATA_RESPONSE,
        DFS.GetFileDataResponse.newBuilder().setRead(100).build(),
        byteBuf);
    ...
    setupRPC(
        DFS.RpcType.GET_FILE_DATA_REQUEST,
            DFS.GetFileDataRequest.newBuilder()
                ...
                .build(),
        DFS.RpcType.GET_FILE_DATA_RESPONSE,
            DFS.GetFileDataResponse.newBuilder().setRead(-1).build());
