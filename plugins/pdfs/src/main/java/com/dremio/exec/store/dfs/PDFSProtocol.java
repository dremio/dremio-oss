...
final class PDFSProtocol extends AbstractProtocol {
  ...
  private static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(PDFSProtocol.class);
  ...
  PDFSProtocol(
    ...
    this.openIterators =
        CacheBuilder.newBuilder()
            ...
            .removalListener(
                (RemovalNotification<ListStatusContinuationHandle, RemoteIterator<FileStatus>>
                        notification) -> {
                  if (notification.getCause() == RemovalCause.EXPLICIT) {
                    return;
                  }
                  logger.info(
                      "Iterator for handle {} expired (cause: {})",
                      notification.getKey(),
                  ...
                  if (iterator instanceof Closeable) {
                    try {
                      ...
                      // swallow exception
                      logger.warn(
                          "Exception thrown when closing iterator for handle {}",
                          notification.getKey(),
                          e);
                    }
                  }
   ...
   * @throws IOException
   */
  public static PDFSProtocol newInstance(
      NodeEndpoint endpoint,
      SabotConfig config,
      BufferAllocator allocator,
      boolean allowLocalHandling)
      throws IOException {
    // we'll grab a raw local file system so append is supported (rather than
    // the checksum local file system).
    Configuration conf = new Configuration();
    return new PDFSProtocol(
        endpoint,
        config,
        allocator,
        PseudoDistributedFileSystem.newLocalFileSystem(conf, allowLocalHandling),
        allowLocalHandling,
        Ticker.systemTicker());
  }
    ...
    switch (rpcType) {
      ...
      default:
        throw UserException.unsupportedError()
            .message("Do not support response for rpc type %d", rpcType)
    ...
    try {
      ...
      if (!allowLocalAccess) {
        throw new RpcException(
            String.format(
                "Attempted to access %s:%d which is a client-only node.",
  ...
  private Response handle(PhysicalConnection connection, DFS.GetFileStatusRequest request)
      throws IOException {
    Path path = new Path(request.getPath());

    FileStatus status = localFS.getFileStatus(path);
    DFS.GetFileStatusResponse response =
        DFS.GetFileStatusResponse.newBuilder()
  ...
  private Response handle(PhysicalConnection connection, DFS.GetFileDataRequest request)
      throws IOException {
    Path path = new Path(request.getPath());
    ...
    try (FSDataInputStream fdis = localFS.open(path)) {
      ...
      ByteBuf[] bodies =
          (read != -1) ? new ByteBuf[] {Unpooled.wrappedBuffer(buf, 0, read)} : NO_BUFS;

      return reply(DFS.RpcType.GET_FILE_DATA_RESPONSE, response, bodies);
    }
  }

  private Response handle(PhysicalConnection connection, DFS.WriteDataRequest request, ByteBuf buf)
      throws IOException {
    ...
    // append, first check last update time and offset. (concurrency danger between check and write
    // but doesn't
    ...
    if (fs.getModificationTime() != request.getLastUpdate()) {
      throw new IOException(
          String.format(
              "Unexpected last modification time. Expected time: %d, Actual time: %d.",
    ...
    if (fs.getLen() != request.getLastOffset()) {
      throw new IOException(
          String.format(
              "Unexpected last offset. Remote offset: %d, Actual offset: %d.",
  ...
  private Response writeData(Path path, ByteBuf buf, boolean create) throws IOException {
    try (FSDataOutputStream output = create ? localFS.create(path) : localFS.append(path)) {
      // empty buffers are just going to be null, we still want to create the file but write nothing
      // to it
      if (buf != null) {
    ...
    final WriteDataResponse response =
        WriteDataResponse.newBuilder().setUpdateTime(fs.getModificationTime()).build();
    return reply(DFS.RpcType.WRITE_DATA_RESPONSE, response);
  }

  private Response handle(PhysicalConnection connection, DFS.ListStatusRequest request)
      throws IOException {
    ...
    if (request.hasHandle()) {
      ...
      if (iterator == null) {
        throw new IOException(
            format(
                "No iterator found for handle %s/path %s. Maybe it expired?",
    ...
    try {
      ...
      if (iterator.hasNext()) {
        ListStatusContinuationHandle handle =
            ListStatusContinuationHandle.newBuilder().setId(UUID.randomUUID().toString()).build();
  ...
  private Response handle(PhysicalConnection connection, DFS.MkdirsRequest request)
      throws IOException {
    Path path = new Path(request.getPath());
    ...
    return reply(
        DFS.RpcType.MKDIRS_RESPONSE, DFS.MkdirsResponse.newBuilder().setValue(result).build());
  }

  private Response handle(PhysicalConnection connection, DFS.RenameRequest request)
      throws IOException {
    ...
    return reply(
        DFS.RpcType.RENAME_RESPONSE, DFS.RenameResponse.newBuilder().setValue(result).build());
  }

  private Response handle(PhysicalConnection connection, DFS.DeleteRequest request)
      throws IOException {
    Path path = new Path(request.getPath());
    ...
    return reply(
        DFS.RpcType.DELETE_RESPONSE, DFS.DeleteResponse.newBuilder().setValue(result).build());
  }
