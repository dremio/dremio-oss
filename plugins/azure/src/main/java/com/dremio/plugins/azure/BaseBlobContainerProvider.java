    ...
    for(String c : list) {
      try {
        ...
        throw UserException.validationError()
          .message("Failure while validating existence of container %s. Error %s", c, e.getCause().getMessage())
          .buildSilently();
      }
    }
  }
  ...
  public void assertContainerExists(final String containerName) {
    ...
    try {
      ...
      throw new AzureStoragePluginException(
        String.format("Error occurred %s while checking for existence of container %s", e.getMessage(), containerName));
    } catch (URISyntaxException e) {
      throw new RuntimeException(
        String.format("Error response %s while checking for existence of container %s", e.getMessage(), containerName));
    }
