...
/**
 * Tests for the {@code CheckAzureConf} constraint,
 * which is validated by the {@code CheckAzureConfValidator}.
 */
public class TestCheckAzureConf {
  ...
  public void testCheckCredentialsMissing() {
    final AzureStorageConf conf = new AzureStorageConf();
    final Set<ConstraintViolation<AzureStorageConf>> violations = validator.validate(conf);
    final Set<String> violationMessages = violations.stream()
      .map(ConstraintViolation::getMessage)
      .collect(Collectors.toSet());
    Assert.assertEquals(2, violationMessages.size());
    Assert.assertTrue(violationMessages.containsAll(
      ImmutableSet.of("Azure storage account name is missing.", "Azure storage account access key is missing.")));
  }
}
