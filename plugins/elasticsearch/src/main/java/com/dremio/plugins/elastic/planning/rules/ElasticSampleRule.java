...
public class ElasticSampleRule extends RelOptRule {
  ...
  public ElasticSampleRule() {
    super(
        RelOptHelper.some(SamplePrel.class, RelOptHelper.any(ElasticsearchIntermediatePrel.class)),
        "ElasticSampleRule");
  }
  ...
  public boolean matches(RelOptRuleCall call) {
    ...
    assert !intermediatePrel.contains(ElasticsearchSample.class)
        : "Cannot have more than one sample per scan";
  ...
  public void onMatch(RelOptRuleCall call) {
    final ElasticsearchIntermediatePrel intermediatePrel = call.rel(1);
    final ElasticsearchSample newSample =
