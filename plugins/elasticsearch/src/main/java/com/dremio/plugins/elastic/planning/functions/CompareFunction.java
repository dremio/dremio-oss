...
class CompareFunction extends ElasticFunction {
  ...
  public FunctionRender render(FunctionRenderer renderer, RexCall call) {
    ...
    // we need special handling in painless for temporal types and comparison other than
    // equality/inequality.
      ...
      case GTE:
        {
          String script =
              String.format(
                  "(%s.equals(%s) || (%s).isAfter(%s))",
                  op1.getScript(), op2.getScript(), op1.getScript(), op2.getScript());
          return new FunctionRender(script, nulls);
        }
