...
public class ElasticScanRule extends SourceLogicalConverter {
  ...
  public Rel convertScan(ScanCrel scan) {
    return new ElasticsearchScanDrel(scan.getCluster(), scan.getTraitSet().plus(Rel.LOGICAL), scan.getTable(), scan.getPluginId(), scan.getTableMetadata(), scan.getProjectedColumns(), scan.getObservedRowcountAdjustment());
  }
