...
/**
 * A boundary Prel. Is used by Elastic rules to ensure that only the right set of operations are
 * pushed down into Elastic. Maintains state of what is pushed down. All Elastic operations are
 * pushed down using an operand of <PrelToFind><ElasticsearchIntermediatePrel>. This ensures that no
 * matching/changes are done below the intermediate prel.
 *
 * <p>This prel exists until we complete planning and is then finalized into a ElasticFinalPrel
 * which is a leaf prel for parallelization and execution purposes
 */
public class ElasticsearchIntermediatePrel extends SinglePrel implements PrelFinalizable {
  ...
  public ElasticsearchIntermediatePrel(
      RelTraitSet traitSet, RelNode input, FunctionLookupContext functionLookupContext) {
    ...
    nodes =
        FluentIterable.from(stack)
            .uniqueIndex(
                new Function<ElasticsearchPrel, Class<? extends ElasticsearchPrel>>() {
                  @Override
  ...
  public PhysicalOperator getPhysicalOperator(PhysicalPlanCreator creator) throws IOException {
    throw new UnsupportedOperationException(
        "Must be finalized before retrieving physical operator.");
  }
  ...
  public <T extends ElasticsearchPrel> T get(Class<T> clazz) {
    return Preconditions.checkNotNull(
        getNoCheck(clazz), "Unable to find %s in stack.", clazz.getName());
  }
  ...
  public <T, X, E extends Throwable> T accept(PrelVisitor<T, X, E> logicalVisitor, X value)
      throws E {
    throw new UnsupportedOperationException(
        "This needs to be finalized before using a PrelVisitor.");
  }
  ...
  public RelOptCost computeSelfCost(RelOptPlanner planner, RelMetadataQuery mq) {
    ...
    return planner
        .getCostFactory()
        .makeCost(Integer.MAX_VALUE, Integer.MAX_VALUE, 0)
   ...
   *
   * <p>- Push limit and sample below project (if either exists) - Pop a project out of the stack if
   * it is an edge project. - Add an external Limit to the lesser of sample or limit. - Add the
   * popped project as a ProjectPrel outside the pushdown.
   */
  @Override
  public Prel finalizeRel() {
    ...
    if (stack.get(0) instanceof ElasticsearchSample || stack.get(0) instanceof ElasticsearchLimit) {
      ...
      if (stack.get(1) instanceof ElasticsearchProject) {
        ...
        ElasticsearchPrel sampleOrLimit =
            (ElasticsearchPrel)
                stack.get(0).copy(traitSet, ImmutableList.<RelNode>of(bottom.get(0)));
        ElasticsearchPrel project =
            (ElasticsearchPrel)
                stack
                    .get(1)
                    .copy(sampleOrLimit.getTraitSet(), ImmutableList.<RelNode>of(sampleOrLimit));
    ...
    // create a final prel.
    final Prel finalPrel = new ElasticScanPrel(getCluster(), getTraitSet(), stack.get(0), builder);
      ...
      if (sample != null) {
        long sampleFetch =
            SampleRelBase.getSampleSizeAndSetMinSampleSize(
                ...
                ElasticsearchSample.SAMPLE_SIZE_DENOMINATOR);
    ...
    if (poppedProject != null) {
      output =
          ProjectPrel.create(
              output.getCluster(),
              poppedProject.getTraitSet(),
              output,
              poppedProject.getProjects(),
              poppedProject.getRowType());
    }
