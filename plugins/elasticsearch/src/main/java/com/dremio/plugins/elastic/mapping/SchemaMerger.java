...
public class SchemaMerger {
  ...
  public MergeResult merge(
      ElasticMapping mapping, BatchSchema schema, boolean forceDoublePrecision) {
    final ResultBuilder resultBuilder = new ResultBuilder();
    Collection<MergeField> fields =
        mergeFields(
            null,
            mapping.getFields(),
            schema != null ? schema.getFields() : ImmutableList.<Field>of(),
            forceDoublePrecision);

    return resultBuilder.toResult(
        FluentIterable.from(fields)
            .transform(
                new Function<MergeField, Field>() {
                  @Override
                  @Nullable
  ...
  List<MergeField> mergeFields(
      SchemaPath parent,
      List<ElasticField> declaredFields,
      List<Field> observedFields,
    ...
    final Map<String, ElasticField> newFieldMap =
        declaredFields.stream().collect(Collectors.toMap(ElasticField::getName, f -> f));
    ...
    for (Field observedField : observedFields) {
      ...
      if (declaredField == null) {
        // this field doesn't exist in new schema, but we still need to add to the list (TODO: add
        // explaination why)
      ...
      outputFields.add(
          mergeField(
              parent, declaredField, CompleteType.fromField(observedField), forceDoublePrecision));
    }

    // Any remaining new fields that are not observed previously should be added with default
    // settings
  ...
  MergeField mergeField(
      SchemaPath parent,
      ElasticField declaredField,
      CompleteType observedType,
      boolean forceDoublePrecision) {
    ...
    if ((declaredField.getType() == Type.OBJECT || declaredField.getType() == Type.NESTED)
      ...
      // nested case
      SchemaPath newParent =
          parent == null
              ? SchemaPath.getSimplePath(declaredField.getName())
              : parent.getChild(declaredField.getName());
      final List<MergeField> fields =
          mergeFields(
              newParent,
              declaredField.getChildren(),
              observedType.getChildren(),
              forceDoublePrecision);
  ...
  private MergeField mergeUnion(
    ...
    if (forceDoublePrecision
        && declaredField.getType() == Type.FLOAT
        && observedType.isUnion()
        && observedType.getChildren().size() == 2) {
    ...
    // check that the basic list types are the same. We don't compare full types here because it
    // could be that the two different structs (only a subset of fields showed up in one or both
    // structs).
    if (!isListTypeMatch(listChild, nonListType, forceDoublePrecision)
        && !listChild.getType().equals(Null.INSTANCE)) {
  ...
  private boolean isListTypeMatch(
      Field listChild, CompleteType nonListType, boolean forceDoublePrecision) {
    ...
    return forceDoublePrecision
        && listChildType.equals(CompleteType.DOUBLE)
        && nonListType.equals(CompleteType.FLOAT);
  }

  private MergeField mergeList(
      SchemaPath parent,
      ElasticField declaredField,
      CompleteType observedType,
      boolean forceDoublePrecision) {
  ...
  private UserException failure(
      SchemaPath path, ElasticField declaredField, CompleteType observedType) {
  ...
  public static class MergeField {
    ...
    public MergeField(
      ...
      if (!CompleteType.fromField(elasticField.toArrowField()).equals(actualType)
          && !(forceDoublePrecision
              && elasticField.getType() == Type.FLOAT
              && CompleteType.DOUBLE.equals(actualType))) {
    ...
    public MergeField asList() {
      return new MergeField(
          parent,
          elasticField,
          CompleteType.fromField(actualField).asList().toField(elasticField.getName()),
          children);
    }

    public Field toField(final ResultBuilder resultToPopulate) {
      final SchemaPath path =
          child(parent, elasticField != null ? elasticField.getName() : actualField.getName());
      ...
      if (!children.isEmpty()) {
        ...
        List<Field> fieldChildren =
            FluentIterable.from(elasticField.getChildren())
                .transform(
                    new Function<ElasticField, Field>() {
                      @Override
                      @Nullable
  ...
  private static void recordAnnotations(
      SchemaPath path, ElasticField elasticField, ResultBuilder resultToPopulate) {
    if (elasticField != null) {
  ...
  private static final class ResultBuilder {
    ...
    public void isStringWithKeywordType(SchemaPath path) {
      annotations.put(
          path, anno(path).setSpecialType(ElasticSpecialType.STRING_WITH_KEYWORD).build());
    }
