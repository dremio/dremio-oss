...
public class ITTestPredicatePushdown extends ElasticPredicatePushdownBase {
  ...
  public void testPartiallyPushedDownComplexConjunction() throws Exception {
    ...
    // only the first part of the expression is pushed down to the index since the last part
    // contains a non pushable operator (is not null)
    // the entire expression is pushed in a script for completeness
    String predicate =
        "(0 = integer_field" // pushed
            + " and "
            + "(7701633953967831279 <> long_field or "
            + NOT_PUSHABLE_TO_QUERY_ONLY_SCRIPTS
            + " ))";
    String sql =
        "select integer_field from elasticsearch." + schema + "." + table + " where " + predicate;
    ...
    testBuilder().sqlQuery(sql).unOrdered().baselineColumns("integer_field").baselineValues(0).go();
  }
  ...
  public void testPartiallyPushedDownComplexConjunctionShouldNotUseScript() throws Exception {
    ...
    String predicate =
        "text_field = 'string_value_0' and (0 = integer_field" // pushed
            ...
            + " ))";
    String sql =
        "select text_field, integer_field from elasticsearch."
            + schema
            + "."
            + table
            + " where "
    ...
    testBuilder()
        .sqlQuery(sql)
        .unOrdered()
        .baselineColumns("text_field", "integer_field")
        .baselineValues("string_value_0", 0)
        .go();
  }
  ...
  @Ignore("DX-5289")
  @Test
  public void testPartiallyPushedDownComplexConjunctionDifferentOrderShouldNotUseScripts()
      throws Exception {
    // same as previous test but changing the order to test for corner cases
    String predicate =
        "(7701633953967831279 <> long_field or "
            + NOT_PUSHABLE_TO_QUERY_ONLY_SCRIPTS
            + " ) and (text_field = 'string_value_0' and 0 = integer_field )";
    String sql =
        "select text_field, integer_field from elasticsearch."
            + schema
            + "."
  ...
  /**
   * The part of the filter that is not pushed down to the index [1] will filter out the matching
   * row testing correctness of the logic
   *
   * <p>[1] (7701633953967831279 = long_field or float_field is null)
   *
   * @throws Exception
   */
  @Test
  ...
  @Ignore("DX-5289")
  @Test
  public void testPartiallyPushedDownComplexConjunctionNoResultShouldNotUseScripts()
      throws Exception {
    // checking the correctness of the push down.
    // the script filter filters more than the index
    String predicate =
  ...
  public void testPartiallyPushedDisjunction() throws Exception {
    ...
    String sql =
        "select text_field, float_field from elasticsearch."
            + schema
            + "."
            + table
            + " where "
    ...
    testBuilder()
        .sqlQuery(sql)
        .unOrdered()
        .baselineColumns("float_field", "text_field")
        .baselineValues(0.0f, "string_value_0")
        .go();
  }
  ...
  public void testPartiallyPushedConjunction() throws Exception {
    ...
    String sql =
        "select float_field from elasticsearch." + schema + "." + table + " where " + predicate;
  ...
  public void testNotLike() throws Exception {
    ...
    String sql =
        String.format(
            "select city from elasticsearch.%s.%s where city not like '%%Cambridge%%' ",
            schema, table);
  ...
  public void testNotEqualWithNulls() throws Exception {
    ...
    String sql =
        String.format(
            "select city from elasticsearch.%s.%s where city <> 'Cambridge'", schema, table);
  ...
  public void testIsNull() throws Exception {
    ...
    String sql =
        String.format("select city from elasticsearch.%s.%s where city is null", schema, table);
  ...
  public void testNotEqual() throws Exception {
    ...
    String sql =
        String.format("select city from elasticsearch.%s.%s where city <> ''", schema, table);
  ...
  public void testLessThanWithNulls() throws Exception {
    ...
    String sql =
        String.format("select stars from elasticsearch.%s.%s where stars < 4", schema, table);
  ...
  public void testBooleanTypeInCompoundExpression() throws Exception {
    ...
    String sql =
        String.format(
            "select * from elasticsearch.%s.%s where \"open\" = 1 and city like '%%Oakland%%' ",
            schema, table);
  ...
  public void testComplexConjunctions() throws Exception {
    String s =
        "((0.7008325824013935 > double_field AND false < boolean_field) AND 0.9144399 = float_field)";
    validate(s);
  }
  ...
  public void testComplexConjunctionAndDisjunction() throws Exception {
    String s =
        "((0.7008325824013935 > double_field AND false < boolean_field) OR 0.9144399 = float_field)";
    validate(s);
  }
