...
public class ITTestNullValues extends ElasticBaseTestQuery {
  ...
  public final void runIsNullProjectFilter() throws Exception {
    String sqlQuery =
        "select stars, state, stars IS NULL from elasticsearch."
            + schema
            + "."
            + table
    ...
    testBuilder()
        .sqlQuery(sqlQuery)
        .unOrdered()
        .baselineColumns("stars", "state", "EXPR$2")
  ...
  public final void runTestArrayWithNulls() throws Exception {
    ...
    String sqlQuery =
        "select t.location_field[1].lat IS NULL from elasticsearch." + schema + "." + table + " t";
    verifyJsonInPlan(
        sqlQuery,
        new String[] {
          "[{\n"
  ...
  public final void runTestFilter() throws Exception {
    // TODO: this could be improved. Since we don't need stars, we could remove that from the
    // includes list. Since Edge projects are typically, off, probably not worth the effort.
    String sqlQuery =
  ...
  public final void runTestFieldsEquality() throws Exception {
    String sqlQuery =
        "select state, city, review_count, stars from elasticsearch."
            ...
            + "."
            + table
            + " where review_count = cast((stars * 33 / stars) as int)";
    verifyJsonInPlan(
        sqlQuery,
        new String[] {
          "[{\n"
  ...
  public final void runTestArrayAccess() throws Exception {
    ...
    String sqlQuery =
        "select min(t.location_field[1].lat) as lat_1 from elasticsearch."
            + schema
            + "."
            + table
            + " t";
    ...
    testBuilder().sqlQuery(sqlQuery).unOrdered().baselineColumns("lat_1").baselineValues(-55D).go();
  }
  ...
  public final void runComplexConditional() throws Exception {
    ...
    // TODO DX-4901 - this pushdown is currently incorrect, the null handling with case (and other
    // functions that are not
    // NULL_IF_NULL) needs to be fixed. For now this is avoiding running the second phase of the
    // test with project pushdown
  ...
  @Ignore("DX-11933: basic_t_Time vs basic_t_time between ES 5 and 6")
  @Test
    ...
    verifyJsonInPlan(
        ...
        new String[] {
          "[{\n"
              ...
              + "    \"script\" : {\n"
              + "      \"script\" : {\n"
              + "        \"inline\" : \"(def) ((doc[\\\"stars\\\"].empty) ? false : ( ( ( ( doc[\\\"stars\\\"].value < 2 ) ) ? (def) ( (doc[\\\"city\\\"].empty) ? false : doc[\\\"city\\\"].value ) : (def) ( (doc[\\\"state\\\"].empty) ? false : doc[\\\"state\\\"].value ) ) == 'San Francisco' ))\",\n"
              + "        \"lang\" : \"painless\"\n"
              + "      },\n"
              + "      \"boost\" : 1.0\n"
              + "    }\n"
  ...
  public final void runTestProjectWithNestedField() throws Exception {
    ...
    testBuilder()
        ...
        .baselineValues(
            listOf(mapOf("lat", 55D, "lon", 55D), mapOf("lat", -55D, "lon", -55D)), null)
        .go();
  }
