...
public class ITTestDateTypeWithMultipleFormatter extends BaseTestDateTypeWithMultipleFormatter {
  ...
  public final void runTestWithDefaultFormatterExtract() throws Exception {
    populateDefaultFormatter();
    final String sql =
        "select extract(year from \"field\"), extract(month from \"field\"),"
            + " extract(day from \"field\"), extract(hour from \"field\"), extract(minute from \"field\"),"
            + " extract(second from \"field\") from elasticsearch."
            + schema
            + "."
            + table;
    verifyJsonInPlan(
  ...
  public final void runTestWithDefaultFormatterConstantExtract() throws Exception {
    populateDefaultFormatter();
    final String sql =
        "select extract(year from date '2016-01-01'), extract(minute from date '2016-01-01') from elasticsearch."
            + schema
            + "."
            + table;
    verifyJsonInPlan(
  ...
  public final void runTestWithTimeFormatterExtract() throws Exception {
    populateTimeFormatter();
    final String sql =
        "select extract(hour from \"field\"), extract(minute from \"field\"), extract(second from \"field\") from elasticsearch."
            + schema
            + "."
            + table;
    verifyJsonInPlan(
