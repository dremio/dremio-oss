...
public class ITTestProjectionsAndFilter extends ElasticBaseTestQuery {
  ...
  public final void countLiteralGroupBy() throws Exception {
    final String sqlQuery =
        "select stars, count(5) as a, count(1) as b, count(*) as c from elasticsearch."
            + schema
            + "."
            + table
            + " where review_count < 10 group by stars";
    testPlanMatchingPatterns(sqlQuery, new String[0], new String[] {"Aggregate"});
    testBuilder()
        .sqlQuery(sqlQuery)
        .unOrdered()
        .baselineColumns("stars", "a", "b", "c")
  ...
  public final void countLiteral() throws Exception {
    final String sqlQuery =
        "select count(5) as a, count(1) as b, count(*) as c from elasticsearch."
            + schema
            + "."
            + table
  ...
  public final void runTestFilterSwapLessThan() throws Exception {
    final String sqlQuery =
        "select stars + 1 as plusOne from elasticsearch."
            + schema
            + "."
            + table
            + " where review_count < 10";
  ...
  public final void runTestFilterSwapLessThanEqual() throws Exception {
    final String sqlQuery =
        "select stars + 1 as plusOne from elasticsearch."
            ...
            + "."
            + table
            + " where review_count <= 11";
    final String sqlQuery2 =
        "select stars + 1 as plusOne from elasticsearch."
            + schema
            + "."
            + table
            + " where 11 >= review_count";
    final String plan =
        ""
            + "[{\n"
            + "  \"from\" : 0,\n"
  ...
  public final void runTestLike() throws Exception {
    String sqlQuery =
        "select city from elasticsearch." + schema + "." + table + " where city LIKE 'San%'";
    verifyJsonInPlan(
        sqlQuery,
        new String[] {
          "[{\n"
  ...
  public final void runTestNotLike() throws Exception {
    final String sqlQuery =
        "select city from elasticsearch." + schema + "." + table + " where city NOT LIKE 'San%'";
  ...
  public final void runTestFilterInClause() throws Exception {
    final String sqlQuery =
        "select review_count from elasticsearch."
            + schema
            + "."
            + table
  ...
  public final void runTestFieldsEquality() throws Exception {
    String sqlQuery1 =
        "select state, city, review_count, stars from elasticsearch."
            + schema
            + "."
            + table
            + " where cast(review_count as double) = stars";
    verifyJsonInPlan(
        sqlQuery1,
        new String[] {
          "[{\n"
  ...
  public final void runTestArrayAccessWithoutAggregate() throws Exception {
    ...
    String sqlQuery =
        "select t.location_field[1].lat as lat_1 from elasticsearch." + schema + "." + table + " t";
    verifyJsonInPlan(
        sqlQuery,
        new String[] {
          "[{\n"
  ...
  public final void runTestComplexProjectWithFilterProject() throws Exception {
    ...
    testBuilder().sqlQuery(sqlQuery).ordered().baselineColumns("plusOne").baselineValues(2.0F).go();
  }

  @Test
  public final void tesFilterRequiringScripts() throws Exception {
    String sqlQuery =
        "select state, stars from elasticsearch."
            + schema
            + "."
            + table
  ...
  public final void testFilterWithTrimFunc() throws Exception {
    ...
    String sqlQuery =
        "select city from elasticsearch."
            + schema
            + "."
            + table
  ...
  public final void runComplexConditionalNoPush() throws Exception {
    ...
    String sqlQuery =
        "select case when stars > 2 then 1 else 0 end as out_col from elasticsearch."
            + schema
            + "."
            + table;
    verifyJsonInPlan(
    ...
    // TODO - Jason review, this baseline changed from double type to long when executed in Dremio
    // instead of elastic
    // scripts, can numeric types other than double be returned from scripts?
    //    - actually I think I know why this is, Minji made the reader always read numbers as double
    // because
    //      elastic would not provide type information for scripted fields and we didn't want schema
    // changes
  ...
  public final void tesFilterWithCase() throws Exception {
    ...
    String sqlQuery =
        "select city from elasticsearch."
            ...
            + "."
            + table
            + " where case when stars < 2 then city else state end = 'San Francisco'";
    verifyJsonInPlan(
        sqlQuery,
        new String[] {
          "[{\n"
  ...
  public final void runTestMathOperators() throws Exception {
    // this complex expression just produces the number 3, all of the terms after the first just
    // cancel each other out
    // the expression is actually being constant folded, so this isn't the best test for the elastic
    // expression
    // pushdown, but keeping it here just to track how planning changes if we start failing to fold
    // constants
    String sqlQuery =
        "select stars + 2 - (2 * floor(power(2.1,2)))/4 + sqrt(4)/exp(0) - mod(5, 3) as complexIdentity "
            + "from elasticsearch."
            + schema
            + "."
            + table;
  ...
  public final void dateLessThan() throws Exception {
    String query =
        "select datefield from elasticsearch."
            + schema
            + "."
            + table
            + " where datefield2 <= datefield";
    testBuilder().sqlQuery(query).expectsEmptyResultSet().go();
  }

  @Test
  public final void dateLessThan2() throws Exception {
    // this test exposes a bug in the groovy scripts, but since this will soon be deprecated, no
    // point in fixing
    ...
    String query =
        "select datefield from elasticsearch."
            ...
            + "."
            + table
            + " where datefield < CURRENT_TIMESTAMP and datefield2 <= datefield";
    testBuilder().sqlQuery(query).expectsEmptyResultSet().go();
  }
  ...
  public final void runTestProjectExtractYearMonth() throws Exception {
    String sqlQuery =
        "select extract(year from \"datefield\"), extract(month from \"datefield\"), "
            + "extract(day from \"datefield\"), extract(hour from \"datefield\"), extract(minute from \"datefield\"),"
            + "extract(second from \"datefield\") from elasticsearch."
            + schema
            + "."
            + table;
    verifyJsonInPlan(
  ...
  public final void runTestProjectExtractDayFilter() throws Exception {
    ...
    String sqlQuery =
        "select datefield from elasticsearch."
            ...
            + "."
            + table
            + " where  extract(day from \"datefield\") = 10";
    verifyJsonInPlan(
        sqlQuery,
        new String[] {
          "[{\n"
              ...
              + "    \"script\" : {\n"
              + "      \"script\" : {\n"
              + "        \"source\" : \"(def) ((doc[\\\"datefield\\\"].empty) ? false : ( LocalDateTime.ofInstant(Instant.ofEpochMilli(doc[\\\"datefield\\\"]"
              + isValueOrIsDate
              + ".millis), ZoneOffset.UTC).getDayOfMonth() == 10L ))\",\n"
              + "        \"lang\" : \"painless\"\n"
              + "      },\n"
              + "      \"boost\" : 1.0\n"
              + "    }\n"
  ...
  public final void runTestProjectWithNestedField() throws Exception {
    final String sqlQuery =
        "select location_field, review_count from elasticsearch." + schema + "." + table;
    verifyJsonInPlan(
        sqlQuery,
        new String[] {
          "[{\n"
    ...
    testBuilder()
        .sqlQuery(sqlQuery)
        .unOrdered()
        .baselineColumns("location_field", "review_count")
  ...
  public final void runTestProjectString() throws Exception {
    String sqlQuery =
        "select city, location_field, review_count from elasticsearch." + schema + "." + table;
  ...
  public final void runTestWhereClauseWithNestedNoExpressionPushdown() throws Exception {
    String sqlQuery =
        "select t.\"open\", t.location_field[1]['lat'] as lat_1 from elasticsearch."
            + schema
            + "."
            + table
            + " t where t.location_field[1]['lat'] < -30";
    verifyJsonInPlan(
        sqlQuery,
        new String[] {
          "[{\n"
  ...
  public final void testCastToShortVarcharTruncates() throws Exception {
    String sqlQuery =
        "select city from elasticsearch."
            ...
            + "."
            + table
            + " where concat(cast(review_count as varchar(1)), '') = '3'";
    verifyJsonInPlan(
        sqlQuery,
        new String[] {
          "[{\n"
  ...
  public final void testMultipleFilters() throws Exception {
    final String sqlQuery =
        "select * from "
            + "(select state, city, review_count from elasticsearch."
            + schema
            + "."
            + table
    ...
    testBuilder()
        .sqlQuery(sqlQuery)
        .unOrdered()
        .baselineColumns("state", "city", "review_count")
        .baselineValues("CA", "San Diego", 33)
        .go();
  }
  ...
  public final void testJsonComparisonFailure() throws Exception {
    ...
    compareJsonExpectFailure("{\"a\":[[5]]}", "{ \"a\" : [[5], 7]}");
    compareJsonExpectFailure(
        "{\"a\":[{\"key\" : [5]}]}", "{ \"a\" : [{\"key\" : [5, {\"sub_key\" : 12345}]}]}");
  }

  // helper for negative json comparison tests
  protected static void compareJsonExpectFailure(String expected, String actual)
      throws IOException {
    ...
    try {
      ...
      if (!ex.getMessage().contains("Comparison between JSON values failed")) {
        throw new RuntimeException(
            "Negative test for json comparison failed with unexpected exception.");
      }
    }
    if (unexpectedSuccess) {
      throw new RuntimeException(
          "Negative test for json comparison, did not throw expected exception.");
    }
  }
  ...
  public void testCastKeywordToFloat() throws Exception {
    ...
    String sql =
        String.format(
            "select max(t1.val) as max_val from (SELECT CAST(\"float_val\" as float) as val FROM elasticsearch.%s.%s) as t1 where t1.val < 50",
            schema, table);
  ...
  public void testCastIntToVarchar() throws Exception {
    ...
    String sql =
        String.format(
            "SELECT count_val FROM elasticsearch.%s.%s where cast(count_val as varchar(100)) like \'2%%\'",
            schema, table);
    testBuilder().sqlQuery(sql).unOrdered().baselineColumns("count_val").baselineValues(200).go();
  }
}
