...
public class ITSampleHiveUDFs extends HiveTestBase {
  ...
  public void testLargeOutputValues() throws Exception {
    final String longVal1 =
        "Not every application requires security. Random assignment can be an efficient way for "
    ...
    String query =
        String.format(
            "SELECT "
                ...
                + "(values('%s'), ('%s'), (cast(null as varchar))) as t(col)",
            longVal1, longVal2);
    ...
    testBuilder()
        ...
        .baselineValues(
            longVal1.substring(0, 255) /* CHAR return type returns max of 255 chars */,
            longVal1,
            longVal1.getBytes(UTF_8))
        .baselineValues(
            longVal2.substring(0, 255) /* CHAR return type returns max of 255 chars */,
            longVal2,
            longVal2.getBytes(UTF_8))
  ...
  public void testDecimalPow() throws Exception {
    final String query = "select pow(c,2) from cp" + ".\"powHiveFunction.parquet\"";
    test(query);
  }
