...
public class TestCoalesceMixedHive extends PlanTestBase {
  ...
  public static void setup() throws Exception {
    testNoResult(
        "ALTER SYSTEM SET \"%s\" = 'concat_ws;unbase64'",
  ...
  public static void tearDown() throws Exception {
    testNoResult(
        "ALTER SYSTEM RESET \"%s\"", ExecConstants.DISABLED_GANDIVA_FUNCTIONS.getOptionName());
  }
  ...
  public void testHiveFunctionsWithGandivaOverridden() throws Exception {
    ...
    final String sql =
        "select CAST(base64(unbase64(t.currency_code)) AS VARCHAR(2)) as c1 "
  ...
  public void testCoalesceMixed() throws Exception {
    final String sql =
        "SELECT * FROM "
            + "( SELECT *, COUNT(*) OVER(PARTITION BY version, date_of_comparison, layer, table_name, source_system_name, period_year, period_name, record_existance, col_diff) AS cnt, ROW_NUMBER() OVER (PARTITION BY version, date_of_comparison, layer, table_name, source_system_name, period_year0, period_name, record_existance, col_diff) AS r FROM "
            + "( SELECT "
            ...
            + " COALESCE(cast(s.period_year as varchar), cast(t.period_year as varchar)) as period_year0, "
