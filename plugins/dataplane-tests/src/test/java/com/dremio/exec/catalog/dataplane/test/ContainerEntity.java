 ...
 *
 * <p>Represents a container (source or folder) that may contain tables, views, and other folders.
 * In the combination test, we are creating a similar set of tables and views, so this class helps
 * us be consistent about that creation.
 */
public class ContainerEntity {
  ...
  public List<String> getFullPath() {
    return Streams.concat(getParentPath().stream(), Stream.of(getName()))
  ...
  public List<String> getPathWithoutRoot() {
    ...
    return Streams.concat(getParentPath().stream().skip(1), Stream.of(getName()))
        .collect(Collectors.toList());
  }

  public List<String> getChildPathWithoutRoot(String childName) {
    return Streams.concat(getPathWithoutRoot().stream(), Stream.of(childName))
        .collect(Collectors.toList());
  }
  ...
  public List<List<String>> getExpectedTablesIncludingViews() {
    return Arrays.asList(
        Arrays.asList("DREMIO", DOT_JOINER.join(getFullPath()), tableAFirst, "TABLE"),
        Arrays.asList("DREMIO", DOT_JOINER.join(getFullPath()), tableBSecond, "TABLE"),
        Arrays.asList("DREMIO", DOT_JOINER.join(getFullPath()), viewCThird, "VIEW"),
        Arrays.asList("DREMIO", DOT_JOINER.join(getFullPath()), viewDFourth, "VIEW"));
  }
  ...
  public List<List<String>> getExpectedViewsForSelect1Query() {
    return Arrays.asList(
        Arrays.asList("DREMIO", DOT_JOINER.join(getFullPath()), viewCThird, "SELECT 1"),
        Arrays.asList("DREMIO", DOT_JOINER.join(getFullPath()), viewDFourth, "SELECT 1"));
  }
  ...
  public List<String> getExpectedSchemata() {
    return Arrays.asList("DREMIO", DOT_JOINER.join(getFullPath()), "<owner>", "SIMPLE", "NO");
  }
