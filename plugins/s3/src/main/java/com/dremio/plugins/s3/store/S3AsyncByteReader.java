...
class S3AsyncByteReader extends ReusableAsyncByteReader {
  ...
  public CompletableFuture<Void> readFully(long offset, ByteBuf dst, int dstOffset, int len) {
    ...
    logger.debug(
        "[{}] Submitted request to read from s3 bucket {}, path {} at starting offset {} for length {} ",
        threadName,
        bucket,
        path,
        offset,
  ...
  private CompletableFuture<Void> asyncReadWithRetry(
      long offset, ByteBuf dst, int dstOffset, int len, int retryAttemptNum) {
    ...
    CompletableFuture<Void> future =
        client.getObject(requestBuilder.build(), new ByteRangeReader(dst, dstOffset));
    ...
    return future
        .whenComplete(
            (response, throwable) -> {
              ...
              if (throwable == null) {
                logger.debug(
                    "Finished read of {}.{} for range {}..{} in {}ms.",
                    bucket,
                    path,
                    offset,
                    offset + len,
        ...
        .exceptionally(
            throwable -> {
              ...
              if (retryRequest || throwable.getCause() instanceof SdkException) {
                ...
                logger.warn(
                    "Retrying S3Async operation, exception was: {}",
  ...
  private static class ByteRangeReader
      implements AsyncResponseTransformer<GetObjectResponse, Void> {
  ...
  protected static String range(long start, long len) {
    ...
    // According to spec, the bytes should be inclusive bounded, thus inclusion of -1 to end
    // boundar.
