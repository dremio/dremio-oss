...
public class RexBiShuttle<P> implements RexBiVisitor<RexNode, P> {
  ...
  public RexWindow visitWindow(RexWindow window, P arg) {
    boolean[] update = {false};
    List<RexFieldCollation> clonedOrderKeys = visitFieldCollations(window.orderKeys, update, arg);
    List<RexNode> clonedPartitionKeys = visitList(window.partitionKeys, update, arg);
    ...
    if (update[0]
      ...
      return new RexWindow(
          clonedPartitionKeys, clonedOrderKeys, lowerBound, upperBound, window.isRows());
    } else {
      return window;
    }
  }
   ...
   * @return Array of visited expressions
   */
  protected List<RexNode> visitList(List<? extends RexNode> exprs, boolean[] update, P arg) {
  ...
  public void visitList(List<? extends RexNode> exprs, List<RexNode> outExprs, P arg) {
