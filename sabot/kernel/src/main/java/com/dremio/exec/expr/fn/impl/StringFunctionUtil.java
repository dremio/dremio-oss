...
public class StringFunctionUtil {
   ...
   */
  public static int getUTF8CharLength(
      ByteBuf buffer, int start, int end, final FunctionErrorContext errCtx) {
    int charCount = 0;
   ...
   *
   */
  public static int getUTF8CharPosition(
      ByteBuf buffer, int start, int end, int charLength, final FunctionErrorContext errCtx) {
    int charCount = 0;
  ...
  public static int stringLeftMatchUTF8(
      ByteBuf str, int strStart, int strEnd, ByteBuf substr, int subStart, int subEnd, int offset) {
  ...
  public static int parseBinaryStringNoFormat(
      ByteBuf str, int strStart, int strEnd, ByteBuf out, FunctionErrorContext errCtx) {
    int dstEnd = 0;
  ...
  /**
   * Returns the length of the valid UTF-8 character sequence that starts at offset 'idx' of
   * 'buffer'
   */
   ...
   * @throws com.dremio.common.exceptions.UserException if the first byte is not a valid encoded
   *     UTF-8 sequence
   */
  public static int utf8CharLen(ByteBuf buffer, int idx, final FunctionErrorContext errCtx) {
    ...
    throw errCtx
        .error()
        .message(
            "Unexpected byte 0x%s at position %d encountered while decoding UTF8 string.",
            Integer.toString(buffer.getByte(idx) & 0xff, 16), idx)
        .build();
  }
   ...
   * @param out the destination buffer, that will receive the UTF8 symbols. The buffer is assumed to
   *     be properly sized The buffer's reader- and writer-index will be set appropriately.
  ...
  public static int copyUtf8(ByteBuf in, final int start, final int end, ArrowBuf out) {
    ...
    while (start + i < end) {
      ...
      if (seqLen == 0
          || (start + i + seqLen) > end
          || !GuavaUtf8.isUtf8(in, start + i, start + i + seqLen)) {
        errBytes++;
  ...
  /**
   * Copy the UTF8 symbols from 'in' to 'out'. Any non-UTF8 symbols in 'in' will be replaced by
   * 'replacement'
   *
   * @param in source of the copy
   * @param start index in 'in' where the copy will begin
   * @param end one past the index in 'in' where the copy will end
   * @param out the destination buffer, that will receive copy/replace. The buffer is assumed to be
   *     properly sized The buffer's reader- and writer-index will be set appropriately.
   * @param replacement replacement value for non-UTF8 symbols in 'in'
   * @return the number of bytes in 'out' that were populated
   */
  public static int copyReplaceUtf8(
      ByteBuf in, final int start, final int end, ByteBuf out, byte replacement) {
    int i = 0;
    while (start + i < end) {
      // Optimize for long runs of ASCII sequences
      byte b = in.getByte(start + i);
  ...
  // input in uppercase.
  public static String soundexCleanUtf8(
      NullableVarCharHolder in, final FunctionErrorContext errCtx) {
  ...
  public static void concatWsWord(
      NullableVarCharHolder out, NullableVarCharHolder in, NullableVarCharHolder separator) {
    if (in.isSet == 0) {
      return;
    }
    ...
    while (s < str.length()) {
      if (p < pattern.length() && pattern.charAt(p) == '%') {
        ...
        p++;
          ...
          && (pattern.charAt(p) == '_' || pattern.charAt(p) == str.charAt(s))) {
        s++;
        p++;
      } else if (starIdx != -1) {
        p = starIdx + 1;
