...
public final class DrelTransformer {
  ...
  public static Rel convertToDrel(SqlHandlerConfig config, final RelNode relNode) throws SqlUnsupportedException {
    try {
      ...
      final RelNode projPush = PlannerUtil.transform(config, PlannerType.HEP_AC, PlannerPhase.PROJECT_PUSHDOWN, rangeConditionRewrite, rangeConditionRewrite.getTraitSet(), true);
      final RelNode projPull = projectPullUp(config, projPush);
      final RelNode filterConstantPushdown = PlannerUtil.transform(config, PlannerType.HEP_AC, PlannerPhase.FILTER_CONSTANT_RESOLUTION_PUSHDOWN, projPull, projPull.getTraitSet(), true);
      final RelNode transitiveFilterPushdown = transitiveFilterPushdown(config, filterConstantPushdown);
      final RelNode preLog = PlannerUtil.transform(config, PlannerType.HEP_AC, PlannerPhase.PRE_LOGICAL, transitiveFilterPushdown, transitiveFilterPushdown.getTraitSet(), true);
      final RelNode logical = PlannerUtil.transform(config, PlannerType.VOLCANO, PlannerPhase.LOGICAL, preLog, preLog.getTraitSet().plus(Rel.LOGICAL), true);
      final RelNode sampledPlan = logical.accept(new InjectSample(plannerSettings.isLeafLimitsEnabled()));
      ...
      // Do Join Planning.
      final RelNode preConvertedRelNode = PlannerUtil.transform(config, PlannerType.HEP_BOTTOM_UP, PlannerPhase.JOIN_PLANNING_MULTI_JOIN, nestedProjectPushdown, nestedProjectPushdown.getTraitSet(), true);
      final RelNode convertedRelNode = PlannerUtil.transform(config, PlannerType.HEP_BOTTOM_UP, PlannerPhase.JOIN_PLANNING_OPTIMIZATION, preConvertedRelNode, preConvertedRelNode.getTraitSet(), true);
      final RelNode postJoinOptimizationRelNode = PlannerUtil.transform(config, PlannerType.HEP_AC, PlannerPhase.POST_JOIN_OPTIMIZATION, convertedRelNode, convertedRelNode.getTraitSet(), true);
  ...
  private static RelNode projectPullUp(SqlHandlerConfig config, RelNode projPush){
    ...
    if(plannerSettings.isProjectPullUpEnabled()){
      return PlannerUtil.transform(config, PlannerType.HEP_AC, PlannerPhase.PROJECT_PULLUP, projPush, projPush.getTraitSet(), true);
    }else{
      return projPush;
    }
  }
  ...
  private static RelNode transitiveFilterPushdown(SqlHandlerConfig config, RelNode filterPushdown) {
    ...
    if (plannerSettings.isTransitiveFilterPushdownEnabled()) {
      ...
      PlanLogUtil.log(PlannerType.HEP, PlannerPhase.TRANSITIVE_PREDICATE_PULLUP, joinPullFilters, LOGGER, watch);
      config.getObserver().planRelTransform(PlannerPhase.TRANSITIVE_PREDICATE_PULLUP, null, filterPushdown, joinPullFilters, watch.elapsed(
        TimeUnit.MILLISECONDS), Collections.emptyMap());
      return PlannerUtil.transform(config, PlannerType.HEP_AC, PlannerPhase.FILTER_CONSTANT_RESOLUTION_PUSHDOWN, joinPullFilters, joinPullFilters.getTraitSet(), true);
    }
  ...
  private static RelNode decorrelate(RelNode relNode) {
    ...
    final RelNode decorrelatedQuery = DremioRelDecorrelator.decorrelateQuery(relNode, relBuilder, true);
  ...
  private static RelNode pushDownJdbcQuery(SqlHandlerConfig config, RelNode relNode) {
    ...
    RelNode relationPlanningRel = PlannerUtil.transform(
      config,
      PlannerType.HEP_AC,
  ...
  private static RelNode nestedProjectPushdown(SqlHandlerConfig config, RelNode relNode){
    ...
    RelNode transformed = PlannerUtil.transform(config, PlannerType.HEP_AC, PlannerPhase.NESTED_SCHEMA_PROJECT_PUSHDOWN,
    wrapped, wrapped.getTraitSet(), true);
    RelNode unwrapped = RexFieldAccessUtils.unwrap(transformed);
    RelNode projectPushedDown = PlannerUtil.transform(config, PlannerType.HEP_BOTTOM_UP, PlannerPhase.FILESYSTEM_PROJECT_PUSHDOWN,
  ...
  private static RelNode getFlattenedPushed(SqlHandlerConfig config, RelNode convertedRelNode) {
    ...
    if (flattenFinder.run(convertedRelNode)) {
      ...
      RelNode transformed = PlannerUtil.transform(config, PlannerType.VOLCANO, PlannerPhase.FLATTEN_PUSHDOWN,
