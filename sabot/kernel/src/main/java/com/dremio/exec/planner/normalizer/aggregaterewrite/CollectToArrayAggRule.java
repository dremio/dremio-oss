 ...
 * For example:
 *
 * SELECT ARRAY(SELECT ENAME FROM EMP WHERE DEPTNO = 20)
 *
 * Get's rewritten to:
 *
 * SELECT ARRAY_AGG(ENAME)
 * FROM EMP
 * WHERE DEPTNO = 20
 *
 * If we want to talk about RelNodes then the plans are:
 *
 * Collect(field=[EXPR$0])
 *   LogicalProject(ENAME=[$1])
 *     LogicalFilter(condition=[=($6, 20)])
...
public final class CollectToArrayAggRule extends RelOptRule {
  ...
  public void onMatch(RelOptRuleCall call) {
    ...
    RelNode rewritenRelNode = rewrite(
      call.builder(),
      ((HepRelVertex) collect.getInput()).getCurrentRel(),
      false);
  ...
  public static RelNode rewrite(RelBuilder relBuilder, RelNode input, boolean isCorrelated) {
    ...
    RelNode rewrittenQuery = relBuilder
      .push(inputInfo.inputForAggregation)
      .aggregate(
        // We need to GROUP BY the ARRAY / JOIN Key
        // otherwise we will be applying ARRAY_AGG on the UNNEST'ing of all ARRAYs in the dataset:
        isCorrelated
          ? relBuilder.groupKey(input.getCluster().getRexBuilder().makeInputRef(inputInfo.inputForAggregation, 0))
          : relBuilder.groupKey(),
  ...
  private static InputInfo extractInputInfo(RelNode input, boolean isCorrelated) {
    ...
    return new InputInfo(inputForAggregation, collectInputExtractor.hasDistinct, collectInputExtractor.hasSort);
  }

  /**
   * Extracts out the actual input we are going to apply ARRAY_AGG on.
   * If the provided RelNode has a DISTINCT or ORDER BY before the ARRAY_AGG,
   * then we need to push this to the AggCall, things it's not defined what "aggregating" on an ordered set means.
   */
  private static final class CollectInputExtractor extends StatelessRelShuttleImpl {
    ...
    public RelNode visit(LogicalAggregate aggregate) {
      ...
      if (!aggregate.getAggCallList().isEmpty()) {
        ...
        throw new UnsupportedOperationException("Did not expect aggregate to have aggregate calls in this context.");
      }
