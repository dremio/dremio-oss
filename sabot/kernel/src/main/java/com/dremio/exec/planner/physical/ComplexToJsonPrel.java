...
public class ComplexToJsonPrel extends SingleRel implements Prel, CustomPrel {
  ...
  public static final LongValidator RESERVE =
      new PositiveLongValidator(
          "planner.op.complexToJson.reserve_bytes", Long.MAX_VALUE, DEFAULT_RESERVE);
  public static final LongValidator LIMIT =
      new PositiveLongValidator(
          "planner.op.complexToJson.limit_bytes", Long.MAX_VALUE, DEFAULT_LIMIT);
  ...
  private static class SchemaConverter extends AbstractSchemaConverter {
    ...
    Field asVarchar() {
      return new Field(
          field.getName(),
          new FieldType(field.isNullable(), Utf8.INSTANCE, field.getDictionary()),
