...
public class MetadataRefreshInfoBroadcaster {
  private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(MetadataRefreshInfoBroadcaster.class);
  ...
  public MetadataRefreshInfoBroadcaster(Provider<ConduitProvider> conduitProvider,
  ...
  public void communicateChange(UpdateLastRefreshDateRequest refreshRequest) {
    ...
    for (NodeEndpoint nodeEndpoint: allCoordinators) {
      ...
      stub.updateRefreshDate(refreshRequest, new StreamObserver<Empty>() {
        @Override
        public void onNext(Empty empty) {}

        @Override
        public void onError(Throwable t) {
          logger.warn("Source '{}' error when CatalogServiceSynchronizer tried to update plugin's last refresh date.",
            refreshRequest.getPluginName(), t);
        }

        @Override
        public void onCompleted() {
          logger.debug("Source '{}' Successfully updated last refresh date in coordinator {}:{}.",
            refreshRequest.getPluginName(), nodeEndpoint.getAddress(), nodeEndpoint.getUserPort());
        }
      });
    }
  }
