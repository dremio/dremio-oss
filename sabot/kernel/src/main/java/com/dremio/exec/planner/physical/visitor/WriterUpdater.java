...
public class WriterUpdater extends BasePrelVisitor<Prel, Void, RuntimeException> {
  ...
  private static final String nonPartitionTableDummyPartitionValue = "p0$DREMIO_nonPartitionTableValue";
  private static String notSmallFileDummyPartitionValue = "p0$DREMIO_notSmallFilePartitionTableValue";
  ...
  public Prel visitWriter(WriterPrel initialPrel, Void value) throws RuntimeException  {
    ...
    Prel updatedWritePrel = updateWriter(prel, input, tableEntry, true, prel.getExpectedInboundRowType());

    // combine small files if needed
    return combineSmallFiles(updatedWritePrel, input, tableEntry, prel.getExpectedInboundRowType());
  }

  private Prel updateWriter(Prel prel, Prel input, CreateTableEntry tableEntry, boolean addSort, RelDataType expectedWriterInboundRowType) {
    ...
    if(options.hasDistributions()){
      ...
      if(distribution.getType() != DistributionType.HASH_DISTRIBUTED && distribution.getType() != DistributionType.ADAPTIVE_HASH_DISTRIBUTED){
      ...
      // last column is the hash modulo expression. this column will be added to sort keys and changeDirection detection columns
      List<Integer> hashFieldIndex = ImmutableList.of(project.getRowType().getFieldCount() - 1);

      return updateWriterWithPartition(prel, project, hashFieldIndex, tableEntry, addSort, expectedWriterInboundRowType);
      ...
      return new WriterPrel(prel.getCluster(), prel.getTraitSet(), sort, tableEntry, expectedWriterInboundRowType);
    } else {
      return prel;
    }
  }

  private Prel addSort(Prel prel, Prel input, WriterOptions options, List<Integer> additionalFieldIndices, List<String> partitionColumns, RelDataType inputRowType) {
    ...
    return SortPrel.create(input.getCluster(), input.getTraitSet().plus(collation), input, collation);
  }

  private Prel updateWriterWithPartition(Prel prel, Prel input, List<Integer> additionalFieldIndices, CreateTableEntry tableEntry,
                                         boolean addSort, RelDataType expectedWriterRowType)  {
    WriterOptions options = tableEntry.getOptions();
    ...
    return new WriterPrel(prel.getCluster(), prel.getTraitSet(), changeDetectionPrel, tableEntry,
   ...
   *
   * Union (combined Small Files + Pass-through Files)
   * |
   * |-------------------------------------------------
   * |                                                |
   ...
   * BridgeExchange------------------------------------
   * |
   * Window (count files by partition values)
   * |
   * Sort( sort files by partition values)
   * |
   * Project (add Partition Values column)
   * |
   * Original Writer Output
   */
  private Prel combineSmallFiles(Prel writerPrel, Prel writerInputPrel, CreateTableEntry tableEntry, RelDataType expectedWriterInboundRowType)  {
    WriterOptions options = tableEntry.getOptions();
    PlannerSettings plannerSettings = PrelUtil.getPlannerSettings(writerPrel.getCluster());
    boolean combineSmallFilesEnabledForPartitionedTable = plannerSettings.getOptions().getOption(ENABLE_ICEBERG_COMBINE_SMALL_FILES_FOR_PARTITIONED_TABLE_WRITES);
    if (!options.getTableFormatOptions().isTableFormatWriter() ||
      options.getCombineSmallFileOptions() == null ||
      // Writing to partitioned tables does not generate small files since the Sort before Writer will guarantee all rows with
      // same partition values will be written together.
      // After the Sort is removed from writer plan (DX-58885), we need turn on small-file combination for partitioned tables too
    ...
    try {
      ...
      RelDataTypeField filePathField = prel.getRowType().getField(RecordWriter.PATH_COLUMN, false, false);
      RelDataTypeField fileSizeField = prel.getRowType().getField(RecordWriter.FILESIZE_COLUMN, false, false);
      final String partitionValuesFieldName = "p0$partitionValuesColumn";

      // add a projected column containing partition values
      prel = addPartitionValueField(rexBuilder, typeFactory, prel, options.hasPartitions(), fileSizeField,
        partitionValuesFieldName, options.getCombineSmallFileOptions());
      ...
      RelDataTypeField partitionValuesField = prel.getRowType().getField(partitionValuesFieldName, false, false);
      final RelCollation collation = getCollation(prel.getTraitSet(), ImmutableList.of(partitionValuesField.getIndex()));
      prel = SortPrel.create(prel.getCluster(), prel.getTraitSet().plus(collation), prel, collation);

      // add SingleMergeExchange to merge local sort result
      prel = new SingleMergeExchangePrel(prel.getCluster(), prel.getTraitSet().plus(Prel.PHYSICAL).plus(DistributionTrait.SINGLETON),
        prel, ((SortPrel)prel).getCollation());

      // add Windowing to count files per partition value
      // The count is used to filter out the single small data file which wont go to the second-round writer
      final String partitionFileCountFieldName = "w0$partitionFileCount";
      prel = addWindowingToCountDataFilesPerPartitionValue(rexBuilder, typeFactory, prel, partitionFileCountFieldName, partitionValuesField);

      String bridgeId = UUID.randomUUID().toString();
      // add bridge exchange so that data files output from first-round writing would be used by both small-file combination side plan and pass-though side plan
      prel = new BridgeExchangePrel(prel.getCluster(),
        prel.getTraitSet(),
        prel,
        bridgeId);

      // construct small file combining side plan
      RelDataTypeField partitionFileCountField = prel.getRowType().getField(partitionFileCountFieldName, false, false);
      Prel smallFilesSidePrel = getSmallFilesSidePrel(writerInputPrel,
        rexBuilder,
        typeFactory,
        prel,
        filePathField,
      ...
      Prel passThroughFilesPrel = getPassThroughFilesPrel(bridgeId, rexBuilder, typeFactory, prel, partitionFileCountField, partitionValuesField);
      ...
      List<RelNode> unionAllInputs = PrelUtil.convertUnionAllInputs(
        plannerSettings, smallFilesSidePrel, passThroughFilesPrel);

      // union combined small files with pass-through files
      return new UnionAllPrel(smallFilesSidePrel.getCluster(),
        smallFilesSidePrel.getTraitSet(),
        unionAllInputs,
        false);
  ...
  /**
   * Add a projected column containing partition values.
   *        a. for non-small files, set dummy partition value "p0$DREMIO_notSmallFilePartitionTableValue", regardless partitioned table or not.
   *           files with this partition value wont go into the second round writing
   *        b. for small files in non-partitioned table, set dummy partition value "p0$DREMIO_nonPartitionTableValue"
   *        c. for small files in partitioned table, set real partition value getting from first-round writer output column RecordWriter.PARTITION_VALUE_COLUMN
   */
  private Prel addPartitionValueField(RexBuilder rexBuilder,
    ...
    List<RexNode> ProjectExprs = prel.getRowType().getFieldList().stream()
      .map(field -> rexBuilder.makeInputRef(field.getType(), field.getIndex()))
      .collect(Collectors.toList());

    RexNode notSmallFileDummyPartitionValueExpr = rexBuilder.makeLiteral(notSmallFileDummyPartitionValue);
    List<String> projectNames = prel.getRowType().getFieldList().stream()
      .map(field -> field.getName())
      .collect(Collectors.toList());
    projectNames.add(partitionValuesFieldName);

    RexNode notSmallFileCondition = rexBuilder.makeCall(
      SqlStdOperatorTable.GREATER_THAN_OR_EQUAL,
      rexBuilder.makeInputRef(fileSizeField.getType(), fileSizeField.getIndex()),
      rexBuilder.makeLiteral(combineSmallFileOptions.getSmallFileSize(), typeFactory.createSqlType(SqlTypeName.BIGINT)));

    RelDataTypeField operationTypeField = prel.getRowType().getField(RecordWriter.OPERATION_TYPE_COLUMN, false, false);
    RexNode addedDataFileOnlyCondition = rexBuilder.makeCall(SqlStdOperatorTable.EQUALS,
      rexBuilder.makeInputRef(operationTypeField.getType(), operationTypeField.getIndex()),
      rexBuilder.makeLiteral(OperationType.ADD_DATAFILE.value, typeFactory.createSqlType(SqlTypeName.INTEGER)));
    ...
    if (!hasPartitions) {
      ...
      partitionValuesFieldExpr = rexBuilder.makeCall(SqlStdOperatorTable.CASE, addedDataFileOnlyCondition,
        rexBuilder.makeLiteral(nonPartitionTableDummyPartitionValue),
        rexBuilder.makeNullLiteral(nullableVarchar));
    } else {
      // Partitioned tables, get the partition value from first-round writer output field RecordWriter.PARTITION_VALUE_COLUMN
      RelDataTypeField partitionValueField = prel.getRowType().getField(RecordWriter.PARTITION_VALUE_COLUMN, false, false);
      partitionValuesFieldExpr = rexBuilder.makeCall(SqlStdOperatorTable.CASE, addedDataFileOnlyCondition,
        rexBuilder.makeInputRef(partitionValueField.getType(), partitionValueField.getIndex()),
        rexBuilder.makeLiteral(nonPartitionTableDummyPartitionValue));
    }
    partitionValuesFieldExpr = rexBuilder.makeCall(SqlStdOperatorTable.CASE, notSmallFileCondition,
      notSmallFileDummyPartitionValueExpr, partitionValuesFieldExpr);
    ProjectExprs.add(partitionValuesFieldExpr);

    RelDataType fileCountFieldRowType = RexUtil.createStructType(prel.getCluster().getTypeFactory(), ProjectExprs,
      projectNames, null);
    ...
    return ProjectPrel.create(
      ...
      prel,
      ProjectExprs,
      fileCountFieldRowType);
  }

  /**
   *  Add Windowing to count files per partition value.
   *  The count is used to filter out single small data file which wont go to the second-round writer
   */
  private Prel addWindowingToCountDataFilesPerPartitionValue(RexBuilder rexBuilder,
                                                             RelDataTypeFactory typeFactory,
                                                             Prel prel,
                                                             String partitionFileCountColumnName,
                                                             RelDataTypeField partitionValuesColumn) {
    // add Windowing to count files per partition value
    Window.RexWinAggCall countOnPartitionColumnCall = new Window.RexWinAggCall(SqlStdOperatorTable.COUNT,
      typeFactory.createSqlType(SqlTypeName.BIGINT),
      ImmutableList.of(rexBuilder.makeInputRef(prel, partitionValuesColumn.getIndex())), 0, false);

    Window.Group group = new Window.Group(
      // for partitioned table, group by partition values, for non-partitioned table, group by operation type so that we could get the count of added files
    ...
    types.addAll(prel.getRowType().getFieldList().stream().map(RelDataTypeField::getType).collect(Collectors.toList()));
    types.add(typeFactory.createSqlType(SqlTypeName.BIGINT));

    return WindowPrel.create(prel.getCluster(), prel.getTraitSet(), prel, ImmutableList.of(), typeFactory.createStructType(types, names), group);
  }
   ...
   */
  private RexNode getSmallFilesConditions(Prel prel,
    ...
    RexNode multipleSmallFileCondition = rexBuilder.makeCall(SqlStdOperatorTable.GREATER_THAN,
      rexBuilder.makeInputRef(partitionFileCountField.getType(), partitionFileCountField.getIndex()),
      rexBuilder.makeLiteral(1, typeFactory.createSqlType(SqlTypeName.BIGINT)));

    // excludes non-small files
    RexNode notSmallFilesCondition = rexBuilder.makeCall(
      SqlStdOperatorTable.NOT_EQUALS,
      rexBuilder.makeInputRef(partitionValuesField.getType(), partitionValuesField.getIndex()),
  ...
  /**
   *  Add small file filter
   *    1. there are two or more small file per partition value
   *    2. the files are not small files
   */
  private Prel addSmallFilesFilterPrel(RexBuilder rexBuilder,
                                       RelDataTypeFactory typeFactory,
                                       Prel prel,
                                       RelDataTypeField partitionFileCountField,
                                       RelDataTypeField partitionValuesField) {
    RexNode smallFilesConditions = getSmallFilesConditions(prel, rexBuilder, typeFactory, partitionFileCountField, partitionValuesField);

    return FilterPrel.create(
      prel.getCluster(),
      prel.getTraitSet(),
      prel,
      smallFilesConditions);
  }
   ...
   * IcebergSplitGen
   * |
   * Project ( from RecordWriter.SCHEMA to IcebergSplitGen required input schema)
   * |
   * Small Data File Records
   */
  private Prel readRowsFromSmallFiles(RexBuilder rexBuilder,
                                      RelDataTypeFactory typeFactory,
                                      Prel prel,
    ...
    List<String> projectNames = ImmutableList.of(
    ...
    RelDataType nullableVarbinary = typeFactory.createTypeWithNullability(nonNullableVarbinary, true);
    projectExprs.add(rexBuilder.makeNullLiteral(nullableVarbinary));

    RelDataType projectRowType = RexUtil.createStructType(prel.getCluster().getTypeFactory(), projectExprs,
      projectNames, null);

    prel = ProjectPrel.create(
      prel.getCluster(),
      prel.getTraitSet(),
      prel,
      projectExprs,
    ...
    // table scan phase
    final FileConfig config = new IcebergFileConfig()
      .setParquetDataFormat(new ParquetFileConfig())
      .asFileConfig()
      .setLocation(tableEntry.getLocation())
      .setCtime(1L)
      .setOwner(tableEntry.getUserName());
    boolean limitDataScanParallelism = PrelUtil.getPlannerSettings(prel.getCluster()).getOptions().getOption(DATA_SCAN_PARALLELISM);
    TableFunctionContext dataScanContext = new TableFunctionContext(
      config,
    ...
    return new TableFunctionPrel(prel.getCluster(),
      prel.getTraitSet(),
      prel.getTable(),
      prel,
      null,
  ...
  /**
   *  Assign file size bucket to files in unpartitioned table.
   *  1. get the running total of file size
   *  2. convert file size running total into bucket number
   *      bucket_number = currentFileSizeRunningTotal / bucketSize
   */
  private Prel assignFileSizeBucketToUnpartitionedTable(RexBuilder rexBuilder,
                                                        RelDataTypeFactory typeFactory,
                                                        Prel prel,
    ...
    Window.RexWinAggCall countOnPartitionColumnCall = new Window.RexWinAggCall(SqlStdOperatorTable.SUM,
      typeFactory.createSqlType(SqlTypeName.BIGINT),
      ImmutableList.of(rexBuilder.makeInputRef(prel, fileSizeField.getIndex())), 0, false);
    ...
    Prel fileSizeRunningTotalPrel = WindowPrel.create(prel.getCluster(), prel.getTraitSet(), prel, ImmutableList.of(), typeFactory.createStructType(types, names), group);

    // convert file size running total into bucket number
    // bucket_number = currentFileSizeRunningTotal / bucketSize
    long fileSizeBucketSize = PrelUtil.getPlannerSettings(prel.getCluster()).getOptions().getOption(PARQUET_BLOCK_SIZE_VALIDATOR);
    List<RexNode> fileSizeBucketProjectExprs = prel.getRowType().getFieldList().stream()
      .map(field -> rexBuilder.makeInputRef(field.getType(), field.getIndex()))
      .collect(Collectors.toList());

    List<String> fileSizeBucketProjectNames = prel.getRowType().getFieldList().stream()
      .map(field -> field.getName())
      .collect(Collectors.toList());
    fileSizeBucketProjectNames.add(fileSizeBucketFieldName);

    final SqlFunction division = new SqlFunction(
      "/",
    ...
    RelDataTypeField fileSizeRunningTotalField = fileSizeRunningTotalPrel.getRowType().getField(fileSizeRunningTotalFieldName, false, false);
    RexNode fileSizeBucketFieldExpr = rexBuilder.makeCall(division, rexBuilder.makeInputRef(fileSizeRunningTotalField.getType(), fileSizeRunningTotalField.getIndex()),  rexBuilder.makeLiteral(Long.toString(fileSizeBucketSize)));
    fileSizeBucketProjectExprs.add(fileSizeBucketFieldExpr);

    RelDataType fileSizeBucketFieldRowType = RexUtil.createStructType(prel.getCluster().getTypeFactory(), fileSizeBucketProjectExprs,
      fileSizeBucketProjectNames, null);
  ...
  /**
   *   WriterPrel (second-round writing to combine small files)
   *   |
   *   Read From Small Files
   *   |
   *   Hash Exchange on (partitioned table: partition value, non-partitioned table: bucket#
   *   |
   *   Assign bucket# for non-partitioned tables (only for non-partitioned tables)
   *   |
   *   Filter (keep non-single small files only)
   */
  private Prel getSmallFilesSidePrel(Prel originalWriterInputPrel,
                                     RexBuilder rexBuilder,
                                     RelDataTypeFactory typeFactory,
                                     Prel prel,
                                     RelDataTypeField filePathField,
    ...
    Prel smallFilesSidePrel = addSmallFilesFilterPrel(rexBuilder, typeFactory, prel, partitionFileCountField, partitionValuesField);
    ...
    if (!tableEntry.getOptions().hasPartitions()) {
      smallFilesSidePrel = assignFileSizeBucketToUnpartitionedTable(rexBuilder, typeFactory, smallFilesSidePrel, fileSizeBucketFieldName, filePathField, fileSizeField);
    }

    // hash distribute files on
    //   * partition value column for partitioned tables
    //            Since the files are already ordered, hash distribution from the single sender(by the MergeExchange before bridge)
    //            would guarantee files belonging to the same partition values pack together on the receiver side
    //            thus, we dont need Sort in the second-round read/write rows
    //   * file size bucket column for non-partitioned tables
    List<String> hashDistributionFields = ImmutableList.of(tableEntry.getOptions().hasPartitions() ? partitionValuesField.getName() : fileSizeBucketFieldName);
    smallFilesSidePrel = getHashToRandomExchangePrel(smallFilesSidePrel, hashDistributionFields);

    // read rows from small files
    Function<RelMetadataQuery, Double> estimateRowCountFn = rm -> rm.getRowCount(originalWriterInputPrel);
    smallFilesSidePrel = readRowsFromSmallFiles(rexBuilder, typeFactory, smallFilesSidePrel, filePathField, fileSizeField, tableEntry, estimateRowCountFn);

    // add second-round Writer
    CreateTableEntry tableEntryWithTargetFileSize = tableEntry.cloneWithFields(tableEntry.getOptions()
      .withCombineSmallFileOptions(CombineSmallFileOptions.builder()
        .setSmallFileSize(tableEntry.getOptions().getCombineSmallFileOptions().getSmallFileSize())
        .setTargetFileSize(PrelUtil.getPlannerSettings(smallFilesSidePrel.getCluster()).getOptions().getOption(ExecConstants.TARGET_COMBINED_SMALL_PARQUET_BLOCK_SIZE_VALIDATOR))
        .build()));

    Prel updatedWriter = updateWriter(prel, smallFilesSidePrel, tableEntryWithTargetFileSize, false, expectedWriterInboundRowType);
    if (updatedWriter == prel) {
      return new WriterPrel(prel.getCluster(), prel.getTraitSet(), smallFilesSidePrel, tableEntryWithTargetFileSize, expectedWriterInboundRowType);
    }
   ...
   */
  private Prel markOrphanFiles(RexBuilder rexBuilder,
                              RelDataTypeFactory typeFactory,
                              Prel prel,
                              RelDataTypeField partitionValuesField,
                              RelDataTypeField partitionFileCountField) {
    ...
    for (RelDataTypeField field : prel.getRowType().getFieldList()) {
      ...
      if (field.getName().equals(OPERATION_TYPE_COLUMN)) {
        RexNode orphanFileOperationType = rexBuilder.makeLiteral(OperationType.ORPHAN_DATAFILE.value, typeFactory.createSqlType(SqlTypeName.INTEGER));
        RexNode operationTypeFieldExpr = rexBuilder.makeCall(SqlStdOperatorTable.CASE, smallFilesConditions,
          orphanFileOperationType, original);
        ...
        RexNode zeroRecordForSmallFilesExpr = rexBuilder.makeCall(SqlStdOperatorTable.CASE, smallFilesConditions,
          zeroRecordExpr, original);
  ...
  /**
   * Those files out from first-round writer will pass through the second round writer. if
   * 1. not small files
   * 2. there is only one such small file per partition value
   *
   * For small files to be combined, mark them as orphan files to delete
   */
  private Prel getPassThroughFilesPrel(String bridgeId,
                                       RexBuilder rexBuilder,
                                       RelDataTypeFactory typeFactory,
                                       Prel prel,
                                       RelDataTypeField partitionFileCountField,
    ...
    Prel receiverPrel = new BridgeReaderPrel(prel.getCluster(),
      prel.getTraitSet(),
      prel.getRowType(),
      1d,
      bridgeId);
    ...
    Prel passThroughPrel = markOrphanFiles(rexBuilder, typeFactory, receiverPrel, partitionValuesField, partitionFileCountField);

    // remove partitionValuesFieldName and partitionFileCountFieldName to align with RecordWriter.SCHEMA
    List<RexNode> projectWriterColumnsExprs = passThroughPrel.getRowType().getFieldList().stream()
      .filter(field -> !field.getName().equals(partitionFileCountFieldName) && !field.getName().equals(partitionValuesFieldName))
      .map(field -> rexBuilder.makeInputRef(field.getType(), field.getIndex()))
      .collect(Collectors.toList());
    List<String> projectWriterColumnsNames = passThroughPrel.getRowType().getFieldList().stream()
      .filter(field -> !field.getName().equals(partitionFileCountFieldName) && !field.getName().equals(partitionValuesFieldName))
      .map(field -> field.getName())
      .collect(Collectors.toList());
    RelDataType projectWriterColumnsRowType = RexUtil.createStructType(passThroughPrel.getCluster().getTypeFactory(), projectWriterColumnsExprs,
      projectWriterColumnsNames, null);
  ...
  BatchSchema getNewBatchSchema(BatchSchema batchSchema, PartitionSpec partitionSpec, List<String> partitionColumns, RelDataType inputRowType) {
    ...
    if (inputRowType.getFieldList().stream().map(RelDataTypeField::getName).anyMatch(ColumnUtils.COPY_INTO_ERROR_COLUMN_NAME::equals)) {
      fields.add(Field.nullablePrimitive(ColumnUtils.COPY_INTO_ERROR_COLUMN_NAME, ArrowType.PrimitiveType.Utf8.INSTANCE));
    }
  ...
  private Prel getTableFunctionOnPartitionColumns(WriterOptions options,
                                                  Prel input,
                                                  Prel prel,
    ...
    IcebergTableProps tableProps = options.getTableFormatOptions().getIcebergSpecificOptions()
      .getIcebergTableProps();
    BatchSchema newBatchSchema = getNewBatchSchema(tableProps.getPersistedFullSchema(), partitionSpec, partitionColumns, input.getRowType());
  ...
  private Prel getHashToRandomExchangePrel(Prel input, List<String> partitionColumns) {
    ...
    boolean supportAdaptiveHash = PrelUtil.getPlannerSettings(input.getCluster()).getOptions().getOption(ADAPTIVE_HASH);
    if (supportAdaptiveHash) {
      return new AdaptiveHashExchangePrel(input.getCluster(), relTraitSet,
