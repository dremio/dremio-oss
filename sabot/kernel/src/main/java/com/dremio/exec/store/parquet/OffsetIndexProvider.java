...
/** OffsetIndexProvider: Given a columnChunkMetadata returns its OffsetIndex. */
public class OffsetIndexProvider implements Closeable {
   ...
   * @param columns offsetIndexMap is populated from (seekableInputStream+columns).
   *     <p>Async readers prepopulate the Map that corresponds to Long and OffsetIndex, they
   *     instantiate directly with Map.
   * @throws IOException
   */
  public OffsetIndexProvider(
    ...
    Preconditions.checkState(
        bulkInputStream != null, "Seekabale InputStream Null in OffsetIndexProvider");
    ...
    if (startOffsetIndex != -1) {
      NettyArrowBuf byteBuffer =
          NettyArrowBuf.unwrapBuffer(allocator.buffer(endOffsetIndex - startOffsetIndex));
      try {
        ...
        for (ColumnChunkMetaData column : columns) {
          ...
          NettyArrowBuf is =
              byteBuffer.slice(
                  (int) (offsetRef.getOffset() - startOffsetIndex), offsetRef.getLength());
          try (ByteBufInputStream offsetInputStream = new ByteBufInputStream(is)) {
            OffsetIndex offsetIndex =
                ParquetMetadataConverter.fromParquetOffsetIndex(
                    Util.readOffsetIndex(offsetInputStream));
