...
public class OperatorMetricRegistry {
  ...
  private static final String[][] OPERATOR_METRICS_NAMES =
      new String[CoreOperatorType.values().length][];
  ...
  static {
    ...
    register(
        builder,
        CoreOperatorType.HASH_PARTITION_SENDER_VALUE,
        PartitionSenderOperator.Metric.class);
    register(
        builder, CoreOperatorType.MERGING_RECEIVER_VALUE, MergingReceiverOperator.Metric.class);
    register(
        builder, CoreOperatorType.UNORDERED_RECEIVER_VALUE, UnorderedReceiverOperator.Metric.class);
    ...
    register(
        builder,
        CoreOperatorType.SMALL_FILE_COMBINATION_WRITER_VALUE,
        ParquetRecordWriter.Metric.class);
    ...
    register(
        builder,
        CoreOperatorType.TABLE_FUNCTION_VALUE,
        Arrays.asList(ScanOperator.Metric.class, TableFunctionOperator.Metric.class));
    ...
    register(
        builder, CoreOperatorType.WRITER_COMMITTER_VALUE, WriterCommitterOperator.Metric.class);
    register(builder, CoreOperatorType.JSON_SUB_SCAN_VALUE, ScanOperator.Metric.class);
    register(
        builder,
        CoreOperatorType.BRIDGE_FILE_WRITER_SENDER_VALUE,
        BridgeFileWriterSenderOperator.Metric.class);
    register(
        builder,
        CoreOperatorType.BRIDGE_FILE_READER_RECEIVER_VALUE,
        BridgeFileReaderReceiverOperator.Metric.class);
    register(
        builder,
        CoreOperatorType.ICEBERG_METADATA_FUNCTIONS_READER_VALUE,
        ScanOperator.Metric.class);
  ...
  private static void register(
      CoreOperatorTypeMetricsMap.Builder builder,
      final Integer operatorType,
      final Class<? extends MetricDef> metricDef) {
  ...
  private static void register(
      CoreOperatorTypeMetricsMap.Builder builder,
      final Integer operatorType,
      final List<Class<? extends MetricDef>> metricsDef) {
  ...
  private static void populateMetricsInMap(
      CoreOperatorTypeMetricsMap.Builder builder,
      final Integer operatorType,
      List<MetricDef> enumConstants) {
    ...
    for (int i = 0; i < enumConstants.size(); i++) {
      metricsDefBuilder.addMetricDef(
          UserBitShared.MetricDef.newBuilder()
              ...
              .setDisplayCode(enumConstants.get(i).getDisplayCode())
              .build());
  ...
  private static CoreOperatorTypeMetricsMap.Builder getMapBuilder() {
    ...
    for (int i = 0; i < CoreOperatorType.values().length; i++) {
      // Initalizing with metricsdef object with empty list => This can be modified later at the
  ...
  private static Optional<MetricsDef> getMetricsDef(
      CoreOperatorTypeMetricsMap coreOperatorTypeMetricsMap, int operatorType) {
  ...
  public static String[] getMetricNames(
    ...
    return metricsDef
        .map(
            (metricDef) ->
                metricDef.getMetricDefList().stream()
                    .map(UserBitShared.MetricDef::getName)
                    .toArray(String[]::new))
  ...
  public static String[] getMetricNames(int operatorType) {
    return Optional.ofNullable(OPERATOR_METRICS_NAMES[operatorType])
        .orElse(ArrayUtils.EMPTY_STRING_ARRAY);
  }
   ...
   */
  public static Integer[] getMetricIds(
      CoreOperatorTypeMetricsMap coreOperatorTypeMetricsMap, int operatorType) {
    Optional<MetricsDef> metricsDef = getMetricsDef(coreOperatorTypeMetricsMap, operatorType);
    return metricsDef
        .map(
            (metricDef) ->
                metricDef.getMetricDefList().stream()
                    .map(UserBitShared.MetricDef::getId)
                    .toArray(Integer[]::new))
        .orElse(ArrayUtils.EMPTY_INTEGER_OBJECT_ARRAY);
  }
   ...
   */
  public static Optional<UserBitShared.MetricDef> getMetricById(
      CoreOperatorTypeMetricsMap coreOperatorTypeMetricsMap, int operatorType, int metricId) {
    Optional<MetricsDef> metricsDef = getMetricsDef(coreOperatorTypeMetricsMap, operatorType);
