...
public class JoinBooleanRewriteRule extends RelRule<JoinBooleanRewriteRule.Config> {
  ...
  /**
   * This rule rewrites boolean based equi-join condition to integer based condition with case statement added below.
   *
   * @param config
   */
  ...
  public void onMatch(RelOptRuleCall call) {
    ...
    RexNode remaining = RelOptUtil.splitJoinCondition(join.getLeft(), join.getRight(), join.getCondition(), leftKeys, rightKeys, filterNulls);
    ...
    final List<RexNode> leftProjects = new ArrayList<>(MoreRelOptUtil.identityProjects(join.getLeft().getRowType()));
    final List<RexNode> rightProjects = new ArrayList<>(MoreRelOptUtil.identityProjects(join.getRight().getRowType()));
    final RelDataType intType = join.getCluster().getTypeFactory().createTypeWithNullability(
      join.getCluster().getTypeFactory().createSqlType(SqlTypeName.INTEGER), true);
    booleanConditions.forEach(t -> {
    ...
    nonBooleanConditions.forEach(t -> updatedEquiConditions.add(Triple.of(t.getLeft(), t.getMiddle(), t.getRight())));
    ...
    final RexNode equiJoinCondition = MoreRelOptUtil.createEquiJoinCondition(newLeft, newRight, updatedEquiConditions, rb);
    final RexNode shiftedRemaining = RexUtil.shift(remaining,
      join.getLeft().getRowType().getFieldCount(), booleanConditions.size());
    ...
    List<RexNode> topProjects =
      MoreRelOptUtil.identityProjects(join.getRowType())
        .stream()
        .map(p -> RexUtil.shift(p, join.getLeft().getRowType().getFieldCount(), booleanConditions.size()))
  ...
  private static RexNode makeCase(RelDataType type, int field, RexBuilder rb, RelDataType outType) {
    return rb.makeCall(SqlStdOperatorTable.CASE,
      rb.makeCall(
  ...
  public interface Config extends RelRule.Config {
    Config DEFAULT = EMPTY
      ...
      .withOperandSupplier(os1 ->
        os1.operand(JoinRel.class).anyInputs())
      .as(Config.class);

    @Override default JoinBooleanRewriteRule toRule() {
