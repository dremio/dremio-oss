...
/**
 * {@link OptionManager} that holds options within {@link com.dremio.sabot.rpc.user.UserSession}
 * context. Options set at the session level only apply to queries that you run during the current
 * Dremio connection. Session level settings override system level settings.
 *
 * <p>NOTE that currently, the effects of deleting a short lived option (see {@link
 * OptionValidator#isShortLived}) are undefined. For example, we inject an exception (passed through
 * an option), then try to delete the option, depending on where the exception was injected, the
 * reset query could either succeed or the exception could actually be thrown in the reset query
 * itself.
 */
public class SessionOptionManagerImpl extends InMemoryOptionManager
    implements SessionOptionManager {
  private static final org.slf4j.Logger LOGGER =
      org.slf4j.LoggerFactory.getLogger(SessionOptionManagerImpl.class);
  ...
  private void logUsage(OptionValue option, boolean isShortLived) {
    String msg =
        String.format(
            "Session option %s set to %s.", option.getName(), option.getValue().toString());
    if (isShortLived) {
      msg += " This option is shortLived";
    }
    LOGGER.debug(msg);
  }
}
