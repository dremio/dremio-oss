...
public abstract class AbstractExchange extends AbstractSingle implements Exchange {
  ...
  protected long computeMemReserve() {
    final boolean enableAccurateMemoryCalculation =
        optionManager.getOption(PlannerSettings.ENABLE_ACCURATE_MEMORY_ESTIMATION);
    if (enableAccurateMemoryCalculation
        && !isMemReserveUpdated
        && receiverLocations != null
        && receiverLocations.size() >= 1) {
  ...
  /**
   * Get a default endpoint affinity map where affinity of a SabotNode is proportional to the number
   * of its occurrances in given endpoint list.
   *
   * @param fragmentEndpoints SabotNode endpoint assignments of fragments.
   * @return List of EndpointAffinity objects for each SabotNode endpoint given
   *     <i>fragmentEndpoints</i>.
   */
  protected static List<EndpointAffinity> getDefaultAffinityMap(
      Collection<NodeEndpoint> fragmentEndpoints) {
  ...
  protected void setupSenders(List<NodeEndpoint> senderLocations)
  ...
  @Override
  public final void setupSenders(int majorFragmentId, List<NodeEndpoint> senderLocations)
      throws PhysicalOperatorSetupException {
  ...
  @Override
  public final void setupReceivers(int majorFragmentId, List<NodeEndpoint> receiverLocations)
      throws PhysicalOperatorSetupException {
