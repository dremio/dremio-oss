...
/**
 * The simple parallelizer determines the level of parallelization of a plan based on the cost of
 * the underlying operations. It doesn't take into account system load or other factors. Based on
 * the cost of the query, the parallelization for each major fragment will be determined. Once the
 * amount of parallelization is done, assignment is done based on round robin assignment ordered by
 * operator affinity (locality) to available execution SabotNodes.
 */
public class SimpleParallelizer implements ParallelizationParameters {
  private static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(SimpleParallelizer.class);
  ...
  private ExecutorSelectionService
      executorSelectionService; // NB: re-assigned in unit tests, hence not final
  ...
  public SimpleParallelizer(
    ...
    this.assignmentCreatorBalanceFactor =
        optionManager.getOption(ExecConstants.ASSIGNMENT_CREATOR_BALANCE_FACTOR);
    this.observer = observer;
    this.fragmentCodec =
        FragmentCodec.valueOf(optionManager.getOption(ExecConstants.FRAGMENT_CODEC).toUpperCase());
    this.executorSelectionService = executorSelectionService;
    this.targetNumFragsPerNode =
        Ints.saturatedCast(
            optionManager.getOption(ExecutorSelectionService.TARGET_NUM_FRAGS_PER_NODE));
    this.shouldIgnoreLeafAffinity =
        optionManager.getOption(ExecConstants.SHOULD_IGNORE_LEAF_AFFINITY);
    handleWithAllExecutors =
        executorSelectionService.getAllActiveExecutors(
  ...
  private void computeMaxWidthPerNode(GroupResourceInformation groupResourceInformation) {
    ...
    final long configuredMaxWidthPerNode =
        groupResourceInformation.getAverageExecutorCores(optionManager);
    if (configuredMaxWidthPerNode == 0) {
      ExecutorSelectionUtils.throwEngineOffline(resourceSchedulingDecisionInfo.getQueueTag());
    }
    final double maxWidthFactor =
        queryContext.getWorkStatsProvider().get().getMaxWidthFactor(groupResourceInformation);
    ...
    if (logger.isDebugEnabled() && maxWidthFactor < 1) {
      ...
      logger.debug(
          "Cluster load {} exceeded cutoff, max_width_factor = {}. current max_width = {}",
          clusterLoad,
          maxWidthFactor,
          maxWidthPerNode);
    }
  ...
  /**
   * Generate a set of assigned fragments based on the provided fragment tree. Do not allow
   * parallelization stages to go beyond the global max width.
   *
   ...
   * @return The list of generated PlanFragment protobuf objects to be assigned out to the
   *     individual nodes.
  ...
  public List<PlanFragmentFull> getFragments(
    ...
    try (final ExecutionPlanningResources resources =
        getExecutionPlanningResources(
            queryContext,
            observer,
  ...
  /**
   * Generate set of assigned fragments based on predefined PlanningSet versus doing parallelization
   * in place QueryContext has to be not null from construction of Parallelizer
   *
   * @param options
   * @param planningSet
   * @param reader
   ...
   * @return
   */
  public static ExecutionPlanningResources getExecutionPlanningResources(
      QueryContext context,
      MaestroObserver observer,
      ExecutorSelectionService executorSelectionService,
      ResourceSchedulingDecisionInfo resourceSchedulingDecisionInfo,
      ...
      throws ExecutionSetupException {
    SimpleParallelizer parallelizer =
        new SimpleParallelizer(
            context,
            observer,
            executorSelectionService,
            resourceSchedulingDecisionInfo,
            context.getGroupResourceInformation());
    PlanningSet planningSet = new PlanningSet();
    parallelizer.initFragmentWrappers(rootFragment, planningSet);
    final Set<Wrapper> leafFragments =
        constructFragmentDependencyGraph(planningSet, priorityAssigner);
    // NB: for queries with hard affinity, we need to use all endpoints, so the parallelizer, below,
    // is given an
    ...
    for (Wrapper wrapper : leafFragments) {
      idealNumFragments +=
          parallelizer.computePhaseStats(
              wrapper, planningSet, hasHardAffinity, context.getOptions());
    }
    int idealNumNodes =
        IntMath.divide(idealNumFragments, parallelizer.targetNumFragsPerNode, RoundingMode.CEILING);
    final Stopwatch stopWatch = Stopwatch.createStarted();
    ExecutorSelectionContext executorContext =
        new ExecutorSelectionContext(resourceSchedulingDecisionInfo);
    ExecutorSelectionHandle executorSelectionHandle =
        hasHardAffinity.value
            ? parallelizer.handleWithAllExecutors
            : executorSelectionService.getExecutors(idealNumNodes, executorContext);

    GroupResourceInformation groupResourceInformation =
        new SelectedExecutorsResourceInformation(executorSelectionHandle.getExecutors());
    parallelizer.computeMaxWidthPerNode(groupResourceInformation);

    final ExecutionPlanningResources executionPlanningResources =
        new ExecutionPlanningResources(
            planningSet, executorSelectionHandle, groupResourceInformation);
    ...
    observer.executorsSelected(
        ...
        executorSelectionHandle.getPlanDetails()
            + " selectedEndpoints: "
            + EndpointHelper.getMinimalString(selectedEndpoints)
            + " hardAffinity: "
            + hasHardAffinity.value);
  ...
  private static void setFragmentDependency(
      Wrapper sender, Wrapper receiver, ParallelizationDependency dependency) {
    switch (dependency) {
   ...
   */
  private static Set<Wrapper> constructFragmentDependencyGraph(
      PlanningSet planningSet, AssignFragmentPriorityVisitor priorityAssigner) {
    ...
    for (Wrapper currentFragmentWrapper : planningSet) {
      ...
      for (Fragment sibling : currentFragmentWrapper.getNode().getSiblingBridgeFragments()) {
        setFragmentDependency(
            currentFragmentWrapper,
            planningSet.get(sibling),
            ParallelizationDependency.RECEIVER_MUST_MATCH_SENDER);
      }
    }
  ...
  /**
   * Compute the parallelization stats for a given phase. Dependent phases are processed first
   * before processing the given phase. Returns the maximum number of (minor) fragments that could
   * be used by this phase and its dependents
   */
  private int computePhaseStats(
      Wrapper fragmentWrapper,
      PlanningSet planningSet,
      Pointer<Boolean> hasHardAffinity,
    ...
    fragmentWrapper
        .getNode()
        .getRoot()
        .accept(new StatsCollector(planningSet, executionMap, options), fragmentWrapper);
    DistributionAffinity fragmentAffinity = fragmentWrapper.getStats().getDistributionAffinity();
    width +=
        fragmentAffinity.getFragmentParallelizer().getIdealFragmentWidth(fragmentWrapper, this);
   ...
   * parallelizing the given phase. Assumes the stats have already been computed for all
   */
  private void parallelizePhase(
      Wrapper fragmentWrapper, PlanningSet planningSet, Collection<NodeEndpoint> activeEndpoints)
  ...
  /**
   * To facilitate generating workunits with the assumption that QueryContext is NOT null it's not
   * always going to be true, since e.g. QueryContextInfo may change between ctor and this method
   *
   * @param options
   * @param reader
   * @param rootNode
  ...
  protected List<PlanFragmentFull> generateWorkUnit(
    ...
    for (Wrapper wrapper : planningSet) {
      ...
      CoordExecRPC.QueryContextInformation queryContextInformation =
          CoordExecRPC.QueryContextInformation.newBuilder(queryContextInfo)
              .setQueryMaxAllocation(wrapper.getMemoryAllocationPerNode())
              .build();
      ...
      for (int minorFragmentId = 0; minorFragmentId < wrapper.getWidth(); minorFragmentId++) {
        assignMap.put(
            builder.addNodeEndpoint(wrapper.getAssignedEndpoint(minorFragmentId)), minorFragmentId);
      }
      ...
      for (int minorFragmentId = 0; minorFragmentId < wrapper.getWidth(); minorFragmentId++) {
        ...
        PhysicalOperator op =
            physicalOperatorRoot.accept(new Materializer(wrapper.getSplitSets(), builder), iNode);
        ...
        if (!majorAdded) {
          ...
          // If any of the operators report ext communicable fragments, fill in the assignment and
          // node details.
          final Set<Integer> extCommunicableMajorFragments =
              physicalOperatorRoot.accept(new ExtCommunicableFragmentCollector(), wrapper);
          majorFragmentAssignmentsCache.populateIfAbsent(
              planningSet, builder, extCommunicableMajorFragments);
          final List<MajorFragmentAssignment> extFragmentAssignments =
              majorFragmentAssignmentsCache.getAssignments(
                  planningSet, builder, extCommunicableMajorFragments);
          major =
        ...
        List<MinorAttr> attrList =
            MinorDataCollector.collect(
                handle, endpoint, root, new MinorDataSerDe(reader, fragmentCodec), indexBuilder);

        // Build minor specific info and attributes.
        PlanFragmentMinor minor =
  ...
  protected static class CountRequiredFragments
      extends AbstractPhysicalVisitor<Void, List<Collector>, RuntimeException> {
    ...
    @Override
    public Void visitReceiver(Receiver receiver, List<Collector> collectors)
  ...
  private class MajorFragmentAssignmentCache {
    ...
    private List<MajorFragmentAssignment> getAssignments(
      ...
      return requiredFragments.stream()
          .map(majorFragmentAssignments::get)
          .collect(Collectors.toList());
    }

    private void populateIfAbsent(
        final PlanningSet planningSet,
        final EndpointsIndex.Builder builder,
        final Set<Integer> requiredFragments) {
      ...
      for (Wrapper wrapper : planningSet) {
        ...
        if (!requiredFragments.contains(majorFragment)
            || majorFragmentAssignments.containsKey(majorFragment)) {
          continue;
        }
        final ArrayListMultimap<Integer, Integer> assignMap = ArrayListMultimap.create();
        for (int minorFragmentId = 0; minorFragmentId < wrapper.getWidth(); minorFragmentId++) {
          assignMap.put(
              builder.addNodeEndpoint(wrapper.getAssignedEndpoint(minorFragmentId)),
              minorFragmentId);
        }

        // create getAssignment lists.
        final List<FragmentAssignment> assignments =
            assignMap.keySet().stream()
                .map(
                    ep ->
                        FragmentAssignment.newBuilder()
                            .setAssignmentIndex(ep)
        ...
        majorFragmentAssignments.putIfAbsent(
            majorFragment,
            MajorFragmentAssignment.newBuilder()
                .setMajorFragmentId(majorFragment)
                .addAllAllAssignment(assignments)
                .build());
      }
    }
