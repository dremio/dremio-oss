...
/**
 * A table function that raises the data found in the partition info to the required outputPartitionSpec level
 * If the partition info does not contain partition on the column specified, or it cannot be raised to
 * outputPartitionSpec level, UNSUPPORTED_PARTITION_EVOLUTION exception is thrown
 * Example1: current partition info is day(col1), target is month(col1) we can raise successfully.
 * Example2: current partition info is year(col1), target is month(col1),
 * we cannot calculate month(col1) so we fail with UNSUPPORTED_PARTITION_EVOLUTION
 * Example3: current partition info is day(col1), target is month(col2),
 * we cannot calculate month(col2) so we fail with UNSUPPORTED_PARTITION_EVOLUTION
 */
public class IcebergIncrementalRefreshJoinKeyTableFunction extends AbstractTableFunction {
  private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(IcebergIncrementalRefreshJoinKeyTableFunction.class);
  ...
  public IcebergIncrementalRefreshJoinKeyTableFunction(
    ...
    functionContext = (IncrementalRefreshJoinKeyTableFunctionContext) functionConfig.getFunctionContext();
    outputPartitionSpec = IcebergSerDe.deserializePartitionSpec(deserializedJsonAsSchema(functionContext.getIcebergSchema()),
  ...
  public VectorAccessible setup(final VectorAccessible accessible) throws Exception {
    ...
    final SupportsIcebergMutablePlugin icebergMutablePlugin =
      fragmentExecutionContext.getStoragePlugin(functionConfig.getFunctionContext().getPluginId());
    final FileIO io = createFileIOForIcebergMetadata(icebergMutablePlugin,
      context,
      functionContext.getPluginId().getName(),
      props,
      getDataset(functionConfig),
      functionContext.getMetadataLocation());
    final TableMetadata tableMetadata = loadTableMetadata(io, context, functionContext.getMetadataLocation());
    ...
    inputPartitionInfoVec = (VarBinaryVector) getVectorFromSchemaPath(incoming, SystemSchemas.PARTITION_INFO);
    outputIncrementalRefreshJoinKey = (VarCharVector) getVectorFromSchemaPath(outgoing, SystemSchemas.INCREMENTAL_REFRESH_JOIN_KEY);
  ...
  public int processRow(final int startOutIndex, final int maxRecords) throws Exception {
    ...
    for(final PartitionField targetPartitionField: outputPartitionSpec.fields()) {
      final Object transformedValue = verifyPartitionSupportedAndTransform(targetPartitionField, outputPartitionSpec, sourcePartitionInfo, sourcePartitionSpec);
      if(isFirst) {
        isFirst = false;
      } else {
  ...
  /**
   * Converts the passed in transformed value into a string suitable for incremental refresh join key comparison
   * In addition does special handling for null
   */
  private String getTransformedValueAsString(final Object transformedValue) {
    ...
    if(transformedValue instanceof ByteString){
      ...
      //we don't want to use a particular encoding such as UTF8, as it might throw a UnsupportedEncodingException
      //instead we Base64 encode the ByteString and use that for join key comparison
      return DREMIO_BYTE_STRING + Base64.getEncoder().encodeToString(((ByteString) transformedValue).toByteArray());
    }
  ...
  /**
   * Given a sourcePartitionInfo apply the transformation stored in targetPartitionField
   * If not possible throw UNSUPPORTED_PARTITION_EVOLUTION exception
   ...
   */
  protected static Object verifyPartitionSupportedAndTransform(final PartitionField targetPartitionField,
                                                               final PartitionSpec targetPartitionSpec,
                                                               final NormalizedPartitionInfo sourcePartitionInfo,
                                                               final PartitionSpec sourcePartitionSpec) {
    final Optional<IcebergTransformType> targetTransformation = safeGetTransformType(targetPartitionField.transform().toString());
    final String targetName = getColumnName(targetPartitionField, targetPartitionSpec.schema());
    for(final PartitionProtobuf.PartitionValue sourcePartitionValue: sourcePartitionInfo.getIcebergValuesList()){
      try {
        if (sourcePartitionValue.getColumn().equals(targetName)) {
          final Optional<PartitionField> sourcePartitionField = sourcePartitionSpec.fields().stream()
            ...
            .filter(f -> safeGetTransformType(f.transform().toString()).isPresent()
                        && getColumnName(f, sourcePartitionSpec.schema()).equalsIgnoreCase(targetName))
            .findAny();
          if(!sourcePartitionField.isPresent()){
            continue;
          }
          final Optional<IcebergTransformType> sourceTransformation = safeGetTransformType(sourcePartitionField.get().transform().toString());
          if (targetTransformation.isPresent() && sourceTransformation.isPresent() &&
            isTransformTypeSupportedBy(targetTransformation.get(),
            ...
            switch (targetTransformation.get()){
              ...
              case TRUNCATE:
                return applyTransformation(targetPartitionField, getValue(sourcePartitionValue));
              case HOUR:
              case DAY:
              case MONTH:
              case YEAR:
                //it is one of the supported datetime transforms
                final Object reversedTransformValue = reverseTransformation(sourceTransformation.get(), getValue(sourcePartitionValue));
                final Transform transform = getUnboundIcebergTransformFromType(targetTransformation.get());
                //we use Timestamp here, because reverseTransformation always converts to timestamp
                return transform.bind(Types.TimestampType.withoutZone()).apply(reversedTransformValue);
              default:
                //do nothing
            }
          }
        ...
        // We will fail with Year if we encounter it first, but eventually we will find a match with Day
      }
    }
    //If we are here, we did not find a partition value that can be transformed
    //As we already verified the current partition spec is correct and compatible with targetPartitionField,
    ...
    final String message = String.format("%s Tried to apply transformation:%s. Found records with partition spec:%s",
  ...
  private static Object applyTransformation(final PartitionField targetPartitionField,final Object reversedTransformValue) {
    if(reversedTransformValue == null){
      //null transforms to null
      return null;
    }
    final Transform transform= targetPartitionField.transform();
    return transform.apply(reversedTransformValue);
  }
   ...
   */
  private static Transform getUnboundIcebergTransformFromType(final IcebergTransformType transformType){
  ...
  /**
   * Attempt to revert a transformation.
   * Not all transforms are reversible
   * The revert will be approximate beginning of interval, and not the exact input value.
   * If not possible throw a IncrementalRefreshTransformNotPossibleException
   ...
   */
  private static Object reverseTransformation(final IcebergTransformType sourceTransformation, final Object value) throws IncrementalRefreshTransformNotPossibleException {
   ...
   * @throws IncrementalRefreshTransformNotPossibleException if unsuccessful
   */
  protected static Object reverseMonthTransform(final Object input) throws IncrementalRefreshTransformNotPossibleException {
    if(input == null){
    ...
    if(input instanceof Long || input instanceof Integer){
      ...
      calendar.set(1970 + (((Number)input).intValue()/ 12),  (((Number)input).intValue() % 12), 1, 0, 0, 0);
   ...
   * @throws IncrementalRefreshTransformNotPossibleException if unsuccessful
   */
  private static Object reverseYearTransform(final Object input) throws IncrementalRefreshTransformNotPossibleException {
    if(input == null){
      //null reverses to null
      return null;
    }
   ...
   * @throws IncrementalRefreshTransformNotPossibleException if unsuccessful
   */
  private static Object reverseTimeTransform(final Object input,final TimeUnit timeUnit) throws IncrementalRefreshTransformNotPossibleException {
    if(input == null){
      //null reverses to null
      return null;
    }
