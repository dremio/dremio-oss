...
/**
 * Rule to convert {@link SamplePrel} to Dremio physical rel which is a {@link LimitPrel} in current
 * implementation of sampling. One difference is traits are not singleton.
 */
  ...
  public void onMatch(RelOptRuleCall call) {
    ...
    final RexNode limit =
        rexBuilder.makeBigintLiteral(
            BigDecimal.valueOf(SampleRelBase.getSampleSizeAndSetMinSampleSize(plannerSettings, 1)));
    final LimitPrel sampleAsLimit =
        new LimitPrel(
            sample.getCluster(),
            sample.getTraitSet().plus(Prel.PHYSICAL),
            convertedInput,
            offset,
            limit);
