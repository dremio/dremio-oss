...
/** ColumnIndexProvider: Given a columnChunkMetadata returns its ColumnIndex. */
public class ColumnIndexProvider implements Closeable {
   ...
   * @param columns
   *     <p>From the bulkInputStream and columns, read is done and Map is populated.
   *     <p>Async readers already have the Map constructed, the class is instantiated with the Map
   *     directly.
   * @throws IOException
   */
  public ColumnIndexProvider(
      BulkInputStream bulkInputStream,
    ...
    Preconditions.checkState(
        bulkInputStream != null, "InputParamaters to ColumnIndexProvider bulkInputStream null");
    Preconditions.checkState(
        allocator != null, "InputParamaters to ColumnIndexProvider allocator null");
    ...
    if (startColumnIndex != -1) {
      NettyArrowBuf byteBuffer =
          NettyArrowBuf.unwrapBuffer(allocator.buffer(endColumnIndex - startColumnIndex));
      try {
        ...
        for (ColumnChunkMetaData column : columns) {
          ...
          NettyArrowBuf is =
              byteBuffer.slice(
                  (int) (columnRef.getOffset() - startColumnIndex), columnRef.getLength());
          try (ByteBufInputStream offsetInputStream = new ByteBufInputStream(is)) {
            ColumnIndex offsetIndex =
                ParquetMetadataConverter.fromParquetColumnIndex(
                    column.getPrimitiveType(), Util.readColumnIndex(offsetInputStream));
