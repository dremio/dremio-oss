 ...
 */
public class SqlAlterTableSetColumnMasking extends SqlAlterTable
  ...
  public static final SqlSpecialOperator OPERATOR =
      new SqlSpecialOperator("SET_COLUMN_MASKING", SqlKind.OTHER) {
        @Override
        public SqlCall createCall(
            SqlLiteral functionQualifier, SqlParserPos pos, SqlNode... operands) {
          Preconditions.checkArgument(
              operands.length == 3,
              "SqlAlterTableSetColumnMasking.createCall() has to get 3 operands!");
  ...
  public SqlAlterTableSetColumnMasking(
      SqlParserPos pos, SqlIdentifier tblName, SqlIdentifier columnName, SqlPolicy columnMasking) {
    super(pos, tblName);
  ...
  public List<SqlNode> getOperandList() {
    return ImmutableList.of(tblName, columnName, columnMasking);
  }
  ...
  public SimpleDirectHandler toDirectHandler(QueryContext context) {
    ...
    try {
      final Class<?> cl =
          Class.forName(
              "com.dremio.exec.planner.sql.handlers.EnterpriseAlterTableSetColumnMasking");
