...
public class DeltaLakeScanPrel extends ScanRelBase
    implements Prel, PrelFinalizable, RowCountEstimator {
  ...
  public Prel finalizeRel() {
    ...
    // Parquet scan phase
    TableFunctionConfig parquetScanTableFunctionConfig =
    ...
    return new TableFunctionPrel(
        ...
        tableMetadata,
        parquetScanTableFunctionConfig,
        getRowType(),
        rm -> (double) tableMetadata.getApproximateRecordCount());
  }
  ...
  public RelNode expandDeltaLakeScan() {
    ...
    if (checkBroadcastConditions(joinRelType, addPathScan, removePathScan)) {
      removePathScan =
          new BroadcastExchangePrel(
              removePathScan.getCluster(), removePathScan.getTraitSet(), removePathScan);
    } else {
      DistributionTrait.DistributionField distributionField =
          new DistributionTrait.DistributionField(0);
      DistributionTrait distributionTrait =
      ...
      addPathScan =
          new HashToRandomExchangePrel(
              addPathScan.getCluster(), relTraitSet, addPathScan, distributionTrait.getFields());

      removePathScan =
          new HashToRandomExchangePrel(
              removePathScan.getCluster(),
              relTraitSet,
              removePathScan,
              distributionTrait.getFields());
    }
    ...
    Pair<Integer, RelDataTypeField> removePathField =
        findFieldWithIndex(removePathScan, SCHEMA_REMOVE_PATH);
    ...
    RexNode joinCondition =
        rexBuilder.makeCall(
            ...
            rexBuilder.makeInputRef(
                joinType,
                addPathScan.getRowType().getFieldCount()
                    + removePathField.left /* Add the offset */));

    HashJoinPrel hashJoinPrel =
        HashJoinPrel.create(
            addPathScan.getCluster(),
            addPathScan.getTraitSet(),
            addPathScan,
            removePathScan,
            joinCondition,
            null,
    ...
    RexNode filterNullRemovePath =
        rexBuilder.makeCall(
            SqlStdOperatorTable.IS_NULL,
            rexBuilder.makeInputRef(removePathField.right.getType(), removePathField.left));

    Pair<Integer, RelDataTypeField> addVersion =
        findFieldWithIndex(hashJoinPrel, SCHEMA_ADD_VERSION);
    Pair<Integer, RelDataTypeField> removeVersion =
        findFieldWithIndex(hashJoinPrel, SCHEMA_REMOVE_VERSION);

    RexNode filterRemoveVersionAddVersion =
        rexBuilder.makeCall(
            SqlStdOperatorTable.EQUALS,
    ...
    RexNode orCondition =
        rexBuilder.makeCall(
            SqlStdOperatorTable.OR, filterNullRemovePath, filterRemoveVersionAddVersion);

    FilterPrel removedNonNullOrAddedVersionEqualRemoveVersion =
        FilterPrel.create(
            hashJoinPrel.getCluster(), hashJoinPrel.getTraitSet(), hashJoinPrel, orCondition);
    // Split generation table function

    TableFunctionConfig splitGenTableFunctionConfig =
        TableFunctionUtil.getSplitGenFunctionConfig(tableMetadata, null);
    return new TableFunctionPrel(
        getCluster(),
        getTraitSet().plus(DistributionTrait.ANY),
        table,
  ...
  private boolean checkBroadcastConditions(JoinRelType joinRelType, RelNode probe, RelNode build) {
    ...
    return JoinPruleBase.checkBroadcastConditions(
        joinRelType, probe, build, probeRowCount, buildRowCount, false);
  }
  ...
  private double getRowCount(RelNode relNode) {
    ...
    throw new RuntimeException(
        String.format(
            "Unable to find DeltaLakeCommitLogScanPrel in:\n%s", RelOptUtil.toString(relNode)));
  }
  ...
  private RelNode createDeltaLakeCommitLogScanWithSideScan(
      RexBuilder rexBuilder, boolean scanForAddedPaths) {
  ...
  private RelNode creteAddSideScan(RelNode deltaLakeCommitLogScanPrel, RexBuilder rexBuilder) {
    ...
    RexNode removeNullAddPathsCond =
        rexBuilder.makeCall(
    ...
    final RexNode partitionCond =
        getPartitionCondition(flattened.getCluster().getRexBuilder(), flattened, pruneCondition);
    if (partitionCond != null) {
      removeNullAddPathsCond =
          rexBuilder.makeCall(SqlStdOperatorTable.AND, removeNullAddPathsCond, partitionCond);
    }

    return FilterPrel.create(
        flattened.getCluster(), flattened.getTraitSet(), flattened, removeNullAddPathsCond);
  }
  ...
  private RelNode createRemoveSideScan(RelNode deltaLakeCommitLogScanPrel, RexBuilder rexBuilder) {
    ...
    Pair<Integer, RelDataTypeField> addDataChange =
        findFieldWithIndex(flattened, SCHMEA_ADD_DATACHANGE);

    // Create Filter Operator

    // remove_path != null
    RexNode removeNullRemovedPathsCond =
        rexBuilder.makeCall(
            SqlStdOperatorTable.IS_NOT_NULL,
            rexBuilder.makeInputRef(removePath.right.getType(), removePath.left));

    // add_dataChange == false
    RexNode addDatachangeFalseCond =
        rexBuilder.makeCall(
    ...
    RexNode orCondition =
        rexBuilder.makeCall(
            SqlStdOperatorTable.OR, removeNullRemovedPathsCond, addDatachangeFalseCond);

    // Filter add_dataChange == false || remove_path != null
    RelNode filterPrel =
        FilterPrel.create(flattened.getCluster(), flattened.getTraitSet(), flattened, orCondition);
    ...
    Pair<Integer, RelDataTypeField> removeVersion =
        findFieldWithIndex(filterPrel, SCHEMA_REMOVE_VERSION);
    ...
    // remove_path != null
    RexNode isNotNullRemovePath =
        rexBuilder.makeCall(SqlStdOperatorTable.IS_NOT_NULL, removePathRef);

    // case(remove_path != null) then remove_path else add_path
    RexNode caseCondition =
        rexBuilder.makeCall(
            SqlStdOperatorTable.CASE, isNotNullRemovePath, removePathRef, addPathRef);
    ...
    ProjectPrel projectPrel =
        ProjectPrel.create(
            getCluster(), filterPrel.getTraitSet(), filterPrel, projectExpressions, newRowType);
    ...
    HashToRandomExchangePrel hashToRandomExchangePrel =
        new HashToRandomExchangePrel(
            getCluster(), relTraitSet, projectPrel, distributionTrait.getFields());
    ...
    // max(remove_version)
    AggregateCall aggByMaxVersion =
        AggregateCall.create(
            SqlStdOperatorTable.MAX,
            false,
            ImmutableList.of(removeVersion.left),
            -1,
            removeVersion.right.getType(),
            SCHEMA_REMOVE_VERSION);
  ...
  private RexNode getPartitionCondition(
      RexBuilder builder, RelNode input, PruneFilterCondition pruneCondition) {
    ...
    return partitionExpression.accept(
        new ExpressionInputRewriter(builder, getRowType(), input, PARTITION_NAME_SUFFIX));
  }
  ...
  private RelNode flattenRowType(RelNode relNode, RexBuilder rexBuilder, boolean scanForAddedPath) {
    ...
    // Remove the version from the schema and project add_version or remove_version depending on the
    // schema
    ...
    RexNode versionProject =
        rexBuilder.makeInputRef(inputVersion.right.getType(), inputVersion.left);
    ...
    return ProjectPrel.create(
        relNode.getCluster(), relNode.getTraitSet(), relNode, flattener.getExps(), newRowType);
  }
