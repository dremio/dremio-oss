...
public class EasyFormatUtils {
  ...
  public static Object getValue(
    ...
    // In case of CSV reader. data value should not have any impact of trim space. so default value
    // is false for CSV reader
    ...
    try {
      if (CompleteType.BIT.getType().equals(type)) {
        ...
        final long dateInMilliseconds =
            getDateTimeValueAsUnixTimestamp(varcharValue, type, extendedFormatOptions);
  ...
  public static RuntimeException handleExceptionDuringCoercion(
      String varcharValue,
      ArrowType type,
      String fieldName,
    ...
    if (CompleteType.BIT.getType().equals(type)) {
      errorMessageBuilder.append(
          String.format("Could not convert \"%s\" to BOOLEAN. Reason: ", varcharValue));
      ...
      errorMessageBuilder.append(
          String.format("Could not convert \"%s\" to BIGINT.", varcharValue));
    } else if (CompleteType.FLOAT.getType().equals(type)) {
      errorMessageBuilder.append(String.format("Could not convert \"%s\" to FLOAT.", varcharValue));
    } else if (CompleteType.DOUBLE.getType().equals(type)) {
      errorMessageBuilder.append(
          String.format("Could not convert \"%s\" to DOUBLE.", varcharValue));
    } else if (ArrowType.ArrowTypeID.Decimal.equals(type.getTypeID())) {
      int precision = ((ArrowType.Decimal) type).getPrecision();
      int scale = ((ArrowType.Decimal) type).getScale();
      errorMessageBuilder.append(
          String.format(
              "Could not convert \"%s\" to DECIMAL(%s,%s).", varcharValue, precision, scale));
      ...
      errorMessageBuilder.append(
          String.format("Could not convert \"%s\" to DATE. Reason: ", varcharValue));
      errorMessageBuilder.append(caughtException.getMessage());
    } else if (CompleteType.TIME.getType().equals(type)) {
      errorMessageBuilder.append(
          String.format("Could not convert \"%s\" to TIME. Reason: ", varcharValue));
      errorMessageBuilder.append(caughtException.getMessage());
    } else if (CompleteType.TIMESTAMP.getType().equals(type)) {
      errorMessageBuilder.append(
          String.format("Could not convert \"%s\" to TIMESTAMP. Reason: ", varcharValue));
      errorMessageBuilder.append(caughtException.getMessage());
    } else {
      throw new RuntimeException("Unsupported data type : " + type);
    }
  ...
  public static boolean isVarcharOptimizationPossible(
      ExtendedFormatOptions extendedFormatOptions, ArrowType type) {
    // We can follow an optimised codepath for writing data when the following conditions are
    // satisfied:
    // 1. The target field type is VARCHAR
    // 2. No string transformations like NULL_IF are needed i.e. areStringTransformationsNeeded is
    // set to false.
    return (!extendedFormatOptions.getAreStringTransformationsNeeded()
        && CompleteType.VARCHAR.getType().equals(type));
  }

  public static String applyStringTransformations(
      String varcharValue, ExtendedFormatOptions extendedFormatOptions, Boolean trimSpace) {
  ...
  public static Function<String, Boolean> TextBooleanFunction =
      s -> {
  ...
  public static Function<String, Integer> JsonBooleanFunction =
      s -> {
  ...
  public static BigDecimal getBigDecimalValue(ArrowType type, String varcharValue) {
    BigDecimal bd =
        new BigDecimal(varcharValue)
            .setScale(((ArrowType.Decimal) type).getScale(), java.math.RoundingMode.HALF_UP);
   ...
   * @param extendedFormatOptions Contains all the format options specified by the user in their
   *     query.
   * @return Unix timestamp equivalent for the given input, represented as a Long type object.
   */
  public static Long getDateTimeValueAsUnixTimestamp(
      String rawInput, ArrowType type, ExtendedFormatOptions extendedFormatOptions) {
    if (rawInput == null) {
      return null;
    }
    ...
    // Check if the format that we receive from the user ends with a timezone abbreviation. If yes,
    // modify:
    // 1. The format string to specify that we will be using the timezone offset instead of the
    // abbreviation.
    // 2. The input where we replace the timezone abbreviation with its corresponding offset.
    String formatString =
        isTimezonePresentInFormat(rawFormat)
            ? replaceTimeZoneWithOffsetFormatString(rawFormat)
            : rawFormat;
    String inputString =
        isTimezonePresentInFormat(rawFormat)
            ? replaceTimeZoneWithOffsetInputString(rawInput)
            : rawInput;

    final DateTimeFormatter formatter =
        DateFunctionsUtils.getSQLFormatterForFormatString(formatString);
  ...
  private static String getDateTimeFormat(
      ArrowType fieldType, ExtendedFormatOptions extendedFormatOptions) {
    String formatString = "";
    if (CompleteType.DATE.getType().equals(fieldType)) {
      formatString =
          extendedFormatOptions.getDateFormat() == null
              ? DEFAULT_DATE_FORMAT
              : extendedFormatOptions.getDateFormat();
    } else if (CompleteType.TIME.getType().equals(fieldType)) {
      formatString =
          extendedFormatOptions.getTimeFormat() == null
              ? DEFAULT_TIME_FORMAT
              : extendedFormatOptions.getTimeFormat();
    } else if (CompleteType.TIMESTAMP.getType().equals(fieldType)) {
      formatString =
          extendedFormatOptions.getTimeStampFormat() == null
              ? DEFAULT_TIMESTAMP_FORMAT
              : extendedFormatOptions.getTimeStampFormat();
    }
  ...
  private static String replaceTimeZoneWithOffsetInputString(String input) {
    ...
    if (!offset.isPresent()) {
      throw new RuntimeException(
          String.format(
              "Invalid timezone abbreviation \"%s\" found in input: \"%s\"", tzAbbr, input));
    }
