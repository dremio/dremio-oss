...
public class GeoHelper {
  ...
  static final Character[] baseHashValues = {
   ...
   *     <p>From Apache's inbucbator hivemall project.
  ...
  public static String encodeGeohash(
      final double lat, final double lon, FunctionErrorContext errCtx) {
    return encodeGeohash(lat, lon, DEFAULT_GEO_HASH_PRECISION, errCtx);
  }

  /**
   * Return the GeoHash of a point, with an optional level of precision. The 'official' site
   * (http://geohash.org/site/tips.html) is sparse and points to WikiPedia for documentaion.
   ...
   */
  public static String encodeGeohash(
      final double lat, final double lon, final long precision, FunctionErrorContext errCtx) {

    if (precision < GEO_HASH_PRECISION_MIN || precision > GEO_HASH_PRECISION_MAX) {
      String errorMessage =
          "precision must be between " + GEO_HASH_PRECISION_MIN + " and " + GEO_HASH_PRECISION_MAX;
      throw errCtx.error().message(errorMessage).build();
    }
    if (lat < -90.0 || lat > 90.0) {
      throw errCtx.error().message("latitude must be between –90° and +90°").build();
    }
    if (lon < -180.0 || lon > 180.0) {
      throw errCtx.error().message("longitude must be between –180° and +180°").build();
    }
  ...
  public static double[] decodeGeohash(String in, FunctionErrorContext errCtx) {
    ...
    for (int id = 0; id < in.length(); id++) {
      ...
      if ((Character.isLetter(currentChar) || Character.isDigit(currentChar))
        ...
        throw errCtx.error().message(INVALID_HASH_MSG).build();
      }
    }
