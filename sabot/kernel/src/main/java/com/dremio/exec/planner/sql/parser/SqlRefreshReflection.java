...
public class SqlRefreshReflection extends SqlCall implements SqlToPlanHandler.Creator {
  ...
  public static final SqlSpecialOperator OPERATOR =
      new SqlSpecialOperator("REFRESH_REFLECTION", SqlKind.OTHER) {
        @Override
        public SqlCall createCall(
            SqlLiteral functionQualifier, SqlParserPos pos, SqlNode... operands) {
          Preconditions.checkArgument(
              operands.length == 2, "SqlRefreshReflection.createCall() has to get 3 operands!");
          return new SqlRefreshReflection(pos, operands[0], operands[1]);
        }
      };
  ...
  public SqlToPlanHandler toPlanHandler() {
    try {
      return (SqlToPlanHandler)
          Class.forName("com.dremio.service.reflection.refresh.RefreshHandler")
