...
/**
 * A wrapping class that can support two key types of jobs: - a basic sql query that return results
 * via a number of push messages - a command response, that returns results via an RPC response.
 */
public class UserRequest {

  private static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(UserRequest.class);

  private static final long MAX_MEMORY_UTIL = 1_000_000;

  private static final QueryPriority NRT =
      QueryPriority.newBuilder()
          .setWorkloadClass(WorkloadClass.NRT)
          .setWorkloadType(WorkloadType.DDL)
          .build();
  private static final QueryPriority GENERAL =
      QueryPriority.newBuilder().setWorkloadClass(WorkloadClass.GENERAL).build();
  ...
  public <X> X unwrap(Class<X> clazz) {
    Preconditions.checkArgument(
        ...
        "Unable to cast request type to class %s",
        clazz.getName());
  ...
  public String getDescription() {
    switch (type) {
      ...
      case CREATE_PREPARED_STATEMENT_ARROW:
        return "[Prepare Statement Arrow Flight] "
            + unwrap(CreatePreparedStatementArrowReq.class).getSqlQuery();
      ...
      case GET_COLUMNS:
        {
          ...
          return String.format(
              "[Get Columns] Catalog Filter: %s, Schema Filter: %s, Table Filter %s, Column Filter %s.",
              req.getCatalogNameFilter(),
              req.getSchemaNameFilter(),
              req.getTableNameFilter(),
              req.getColumnNameFilter());
        }
      ...
      case GET_SCHEMAS:
        {
          ...
          return String.format(
              "[Get Schemas] Catalog Filter: %s, Schema Filter: %s.",
      ...
      case GET_TABLES:
        {
          ...
          return String.format(
              "[Get Tables] Catalog Filter: %s, Schema Filter: %s, Table Filter %s.",
    ...
    if (q.getType() == QueryType.PREPARED_STATEMENT) {
      try {
        ...
        throw UserException.connectionError(ex)
            .message("Prepared statement provided is corrupt.")
            .build(logger);
      }
    } else {
      return q.getPlan();
