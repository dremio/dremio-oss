 ...
 * Expected to be used by RuntimeFiltering when composite keys are in picture. The class will be
 * used to ensure there is a fair representation of all keys within the feed going for the hash.
 */
public class KeyFairSliceCalculator {
  ...
  /**
   * The function first sorts all the entries by key sizes. If total of all key sizes exceeds the
   * cap (maxTotalSize), we calculate fair allocation by dividing max size with number of keys.
   *
   * <p>In case key size is less than the fair allocation, remaining length is equally distributed
   * to keys of larger sizes.
   *
   * @param originalKeySizes Original keysizes, Integer.MAX_VALUE for variable length fields and -1
   *     for bit type keys
   * @param maxTotalSize Max size in which all key slices should fit. Total size can be less than
   *     max size in case total size of all keys is less than maxTotalSize.
   */
  public KeyFairSliceCalculator(Map<String, Integer> originalKeySizes, int maxTotalSize) {
    ...
    checkArgument(
        numValueBytes >= numNonBooleanKeys,
        "Not possible to fit %s keys (%s boolean) in %s bytes",
        numKeys,
        numBooleanKeys,
        maxTotalSize);
    ...
    Collections.sort(
        listKeySizes,
        VALUE_COMPARATOR); // Assuming Collections.sort will maintain original order when values are
    // equal
