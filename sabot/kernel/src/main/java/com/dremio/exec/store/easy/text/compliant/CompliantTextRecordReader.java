...
public class CompliantTextRecordReader extends AbstractRecordReader {
  ...
  private boolean isOnErrorContinueMode = false;
  ...
  public CompliantTextRecordReader(FileSplit split, CompressionCodecFactory codecFactory, FileSystem dfs,
                                   OperatorContext context, TextParsingSettings settings, List<SchemaPath> columns, ExtendedEasyReaderProperties properties, ByteString extendedProperties) {
    ...
    if (properties != null) {
      ...
      this.isOnErrorContinueMode = this.schemaImposedMode && this.copyIntoQueryProperties != null &&
        CopyIntoQueryProperties.OnErrorOption.CONTINUE == copyIntoQueryProperties.getOnErrorOption();
  ...
  public void setup(OutputMutator outputMutator) throws ExecutionSetupException {
    ...
    try {
      try {
        ...
        if (isSkipQuery()) {
          ...
          if (schemaImposedMode) {
            ...
            if (settings.isHeaderExtractionEnabled()) {
              ...
              try {
                output = new SchemaImposedOutput(outputMutator, fieldNames, sizeLimit, extendedFormatOptions,
                  filePathForError, isValidationMode, validatedTableSchema, validationErrorRowWriter, isOnErrorContinueMode);
              } catch (SchemaMismatchException e) {
                if (isOnErrorContinueMode) {
                  SchemaImposedOutput imposedOutput = new SchemaImposedOutput(outputMutator, sizeLimit, extendedFormatOptions, isValidationMode,
                    validatedTableSchema, validationErrorRowWriter);
                  reader = new TextReader(settings, input, imposedOutput, whitespaceBuffer, filePathForError, schemaImposedMode,
                    copyIntoQueryProperties, queryContext, isValidationMode);
                }
                throw UserException.dataReadError().message(e.getMessage()).buildSilently();
              }
            } else {
              output = new SchemaImposedOutput(outputMutator, sizeLimit, extendedFormatOptions, isValidationMode,
                validatedTableSchema, validationErrorRowWriter);
            }
        ...
        reader = new TextReader(settings, input, output, whitespaceBuffer, filePathForError, schemaImposedMode,
          copyIntoQueryProperties, queryContext, isValidationMode);
        reader.start();
      } catch (Throwable e) {
        if (isValidationMode || isOnErrorContinueMode) {
  ...
  private void setupForValidationMode(OutputMutator outputMutator) {
    if (isValidationMode) {
      ...
      this.validationErrorRowWriter = ValidationErrorRowWriter.newVectorWriter(
        validationResult, filePathForError, originalJobId, () -> validationErrorCountInBatch++);
    }
  }
  ...
  private String[] readFirstLineForColumnNames() throws ExecutionSetupException, SchemaChangeException, IOException {
    ...
    try (HeaderOutputMutator hOutputMutator = new HeaderOutputMutator();
      ...
      // setup Reader using Input and Output
      this.reader = new TextReader(settings, hInput, hOutput, whitespaceBufferInReader, isValidationMode);
      reader.start();
  ...
  private String[] extractHeader() throws SchemaChangeException, IOException, ExecutionSetupException {
    ...
    if(schemaImposedMode) {
      ...
      if (isOnErrorContinueMode) {
        ...
        fieldNamesWithError[fieldNames.length] = ColumnUtils.COPY_INTO_ERROR_COLUMN_NAME;
  ...
  public int next() {
    ...
    // We return a maximum of configured batch size worth of records. In normal mode these are records from the source
    // file. In copy_errors validation mode these are validation error records, so we may traverse in the source file
    // as long as we saw a batch worth of validation errors (or EOF), thus in this case (source) recordCount can go
    // beyond batch size.
    try {
      loop: while (isValidationMode ? validationErrorCount < numRowsPerBatch : recordCount < numRowsPerBatch) {
        TextReader.RecordReaderStatus status = reader.parseNext();
        switch (status) {
          case SUCCESS:
            recordCount++;
          ...
          case SKIP:
            continue;
          default:
            throw UserException.parseError().message("Unrecognized read state %s while parsing text file", status.name())
              .buildSilently();
        }
      }
      reader.finishBatch();
    ...
    if (isOnErrorContinueMode) {
      if (schemaImposedMode && reader.getOutput() instanceof SchemaImposedOutput) {
        ...
        if (errorColIndex.isPresent()) {
          schemaImposedOutput.startField(errorColIndex.getAsInt());
          String infoJson = ErrorInfo.Util.getJson(new CopyIntoErrorInfo.Builder(queryContext.getQueryId(),
            queryContext.getUserName(), queryContext.getTableNamespace(), copyIntoQueryProperties.getStorageLocation(),
            filePathForError, extendedFormatOptions, FileType.CSV.name(), CopyIntoErrorInfo.CopyIntoFileState.SKIPPED)
            .setRecordsLoadedCount(0).setRecordsRejectedCount(0).setRecordDelimiter(new String(settings.getNewLineDelimiter()))
            .setFieldDelimiter(new String(settings.getDelimiter())).setQuoteChar(new String(settings.getQuote()))
            .setEscapeChar(new String(settings.getQuoteEscape())).build());
