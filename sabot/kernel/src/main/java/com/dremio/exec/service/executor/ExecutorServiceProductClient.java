...
/**
 * Product version of the executor client. Acts as a bridge from gRPC interface to the netty
 * interface.
 */
public class ExecutorServiceProductClient implements ExecutorServiceClient {
  ...
  public ExecutorServiceProductClient(
      CoordToExecTunnelCreator tunnelCreator, CoordinationProtos.NodeEndpoint endpoint) {
  ...
  public void reconcileActiveQueries(
      CoordExecRPC.ActiveQueryList activeQueryList, StreamObserver<Empty> responseObserver) {
  ...
  public void propagatePluginChange(
      CoordExecRPC.SourceWrapper sourceWrapper, StreamObserver<Empty> responseObserver) {
  ...
  public void getNodeStats(
    ...
    RpcOutcomeListener outcomeListener =
        new RpcOutcomeListener<CoordExecRPC.NodeStatResp>() {
          @Override
  ...
  @SuppressWarnings("DremioGRPCStreamObserverOnError")
  private <T extends MessageLite> EndpointListener getEndpointListener(
      T value, StreamObserver<Empty> responseObserver) {
