...
/**
 * A class representing extended properties that can be stored as key-value pairs and serialized
 * to/from JSON format. Used to propagate COPY INTO command details.
 */
public class CopyIntoExtendedProperties {
  ...
  /** Constructs an empty instance of CopyIntoExtendedProperties. */
  public CopyIntoExtendedProperties() {}

  private CopyIntoExtendedProperties(Map<String, String> extendedProperties) {
   ...
   * @return The deserialized extended property object, or null if the property key is not found or
   *     the JSON is empty.
   */
  ...
  public String toString() {
    StringBuilder builder = new StringBuilder("CopyIntoExtendedProperties{\n");
  ...
  /**
   * A utility class providing methods to convert CopyIntoExtendedProperties to/from ByteString
   * format.
   */
  public static final class Util {
     ...
     */
    public static ByteString getByteString(CopyIntoExtendedProperties properties) {
      String json;
      try {
        json =
            MAPPER
                .writerWithDefaultPrettyPrinter()
                .writeValueAsString(properties.extendedProperties);
      } catch (JsonProcessingException e) {
        throw new IllegalStateException(
            String.format("Cannot serialize object %s to json", properties), e);
      }
      return ByteString.copyFromUtf8(json);
    }

    /**
     * Deserialize a ByteString in JSON format to an CopyIntoExtendedProperties object.
     *
     * @param byteString The ByteString to deserialize.
     * @return The deserialized CopyIntoExtendedProperties object.
     * @throws IllegalStateException If there is an error during deserialization.
     */
    public static Optional<CopyIntoExtendedProperties> getProperties(ByteString byteString) {
      ...
      try {
        return Optional.of(
            new CopyIntoExtendedProperties(MAPPER.readValue(byteString.toStringUtf8(), mapType)));
      } catch (JsonProcessingException e) {
        return Optional.empty();
      }
    }
