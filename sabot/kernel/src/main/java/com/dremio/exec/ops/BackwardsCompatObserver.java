...
/**
 * Will patch a record batch before sending it to be backward compatible with the old drill protocol
 */
public class BackwardsCompatObserver extends DelegatingAttemptObserver {
  private static final Logger logger = LoggerFactory.getLogger(BackwardsCompatObserver.class);

  private final BufferAllocator allocator;

  private BackwardsCompatObserver(AttemptObserver observer, BufferAllocator allocator) {
    super(observer);
  ...
  @Override
  public void execDataArrived(final RpcOutcomeListener<Ack> outcomeListener, final QueryWritableBatch batch) {
    ...
    QueryWritableBatch newBatch = new QueryWritableBatch(newHeader, buffers);
    super.execDataArrived(outcomeListener, newBatch);
  }
  ...
  public static ArrowBuf convertBitsToBytes(BufferAllocator allocator,
  ...
  public static ArrowBuf padValues(BufferAllocator allocator, SerializedField.Builder fieldBuilder, ByteBuf oldBuf,
  ...
  /**
   * Wraps the observer with BackwardsCompatObserver if the client on the session is using an older record batch
   * format. So what the original observer is notified is different ("patched" batches) what the backwards
   * compatibility observer is notified.
   *
   * @param session user session
   * @param observer observer that relays results to the user somehow
   * @param allocator allocator
   *
   * @return observer, with backward compatibility if needed
   */
  public static AttemptObserver wrapIfOld(UserSession session, AttemptObserver observer, BufferAllocator allocator) {
    return session.getRecordBatchFormat() == RecordBatchFormat.DRILL_1_0
      ? new BackwardsCompatObserver(observer, allocator)
      : observer;
