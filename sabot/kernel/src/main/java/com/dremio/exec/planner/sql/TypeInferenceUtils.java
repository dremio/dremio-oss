...
public final class TypeInferenceUtils {
  ...
  private static final ImmutableMap<TypeProtos.MinorType, SqlTypeName>
      MINOR_TO_CALCITE_TYPE_MAPPING =
          ImmutableMap.<TypeProtos.MinorType, SqlTypeName>builder()
              ...
              // Calcite types currently not supported by Dremio, nor defined in the Dremio type
              // list:
              //      - CHAR, SYMBOL, MULTISET, DISTINCT, STRUCTURED, ROW, OTHER, CURSOR,
              // COLUMN_LIST
              .build();
  ...
  private static final ImmutableMap<SqlTypeName, TypeProtos.MinorType> CALCITE_TO_MINOR_MAPPING =
      ImmutableMap.<SqlTypeName, TypeProtos.MinorType>builder()
          ...
          // SqlTypeName.CHAR is the type for Literals in Calcite, Dremio treats Literals as VARCHAR
          // also
          .put(SqlTypeName.CHAR, TypeProtos.MinorType.VARCHAR)

          // (2) These 2 types are defined in the Dremio type system but have been turned off for
          // now
          // .put(SqlTypeName.TINYINT, TypeProtos.MinorType.TINYINT)
          // .put(SqlTypeName.SMALLINT, TypeProtos.MinorType.SMALLINT)

          // (3) Calcite types currently not supported by Dremio, nor defined in the Dremio type
          // list:
  ...
  private static class DefaultSqlReturnTypeInference implements SqlReturnTypeInference {
      ...
      for (RelDataType type : opBinding.collectOperandTypes()) {
        if (getMinorTypeFromCalciteType(type) == TypeProtos.MinorType.LATE) {
          ...
          for (AbstractFunctionHolder function : functions) {
            if (!function.isReturnTypeIndependent()
                || function.getReturnType(null).toMinorType() != TypeProtos.MinorType.BIT) {
    ...
    private static RelDataType getReturnType(
        final SqlOperatorBinding opBinding, final AbstractFunctionHolder func) {
      final RelDataTypeFactory factory = opBinding.getTypeFactory();
      ...
      return createCalciteTypeWithNullability(
          factory, sqlTypeName, true, returnType.getPrecision());
    }
  }

  private static AbstractFunctionHolder resolveFunctionHolder(
      final SqlOperatorBinding opBinding, final List<AbstractFunctionHolder> functions) {
   ...
   * @param precision precision, null if the type does not support precision, or to fallback to
   *     default precision
   * @return RelDataType Type of call
   */
  public static RelDataType createCalciteTypeWithNullability(
   ...
   */
  public static FunctionCall convertSqlOperatorBindingToFunctionCall(
    ...
    final String funcName =
        FunctionCallFactory.replaceOpWithFuncName(opBinding.getOperator().getName());
