...
public class ValueListFilter implements AutoCloseable {
  ...
  public void initializeMetaFromBuffer() {
    ...
    // [0:24 - string name|24:28 - int valCount| 28:29 - byte blockSize|29:30 - arrow type|30:31 -
    // precision|31:32 - scale|32:33 - control bytes]
    ...
    // Control bits are organized as - 0:isFixedWidth, 1:containsNull, 2:isBoolField,
    // 3:containsTrue, 4:containsFalse
    // 3 & 4 are applicable only if vector is representing a boolean value set. Set to zero
    // otherwise.
  ...
  public boolean isCompatible(ValueListFilter that) {
    return (this.blockSize == that.blockSize) && (this.fieldType.equals(that.fieldType));
  }
  ...
  protected ArrowCrossBufComparator getComparator() {
    return this.isFixedWidth()
        ? ArrowCrossBufComparatorProvider.get(this.getFieldType())
        : new ValueListVarWidthFilterComparator(blockSize);
  }
  ...
  /**
   * Performs a merge of two value lists and puts in the supplied mergedVal list. Assumes that both
   * incoming value lists are compatible, and contain distinct values in sorted fashion.
   *
   * @param valList1
   * @param valList2
   * @param mergedValList
  ...
  public static void merge(
      final ValueListFilter valList1,
      final ValueListFilter valList2,
      final ValueListFilter mergedValList) {
    checkArgument(
        valList1.isCompatible(valList2),
        "Incompatible value list filters %s %s",
        valList1,
        valList2);
    ...
    if (valList1.isBoolField()) {
      ...
      checkState(
          !mergedValList.containsAllBoolCombinations(),
          "Merged buffer contains all boolean combinations. " + "Unlikely to filter anything.");
    ...
    while (idx1 < valList1.getValueCount() || idx2 < valList2.getValueCount()) {
      ...
      if (idx1 < valList1.getValueCount()
          ...
          (idx2 == valList2.getValueCount()
              || comparator.compare(valList1.valOnlyBuf(), idx1, valList2.valOnlyBuf(), idx2)
                  < 0)) {
        // Insert if not duplicate of last inserted value, skip otherwise
        if ((mergedIdx == 0)
            || comparator.compare(
                    mergedValList.valOnlyBuf(), mergedIdx - 1, valList1.valOnlyBuf(), idx1)
                != 0) {
        ...
        if ((mergedIdx == 0)
            || comparator.compare(
                    mergedValList.valOnlyBuf(), mergedIdx - 1, valList2.valOnlyBuf(), idx2)
                != 0) {
  ...
  private static void copyValue(ValueListFilter src, int srcIdx, ValueListFilter dst, int dstIdx) {
    Copier.copy(
        src.valOnlyBuf().memoryAddress() + (srcIdx * src.getBlockSize()),
  ...
  public static void copyValueList(ValueListFilter src, ValueListFilter dst) {
    Preconditions.checkArgument(dst.valOnlyBuf().capacity() >= src.valOnlyBuf().capacity());
    Copier.copy(
        src.valOnlyBuf().memoryAddress(),
        dst.valOnlyBuf().memoryAddress(),
        src.getBlockSize() * src.getValueCount());
  }
  ...
  public ValueListFilter createCopy(BufferAllocator allocator) {
    try (AutoCloseables.RollbackCloseable rollbackCloseable =
      ...
      PlatformDependent.copyMemory(
          nonPColFilterBuf.memoryAddress(),
          nonPColFilterBufCopy.memoryAddress(),
          nonPColFilterBuf.capacity());
