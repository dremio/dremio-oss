...
public abstract class SargPrunableEvaluator {
  ...
  /**
   * Creates a new instance of specific implementation of SargPrunableEvaluator based on support keys enabled
   ...
   * @return a new instance of specific implementation of SargPrunableEvaluator based on support keys enabled
   */
  public static SargPrunableEvaluator newInstance(final PartitionStatsBasedPruner pruner, final Function<RexNode, List<Integer>> usedIndexes,
                                                  final List<SchemaPath> projectedColumns,final  FindSimpleFilters rexVisitor,
                                                  final ImmutableList<RexCall> rexConditions,final RelDataType rowType,final RelOptCluster cluster) {
    if(pruner.optimizerContext.getPlannerSettings().getOptions().getOption(ENABLE_ICEBERG_NATIVE_PARTITION_PRUNER)){
      return IcebergNativeEvaluator.buildSargPrunableConditions(pruner, rexConditions, rowType, cluster, rexVisitor);
    }
    return ConditionsByColumn.buildSargPrunableConditions(pruner, usedIndexes, projectedColumns, rexVisitor, rexConditions);
  }
   ...
   */
  protected static List<UnprocessedCondition> buildUnprocessedConditions(final ImmutableList<RexCall> rexConditions, final FindSimpleFilters rexVisitor){
    ...
    while (pruner.getStatsEntryIterator().hasNext()) {
      ...
      if (isRecordMatch(partitionData)) {
        ...
        if (statsEntryRecordCount < 0 || qualifiedCount < 0) {
          ...
          RecordPruner.getLogger().info("Record count overflowed while evaluating partition filter. Partition stats file {}. Partition Expression {}", pruner.getFileLocation(), this);
          break;
        }
        ...
        if (statsEntryFileCount < 0 || qualifiedFileCount < 0) {
          ...
          RecordPruner.getLogger().info("File count overflowed while evaluating partition filter. Partition stats file {}. Partition Expression {}", pruner.getFileLocation(), this);
          break;
        }
      }
    }
    RecordPruner.getLogger().debug("Elapsed time to find surviving records: {}ms", pruner.timer.elapsed(TimeUnit.MILLISECONDS));
    pruner.timer.reset();
...
class UnprocessedCondition{
  ...
  UnprocessedCondition(final RexCall condition, final FindSimpleFilters.StateHolder column, final FindSimpleFilters.StateHolder constant) {
