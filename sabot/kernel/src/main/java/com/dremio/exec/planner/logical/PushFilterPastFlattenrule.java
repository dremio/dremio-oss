...
public class PushFilterPastFlattenrule extends RelOptRule {
  ...
  public void onMatch(RelOptRuleCall call) {
    ...
    RexNode belowFilter = FilterExtractor.extractFilter(rexBuilder, filterCondition,
      rexNode -> {
    ...
    final RexNode aboveFilter = simplifyFilterCondition(rexBuilder, filterCondition,
      belowFilters.stream().collect(Collectors.toSet()));
    ...
    // filter out the predicates which are already present below the flatten, so that we don't push it down again.
    belowFilter = RexUtil.composeConjunction(rexBuilder, belowFilters.stream()
      .filter(rexNode -> !preds.pulledUpPredicates.contains(rexNode))
      .collect(Collectors.toList()), false);

    //create a filter from the pushed down predicates.
    RelNode filterBelowFlatten = belowFilter.isAlwaysTrue()? flatten.getInput():
      filter.copy(filter.getTraitSet(), flatten.getInput(), belowFilter);
    //change the input of flatten to the above filter.
    RelNode flattenWithNewInput = flatten.copy(flatten.getTraitSet(), Arrays.asList(filterBelowFlatten));
    //change the original filter with this new simplified filter after push down.
    RelNode filterAboveFlatten = aboveFilter.isAlwaysTrue() ? flattenWithNewInput : filter.copy(filter.getTraitSet(),
      flattenWithNewInput, aboveFilter);
   ...
   */
  private RexNode simplifyFilterCondition(RexBuilder rexBuilder, RexNode filterCondition, Set<RexNode> belowPredicates){
