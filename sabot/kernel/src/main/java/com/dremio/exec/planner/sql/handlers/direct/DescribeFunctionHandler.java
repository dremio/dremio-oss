...
public class DescribeFunctionHandler implements SqlDirectHandler<DescribeFunctionHandler.DescribeResult> {
  private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(DescribeFunctionHandler.class);
  ...
  public List<DescribeResult> toResult(String sql, SqlNode sqlNode) throws RelConversionException, ForemanSetupException {
    ...
    try {
      ...
      final UserDefinedFunction function = catalog.getFunction(namespaceKey);
      if (function == null) {
      ...
      DescribeResult describeResult =  new DescribeResult(
          (function.getName() != null)? function.getName() : "",
          (function.getFunctionArgsList() != null)? function.getFunctionArgsList().toString() : null ,
      ...
      throw UserException.permissionError().message("Not authorized to describe Function.").build(logger);
    } catch (Exception ex) {
      throw UserException.planError(ex)
        .message("Error while rewriting DESCRIBE query: %s", ex.getMessage()).buildSilently();
    }
  }
  ...
  public static class DescribeResult {
    ...
    public DescribeResult(String functionName,
      String argList,
      String returnType,
      String body,
  ...
  public static DescribeFunctionHandler create(QueryContext queryContext) {
    try {
      final Class<?> cl = Class.forName("com.dremio.exec.planner.sql.handlers.EnterpriseDescribeFunctionHandler");
