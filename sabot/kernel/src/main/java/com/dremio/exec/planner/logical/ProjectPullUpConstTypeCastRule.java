 ...
 * WHERE TS3 = '2023-02-03 12:30:00';
 */
public final class ProjectPullUpConstTypeCastRule extends RelOptRule {
  public static final ProjectPullUpConstTypeCastRule INSTANCE = new ProjectPullUpConstTypeCastRule();

  private ProjectPullUpConstTypeCastRule() {
    super(operand(LogicalProject.class, any()), "ProjectPullUpConstTypeCastRule");
  }
  ...
  public void onMatch(RelOptRuleCall relOptRuleCall) {
    ...
    if (predicates != null && !predicates.constantMap.isEmpty()) {
      List<Map.Entry<RexNode, RexNode>> pairs = Lists.newArrayList(predicates.constantMap.entrySet());
  ...
  protected static class RexReplacer extends RexShuttle {
    ...
    RexReplacer(RexBuilder rexBuilder, List<RexNode> replaceableExps, List<RexNode> replacementExps) {
    ...
    private RexNode visit(RexNode call) {
      int i = replaceableExps.indexOf(call);
      if (i == -1) {
        return null;
      }

      RexNode replacement = replacementExps.get(i);
      if (call.getType().getSqlTypeName().equals(SqlTypeName.TIMESTAMP) && replacement.getType() != call.getType()) {
        return this.rexBuilder.makeAbstractCast(call.getType(), replacement);
      }
