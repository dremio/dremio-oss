...
public class PreparedStatementProvider {
  ...
  /**
   * Static list of mappings from {@link MinorType} to JDBC ResultSet class name (to be returned
   * through {@link ResultSetMetaData#getColumnClassName(int)}.
   */
  private static final Map<MinorType, String> MINOR_TYPE_TO_JDBC_CLASSNAME =
  ...
  public static CreatePreparedStatementResp build(
     ...
     */
    final Function<CompleteType, String> completeTypeToSqlTypeNameMap =
        isRecordBatchFormatLessThan23(recordBatchFormat)
            ? ((type) ->
                type.isStruct() ? OLDER_CLIENT_STRUCT_SQL_TYPE_NAME : type.getSqlTypeName())
   ...
   * @return
   */
  private static ResultColumnMetadata serializeColumn(
      Field field, String catalogName, Function<CompleteType, String> converter) {
  ...
  private static boolean isRecordBatchFormatLessThan23(
      UserProtos.RecordBatchFormat recordBatchFormat) {
  ...
  public static class PreparedStatementHandler
      extends ResponseSenderHandler<CreatePreparedStatementResp> {
  ...
  public static class PreparedStatementArrowHandler
      extends ResponseSenderHandler<CreatePreparedStatementArrowResp> {
