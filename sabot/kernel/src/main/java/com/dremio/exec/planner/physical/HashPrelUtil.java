...
/**
 * Contains utility methods for creating hash expression for either distribution (in
 * PartitionSender) or for HashTable.
 */
public class HashPrelUtil {
  ...
  // the hash based operators make use of 4 bytes of hash value, not 8 bytes (for reduced memory
  // use).
  public static final String HASH32_FUNCTION_NAME = "hash32";
  public static final String DATA_FILE_DISTRIBUTE_HASH_FUNCTION_NAME =
   ...
   *     constructed.
   * @param helper Implementation of {@link HashExpressionCreatorHelper} which is used to create
   *     function expressions.
   * @param <T> Input and output expression type. Currently it could be either {@link RexNode} or
   *     {@link LogicalExpression}
   * @return
   */
  public static <T> T createHashBasedPartitionExpression(
      List<T> distFields, HashExpressionCreatorHelper<T> helper) {
    return createHashExpression(distFields, helper);
  }

  public static <T> T createHashBasedPartitionExpression(
      List<T> distFields, HashExpressionCreatorHelper<T> helper, final String functionName) {
  ...
  public static <T> T createHashExpression(
      List<T> inputExprs, HashExpressionCreatorHelper<T> helper) {
    return createHashExpression(inputExprs, helper, HASH32_FUNCTION_NAME);
  }

  public static <T> T createHashExpression(
      List<T> inputExprs, HashExpressionCreatorHelper<T> helper, final String functionName) {
  ...
  public static ProjectPrel addHashProject(
      List<DistributionField> distFields, Prel input, Integer ringCount) {
    return addHashProject(distFields, input, ringCount, HASH32_FUNCTION_NAME);
  }

  public static ProjectPrel addHashProject(
      List<DistributionField> distFields, Prel input, Integer ringCount, String hashFunctionName) {

    // Insert Project SqlOperatorImpl with new column that will be a hash for HashToRandomExchange
    // fields

    final List<String> outputFieldNames = Lists.newArrayList(input.getRowType().getFieldNames());
    final String fieldName =
        ringCount == null ? HashPrelUtil.HASH_EXPR_NAME : WriterPrel.BUCKET_NUMBER_FIELD;
    ...
    if (ringCount != null) {
      ...
      hashExpression =
          rexBuilder.makeCall(
              ...
              ImmutableList.of(
                  hashExpression,
                  rexBuilder.makeExactLiteral(BigDecimal.valueOf(ringCount), intType)));
      hashExpression =
          rexBuilder.makeCall(SqlStdOperatorTable.ABS, Collections.singletonList(hashExpression));
    }
    updatedExpr.add(hashExpression);

    RelDataType rowType =
        RexUtil.createStructType(
            input.getCluster().getTypeFactory(), updatedExpr, outputFieldNames);

    ProjectPrel addColumnprojectPrel =
        ProjectPrel.create(input.getCluster(), input.getTraitSet(), input, updatedExpr, rowType);
   ...
   * @return
   */
  public static LogicalExpression getHashExpression(
      List<DistributionField> fields, RelDataType rowType) {
    ...
    public RexNode createCall(String funcName, List<RexNode> inputFields) {
      final SqlFunction op =
          SqlFunctionImpl.create(
              funcName, DynamicReturnType.INSTANCE, Checker.of(inputFields.size()));
