...
public class PageReader {
  ...
  // read position in the current page, stored in the ByteBuf in DeprecatedParquetVectorizedReader called bufferWithAllData
  long readPosInBytes;
  // bit shift needed for the next page if the last one did not line up with a byte boundary
  int bitShift;
  // storage space for extra nodes at the end of a page if they did not line up with a byte boundary
