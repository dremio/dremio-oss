...
public abstract class JoinPruleBase extends Prule {
  ...
  private static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(JoinPruleBase.class);
  ...
  public static boolean isInequalityHashJoinSupported(JoinRelBase join, OptionResolver options) {
    ...
    final boolean hasInequalityExpression =
        join.getJoinCategory() == JoinCategory.INEQUALITY; // Must have an inequality expression
    final boolean hasEqualityExpression =
        join.getLeftKeys().size() != 0
            && join.getRightKeys().size() != 0; // Must have an equality expression
    ...
    return keysAreJoinable(join.getLeft(), join.getLeftKeys())
        && keysAreJoinable(join.getRight(), join.getRightKeys());
  }
  ...
  public static boolean checkBroadcastConditions(
    ...
    if (probe
            ...
            .equals(DistributionTrait.SINGLETON)
        || (joinType != JoinRelType.INNER && joinType != JoinRelType.LEFT)) {
      return false;
    }

    // Now that we have checked this join supports broadcast, we can go ahead and return true if the
    // user specified
    ...
    // DX-3862:  For broadcast joins, the cost should not just consider the traits and join type.
    // If the broadcast table is small enough,
    // we shouldn't need to worry too much and allow broadcast join and see what the planner picks.
    double cellCountThreshold =
        plannerSettings.getOptions().getOption(PlannerSettings.BROADCAST_CELL_COUNT_THRESHOLD);
    if (buildCellCount > cellCountThreshold) {
      // DX-17913 : For cases when the table is too big due to large number of columns, we should
      // not do the broadcast join.
      logger.debug(
          "Won't do broadcast join if the size of the table is too big based of total number of cells (rows x columns)");
      return false;
    }
    if (buildRowCount
        <= plannerSettings.getOptions().getOption(PlannerSettings.BROADCAST_MIN_THRESHOLD)) {
      logger.debug(
          "Enable broadcast plan? true (rightRowCount {} smaller than minimum broadcast threshold)",
          buildRowCount);
      return true;
    }
    ...
    // In this case, the broadcast table is big-ish.  So, we should check to see if it is reasonable
    // to do broadcast.
    // The broadcasted table will be sent at most (numEndPoints * maxWidthPerNode) times,
    // (rightRowCount) rows.  We add a
    ...
    final long maxWidthPerQuery =
        plannerSettings.getOptions().getOption(ExecConstants.MAX_WIDTH_GLOBAL);
    final long sliceTarget = plannerSettings.getSliceTarget();
    final double minFactor =
        Doubles.min(
            probeRowCount * 1.0 / sliceTarget, numEndPoints * maxWidthPerNode, maxWidthPerQuery);
    final boolean enableBroadCast = (minFactor * broadcastFactor * buildRowCount < probeRowCount);
    logger.debug(
        "Enable broadcast plan? {} minFactor {} (numEndPoints {}, maxWidthPerNode {}, rightRowCount {}, broadcastFactor {}, leftRowCount {}, sliceTarget {}, maxWidthPerQuery {})",
        enableBroadCast,
        minFactor,
        numEndPoints,
        maxWidthPerNode,
  ...
  protected void createDistBothPlan(
      ...
      throws InvalidRelException, UnsupportedRelOperatorException {
    ...
    DistributionTrait hashLeftPartition =
        new DistributionTrait(
            DistributionTrait.DistributionType.HASH_DISTRIBUTED, leftDistributionFields);
    DistributionTrait hashRightPartition =
        new DistributionTrait(
            DistributionTrait.DistributionType.HASH_DISTRIBUTED, rightDistributionFields);
      ...
      for (int i = 0; i < numJoinKeys; i++) {
        hashLeftPartition =
            new DistributionTrait(
                ...
                leftDistributionFields.subList(i, i + 1));
        hashRightPartition =
            new DistributionTrait(
                DistributionTrait.DistributionType.HASH_DISTRIBUTED,
                rightDistributionFields.subList(i, i + 1));
  ...
  // is MergeJoin, a collation must be provided for both left and right child and the plan will
  // contain
  // sort converter if necessary to provide the collation.
  protected abstract void createDistBothPlan(
      RelOptRuleCall call,
      JoinRel join,
      RelNode left,
      RelNode right,
  ...
  // Create join plan with left child ANY distributed and right child BROADCAST distributed. If the
  // physical join type
  // is MergeJoin, a collation must be provided for both left and right child and the plan will
  // contain sort converter
  // if necessary to provide the collation.
  protected abstract void createBroadcastPlan(
