 ...
 *
 * <p>check {@code MaterializedSplitsPointer} and {@code LazySplitsPointer} for concrete
 * implementations of {@code SplitsPointer}.
 *
 * <p>Note that this class doesn't provide implementations for hashCode and equals methods: each
 * concrete class should provides a fast implementation for those methods as we are more concerned
 * about performance over strict equality checking for the pointers.
 */
public abstract class AbstractSplitsPointer implements SplitsPointer {
  private static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(AbstractSplitsPointer.class);
  ...
  public int getSplitsCount() {
    ...
    splitsCount =
        StreamSupport.stream(getPartitionChunks().spliterator(), false)
  ...
  public boolean isPruned() {
    ...
    if (isPruned) {
      logger.warn(
          "Version: {}, total count: {}, split count: {}, partition chunks: {}, \nsplits: {}",
          getSplitVersion(),
          getTotalSplitsCount(),
          getSplitsCount(),
          FluentIterable.from(getPartitionChunks()).size(),
              ...
              .transform(
                  input ->
                      "[key: "
                          ...
                          + input.getSplitCount()
                          + ", split count: "
                          + FluentIterable.from(input.getDatasetSplits()).size()
                          + ", splits: "
                          + FluentIterable.from(input.getDatasetSplits())
                              .limit(5)
                              .join(Joiner.on(","))
                          + "]")
              .limit(5)
