...
/**
 * Captures parallelization parameters for a given operator/fragments. It consists of min and max
 * width of parallelization and affinity to node endpoints.
 */
public class ParallelizationInfo {
  /**
   * Constraints on the width of parallelization. Constraints listed in strictly decreasing order --
   * the further the constraint is, the more restrictive it is In particular, applying one
   * constraint over another results in the higher-ordinal constraint. For example: applying
   * affinity-limited width to an unlimited constraint results in affinity-limited constraint
   */
  ...
  private ParallelizationInfo(
      int minWidth, int maxWidth, Map<NodeEndpoint, EndpointAffinity> affinityMap) {
  ...
  private static WidthConstraint moreRestrictive(
      WidthConstraint constraint1, WidthConstraint constraint2) {
    // NB: implementation relies on the fact that the widths are defined in strictly decreasing
    // order
    return WidthConstraint.values()[Math.max(constraint1.ordinal(), constraint2.ordinal())];
  }

  /**
   * Collects/merges information that results in a ParallelizationInfo instance The information
   * collected includes: - explicit widths (typically supplied by scans, generated from the splits
   * that belong to this scan) - width constraints (typically supplied by exchanges) - affinity
   * suppliers The information is lazily put together when the collector is asked for a
   * ParallelizationInfo instance (at which point further collection is disallowed): - The affinity
   * suppliers are materialized, and merged together into a single affinity map - the width
   * constraints are instantiated (may be based on the affinity map, above), then applied to the
  ...
  public static class ParallelizationInfoCollector {
    ...
    private final List<Supplier<Collection<EndpointAffinity>>> uninstantiatedAffinities =
        new ArrayList<>();
    private ParallelizationInfo pInfo = null; // Computed lazily at get()

    void addMaxWidth(int newMaxWidth) {
      Preconditions.checkState(
          pInfo == null, "Invalid use of ParallelizationInfoCollector past get()");
    ...
    /**
     * Get the minimum width so far. Please note that the endpoint affinity width constraints may
     * not be applied unless the endpoint affinities were materialized
     */
    int getMinWidth() {
      if (pInfo != null) {
    ...
    /**
     * Get the maximum width so far. Please note that the endpoint affinity width constraints may
     * not be applied unless the endpoint affinities were materialized
     */
    int getMaxWidth() {
      if (pInfo != null) {
      ...
      switch (widthConstraint) {
        ...
        case AFFINITY_LIMITED:
          if (affinitiesMaterialized) {
            int maxWidth =
                affinityMap.values().stream().mapToInt(EndpointAffinity::getMaxWidth).sum();
            int minWidth =
                affinityMap.values().stream().mapToInt(EndpointAffinity::getMinWidth).sum();
        ...
        default:
          throw new IllegalStateException(
              String.format("Invalid width constraint %s", widthConstraint));
      }
    }
