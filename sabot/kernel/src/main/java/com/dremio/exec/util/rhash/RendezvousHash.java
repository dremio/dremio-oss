 ...
 *       and removing nodes however blocks each other
 *   <li>Low overhead: providing using a hash function of low overhead
 *   <li>Load balancing: Since the hash function is randomizing, each of the n nodes is equally
...
public class RendezvousHash<K, N extends Comparable<? super N>> {
  ...
  // Getter methods
  protected HashFunction getHasher() {
    return hasher;
  }

  protected Funnel<K> getKeyFunnel() {
    return keyFunnel;
  }

  protected Funnel<N> getNodeFunnel() {
    return nodeFunnel;
  }

  protected ConcurrentSkipListSet<N> getOrdered() {
    return ordered;
  }

  /**
   * Creates a new RendezvousHash with a starting set of nodes provided by init. The funnels will be
   * used when generating the hash that combines the nodes and keys. The hasher specifies the
  ...
  /**
   * Removes a node from the pool. Keys that referenced it should after this be evenly distributed
  ...
  public N get(K key) {
    ...
    for (N node : ordered) {
      long nodesHash =
          hasher.newHasher().putObject(key, keyFunnel).putObject(node, nodeFunnel).hash().asLong();
