...
public final class FileBasedDatasetUpdateTimePruner {
  ...
  /**
   * Checks if a dataset supports non-partition pruning of splits during planning based on last modified times.
   */
  public static boolean scanSupportsSplitUpdateTimePruning(TableScan tableScan) {
    ...
    // if update column is a partition column, standard sarg-based partition pruning can be used instead to prune
    List<String> partitionColumns = dataset.getReadDefinition() == null ? null :
        dataset.getReadDefinition().getPartitionColumnsList();
    if (partitionColumns != null && partitionColumns.contains(IncrementalUpdateUtils.UPDATE_COLUMN)) {
      return false;
    }
  ...
  private static TableMetadata filterSplits(TableMetadata tableMetadata, long minModTime) {
    List<PartitionChunkMetadata> filteredSplits = Streams.stream(tableMetadata.getSplits())
  ...
  private static PartitionChunkMetadata filterPartitionChunk(PartitionChunkMetadata metadata, long minModTime) {
    long size = 0;
    long rowCount = 0;
    ...
    for (PartitionProtobuf.DatasetSplit split : metadata.getDatasetSplits()) {
      ...
      if (xattr == null || !xattr.hasUpdateKey() || !xattr.getUpdateKey().hasLastModificationTime()) {
        throw UserException.invalidMetadataError()
            .message("Dataset is missing last modified times metadata for data files.  Please refresh metadata.")
            .buildSilently();
      }
