...
public class FilterFlattenTransposeRule extends RelOptRule {
  ...
  private static class PushConjunct {
    ...
    public PushConjunct(RexNode condition, Set<Integer> requiredFlattens) {
      super();
    ...
    public static PushConjunct merge(RexBuilder rexBuilder, Set<PushConjunct> conjuncts) {
      ...
      Set<Integer> totalRequired =
          FluentIterable.from(conjuncts)
              .transformAndConcat(
                  new Function<PushConjunct, Set<Integer>>() {
                    @Override
      ...
      List<RexNode> nodes =
          FluentIterable.from(conjuncts)
              .transform(
                  new Function<PushConjunct, RexNode>() {
  ...
  public void onMatch(RelOptRuleCall call) {
    ...
    final RexNode neverPushableCondition =
        RexUtil.composeConjunction(filter.getCluster().getRexBuilder(), neverPushable, false);
    final RexNode alwaysPushableCondition =
        RexUtil.composeConjunction(filter.getCluster().getRexBuilder(), alwaysPushable, false);

    // for each flatten, take always pushable and then add each conjunct that is not in the
    // dependent set and push those.
    for (Integer i : flattenedFields) {
      ...
      RexNode fullPushCondition =
          RexUtil.composeConjunction(
              filter.getCluster().getRexBuilder(),
              ImmutableList.<RexNode>of(alwaysPushableCondition, merged.condition),
              false);
      ...
      RelNode newInput =
          belowFlattenFields.isEmpty()
              ? child
              : new FlattenRel(
                  filter.getCluster(),
                  flatten.getTraitSet(),
                  child,
                  asNodes(filter.getCluster(), belowFlattenFields),
                  flatten.getAliases(),
                  flatten.getNumProjectsPushed());
      RelNode newFilter =
          new FilterRel(filter.getCluster(), filter.getTraitSet(), newInput, fullPushCondition);
      RelNode topFlatten =
          new FlattenRel(
              filter.getCluster(),
              filter.getTraitSet(),
              newFilter,
              asNodes(filter.getCluster(), aboveFlattenFields),
              flatten.getAliases(),
              flatten.getNumProjectsPushed());
      RelNode top =
          neverPushableCondition.isAlwaysTrue()
              ? topFlatten
              : new FilterRel(
                  filter.getCluster(), filter.getTraitSet(), topFlatten, neverPushableCondition);
  ...
  private static List<RexInputRef> asNodes(final RelOptCluster cluster, Iterable<Integer> indices) {
    ...
    return FluentIterable.from(indices)
        .transform(
            new Function<Integer, RexInputRef>() {
              @Override
              public RexInputRef apply(Integer input) {
                return builder.makeInputRef(
