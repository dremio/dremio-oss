 ...
 */
public abstract class SimplifyNLJConditionRuleBase extends RelOptRule {

  protected static final Logger tracer = CalciteTrace.getPlannerTracer();

  private SimplifyNLJConditionRuleBase(RelOptRuleOperand operand, String name) {
    super(operand, name);
  }

  public static class SimplifyProjectNLJConditionRule extends SimplifyNLJConditionRuleBase {
    public static final RelOptRule INSTANCE = new SimplifyProjectNLJConditionRule();
    public SimplifyProjectNLJConditionRule() {
      super(RelOptHelper.some(ProjectPrel.class, RelOptHelper.any(NestedLoopJoinPrel.class)), "SimplifyProjectNLJConditionRule");
    }

    @Override
    public boolean matches(RelOptRuleCall call) {
      final NestedLoopJoinPrel nestedLoopJoinPrel = call.rel(1);
      return nestedLoopJoinPrel.getProjectedFields() != null && !nestedLoopJoinPrel.hasVectorExpression();
    }

    @Override
    public void onMatch(RelOptRuleCall call) {
      final ProjectPrel projectPrel = call.rel(0);
      final NestedLoopJoinPrel nestedLoopJoinPrel = call.rel(1);
      doOnMatch(call, projectPrel, nestedLoopJoinPrel);
    }
  }

  public static class SimplifyNLJConditionRule extends SimplifyNLJConditionRuleBase {
    ...
    public void onMatch(RelOptRuleCall call) {
      final NestedLoopJoinPrel nestedLoopJoinPrel = call.rel(0);
      doOnMatch(call, null, nestedLoopJoinPrel);
    }
  }

  public void doOnMatch(RelOptRuleCall call, ProjectPrel projectPrel, NestedLoopJoinPrel nlj) {
    ...
    if (nlj.getProjectedFields() == null) {
      ...
      oldProjectedFields = nlj.getProjectedFields().asList();
    }
    ...
    for (int i = 0; i < oldProjectedFields.size(); i++) {
      int index = oldProjectedFields.get(i);
      if (index >= leftCount) {
    ...
    NestedLoopJoinPrel newJoin = NestedLoopJoinPrel.create(nlj.getCluster(), nlj.getTraitSet(), newLeft, newRight, nlj.getJoinType(), newCondition, projected);

    if (projectPrel != null) {
      final List<Integer> newProjectedFields = newJoin.getProjectedFields().asList();
      final IntIntHashMap oldToNewFields = new IntIntHashMap();
      for(int i = 0 ; i < oldProjectedFields.size() ; i++) {
        oldToNewFields.put(oldProjectedFields.get(i), newProjectedFields.get(i));
      }
      List<RexNode> newExprs = new ArrayList<>();
      PrelUtil.InputRewriter rewriter = new PrelUtil.InputRewriter(oldToNewFields);
      for (RexNode expr : projectPrel.getChildExps()) {
        newExprs.add(expr.accept(rewriter));
      }
      call.transformTo(ProjectPrel.create(newJoin.getCluster(), newJoin.getTraitSet(), newJoin, newExprs, projectPrel.getRowType()));
    } else {
   ...
   */
  static class ExpressionPusher extends RexBiShuttle<Pointer<SimplifyNLJConditionRule.Side>> {
