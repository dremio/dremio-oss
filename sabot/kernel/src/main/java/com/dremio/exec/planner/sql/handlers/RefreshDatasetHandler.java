...
public class RefreshDatasetHandler implements SqlToPlanHandler {
  ...
  public PhysicalPlan getPlan(SqlHandlerConfig config, String sql, SqlNode sqlNode)
    ...
    config
        .getObserver()
        .planValidated(
            new PojoDataType(SimpleCommandResult.class).getRowType(JavaTypeFactoryImpl.INSTANCE),
            sqlNode,
            0,
            true);

    final SqlRefreshDataset sqlRefreshDataset =
        SqlNodeUtil.unwrap(sqlNode, SqlRefreshDataset.class);

    try {
      final MetadataRefreshPlanBuilder refreshDatasetPlanBuilder =
          MetadataRefreshPlanBuilderFactory.getPlanBuilder(config, sqlRefreshDataset);

      final Prel rootPrel = refreshDatasetPlanBuilder.buildPlan();

      final Prel finalizedPlan = (Prel) rootPrel.accept(new PrelFinalizer());
      final Pair<Prel, String> transformedPrel =
          PrelTransformer.applyPhysicalPrelTransformations(config, finalizedPlan);
