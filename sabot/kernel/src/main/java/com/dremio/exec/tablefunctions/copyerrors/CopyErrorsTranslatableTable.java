...
public final class CopyErrorsTranslatableTable implements TranslatableTable {
  ...
  public static final List<Triple<String, SqlTypeName, ArrowType>> COPY_ERRORS_TABLEFUNCTION_SCHEMA = new ArrayList() {{
  ...
  public RelNode toRel(RelOptTable.ToRelContext context, RelOptTable relOptTable) {
    return new CopyErrorsCrel(
      ...
      this.context,
      new CopyErrorsCatalogMetadata(getBatchSchema(), getRowType(context.getCluster().getTypeFactory())));
  }

  private static BatchSchema getBatchSchema() {
    SchemaBuilder schemaBuilder = BatchSchema.newBuilder();
    COPY_ERRORS_TABLEFUNCTION_SCHEMA.stream().map(p -> Field.nullable(p.getLeft(), p.getRight())).forEach(schemaBuilder::addField);
