 ...
 */
public class SqlGrantOnAllCatalogDatasets extends SqlGrantOnAllDatasets implements SimpleDirectHandler.Creator {
  public static final SqlSpecialOperator OPERATOR = new SqlSpecialOperator("GRANT", SqlKind.OTHER) {
    @Override
    public SqlCall createCall(SqlLiteral functionQualifier, SqlParserPos pos, SqlNode... operands) {
      Preconditions.checkArgument(operands.length == 5, "SqlGrantOnAllCatalogDatasets.createCall() has to get 5 operands!");
  ...
  public SqlGrantOnAllCatalogDatasets(SqlParserPos pos,
  ...
  public SimpleDirectHandler toDirectHandler(QueryContext context) {
    try {
      final Class<?> cl = Class.forName("com.dremio.exec.planner.sql.handlers.GrantOnAllCatalogDatasetsHandler");
