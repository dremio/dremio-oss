...
public final class FormatPluginOptionExtractor {
  private static final Logger logger =
      org.slf4j.LoggerFactory.getLogger(FormatPluginOptionExtractor.class);

  private final Map<String, FormatPluginOptionsDescriptor> optionsByTypeName;

  /**
   * extracts the format plugin options based on the scanned implementations of {@link
   * FormatPluginConfig}
   *
   * @param scanResult
   */
  FormatPluginOptionExtractor(ScanResult scanResult) {
    ...
    for (Class<? extends FormatPluginConfig> pluginConfigClass : pluginConfigClasses) {
      FormatPluginOptionsDescriptor optionsDescriptor =
          new FormatPluginOptionsDescriptor(pluginConfigClass);
  ...
  /**
   * given a table function signature and the corresponding parameters return the corresponding
   * formatPlugin configuration
   *
   * @param t the signature and parameters (it should be one of the signatures returned by {@link
   *     FormatPluginOptionExtractor#getTableSignatures(String)})
   * @return the config
   */
   ...
   * @return
   */
  FormatPluginConfig createConfigForTable(
      final String tableName, final Map<String, Object> formatOptions) {
  ...
  public List<Function> getFunctions(
    ...
    return FluentIterable.from(sigs)
        .transform(
            new com.google.common.base.Function<TableSignature, Function>() {
              @Override
