...
public class HostAffinityComputer {
  // Computes the host affinities for the given split and returns a list sorted in decreasing order of affinities
  public static List<EndpointsAffinity> computeSortedAffinitiesForSplit(long splitStart, long splitLength,
                                                                        Iterable<BlockLocations> blockLocations) {
    Map<HostAndPort, Float> hostAffinityMap = new HashMap<>();
    for (BlockLocations blockLocation : blockLocations) {
      long overlapLength = getOverlapLength(splitStart, splitLength, blockLocation.getOffset(), blockLocation.getSize());
      if (overlapLength <= 0) {
        continue;
      }
  ...
  public static Map<HostAndPort, Float> computeAffinitiesForSplit(long splitStart, long splitLength,
  ...
  private static long getOverlapLength(long splitStart, long splitLength, long blockStart, long blockLength) {
