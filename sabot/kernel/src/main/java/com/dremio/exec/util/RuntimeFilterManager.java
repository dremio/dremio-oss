...
/**
 * Intended to serve as a temporary cache for the runtime filters until assembly is completed from
 * all minor fragments in shuffle join case.
 */
@NotThreadSafe
public class RuntimeFilterManager implements AutoCloseable {
  ...
  public RuntimeFilterManager(
      final BufferAllocator allocator,
      final int valFilterMaxSize,
   ...
   * @throws Exception
   */
  public RuntimeFilterManagerEntry coalesce(
      RuntimeFilter filter,
      Optional<BloomFilter> partitionColFilter,
      List<ValueListFilter> nonPartitionColFilters,
      int minorFragmentId) {
    Optional<RuntimeFilterManagerEntry> filterEntry =
        filterEntries.stream().filter(f -> f.isTargetedToSameScan(filter)).findAny();
    if (filterEntry.isPresent()) {
      return merge(
          filter, partitionColFilter, nonPartitionColFilters, filterEntry.get(), minorFragmentId);
    } else {
      // Make a copy and refer it as base to merge into.
      final Optional<BloomFilter> partitionColFilterCopy =
          partitionColFilter.map(f -> f.createCopy(allocator));
      final List<ValueListFilter> nonPartitionColFiltersCopy =
          nonPartitionColFilters.stream()
              .map(v -> v.createCopy(allocator))
              .collect(Collectors.toList());
      final RuntimeFilterManagerEntry newEntry =
          new RuntimeFilterManagerEntry(
              filter, allMinorFragments, partitionColFilterCopy, nonPartitionColFiltersCopy);
      newEntry.remainingMinorFragments.remove(minorFragmentId);
      logger.debug(
          "New filter entry created. remaining fragments {}", newEntry.remainingMinorFragments);
  ...
  /**
   * Merges the filter piece with the respective entry present in the RuntimeFilterManagerEntry. The
   * matching is done on the basis of probe scan coordinates (operator id, major fragment id).
   *
   * @param filterPiece
   * @param partitionColFilter
   * @param minorFragmentId
   */
  private RuntimeFilterManagerEntry merge(
      RuntimeFilter filterPiece,
      Optional<BloomFilter> partitionColFilter,
      List<ValueListFilter> nonPartitionColFilters,
    ...
    try {
      checkArgument(
          baseEntry.getRemainingMinorFragments().contains(minorFragmentId),
          "Not expecting filter piece from "
              + minorFragmentId
              + ", remaining minor fragments: "
              + baseEntry.getRemainingMinorFragments());
      baseEntry.remainingMinorFragments.remove(minorFragmentId);

      if (baseEntry.isDropped()) {
        logger.info(
            "Skipping merge of filter piece from {} in {}",
            minorFragmentId,
            toCoordinates(filterPiece));
        return baseEntry;
      }

      logger.debug(
          "Merging incoming filter from minor fragment {}, targeted to {}. Remaining fragments {}",
          minorFragmentId,
          baseEntry.getProbeScanCoordinates(),
          baseEntry.getRemainingMinorFragments());
      ...
      logger.error(
          "Error while merging the filter from "
              + minorFragmentId
              + ", target "
              + baseEntry.getProbeScanCoordinates(),
          e);
  ...
  private String toCoordinates(RuntimeFilter filter) {
    return String.format(
        "OperatorId: %s, MajorFragmentID: %s",
        filter.getProbeScanOperatorId(), filter.getProbeScanMajorFragmentId());
  }
  ...
  public class RuntimeFilterManagerEntry implements AutoCloseable {
    ...
    private RuntimeFilterManagerEntry(
        RuntimeFilter compositeFilter,
        Set<Integer> remainingMinorFragments,
        Optional<BloomFilter> partitionColFilter,
        List<ValueListFilter> nonPartitionColFilters) {
      this.compositeFilter = compositeFilter;
      this.remainingMinorFragments.addAll(remainingMinorFragments);
      this.partitionColFilter = partitionColFilter.orElse(null);
      this.nonPartitionColFilters =
          nonPartitionColFilters.stream().collect(Collectors.toMap(v -> v.getFieldName(), v -> v));
    }
    ...
    public List<ValueListFilter> getNonPartitionColFilters() {
      return compositeFilter.getNonPartitionColumnFilterList().stream()
    ...
    public boolean isTargetedToSameScan(RuntimeFilter that) {
      return this.compositeFilter.getProbeScanMajorFragmentId()
              == that.getProbeScanMajorFragmentId()
    ...
    public void merge(Optional<BloomFilter> incomingFilter) {
      ...
      try {
        logger.debug(
            "Merging incoming filter {} into existing filter {}", incomingName, partitionColFilter);
    ...
    public void merge(String fieldName, ValueListFilter thatFilter) {
      ...
      try {
        ...
        checkArgument(
            thisFilter.isBoolField() == thatFilter.isBoolField(),
            "Cannot merge a boolean filter from a non-boolean one.");
        ValueListFilter mergedFilter =
            ValueListFilterBuilder.buildPlainInstance(
                allocator, thatFilter.getBlockSize(), valFilterMaxSize, thisFilter.isBoolField());
        closeables.add(mergedFilter);

        ValueListFilter.merge(thisFilter, thatFilter, mergedFilter);
        checkState(
            mergedFilter.getValueCount() <= valFilterMaxSize,
            "Merged valuelistfilter overflown for %s.",
            fieldName);
    ...
    private void dropPartitionColFilter() {
      try {
        ...
        compositeFilter =
            RuntimeFilter.newBuilder(compositeFilter).clearPartitionColumnFilter().build();
      } catch (Exception e) {
        logger.warn(
            "Error while closing partition col filter for " + getProbeScanCoordinates(),
            e.getMessage());
      } finally {
    ...
    private void dropNonPartitionColFilter(String name) {
      ...
      try {
        ...
        final List<CompositeColumnFilter> compositeColumnFilters =
            compositeFilter.getNonPartitionColumnFilterList();
        ...
        compositeColumnFilters.stream()
            .filter(c -> !c.getColumnsList().contains(name))
        ...
        logger.warn(
            String.format(
                "Error while closing non-partition col filter for field %s, target %s",
                name, getProbeScanCoordinates()),
            e.getMessage());
      } finally {
    ...
    private void resetValueCounts() {
      ...
      if (this.partitionColFilter != null) {
        final CompositeColumnFilter partitionColFilter =
            CompositeColumnFilter.newBuilder(compositeFilter.getPartitionColumnFilter())
                .setValueCount(this.partitionColFilter.getNumBitsSet())
                .build();
      ...
      for (int i = 0; i < this.compositeFilter.getNonPartitionColumnFilterCount(); i++) {
        ...
        final ValueListFilter valueListFilter =
            this.getNonPartitionColFilter(current.getColumns(0));
        final CompositeColumnFilter nonPartitionColFilter =
            CompositeColumnFilter.newBuilder(current)
