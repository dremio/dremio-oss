...
/**
 * Generates constants defined as class variables either as an array (to reduce the JAVA constant
 * pool usage) or as individual class variables. This is part of the class generator that generates
 * code in JAVA for expressions inside operators.
 *
 * <p>If the constant extractor has found more than a threshold number of constants this class
 * defines an array for the different constants in the query (a unique array position is provided
 * for each constant). There will be a different array for each type of constant and the extractor
 * is used to understand the array size for each type. If the constant extractor has found less than
 * a threshold number of constants in the expression, this class generates a unique class variable
...
public class ConstantGenerator {
  ...
  // information tracked per constant type, which includes information about next array index to
  // use.
  ...
  public JAssignmentTarget nextConstant(CompleteType type, JType holderType, int uniqueIndex) {
    ...
    if (constantExtractor != null
        && constantExtractor.isLarge(constantArrayThreshold)
    ...
    final ConstantArrayHolder holder =
        holders.compute(
            type,
            (k, v) ->
                (v == null) ? new ConstantArrayHolder(holderType, numConstants, uniqueIndex) : v);
  ...
  private final class ConstantArrayHolder {
    ...
    private ConstantArrayHolder(JType holderType, int numConstants, int uniqueIndex) {
      ...
      arrayDecl =
          clazz.field(JMod.FINAL, holderType.array(), "constantArray" + uniqueIndex, holders);
    }
