...
public class SimpleCastFunctions {
  ...
  public static class CastVarCharBoolean implements SimpleFunction {
    ...
    public void eval() {
      ...
      if ("true".equals(input) || "1".equals(input)) {
        ...
        throw errCtx.error().message("Invalid value for boolean: '%s'", input).build();
      }
    }
  }
  ...
  public static class CastBooleanVarChar implements SimpleFunction {
    ...
    public void eval() {
      byte[] outB =
          in.value == 1
              ? com.dremio.exec.expr.fn.impl.SimpleCastFunctions.TRUE
              : com.dremio.exec.expr.fn.impl.SimpleCastFunctions.FALSE;
      ...
      out.end =
          Math.min(
              (int) len.value,
              outB.length); // truncate if target type has length smaller than that of input's
      // string
    }
  }
}
