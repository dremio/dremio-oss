...
class AggrFunctionHolder extends BaseFunctionHolder {
  static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(AggrFunctionHolder.class);
  ...
  public AggrFunctionHolder(FunctionAttributes attributes, FunctionInitializer initializer) {
    super(attributes, initializer);
    checkArgument(
        attributes.getNullHandling() == NullHandling.INTERNAL,
  ...
  public JVar[] renderStart(
    ...
    if (!g.getMappingSet().isHashAggMapping()) { // Declare workspace vars for non-hash-aggregation.
        ...
        if (!workspaceVars[id].isInject()) {
          setupBlock.assign(
              ...
              mathClass
                  .staticInvoke("min")
                  .arg(sizeVar)
                  .arg(g.getWorkspaceVectors().get(workspaceVars[id]).invoke("getValueCapacity")));
        }
      }
      ...
      for (int i = 0; i < workspaceVars.length; i++) {
        if (!workspaceVars[i].isInject()) {
          setupBlock.assign(
              workspaceJVars[i],
              JExpr._new(
                  CodeModelArrowHelper.getHolderType(workspaceVars[i].completeType, g.getModel())));
        }
      }
      ...
      JBlock subBlock =
          generateInitWorkspaceBlockHA(g, BlockType.SETUP, setup(), workspaceJVars, ivar);
  ...
  public HoldingContainer renderEnd(
    ...
    // hash aggregate uses workspace vectors. Initialization is done in "setup" and does not require
    // "reset" block.
      ...
      if (workspaceVar.isInject()) {
        ...
        if (function == null) {
          throw UserException.validationError()
              .message("Unknown inject type %s.", workspaceVar.type.getName())
              .build(logger);
        }
        workspaceJVars[i] = g.declareClassField("work", g.getModel()._ref(workspaceVars[i].type));
        g.getBlock(BlockType.SETUP)
            .assign(workspaceJVars[i], JExpr.direct("context").invoke(function));
      } else {
        Preconditions.checkState(
            workspaceVars[i].completeType.isFixedWidthScalar(),
            "Workspace variable '%s' in aggregation function '%s' is not allowed to have variable length type.",
            workspaceVars[i].name,
            registeredNames[0]);
        ...
        TypedFieldId typedFieldId =
            new TypedFieldId(workspaceVars[i].completeType, g.getWorkspaceTypes().size());
        JVar vv =
            g.declareVectorValueSetupAndMember(g.getMappingSet().getWorkspace(), typedFieldId);
  ...
  private JBlock generateInitWorkspaceBlockHA(
      ClassGenerator<?> g,
      BlockType bt,
      String body,
      JVar[] workspaceJVars,
      JExpression wsIndexVariable) {
   ...
   */
  private void addProtectedBlockHA(
      ClassGenerator<?> g,
      JBlock sub,
      String body,
      HoldingContainer[] inputVariables,
    ...
    for (int i = 0; i < workspaceJVars.length; i++) {
      ...
      } else if (workspaceVars[i].completeType.getType().getTypeID()
        ...
        if (!type.isFixedWidthScalar()) {
          setMeth =
              g.getWorkspaceVectors()
                  ...
                  .invoke("setSafe")
                  .arg(wsIndexVariable)
                  .arg(workspaceJVars[i].ref("value"));
        } else {
          setMeth =
              g.getWorkspaceVectors()
                  .get(workspaceVars[i])
