...
public class JoinComputationExtractor {
  public static ExtractedComputation extractedComputation(
    ...
    JoinSideDetector joinSideDetector =
        new JoinSideDetector(
    ...
    RexNode newJoinCondition =
        rewriteCondition(
            rexBuilder,
            joinCondition,
            expressions.rexNodeToComputationIndex,
            leftCount,
            expressions.leftComputations.size());
    RexNode newExtraCondition =
        rewriteCondition(
            rexBuilder,
            extraCondition,
            expressions.rexNodeToComputationIndex,
    ...
    List<RexNode> leftExprs =
        ImmutableList.<RexNode>builder()
            .addAll(MoreRelOptUtil.identityProjects(leftType))
            .addAll(expressions.leftComputations)
            .build();
    RelDataType leftProjectType =
        RexUtil.createStructType(typeFactory, leftExprs, null, SqlValidatorUtil.F_SUGGESTER);
    RelNode newLeft =
        expressions.leftComputations.isEmpty()
            ? leftNode
            : ProjectPrel.create(
                cluster, leftNode.getTraitSet(), leftNode, leftExprs, leftProjectType);

    List<RexNode> rightExprs =
        ImmutableList.<RexNode>builder()
            .addAll(MoreRelOptUtil.identityProjects(rightType))
            .addAll(expressions.rightComputations)
            .build();

    RelDataType rightProjectType =
        RexUtil.createStructType(typeFactory, rightExprs, null, SqlValidatorUtil.F_SUGGESTER);

    RelNode newRight =
        rightCount == rightExprs.size()
            ? rightNode
            : ProjectPrel.create(
                cluster, rightNode.getTraitSet(), rightNode, rightExprs, rightProjectType);

    List<RexNode> topProject =
        ImmutableList.<RexNode>builder()
            .addAll(MoreRelOptUtil.identityProjects(joinRowType, ImmutableBitSet.range(leftCount)))
            .addAll(
                identityProjectsWithShifting(
                    joinRowType,
                    ImmutableBitSet.range(leftCount, leftCount + rightCount),
                    expressions.leftComputations.size()))
            .build();

    return new ExtractedComputation(
        newJoinCondition, newExtraCondition, newLeft, newRight, topProject);
  }

  private static RexNode rewriteCondition(
      RexBuilder rexBuilder,
      RexNode rexNode,
      Map<String, Integer> rexToComputationOffset,
      int leftOriginalCount,
      int shiftRightBy) {
        ...
        new RexShuttle() {
          ...
          public RexNode visitCall(RexCall call) {
            if (rexToComputationOffset.containsKey(call.toString())) {
              return rexBuilder.makeInputRef(
                  call.type, rexToComputationOffset.get(call.toString()));
            } else {
          ...
          public RexNode visitInputRef(RexInputRef inputRef) {
            if (inputRef.getIndex() < leftOriginalCount) {
              ...
              return rexBuilder.makeInputRef(
                  inputRef.getType(), inputRef.getIndex() + shiftRightBy);
            }
          }
        });
  }

  private static List<RexNode> identityProjectsWithShifting(
      RelDataType relDataType, ImmutableBitSet columns, int shift) {
    List<RexNode> projectedColumns = MoreRelOptUtil.identityProjects(relDataType, columns);
    return ImmutableList.<RexNode>builder().addAll(RexUtil.shift(projectedColumns, shift)).build();
  }
