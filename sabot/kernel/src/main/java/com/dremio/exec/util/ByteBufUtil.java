...
public class ByteBufUtil {
   ...
   */
  public static void checkBufferLength(
      FunctionErrorContext errorContext, ArrowBuf buffer, int start, int end, int requiredLen) {
    ...
    if (actualLen != requiredLen) {
      throw errorContext
          ...
          .addContext(
              "Buffer",
              DremioStringUtils.toBinaryString(NettyArrowBuf.unwrapBuffer(buffer), start, end))
          .build();
    }
  }

  /**
   * Modeled after {@code org.apache.hadoop.io.WritableUtils}. We copy the code to avoid wrapping
   * {@link ArrowBuf} to/from {@link DataInput}.
   */
  public static class HadoopWritables {
     ...
     * indicates whether the integer is positive or negative, and the number of bytes that follow.
     * If the first byte value v is between -121 and -124, the following integer is positive, with
     * number of bytes that follow are -(v+120). If the first byte value v is between -125 and -128,
     * the following integer is negative, with number of bytes that follow are -(v+124). Bytes are
    ...
    public static void writeVInt(
        FunctionErrorContext errorContext, ArrowBuf buffer, int start, int end, int i) {
     ...
     */
    public static void writeVLong(
        FunctionErrorContext context, ArrowBuf buffer, int start, int end, long i) {
     ...
     */
    public static long readVLong(
        FunctionErrorContext errorContext, ArrowBuf buffer, int start, int end) {
