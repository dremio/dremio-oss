...
public abstract class PruneScanRuleBase<T extends ScanRelBase & PruneableScan> extends RelOptRule {
  ...
  private final Map<EvaluationPruningKey, EvaluationPruningResult> evalutationPruningCache =
      new HashMap<>();

  private PruneScanRuleBase(
  ...
  public static class PruneScanRuleFilterOnProject<T extends ScanRelBase & PruneableScan>
      extends PruneScanRuleBase<T> {
    public PruneScanRuleFilterOnProject(
        StoragePluginId pluginId, Class<T> clazz, OptimizerRulesContext optimizerContext) {
      super(
          pluginId.getType(),
          RelOptHelper.some(
              FilterRel.class, RelOptHelper.some(ProjectRel.class, RelOptHelper.any(clazz))),
          pluginId.getType().value()
              + "NewPruneScanRule:Filter_On_Project."
              + SLUGIFY.slugify(pluginId.getName())
              + "."
    ...
    public PruneScanRuleFilterOnProject(
        SourceType pluginType, Class<T> clazz, OptimizerRulesContext optimizerContext) {
      super(
          pluginType,
          RelOptHelper.some(
              FilterRel.class, RelOptHelper.some(ProjectRel.class, RelOptHelper.any(clazz))),
          pluginType.value() + "NewPruneScanRule:Filter_On_Project",
          optimizerContext);
    }
  ...
  public static class PruneScanRuleFilterOnScan<T extends ScanRelBase & PruneableScan>
      extends PruneScanRuleBase<T> {
    public PruneScanRuleFilterOnScan(
        StoragePluginId pluginId, Class<T> clazz, OptimizerRulesContext optimizerContext) {
      super(
          pluginId.getType(),
          RelOptHelper.some(FilterRel.class, RelOptHelper.any(clazz)),
          pluginId.getType().value()
              + "NewPruneScanRule:Filter_On_Scan."
              + SLUGIFY.slugify(pluginId.getName())
              + "."
              + UUID.randomUUID().toString(),
          optimizerContext);
    ...
    public PruneScanRuleFilterOnScan(
      ...
      super(
          pluginType,
          RelOptHelper.some(FilterRel.class, RelOptHelper.any(clazz)),
          pluginType.value() + "NewPruneScanRule:Filter_On_Scan",
          optimizerContext);
    }
  ...
  // new prune scan rule for filter on SampleRel, fires when filter above a sample rel with
  // pruneable scan.
  public static class PruneScanRuleFilterOnSampleScan<T extends ScanRelBase & PruneableScan>
      extends PruneScanRuleBase<T> {
    public PruneScanRuleFilterOnSampleScan(
        StoragePluginId pluginId, Class<T> clazz, OptimizerRulesContext optimizerContext) {
      super(
          pluginId.getType(),
          RelOptHelper.some(
              FilterRel.class, RelOptHelper.some(SampleRel.class, RelOptHelper.any(clazz))),
          pluginId.getType().value()
              + "NewPruneScanRule:Filter_On_SampleRel_Scan."
              + SLUGIFY.slugify(pluginId.getName())
              + "."
              + UUID.randomUUID().toString(),
          optimizerContext);
    ...
    public PruneScanRuleFilterOnSampleScan(
      ...
      super(
          ...
          RelOptHelper.some(
              FilterRel.class, RelOptHelper.some(SampleRel.class, RelOptHelper.any(clazz))),
          pluginType.value() + "NewPruneScanRule:Filter_On_SampleRel_Scan",
          optimizerContext);
    }
  ...
  private boolean doSargPruning(
    ...
    final FindSimpleFilters.StateHolder holder =
        pruneCondition.accept(new FindSimpleFilters(builder, true, false));
    ...
    final List<FilterProperties> filters =
        FluentIterable.from(conditions)
            .transform(
                new Function<RexCall, ParquetFilterCondition.FilterProperties>() {
                  @Override
    ...
    for (Map.Entry<String, FilterProperties> entry : map.entries()) {
      splitFilters.add(
          MetadataUtils.toSplitsSearchQuery(
              Collections.singletonList(entry.getValue()), fieldMap.get(entry.getKey())));
    }

    final TableMetadata prunedDatasetPointer =
        scanRel.getTableMetadata().prune(SearchQueryUtils.and(splitFilters));
  ...
  private boolean doEvalPruning(
    ...
    LogicalExpression pruningExpression =
        RexToExpr.toExpr(
            new ParseContext(settings),
            scanRel.getRowType(),
            scanRel.getCluster().getRexBuilder(),
            pruneCondition);

    // Check cache if pruning condition was eval'ed against the same batch
    final EvaluationPruningKey cacheKey =
        new EvaluationPruningKey(pruningExpression, tableMetadata.getSplitsKey());
    ...
    if (cacheResult != null) {
      ...
      logger.debug(
          "In cache: total records: {}, qualified records: {}",
          cacheResult.totalRecords,
          cacheResult.finalSplits.size());
    ...
    do {
      ...
      logger.debug(
          "Elapsed time to get list of splits for the current batch: {} ms within batchIndex: {}",
      ...
      try (final BufferAllocator allocator =
              optimizerContext
                  .getAllocator()
                  .newChildAllocator("prune-scan-rule", 0, Long.MAX_VALUE);
        ...
        for (int partitionColumnIndex : BitSets.toIter(partitionColumnBitSet)) {
          ...
          final CompleteType completeType =
              scanRel.getBatchSchema().getFieldId(column).getFinalType();
          ...
          final ValueVector v =
              TypeHelper.getNewVector(
                  getFieldForNameAndMajorType(column.getAsUnescapedPath(), type), allocator);
          v.allocateNew();
        ...
        for (PartitionChunkMetadata split : splitsInBatch) {
          ...
          for (PartitionValue partitionValue : split.getPartitionValues()) {
            ...
            // TODO (AH) handle invisible columns partitionColumnIdToTypeMap is built from row data
            // type which may or may not have $update column
            if (partitionColumnIdToTypeMap.containsKey(columnIndex)) {
              ...
              writePartitionValue(
                  ...
                  splitsLoaded,
                  partitionValue,
                  partitionColumnIdToTypeMap.get(columnIndex),
                  allocator);
            }
          }
        ...
        logger.debug(
            "Elapsed time to populate partitioning column vectors: {} ms within batchIndex: {}",
            miscTimer.elapsed(TimeUnit.MILLISECONDS),
            batchIndex);
        miscTimer.reset();
        ...
        if (batchIndex == 0) {
          ...
          if (materializedExpr == null) {
            throw new IllegalStateException(
                "Unable to materialize prune expression: " + pruneCondition.toString());
          }
        }
        ...
        InterpreterEvaluator.evaluate(
            splitsLoaded, optimizerContext, container, output, materializedExpr);
        logger.debug(
        ...
        logger.debug(
            "Within batch {}: total records: {}, qualified records: {}",
            batchIndex,
            recordCount,
            qualifiedCount);
        batchIndex++;
  ...
  public void doOnMatch(
      RelOptRuleCall call, Filter filterRel, Project projectRel, T scanRel, Boolean hasSampleRel) {
    ...
    try {
      ...
      RewriteAsBinaryOperators visitor =
          new RewriteAsBinaryOperators(true, filterRel.getCluster().getRexBuilder());
      ...
      int index = 0;
      for (String column :
          scanRel.getTableMetadata().getReadDefinition().getPartitionColumnsList()) {
      ...
      FindPartitionConditions c =
          new FindPartitionConditions(columnBitset, filterRel.getCluster().getRexBuilder());
      c.analyze(condition);
      RexNode pruneCondition = c.getFinalCondition();

      final long elapsed = miscTimer.elapsed(TimeUnit.MILLISECONDS);
      logger.debug("Total elapsed time to build and analyze filter tree: {} ms", elapsed);
      miscTimer.reset();
      ...
      boolean sargPruned =
          doSargPruning(filterRel, pruneCondition, scanRel, fieldMap, dataset, outputCondition);
      stopwatch.stop();
      logger.debug(
          "Partition pruning using search index took {} ms",
      ...
      if (!outputCondition.value.isAlwaysTrue()) {
        ...
        evalPruned =
            doEvalPruning(
                filterRel,
                fieldNameMap,
                partitionColumnsToIdMap,
                partitionColumnBitSet,
                dataset.value,
                settings,
                outputCondition.value,
                scanRel,
        ...
        logger.debug(
            "Partition pruning using expression evaluation took {} ms",
            stopwatch.elapsed(TimeUnit.MILLISECONDS));
      } else {
      ...
      final Set<String> prunedConjunctsStr =
          FluentIterable.from(pruneConjuncts)
              .transform(
      ...
      final RexNode newCondition =
          RexUtil.composeConjunction(filterRel.getCluster().getRexBuilder(), conjuncts, false);
      ...
      if (newCondition.isAlwaysTrue()) {
        ...
        // TODO: pruning a filter should remove its conditions. For now, it doesn't so we'll avoid
        // creating a duplicate filter/scan if the scan didn't change.
        if (scanUnchangedAfterPruning) {
          return;
        }

        final RelNode newFilter =
            filterRel.copy(filterRel.getTraitSet(), Collections.singletonList(inputRel));
  ...
  private void writePartitionValue(
      ValueVector vv,
      int index,
      PartitionValue pv,
      MajorType majorType,
  ...
  private LogicalExpression materializePruneExpr(
      LogicalExpression pruneCondition,
      PlannerSettings settings,
      RelNode scanRel,
      VectorContainer container) {
    // materialize the expression
    container.buildSchema();
    return ExpressionTreeMaterializer.materializeAndCheckErrors(
        pruneCondition, container.getSchema(), optimizerContext.getFunctionRegistry());
  }
}
