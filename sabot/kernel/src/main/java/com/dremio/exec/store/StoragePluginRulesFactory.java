...
/**
 * Interface that must be implemented by classes used to get rules associated with a storage plugin.
 * Note that implementations must have a zero-argument constructor.
 */
public interface StoragePluginRulesFactory {

  /**
   * Get rules for this type of plugin. We only add rules for types once across instances of each
   * type.
   *
   ...
   */
  Set<RelOptRule> getRules(
      OptimizerRulesContext optimizerContext, PlannerPhase phase, StoragePluginId pluginId);
