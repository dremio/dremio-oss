...
/**
 *  This class is responsible for building {@link VectorContainer} to represent records in the "copy_job_history" table
...
public final class CopyJobHistoryTableRecordBuilder {
  ...
  private static final Logger logger = LoggerFactory.getLogger(CopyJobHistoryTableRecordBuilder.class);
   ...
   * @param info          The {@link CopyIntoErrorInfo} containing error information to include in the record.
   ...
   */
  public static VectorContainer buildVector(BufferAllocator allocator, long schemaVersion, CopyIntoErrorInfo info, long numSuccess,
                                            long numErrors) {
   ...
   */
  private static VectorContainer buildVectorForV1Schema(BufferAllocator allocator, Schema schema, CopyIntoErrorInfo info, long numSuccess,
                                                       long numErrors) {
    ...
    for (Types.NestedField col : schema.columns()) {
      ...
      switch (col.fieldId()) {
        ...
        case 6:
          vector = new VarCharVector(col.name(), allocator);
          ((VarCharVector) vector).allocateNew(1);
          ((VarCharVector) vector).set(0, new Text(CopyIntoErrorInfo.Util.getJson(info.getFormatOptions())));
          break;
        case 7:
          vector = new VarCharVector(col.name(), allocator);
          ((VarCharVector) vector).allocateNew(1);
    ...
    logger.debug("Created copy_job_history iceberg table records vector matching iceberg schema {}", schema);
    return container;
  }
