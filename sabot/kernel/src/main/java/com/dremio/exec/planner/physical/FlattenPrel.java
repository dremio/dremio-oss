...
public class FlattenPrel extends SinglePrel implements Prel {
  ...
  private static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(FlattenPrel.class);

  public static final LongValidator RESERVE =
      new PositiveLongValidator(
          "planner.op.flatten.reserve_bytes", Long.MAX_VALUE, DEFAULT_RESERVE);
  public static final LongValidator LIMIT =
      new PositiveLongValidator("planner.op.flatten.limit_bytes", Long.MAX_VALUE, DEFAULT_LIMIT);
  ...
  public PhysicalOperator getPhysicalOperator(PhysicalPlanCreator creator) throws IOException {
    ...
    SchemaPath column =
        (SchemaPath) getFlattenExpression(new ParseContext(PrelUtil.getSettings(getCluster())));
    ...
    return new FlattenPOP(creator.props(this, null, schema, RESERVE, LIMIT), childPOP, column);
  }
  ...
  protected LogicalExpression getFlattenExpression(ParseContext context) {
    return RexToExpr.toExpr(
        context, getInput().getRowType(), getCluster().getRexBuilder(), toFlatten);
  }
  ...
  private static class SchemaConverter extends AbstractSchemaConverter {
      ...
      if (field.getName().equals(column.getAsUnescapedPath())) {
        ...
        if (child.getFieldType().getType().equals(ArrowType.Null.INSTANCE)) {
          throw UserException.validationError()
              .message("Flatten does not support empty list.")
              .build(logger);
        }
        return new Field(
            field.getName(),
            new FieldType(child.isNullable(), child.getType(), child.getDictionary()),
            child.getChildren());
      }
      return field;
    }
    ...
    public Field visitGeneric(ArrowType type) {
      if (field.getName().equals(column.getAsUnescapedPath())) {
        throw UserException.validationError()
            .message(
                "You're trying to flatten a field that is not a list. The offending field is %s.",
                Describer.describe(field))
            .build(logger);
      }
