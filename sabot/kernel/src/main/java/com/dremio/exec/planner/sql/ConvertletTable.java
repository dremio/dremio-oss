...
public class ConvertletTable extends ReflectiveConvertletTable {
   ...
   */
  private static final SqlRexConvertlet DEFAULT_CONVERTLET =
      new SqlRexConvertlet() {
        ...
        public RexNode convertCall(SqlRexContext cx, SqlCall call) {
          ...
          final RelDataType returnType =
              cx.getRexBuilder().deriveReturnType(call.getOperator(), exprs);
          return cx.getRexBuilder().makeCall(returnType, call.getOperator(), exprs);
        }
      };

  public ConvertletTable(boolean ieee756DivideBehavior) {
    super();
    ...
    registerOp(
        ...
        new SqlRexConvertlet() {
          ...
          public RexNode convertCall(SqlRexContext cx, SqlCall call) {
            final RexCall e =
                (RexCall) ConvertletTable.this.convertCall(cx, call, call.getOperator());
    ...
    registerOp(
        ...
        new SqlRexConvertlet() {
          ...
          public RexNode convertCall(SqlRexContext cx, SqlCall call) {
            ...
            return rexBuilder.makeCall(
                SqlStdOperatorTable.DATETIME_PLUS, timestampNode, multiplyNode);
          }
        });
  ...
  private static List<RexNode> convertExpressionList(
      SqlRexContext cx, List<SqlNode> nodes, SqlOperandTypeChecker.Consistency consistency) {
  ...
  private static RelDataType consistentType(
      SqlRexContext cx, SqlOperandTypeChecker.Consistency consistency, List<RelDataType> types) {
    switch (consistency) {
      case COMPARE:
