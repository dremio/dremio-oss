...
public class ParserUtil {
  ...
  public static void validateParsedViewQuery(SqlNode viewQuery) throws UserException {
    if (viewQuery.isA(SqlKind.DML) || viewQuery.isA(SqlKind.DDL)) {
      throw UserException.unsupportedError()
          .message("Cannot create view on statement of this type")
          .buildSilently();
    }
  }
  ...
  public static void validateViewQuery(String viewQuery) throws UserException {
    ...
    try {
      ...
      // Don't catch real parser exceptions here. The purpose for this methodis only for catching
      // unsupported
      // query types from successful parsed statments. If there is areal parser error, the existing
      // flow
      // to run the query and get back job results will catch and handle the parse exception
      // correctly with the
