...
public class IcebergManifestWriterPrel extends WriterPrel {
  public static final TypeValidators.LongValidator RESERVE =
      new TypeValidators.PositiveLongValidator(
          "planner.op.manifest_writer.reserve_bytes", Long.MAX_VALUE, DEFAULT_RESERVE);
  public static final TypeValidators.LongValidator LIMIT =
      new TypeValidators.PositiveLongValidator(
          "planner.op.manifest_writer.limit_bytes", Long.MAX_VALUE, DEFAULT_LIMIT);
  ...
  public WriterPrel copy(RelTraitSet traitSet, List<RelNode> inputs) {
    return new IcebergManifestWriterPrel(
        getCluster(), traitSet, sole(inputs), getCreateTableEntry(), singleWriter);
  }
}
