...
public final class CopyFileHistoryTableRecordBuilder {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(CopyFileHistoryTableRecordBuilder.class);

  private CopyFileHistoryTableRecordBuilder() {}

  public static VectorContainer buildVector(
      BufferAllocator allocator, long schemaVersion, List<CopyIntoFileLoadInfo> errorInfos) {
  ...
  private static VectorContainer buildVectorForV1Schema(
      BufferAllocator allocator, Schema schema, List<CopyIntoFileLoadInfo> errorInfos) {
      ...
      switch (col.fieldId()) {
        case 1:
          ...
          IntStream.range(0, recordCount)
              .forEach(i -> ((TimeStampMilliVector) vector).setSafe(i, currentTimeMillis));
          break;
        case 2:
        ...
        case 3:
          ...
          IntStream.range(0, recordCount)
              .forEach(
                  i ->
                      ((VarCharVector) vector)
                          .setSafe(i, new Text(errorInfos.get(i).getFilePath())));
          break;
        case 4:
          vector = new VarCharVector(col.name(), allocator);
          ((VarCharVector) vector).allocateNew(recordCount);
          IntStream.range(0, recordCount)
              .forEach(
                  i ->
                      ((VarCharVector) vector)
                          .setSafe(i, new Text(errorInfos.get(i).getFileState().name())));
          break;
        case 5:
          ...
          IntStream.range(0, recordCount)
              .forEach(
                  i ->
                      ((BigIntVector) vector)
                          .setSafe(i, errorInfos.get(i).getRecordsLoadedCount()));
          break;
        case 6:
          vector = new BigIntVector(col.name(), allocator);
          ((BigIntVector) vector).allocateNew(recordCount);
          IntStream.range(0, recordCount)
              .forEach(
                  i ->
                      ((BigIntVector) vector)
                          .setSafe(i, errorInfos.get(i).getRecordsRejectedCount()));
          break;
        default:
          throw new UnsupportedOperationException(
    ...
    logger.debug(
        "Created copy_file_history iceberg table records vector matching iceberg schema {}",
        schema);
    return container;
  }
}
