...
/**
 * Rule that implements {@link org.apache.calcite.rel.core.Minus} using aggregate, join, filter and
 * project High-level steps: 1. Compute the count of each unique rows of every input 2. Left join
 * the results above one by one 3. After each left join, filter out the rows in which the values of
 * the right input are not NULL 4. After all joins, project only the fields in the original input
 * schema
 */
public class MinusToJoin {
  public static final RelOptRule RULE =
      new RelOptRule(RelOptHelper.any(Minus.class, RelNode.class), "MinusToJoin") {
        ...
        public void onMatch(RelOptRuleCall call) {
          ...
          RelNode newRel =
              new MinusToJoin(minus.getCluster().getRexBuilder()).rewrite(call::builder, minus);
  ...
  public RelNode rewrite(Supplier<RelBuilder> relBuilderSupplier, Minus minus) {
    ...
    RelBuilder relBuilder =
        relBuilderSupplier.get().push(transformToUnique.apply(minus.getInput(0)));
    ...
    IntStream.range(1, minus.getInputs().size())
        .forEach(
            minusInputIndex -> {
              ...
              relBuilder
                  // Apply the aggregation to each subsequent input (subtractors), and add it to the
                  // relBuilder
                  .push(transformToUnique.apply(minus.getInput(minusInputIndex)))
                  // Join two inputs for each field in the original schema
                  .join(
                      JoinRelType.LEFT,
                      buildCondition(minus, left, relBuilder.peek(), minusInputIndex))
  ...
  private RexNode buildCondition(Minus minus, RelNode left, RelNode right, int minusIndex) {
    ...
    Stream<RexNode> minusRex =
        IntStream.range(0, columnCount)
            .mapToObj(
                (columnIndex) -> {
                  RelDataType lColType =
                      left.getRowType().getFieldList().get(columnIndex).getType();
                  RelDataType rColType =
                      right.getRowType().getFieldList().get(columnIndex).getType();
    ...
    return (RexUtil.composeConjunction(
        rexBuilder, minusRex.collect(ImmutableList.toImmutableList()), false));
  }
  ...
  private Function<RelNode, RelNode> transformRowToUnique(Supplier<RelBuilder> relBuilderSupplier) {
    ...
    return (relNode) -> {
      ...
      return relBuilder
          .push(relNode)
          .aggregate(relBuilder.groupKey(relBuilder.fields()), relBuilder.countStar(null))
          .build();
    };
  }
