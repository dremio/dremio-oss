...
public class IncrementalRefreshByPartitionWriterPrule extends WriterPrule {
  ...
  public IncrementalRefreshByPartitionWriterPrule() {
    super(RelOptHelper.some(IncrementalRefreshByPartitionWriterRel.class, Rel.LOGICAL, RelOptHelper.any(RelNode.class)),
  ...
  protected Prel convertWriter(final WriterRel writer, final RelNode rel) {
    ...
    final IncrementalRefreshByPartitionWriterRel incrementalRefreshByPartitionWriterRel = (IncrementalRefreshByPartitionWriterRel)writer;
    if(incrementalRefreshByPartitionWriterRel.getOldReflection() != null) {
      ...
      //Otherwise no changes to this function, and we will call convertWriter with null for finalize and datasetConfig as before
      //We will populate arguments finalize and datasetConfig and pass them down to the call to convertWriter
      //This will result in delete sub-plan being added to the plan
      //The delete plan will remove all files belonging to partitions
      //that are modified between the two snapshots in incrementalRefreshByPartitionWriterRel.getSnapshotDiffContext()
      final IncrementalRefreshByPartitionDeletePlanGenerator incrementalRefreshByPartitionDeletePlanGenerator = new IncrementalRefreshByPartitionDeletePlanGenerator(
    ...
    return convertWriter(writer, rel, writer.getExpectedInboundRowType(), datasetConfig, writer.getCreateTableEntry(), finalize);
  }
}
