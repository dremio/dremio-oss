...
public class ExcelFormatPlugin extends EasyFormatPlugin<ExcelFormatPluginConfig> {
  private static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(ExcelFormatPlugin.class);
  ...
  public RecordReader getRecordReader(
    ...
    return new ExcelRecordReader(context, dfs, path, excelFormatConfig, columns);
  }
    ...
    try {
      ...
      if (excelFileSize > maxExcelFileSize) {
        final String errorMessage =
            String.format(
                "File %s exceeds maximum size limit for Excel files of %d bytes",
                path, maxExcelFileSize);
        throw UserException.unsupportedError().message(errorMessage).build(logger);
      }
    } catch (IOException e) {
  ...
  public int getMaxFilesLimit() {
    return Math.toIntExact(
        getContext().getOptionManager().getOption(FileDatasetHandle.DFS_MAX_EXCEL_FILES));
  }
}
