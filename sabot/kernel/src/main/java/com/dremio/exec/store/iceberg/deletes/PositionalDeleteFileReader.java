...
/**
 * A PositionalDeleteFileReader wraps a RecordReader for an Iceberg positional delete file, and
 * exposes a sequence of PositionalDeleteIterators for all or a subset of the data files tracked in
 * the delete file.
 *
 * <p>PositionalDeleteFileReader is reference counted. The caller must set the refcount - either via
 * the constructor or by explicit calls to retain() - to be equal to the number of iterators it
 * expects to create via calls to createIteratorForDataFile(). The refcount is decremented when an
 * iterator is closed. When the refcount hits 0 the PositionalDeleteFileReader and child objects
 * such as the underlying RecordReader will be closed.
 */
public class PositionalDeleteFileReader implements AutoCloseable {
  ...
  public static final BatchSchema SCHEMA =
      new BatchSchema(
          ImmutableList.of(
  ...
  private ArrowBuf currentDataFilePathBuf;
  ...
  public PositionalDeleteFileReader(
      OperatorContext context, RecordReader reader, int initialRefCount) {
  ...
  public PositionalDeleteIterator createIteratorForDataFile(String dataFilePath) {
    ...
    Preconditions.checkArgument(
        currentDataFilePath == null || currentDataFilePath.compareTo(dataFilePath) < 0,
  ...
  private void getNextRecordBatch() {
    ...
    records = reader.next();
    // check if the current batch has rows only for data files less than the current data file we're
    // looking for,
    // and skip if so
  ...
  private void advanceToDataFile(String dataFilePath) {
    ...
    currentDataFilePathBuf = currentDataFilePathBuf.reallocIfNeeded(bytes.length);
    ...
    // find the first matching row for the current data file in the current batch - if no match is
    // found, we have
  ...
  private int findFirstRecordInBatchForCurrentDataFile() {
    // perform a binary search looking for the 1st record in the batch which matches the current
    // data file path
    int left = current;
    int right = records - 1;
    int cmp = 0;
  ...
  private int comparePathAtIndex(int index) {
    ...
    return ByteFunctionHelpers.compare(
        holder.buffer, holder.start, holder.end, currentDataFilePathBuf, 0, currentDataFilePathEnd);
  }
  ...
  /**
   * PositionalDeleteIterator implementation which will iterate over all the records for the current
   * data file. When done it will leave the reader at its current position so that a new data file
   * iteration may start from that point.
   */
