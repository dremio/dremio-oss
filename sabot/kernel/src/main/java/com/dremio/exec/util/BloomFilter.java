...
/**
 * Arrow compliant BloomFilter implementation. The filter expects a 16 byte buf as the key. <b>The
 * class is not thread safe while adding or checking memberships.</b>
 *
 * <p>The bloomfilter implementation uses MurMur3 as hashing algorithm. The same hash value is
 * reused for all keys See "Less Hashing, Same Performance: Building a Better Bloom Filter" by Adam
 * Kirsch and Michael Mitzenmacher. The paper argues that this trick doesn't significantly
 * deteriorate the performance of a Bloom filter (yet only needs two hash functions).
 */
@NotThreadSafe
public class BloomFilter implements AutoCloseable {
  ...
  /**
   * Initialise. The dataBuffer memory is used for keeping the bloomfilter bits. The num of hash
   * functions are automatically computed from the datasize assuming a FPP of 0.01.
   *
   * @param bufferAllocator
   * @param name
   * @param minSizeBytes Min size for the filter. Actual size will be the buffer capacity
   *     considering the rounding policy used by the allocator.
   */
  ...
  private BloomFilter(ArrowBuf dataBuffer) {
    ...
    this.numBitsSetLoc =
        dataBuffer.memoryAddress() + sizeInBytes + META_BYTES_CNT - 8; // last 8 bytes in buffer
  }
  ...
  public void setup() {
    ...
    System.arraycopy(
        ...
        Math.max(0, nameBytesAll.length - 24),
        metaBytes,
        0,
        Math.min(24, nameBytesAll.length));
  ...
  /**
   * Deserialization helper, that prepares the bloomfilter from the input stream. IMPORTANT:
   * BloolFilter::setup should not be called on this filter object.
   *
  ...
  /**
   * Returns the data buffer, which has the bloomfilter bits set. The buffer is released when
   * BloomFilter::close is invoked.
   *
   * @return
   */
   ...
   * @param bloomFilterKey Arrowbuf containg the key, assuming key starts at index zero
   * @param length length of the key within arrowbuf
   * @return Returns true if it caused a change in the state. True return value guarantees that key
  ...
  /**
   * Returns the probability that {@linkplain #mightContain(ArrowBuf, int)} will erroneously return
   * {@code true} for an object that has not actually been put in the {@code BloomFilter}.
   *
   * <p>FPP increases exponentially when the number of distinct keys in the bloomfilter increase
   * linearly.
   *
   * @return
   */
  ...
  /**
   * Maximum distinct keys, that can be inserted, to stay below MAX_FPP error probability. If more
   * keys are inserted, the error rate might go beyond permissible limit.
   *
   * @return
   */
  ...
  public static long getOptimalSize(long expectedInsertions) {
    ...
    long optimalSize =
        (long) (-expectedInsertions * Math.log(FPP) / (Math.log(2) * Math.log(2))) / 8;
    optimalSize = ((optimalSize + 8) / 8) * 8; // next multiple of 8
    return optimalSize + META_BYTES_CNT;
  }

  /**
   * Merges the bits from another bloomfilter into this one. This operation will work only if both
   * filters are compatible for merge.
   *
   * <p>The filter passed in the parameter will be accessed in read only mode.
   *
   * @param that
   */
  public void merge(BloomFilter that) {
    ...
    checkArgument(
        this.sizeInBits == that.sizeInBits,
        "Incompatible BloomFilter, different sizes (%s, %s).",
        this.sizeInBytes,
        that.sizeInBytes);
    ...
    for (long bytePos = 0; bytePos < sizeInBytes; bytePos += 8) {
      ...
      numBitsSet +=
          (Long.bitCount(mergedBits) - Long.bitCount(thisBits)); // add newly set bits to the count.
    }
    setNumBitsSet(numBitsSet);
  }
  ...
  public BloomFilter createCopy(BufferAllocator allocator) {
    try (AutoCloseables.RollbackCloseable rollbackCloseable =
      ...
      PlatformDependent.copyMemory(
          pColFilterData.memoryAddress(),
          pColFilterDataCopy.memoryAddress(),
          pColFilterData.capacity());
  ...
  public void close() {
    logger.debug(
        "Closing bloomfilter {}'s data buffer. RefCount {}", this.name, dataBuffer.refCnt());
    try {
      dataBuffer.close();
    } catch (Exception e) {
