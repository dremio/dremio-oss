...
public class FilterJoinRulesUtil {
  /**
   * Predicate that always returns true for any filter in OUTER join, and only true for EQUAL or
   * IS_NOT_DISTINCT_FROM over RexInputRef in INNER join. With this predicate, the filter expression
   * that return true will be kept in the JOIN OP. Example: INNER JOIN, L.C1 = R.C2 and L.C3 + 100 =
   * R.C4 + 100 will be kepted in JOIN. L.C5 < R.C6 will be pulled up into Filter above JOIN. OUTER
   * JOIN, Keep any filter in JOIN.
   */
  public static final FilterJoinRule.Predicate EQUAL_IS_NOT_DISTINCT_FROM =
      (join, joinType, exp) -> {
        ...
        try {
          ...
          // if the remaining condition can be pushed down (i.e. a = 1), we should not push the
          // condition above
          ...
          // if the conditions list is empty, that means we were able to push the condition below
          // the join
