...
public class UserDefinedFunctionServiceImpl implements UserDefinedFunctionService {
  ...
  public Iterable<FunctionInfo> functionInfos() {
    if (isMaster ||
      ...
      return namespaceServiceProvider.get().getFunctions().stream().map(
        functionConfig -> {
          FunctionInfo functionInfo = UserDefinedFunctionService.getFunctionInfoFromConfig(functionConfig);
    ...
    try {
      ...
      return udfInfoResp.getFunctionInfoList().stream()
        .map(UserDefinedFunctionService.FunctionInfo::fromProto).collect(Collectors.toList());
