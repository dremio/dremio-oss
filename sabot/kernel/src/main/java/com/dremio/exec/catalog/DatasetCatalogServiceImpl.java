 ...
 */
public class DatasetCatalogServiceImpl
    extends DatasetCatalogServiceGrpc.DatasetCatalogServiceImplBase {
  private static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(DatasetCatalogServiceImpl.class);
  ...
  public DatasetCatalogServiceImpl(
   ...
   *
   * <p>Calls to this method should be preceded with a call to getDataset() to either retrieve the
   * previous version or determine that there is no existing configuration for the dataset.
   *
   * <p>If there is an existing version of the dataset, the incoming request's operation type must
   * be set to UPDATE and the tag of the previous version must be supplied to the request.
   *
   * <p>If there is not an existing version of the dataest, the incoming request's operation type
   * must be set to CREATE.
   *
   * <p>Exceptions cannot escape this method, however the supplied StreamObserver can return gRPC
   * Status.ABORTED to indicate the write failed due to a ConcurrentModificationException.
   */
  @Override
  public void addOrUpdateDataset(
      AddOrUpdateDatasetRequest request, StreamObserver<Empty> responseObserver) {
    ...
    try {
      ...
      final NamespaceService namespaceService =
          namespaceServiceFactoryProvider.get().get(SystemUser.SYSTEM_USERNAME);
      ...
      if (request.getOperationType() == OperationType.UPDATE && !datasetExists) {
        // Note that if the caller used an UPDATE operation, and the dataset does not exist, and if
        // they previously called
        // getDataset(), this would imply the dataset previously existed but is now gone (in other
        // words, it was deleted).
        // We throw a CME here to indicate that the caller needs to retry to read-update-write
        // sequence again.
        ...
        // Similarly, if the caller used a CREATE operation, and the dataset does exisst, and the
        // caller previously called
        // getDataset()l it implies that the caller previously found that the dataset didn't yet
        // exist but does now (in
      ...
      // There are 2 paths.
      // 1. The dataset already exists and the request is updating the read definition and/or the
      // schema.
      // 2. The dataset is new.
      // Short-circuit if the request is just updating the schema on an existing dataset.
      if (!datasetExists) {
        Preconditions.checkArgument(
            request.getDatasetConfig().hasBatchSchema(),
            "BatchSchema must be supplied for new datasets.");
        Preconditions.checkArgument(
            request.getDatasetConfig().hasDatasetType(),
            "DatasetType must be supplied for new datasets.");
      ...
      if (request.getDatasetConfig().hasFileFormat()) {
        fileConfig =
            toProtoStuff(request.getDatasetPathList(), request.getDatasetConfig().getFileFormat());
      }

      if (!datasetExists
          || config.getPhysicalDataset() == null
          || config.getPhysicalDataset().getFormatSettings() == null
          || !Boolean.valueOf(request.getDatasetConfig().getIcebergMetadataEnabled())
              .equals(config.getPhysicalDataset().getIcebergMetadataEnabled())) {
        final PhysicalDataset physicalDataset = new PhysicalDataset();
        if (fileConfig != null) {
          physicalDataset.setFormatSettings(fileConfig);
        }
        physicalDataset.setIcebergMetadataEnabled(
            Boolean.TRUE.equals(request.getDatasetConfig().getIcebergMetadataEnabled()));
        config.setPhysicalDataset(physicalDataset);
        config.setType(
            DatasetType.valueOf(request.getDatasetConfig().getDatasetType().getNumber()));
        config.setFullPathList(request.getDatasetPathList());
      }

      if (request.getDatasetConfig().hasReadDefinition()) {
        config.setReadDefinition(
            populateReadDefinitionFromProtoBuf(
                request.getDatasetConfig().getReadDefinition(), resultReadDefinition));
      }

      if (request.getDatasetConfig().hasBatchSchema()) {
        config.setRecordSchema(
            ByteStringUtil.wrap(request.getDatasetConfig().getBatchSchema().toByteArray()));
      }
      ...
      if (request.getDatasetConfig().getIcebergMetadataEnabled()) {
        Preconditions.checkState(
            request.getDatasetConfig().hasIcebergMetadata(), "Unexpected state");
        ...
        DatasetCommonProtobuf.IcebergMetadata metadata =
            request.getDatasetConfig().getIcebergMetadata();
        ...
        icebergMetadata.setPartitionSpecsJsonMap(
            ByteStringUtil.wrap(metadata.getPartitionSpecsJsonMap().toByteArray()));
      ...
      responseObserver.onError(
          Status.ABORTED.withCause(e).withDescription(e.getMessage()).asException());
      ...
      switch (e.getErrorType()) {
        ...
        default:
          responseObserver.onError(
              Status.UNKNOWN.withCause(e).withDescription(e.getMessage()).asException());
      }
    } catch (Exception e) {
  ...
  private void saveDataset(
      ...
      UpdatableDatasetConfigFields datasetConfigFields,
    ...
    final PartitionProtobuf.PartitionChunk partitionChunkProto =
        datasetConfigFields.getPartitionChunk();
    final PartitionProtobuf.DatasetSplit splitProto = partitionChunkProto.getDatasetSplit();

    final List<PartitionValue> partition =
        partitionChunkProto.getPartitionValuesList().stream()
            .map(MetadataProtoUtils::fromProtobuf)
            .collect(Collectors.toList());
    final List<DatasetSplitAffinity> affinities =
        splitProto.getAffinitiesList().stream()
            .map(a -> DatasetSplitAffinity.of(a.getHost(), a.getFactor()))
            .collect(Collectors.toList());
    final DatasetSplit datasetSplit =
        DatasetSplit.of(
            affinities,
            splitProto.getSize(),
            splitProto.getRecordCount(),
    ...
    final DatasetMetadataSaver saver =
        namespaceService.newDatasetMetadataSaver(
            namespaceKey, config.getId(), NamespaceService.SplitCompression.SNAPPY, 500L, false);
   ...
   *
   * <p>Exceptions cannot be thrown from this method, however a Grpc Status.NOT_FOUND exception will
   * be written to the StreamObserver if the dataset requested does not exist.
   *
   * <p>The intent of this method is to supply metadata that can be modified and sent back in a call
   * to {@link #addOrUpdateDataset(AddOrUpdateDatasetRequest, StreamObserver)}.
   *
   * <p>If this method returned a NOT_FOUND exception, a subsequent call to {@link
   * #addOrUpdateDataset(AddOrUpdateDatasetRequest, StreamObserver)} should be sent with the CREATE
   * operation type. If this method successfully returned a dataset, then a subsequent call to
   * {@link #addOrUpdateDataset(AddOrUpdateDatasetRequest, StreamObserver)} should be sent with the
   * UPDATE operation type and use the tag that was returned.
   */
  @Override
  public void getDataset(
      GetDatasetRequest request, StreamObserver<UpdatableDatasetConfigFields> responseObserver) {
    logger.debug("Request received: {}", request);
    try {
      Preconditions.checkArgument(!request.getDatasetPathList().isEmpty());
      final DatasetConfig config =
          namespaceServiceFactoryProvider
              ...
              .getDataset(new NamespaceKey(request.getDatasetPathList()));

      final UpdatableDatasetConfigFields.Builder resultBuilder =
          UpdatableDatasetConfigFields.newBuilder()
              .setDatasetType(
                  DatasetCommonProtobuf.DatasetType.forNumber(config.getType().getNumber()))
              .setBatchSchema(
                  UnsafeByteOperations.unsafeWrap(config.getRecordSchema().asReadOnlyByteBuffer()))
              .setReadDefinition(toProtoBuf(config.getReadDefinition()))
              .setTag(config.getTag());

      if (config.getPhysicalDataset() != null
          && config.getPhysicalDataset().getFormatSettings() != null) {
  ...
  private static FileConfig toProtoStuff(
      List<String> datasetPath, FileProtobuf.FileConfig newFileConfig) {
    ...
    if (newFileConfig.hasExtendedConfig()) {
      fileConfig.setExtendedConfig(
          ByteStringUtil.wrap(newFileConfig.getExtendedConfig().toByteArray()));
    }
  ...
  private ReadDefinition populateReadDefinitionFromProtoBuf(
      DatasetCommonProtobuf.ReadDefinition newReadDefinition, ReadDefinition outputReadDefinition) {
    if (newReadDefinition.hasReadSignature()) {
      outputReadDefinition.setReadSignature(
          ByteStringUtil.wrap(newReadDefinition.getReadSignature().toByteArray()));
    }

    if (newReadDefinition.hasExtendedProperty()) {
      outputReadDefinition.setExtendedProperty(
          ByteStringUtil.wrap(newReadDefinition.getExtendedProperty().toByteArray()));
    }

    if (newReadDefinition.hasManifestScanStats()) {
      outputReadDefinition.setManifestScanStats(
          toProtoStuff(newReadDefinition.getManifestScanStats()));
    }
  ...
  private static FileProtobuf.FileConfig toProtoBuf(FileConfig fileConfig) {
    final FileProtobuf.FileConfig.Builder newFileConfigBuilder =
        FileProtobuf.FileConfig.newBuilder();
    ...
    if (fileConfig.getExtendedConfig() != null && !fileConfig.getExtendedConfig().isEmpty()) {
      newFileConfigBuilder.setExtendedConfig(
          UnsafeByteOperations.unsafeWrap(fileConfig.getExtendedConfig().asReadOnlyByteBuffer()));
    }
  ...
  private DatasetCommonProtobuf.ReadDefinition toProtoBuf(ReadDefinition readDefinition) {
    final DatasetCommonProtobuf.ReadDefinition.Builder builder =
        DatasetCommonProtobuf.ReadDefinition.newBuilder();

    if (readDefinition.getReadSignature() != null) {
      builder.setReadSignature(
          UnsafeByteOperations.unsafeWrap(
              readDefinition.getReadSignature().asReadOnlyByteBuffer()));
    }

    if (readDefinition.getExtendedProperty() != null) {
      builder.setExtendedProperty(
          UnsafeByteOperations.unsafeWrap(readDefinition.getExtendedProperty().toByteArray()));
    }
  ...
  private static DatasetCommonProtobuf.ScanStats toProtoBuf(ScanStats scanStats) {
    final DatasetCommonProtobuf.ScanStats.Builder builder =
        DatasetCommonProtobuf.ScanStats.newBuilder();

    if (scanStats.getType() != null) {
      builder.setType(
          DatasetCommonProtobuf.ScanStatsType.forNumber(scanStats.getType().getNumber()));
    }
