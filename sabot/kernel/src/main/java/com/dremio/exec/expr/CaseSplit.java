 ...
 *
 * <p>startIndex is inclusive and endIndex is exclusive. An endIndex of case expressions condition
 * list size indicates the else expression. From a vertical slicing perspective, both "then" and
 * "else" expressions are on the same vertical side, while "when" expressions are on the other side
 * of the vertical.
 */
public class CaseSplit {
  private static final CaseSplit PREFERRED_ZERO_SPLIT =
      new CaseSplit(CaseSplit.CaseSplitType.PREFERRED, false);
  private static final CaseSplit NON_PREFERRED_ZERO_SPLIT =
      new CaseSplit(CaseSplit.CaseSplitType.NON_PREFERRED, false);
  private static final CaseSplit PREFERRED_ZERO_SPLIT_FLIPPED =
      new CaseSplit(CaseSplit.CaseSplitType.PREFERRED, true);
  ...
  public CaseSplit(
      int start, int end, int origSz, boolean when, CaseSplitType splitType, boolean flipped) {
  ...
  public static CaseSplit getZeroSplitFlipped(CaseSplitType splitType) {
    return splitType == CaseSplitType.PREFERRED
        ? PREFERRED_ZERO_SPLIT_FLIPPED
        : NON_PREFERRED_ZERO_SPLIT_FLIPPED;
  }
  ...
  /**
   * Get a constant int expression wrapped with a codegen context annotated with engine type based
   * on this split type.
   *
  ...
  public CodeGenContext getIntExpression(int value) {
    ...
    CodeGenContext context =
        CodeGenContext.buildWithNoDefaultSupport(ValueExpressions.getInt(value));
   ...
   *
   * <p>If the end index does not correspond to the size of the original expression, then this
   * "when" split is a continuation and so the else expression should return a -1, otherwise it
   * should return a positive integer that corresponds to the index of the actual "else" expression.
   *
  ...
  public CodeGenContext getEqualExpression(CodeGenContext leftExpr, int right) {
    ...
    final CodeGenContext context =
        CodeGenContext.buildWithNoDefaultSupport(CaseFunctions.getInstance().getEqFnExpr(args));
    addEngineToContext(context);
    return context;
  }
   ...
   *
   * @param prevSplit Previous expression split from which we can read the result of previous "when"
   *     split
   * @return a codegen context that wraps a function expression which acts as a condition to skip
   *     this "when" split
   */
  public CodeGenContext getWhenSkipperExpression(ExpressionSplit prevSplit) {
    ...
    final List<LogicalExpression> args = Arrays.asList(leftExpr, rightExpr);
    final CodeGenContext context =
        CodeGenContext.buildWithNoDefaultSupport(CaseFunctions.getInstance().getGtFnExpr(args));
    addEngineToContext(context);
    return context;
  }
  ...
  public CodeGenContext getModifiedExpressionForSplit(
      CodeGenContext expr, ExpressionSplit prevSplit) {
    if (prevSplit == null) {
      // no wrapper required as there is no prev when split
      return expr;
    }
    final CodeGenContext nullExprContext =
        new CodeGenContext(new TypedNullConstant(expr.getCompleteType()));
    if (partOfWhen) {
      // this is a split happening inside a "when" expression of case, so this split should be
      // changed
      ...
      // this is a split happening inside a "then" expression of case, so this split should be
      // changed
  ...
  public void addEngineToContextMixed(CodeGenContext splitWhenExpr, CodeGenContext splitContext) {
    if (this.splitType.equals(CaseSplitType.MIXED)) {
      final SupportedEngines.Engine p =
          (flipped) ? SupportedEngines.Engine.JAVA : SupportedEngines.Engine.GANDIVA;
      final SupportedEngines.Engine np =
          (flipped) ? SupportedEngines.Engine.GANDIVA : SupportedEngines.Engine.JAVA;
  ...
  private CodeGenContext getWhenExpressionWrapperFunction(
      CodeGenContext whenExpr, ExpressionSplit prevSplit) {
    assert !zeroSplit;
    final LogicalExpression leftExpr = prevSplit.getReadExpressionContext();
    final LogicalExpression rightExpr = getIntExpression(0);
    final List<LogicalExpression> args = Arrays.asList(leftExpr, rightExpr);
    return CodeGenContext.buildWithCurrentCodegen(
        whenExpr, CaseFunctions.getInstance().getLtFnExpr(args));
  }

  private CodeGenContext getThenExpressionWrapperFunction(
      CodeGenContext whenExpr, ExpressionSplit vwSplit) {
    assert !zeroSplit;
    ...
    final List<LogicalExpression> args = Arrays.asList(leftExpr, rightExpr);
    return CodeGenContext.buildWithCurrentCodegen(
        whenExpr, CaseFunctions.getInstance().getEqFnExpr(args));
  }

  private void addEngineToContext(CodeGenContext context) {
    switch (this.splitType) {
      case PREFERRED:
        SupportedEngines.Engine preferred =
            (flipped) ? SupportedEngines.Engine.JAVA : SupportedEngines.Engine.GANDIVA;
