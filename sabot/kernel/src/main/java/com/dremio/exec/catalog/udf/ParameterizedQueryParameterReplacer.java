...
public final class ParameterizedQueryParameterReplacer {
  public static RelNode replaceParameters(
    ...
    return functionPlan.accept(createRelParameterReplacer(functionParameters, values, rexBuilder));
  }

  public static RelShuttle createRelParameterReplacer(
      List<FunctionParameter> functionParameters, List<RexNode> values, RexBuilder rexBuilder) {
    return new RelArgumentReplacer(
        (RexArgumentReplacer) createRexParameterReplacer(functionParameters, values, rexBuilder));
  }

  public static RexShuttle createRexParameterReplacer(
      List<FunctionParameter> functionParameters, List<RexNode> values, RexBuilder rexBuilder) {
    Map<String, RexNode> replacementMapping =
        createReplacementMapping(functionParameters, values, rexBuilder);
  ...
  public static RexNode replaceParameters(
    ...
    return functionExpression.accept(
        createRexParameterReplacer(functionParameters, values, rexBuilder));
  }
  ...
  private static Map<String, RexNode> createReplacementMapping(
    ...
    for (int i = 0; i < functionParameters.size(); i++) {
      ...
      if ((valueType.getSqlTypeName() != SqlTypeName.ANY) && (valueType != parameterType)) {
        ...
        // It's probably a function like CONVERT_FROM that type validation bug that needs to be
        // fixed.
  ...
  //  then we could call into convertSelect(...) with the mapping, but for now we will manually
  // replace the UDF arguments.
  private static final class RelArgumentReplacer extends StatelessRelShuttleImpl {
    ...
    public RelNode visit(LogicalProject project) {
      ...
      boolean rewriteHappened =
          (rewrittenInput != project.getInput()) || (rewrittenProject != project);
      if (!rewriteHappened) {
        return project;
      }

      return rewrittenProject.copy(
          rewrittenProject.getTraitSet(), ImmutableList.of(rewrittenInput));
    }
    ...
    public RelNode visit(LogicalFilter filter) {
      ...
      boolean rewriteHappened =
          (rewrittenInput != filter.getInput()) || (rewrittenFilter != filter);
      if (!rewriteHappened) {
        return filter;
      }

      return rewrittenFilter.copy(
          rewrittenFilter.getTraitSet(), rewrittenInput, rewrittenFilter.getCondition());
    }
  }
