...
/**
 * {@link RecordReader} implementation for reading Arrow format files. Currently this reader can
 * only read files written by writer {@link ArrowRecordWriter}.
 */
public class ArrowRecordReader extends AbstractRecordReader {
  ...
  private Map<Integer, ValueVector> vectors =
      Maps.newHashMap(); // map of column index and output vector
  ...
  public void setup(OutputMutator output) throws ExecutionSetupException {
    try {
      ...
      // From the footer, construct selected column field type info and vectors. Also add the types
      // to output mutator.
      int i = 0;
      if (!getColumns().isEmpty()) {
        final FieldSelection select =
            FieldSelection.getFieldSelection(Lists.newArrayList(getColumns()));
        for (final SerializedField field : footer.getFieldList()) {
          ...
          if (!select.getChild(f.getName()).isNeverValid()) {
            ...
            // 2. When the projected field is a child of current field (possible when the field is a
            // map).
            vectors.put(
                i,
                (output.addField(
                    f, (Class<? extends ValueVector>) TypeHelper.getValueVectorClass(f))));
          }
          i++;
        }
      }
  ...
  public int next() {
    ...
    try {
      ...
      final UserBitShared.RecordBatchDef batchDef =
          UserBitShared.RecordBatchDef.parseDelimitedFrom(inputStream);
      ...
      BatchSchema batchSchema =
          BatchSchema.newBuilder().addSerializedFields(fieldListFromBatch).build();
