...
public class VersionedDatasetAdapterFactory {
  ...
  public VersionedDatasetAdapter newInstance(List<String> versionedTableKey,
    ...
    return tryGetDatasetHandle(
      storagePlugin,
      versionedDatasetConfig,
      versionContext).map(datasetHandle ->
      newInstance(
        versionedTableKey,
        versionContext,
        storagePlugin,
        ...
        optionManager,
        datasetHandle,
        versionedDatasetConfig)).orElse(null);
  }

  protected VersionedDatasetAdapter newInstance(List<String> versionedTableKey,
                                                ResolvedVersionContext versionContext,
                                                StoragePlugin storagePlugin,
                                                StoragePluginId storagePluginId,
                                                OptionManager optionManager,
  ...
  /**
   * This is a helper method that creates a shell datasetConfig that populates the format so the FileSystem plugin
   * knows what format we will be using to access the Iceberg tables.
   *
   * @param versionedTableKey Namespace key
   * @return DatasetConfig populated with the basic info needed by the FileSsystem plugin to match and unwrap to
   ...
   */
  private Optional<DatasetHandle> tryGetDatasetHandle(StoragePlugin storagePlugin,
                                                      DatasetConfig versionedDatasetConfig,
                                                      ResolvedVersionContext versionContext) {
