...
class SetUdfLocation implements UpdateLocation {
  private final UdfOperations ops;
  private String newLocation = null;

  SetUdfLocation(UdfOperations ops) {
    this.ops = ops;
  }

  @Override
  public String apply() {
    Preconditions.checkState(null != newLocation, "Invalid UDF location: null");
    return newLocation;
  }

  @Override
  public void commit() {
    UdfMetadata base = ops.refresh();
    Tasks.foreach(ops)
        .retry(
        ...
        .run(
            taskOps ->
                taskOps.commit(base, UdfMetadata.buildFrom(base).setLocation(apply()).build()));
  }
