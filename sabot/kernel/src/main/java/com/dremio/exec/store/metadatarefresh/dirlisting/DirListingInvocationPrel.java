...
public class DirListingInvocationPrel extends ScanPrelBase implements Prel, PrelFinalizable, Rel {
  ...
  public Prel finalizeRel() {
    InternalIcebergScanTableMetadata icebergScanTableMetadata =
        new InternalIcebergScanTableMetadata(tableMetadata, metaStoragePlugin, tableUUID);
    Prel manifestListPrel = generateManifestListScanPrel(icebergScanTableMetadata);
    Prel manifestReadPrel =
        generateManifestFileReadScanPrel(manifestListPrel, icebergScanTableMetadata);

    Prel dirListingScanPrel =
        new DirListingScanPrel(
            getCluster(),
            traitSet,
            table,
            pluginId,
  ...
  private Prel generateManifestListScanPrel(TableMetadata icebergScanTableMetadata) {
    ...
    final RelDataType manifestListRowType =
    ...
    IcebergManifestListPrel manifestListPrel =
        new IcebergManifestListPrel(
            ...
            manifestListRowType,
            null,
            ManifestContentType.DATA); // TODO: check icebergExpression can be null or not
    return manifestListPrel;
  }

  private Prel generateManifestFileReadScanPrel(
      Prel manifestListPrel, TableMetadata icebergScanTableMetadata) {
    ...
    HashToRandomExchangePrel manifestSplitsExchange =
        new HashToRandomExchangePrel(
            ...
            TableFunctionUtil.getHashExchangeTableFunctionCreator(icebergScanTableMetadata, true));

    BatchSchema manifestFileReaderSchema =
        MetadataRefreshExecConstants.PathGeneratingDataFileProcessor.OUTPUT_SCHEMA.BATCH_SCHEMA;

    List<SchemaPath> manifestFileReaderColumns =
        manifestFileReaderSchema.getFields().stream()
            .map(x -> SchemaPath.getSimplePath(x.getName()))
            .collect(Collectors.toList());

    // Manifest scan phase
    TableFunctionConfig manifestScanTableFunctionConfig =
        TableFunctionUtil.getMetadataManifestScanTableFunctionConfig(
            icebergScanTableMetadata, manifestFileReaderColumns, manifestFileReaderSchema, null);

    final RelDataType outputRowType =
        getRowTypeFromProjectedColumns(
            manifestFileReaderColumns, manifestFileReaderSchema, getCluster());

    TableFunctionPrel manifestScanTF =
  ...
  private Prel addHashJoin(Prel dirListingScanPrel, Prel icebergScanPrel) {
    ...
    Pair<Integer, RelDataTypeField> leftSidePath =
        findFieldWithIndex(
            dirListingScanPrel, MetadataRefreshExecConstants.DirList.OUTPUT_SCHEMA.FILE_PATH);
    Pair<Integer, RelDataTypeField> rightSidePath =
        findFieldWithIndex(
            icebergScanPrel,
    ...
    RexNode joinCondition =
        rexBuilder.makeCall(
            ...
            rexBuilder.makeInputRef(
                rightSidePath.right.getType(),
                dirListingScanPrel.getRowType().getFieldCount() + rightSidePath.left));
  ...
  private Prel addFilterForAlreadyPresetFiles(Prel hashJoinPrel) {
    ...
    Pair<Integer, RelDataTypeField> dirListPath =
        findFieldWithIndex(
            hashJoinPrel, MetadataRefreshExecConstants.DirList.OUTPUT_SCHEMA.FILE_PATH);
    Pair<Integer, RelDataTypeField> icebergScanPath =
        findFieldWithIndex(hashJoinPrel, RecordReader.DATAFILE_PATH);

    RexNode dirListPathRef = rexBuilder.makeInputRef(dirListPath.right.getType(), dirListPath.left);
    RexNode icebergScanPathRef =
        rexBuilder.makeInputRef(icebergScanPath.right.getType(), icebergScanPath.left);

    RexNode isNullDirListPath = rexBuilder.makeCall(SqlStdOperatorTable.IS_NULL, dirListPathRef);
    RexNode isNullIcebergPath =
        rexBuilder.makeCall(SqlStdOperatorTable.IS_NULL, icebergScanPathRef);

    RexNode isNotEqualDirListAndIcebergPath =
        rexBuilder.makeCall(SqlStdOperatorTable.OR, isNullDirListPath, isNullIcebergPath);
    FilterPrel filterAlreadyExisting =
        FilterPrel.create(getCluster(), traitSet, hashJoinPrel, isNotEqualDirListAndIcebergPath);
  ...
  private Prel coaleseToOnePathProject(Prel filterAlreadyPresetFilesPrel) {
    ...
    Pair<Integer, RelDataTypeField> dirListPath =
        findFieldWithIndex(
            filterAlreadyPresetFilesPrel,
            MetadataRefreshExecConstants.DirList.OUTPUT_SCHEMA.FILE_PATH);
    Pair<Integer, RelDataTypeField> icebergScanPath =
        findFieldWithIndex(filterAlreadyPresetFilesPrel, RecordReader.DATAFILE_PATH);

    RexInputRef dirListPathRef =
        rexBuilder.makeInputRef(dirListPath.right.getType(), dirListPath.left);
    RexInputRef icebergScanPathRef =
        rexBuilder.makeInputRef(icebergScanPath.right.getType(), icebergScanPath.left);

    // dirListPath != null
    RexNode isNotNullDirListPath =
        rexBuilder.makeCall(SqlStdOperatorTable.IS_NOT_NULL, dirListPathRef);

    //  case(dataFilePath != null) then dataFilePath else dirListPath
    RexNode coalesce =
        rexBuilder.makeCall(
            SqlStdOperatorTable.CASE, isNotNullDirListPath, dirListPathRef, icebergScanPathRef);

    RexNode isDeleted = rexBuilder.makeCall(SqlStdOperatorTable.IS_NULL, dirListPathRef);

    Pair<Integer, RelDataTypeField> dirListPartition =
        findFieldWithIndex(
            filterAlreadyPresetFilesPrel,
            MetadataRefreshExecConstants.DirList.OUTPUT_SCHEMA.PARTITION_INFO);
    Pair<Integer, RelDataTypeField> icebergScanPartition =
        findFieldWithIndex(
            filterAlreadyPresetFilesPrel,
            MetadataRefreshExecConstants.PathGeneratingDataFileProcessor.OUTPUT_SCHEMA
                .PARTITION_DATA_PATH);

    RexInputRef dirListPartitionRef =
        rexBuilder.makeInputRef(dirListPartition.right.getType(), dirListPartition.left);
    RexInputRef icebergScanPartitionRef =
        rexBuilder.makeInputRef(icebergScanPartition.right.getType(), icebergScanPartition.left);

    RexNode isNotNullDirListPart =
        rexBuilder.makeCall(SqlStdOperatorTable.IS_NOT_NULL, dirListPartitionRef);

    RexNode combinePartitionInfo =
        rexBuilder.makeCall(
            SqlStdOperatorTable.CASE,
            isNotNullDirListPart,
            dirListPartitionRef,
            icebergScanPartitionRef);
        ...
        x -> {
          if (!ignoreRefs.contains(x.getName())) {
            ...
            Pair<Integer, RelDataTypeField> tempRef =
                findFieldWithIndex(filterAlreadyPresetFilesPrel, x.getName());
    ...
    ProjectPrel projectPrel =
        ProjectPrel.create(
            getCluster(),
            filterAlreadyPresetFilesPrel.getTraitSet(),
            filterAlreadyPresetFilesPrel,
            projectExpressions,
            newRowType);
  ...
  private Prel addFilterForFilteringPerPartition(Prel manifestScanTf) {
    RexBuilder rexBuilder = getCluster().getRexBuilder();
    Pair<Integer, RelDataTypeField> fieldPair =
        findFieldWithIndex(manifestScanTf, RecordReader.DATAFILE_PATH);
    ...
    RexNode isEqualPartitionPath =
        rexBuilder.makeCall(SqlStdOperatorTable.LIKE, inputRef, partialPath);

    // Only one level allowed. SubDir of path not allowed.
    RexNode subDirNotAllowed = rexBuilder.makeLiteral("%" + path + "/%/%");
    RexNode isEqualPartitionPathSubDir =
        rexBuilder.makeCall(SqlStdOperatorTable.LIKE, inputRef, subDirNotAllowed);
