 ...
 */
public class PreferenceBasedSplitter
    extends AbstractExprVisitor<
        CodeGenContext, Tuple<CodeGenContext, SplitDependencyTracker>, Exception> {
  private static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(PreferenceBasedSplitter.class);
  ...
  CodeGenContext visitCodeGenContext(CodeGenContext context, SplitDependencyTracker myTracker)
    ...
    return context
        .getChild()
        .accept(this, Tuple.<CodeGenContext, SplitDependencyTracker>of(context, myTracker));
  }

  @Override
  public CodeGenContext visitFunctionHolderExpression(
      FunctionHolderExpression holder, Tuple<CodeGenContext, SplitDependencyTracker> value)
      throws Exception {
    ...
    SupportedEngines executionEngine =
        splitter.getEffectiveNodeEvaluationType(parentEvalType, context);
    return splitFunctionExpression(holder, myTracker, executionEngine);
  }

  private CodeGenContext splitFunctionExpression(
    ...
    for (LogicalExpression arg : holder.args) {
      ...
      // When there is a split, the expression changes as the operator is replaced by the newly
      // created split
      SplitDependencyTracker argTracker =
          new SplitDependencyTracker(
              executionEngine, myTracker.getIfExprBranches(), myTracker.getCaseBlocks());
      ...
      logger.trace(
          "Splitting at arg {} of function {}, expression {}", i, holder.getName(), newArg);
      ExpressionSplit split =
          splitter.splitAndGenerateVectorReadExpression(newArg, myTracker, argTracker);
  ...
  boolean canExecuteInAllCodeGenerators(LogicalExpression expr) {
    ...
    return (expr instanceof CodeGenContext
            && ((CodeGenContext) expr).getChild() instanceof ValueExpressions.BooleanExpression)
        || (expr instanceof ValueExpressions.BooleanExpression);
  }

  @Override
  public CodeGenContext visitIfExpression(
      IfExpression ifExpr, Tuple<CodeGenContext, SplitDependencyTracker> value) throws Exception {
    CodeGenContext context = value.first;
    SplitDependencyTracker myTracker = value.second;
    if (context.isSubExpressionExecutableInEngine(this.preferredEngine)) {
    ...
    // Need to split either condExpr, thenExpr or elseExpr - dont know which one at this point of
    // time
    // Consider cases where thenExpr or elseExpr contain if expressions. Examples:
    // 1) nested-if: if (c1) then (if (c2) then (t1) else (e1) end) else (if (c3) then (t2) else
    // (e2) end) end
    // 2) arg-if: if (c1) then (t1 + (if (c2) then (t2) else (e2) end)) else (e2) end
    //
    // First, split the condition. Let this output be in c1
    // For splits in the then path, any split (s) has to be modified as if (c1) then (s) else
    // (internal null)
    // For splits in the else path, any split (s) has to be modified as if (c1) then (internal null)
    // else (s)
    //
    // The splits in either path (then or else) are done later. Pass the condition and context in
    // the tracker
    ...
    SplitDependencyTracker condTracker =
        new SplitDependencyTracker(ifEvalType, myPath, myTracker.getCaseBlocks());
    CodeGenContext condExprContext =
        visitCodeGenContext((CodeGenContext) ifExpr.ifCondition.condition, condTracker);

    // Split the condition and remember the ValueVectorReadExpression for the condition
    ExpressionSplit condSplit =
        splitter.splitAndGenerateVectorReadExpression(condExprContext, myTracker, condTracker);
    ...
    SplitDependencyTracker thenTracker =
        new SplitDependencyTracker(ifEvalType, myPath, myTracker.getCaseBlocks());
    thenTracker.addIfBranch(condSplit, true);
    logger.trace("Visiting then expr of if {}", ifExpr.ifCondition.expression);
    LogicalExpression thenExpr =
        visitCodeGenContext((CodeGenContext) ifExpr.ifCondition.expression, thenTracker);

    // visit the else expression
    SplitDependencyTracker elseTracker =
        new SplitDependencyTracker(ifEvalType, myPath, myTracker.getCaseBlocks());
    elseTracker.addIfBranch(condSplit, false);
    logger.trace("Visiting else expr of if {}", ifExpr.elseExpression);
    LogicalExpression elseExpr =
        visitCodeGenContext((CodeGenContext) ifExpr.elseExpression, elseTracker);
    ...
    boolean condInPreferred =
        conditionValueVec.isExpressionExecutableInEngine(this.preferredEngine);
    boolean thenInPreferred =
        ((CodeGenContext) thenExpr).isExpressionExecutableInEngine(this.preferredEngine);
    boolean elseInPreferred =
        ((CodeGenContext) elseExpr).isExpressionExecutableInEngine(this.preferredEngine);

    if (((condInPreferred == thenInPreferred) && (condInPreferred == elseInPreferred))
        || (condInPreferred
            && (thenInPreferred || canExecuteInAllCodeGenerators(thenExpr))
            && (elseInPreferred || canExecuteInAllCodeGenerators(elseExpr)))) {
    ...
    IfExpression.Builder finalSplit =
        new IfExpression.Builder().setOutputType(ifExpr.getCompleteType());
    ...
    if ((thenInPreferred == elseInPreferred)
        || canExecuteInAllCodeGenerators(thenExpr)
        || canExecuteInAllCodeGenerators(elseExpr)) {
      // case 3
      // splits in case 3
      ...
      lastSplitInPreferredCodeGenerator =
          ((CodeGenContext) elseExpr).isExpressionExecutableInEngine(preferredEngine)
      ...
      IfExpression evalThen =
          new IfExpression.Builder()
              .setIfCondition(new IfExpression.IfCondition(conditionValueVec, thenExpr))
              .setElse(nullExprContext)
              .setOutputType(ifExpr.getCompleteType())
              .build();
      SplitDependencyTracker splitTracker =
          new SplitDependencyTracker(ifEvalType, myPath, myTracker.getCaseBlocks());
      ...
      ExpressionSplit thenSplit =
          splitter.splitAndGenerateVectorReadExpression(
              evalThenContextNode, myTracker, splitTracker);
      CodeGenContext thenValueVec = thenSplit.getReadExpressionContext();

      // build the if-expression for the last split
      finalSplit
          .setIfCondition(new IfExpression.IfCondition(conditionValueVec, thenValueVec))
          .setElse(elseExpr);
      ...
      ExpressionSplit elseSplit =
          splitter.splitAndGenerateVectorReadExpression(evalElseContext, myTracker, splitTracker);
  ...
  CodeGenContext handleAndExpr(BooleanOperator andExpr, SplitDependencyTracker myTracker)
      throws Exception {
    // TODO: Combine args executed by the same code generators to reduce the number of if-statements
    // that get generated
    // For e.g. AND(e1, 22, e3, ...) can be grouped as
    // AND(E1, E2, ..) where
    // each E = AND(e1, e2, ...) and all arguments can be evaulated by the preferred code generator
    // or not evaluated by the
    ...
    CodeGenContext ifExpressionContext =
        booleanAndToIfExpr(andExpr.args.get(curIndex - 2), andExpr.args.get(curIndex - 1));
    curIndex -= 2;
  ...
  @Override
  public CodeGenContext visitBooleanOperator(
      BooleanOperator op, Tuple<CodeGenContext, SplitDependencyTracker> value) throws Exception {
    CodeGenContext context = value.first;
    SplitDependencyTracker myTracker = value.second;
    if (context.isSubExpressionExecutableInEngine(this.preferredEngine)) {
      return context;
  ...
  @Override
  public CodeGenContext visitUnknown(
      LogicalExpression expr, Tuple<CodeGenContext, SplitDependencyTracker> value)
      throws Exception {
    // All other cases, return the expression as-is
    // TODO: Need to check how the splitter handles complex data types
    // Ideally like(a.b, "Barbie") should execute in Gandiva with a.b evaluated in Java to produce a
    // ValueVector
    return value.first;
  }
  ...
  public CodeGenContext visitCaseExpression(
      CaseExpression caseExpression, Tuple<CodeGenContext, SplitDependencyTracker> value)
      throws Exception {
    ...
    CaseExpressionSplitter caseSplitter =
        new CaseExpressionSplitter(
            caseExpressionContext, parentTracker, splitter, caseSplits, this);
