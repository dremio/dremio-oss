...
public class VacuumTableExpireSnapshotsPlanGenerator extends VacuumPlanGenerator {
  ...
  public VacuumTableExpireSnapshotsPlanGenerator(RelOptCluster cluster, RelTraitSet traitSet, List<PartitionChunkMetadata> splits,
  ...
  public Prel buildPlan() {
    try {
      if (icebergCostEstimates.getSnapshotsCount() == 1 || vacuumOptions.getRetainLast() >= icebergCostEstimates.getSnapshotsCount()) {
        return outputZerosPlan();
      }
      Prel expiredSnapshotFilesPlan = filePathAndTypeScanPlan(snapshotsScanPlan(SnapshotsScanOptions.Mode.EXPIRED_SNAPSHOTS));
      Prel liveSnapshotsFilesPlan = deDupFilePathAndTypeScanPlan(snapshotsScanPlan(SnapshotsScanOptions.Mode.LIVE_SNAPSHOTS));
