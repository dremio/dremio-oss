...
public abstract class AbstractReadSignatureProvider implements ReadSignatureProvider {
  ...
  protected AbstractReadSignatureProvider(
      final String tableRoot, final long queryStartTime, Predicate<String> partitionExists) {
    this.tableRoot = tableRoot;
    this.queryStartTime = queryStartTime;
    fileSystemPartitionToPathMapper =
        ipd -> {
          ...
          for (int i = 0; i < ipd.size(); i++) {
            ...
            if (partVal == null) {
              ...
              // Only use partition columns that match the standard "dirN" format when constructing
              // paths.
  ...
  protected static com.dremio.exec.store.file.proto.FileProtobuf.FileUpdateKey
      decodeReadSignatureByteString(ByteString readSig) {
    try {
