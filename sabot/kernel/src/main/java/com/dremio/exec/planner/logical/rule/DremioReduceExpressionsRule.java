 ...
 *
 * <ul>
 *   <li>Remove SubstitutionRule interface.
 *   <li>Added set to check for replacements.
 *   <li>case_ variable to caseRex.
 *   <li>Remove Safe Cast boolean flag `e = rexBuilder.makeAbstractCast(type, e, false);`
 *   <li>Setting Values.Immutable singleton to true to maintain compatibility
 *   <li>Overriding Config::as to work with immutables
 *   <li>removing unused constructors
 * </ul>
 *
 * <p>Collection of planner rules that apply various simplifying transformations on RexNode trees.
 ...
 */
public abstract class DremioReduceExpressionsRule<C extends DremioReduceExpressionsRule.Config>
    extends RelRule<C> {
  ...
  public static class FilterReduceExpressionsRule
      extends DremioReduceExpressionsRule<
    ...
    public interface FilterReduceExpressionsRuleConfig extends DremioReduceExpressionsRule.Config {
  ...
  public static class ProjectReduceExpressionsRule
    ...
    /** Rule configuration. */
    @Value.Immutable(singleton = true)
    public interface ProjectReduceExpressionsRuleConfig extends DremioReduceExpressionsRule.Config {
      ProjectReduceExpressionsRuleConfig DEFAULT =
  ...
  public static class JoinReduceExpressionsRule
    ...
    @Deprecated // to be removed before 2.0
    public JoinReduceExpressionsRule(
    ...
    /** Rule configuration. */
    @Value.Immutable(singleton = true)
    public interface JoinReduceExpressionsRuleConfig extends DremioReduceExpressionsRule.Config {
      JoinReduceExpressionsRuleConfig DEFAULT =
          ImmutableJoinReduceExpressionsRuleConfig.of()
  ...
  public static class CalcReduceExpressionsRule
      extends DremioReduceExpressionsRule<
          CalcReduceExpressionsRule.CalcReduceExpressionsRuleConfig> {
    ...
    public CalcReduceExpressionsRule(
      ...
      this(
          CalcReduceExpressionsRuleConfig.DEFAULT
              .withRelBuilderFactory(relBuilderFactory)
    ...
    public void onMatch(RelOptRuleCall call) {
      ...
      if (reduceExpressions(
        ...
        for (RexLocalRef projectExpr : program.getProjectList()) {
          ...
          builder.addProject(
              list.get(index).getIndex(), program.getOutputRowType().getFieldNames().get(k++));
        }
     ...
     *
     * <p>The default implementation of this method is to call {@link RelBuilder#empty}, which for
     * the static schema will be optimized to an Immutable.Config.of() {@link
    ...
    /** Rule configuration. */
    @Value.Immutable(singleton = true)
    public interface CalcReduceExpressionsRuleConfig extends DremioReduceExpressionsRule.Config {
      CalcReduceExpressionsRuleConfig DEFAULT =
          ImmutableCalcReduceExpressionsRuleConfig.of()
              .withMatchNullability(true)
  ...
  public static class WindowReduceExpressionsRule
      extends DremioReduceExpressionsRule<
          WindowReduceExpressionsRule.WindowReduceExpressionsRuleConfig> {
    ...
    @Deprecated // to be removed before 2.0
    public WindowReduceExpressionsRule(
        Class<? extends Window> windowClass,
        boolean matchNullability,
        RelBuilderFactory relBuilderFactory) {
      this(
          WindowReduceExpressionsRuleConfig.DEFAULT
              .withRelBuilderFactory(relBuilderFactory)
    ...
    public void onMatch(RelOptRuleCall call) {
      ...
      final RelMetadataQuery mq = call.getMetadataQuery();
      final RelOptPredicateList predicates = mq.getPulledUpPredicates(window.getInput());
      ...
      for (Window.Group group : window.groups) {
        ...
        RelCollation relCollation =
            collationReduced ? RelCollations.of(collationsList) : group.orderKeys;
        groups.add(
            new Window.Group(
                keys, group.isRows, group.lowerBound, group.upperBound, relCollation, aggCalls));
      }
      if (reduced) {
        call.transformTo(
            LogicalWindow.create(
                ...
                window.getRowType(),
                groups));
        call.transformTo(window);
      }
    }

    /** Rule configuration. */
    @Value.Immutable(singleton = true)
    public interface WindowReduceExpressionsRuleConfig extends DremioReduceExpressionsRule.Config {
      WindowReduceExpressionsRuleConfig DEFAULT =
          ImmutableWindowReduceExpressionsRuleConfig.of()
              .withMatchNullability(true)
  ...
  protected DremioReduceExpressionsRule(C config) {
    super(config);
  }
  ...
  @Deprecated // to be removed before 2.0
  protected static boolean reduceExpressions(
      RelNode rel, List<RexNode> expList, RelOptPredicateList predicates, boolean unknownAsFalse) {
   ...
   * Reduces a list of expressions.
   *
   * <p>The {@code matchNullability} flag comes into play when reducing an expression whose type is
  ...
  public static RexCall pushPredicateIntoCase(RexCall call) {
    ...
    for (int i = 0; i < caseRex.getOperands().size(); i++) {
      RexNode node = caseRex.getOperands().get(i);
      if (!RexUtil.isCasePredicate(caseRex, i)) {
        node = substitute(call, caseOrdinal, node);
      }
      nodes.add(node);
    }
    return caseRex.clone(call.getType(), nodes);
  }
  ...
  protected static class RexReplacer extends RexShuttle {
    ...
    private final Set<RexNode> reducibleExpSet;
    ...
    RexReplacer(
      ...
      this.reducibleExpSet = ImmutableSet.copyOf(reducibleExps);
    }
    ...
    private @Nullable RexNode visit(final RexNode call) {
      if (!reducibleExpSet.contains(call)) {
        return null;
      }
      int i = reducibleExps.indexOf(call);
      Preconditions.checkState(-1 != i, "We should find the expression");
      ...
      if (addCasts.get(i) && (replacement.getType() != call.getType())) {
        ...
        replacement = simplify.rexBuilder.makeAbstractCast(type, replacement);
      }
      return replacement;
    }
  }
  ...
  public interface Config extends RelRule.Config {
    @Override
    DremioReduceExpressionsRule<?> toRule();
