...
public class TDigestFunctions {
   ...
   */
  @FunctionTemplate(name = "tdigest_quantile", scope = FunctionTemplate.FunctionScope.SIMPLE, nulls = FunctionTemplate.NullHandling.NULL_IF_NULL)
  public static class TDigestQuantile implements SimpleFunction {

    @Param
    Float8Holder quantile;
    @Param
    ...
    @Override
    public void eval() {
//    todo:  AVLTreeDigest is buggy here, looks like a upstream problem since it is not tested.
      com.tdunning.math.stats.TDigest tDigest = com.tdunning.math.stats.MergingDigest.fromBytes(in.buffer.nioBuffer(in.start, in.end - in.start));
