...
/**
 * An interface representing an extended property that can be serialized and deserialized to/from
 * JSON.
 */
  ...
  public final class Util {
    ...
    public static <T extends ExtendedProperty> String serialize(T property) {
      try {
        ...
        throw new IllegalStateException(
            String.format("Cannot serialize object %s to json", property), e);
      }
    }
    ...
    public static <T extends ExtendedProperty> T deserialize(String json, Class<T> paramClass) {
      try {
        ...
        throw new IllegalStateException(
            String.format("Cannot deserialize json '%s' to object %s", json, paramClass), e);
      }
    }
  }
}
