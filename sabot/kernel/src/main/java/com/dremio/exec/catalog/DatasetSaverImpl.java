...
public class DatasetSaverImpl implements DatasetSaver {
  ...
  private static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(DatasetSaverImpl.class);
  public static final String unsupportedPartitionListingError =
      "Found unsupported partition listing in the table";
   ...
   *
   * <p>The dataset is represented by the given config and handle. If plugin supports unlimited
   * splits, runs internal refresh query else the source metadata API is used to fetch metadata, and
   * save the metadata to namespace. The dataset mutator is the invoked before saving the metadata
   * to namespace; this allow the callers to mutate the dataset config before saving.
   *
   * @param datasetConfig dataset config
   * @param handle dataset handle
   * @param sourceMetadata source metadata
   * @param opportunisticSave if set, will only attempt a save once, without attempting to handle
  ...
  public void save(
    ...
    if (sourceMetadata instanceof SupportsUnlimitedSplits) {
      ...
      if (refreshUsingQuery) {
        boolean successUsingRefreshQuery =
            saveUsingInternalRefreshDatasetQuery(
                datasetConfig, handle, options, datasetMutator, s, canonicalKey, queryUser);
    ...
    Preconditions.checkState(
        ...
        "Found a dataset with Iceberg metadata in non-Iceberg refresh."
            + " Please forget metadata for dataset '%s'",
        canonicalKey);
  ...
  private boolean saveUsingInternalRefreshDatasetQuery(
      DatasetConfig datasetConfig,
      DatasetHandle handle,
      DatasetRetrievalOptions options,
      Function<DatasetConfig, DatasetConfig> datasetMutator,
    ...
    try {
      ...
      logger.debug(
          ...
          String.join(".", handle.getDatasetPath().getComponents()));
      ...
      DatasetConfig savedConfig =
          systemNamespace.getDataset(MetadataObjectsUtils.toNamespaceKey(handle.getDatasetPath()));
      ...
      UserException uex =
          (e.getCause() instanceof UserException) ? (UserException) e.getCause() : null;
      if (uex == null) {
        throw UserException.refreshDatasetError(e)
            .message(
                String.format(
                        "Refresh dataset [%s] error: ",
      ...
      switch (errorCode) {
        case CONCURRENT_MODIFICATION:
          ...
          throw UserException.invalidMetadataError().message(e.getMessage()).buildSilently();
  ...
  private void saveUsingV1Flow(
    ...
    try (DatasetMetadataSaver saver =
        systemNamespace.newDatasetMetadataSaver(
            canonicalKey,
            datasetConfig.getId(),
            splitCompression,
