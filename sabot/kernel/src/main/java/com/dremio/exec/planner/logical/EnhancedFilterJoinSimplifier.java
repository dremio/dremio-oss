...
public final class EnhancedFilterJoinSimplifier {
  ...
  /**
   * In conjunction, we may simplify if one child is entirely pushed. In the root level, we can
   * remove child nodes which are entirely pushed In non-root levels, whether we can remove those
   * entirely pushed child nodes depends on ORs on the top, so we handle in {@link
   * #simplifyDisjunction(RexBuilder, List, List, boolean)}
   */
  public static RexNode simplifyConjunction(
      RexBuilder rexBuilder,
      List<RexNode> originalFilters,
      List<Pair<RexNode, RexNode>> extractions,
      boolean rootLevel) {
   ...
   * E.x. (a and b) or (a and c) -> push: a -> remaining: b or c If in the root level all children
   * are entirely pushed, then we can return {@code true}.
   */
  public static RexNode simplifyDisjunction(
      RexBuilder rexBuilder,
      List<RexNode> originalFilters,
      List<Pair<RexNode, RexNode>> extractions,
      boolean rootLevel) {
    ...
    // If all extractions are entirely pushed, then return the disjunction of their remaining
    // filters
    if (nonEntirelyPushedExtractions.isEmpty()) {
      if (rootLevel) {
        ...
        List<RexNode> remainingFilters =
            entirelyPushedExtractions.stream().map(Pair::getValue).collect(Collectors.toList());
    ...
    for (Pair<RexNode, RexNode> extraction : nonEntirelyPushedExtractions) {
      ...
      if (commonExtractedFilter == null) {
        ...
        commonExtractedFilter =
            getCommonChildNodes(commonExtractedFilter, extractedFilter, SqlKind.AND, rexBuilder);
      }
    }
    ...
    if (commonExtractedFilter == null || commonExtractedFilter.isAlwaysTrue()) {
      ...
      // non-common part) and entirely pushed child nodes. We need to preserve the order of child
      // nodes
      ...
      for (Pair<RexNode, RexNode> extraction : nonEntirelyPushedExtractions) {
        ...
        RexNode simplifiedFilter =
            removeChildNodes(remainingFilter, commonExtractedFilter, SqlKind.AND, rexBuilder);
        // Supply non-common part
        RexNode nonCommonSupply =
            removeChildNodes(extractedFilter, commonExtractedFilter, SqlKind.AND, rexBuilder);
        RexNode simplifiedFilterSupplied =
            RexUtil.composeConjunction(
                rexBuilder, Lists.newArrayList(simplifiedFilter, nonCommonSupply), false);
      ...
      simplifiedFilters.sort(
          Comparator.comparingInt(f -> extractionIndex.get(simplifiedToExtractionMap.get(f))));
   ...
   */
  private static RexNode getCommonChildNodes(
      RexNode e1, RexNode e2, SqlKind sqlKind, RexBuilder rexBuilder) {
    ...
    if (e1.getKind() == sqlKind) {
      MoreRelOptUtil.conDisjunctions(e1)
          .forEach(rexNode -> childNodeMap1.put(rexNode.toString(), rexNode));
    } else {
    ...
    if (e2.getKind() == sqlKind) {
      MoreRelOptUtil.conDisjunctions(e2)
          .forEach(rexNode -> childNodeMap2.put(rexNode.toString(), rexNode));
    } else {
  ...
  /**
   * Remove a subset of child nodes which belong to a parent node E.x. parentNode = (a and b and c),
   * nodeToRemove = (a and b), result = c
   *
   ...
   */
  private static RexNode removeChildNodes(
      RexNode parentNode, RexNode nodeToRemove, SqlKind sqlKind, RexBuilder rexBuilder) {
    if (!(sqlKind == SqlKind.AND || sqlKind == SqlKind.OR)) {
      return parentNode;
    }
    ...
    if (nodeToRemove.getKind() == sqlKind) {
      childNodesToRemoveString =
          MoreRelOptUtil.conDisjunctions(nodeToRemove).stream()
