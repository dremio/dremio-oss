...
/**
 * Class responsible for handling arrow cache creation with unlimited splits. One instance is
 * created per minor fragment of ParquetScanTableFunction Has 3 responsibilities
 *
 * <p>1) Has a Varbinary vector of size 4k which has serialised splits into it. Whenever vector size
 * reaches 4k the splits are flushed onto the the disk into an running arrow file. Location of file
 * is one of the spill directories taken from spill service into a subDir with name
 * boostSplits-queryId-minorFragId-majorFragId.
 *
 * <p>2) On close it creates a plan to fire a query on local machine which should read the arrowFile
 * with splits in the background and then boost those splits.
 */
public class BoostBufferManager {
  private static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(BoostBufferManager.class);
  ...
  public BoostBufferManager(
    ...
    this.isArrowCachingEnabled =
        functionConfig.getFunctionContext().isArrowCachingEnabled()
    ...
    id =
        String.format("boostSplits-%s.%s.%s.%s", qid, majorFragmentId, minorFragmentId, operatorId);
  ...
  public void addSplit(
      SplitAndPartitionInfo inputSplit,
    ...
    NormalizedDatasetSplitInfo normalizedDatasetSplitInfo =
  ...
  public void setupSplitsVectorContainer() throws IOException {
    ...
    ScanOperator.ScanMutator mutator =
        new ScanOperator.ScanMutator(container, Maps.newHashMap(), context, null);
    ...
    arrowFilePath =
        Path.of(
            (new org.apache.hadoop.fs.Path(
                    spillDirectory.getSpillDirPath(), "splitSpill.dremarrow1"))
                .toUri());

    writer =
        new ArrowRecordWriter(
            context,
            arrowFilePath.toString(),
            new ArrowFormatPluginConfig(),
            HadoopFileSystem.get(spillDirectory.getFileSystem()));
  ...
  void createAndFireBoostQuery() throws ForemanSetupException {
    UnlimitedSplitsBoostPlanCreator planCreator =
        new UnlimitedSplitsBoostPlanCreator(
            fec, context, props, functionConfig, arrowFilePath, columnsToBoost, id);
    ...
    try {
      plan =
          ProtobufByteStringSerDe.writeValue(
              context.getLpPersistence().getMapper(), rootPrel, ProtobufByteStringSerDe.Codec.NONE);
      optionsData =
          ProtobufByteStringSerDe.writeValue(
    ...
    CoordExecRPC.PlanFragmentMajor major =
        CoordExecRPC.PlanFragmentMajor.newBuilder()
            ...
            .setCredentials(
                UserBitShared.UserCredentials.newBuilder().setUserName(props.getUserName()).build())
            .setPriority(
    ...
    logger.debug(
        ...
        columnsToBoost,
        functionConfig.getFunctionContext().getReferencedTables().stream()
