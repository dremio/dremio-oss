...
public class HardAssignmentCreator<T extends CompleteWork> {
  private static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(HardAssignmentCreator.class);
  ...
  public <T extends CompleteWork> ListMultimap<Integer, T> getMappings(
      final List<NodeEndpoint> endpoints, final List<T> units)
      throws PhysicalOperatorSetupException {
    verify(
        endpoints,
        units,
        units.size() >= endpoints.size(),
        "There should be at least one work unit for each hard affinity node.");
    ...
    for (T unit : units) {
      ...
      if (endpointsOnHost == null) {
        verify(
            ...
            units,
            false,
            "There are no endpoints in assigned list running on host %s",
            host);
      }
    ...
    // otherwise we will end up with fragments that don't have any work assigned. If an assignment
    // is not present for
  ...
  @FormatMethod
  private static <T extends CompleteWork> void verify(
