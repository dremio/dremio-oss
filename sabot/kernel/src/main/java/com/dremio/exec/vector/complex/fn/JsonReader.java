...
public class JsonReader extends BaseJsonProcessor {
  ...
  private final Map<BaseWriter.StructWriter,Map<String, Field>> structWriterToFieldMap = new HashMap<>();
  ...
  public JsonReader(ArrowBuf managedBuf, int maxFieldSize, int maxLeafLimit, boolean allTextMode, boolean skipOuterList, boolean readNumbersAsDouble,
    ...
    this(managedBuf, GroupScan.ALL_COLUMNS, maxFieldSize, maxLeafLimit, allTextMode, skipOuterList, readNumbersAsDouble,
      ...
      enforceValidJsonDateFormat, null, false, null);
  }

  public JsonReader(ArrowBuf managedBuf, List<SchemaPath> columns, int maxFieldSize, int maxLeafLimit, boolean allTextMode,
                    boolean skipOuterList, boolean readNumbersAsDouble, boolean schemaImposedMode,
                    ExtendedFormatOptions extendedFormatOptions, CopyIntoQueryProperties copyIntoQueryProperties,
                    SimpleQueryContext queryContext, OperatorContext context, BatchSchema targetSchema,
                    boolean enforceValidJsonDateFormat, String filePath, boolean isValidationMode,
    ...
    if (isValidationMode) {
      columns = targetSchema.getFields().stream().map(Field::getName).map(SchemaPath::getSimplePath).collect(Collectors.toList());
    }
  ...
  private Field getField(BaseWriter.StructWriter map, String fieldName, Field originalField) {
    if (schemaImposedMode) {
      if (!isValidationMode) {
        ...
        Function<List<Field>, Field> lookupFunction = l -> l.stream().filter(f -> f.getName().toLowerCase()
            .equals(fieldName.toLowerCase())).findFirst().orElse(null);
        if (originalField == null) {
          return lookupFunction.apply(targetSchema.getFields());
        }
        return lookupFunction.apply(originalField.getChildren());
      }
    }
    return null;
  }
  ...
  private void writeStructDataAllText(Field structField, BaseWriter.StructWriter map, FieldSelection selection, boolean moveForward, boolean rootStruct) throws IOException {
    ...
    while (true) {
      ...
      if(schemaImposedMode) {
        ...
        if(!fieldNamesSet.contains(originalName)) {
          ...
          throw new TransformationException(String.format("Duplicate column name %s.", fieldName), parser.getCurrentLocation().getLineNr(), fieldName);
        }
      ...
      switch (token) {
      case START_ARRAY:
        startList(schemaImposedMode ? originalField.getChildren().get(0) : null, map, fieldName, childSelection);
        break;
      case START_OBJECT:
        if (!writeMapDataIfTyped(map, fieldName)) {
...
//            writeStructDataAllText(map.struct(fieldName), childSelection, false, false);
  ...
  private void startStruct(Field field, BaseWriter.StructWriter writer, String fieldName, FieldSelection selection,
                           boolean moveForward, boolean rootStruct) throws IOException {
  ...
  private void startStruct(Field field, BaseWriter.ListWriter writer, FieldSelection selection, boolean moveForward,
                           boolean rootStruct) throws IOException {
    BaseWriter.StructWriter structWriter = null;
    try {
      if (!isValidationMode) {
  ...
  private void startList(Field field, BaseWriter.StructWriter writer, String fieldName, FieldSelection selection)
      throws IOException {
  ...
  private void startList(Field field, BaseWriter.ListWriter writer, FieldSelection selection) throws IOException {
    BaseWriter.ListWriter listWriter = null;
    try {
      if (!isValidationMode) {
  ...
  private void handleStringToType(JsonParser parser, BaseWriter.StructWriter writer, String fieldName, ArrowType type) throws IOException {
    try {
      incrementLeafCount();
      writeValue(fieldName, type, writer,parser);
    } catch (Exception e) {
      throw new TransformationException(e.getMessage(), parser.getCurrentLocation().getLineNr(), fieldName);
    }
  }

  private void handleStringToType(JsonParser parser, BaseWriter.ListWriter writer, ArrowType type) throws IOException {
    try {
      incrementLeafCount();
   ...
   */
  private Object convertValue(String fieldName, ArrowType type, JsonParser parser) throws IOException {
    if (parser.getCurrentToken() == VALUE_NULL) {
      return null;
    }
    String value = EasyFormatUtils.applyStringTransformations(parser.getText(), extendedFormatOptions, trimSpace);
    if (value == null) {
      return null;
    }
    try {
      if (CompleteType.BIT.getType().equals(type)) {
        ...
        return value;
      } else if (CompleteType.DATE.getType().equals(type) || CompleteType.TIME.getType().equals(type) || CompleteType.TIMESTAMP.getType().equals(type)) {
  ...
  private void writeValue(String fieldName, ArrowType type, BaseWriter.StructWriter writer, JsonParser parser) throws IOException {
  ...
  private void writeValue(ArrowType type, BaseWriter.ListWriter writer, JsonParser parser) throws IOException {
  ...
  /**
   * This utility method handles errors related to transformations during the reading of a JSON file.
   * Based on the reading mode configured by {@link JsonReader#copyIntoQueryProperties}, the provided exception is either
   * rethrown or the error metadata is stored in JSON format in an "error" column on the target table.
   * <p>
   *   Reader modes:
   * </p>
   * <p>
   *   ABORT -> the exception is rethrown.
   *   CONTINUE -> a new record is stored in the error column.
   * </p>
   * @param writer output writer object
   * @param exception exception object raised during parsing
   * @return if the exception is not rethrown the {@link com.dremio.exec.store.easy.json.JsonProcessor.ReadState} is returned
   */
  private ReadState handleOrRaiseException(ComplexWriter writer, Exception exception) throws IOException {
  ...
  private boolean processError(ComplexWriter writer, Exception exception) throws IOException {
    if (copyIntoQueryProperties != null) {
      logger.debug(String.format("Encountered error while reading json file. JsonReader is running in '%s' mode.",
        copyIntoQueryProperties.getOnErrorOption()), exception);
      if (schemaImposedMode && CONTINUE == copyIntoQueryProperties.getOnErrorOption()) {
        writeError(writer.rootAsStruct());
        parser.close();
        return true;
      }
    }
  ...
  private <T extends Exception> boolean processErrorDuringValidation(ComplexWriter writer, T exception) throws IOException {
    if (isValidationMode) {
   ...
   */
  private void writeError(BaseWriter.StructWriter structWriter) throws IOException {
    // when the error was raised we were in the middle of processing a json record, which was already submitted to the
    ...
    String infoJson = ErrorInfo.Util.getJson(new CopyIntoErrorInfo.Builder(queryContext.getQueryId(),
      ...
      extendedFormatOptions, FileType.JSON.name(), recordsLoadedCount == 0 ? CopyIntoErrorInfo.CopyIntoFileState.SKIPPED : CopyIntoErrorInfo.CopyIntoFileState.PARTIALLY_LOADED)
      .setRecordsLoadedCount(recordsLoadedCount).setRecordsRejectedCount(1L).build());
    writeString(infoJson, structWriter, ColumnUtils.COPY_INTO_ERROR_COLUMN_NAME);
  }
  ...
  private void writeListDataAllText(Field listField, BaseWriter.ListWriter list, FieldSelection selection) throws IOException {
    final int originalLeafCount = currentLeafCount;
    int maxArrayLeafCount = 0;
    outside: while (true) {
      currentLeafCount = originalLeafCount;
  ...
  private boolean skipObject(Field field, String fieldName, FieldSelection selection, BaseWriter.StructWriter map) throws TransformationException {
    ...
    if (schemaImposedMode) {
      ...
      if(!Objects.isNull(currentField) && CompleteType.MAP.getType().equals(currentField.getType())) {
        throw new TransformationException(String.format("'COPY INTO Command' does not support MAP Type. Found Map type field : '%s'. ", fieldName), parser.getCurrentLocation().getLineNr(), fieldName);
      }
  ...
  private void checkForComplexCoercions(ArrowType type, JsonToken token, String fieldName) throws TransformationException {
    ...
    switch (token) {
      case START_ARRAY:
        if (!ArrowType.ArrowTypeID.List.equals(type.getTypeID())) {
          errorMessage = String.format("Field %s having %s datatype in the file cannot be coerced into %s datatype " +
              "of the target table.", fieldName, "List", type);
        }
        break;
      case START_OBJECT:
        if (!ArrowType.ArrowTypeID.Struct.equals(type.getTypeID())) {
          errorMessage = String.format("Field %s having %s datatype in the file cannot be coerced into %s datatype " +
              "of the target table.", fieldName, "Struct", type);
        }
        break;
      default:
    ...
    if (errorMessage != null) {
      throw new TransformationException(errorMessage, parser.getCurrentLocation().getLineNr(), fieldName);
    }
  }
}
