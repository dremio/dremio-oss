...
public class DeltaLakeHistoryScanPlanBuilder {
  ...
  public DeltaLakeHistoryScanPlanBuilder(RelOptCluster cluster, RelTraitSet traitSet, RelOptTable table, TableMetadata tableMetadata,
  ...
  public static DeltaLakeHistoryScanPlanBuilder fromDrel(ScanRelBase drel) {
    return new DeltaLakeHistoryScanPlanBuilder(drel.getCluster(), drel.getTraitSet().plus(Prel.PHYSICAL),
      drel.getTable(),drel.getTableMetadata(), drel.getProjectedColumns(), drel.getObservedRowcountAdjustment());
  }
  ...
  private Prel createDirListingScan() {
    ...
    RefreshExecTableMetadata dirScanMetadata = new RefreshExecTableMetadata(pluginId, datasetConfig, tableMetadata.getUser(), splitsPointer, BatchSchema.EMPTY, null);
    return new DirListingScanPrel(cluster, traitSet, table, pluginId, dirScanMetadata, observedRowcountAdjustment,
      ImmutableList.of(), false, x -> estimateRowCount(), ImmutableList.of());
  }
  ...
  private Prel createHistoryScan(Prel input) {
    ...
    final TableFunctionConfig historyScanConfig = TableFunctionUtil.getTableFunctionConfig(
      TableFunctionConfig.FunctionType.DELTALAKE_HISTORY_SCAN, schema, tableMetadata.getStoragePluginId());
    final RelDataType rowType = ScanPrelBase.getRowTypeFromProjectedColumns(
      schema.getFields().stream().map(f -> SchemaPath.getSimplePath(f.getName())).collect(Collectors.toList()), schema, cluster);

    return new TableFunctionPrel(cluster, traitSet, input, historyScanConfig, rowType, x -> estimateRowCount(),
      (long) estimateRowCount(), tableMetadata.getUser());
  }
  ...
  private double estimateRowCount() {
    ...
    double rowMultiplier = ((double) plannerSettings.getSliceTarget() /
      plannerSettings.getOptions().getOption(PlannerSettings.DELTALAKE_HISTORY_SCAN_FILES_PER_THREAD));
  ...
  private Prel createProjectionScan(Prel input) {
    ...
    tableMetadata.getSchema().getFields().forEach(field -> {
      ...
      switch (field.getName()) {
        ...
        default:
          projectExpressions.add(cluster.getRexBuilder().makeNullLiteral(convertFieldType(field)));
          break;
      }
    });
    RelDataType rowType = ScanPrelBase.getRowTypeFromProjectedColumns(projectedColumns, tableMetadata.getSchema(), cluster);
  ...
  private RexNode createInputRef(Prel input, String fieldName) {
    Pair<Integer, RelDataTypeField> fieldPair = MoreRelOptUtil.findFieldWithIndex(input.getRowType().getFieldList(), fieldName);
    if (fieldPair == null) {
      throw new RuntimeException(String.format("Unable to find field '%s' in the input", fieldName));
    }
  ...
  private RelDataType convertFieldType(Field field) {
    return CalciteArrowHelper.wrap(CompleteType.fromField(field))
      .toCalciteType(cluster.getTypeFactory(), PrelUtil.getPlannerSettings(cluster).isFullNestedSchemaSupport());
  }
}
