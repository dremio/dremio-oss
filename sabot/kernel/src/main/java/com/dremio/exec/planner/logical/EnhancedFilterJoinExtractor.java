...
public final class EnhancedFilterJoinExtractor {
  ...
  public EnhancedFilterJoinExtractor(
      Filter filterRel, Join joinRel, FilterJoinRule.Predicate predicate) {
    ...
    RexNode inputFilterCondition =
        (filterRel != null) ? filterRel.getCondition() : rexBuilder.makeLiteral(true);
    RexNode inputJoinCondition = joinRel.getCondition();

    // Convert to Nnf
    RexNode inputFilterConditionNnf =
        SimpleExpressionCanonicalizer.toNnf(inputFilterCondition, rexBuilder);
    RexNode inputJoinConditionNnf =
        SimpleExpressionCanonicalizer.toNnf(inputJoinCondition, rexBuilder);

    // Prune if some child nodes are the superset of some others
    this.inputFilterConditionPruned =
        EnhancedFilterJoinPruner.pruneSuperset(rexBuilder, inputFilterConditionNnf, true);
    this.inputJoinConditionPruned =
        EnhancedFilterJoinPruner.pruneSuperset(rexBuilder, inputJoinConditionNnf, true);
  ...
  public EnhancedFilterJoinExtraction extract() {
    ...
    ImmutableList<RexNode> aboveFilters =
        ImmutableList.copyOf(RelOptUtil.conjunctions(inputFilterConditionPruned));
  ...
  private EnhancedFilterJoinExtraction extractInnerJoin() {
    ...
    RexNode inputCondition =
        RexUtil.composeConjunction(
            rexBuilder,
            Lists.newArrayList(inputFilterConditionPruned, inputJoinConditionPruned),
            false);
    ...
    RexNode inputConditionPruned =
        EnhancedFilterJoinPruner.pruneSuperset(rexBuilder, inputCondition, false);
    ...
    return new EnhancedFilterJoinExtraction(
        inputFilterConditionPruned,
        inputJoinConditionPruned,
        joinCondition,
  ...
  /**
   * Fixes up the type of all Rex nodes in an expression to match differences in nullability.
   *
  ...
  private RexNode fixUpNullability(RexNode joinFilter) {
    ...
    final ImmutableList<RelDataType> fieldTypes =
        ImmutableList.<RelDataType>builder()
            ...
            .addAll(RelOptUtil.getFieldTypeList(joinRel.getRight().getRowType()))
            .build();

    return RexUtil.composeConjunction(
        rexBuilder, RexUtil.fixUp(rexBuilder, Arrays.asList(joinFilter), fieldTypes), false);
  }

  /**
   * Perform extraction for LEFT outer join As the LEFT join is the type that already simplified, so
   * there are nulls on the right: join condition [filter -> join]: NO left filter [filter -> left
   * of join]: YES right filter [filter -> right of join]: NO left filter [join -> left of join]: NO
  ...
  /**
   * Perform extraction for RIGHT outer join As the RIGHT join is the type that already simplified,
   * so there are nulls on the left: join condition [filter -> join]: NO left filter [filter -> left
   * of join]: NO right filter [filter -> right of join]: YES left filter [join -> left of join]:
   * YES right filter [join -> right of join]: NO
   */
  ...
  /**
   * Perform extraction for FULL outer join As the FULL join is the type that already simplified, so
   * there are nulls on both sides: join condition [filter -> join]: NO left filter [filter -> left
   * of join]: NO right filter [filter -> right of join]: NO left filter [join -> left of join]: NO
   ...
   * @return A pair of extracted join Condition to be kept and the simplified remaining join
   *     Condition considered for pushDown
   */
  private Pair<RexNode, RexNode> extractJoinCondition(RexNode inputFilter, JoinRelType joinType) {
    Predicate<RexNode> leafValidator =
        (leafFilter) -> {
          ...
          // ToDO: DX44431 removes predicate checks for AND and ORs, remove this check for general
          // operations
   ...
   */
  private Pair<RexNode, RexNode> extractPushdownPredicates(
      RexNode inputFilter, ImmutableBitSet targetBitSet) {
    Predicate<RexNode> leafValidator =
        (leafFilter) -> {
          RelOptUtil.InputFinder inputFinder = RelOptUtil.InputFinder.analyze(leafFilter);
          ImmutableBitSet filterBitSet = inputFinder.build();
  ...
  private class RecurseExtractor {
     ...
     * @param isRoot Whether the current filter is the root, to handle the case that a filter is
    ...
    private Pair<RexNode, RexNode> extractConjunction(RexNode filter, boolean isRoot) {
      ...
      RexNode extractedFilter = RexUtil.composeConjunction(rexBuilder, extractedChildNodes, false);
      RexNode simplifiedRemainingFilter =
          EnhancedFilterJoinSimplifier.simplifyConjunction(
              rexBuilder, childNodes, extractions, isRoot);

      // Prune superset
      RexNode extractedFilterPruned =
          EnhancedFilterJoinPruner.pruneSuperset(rexBuilder, extractedFilter, false);
      RexNode simplifiedRemainingFilterPruned =
          EnhancedFilterJoinPruner.pruneSuperset(rexBuilder, simplifiedRemainingFilter, false);
    ...
    private Pair<RexNode, RexNode> extractDisjunction(RexNode filter, boolean isRoot) {
      ...
      RexNode extractedFilter = RexUtil.composeDisjunction(rexBuilder, extractedChildNodes, false);
      RexNode simplifiedRemainingFilter =
          EnhancedFilterJoinSimplifier.simplifyDisjunction(
              rexBuilder, childNodes, extractions, isRoot);
