...
public class StAXBasedParser implements ExcelParser {
  ...
  private static final Ordering<MergeCellRegion> MERGE_CELL_REGION_ORDERING =
      Ordering.from(
          new Comparator<MergeCellRegion>() {
            @Override
   ...
   * @param hasMergedCells Does the sheet has any merged cells. Supporting merged cells is an
   *     expensive operation. Indicating there are no merged cells, improves the read time.
   * @throws XMLStreamException
   * @throws IOException
   */
  private void init(final boolean extractHeader, final boolean hasMergedCells)
  ...
  /**
   * Helper method to parse the first row contents as column names. Assumes the current cursor is
   * set to start of the first row.
   *
   * @throws XMLStreamException
   */
        ...
        case START_ELEMENT:
          {
            ...
            if (ExcelUtil.VALUE.equals(name)
              ...
              lookupValueInSST =
                  ExcelUtil.SST_STRING.equals(
                      xmlStreamReader.getAttributeValue(/* namespaceURI= */ null, ExcelUtil.TYPE));
              String cellRef =
                  xmlStreamReader.getAttributeValue(/* namespaceURI= */ null, ExcelUtil.CELL_REF);
  ...
  /**
   * Helper method which parses the merge cell boundaries and creates data structures for fast
   * lookup.
   *
   * @throws XMLStreamException
   */
        ...
        case START_ELEMENT:
          {
            ...
            if (ExcelUtil.MERGE_CELL.equals(name)) {
              String[] corners =
                  xmlStreamReader
                      .getAttributeValue(/* namespaceURI= */ null, ExcelUtil.MERGE_CELL_REF)
                      .split(":");
   ...
   *
   * @return {@link State#READ_SUCCESSFUL} when a row is read. {@link State#END_OF_STREAM} when
  ...
  /**
   * Helper method to handle start of the "cell" element. It reads required attributes and set the
   * workspace variables.
   */
  private void handleCellStart() {
    ...
    determineOutputTypes(
        typeValue, xmlStreamReader.getAttributeValue(/* namespaceURI= */ null, ExcelUtil.STYLE));
    currentCellRef =
        xmlStreamReader.getAttributeValue(/* namespaceURI= */ null, ExcelUtil.CELL_REF);
  ...
  /**
   * Handle string value read in "value" element. Based on the workspace variable, determine its
   * type and set it in vector container.
   *
   * @param value
   */
  private void handleValue(String value) {
    if (value.length() != 0) {
      ...
      final boolean projectedAndNotSkipQuery =
          !skipQuery && (columnsToProject == null || columnsToProject.contains(finalColumnName));

      final MergeCellRegion mergeCellRegion =
          mergeCells != null ? mergeCells.get(currentCellRef) : null;
      ...
      switch (valueTypeFromAttribute) {
        ...
        case VARCHAR:
          ...
          FieldSizeLimitExceptionHelper.checkSizeLimit(
              b.length, maxCellSize, currentColumnIndex, logger);
  ...
  /**
   * Ensure a null value is set for the writer by retrieving the writer for the column but not
   * writing a value.
   */
  ...
  /**
   * Resolve the given value with respect to whether it is a reference to element in shared strings
   * table. Also decode the final value.
   *
  ...
  /**
   * Populate the values for merged cells in current row. Also delete the merged regions that are
   * not valid from current row onwards.
   */
