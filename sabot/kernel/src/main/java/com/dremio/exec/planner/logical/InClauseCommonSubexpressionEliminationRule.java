 ...
 * SELECT *
 * FROM people
 * WHERE FOO(name) IN ('son', 'john', 'bob')
 *
 * It can get translated to:
 *
 * SELECT *
 * FROM people
 * WHERE (FOO(name) = 'son') OR (FOO(name) = 'john') OR (FOO(name) = 'bob')
 *
 * Notice that FOO(name) gets evaluated 3 times and FOO(x) might be an expensive expression to evaluate.
 *
 * We can apply common subexpression elimination (CSE) to mitigate this by rewriting the query like so:
 *
 * let temp = FOO(name);
 * SELECT *
 * FROM people
 * WHERE temp IN ('son', 'john', 'bob')
 *
 * Basically store the result of the common subexpression in a temp variable (or projection rel node) and reference that.
 */
public final class InClauseCommonSubexpressionEliminationRule extends RelOptRule {
  public static final InClauseCommonSubexpressionEliminationRule INSTANCE = new InClauseCommonSubexpressionEliminationRule(DremioRelFactories.LOGICAL_BUILDER);

  private InClauseCommonSubexpressionEliminationRule(RelBuilderFactory factory) {
    super(
      operand(FilterRel.class, any()),
      factory,
      "InClauseCommonSubexpressionEliminationRule");
  }
  ...
  public boolean matches(RelOptRuleCall relOptRuleCall) {
    ...
    return RelOptUtil
      .conjunctions(filterRel.getCondition())
      .stream()
  ...
  public void onMatch(RelOptRuleCall relOptRuleCall) {
    ...
    RexNode rewrittenCondition = getRewrittenFilter(
      filterRel,
    ...
    final RelNode rewrittenRelNode = relOptRuleCall
      ...
      // We do this by pushing the common sub-expression to the projection and referencing that.
  ...
  private static HashSet<RexNodeWithDeepEqualsAndHash> getCommonComplexSubexpressions(RexCall condition) {
    ...
    for (RexNode operand : operands) {
      ...
      RexNodeWithDeepEqualsAndHash rexNodeWithDeepEqualsAndHash = new RexNodeWithDeepEqualsAndHash(complexChildExpression);
  ...
  private static RexNode getRewrittenFilter(
    ...
    for (RexNode operand : operands) {
      ...
      if(!optionalComplexChildExpression.isPresent()) {
        ...
        RexNodeWithDeepEqualsAndHash rexNodeWithDeepEqualsAndHash = new RexNodeWithDeepEqualsAndHash(complexChildExpression);
        Integer projectionIndex = commonSubexpressionToProjectionIndex.get(rexNodeWithDeepEqualsAndHash);
        if(projectionIndex == null) {
          ...
          RexNode nonComplexSubexpression = complexChildExpression ==  binaryOperand0 ? binaryOperand1 : binaryOperand0;

          RexInputRef rexInputRef = new RexInputRef(projectionIndex, complexChildExpression.getType());
          ...
          rewrittenOperand = rexBuilder.makeCall(binaryExpression.type, binaryExpression.op, rewrittenBinaryOperands);
        }
      }
    ...
    RexNode rewrittenCondition = rexBuilder.makeCall(condition.type, condition.op, rewrittenOperands);
  ...
  private static final class RexNodeWithDeepEqualsAndHash  {
    ...
    public boolean equals(final Object obj) {
      ...
      return (this.rexNode == other.rexNode) || (this.rexNode.toString().equals(other.rexNode.toString()));
    }
  }
}
