...
public class MergeJoinBatch {
  ...
  //
  //  protected MergeJoinBatch(MergeJoinPOP popConfig, OperatorContext context, RecordBatch left,
  // RecordBatch right) throws OutOfMemoryException {
  //    super(popConfig, context, true);
  //
  //    if (popConfig.getConditions().size() == 0) {
  //      throw new UnsupportedOperationException("Merge Join currently does not support cartesian
  // join.  This join operator was configured with 0 conditions");
  //    }
  //    this.left = left;
  ...
  //    // we do this in the here instead of the constructor because don't necessary want to start
  // consuming on construction.
  //    status.prepare();
  ...
  //          return (first? IterOutcome.OK_NEW_SCHEMA : (status.getOutPosition() > 0 ?
  // IterOutcome.OK: IterOutcome.NONE));
  //        case SCHEMA_CHANGED:
  //          worker = null;
  ...
  // context.getClassProducer().createGenerator(JoinWorker.TEMPLATE_DEFINITION).getRoot();
  //
  ...
  //    cg.getSetupBlock().assign(JExpr._this().ref(outgoingVectorContainer),
  // JExpr.direct("outgoing"));
  //
  ...
  //    cg.getSetupBlock().assign(JExpr._this().ref(incomingLeftRecordBatch),
  // joinStatus.ref("left"));
  //
  //    // declare and assign incoming right RecordBatch member
  //    JVar incomingRightRecordBatch = cg.clazz.field(JMod.NONE, recordBatchClass,
  // "incomingRight");
  //    cg.getSetupBlock().assign(JExpr._this().ref(incomingRightRecordBatch),
  // joinStatus.ref("right"));
  //
  //    // declare 'incoming' member so VVReadExpr generated code can point to the left or right
  // batch
  ...
  //     * have the same return type, if not then inject casts so that comparison function will work
  // as
  //     * expected
  //     */
  ...
  //      rightExpr[i] = materializeExpression(condition.getRight(), lastRightStatus,
  // rightIterator);
  //    }
  //
  //    // if right side is empty, rightExpr will most likely default to NULLABLE INT which may
  // cause the following
  //    // call to throw an exception. In this case we can safely skip adding the casts
  //    if (lastRightStatus != IterOutcome.NONE) {
  //      JoinUtils.addLeastRestrictiveCasts(leftExpr, leftIterator, rightExpr, rightIterator,
  ...
  //          outputType = new MajorType(inputType.getMinorType(), DataMode.OPTIONAL,
  // inputType.getPrecision(), inputType.getScale(), inputType.getTimezone(),
  // inputType.getSubTypes());
  //        } else {
  ...
  //    if (status.getRightStatus() != IterOutcome.NONE && (worker == null ||
  // !status.right.finished())) {
  ...
  // MajorTypeHelper.getFieldForNameAndMajorType(w.getField().getPath(), outputType);
  ...
  //                                 LogicalExpression[] leftExpression, JVar
  // incomingLeftRecordBatch, LogicalExpression[] rightExpression,
  //                                 JVar incomingRightRecordBatch) throws
  ...
  //        ClassGenerator.HoldingContainer compareLeftExprHolder = cg.addExpr(leftExpression[i],
  // false);
  //
  //        cg.setMappingSet(compareRightMapping);
  //        cg.getSetupBlock().assign(JExpr._this().ref(incomingRecordBatch),
  // JExpr._this().ref(incomingRightRecordBatch));
  //        ClassGenerator.HoldingContainer compareRightExprHolder = cg.addExpr(rightExpression[i],
  // false);
  //
  ...
  //        // Null compares to Null should returns null (unknown). In such case, we return 1 to
