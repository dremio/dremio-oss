...
public class IcebergExpiryAction {
  ...
  public IcebergExpiryAction(SupportsIcebergMutablePlugin icebergMutablePlugin,
                             OpProps props,
    ...
    try {
      IcebergModel icebergModel = icebergMutablePlugin.getIcebergModel(getTableProps(), props.getUserName(), this.context, fileIO);
      ...
      if (!commitExpiry) {
        if (vacuumOptions.isRemoveOrphans()) {
          this.liveSnapshots = allSnapshots.stream().map(s -> new SnapshotEntry(tableMetadata.metadataFileLocation(), s)).collect(Collectors.toList());
          Set<Long> liveSnapshotIds = liveSnapshots.stream().map(SnapshotEntry::getSnapshotId).collect(Collectors.toSet());
          this.expiredSnapshots = allSnapshots.stream().filter(s -> !liveSnapshotIds.contains(s.snapshotId()))
            .map(s -> new SnapshotEntry(tableMetadata.metadataFileLocation(), s)).collect(Collectors.toList());
        ...
        Preconditions.checkState(vacuumOptions.isExpireSnapshots(), "Should be ExpireSnapshots status");
        if (allSnapshots.size() <= vacuumOptions.getRetainLast()) {
          // nothing to expire
          LOGGER.info("Skipping {} because snapshot count <= {}", tableId, vacuumOptions.getRetainLast());
          this.liveSnapshots = allSnapshots.stream().map(s -> new SnapshotEntry(tableMetadata.metadataFileLocation(), s))
            .collect(Collectors.toList());
        } else {
          this.liveSnapshots = icebergModel.expireSnapshots(tableId, vacuumOptions.getOlderThanInMillis(), vacuumOptions.getRetainLast());
        }
  ...
  protected List<String> computeMetadataPathsToRetain(TableMetadata tableMetadata, Set<Long> liveSnapshotIds) {
    // Metadata files are only needed to be appended for Remove Orphan Files.
    if (!vacuumOptions.isRemoveOrphans()) {
      return Collections.emptyList();
    }

    // Old metadata is kept for history by default. The retention is determined by the table properties
    // write.metadata.delete-after-commit.enabled and write.metadata.previous-versions-max during writes.
    // Add version hint file location
    String versionHintFilePath = tableMetadata.location() + Path.SEPARATOR + METADATA_FOLDER_NAME + Path.SEPARATOR  + VERSION_HINT_FILENAME;
    return Stream.concat(
      Stream.of(tableMetadata.metadataFileLocation(), versionHintFilePath),
      tableMetadata.previousFiles().stream().map(TableMetadata.MetadataLogEntry::file)).collect(Collectors.toList()
    );
  }
  ...
  public List<SnapshotEntry> getAllSnapshotEntries() {
    return allSnapshots.stream().map(s -> new SnapshotEntry(tableMetadata.metadataFileLocation(), s)).collect(Collectors.toList());
  }
  ...
  public static ExpireSnapshots getIcebergExpireSnapshots(Table table, long olderThanInMillis, int retainLast) {
    olderThanInMillis = getOlderThanInMillisWithTableProperties(table.properties(), olderThanInMillis);
    retainLast = getRetainLastWithTableProperties(table.properties(), retainLast);
    return table.expireSnapshots().expireOlderThan(olderThanInMillis).retainLast(retainLast)
        .cleanExpiredFiles(false);
  }

  private static long getOlderThanInMillisWithTableProperties(Map<String, String> tableProperties, long olderThanInMillis) {
    long tablePropOlderThanInMillis = System.currentTimeMillis() - PropertyUtil.propertyAsLong(tableProperties,
      MAX_SNAPSHOT_AGE_MS, 0);
    return Math.min(olderThanInMillis, tablePropOlderThanInMillis);
  }

  private static int getRetainLastWithTableProperties(Map<String, String> tableProperties, int retainLast) {
    return Math.max(retainLast,
  ...
  protected IcebergTableProps getTableProps() {
    ...
    return IcebergTableProps.createInstance(IcebergCommandType.VACUUM, tableName, dbName,
      tableMetadata, versionContext);
  }
