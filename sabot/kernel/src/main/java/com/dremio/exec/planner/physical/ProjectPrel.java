...
public class ProjectPrel extends ProjectRelBase implements Prel {
  ...
  public static final LongValidator RESERVE =
      new PositiveLongValidator(
          "planner.op.project.reserve_bytes", Long.MAX_VALUE, DEFAULT_RESERVE);
  public static final LongValidator LIMIT =
      new PositiveLongValidator("planner.op.project.limit_bytes", Long.MAX_VALUE, DEFAULT_LIMIT);
  ...
  public PhysicalOperator getPhysicalOperator(PhysicalPlanCreator creator) throws IOException {
    ...
    List<NamedExpression> exprs =
        getProjectExpressions(new ParseContext(PrelUtil.getSettings(getCluster())));
    final BatchSchema schema =
        ExpressionTreeMaterializer.materializeFields(
                ...
                ALLOW_COMPLEX,
                ALLOW_GANDIVA_FUNCTIONS)
            .setSelectionVectorMode(childSchema.getSelectionVectorMode())
            .build();

    return new Project(creator.props(this, null, schema, RESERVE, LIMIT), childPOP, exprs);
  }
   ...
   */
  public static ProjectPrel create(
      RelOptCluster cluster,
      RelTraitSet traits,
      RelNode child,
      List<RexNode> exps,
    ...
    return ProjectRelBase.adjustTraits(cluster, input, exps, traits)
        .replaceIf(
            ...
            () -> {
              ...
              final DistributionTrait inputDistribution =
                  input.getTraitSet().getTrait(DistributionTraitDef.INSTANCE);
              final Map<Integer, Integer> index = new HashMap<>();

              for (Ord<? extends RexNode> exp : Ord.zip(exps)) {
                // For distribution, either $0 or cast($0 as ...) would keep the distribution after
                // projection.
              ...
              // original distribution field, we should replace with either SINGLETON or
              // RANDOM_DISTRIBUTED.
              if (newFields.isEmpty() || newFields.size() < inputDistribution.getFields().size()) {
                ...
                return new DistributionTrait(
                    inputDistribution.getType(), ImmutableList.copyOf(newFields));
              }
            });
  }
}
