...
public class MetadataRefreshPlanBuilderFactory {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(MetadataRefreshPlanBuilderFactory.class);

  @WithSpan
  public static MetadataRefreshPlanBuilder getPlanBuilder(
      SqlHandlerConfig config, SqlRefreshDataset sqlRefreshDataset) throws IOException {

    SupportsInternalIcebergTable plugin = getPlugin(config, sqlRefreshDataset);
    final boolean isHiveDataset =
        getPlugin(config, sqlRefreshDataset).canGetDatasetMetadataInCoordinator();
    ...
    UnlimitedSplitsMetadataProvider metadataProvider =
        new UnlimitedSplitsMetadataProvider(config, nsKey);
    boolean isFullRefresh = !metadataProvider.doesMetadataExist();

    if (isFullRefresh && sqlRefreshDataset.isPartialRefresh()) {
      throw new UnsupportedOperationException(
          "Selective refresh is not allowed on unknown datasets. Please run full refresh on "
              + nsKey.getSchemaPath());
    }
    AbstractRefreshPlanBuilder planBuilder =
        plugin.createRefreshDatasetPlanBuilder(
  ...
  private static SupportsInternalIcebergTable getPlugin(
      SqlHandlerConfig config, SqlRefreshDataset sqlRefreshDataset) {
    Catalog catalog = config.getContext().getCatalog();
    ...
    if (plugin instanceof SupportsInternalIcebergTable) {
      ...
      throw UserException.validationError()
          .message(
              "Source identified was invalid type. REFRESH DATASET is only supported on sources that can contain files or folders")
          .build(logger);
    }
  }
}
