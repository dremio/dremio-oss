...
public class ArrowCrossBufComparatorProvider {
  ...
  public static ArrowCrossBufComparator get2ByteNumComparator() {
    return (buf1, idx1, buf2, idx2) ->
        Short.compare(buf1.getShort(idx1 * 2), buf2.getShort(idx2 * 2));
  }

  public static ArrowCrossBufComparator get4ByteNumComparator() {
    return (buf1, idx1, buf2, idx2) ->
        Integer.compare(buf1.getInt(idx1 * 4), buf2.getInt(idx2 * 4));
  }
    ...
    return (buf1, idx1, buf2, idx2) -> {
      ...
      BigDecimal decimal1 =
          DecimalUtils.getBigDecimalFromLEBytes(
              buf1.memoryAddress() + (idx1 * blockSize), bytes1, 0);
      BigDecimal decimal2 =
          DecimalUtils.getBigDecimalFromLEBytes(
              buf1.memoryAddress() + (idx2 * blockSize), bytes1, 0);
