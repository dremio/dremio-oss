...
public class InformationSchemaRecordReader extends AbstractRecordReader {
  ...
  private TableWriter<?> createTableWriter() {
    final Set<String> selectedFields =
        isStarQuery()
            ? InformationSchemaMetadata.getAllFieldNames(table.getRecordSchema())
            : getGivenFields();
      ...
      case CATALOGS:
        {
          ...
          return new CatalogsTableWriter(
              catalogStub.listCatalogs(catalogsRequest.build()), selectedFields, catalogName);
        }
      ...
      case SCHEMATA:
        {
          ...
          return new SchemataTableWriter(
              catalogStub.listSchemata(schemataRequest.build()), selectedFields, catalogName);
        }
      ...
      case TABLES:
        {
          ...
          return new TablesTableWriter(
              catalogStub.listTables(tablesRequest.build()), selectedFields, catalogName);
        }
      ...
      case VIEWS:
        {
          ...
          return new ViewsTableWriter(
              catalogStub.listViews(viewsRequest.build()), selectedFields, catalogName);
        }
      ...
      case COLUMNS:
        {
          ...
          final Iterator<TableSchema> tableSchemata =
              catalogStub.listTableSchemata(columnsRequest.build());

          // For each TableSchema, iterates over #flatMap of batch_schema field, which represents
          // the records in the
          // "COLUMNS" table, and not the TableSchema message itself (unlike other tables).
          final Iterator<Column> columnIterator =
              new AbstractIterator<Column>() {
                ...
                protected Column computeNext() {
                  while (true) {
                    ...
                    // Gets next TableSchema from the catalog service only after exhausting the
                    // current one. See comment in
                    // TableWriter#write.
                    final TableSchema currentSchema = tableSchemata.next();
                    BatchSchema bs =
                        BatchSchema.deserialize(currentSchema.getBatchSchema().toByteArray());
                    // If an inconsistency is detected don't attempt converting to Arrow format
                    // since it will cause an assertion failure.  Put out a warning and move on to
                    // next row.
                    if (bs.getFieldCount() == 0) {
                      // Add a warning message to indicate this table has missing fields
                      logger.warn(
                          "{}.{}.{} has missing fields or incorrect format. ",
                          currentSchema.getCatalogName(),
