...
public class MaestroForwarderImpl implements MaestroForwarder {
  private static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(MaestroForwarderImpl.class);
  ...
  public void screenCompleted(NodeQueryScreenCompletion completion) {
    if (mustForwardRequest(completion.getForeman())) {
      logger.debug(
          "Forwarding NodeQueryScreenCompletion request for Query {} from {} to target {}",
  ...
  public void nodeQueryMarkFirstError(NodeQueryFirstError error) {
    if (mustForwardRequest(error.getForeman())) {
      logger.debug(
          "Forwarding NodeQueryFirstError request for Query {} from {} to target {}",
  ...
  public void dataArrived(
      JobResultsRequestWrapper jobResultsRequestWrapper, ResponseSender sender) {
  ...
  private void dataArrived(
      JobResultsRequest jobResultsRequest,
      JobResultsRequestWrapper jobResultsRequestWrapper,
      ResponseSender sender) {
    Preconditions.checkArgument(
        jobResultsRequest == null ^ jobResultsRequestWrapper == null,
    ...
    if (mustForwardRequest(foreman)) {
      ...
      logger.warn(
          "User data arrived post query termination from {}, dropping. Data was from "
              + "QueryId: {}.",
          foreman.getAddress(),
          queryId);
      sender.sendFailure(
          new UserRpcException(
              foreman,
              "Failed to forward job results request",
  ...
  private void forwardRequest(
      ...
      CoordinationProtos.NodeEndpoint foreman) {
    logger.info(
        "Forwarding dataArrived request for Query {} to target {}", queryId, foreman.getAddress());

    if (jobResultsRequest != null) {
      logger.debug("forwarding request.");
      StreamObserver<JobResultsRequest> streamObserver =
          resultForwardingStreams.computeIfAbsent(
              queryId, k -> getJobResultsRequestStreamObserver(foreman, sender, queryId));
      ...
      StreamObserver<JobResultsRequestWrapper> streamObserver =
          resultForwardingStreams.computeIfAbsent(
              queryId, k -> getJobResultsRequestWrapperStreamObserver(foreman, sender, queryId));
  ...
  private StreamObserver<JobResultsRequest> getJobResultsRequestStreamObserver(
      CoordinationProtos.NodeEndpoint foreman, ResponseSender sender, String queryId) {
    final JobResultsServiceGrpc.JobResultsServiceStub stub =
        getJobResultsServiceStub(foreman, queryId);
    Pointer<StreamObserver<JobResultsRequest>> streamObserverInternal = new Pointer<>();
    Context.current()
        .fork()
        .run(
            () -> {
              streamObserverInternal.value =
                  stub.jobResults(getResponseObserver(foreman, sender, queryId));
            });
   ...
   */
  private StreamObserver<JobResultsRequestWrapper> getJobResultsRequestWrapperStreamObserver(
      CoordinationProtos.NodeEndpoint foreman, ResponseSender sender, String queryId) {
    final JobResultsServiceGrpc.JobResultsServiceStub stub =
        getJobResultsServiceStub(foreman, queryId);
            ...
            () -> {
              ...
              streamObserverInternal.value =
                  ClientCalls.asyncBidiStreamingCall(
                      clientCall, getResponseObserver(foreman, sender, queryId));
            });
    return streamObserverInternal.value;
  }

  private JobResultsServiceGrpc.JobResultsServiceStub getJobResultsServiceStub(
      CoordinationProtos.NodeEndpoint foreman, String queryId) {
  ...
  private StreamObserver<JobResultsResponse> getResponseObserver(
      CoordinationProtos.NodeEndpoint foreman, ResponseSender sender, String queryId) {
    return new StreamObserver<JobResultsResponse>() {
      ...
      public void onError(Throwable t) {
        try {
          logger.error(
              "Failed to forward job results request to {} for queryId {}",
              foreman.getAddress(),
              queryId,
              t);
          ...
          sender.sendFailure(
              new UserRpcException(foreman, "Failed to forward job results request", t));
        } catch (IllegalStateException e) {
          // if the "sender" has illegal state, then no point of sending ack. So ignore the
          // exception.
