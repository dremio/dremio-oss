...
public class HiveDeltaLakeScanPrel extends DeltaLakeScanPrel {
  public HiveDeltaLakeScanPrel(RelOptCluster cluster, RelTraitSet traitSet, RelOptTable table,
    ...
    super(cluster, traitSet, table, pluginId, tableMetadata, projectedColumns, observedRowcountAdjustment, hints, filter, rowGroupFilter, arrowCachingEnabled, pruneCondition);
  }

  @Override
  public RelNode copy(RelTraitSet traitSet, List<RelNode> inputs) {
    return new HiveDeltaLakeScanPrel(getCluster(), getTraitSet(), getTable(), getPluginId(), getTableMetadata(),
      getProjectedColumns(), getObservedRowcountAdjustment(), hints, getFilter(), getRowGroupFilter(),
      isArrowCachingEnabled(), getPruneCondition());
  }

  @Override
  public ScanRelBase cloneWithProject(List<SchemaPath> projection) {
    return new HiveDeltaLakeScanPrel(getCluster(), getTraitSet(), getTable(), getPluginId(), getTableMetadata(),
      projection, getObservedRowcountAdjustment(), hints, getFilter() == null ? null : getFilter().applyProjection(projection, rowType, getCluster(), getBatchSchema()),
      getRowGroupFilter() == null ? null : getRowGroupFilter().applyProjection(projection, rowType, getCluster(), getBatchSchema()),
      isArrowCachingEnabled(), getPruneCondition() == null ? null : getPruneCondition().applyProjection(projection, rowType, getCluster(), getBatchSchema()));
  }
