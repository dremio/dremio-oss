...
/**
 * TODO: Currently pivots created here process the entire input batch. In this process they may
 * expand the output buffers. We are adding version of pivot function that process only a subset of
 * records that can fit in existing output buffers. We may end up deleting this once HashAgg or
 * HashJoin spilling is complete.
 */
public class Pivots {
  ...
  private static void pivotVariableLengths(
    ...
    targetVariable.ensureAvailableDataSpace(
        ((int) totalData) + 4 * fields.size() * count + (LBlockHashTable.VAR_LENGTH_SIZE * count));
  ...
  static void pivotBit(VectorPivotDef def, FixedBlockVector fixedBlock, final int count) {
  ...
  static void pivot4Bytes(VectorPivotDef def, FixedBlockVector fixedBlock, final int count) {
    final FieldVector field = def.getIncomingVector();
    final List<ArrowBuf> buffers = field.getFieldBuffers();
    ...
    // NB: DX-12276, DX-122458 workaround: explicitly using longs in the arithmetic below to avoid
    // crash in the C2 JIT compiler
    ...
    while (srcDataAddr < finalWordAddr) {
      ...
      if (bitValues == NONE_SET) {
        ...
        // all set, set the bit values using a constant AND. Independently set the data values
        ...
        for (int i = 0;
            i < WORD_BITS;
            i++, valueTargetAddr += blockLength, srcDataAddr += fourByteL) {
  ...
  static void pivot8Bytes(VectorPivotDef def, FixedBlockVector fixedBlock, final int count) {
    final FieldVector field = def.getIncomingVector();
    final List<ArrowBuf> buffers = field.getFieldBuffers();

    Preconditions.checkArgument(
        buffers.size() == 2,
        "A eight byte vector should have two field buffers. %s has %s buffers.",
        Describer.describe(field.getField()),
        buffers.size());
    ...
    // NB: DX-12276 workaround: explicitly using longs in the arithmetic below to avoid crash in the
    // C2 JIT compiler
    final long wordBitsL = WORD_BITS;
    ...
    while (srcDataAddr < finalWordAddr) {
      ...
      if (bitValues == NONE_SET) {
        ...
        for (int i = 0;
            i < WORD_BITS;
            i++, valueTargetAddr += blockLength, srcDataAddr += eightByteL) {
  ...
  static void pivot16Bytes(VectorPivotDef def, FixedBlockVector fixedBlock, final int count) {
    final FieldVector field = def.getIncomingVector();
    final List<ArrowBuf> buffers = field.getFieldBuffers();

    Preconditions.checkArgument(
        buffers.size() == 2,
        "A sixteen byte vector should have two field buffers. %s has %s buffers.",
        Describer.describe(field.getField()),
        buffers.size());
    ...
    // NB: DX-12276, DX-12458 workaround: explicitly using longs in the arithmetic below to avoid
    // crash in the C2 JIT compiler
    final long wordBitsL = WORD_BITS;
    final long eightByteL = EIGHT_BYTE;
    ...
    while (srcDataAddr < finalWordAddr) {
      ...
      if (bitValues == NONE_SET) {
        ...
        for (int i = 0;
            i < WORD_BITS;
            i++, valueTargetAddr += blockLength, srcDataAddr += sixteenByteL) {
          PlatformDependent.putLong(valueTargetAddr, PlatformDependent.getLong(srcDataAddr));
          PlatformDependent.putLong(
              valueTargetAddr + eightByteL, PlatformDependent.getLong(srcDataAddr + eightByteL));
        }
        ...
        for (int i = 0;
          ...
          PlatformDependent.putLong(
              valueTargetAddr, PlatformDependent.getLong(srcDataAddr) * bitVal);
          PlatformDependent.putLong(
              valueTargetAddr + eightByteL,
              PlatformDependent.getLong(srcDataAddr + eightByteL) * bitVal);
        }
      }
      srcBitsAddr += wordBytesL;
    }
