...
public class HashJoinOperator implements DualInputOperator {
  ...
  public static final BooleanValidator ENABLE_SPILL =
      new BooleanValidator("exec.op.join.spill", true);

  public static final LongValidator SPILL_ENABLE_THRESHOLD =
      new LongValidator("exec.op.join.spill.threshold", 20_000_000);

  public static final PowerOfTwoLongValidator NUM_PARTITIONS =
      new PowerOfTwoLongValidator("exec.op.join.spill.num_partitions", 16, 8);
  // The default batch size is 256K. Making this smaller than that value will cause a proportionate
  // increase in the
  ...
  public static final PositiveLongValidator PAGE_SIZE =
      new TypeValidators.PositiveLongValidator(
          "exec.op.join.spill.page_size", Integer.MAX_VALUE, 256 * 1024);

  // For unit tests, always use with DEBUG flag only.
  public static final StringValidator TEST_SPILL_MODE =
      new StringValidator("exec.op.join.spill.test_spill_mode", "none");
  ...
  private static final GeneratorMapping PROJECT_BUILD =
      GeneratorMapping.create("doSetup", "projectBuildRecord", null, null);
  ...
  private static final GeneratorMapping PROJECT_PROBE =
      GeneratorMapping.create("doSetup", "projectProbeRecord", null, null);
  ...
  public HashJoinOperator(OperatorContext context, HashJoinPOP popConfig)
  ...
  private void setupHashTable()
      throws IOException, SchemaChangeException, ClassTransformationException {
    ...
    for (int i = 0; i < conditionsSize; i++) {
      rightExpr.add(
          new NamedExpression(conditions.get(i).getRight(), new FieldReference("build_side_" + i)));
      leftExpr.add(
          new NamedExpression(conditions.get(i).getLeft(), new FieldReference("probe_side_" + i)));
    }
  ...
  public HashJoinProbe setupHashJoinProbe() throws ClassTransformationException, IOException {
    final CodeGenerator<HashJoinProbe> cg =
        context.getClassProducer().createGenerator(HashJoinProbe.TEMPLATE_DEFINITION);
    ...
    for (final Field field : right.getSchema()) {
      final JVar inVV =
          g.declareVectorValueSetupAndMember(
              "buildBatch", new TypedFieldId(CompleteType.fromField(field), true, fieldId));
      final JVar outVV =
          g.declareVectorValueSetupAndMember(
              "outgoing", new TypedFieldId(CompleteType.fromField(field), false, fieldId));
      g.getEvalBlock()
          .add(
              outVV
                  .invoke("copyFromSafe")
                  .arg(
                      JExpr.cast(
                          g.getModel().INT, buildIndex.band(JExpr.lit((int) Character.MAX_VALUE))))
                  .arg(outIndex)
                  .arg(
                      inVV.component(
                          JExpr.cast(g.getModel().INT, buildIndex.shrz(JExpr.lit(16))))));
    ...
    for (final Field field : left.getSchema()) {
      final JVar inVV =
          g.declareVectorValueSetupAndMember(
              "probeBatch", new TypedFieldId(CompleteType.fromField(field), false, fieldId));
      final JVar outVV =
          g.declareVectorValueSetupAndMember(
              "outgoing", new TypedFieldId(CompleteType.fromField(field), false, outputFieldId));
  ...
  private void setCurrentIndex(
      final int hashTableIndex, final int buildBatch, final int incomingRecordIndex)
    ...
    final long startIndexMemStart =
        startIndex.memoryAddress() + hashTableOffset * BUILD_RECORD_LINK_SIZE;
    ...
    if (linkBatch == INDEX_EMPTY) {
      ...
      final long firstLinkMemStart =
          firstLink.memoryAddress() + hashTableOffset * BUILD_RECORD_LINK_SIZE;
      ...
      if (firstLinkBatch == INDEX_EMPTY) {
        ...
        final long nextLinkMemStart =
            nextLink.memoryAddress() + incomingRecordIndex * BUILD_RECORD_LINK_SIZE;
  ...
  public static class Creator implements DualInputOperator.Creator<HashJoinPOP> {
    @Override
    public DualInputOperator create(OperatorContext context, HashJoinPOP config)
