  ...
  public static PivotDef getBlockDefinition(Iterable<FieldVectorPair> fieldVectors) {
    ...
    for (FieldVectorPair v : fieldVectors) {
      ...
      switch (type.toMinorType()) {
        case BIT:
          // Would be too complicated to deal with the case where the valid bit is in one 4byte
          // word, and the value is in the next 4byte word
          ...
          defs.add(
              new VectorPivotDef(
                  FieldType.BIT,
                  (bitOffset >>> BITS_TO_FOUR_BYTES) << FOUR_BYTES_TO_BYTES,
                  bitOffset & BIT_OFFSET_MASK,
                  (bitOffset + 1) & BIT_OFFSET_MASK,
                  v));
          bitOffset += 2;
          break;
        ...
        default:
          throw new UnsupportedOperationException(
              "Unable to build table for field: " + Describer.describe(v.getIncoming().getField()));
      }
    }
    ...
    // rewrite field defs to a new list that adds the varlength, allBitsWidth, and VARLENGTH_ADDRESS
    // in bytes to the fixed offset to get the block offset.
    final int nullShift = 0;
    final int valueShift = allBitsWidthInBytes;
    final ImmutableList<VectorPivotDef> shiftedDefs =
        FluentIterable.from(defs)
            .transform(
                new Function<VectorPivotDef, VectorPivotDef>() {
                  @Override
                  public VectorPivotDef apply(VectorPivotDef input) {
                    if (input.getType().mode == FieldMode.BIT
                        || input.getType().mode == FieldMode.VARIABLE) {
  ...
  public static PivotInfo getBlockInfo(Iterable<FieldVector> fieldVectors) {
    ...
    for (FieldVector v : fieldVectors) {
      ...
      switch (type.toMinorType()) {
        ...
        default:
          throw new UnsupportedOperationException(
              "Unable to build table for field: " + Describer.describe(v.getField()));
      }
    }
