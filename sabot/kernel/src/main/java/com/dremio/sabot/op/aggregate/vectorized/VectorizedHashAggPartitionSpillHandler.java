...
/**
 * {@link VectorizedHashAggOperator} uses an instance of this class to manage spilling of partitions
 * as and when decided by the operator. Currently we have the basic infrastructure to spill/read
 * partitions but this needs to be augmented further in the next iteration of code when recursion
 * algorithm is complete.
 *
 * <p>We maintain two different data structures for managing spilled partitions. The main reason is
 * to optimize algorithms like:
 *
 * <p>-- chooseVictimPartition() -- transitionPartitionState() --
 * spillAnyInMemoryDataForSpilledPartitions()
 *
 * <p>Not only optimization, maintaining two data structures is somewhat essential to ensure
 * correctness of state machine in {@link VectorizedHashAggOperator}
 *
 * <p>Here is how the two data structures are used for spilled partitions:
 *
 * <p>(1) activeSpilledPartitions --
 *
 * <p>An {@link ArrayList} of {@link VectorizedHashAggDiskPartition}. This contains the partitions
 * spilled in the current iteration of hash aggregation algorithm being executed by {@link
 * VectorizedHashAggOperator}. The number of partitions in this list should never exceed the list of
 * partitions operator is working with. This list identifies spilled partitions from the set of
 * fixed number of partitions being used by the operator for hash aggregation. The reason they are
 * called as active spilled partitions is because they are being actively used by the operator for
 * incoming data distribution and aggregation but during the processing they may get spilled.
 *
 * <p>This list is not used for processing spilled partitions. Once an iteration is over, we add all
 * the partitions from this list to the FIFO queue of spilled partitions and clear this list to
 * prepare for next iteration.
 *
 * <p>Maintaining a second list helps us to uniquely identify the set of partitions that have been
 * spilled in the current iteration (active and spilled) of hash aggregation without being mixed
 * with "full disk" based spilled partitions.
 *
 * <p>This becomes necessary as recursion goes deeper and deeper. If we don't do this, then the
 * performance of above mentioned 3 functions will suffer as we needlessly will have to go through
 * several spilled partitions only to skip them.
 *
 * <p>We sometimes refer to these partitions as "partitions spilled in current iteration".
 *
 * <p>(2) spilledPartitions --
 *
 * <p>A FIFO {@link Queue} of {@link VectorizedHashAggDiskPartition}. The sole purpose is to process
 * the spilled partitions. These are fully "disk-based" partitions. In other words,
 */
public class VectorizedHashAggPartitionSpillHandler implements AutoCloseable {
  private static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(VectorizedHashAggPartitionSpillHandler.class);
  ...
  public VectorizedHashAggPartitionSpillHandler(
    ...
    this.spillManager =
        new SpillManager(sabotConfig, optionManager, id, null, spillService, "agg spilling", stats);

    Preconditions.checkArgument(
        loadingPartition != null, "Error: need a valid handle for loading partition");
  ...
  /**
   * Pick a victim partition to spill. We use the following strategy to choose the victim partition:
   *
   * <p>(1) Go over active spilled partitions: This effectively implies going over the memory
   * portion of these partitions and consider the partitions larger than certain threshold. Select
   * the partition with maximum size.
   *
   * <p>(2) The above step may fail to select a victim partition because either there are no spilled
   * partitions in the current iteration or none of them is greater than threshold. So now just go
   * over all partitions and pick the one with highest memory usage
   *
   * <p>Let's take an example:
   *
   * <p>Say we start with 4 partitions -- p0, p1, p2, p3
   *
   * <p>State is:
   *
   * <p>activePartitions: [p0, p1, p2, p3] activeSpilledPartitions: [] spilledPartitions: []
   *
   * <p>Some time later we decide to spill. Since there are no active spilled partitions, step (1)
   * returns null and we basically look at memory consumption of each of these partitions and spill
   * the one with highest memory usage.
   *
   * <p>Say p0 is the chosen one. State is:
   *
   * <p>activePartitions: [p0, p1, p2, p3] activeSpilledPartitions: [p0] spilledPartitions: []
   *
   * <p>aggregation proceeds further and we decide to spill again. now we look at active spilled
   * partitions [p0] and see what is the memory usage of its memory portion. if it is larger than
   * threshold, we spill it.
   *
   * <p>otherwise, we look at [p0, p1, p2, p3] and [p0] and pick the partition with highest memory
   * usage. it is evident that looking at second list is redundant and we should just look at [p0,
   * p1, p2, p3]. however, it is important if we are in the stage of processing spilled partitions.
   *
   * <p>Say p1 is chosen.
   *
   * <p>State is:
   *
   * <p>activePartitions: [p0, p1, p2, p3] activeSpilledPartitions: [p0, p1] spilledPartitions:[]
   *
   * <p>say all incoming data is consumed. we do the following
   *
   * <p>(1) output p2, p3. (2) flush in-memory (if any) portions of p0 and p1. (3) transition state
   * for both sets of partitions which also involves adding the set of spilled partitions in current
   * iteration to the global queue of spilled partitions and emptying the former.
   *
   * <p>State is:
   *
   * <p>activePartitions: [p0, p1, p2, p3] -- operator's set of in-memory partitions downsized to
   * minimum preallocation and absolutely no association to spilled partitions
   * activeSpilledPartitions: [] spilledPartitions: [p0, p1]
   *
   * <p>Start processing p0 from disk. This requires removing p0 from the queue of spilled
   * partitions and updating names of operator's partitions.
   *
   * <p>State is:
   *
   * <p>activePartitions: [p0.p0, p0.p1, p0.p2, p0.p3] activeSpilledPartitions: []
   * spilledPartitions: [p1]
   *
   * <p>The disk iterator for p0 will get to incoming batches and repartition into p0.p0, p0.p1,
   * p0.p2 and p0.p3.
   *
   * <p>We decide to spill:
   *
   * <p>step (1) scan activeSpilledPartitions which is empty since in this iteration we haven't
   * spilled any partition yet.
   *
   * <p>step (2) scan activePartitions [p0.p0, p0.p1, p0.p2, p0.p3] and activeSpilledPartitions []
   * and pick the one with highest memory usage.
   *
   * <p>Say we choose p0.p3.
   *
   * <p>State is:
   *
   * <p>activePartitions: [p0.p0, p0.p1, p0.p2, p0.p3] activeSpilledPartitions: [p0.p3]
   * spilledPartitions: [p1]
   *
   * <p>say all incoming data is consumed from p0. we do the following
   *
   * <p>(1) output p0.p0, p0.p1 and p0.p2 (2) flush in-memory (if any) portions of p0.p3. (3)
   * transition state for both sets of partitions which also involves adding the set of spilled
   * partitions in current iteration to the global queue of spilled partitions.
   *
   * <p>State is:
   *
   * <p>activePartitions: [p0, p1, p2, p3] activeSpilledPartitions: [] spilledPartitions: [p1,
   * p0.p3]
   *
   * <p>The process continues
   *
  ...
  /**
   * Go over the set of spilled partitions and get the partition with highest memory usage and
   * beyond minimum allocation.
   *
   * <p>Our goal is to keep the number of unique spilled partitions to as low as possible in order
   * to maximize the number of partitions that are always in memory. so first we target the set of
   * active spilled partitions (if any) to see if there is a candidate partition with memory usage
   * greater than minimum preallocation and largest amongst all active spilled partitions. this also
   * means that as long as there is one spilled partition with memory usage beyond minimum
   * preallocation, we will choose it and this may come at the expense of ignoring a non-spilled
   * partition that has higher memory usage and thus potential to release more memory.
   *
   * <p>In general, this approach helps since it tends to prevent situations where all partitions
   * are spilled. But we may see a slightly higher number of OOMs. For testing/debugging purposes,
   * we have made the algorithm configurable where we can disable the first part of selection
   * algorithm and just directly look at all partition (spilled or not-spilled) and choose the one
   * with highest memory usage.
   */
   ...
   * memory usage beyond minimum preallocation, there is no need to check the number of blocks in
   * the partition.
   *
  ...
  /**
   * This method is used by {@link VectorizedHashAggOperator} to spill a partition. During its life
   * cycle, operator may decide to spill at some points and invokes this method.
   *
   * <p>Here we choose a victim partition and spill it.
   *
   * @throws Exception
   */
  public void spillPartition(final VectorizedHashAggPartition victimPartition) throws Exception {
    ...
    final VectorizedHashAggPartitionSerializable partitionSerializable =
    ...
    updatePartitionSpillState(
        victimPartition, partitionSpillFile, partitionSpillFileStream, batchesSpilled);
    ...
    if (!isPartitionSpilled) {
      logger.debug(
          "Successfully spilled a partition: {} for the first time, batches spilled: {}, spill file path: {}, active spill partition count: {}, spill queue size: {}",
      ...
      logger.debug(
          "Successfully spilled a partition: {}, batches spilled: {}, spill file path: {}, pactive spill partition count: {}, spill queue size: {}",
          victimPartition.getIdentifier(),
          batchesSpilled,
          partitionSpillFile.getPath(),
          activeSpilledPartitions.size(),
  ...
  /**
   * Once we spill a partition (entire or single batch), we track the spill state of the partition
   *
   ...
   */
  private void updatePartitionSpillState(
      final VectorizedHashAggPartition victimPartition,
    ...
    if (!isPartitionSpilled) {
       ...
       */
      partitionSpillInfo =
          new VectorizedHashAggDiskPartition(
              batchesSpilled, partitionSpillFile,
  ...
  private static class SpillFileHandle {
    ...
    SpillFileHandle(
        final SpillFile partitionSpillFile, final FSDataOutputStream partitionSpillFileStream) {
  ...
  private SpillFileHandle getSpillFileHandle(final VectorizedHashAggPartition victimPartition) {
    ...
    Preconditions.checkState(
        (isPartitionSpilled && partitionSpillInfo != null)
            || (!isPartitionSpilled && partitionSpillInfo == null),
        "Error: Detected invalid spill state of hash agg partition");
   ...
   */
  boolean spillSingleBatchFromPartition(final VectorizedHashAggPartition victimPartition)
      throws Exception {
    final SpillFileHandle spillFileHandle = getSpillFileHandle(victimPartition);
    final SpillFile partitionSpillFile = spillFileHandle.partitionSpillFile;
    final FSDataOutputStream partitionSpillFileStream = spillFileHandle.partitionSpillFileStream;
  ...
  /**
   * When a partition is spilled, it is spilled in 1 or more batches and each batch has 1 or more
   * records. We store this information here to be used later by the operator for stats.
   *
  ...
  private void updateLocalStats(
      final long batchesSpilled, final long recordsSpilled, final long spilledDataSize) {
  ...
  /**
   * Get the maximum number of batches spilled across all spills done by the operator.
   *
   * @return maximum batches spilled.
   *     <p>Note: these batches are the hashtable (and accumulator) blocks/batches and not
   *     operator's incoming batches.
   */
  ...
  /**
   * Get the total number of batches spilled across all spills done by the operator.
   *
  ...
  /**
   * Get the maximum number of records spilled across all spills done by the operator.
   *
  ...
  /**
   * Get the total number of records spilled across all spills done by the operator.
   *
  ...
  /**
   * Get the maximum size (in bytes) of data spilled by the operator across all spill cycles. We
   * compute the total length of data spilled in a single spill cycle and a global maximum is
  ...
  /**
   * Get the total size (in bytes) of data spilled by the operator. Every time operator spills a
   * partition, we compute the total length of buffers spilled. A global counter is used to track
   * cumulative length of data spilled across all spill cycles
   *
  ...
  /**
   * Get the number of spilled partitions in the current iteration of aggregation algorithm --
   * active and spilled
   *
  ...
  /**
   * Get the number of "disk based" partitions. In other words, get the size of FIFO queue used to
   * process the spilled partitions.
   *
  ...
  /**
   * When all the input data has been consumed, operator starts the recursion for aggregation
   * algorithm by starting to process one spilled partition at a time. This involves reading spilled
   * partition's data (batch by batch), using it as new input to be fed into the operator and
   * re-partitioned into same number of partitions. Before we do this, transition the state of two
   * sets of partitions such that they no longer refer to each other.
   *
   * <p>The partitions managed by operator become "MEMORY ONLY" partitions. In other words, they no
   * longer hold references to their disk-based counterparts. The spilled partitions managed by this
   * module become "DISK ONLY" partitions. In other words, they no longer hold references to their
   * in-memory counterparts.
   *
   * <p>Doing this allows the recursion algorithm to treat the operator's partition list as fresh
   * set of in-memory partitions into which spilled batch's data can be repartitioned.
   */
  public void transitionPartitionState() {
     ...
     */
    Preconditions.checkArgument(
        activeSpilledPartitions.size() > 0 || spilledPartitions.size() > 0,
    ...
    while (iterator.hasNext()) {
      ...
      final VectorizedHashAggPartition inmemoryPartition =
          spilledPartition.getInmemoryPartitionBackPointer();
      Preconditions.checkArgument(
          inmemoryPartition != null, "Error: inconsistent state of spilled partition");
     ...
     */
    Preconditions.checkArgument(
        activeSpilledPartitions.size() == 0, "inconsistent state of active spilled partition list");
    Preconditions.checkArgument(
        spilledPartitions.size() == size1 + size2, "inconsistent state of spilled partition queue");
  }

  /**
   * This is used by the operator to get the next spilled partition to process. When {@link
   * VectorizedHashAggOperator} processes a spilled partition from disk by reading spilled batches,
   * it doesn't consume all the spilled batches at one go. Instead, a single batch is consumed by
   * the operator from spill file and control is returned to Smart OP. Upon the next entry into
   * outputData, the state machine determines if a spilled partition needs to be processed and if
   * that is the case, operator invokes this method to either get a brand new disk iterator for a
   * spilled partition or an existing open disk iterator for a spilled partition whose one or more
   * batches have already been processed.
   *
  ...
  public SpilledPartitionIterator getNextSpilledPartitionToProcess() throws Exception {
    Preconditions.checkArgument(
        spilledPartitions.size() > 0 || spilledPartitionIterator != null,
        "Error: Nothing to process from disk. Not allowed to get disk iterator");
    if (spilledPartitionIterator == null) {
      logger.debug(
          "Starting to process spilled partition from FIFO queue, spill queue size:{}",
          spilledPartitions.size());
      this.spilledPartitionIterator =
          new SpilledPartitionIterator(spilledPartitions.remove(), this.operatorStats);
    } else {
      Preconditions.checkArgument(
          spilledPartitionIterator.getCurrentBatchIndex() > 0,
          "Error: At least one batch should have already been read by an open disk iterator");
      logger.debug(
          "Returning an existing iterator for spilled partition: {}, spill queue size:{}",
          spilledPartitionIterator.getIdentifier(),
          spilledPartitions.size());
    }
  ...
  public void closeSpilledPartitionIterator() throws Exception {
    Preconditions.checkArgument(
        spilledPartitionIterator != null, "Error: expecting a valid disk iterator");
    spilledPartitionIterator.close();
    spilledPartitionIterator = null;
  }

  /**
   * Called by {@link VectorizedHashAggOperator} to ensure all spilled partitions are entirely on
   * disk. This is needed before the operator starts pumping out data from in-memory partitions.
   *
   * <p>The reason we do this is because once a partition has been spilled, it is very likely that
   * there would be subsequent incoming data belonging to the spilled partition. We will continue to
   * do the in-memory aggregation (contraction) or buffering (non-contraction). The partition may or
   * may not get spilled again. However, once all the input data has been processed by the operator,
   * spilled partition(s) could be holding data in-memory and we need to spill/flush this data too.
   *
   * @throws Exception
   */
  public void spillAnyInMemoryDataForSpilledPartitions() throws Exception {
    ...
    for (VectorizedHashAggDiskPartition partitionToSpill : activeSpilledPartitions) {
       ...
       */
      final VectorizedHashAggPartition inmemoryPartition =
          partitionToSpill.getInmemoryPartitionBackPointer();
      final SpillFile partitionSpillFile = partitionToSpill.getSpillFile();
      final VectorizedHashAggPartitionSerializable partitionSerializable =
          new VectorizedHashAggPartitionSerializable(
              inmemoryPartition, this.operatorStats, this.warnMaxSpillTime);
      ...
      logger.debug(
          "Flushed in-memory data for partition: {}, batches spilled: {}, spill file path: {}",
          inmemoryPartition.getIdentifier(),
          batchesSpilled,
          partitionSpillFile.getPath());
      outputStream.flush();
  ...
  /**
   * This iterator is created by {@link VectorizedHashAggOperator} to start processing spilled data
   * of a partition. This happens when the entire initial input data into the operator has been
   * consumed, the operator has pumped output from MEMORY-ONLY partitions and starts recursion for
   * spilled partitions.
   *
   * <p>We iterate over the spilled batches and return one spilled batch at a time which is then
   * used by the operator to feed as new input into aggregation algorithm.
   */
  public class SpilledPartitionIterator implements AutoCloseable {
    ...
    SpilledPartitionIterator(
        final VectorizedHashAggDiskPartition spilledPartition, final OperatorStats stats)
        throws Exception {
      Preconditions.checkArgument(
          spilledPartition != null,
          "Error: need a valid partition handle to create a disk iterator");
      Preconditions.checkArgument(
          spilledPartition.getNumberOfBatches() > 0,
      ...
      this.operatorStats = stats;
      logger.debug(
          "Created disk iterator for spilled partition: {}, batches to read: {}, spill file: {}",
          diskPartition.getIdentifier(),
          batchCount,
          partitionSpillFile.getPath());
    }

    /**
     * After creating the disk iterator, this method is used by {@link VectorizedHashAggOperator} to
     * get the next batch from spilled data. The batch will act as input into the operator.
     *
    ...
    public int getNextBatch() throws Exception {
      ...
      logger.debug(
          "Reading spilled batch:{} for partitions:{}",
          currentBatchIndex,
          diskPartition.getIdentifier());
      final VectorizedHashAggPartitionSerializable partitionSerializable =
          new VectorizedHashAggPartitionSerializable(
              loadingPartition, this.operatorStats, warnMaxSpillTime);
  ...
  /** Dump information about all spilled partitions. Function can be useful for debugging. */
  public void dumpSpilledPartitionInfo() {
    ...
    while (iterator.hasNext()) {
      ...
      logger.debug(
          "Spilled partition info, ID:{}, batches:{}, file:{}", name, batches, spillFile.getPath());
    }
  }
  ...
  VectorizedHashAggDiskPartition getActiveSpilledPartition(final int index) {
    Preconditions.checkArgument(
        index >= 0 && index < activeSpilledPartitions.size(),
        "Error: invalid active spilled partition index");
  ...
  public SpilledPartitionIterator getActiveSpilledPartition() throws Exception {
    Preconditions.checkArgument(
        activeSpilledPartitions.size() == 1,
