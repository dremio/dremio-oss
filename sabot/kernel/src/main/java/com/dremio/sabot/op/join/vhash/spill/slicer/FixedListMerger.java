...
/**
 * A {@link Merger} implementation for fixed size list Arrow vectors {@link FixedSizeListVector}
 ...
 *
 * This merger provides functionality of merging together data of multiple FixedSizeListVector vectors into one, it merges
 * validity buffers as well as data vectors.
 *
...
public class FixedListMerger implements Merger {
  ...
  public FixedListMerger(FixedSizeListVector vector, final int wrapperIdx, final BufferAllocator allocator) {
  ...
  public void merge(final VectorContainerList srcContainers, final Page dst, final List<FieldVector> vectorOutput) {
    ...
    final FixedSizeListVector outgoing = (FixedSizeListVector) vectorListToBeMerged.get(0).getTransferPair(allocator).getTo();
