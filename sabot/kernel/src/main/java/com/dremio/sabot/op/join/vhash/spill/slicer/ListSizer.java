...
/**
 * A {@link Sizer} implementation for variable size list Arrow vectors {@link ListVector} A
 * ListVector's children can be variable sized lists of primitive or complex types. This Sizer
 * provides support for copier related operations on ListVector.
 *
 * <p>Arrow vector layout types including variable size list vector documented at -
    ...
    return (page) -> {
      ...
      try (final ArrowBuf validityBuf = page.sliceAligned(validityLen);
        ...
        outgoing.loadFieldBuffers(
            new ArrowFieldNode(numberOfRecords, -1), ImmutableList.of(validityBuf, offsetBuf));
