  ...
  public static class IntMaxAccumulatorNoSpill extends BaseSingleAccumulatorNoSpill {
    ...
    public void accumulate(final long memoryAddr, final int count) {
      ...
      for (long ordinalAddr = memoryAddr;
          ordinalAddr < maxMemAddr;
          ordinalAddr += 4, incomaxgIndex++) {
        ...
        final int bitVal =
            (PlatformDependent.getByte(incomaxgBit + ((incomaxgIndex >>> 3)))
                    >>> (incomaxgIndex & 7))
                & 1;
        ...
        if (holder.isSet == 0) {
          ...
          final int cmp =
              ByteFunctionHelpers.compare(
                  inputBuf, startOffset, endOffset, holder.buffer, holder.start, holder.end);
          swap = (cmp == 1);
        }
  ...
  public static class FloatMaxAccumulatorNoSpill extends BaseSingleAccumulatorNoSpill {
    ...
    public void accumulate(final long memoryAddr, final int count) {
      ...
      for (long ordinalAddr = memoryAddr;
          ...
          ordinalAddr += 4, incomaxgIndex++) {
        final float newVal =
            Float.intBitsToFloat(PlatformDependent.getInt(incomaxgValue + (incomaxgIndex * WIDTH)));
        final int tableIndex = PlatformDependent.getInt(ordinalAddr);
        int chunkIndex = tableIndex >>> LBlockHashTableNoSpill.BITS_IN_CHUNK;
        int chunkOffset = tableIndex & LBlockHashTableNoSpill.CHUNK_OFFSET_MASK;
        final long maxAddr = getValueAddress(chunkIndex) + (chunkOffset * 4);
      ...
      for (long ordinalAddr = memoryAddr;
        ...
        final double newVal =
            Double.longBitsToDouble(
                PlatformDependent.getLong(incomaxgValue + (incomaxgIndex * WIDTH)));
        final int tableIndex = PlatformDependent.getInt(ordinalAddr);
        int chunkIndex = tableIndex >>> LBlockHashTableNoSpill.BITS_IN_CHUNK;
        int chunkOffset = tableIndex & LBlockHashTableNoSpill.CHUNK_OFFSET_MASK;
        final long maxAddr = getValueAddress(chunkIndex) + (chunkOffset * 8);
    ...
    public void accumulate(final long memoryAddr, final int count) {
      ...
      for (long inputAddr = incomingValue, inputBitAddr = incomingBit, batchCount = 0;
        ...
        for (long bitNum = 0;
          ...
          // Because the operations are all done in a word length (and not on an individual bit),
          // the AND value for
