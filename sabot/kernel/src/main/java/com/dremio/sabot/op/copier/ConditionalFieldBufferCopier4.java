...
/**
 * Vectorized copiers for 4 byte offset vectors.
 */
public class ConditionalFieldBufferCopier4 {
  ...
  private ConditionalFieldBufferCopier4(){};
    ...
    public void copy(long sv4Addr, int count) {
      ...
      for(; sv4Addr < maxSV4; sv4Addr += STEP_SIZE, dstOffsetAddr += 4){
        ...
        if(batchNOFF == SKIP){
          ...
          lastOffset += len;
          PlatformDependent.putInt(dstOffsetAddr, lastOffset);
          com.dremio.sabot.op.common.ht2.Copier.copy(srcDataAddr[batchNumber] + firstOffset, curDataAddr, len);
          curDataAddr += len;
        }
