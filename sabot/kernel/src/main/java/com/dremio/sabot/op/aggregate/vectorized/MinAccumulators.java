...
public class MinAccumulators {
      ...
      for (long partitionAndOrdinalAddr = memoryAddr;
        ...
        PlatformDependent.putInt(
            ...
            Float.floatToIntBits(
                min(Float.intBitsToFloat(PlatformDependent.getInt(minAddr)), newVal, bitVal)));
        PlatformDependent.putInt(
            bitUpdateAddr, PlatformDependent.getInt(bitUpdateAddr) | bitUpdateVal);
      }
    }
  ...
  public static class DoubleMinAccumulator extends BaseSingleAccumulator {
     ...
     *     computed values
     */
    DoubleMinAccumulator(
        final MinAccumulators.DecimalMinAccumulator decimalMinAccumulator,
        final FieldVector input,
        final int maxValuesPerBatch,
        final BufferAllocator computationVectorAllocator) {
      this(
          input,
          decimalMinAccumulator.getOutput(),
      ...
      for (long partitionAndOrdinalAddr = memoryAddr;
        ...
        PlatformDependent.putLong(
            ...
            Double.doubleToLongBits(
                min(Double.longBitsToDouble(PlatformDependent.getLong(minAddr)), newVal, bitVal)));
        PlatformDependent.putInt(
            bitUpdateAddr, PlatformDependent.getInt(bitUpdateAddr) | bitUpdateVal);
      }
    }
    ...
    public void accumulate(
      ...
      for (long partitionAndOrdinalAddr = memoryAddr;
        ...
        PlatformDependent.putLong(
            ...
            Double.doubleToLongBits(
                min(
                    Double.longBitsToDouble(PlatformDependent.getLong(minAddr)),
                    newVal.doubleValue(),
                    bitVal)));
        PlatformDependent.putInt(
            bitUpdateAddr, PlatformDependent.getInt(bitUpdateAddr) | bitUpdateVal);
  ...
  public static class DecimalMinAccumulatorV2 extends BaseSingleAccumulator {
    ...
    public DecimalMinAccumulatorV2(
        FieldVector input,
        FieldVector output,
        FieldVector transferVector,
        int maxValuesPerBatch,
    ...
    public void accumulate(
      ...
      for (long partitionAndOrdinalAddr = memoryAddr;
        ...
        int compare =
            DecimalUtils.compareDecimalsAsTwoLongs(newValHigh, newValLow, curValHigh, curValLow);
  ...
  public static class BitMinAccumulator extends BaseSingleAccumulator {
    ...
    public void accumulate(
      ...
      for (long partitionAndOrdinalAddr = memoryAddr;
          partitionAndOrdinalAddr < maxOrdinalAddr;
          partitionAndOrdinalAddr += PARTITIONINDEX_HTORDINAL_WIDTH) {
        /* get the hash table ordinal */
        final int tableIndex = PlatformDependent.getInt(partitionAndOrdinalAddr + HTORDINAL_OFFSET);
        /* get the index of data in input vector */
        ...
        // Thus, the logical function for updating the accumulator is: oldVal AND (NOT(inputBit) OR
        // inputValue)
        ...
        // NB: ~inputBitVal will set all the bits to 1, with only the LSB set to 0 or 1. Shifting
        // left leaves zeroes
        ...
        final long minAddr =
            valueAddresses[chunkIndex] + ((chunkOffset >>> 5) * 4); // 32-bit read-update-write
        PlatformDependent.putInt(minAddr, PlatformDependent.getInt(minAddr) & minUpdateVal);
        PlatformDependent.putInt(
            minBitUpdateAddr, PlatformDependent.getInt(minBitUpdateAddr) | minBitUpdateVal);
      }
    }
  }
  ...
  public static class IntervalDayMinAccumulator extends BaseSingleAccumulator {
    ...
    public IntervalDayMinAccumulator(
        FieldVector input,
        FieldVector output,
        FieldVector transferVector,
        int maxValuesPerBatch,
    ...
    public void accumulate(
      ...
      for (long partitionAndOrdinalAddr = memoryAddr;
        ...
        // To compare the pairs of day/milli, we swap them, with days getting the most significant
        // bits
        // The incoming value is updated to either be MAX (if incoming is null), or keep as is (if
        // the value is not null)
        final long newSwappedVal =
            ((((long) newDays) << 32) | newMillis) * bitVal + Long.MAX_VALUE * (bitVal ^ 1);
      ...
      for (long partitionAndOrdinalAddr = memoryAddr;
        ...
        if (holder.isSet == 0
            ...
            ByteFunctionHelpers.compare(
                    inputDataBuf, startOffset, endOffset, holder.buffer, holder.start, holder.end)
                < 0) {
  ...
  private static final long min(long a, long b, int bitVal) {
    // update the incoming value to either be the max (if the incoming is null) or keep as is (if
    // the value is not null)
