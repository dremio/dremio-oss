...
public class DenseUnionMerger implements Merger {
  ...
  public DenseUnionMerger(
      DenseUnionVector vector, final int wrapperIdx, final BufferAllocator allocator) {
  ...
  public void merge(
    ...
    final DenseUnionVector outgoing =
        (DenseUnionVector) vectorListToBeMerged.get(0).getTransferPair(allocator).getTo();
    ...
    try (final ArrowBuf typesBuffer = dst.sliceAligned(typesBufferLen);
      ...
      outgoing.loadFieldBuffers(
          new ArrowFieldNode(recordCount, -1), ImmutableList.of(typesBuffer, offsetBuffer));
      ...
      /**
       * Union vector can accommodate 127 possible types. While merging offset buffers together, for
       * each of those types, we need max offset of this type in last vector merged, so we can
       * continue merging from that offset. prevVectorLastPosition provides that last vector offset.
       * currentVectorLastPosition keeps track of last offset of current vector so it can be used
       * for next vector.
       */
      ...
      for (final DenseUnionVector current : vectorListToBeMerged) {
        ...
        prevVectorLastPosition =
            Arrays.copyOf(currentVectorLastPosition, currentVectorLastPosition.length);
        Arrays.fill(currentVectorLastPosition, 0);
      }

      // all data vectors merged separately
      final Map<Byte, List<ValueVector>> ordinalToVectorField =
          getFieldsByTypeId(vectorListToBeMerged, allTypesIds);
      ...
      for (final Map.Entry<Byte, List<ValueVector>> entry : ordinalToVectorField.entrySet()) {
        ...
        final TransferPair transferPair =
            mergedDataVectors.get(0).makeTransferPair(outgoing.getVectorByType(type));
        transferPair.transfer();
      }
    }
  }

  private Map<Byte, List<ValueVector>> getFieldsByTypeId(
      final List<DenseUnionVector> vectorList, final Set<Byte> typeIds) {
