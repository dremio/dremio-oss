...
public class MaxAccumulators {
      ...
      for (long partitionAndOrdinalAddr = memoryAddr;
        ...
        PlatformDependent.putInt(
            ...
            Float.floatToRawIntBits(
                max(Float.intBitsToFloat(PlatformDependent.getInt(maxAddr)), newVal, bitVal)));
        PlatformDependent.putInt(
            bitUpdateAddr, PlatformDependent.getInt(bitUpdateAddr) | bitUpdateVal);
      }
    }
  ...
  public static class DoubleMaxAccumulator extends BaseSingleAccumulator {
     ...
     *     computed values
     */
    DoubleMaxAccumulator(
        final MaxAccumulators.DecimalMaxAccumulator decimalMaxAccumulator,
        final FieldVector input,
        final int maxValuesPerBatch,
        final BufferAllocator computationVectorAllocator) {
      this(
          input,
          decimalMaxAccumulator.getOutput(),
      ...
      for (long partitionAndOrdinalAddr = memoryAddr;
        ...
        PlatformDependent.putLong(
            ...
            Double.doubleToRawLongBits(
                max(Double.longBitsToDouble(PlatformDependent.getLong(maxAddr)), newVal, bitVal)));
        PlatformDependent.putInt(
            bitUpdateAddr, PlatformDependent.getInt(bitUpdateAddr) | bitUpdateVal);
      }
    }
      ...
      for (long partitionAndOrdinalAddr = memoryAddr;
        ...
        PlatformDependent.putLong(
            ...
            Double.doubleToLongBits(
                max(
                    Double.longBitsToDouble(PlatformDependent.getLong(maxAddr)),
                    newVal.doubleValue(),
                    bitVal)));
        PlatformDependent.putInt(
            bitUpdateAddr, PlatformDependent.getInt(bitUpdateAddr) | bitUpdateVal);
  ...
  public static class DecimalMaxAccumulatorV2 extends BaseSingleAccumulator {
    ...
    public DecimalMaxAccumulatorV2(
        FieldVector input,
        FieldVector output,
        FieldVector transferVector,
        int maxValuesPerBatch,
  ...
  public static class BitMaxAccumulator extends BaseSingleAccumulator {
    ...
    public void accumulate(
      ...
      for (long partitionAndOrdinalAddr = memoryAddr;
        ...
        // Thus, the logical function for updating the accumulator is: oldVal OR (inputBit AND
        // inputValue)
        // Thus, the logical function for updating the accumulator is: oldBitVal OR inputBit
        // Because the operations are all done in a word length (and not on an individual bit), the
        // AND value for
        ...
        PlatformDependent.putInt(
            maxBitUpdateAddr, PlatformDependent.getInt(maxBitUpdateAddr) | maxBitUpdateVal);
      }
    }
  }
  ...
  public static class IntervalDayMaxAccumulator extends BaseSingleAccumulator {
    ...
    public IntervalDayMaxAccumulator(
        FieldVector input,
        FieldVector output,
        FieldVector transferVector,
        int maxValuesPerBatch,
