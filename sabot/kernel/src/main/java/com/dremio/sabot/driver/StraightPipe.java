...
class StraightPipe extends Pipe {
  ...
  public VectorAccessible setup() throws Exception {
    final VectorAccessible sourceData =
        upstream == null ? source.accept(new SetupVisitor(), null) : upstream.setup();
    return sink.accept(new SetupVisitor(), sourceData);
  }

  @Override
  public String toString() {
    return MoreObjects.toStringHelper(this).add("sink", sink).add("source", source).toString();
  }
}
