...
/**
 * This implementation of RawBatchBuffer starts writing incoming batches to disk once the buffer
 * size reaches a threshold. The order of the incoming buffers is maintained.
 */
public class SpoolingRawBatchBuffer
    extends BaseRawBatchBuffer<SpoolingRawBatchBuffer.RawFragmentBatchWrapper> {
  private static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(SpoolingRawBatchBuffer.class);
   ...
   *
   * <ul>
   *   <li>available in memory
   *   <li>waiting to be spilled, but still available in memory
   *   <li>spilled to disk
   * </ul>
   */
  ...
  private AtomicReference<SpoolingState> spoolingState =
      new AtomicReference<>(SpoolingState.PAUSE_SPOOLING);
  ...
  public void init() {
    final String name =
        String.format("%s:spoolingBatchBuffer", QueryIdHelper.getFragmentId(handle));
    this.allocator =
        parentAllocator.newChildAllocator(
            name, ALLOCATOR_INITIAL_RESERVATION, ALLOCATOR_MAX_RESERVATION);
  }
  ...
  private void setupOutputStream() {
    try {
      ...
      this.spillManager =
          new SpillManager(
              config, null, id, SPOOLING_CONFIG, spillService, "spooling sorted exchange", null);
  ...
  protected void enqueueInner(RawFragmentBatch batch) {
    ...
    logger.debug(
        "Enqueue batch. Current buffer size: {}. Sending fragment: {}",
        bufferQueue.size(),
    ...
    if (!spoolCurrentBatch && currentBatchesInMemory >= threshold) {
      logger.debug(
          "Buffer size {} greater than threshold {}. Start spooling to disk",
  ...
  protected void upkeep(RawFragmentBatch batch) {
    ...
    final long currentBatchesInMemory =
        body != null ? batchesInMemory.decrementAndGet() : batchesInMemory.get();
    if (isCurrentlySpooling() && currentBatchesInMemory < threshold * STOP_SPOOLING_FRACTION) {
      logger.debug(
          "buffer size {} less than {}x threshold. Stop spooling.",
          currentBatchesInMemory,
          STOP_SPOOLING_FRACTION);
      pauseSpooling();
    }
  ...
  public void close() throws Exception {
    ...
    final AutoCloseable superCloser =
        new AutoCloseable() {
          @Override
          public void close() throws Exception {
            bufferQueue.clear();
    ...
    AutoCloseables.close(
        allocator, outputStream, inputStream, spillFile, this.spillManager, superCloser, deferred);
  }
  ...
  class RawFragmentBatchWrapper {
    ...
    public void writeToStream(FSDataOutputStream stream) throws IOException {
      ...
      try {
        ...
        logger.debug(
            "After spooling batch, stream at position {}. File length {}", stream.getPos(), len);
        ...
        // even if the try block throws an exception we still want to send an ACK and release the
        // lock
        // the caller will add the exception to deferred attribute and it will be thrown when the
    ...
    public void readFromStream() throws IOException, InterruptedException {
      ...
      while (tryAgain) {
        ...
        // Sometimes, the file isn't quite done writing when we attempt to read it. As such, we need
        // to wait and retry.
        Thread.sleep(duration);

        try (final ArrowBuf buf = allocator.buffer(bodyLength)) {
          if (start + totalLength > inputStreamLastKnownLen) {
            // Try to reuse the inputStream opened for earlier reads, but if current buffer was
            // writtenToStream
            // after this inputStream was opened, close and reopen a new one to avoid EOF or short
            // read errors.
            if (inputStream != null) {
              inputStream.close();
              inputStream = null;
            ...
            logger.debug(
                "Opening new inputStream for file {}, start {}, totalLength {}, newLen {}",
                spillFile.getPath(),
                start,
                totalLength,
                newLen);
            assert newLen >= inputStreamLastKnownLen
                : String.format(
                    "newLen %d should not be less than prevLen %d",
                    newLen, inputStreamLastKnownLen);
            assert newLen >= start + totalLength
                : String.format(
                    "file len %d too small for buffer, start %d, len %d",
          ...
          assert check == this.check
              : String.format(
                  "Check values don't match: %d %d, Position %d", this.check, check, currentPos);
          Stopwatch watch = Stopwatch.createStarted();
          ...
          long t = watch.elapsed(TimeUnit.MICROSECONDS);
          logger.debug(
              "Took {} us to read {} from disk. Rate {} mb/s", t, bodyLength, bodyLength / t);
          tryAgain = false;
          state = BatchState.AVAILABLE;
          ...
          FileStatus status = spillFile.getFileStatus();
          logger.warn(
              "EOF reading from file {} at pos {}. Current file size: {}. Read start {} & total length {}.",
              spillFile.getPath(),
              pos,
              status.getLen(),
              start,
