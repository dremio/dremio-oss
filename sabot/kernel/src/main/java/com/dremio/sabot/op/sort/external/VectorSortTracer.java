...
public class VectorSortTracer {
  ...
  /* Operator's allocator; passed to Operator as part of OperatorContext */
  private VectorSorterAllocatorState sortAllocatorState = new VectorSorterAllocatorState();

  protected static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(VectorSortTracer.class);
  ...
  public void setVectorSorterAllocatorState(final BufferAllocator sortAllocator) {
    /* this is the parent allocator passed to VectorSorter as part of
  ...
  public UserException prepareAndThrowException(
      final Exception ex, final BufferAllocator allocator, final String message) {
    final UserException.Builder exBuilder = getExceptionBuilder(ex, allocator, message);
    if (message != null) {
      exBuilder.addContext(message);
    }
    // exBuilder.addContext("Detailed VectorSorter Error Tracing follows: \n\n" + toString() +
    // Joiner.on("\n").join(events));
  ...
  private UserException.Builder getExceptionBuilder(
      final Exception ex, final BufferAllocator allocator, final String msg) {
    final UserException.Builder exceptionBuilder;
    if (ErrorHelper.isDirectMemoryException(ex)) {
      StringBuilder sb = (msg != null ? new StringBuilder(msg) : new StringBuilder());
      sb.append("\n");
      sb.append(MemoryDebugInfo.getSummaryFromRoot(allocator));
      exceptionBuilder =
      ...
      exceptionBuilder = UserException.resourceError(ex).message(msg);
    }
  ...
  private static class VectorSorterAllocatorState extends AllocatorState {

    VectorSorterAllocatorState() {}
  }
