...
/**
 * Stats for {@link VectorizedHashAggOperator} and {@link
 * com.dremio.sabot.op.aggregate.hash.HashAggOperator}
 *
...
public class HashAggStats {
  ...
  public enum Metric implements MetricDef {
    ...
    SPILL_TIME(
        ...
        AggregationType.SUM,
        "Cumulative time taken to spill all partition"), /* cumulative time taken to spill all partitions */
    READ_SPILLED_BATCH_TIME(
        DisplayType.DISPLAY_BY_DEFAULT,
        AggregationType.SUM,
        "Cumulative time taken to read all spilled batches"), /* cumulative time taken to read all spilled batches */
    ...
    TOTAL_RECORDS_SPILLED(
        DisplayType.DISPLAY_BY_DEFAULT,
        AggregationType.SUM,
        "Number of records spilled"), /* total number of records spilled across all spills */
    ...
    TOTAL_SPILLED_DATA_SIZE(
        DisplayType.DISPLAY_BY_DEFAULT,
        AggregationType.SUM,
        "Number of bytes spilled"), /* total size (in bytes) of data spilled by vectorized hash agg operator */
    ...
    AVG_MEMORY_ALLOC_PER_PUMP_IN_MB, /* Average memory allocation by operator per pump */
    MAX_MEMORY_ALLOC_PER_PUMP_IN_MB, /* Max memory allocation by operator per pump */
    EXCEED_MEMORY_PER_PUMP_COUNT, /* Count of instances where memory allocation was more than grant */
    AVG_TIME_SPENT_PER_PUMP_IN_MILLI, /* Average time spent per pump */
    MAX_TIME_SPENT_PER_PUMP_IN_MILLI, /* Max time spent per pump */
    EXCEED_TIME_SPENT_PER_PUMP_COUNT, /* Count of instances where time taken to execute was more than time slice */
    ...
    // the threshold.
    OOB_DROP_NO_VICTIM, // Number of times OOB dropped spilling notification as all allocations were
    // minimal.
