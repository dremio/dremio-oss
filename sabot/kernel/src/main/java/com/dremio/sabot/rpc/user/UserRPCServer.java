...
public class UserRPCServer extends BasicServer<RpcType, UserRPCServer.UserClientConnectionImpl> {
  private static final org.slf4j.Logger CONNECTION_LOGGER =
      org.slf4j.LoggerFactory.getLogger("com.dremio.ConnectionLog");
  private static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(UserRPCServer.class);
  ...
  protected Response handle(
      UserClientConnectionImpl connection, int rpcType, byte[] pBody, ByteBuf dBody)
      throws RpcException {
    // there are two #handle methods, since the other one is overridden, this is never invoked
    throw new IllegalStateException(
        "UserRPCServer#handle must not be invoked without ResponseSender");
  }
  ...
  protected void feedWork(
    ...
    try {
      // Connection will always have the user session; Without user credentials, we must throw error
      // if user not found
    ...
    try {
      RequestContext.current()
          ...
          .call(
              () -> {
                feedWorkHelper(
                    connection, rpcType, pBody, dBody, requestHandle, span, wrappedSender);
                return null;
              });
    } catch (Exception e) {
  ...
  protected void feedWorkHelper(
      ...
      ResponseSender wrappedSender)
      throws RpcException {
    // If there's an error parsing the request, the failure will not be sent through the response
    // sender.
  ...
  static RecordBatchFormat chooseDremioRecordBatchFormat(UserToBitHandshake inbound) {
    ...
    List<RecordBatchFormat> supportedRecordBatchFormatsList =
        inbound.getSupportedRecordBatchFormatsList();
    if (supportedRecordBatchFormatsList.isEmpty()) {
      supportedRecordBatchFormatsList =
          ImmutableList.of(
              RecordBatchFormat.DREMIO_0_9); // Backward compatibility with older dremio clients
    }

    try {
      RecordBatchFormat recordBatchFormat =
          Ordering.explicit(
                  RecordBatchFormat.UNKNOWN,
                  RecordBatchFormat.DREMIO_0_9,
                  RecordBatchFormat.DREMIO_1_4,
                  RecordBatchFormat.DREMIO_23_0)
  ...
  /**
   * Interface for getting user session properties and interacting with user connection. Separating
   * this interface from {@link RemoteConnection} implementation for user connection:
   *
   * <p>
   *
   * <ul>
   *   <li>Connection is passed to AttemptManager and Screen operators. Instead passing this
   *       interface exposes few details.
   *   <li>Makes it easy to have wrappers around user connection which can be helpful to tap the
   *       messages and data going to the actual client.
   * </ul>
   */
  ...
  /**
   * Based on rpc type, work is either performed synch or asynchronously. If work is done
   * asynchronously, assemble the dependencies and hand them off to the worker via submitWork.
   */
  static class WorkIngestorImpl implements WorkIngestor {
    ...
    private <M extends Message> M parse(byte[] body, Parser<M> parser, Class<M> type)
        throws RpcException {
      ...
      try {
        ...
        throw new RpcException(
            String.format("Failure while decoding %s body.", type.getSimpleName()), e);
      }
    }

    @Override
    public void feedWork(
        UserClientConnectionImpl connection,
        int rpcType,
        byte[] pBody,
        ByteBuf dBody,
        ...
        case RpcType.RUN_QUERY_VALUE:
          {
            ...
            worker.submitWork(
                ...
                registry);
            responseSender.send(
                new Response(RpcType.QUERY_HANDLE, ExternalIdHelper.toQueryId(externalId)));
            break;
          }
        ...
        case RpcType.GET_CATALOGS_VALUE:
          {
            ...
            worker.submitWork(
                connection.getSession(),
                new MetadataProvider.CatalogsHandler(responseSender),
                request,
                registry);
            break;
          }
        ...
        case RpcType.GET_SCHEMAS_VALUE:
          {
            ...
            worker.submitWork(
                connection.getSession(),
                new MetadataProvider.SchemasHandler(responseSender),
                request,
                registry);
            break;
          }
        ...
        case RpcType.GET_TABLES_VALUE:
          {
            ...
            worker.submitWork(
                connection.getSession(),
                new MetadataProvider.TablesHandler(responseSender),
                request,
                registry);
            break;
          }
        ...
        case RpcType.GET_COLUMNS_VALUE:
          {
            ...
            worker.submitWork(
                connection.getSession(),
                new MetadataProvider.ColumnsHandler(responseSender),
                request,
                registry);
            break;
          }
        ...
        case RpcType.CREATE_PREPARED_STATEMENT_VALUE:
          {
            ...
            worker.submitWork(
                connection.getSession(),
                new PreparedStatementProvider.PreparedStatementHandler(responseSender),
                request,
                registry);
            break;
          }
        ...
        case RpcType.CREATE_PREPARED_STATEMENT_ARROW_VALUE:
          {
            ...
            worker.submitWork(
                connection.getSession(),
                new PreparedStatementProvider.PreparedStatementArrowHandler(responseSender),
                request,
                registry);
            break;
          }
        ...
        case RpcType.GET_SERVER_META_VALUE:
          {
            ...
            worker.submitWork(
                connection.getSession(),
                new ServerMetaProvider.ServerMetaHandler(responseSender),
                request,
                registry);
            break;
          }
  ...
  /**
   * {@link RemoteConnection} implementation for user connection. Also implements {@link
  ...
  public class UserClientConnectionImpl extends RemoteConnection implements UserClientConnection {
    ...
    void setUser(final UserToBitHandshake inbound) throws IOException {
      ...
      SessionOptionManager sessionOptionManager =
          sessionOptionManagerFactory.getOrCreate(uuid.toString());
      builder
          .withCredentials(inbound.getCredentials())
          .withSessionOptionManager(
              sessionOptionManager, UserRPCServer.this.worker.get().getSystemOptions())
          ...
          session
              ...
              .setOption(
                  OptionValue.createBoolean(
                      OptionType.SESSION,
                      ExecConstants.USE_LEGACY_CATALOG_NAME.getOptionName(),
                      true));
        }
    ...
    private void scheduleExpiryHandler(long delay, TimeUnit timeUnit) {
      ...
      getChannel().eventLoop().schedule(expiryHandler, delay, timeUnit);
    }
  }
  ...
  @Override
  protected ChannelFutureListener newCloseListener(
      SocketChannel channel, final UserClientConnectionImpl connection) {
  ...
  @Override
  protected ServerHandshakeHandler<UserToBitHandshake> newHandshakeHandler(
    ...
    return new ServerHandshakeHandler<UserToBitHandshake>(
      ...
      protected void consumeHandshake(ChannelHandlerContext ctx, UserToBitHandshake inbound)
        ...
        OutboundRpcMessage msg =
            new OutboundRpcMessage(
                RpcMode.RESPONSE, this.handshakeType, coordinationId, handshakeResp);
        ctx.writeAndFlush(msg);

        if (handshakeResp.getStatus() != HandshakeStatus.SUCCESS) {
          // If handling handshake results in an error, throw an exception to terminate the
          // connection.
      ...
      public BitToUserHandshake getHandshakeResponse(UserToBitHandshake inbound) throws Exception {
        ...
        if (!inbound.hasSupportTimeout() || !inbound.getSupportTimeout()) {
          ...
          logger.warn(
              "Timeout Disabled as client doesn't support it. Connection: {}",
              connection.getName());
        }
        ...
        try {
          ...
          try {
            ...
            inbound =
                inbound.toBuilder()
                    .setCredentials(
                        UserCredentials.newBuilder().setUserName(authResult.getUserName()))
                    .build();
          ...
          // Note that the allocators created below are used for backward compatibility encoding.
          // The encoder is
          // responsible for closing the allocator during its removal; this ensures any outstanding
          // buffers in
          ...
          switch (recordBatchFormat) {
            case DRILL_1_0:
              {
                final BufferAllocator bcAllocator =
                    allocator.newChildAllocator(
                        connection.uuid.toString() + "-backward-compatibility-allocator",
                        0,
                        Long.MAX_VALUE);
                ...
                connection
                    ...
                    .addAfter(
                        PROTOCOL_ENCODER,
                        DRILL_COMPATIBILITY_ENCODER,
                        new BackwardsCompatibilityEncoder(
                            new DrillBackwardsCompatibilityHandler(bcAllocator)));
              }
              break;
            ...
            case DREMIO_0_9:
              {
                ...
                final BufferAllocator bcAllocator =
                    allocator.newChildAllocator(
                        connection.uuid.toString() + "-dremio09-backward", 0, Long.MAX_VALUE);
                ...
                connection
                    ...
                    .addAfter(
                        PROTOCOL_ENCODER,
                        UserRpcUtils.DREMIO09_COMPATIBILITY_ENCODER,
                        new BackwardsCompatibilityEncoder(
                            new Dremio09BackwardCompatibilityHandler(bcAllocator)));
              }
              break;
            ...
            case DREMIO_1_4:
              {
                ...
                final BufferAllocator bcAllocator =
                    allocator.newChildAllocator(
                        connection.uuid.toString() + "-dremio14-backward", 0, Long.MAX_VALUE);
                ...
                connection
                    ...
                    .addAfter(
                        PROTOCOL_ENCODER,
                        DREMIO14_COMPATIBILITY_ENCODER,
                        new BackwardsCompatibilityEncoder(
                            new Dremio14BackwardCompatibilityHandler(bcAllocator)));
              }
              break;
  ...
  /**
   * Complete building the given builder for <i>BitToUserHandshake</i> message with given status and
   * error details.
   *
   * @param respBuilder Instance of {@link BitToUserHandshake} builder which has RPC version field
   *     already set.
   ...
   * @return
   */
  private static BitToUserHandshake handleFailure(
    ...
    if (exception != null) {
      logger.error(
          "Error {} in Handling handshake request: {}, {}", errorId, status, errMsg, exception);
    } else {
   ...
   */
  protected AuthResult authenticate(String username, String password, String tokenType)
