...
public interface Partition extends AutoCloseable {
   ...
   * @return number of records written to output. If -ve, it means this batch needs to be processed
   *     some more.
   * @throws Exception
   */
   ...
   * @param startOutputIndex start index in output batch
   * @param maxOutputIndex max index in output batch
   * @return number of records written to output. If -ve, it means there are still some unmatched
   *     records.
   * @throws Exception
   */
  ...
  boolean isFiltersPreparedWithException();
