...
/**
 *  * A {@link Sizer} implementation for dense union Arrow vectors {@link DenseUnionVector}
 *  * A DenseUnionVector is ordered list of records of various data types. A separate vector is assigned to each vector type in the union and
  ...
  public Copier getCopier(final BufferAllocator allocator, final ArrowBuf sv2, final int startIdx, final int numberOfRecords, final List<FieldVector> vectorOutput) {
    ...
    return (page) -> {
      ...
      try (final ArrowBuf offsetBuf = page.sliceAligned(offsetLen);
           final ArrowBuf typeBuf = page.sliceAligned(numberOfRecords)) { //length of type array is 1 byte each record. i.e. numberOfRecords

        outgoing.loadFieldBuffers(new ArrowFieldNode(numberOfRecords, -1),
          ImmutableList.of(offsetBuf, typeBuf));
