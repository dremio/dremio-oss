...
/**
 * A {@link Merger} implementation for Arrow vector type {@link UnionVector} In a sparse
 * UnionVector, each record can be any one of the fixed set of types. TypeId of each record is
 * defined at the same index in a types buffer. For each of the types, there's a separate data
 * vector.
 *
 * <p>This merger provides functionality of merging together data of multiple sparse union vector
 * instances into one, it merges types buffers and underlying fields' data vectors.
 *
  ...
  public void merge(
    ...
    try (final ArrowBuf typesBuffer = dst.sliceAligned(typesBufferLen)) {
      ...
      final Map<Byte, List<ValueVector>> ordinalToVectorField =
          getVectorsByTypeId(vectorListToBeMerged, allTypesIds);
      ...
      for (final Map.Entry<Byte, List<ValueVector>> entry : ordinalToVectorField.entrySet()) {
        ...
        final TransferPair transferPair =
            mergedDataVectors.get(0).makeTransferPair(outgoing.getVectorByType(typeId));
        transferPair.transfer();
      }
    }
  }

  private Map<Byte, List<ValueVector>> getVectorsByTypeId(
      final List<UnionVector> vectorList, final Set<Byte> typeIds) {
