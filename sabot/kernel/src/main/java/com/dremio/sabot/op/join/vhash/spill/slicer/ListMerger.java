...
public class ListMerger implements Merger {
  ...
  /**
   * A {@link Merger} implementation for list Arrow vectors {@link ListVector} A ListVector consists
   * of variable sized lists of primitive or complex types. It provides validity bitmap to keep
   * track of null values and offset buffer to that provide start and end offsets of lists in the
   * vector.
   *
   * <p>This merger provides functionality of merging together data of multiple ListVector vectors
   * into one, it merges validity buffers, offset buffers and underlying data vectors.
   *
  ...
  public void merge(
    ...
    try (final ArrowBuf validityBuf = dst.sliceAligned(validityLen);
      ...
      outgoing.loadFieldBuffers(
          new ArrowFieldNode(recordCount, -1), ImmutableList.of(validityBuf, offsetBuf));
