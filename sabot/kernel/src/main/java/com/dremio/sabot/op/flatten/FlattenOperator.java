...
// TODO - handle the case where a user tries to flatten a scalar, should just act as a project all
// of the columns exactly
// as they come in
public class FlattenOperator implements SingleInputOperator {
  private static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(FlattenOperator.class);
  ...
  public VectorAccessible setup(VectorAccessible incoming) throws Exception {
    ...
    final ClassGenerator<Flattener> cg =
        context.getClassProducer().createGenerator(Flattener.TEMPLATE_DEFINITION).getRoot();
    final IntHashSet transferFieldIds = new IntHashSet();

    final NamedExpression flattenExpr =
        new NamedExpression(config.getColumn(), new FieldReference(config.getColumn()));
    final ValueVectorReadExpression vectorRead =
        (ValueVectorReadExpression)
            context.getClassProducer().materializeAndAllowComplex(flattenExpr.getExpr(), incoming);
    ...
    long outputMemoryLimit =
        context.getOptions().getOption(ExecConstants.FLATTEN_OPERATOR_OUTPUT_MEMORY_LIMIT);
    ...
    try {
      ...
      final ListVector vector =
          ListVector.class.cast(
              incoming
                  .getValueAccessorById(
                      TypeHelper.getValueVectorClass(field), typedFieldId.getFieldIds())
                  .getValueVector());

      flattener.setFlattenField(vector);
    } catch (Exception ex) {
      throw UserException.unsupportedError(ex)
          .message("Trying to flatten a non-repeated field.")
          .build(logger);
    }
  ...
  /**
   * The data layout is the same for the actual data within a repeated field, as it is in a scalar
   * vector for the same sql type. For example, a repeated int vector has a vector of offsets into a
   * regular int vector to represent the lists. As the data layout for the actual values in the same
   * in the repeated vector as in the scalar vector of the same type, we can avoid making individual
   * copies for the column being flattened, and just use vector copies between the inner vector of
   * the repeated field to the resulting scalar vector from the flatten operation. This is completed
   * after we determine how many records will fit (as we will hit either a batch end, or the end of
   * one of the other vectors while we are copying the data of the other vectors alongside each new
   * flattened value coming out of the repeated field.)
   */
  private TransferPair getFlattenFieldTransferPair(FieldReference outputName) {
    ...
    final Class<? extends ValueVector> vectorClass =
        TypeHelper.getValueVectorClass(incoming.getSchema().getColumn(fieldId.getFieldIds()[0]));
  ...
  private List<NamedExpression> getExpressionList() {
    ...
    for (Field field : incoming.getSchema()) {
      ...
      exprs.add(
          new NamedExpression(
              SchemaPath.getSimplePath(field.getName()), new FieldReference(field.getName())));
    }
    return exprs;
  }
  ...
  public static class FlattenBatchCreator implements SingleInputOperator.Creator<FlattenPOP> {
    ...
    @Override
    public SingleInputOperator create(OperatorContext context, FlattenPOP operator)
