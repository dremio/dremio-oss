 ...
 *
 * When dynamic DOP adjustment is enabled,
 *    * collect partitions/counts for some time
 *    * send to other peer operators about partitions/count seen via OOB
 *    * collect partitions/counts from all peers via OOB
 *    * calculate DOP based on partitions/counts
 *
 * Note:
 *    * DOP could be at partition value level, currently, it is a single value set for all partition values
 *    * DOP could range from 1 ~ maximal width. Currently, it is set either 1, or maximal width (when data skew is detected)
 */
public class AdaptiveVectorizedPartitionSenderOperator extends VectorizedPartitionSenderOperator {

  private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(AdaptiveVectorizedPartitionSenderOperator.class);
  ...
  // To limit the OOB message size, only add top n partition counts from each fragment in the message
  private static int TOP_N_PARTITIONS = 5;

  // maximal number of allowed partitions. Adaptive hashing will be disabled if the number of seen partitions across all fragments exceeds this value
  ...
  private Map<Integer, ExecProtos.HashDistributionValueCounts> participantsPartitionCounters = new HashMap<>();
  ...
  public AdaptiveVectorizedPartitionSenderOperator(final OperatorContext context,
  ...
  public void setup(VectorAccessible incoming) throws Exception {
    ...
    Preconditions.checkArgument(dopRatio >= 0 && dopRatio <= 1, "HASH_GROUP_DOP_RATIO should between 0 and 1");
    ...
    expectedReceivedPartitionCountsMsgCount = context.getAssignments().stream().mapToInt(a -> a.getMinorFragmentIdList().size()).sum();
  }
  ...
  protected OutgoingBatch getBatch(int partition, OutgoingBatch[] modLookup) {
    ...
    long partitionValueCount = partitionCounters.put(partition, partitionCounters.computeIfAbsent(partition, p -> 0L) + 1);
    ...
    try {
      ...
      for(CoordExecRPC.FragmentAssignment a : context.getAssignments()) {
        OutOfBandMessage message = new OutOfBandMessage(
          ...
          a.getMinorFragmentIdList(), // include myself. thus, we could calculate OOB receiving window more accurately.
          config.getProps().getOperatorId(),
          context.getFragmentHandle().getMinorFragmentId(),
          payload, true);

        CoordinationProtos.NodeEndpoint endpoint = context.getEndpointsIndex().getNodeEndpoint(a.getAssignmentIndex());
        context.getTunnelProvider().getExecTunnel(endpoint).sendOOBMessage(message);
        oobSends++;
      }
      if (logger.isDebugEnabled()) {
        logger.debug("partition value counts from major fragment:{} minor fragment:{} are sent",
          context.getFragmentHandle().getQueryId(),
          context.getFragmentHandle().getMinorFragmentId());
      }
  ...
  public void workOnOOB(OutOfBandMessage message) {
    ...
    final ExecProtos.HashDistributionValueCounts partitionValueCounts = message.getPayload(ExecProtos.HashDistributionValueCounts.PARSER);
  ...
  /** do adjust Dop if
   * received OOB messages from all AdaptiveVectorizedPartitionSenderOperator instances
   * or, timeout, in case of OOB message loss
   **/
  private boolean shouldAdjustDop() {
    ...
    // timeout
    if (System.currentTimeMillis() - partitionCountsOOBMessageSentTime >= DEFAULT_PARTITION_COUNTS_OOB_WAIT_TIME_MS) {
      if ((double)oobReceives / expectedReceivedPartitionCountsMsgCount > PARTITION_COUNTS_MSG_RECEIVING_RATE) {
  ...
  private void adjustDop() {
    dop = calculateDop(participantsPartitionCounters.values(), TOP_N_PARTITIONS,
      DATA_SKEW_RATIO, numReceivers, initialDop);

    logger.debug("major fragment:{} minor fragment:{}, Dop is adjusted to {} ",
      context.getFragmentHandle().getQueryId(),
      context.getFragmentHandle().getMinorFragmentId(),
      dop);
  ...
  @VisibleForTesting
  public static int calculateDop(Collection<ExecProtos.HashDistributionValueCounts> participantsPartitionCounters,
                                 int topNPartitions,
    ...
    // we know unique partition counts from each fragment. However, we dont know the total unique partition counts across all fragments
    // since each fragment only send its top N partition values. We could use the max count from all fragments as the lower bound estimate
    long uniquePartitionValueCountLowerBound = 0;
    Map<Long, Long> mergedPartitionCounters = new HashMap<>();
    for(ExecProtos.HashDistributionValueCounts participantsPartitionCounter : participantsPartitionCounters) {
      totalSeenRecords += participantsPartitionCounter.getTotalSeenRecords();
      uniquePartitionValueCountLowerBound = Math.max(uniquePartitionValueCountLowerBound, participantsPartitionCounter.getUniqueValueCount());
      for (ExecProtos.HashDistributionValueCount valueCount : participantsPartitionCounter.getHashDistributionValueCountsList()) {
        mergedPartitionCounters.put(valueCount.getHashDistributionKey(),
          mergedPartitionCounters.computeIfAbsent(valueCount.getHashDistributionKey(), p -> 0L) + valueCount.getCount());
      }
    }
    uniquePartitionValueCountLowerBound = Math.max(uniquePartitionValueCountLowerBound, mergedPartitionCounters.size());
    ...
    PriorityQueue<Map.Entry<Long, Long>> pq = new PriorityQueue<>(
