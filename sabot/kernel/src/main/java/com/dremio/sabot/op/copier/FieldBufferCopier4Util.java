    ...
    public void copy(long offsetAddr, int count) {
      ...
      for (long addr = offsetAddr; addr < max; addr += STEP_SIZE, dstAddr += SIZE) {
        ...
        PlatformDependent.putInt(
            dstAddr,
            PlatformDependent.getInt(srcAddrs[sv4 >>> BATCH_BITS] + (sv4 & MAX_BATCH) * SIZE));
      }
    }
    ...
    public void copy(long offsetAddr, int count) {
      ...
      for (long addr = offsetAddr; addr < max; addr += STEP_SIZE, dstAddr += SIZE) {
        ...
        PlatformDependent.putLong(
            dstAddr,
            PlatformDependent.getLong(srcAddrs[sv4 >>> BATCH_BITS] + (sv4 & MAX_BATCH) * SIZE));
      }
    }
    ...
    public void copy(long sv4Addr, int count) {
      ...
      for (; sv4Addr < maxSV4; sv4Addr += STEP_SIZE, dstOffsetAddr += 4) {
        ...
        final long startAndEnd =
            PlatformDependent.getLong(srcOffsetAddrs[batchNumber] + batchOffset * 4);
        ...
        com.dremio.sabot.op.common.ht2.Copier.copy(
            srcDataAddrs[batchNumber] + firstOffset, curDataAddr, len);
        curDataAddr += len;
      }
  ...
  static class BitCopier implements FieldBufferCopier {
    ...
    public void copy(long offsetAddr, int count) {
      ...
      for (; offsetAddr < maxAddr; offsetAddr += STEP_SIZE, targetIndex++) {
        ...
        final int byteValue =
            PlatformDependent.getByte(srcAddr[batchNOFF >>> BATCH_BITS] + (recordIndex >>> 3));
