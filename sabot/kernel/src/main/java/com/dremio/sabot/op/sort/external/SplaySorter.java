...
/**
 * - Sort each batch locally using an SV2 - Insert each batch into a SplayTree as it arrives, final
 * sorted list is generated by traversing this SplayTree.
 */
public class SplaySorter implements Sorter {
  ...
  public boolean expandMemoryIfNecessary(int newRequiredSize) {
    try {
      ...
      while (splayTreeBuffer.capacity() < requiredSize) {
        ...
        this.splayTreeBuffer =
            allocator.buffer(Math.max(splayTreeBuffer.capacity() * 2, 4096 * SplayTree.NODE_SIZE));
        splayTreeBuffer.setBytes(0, oldSplayTree, 0, oldSplayTree.capacity());
        splayTreeBuffer.setZero(
            oldSplayTree.capacity(), splayTreeBuffer.capacity() - oldSplayTree.capacity());
        if (treeManager != null) {
  ...
  public void setup(VectorAccessible batch)
    ...
    { // Local (single batch) sorter
      ...
      VectorSorter.generateComparisons(g, batch, sortOrderings, classProducer);
      this.localSorter = cg.getImplementationClass();
    }

    { // Tree sorter
      CodeGenerator<SplaySorterInterface> cg =
          classProducer.createGenerator(SplaySorterInterface.TEMPLATE_DEFINITION);
  ...
  public void addBatch(RecordBatchData data, BufferAllocator copyTargetAllocator)
    ...
    try (SelectionVector2 localSortVector = new SelectionVector2(copyTargetAllocator)) {
      ...
      if (incomingSv2 != null) {
        ...
        buffer
            .arrowBuf()
            .setBytes(buffer.writerIndex(), incomingSv2.getBuffer(false), 0, recordCount * 2);
      } else {
      ...
      // Quicksort for cache-local and SplayTree performance benefits (includes resetting vector
      // references)
      localSorter.setup(
          classProducer.getFunctionContext(), localSortVector, data.getVectorAccessible());
