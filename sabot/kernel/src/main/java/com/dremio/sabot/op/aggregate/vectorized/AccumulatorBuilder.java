...
public class AccumulatorBuilder {
  ...
  private static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(AccumulatorBuilder.class);
   ...
   * @return Materialized expressions along with input vectors and field info on output vectors for
   *     accumulators
   *     <p>We do this exactly once to avoid incurring the cost of materializing the expressions and
   *     parsing them repeatedly as we setup accumulators for each partition at the very beginning
   *     when setup is done for {@link VectorizedHashAggOperator}
   */
  public static MaterializedAggExpressionsResult getAccumulatorTypesFromMaterializedExpressions(
    ...
    for (int i = 0; i < aggregateExpressions.size(); i++) {
      ...
      if ("count".equals(funcName)
          && (exprCopy.isEmpty() || (exprCopy.size() == 1 && isCountLiteral(exprCopy.get(0))))) {
      ...
      if ((exprCopy.size() != 1 && !(expr instanceof ListAggExpression))
          || !(exprCopy.get(0) instanceof ValueVectorReadExpression)) {
        throw unsup(
            "Accumulation expression has an unexpected number of type of arguments: " + exprCopy);
      }
    ...
    return new MaterializedAggExpressionsResult(
        accumulatorTypes, inputVectors, outputVectorFields, exprs);
  }
  ...
  public static MaterializedAggExpressionsResult getAccumulatorTypesFromExpressions(
    ...
    List<LogicalExpression> materializedExprs =
        namedExpressions.stream()
            .map(ne -> producer.materialize(ne.getExpr(), incoming))
            .collect(Collectors.toList());

    return getAccumulatorTypesFromMaterializedExpressions(
        namedExpressions, materializedExprs, incoming);
  }
  ...
  private static ListAggParams buildListAggParams(
      final ListAggExpression listAggExpression, final int maxListAggSize) {
    ...
    if (listAggExpression.getOrderings().size() > 0) {
      ...
      if (RelFieldCollation.Direction.valueOf(direction) == RelFieldCollation.Direction.ASCENDING
          || RelFieldCollation.Direction.valueOf(direction)
    ...
    if (listAggExpression.getExtraExpressions().size() > 0) {
      ValueExpressions.QuotedString quotedString =
          (ValueExpressions.QuotedString) listAggExpression.getExtraExpressions().get(0);
      delimiter = quotedString.value;
    }
    return new ListAggParams(
        maxListAggSize, listAggExpression.isDistinct(), delimiter, orderby, asc);
  }

  /**
   * Create a set of accumulators. For each accumulator, add an output vector to outgoing. Wraps all
   * accumulators in a single parent.
   *
   ...
   * @param materializedAggExpressions holder for materialized aggregate expressions and info on
   ...
   *     <p>With partitioning in VectorizedHashAgg operator, accumulators are handled on a
   *     per-partition basis. However, we want to add the target accumulation vector that stores the
   *     computed values to the outgoing vector container EXACTLY ONCE since that is used for
   *     projection and each partition's output accumulator vector simply does a transfer to the
   *     vector in outgoing container when operator stats outputting data.
   */
  public static AccumulatorSet getAccumulator(
    ...
    for (int i = 0; i < accumulatorTypes.length; i++) {
      ...
      final FieldVector outputVector =
          TypeHelper.getNewVector(outputVectorFields.get(i), outputVectorAllocator);
    ...
    return new AccumulatorSet(
        jointAllocationMin, jointAllocationLimit, computationVectorAllocator, accums);
  }

  private static Accumulator getAccumulator(
      byte accumulatorType,
    ...
    switch (accumulatorType) {
      case 0 /* SUM */:
        {
          switch (type) {
            case INT:
              return new SumAccumulators.IntSumAccumulator(
                  incomingValues,
                  outputVector,
                  transferVector,
                  maxValuesPerBatch,
                  computationVectorAllocator);
            case FLOAT4:
              return new SumAccumulators.FloatSumAccumulator(
                  incomingValues,
                  outputVector,
                  transferVector,
                  maxValuesPerBatch,
                  computationVectorAllocator);
            case BIGINT:
              return new SumAccumulators.BigIntSumAccumulator(
                  incomingValues,
                  outputVector,
                  transferVector,
                  maxValuesPerBatch,
                  computationVectorAllocator);
            case FLOAT8:
              return new SumAccumulators.DoubleSumAccumulator(
                  incomingValues,
                  outputVector,
                  transferVector,
                  maxValuesPerBatch,
            ...
            case DECIMAL:
              if (decimalCompleteEnabled) {
                return new SumAccumulators.DecimalSumAccumulatorV2(
                    incomingValues,
                    outputVector,
                    transferVector,
                    maxValuesPerBatch,
                ...
                return new SumAccumulators.DecimalSumAccumulator(
                    incomingValues,
                    outputVector,
                    transferVector,
                    maxValuesPerBatch,
      ...
      case 1 /* MIN */:
        {
          switch (type) {
            case INT:
              return new MinAccumulators.IntMinAccumulator(
                  incomingValues,
                  outputVector,
                  transferVector,
                  maxValuesPerBatch,
                  computationVectorAllocator);
            case FLOAT4:
              return new MinAccumulators.FloatMinAccumulator(
                  incomingValues,
                  outputVector,
                  transferVector,
                  maxValuesPerBatch,
                  computationVectorAllocator);
            case BIGINT:
              return new MinAccumulators.BigIntMinAccumulator(
                  incomingValues,
                  outputVector,
                  transferVector,
                  maxValuesPerBatch,
                  computationVectorAllocator);
            case FLOAT8:
              return new MinAccumulators.DoubleMinAccumulator(
                  incomingValues,
                  outputVector,
                  transferVector,
                  maxValuesPerBatch,
            ...
            case DECIMAL:
              if (decimalCompleteEnabled) {
                return new MinAccumulators.DecimalMinAccumulatorV2(
                    incomingValues,
                    outputVector,
                    transferVector,
                    maxValuesPerBatch,
                ...
                return new MinAccumulators.DecimalMinAccumulator(
                    incomingValues,
                    outputVector,
                    transferVector,
                    maxValuesPerBatch,
            ...
            case BIT:
              return new MinAccumulators.BitMinAccumulator(
                  incomingValues,
                  outputVector,
                  transferVector,
                  maxValuesPerBatch,
            ...
            case TIMESTAMP:
              ...
              return new MinAccumulators.BigIntMinAccumulator(
                  ...
                  computationVectorAllocator);
            case INTERVALDAY:
              return new MinAccumulators.IntervalDayMinAccumulator(
                  incomingValues,
                  outputVector,
                  transferVector,
                  maxValuesPerBatch,
            ...
            case VARCHAR:
            case VARBINARY:
              return new MinAccumulators.VarLenMinAccumulator(
                  incomingValues,
                  transferVector,
                  maxValuesPerBatch,
                  computationVectorAllocator,
      ...
      case 2 /* MAX */:
        {
          switch (type) {
            case INT:
              return new MaxAccumulators.IntMaxAccumulator(
                  incomingValues,
                  outputVector,
                  transferVector,
                  maxValuesPerBatch,
                  computationVectorAllocator);
            case FLOAT4:
              return new MaxAccumulators.FloatMaxAccumulator(
                  incomingValues,
                  outputVector,
                  transferVector,
                  maxValuesPerBatch,
                  computationVectorAllocator);
            case BIGINT:
              return new MaxAccumulators.BigIntMaxAccumulator(
                  incomingValues,
                  outputVector,
                  transferVector,
                  maxValuesPerBatch,
                  computationVectorAllocator);
            case FLOAT8:
              return new MaxAccumulators.DoubleMaxAccumulator(
                  incomingValues,
                  outputVector,
                  transferVector,
                  maxValuesPerBatch,
            ...
            case DECIMAL:
              if (decimalCompleteEnabled) {
                return new MaxAccumulators.DecimalMaxAccumulatorV2(
                    incomingValues,
                    outputVector,
                    transferVector,
                    maxValuesPerBatch,
                ...
                return new MaxAccumulators.DecimalMaxAccumulator(
                    incomingValues,
                    outputVector,
                    transferVector,
                    maxValuesPerBatch,
            ...
            case BIT:
              return new MaxAccumulators.BitMaxAccumulator(
                  incomingValues,
                  outputVector,
                  transferVector,
                  maxValuesPerBatch,
            ...
            case TIMESTAMP:
              ...
              return new MaxAccumulators.BigIntMaxAccumulator(
                  ...
                  computationVectorAllocator);
            case INTERVALDAY:
              return new MaxAccumulators.IntervalDayMaxAccumulator(
                  incomingValues,
                  outputVector,
                  transferVector,
                  maxValuesPerBatch,
            ...
            case VARCHAR:
            case VARBINARY:
              return new MaxAccumulators.VarLenMaxAccumulator(
                  incomingValues,
                  transferVector,
                  maxValuesPerBatch,
                  computationVectorAllocator,
      ...
      case 3 /* SUM0 */:
        {
          switch (type) {
            case INT:
              return new SumZeroAccumulators.IntSumZeroAccumulator(
                  incomingValues,
                  outputVector,
                  transferVector,
                  maxValuesPerBatch,
                  computationVectorAllocator);
            case FLOAT4:
              return new SumZeroAccumulators.FloatSumZeroAccumulator(
                  incomingValues,
                  outputVector,
                  transferVector,
                  maxValuesPerBatch,
                  computationVectorAllocator);
            case BIGINT:
              return new SumZeroAccumulators.BigIntSumZeroAccumulator(
                  incomingValues,
                  outputVector,
                  transferVector,
                  maxValuesPerBatch,
                  computationVectorAllocator);
            case FLOAT8:
              return new SumZeroAccumulators.DoubleSumZeroAccumulator(
                  incomingValues,
                  outputVector,
                  transferVector,
                  maxValuesPerBatch,
            ...
            case DECIMAL:
              if (decimalCompleteEnabled) {
                return new SumZeroAccumulators.DecimalSumZeroAccumulatorV2(
                    incomingValues,
                    outputVector,
                    transferVector,
                    maxValuesPerBatch,
                ...
                return new SumZeroAccumulators.DecimalSumZeroAccumulator(
                    incomingValues,
                    outputVector,
                    transferVector,
                    maxValuesPerBatch,
      ...
      case 4 /* COUNT */:
        {
          return new CountColumnAccumulator(
              incomingValues,
              outputVector,
              transferVector,
              maxValuesPerBatch,
      ...
      case 6 /* HLL */:
        {
          switch (type) {
            case INT:
              return new NdvAccumulators.IntNdvAccumulators(
                  incomingValues,
                  transferVector,
                  maxValuesPerBatch,
                  computationVectorAllocator,
                  tempAccumulatorHolder);
            case FLOAT4:
              return new NdvAccumulators.FloatNdvAccumulator(
                  incomingValues,
                  transferVector,
                  maxValuesPerBatch,
                  computationVectorAllocator,
                  tempAccumulatorHolder);
            case BIGINT:
              return new NdvAccumulators.BigIntNdvAccumulator(
                  incomingValues,
                  transferVector,
                  maxValuesPerBatch,
                  computationVectorAllocator,
                  tempAccumulatorHolder);
            case FLOAT8:
              return new NdvAccumulators.DoubleNdvAccumulator(
                  incomingValues,
                  transferVector,
                  maxValuesPerBatch,
                  computationVectorAllocator,
            ...
            case DECIMAL:
              if (decimalCompleteEnabled) {
                return new NdvAccumulators.DecimalNdvAccumulatorV2(
                    incomingValues,
                    transferVector,
                    maxValuesPerBatch,
                    computationVectorAllocator,
                ...
                return new NdvAccumulators.DecimalNdvAccumulator(
                    incomingValues,
                    transferVector,
                    maxValuesPerBatch,
                    computationVectorAllocator,
            ...
            case BIT:
              return new NdvAccumulators.BitNdvAccumulator(
                  incomingValues,
                  transferVector,
                  maxValuesPerBatch,
                  computationVectorAllocator,
            ...
            case TIMESTAMP:
              ...
              return new NdvAccumulators.BigIntNdvAccumulator(
                  ...
                  tempAccumulatorHolder);
            case INTERVALDAY:
              return new NdvAccumulators.IntervalDayNdvAccumulator(
                  incomingValues,
                  transferVector,
                  maxValuesPerBatch,
                  computationVectorAllocator,
            ...
            case VARCHAR:
            case VARBINARY:
              return new NdvAccumulators.VarLenNdvAccumulators(
                  incomingValues,
                  transferVector,
                  maxValuesPerBatch,
                  computationVectorAllocator,
      ...
      case 7 /* HLL_MERGE */:
        {
          return new NdvAccumulators.NdvUnionAccumulators(
              incomingValues,
              transferVector,
              maxValuesPerBatch,
              computationVectorAllocator,
      ...
      case 9 /* LOCAL_LISTAGG */:
        {
          return new LocalListAggAccumulator(
              incomingValues,
              transferVector,
              maxValuesPerBatch,
              computationVectorAllocator,
      ...
      case 10 /* LISTAGG_MERGE */:
        {
          return new ListAggMergeAccumulator(
              incomingValues,
              transferVector,
              maxValuesPerBatch,
              computationVectorAllocator,
      ...
      case 11 /* ARRAY_AGG */:
        {
          switch (type) {
            case BIGINT:
              return new BigIntArrayAggAccumulator(
                  incomingValues,
                  transferVector,
                  tempAccumulatorHolder,
                  computationVectorAllocator,
  ...
  public static class MaterializedAggExpressionsResult {
    ...
    public MaterializedAggExpressionsResult(
