...
public class TestTpchExplain extends PlanTestBase {
  ...
  public static void checkPlan(final String fileName, String[] expected, String[] unexpected)
      throws Exception {
  ...
  public void tpch12() throws Exception {
    checkPlan(
        ...
        new String[] {
          "HashJoin",
          "Project",
          "TableFunction.*orders.parquet",
          "Filter",
          "TableFunction.*lineitem.parquet"
        },
        new String[] {"HashToRandomExchange"});
    ...
    try {
      ...
      checkPlan(
          "queries/tpch/12.sql",
          new String[] {
            "HashJoin", "TableFunction.*tpch/orders.parquet", "TableFunction.*tpch/lineitem.parquet"
          },
          null);
    } finally {
      test("alter session set \"planner.slice_target\" = " + ExecConstants.SLICE_TARGET_DEFAULT);
  ...
  public void tpch15() throws Exception {
    try {
      ...
      // NB: can't use query 15 directly, as it has four distinct parts. doExplain(), above, assumes
      // a single query
      test("use dfs_test");
      try {
        test(
