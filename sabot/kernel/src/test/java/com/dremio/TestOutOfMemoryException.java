...
public class TestOutOfMemoryException extends BaseTestQuery {
  ...
  public void testIncomingBuffersOOM() throws Exception {
    ...
    final String controlsString =
        Controls.newBuilder()
            .addException(
                IncomingBuffers.class, IncomingBuffers.INJECTOR_DO_WORK, OutOfMemoryException.class)
            .build();
  ...
  public void testFragmentExecutorOOM() throws Exception {
    final String controlsString =
        Controls.newBuilder()
            .addException(
                FragmentExecutor.class, FragmentExecutor.INJECTOR_DO_WORK, OutOfMemoryError.class)
            .build();
    ControlsInjectionUtil.setControls(client, controlsString);
  ...
  public void testFragmentExecutorBuildOOM() throws Exception {
    final String controlsString =
        Controls.newBuilder()
            .addException(
                FragmentExecutorBuilder.class,
                FragmentExecutorBuilder.INJECTOR_DO_WORK,
    ...
    try (AutoCloseable ignored = withOption(ExecConstants.SLICE_TARGET_OPTION, 10)) {
      ...
      // Should never reach here.
      Assert.fail(
          "Query did not hit the injected out-of-memory exception in FragmentExecutorBuilder#build");
    } catch (UserException uex) {
      // Verify that query has hit the injected out-of-memory exception.
      DremioPBError error = uex.getOrCreatePBError(false);
      Assert.assertEquals(DremioPBError.ErrorType.OUT_OF_MEMORY, error.getErrorType());
