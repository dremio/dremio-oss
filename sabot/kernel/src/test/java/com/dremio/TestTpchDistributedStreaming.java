...
public class TestTpchDistributedStreaming extends BaseTestQuery {
  ...
  private void testDistributed(String fileName) throws Exception {
    ...
    test(
        "alter session set \"planner.slice_target\" = 10; alter session set \"planner.enable_hashjoin\" = false; "
            + "alter session set \"planner.enable_hashagg\" = false; alter session set \"planner.enable_mergejoin\" = true;"
            + query);
  }
