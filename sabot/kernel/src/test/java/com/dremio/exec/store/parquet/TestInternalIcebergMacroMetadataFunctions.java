...
/**
 * functions defined in MetadataFunctionsMacro.java are no longer supported for Internal Iceberg Tables
 * Examples of all Table Functions No longer Supported:
 ...
 * <p> All methods expect an exception. Proof of query success - if feature is ever turned back on - is not guaranteed
 */
public class TestInternalIcebergMacroMetadataFunctions extends BaseTestQuery {
  ...
  public void testPartitionFunction() throws Exception {
    try (AutoCloseable c1 = enableUnlimitedSplitsSupportFlags()) {
      ...
      assertThatThrownBy(() -> {
        testBuilder()
          .ordered()
          .sqlQuery("select count(*) as cnt from table(table_partitions('dfs.tmp.metadatarefresh.onlyFull'))")
          .baselineColumns("cnt")
          .baselineValues(Long.valueOf(icebergTable.history().size()))
          .go();
      }).isInstanceOf(Exception.class)
        .hasMessageContaining("Metadata function ('table_partitions') is not supported on table '[dfs, tmp, metadatarefresh, onlyFull]'");
    }
  }
  ...
  public void testTableFilesFunction() throws Exception {
    try (AutoCloseable c1 = enableUnlimitedSplitsSupportFlags()) {
      ...
      runSQL(sql);
      String selectTableFilesQuery = String.format("SELECT count(file_path) as cnt FROM table(table_files('dfs.tmp.metadatarefresh.incrementalRefresh'))");
      ...
      assertThatThrownBy(() -> {
        ...
        .hasMessageContaining("Metadata function ('table_files') is not supported on table '[dfs, tmp, metadatarefresh, incrementalRefresh]'");
    }
  }
    ...
    try (AutoCloseable c1 = enableUnlimitedSplitsSupportFlags()) {
      ...
      assertThatThrownBy(() -> {
        testBuilder()
          ...
          .baselineValues(Long.valueOf(icebergTable.history().size()))
          .go();
      }).isInstanceOf(Exception.class).hasMessageContaining("Metadata function ('table_history') is not supported on table '[dfs, tmp, metadatarefresh, onlyFull]'");
    }
  }
  ...
  public void testSnapshotFunction() throws Exception {
    try (AutoCloseable c1 = enableUnlimitedSplitsSupportFlags()) {
      ...
      assertThatThrownBy(() -> {
        testBuilder()
          .ordered()
          .sqlQuery("SELECT parent_id,snapshot_id,operation,manifest_list FROM TABLE(table_snapshot('dfs.tmp.metadatarefresh.onlyFull'))")
          .baselineColumns("committed_at", "parent_id", "snapshot_id", "operation", "manifest_list")
          .baselineRecords(recordBuilder.build())
          .go();
      }).isInstanceOf(Exception.class)
        .hasMessageContaining("Metadata function ('table_snapshot') is not supported on table '[dfs, tmp, metadatarefresh, onlyFull]'");
    }
  }
  ...
  public void testManifestsFunction() throws Exception {
    try (AutoCloseable c1 = enableUnlimitedSplitsSupportFlags()) {
      ...
      assertThatThrownBy(() -> {
        testBuilder()
          .ordered()
          .sqlQuery("select * from table(table_manifests('dfs.tmp.metadatarefresh.onlyFull'))")
          .baselineColumns("cnt")
          .baselineValues(Long.valueOf(icebergTable.history().size()))
          .go();
      }).isInstanceOf(Exception.class)
        .hasMessageContaining("Metadata function ('table_manifests') is not supported on table '[dfs, tmp, metadatarefresh, onlyFull]'");
    }
  }
