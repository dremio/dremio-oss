...
public class TestJoinBooleanRewriteRule extends PlanTestBase {
  ...
  public void testEqual() throws Exception {
    String sql =
        "SELECT * FROM (SELECT total_children = 1 as x FROM cp.\"customer.json\") a,"
            + "(SELECT total_children = 1 as x FROM cp.\"customer.json\") b WHERE a.x = b.x";

    testPlanMatchingPatterns(
        sql,
        new String[] {
          "HashJoin\\(condition\\=\\[\\=\\(\\$1, \\$3\\)\\], joinType\\=\\[inner\\]\\) \\: rowType \\= RecordType\\(BOOLEAN x, INTEGER \\$f1, BOOLEAN x0, INTEGER \\$f10\\)"
        },
        new String[] {});
  }
  ...
  public void testIsNotDistinctFrom() throws Exception {
    ...
    testPlanMatchingPatterns(
        sql,
        new String[] {
          "HashJoin\\(condition\\=\\[IS NOT DISTINCT FROM\\(\\$1, \\$3\\)\\], joinType\\=\\[inner\\]\\) "
  ...
  public void testCompound() throws Exception {
    String sql =
        "SELECT * FROM (SELECT total_children = 1 as x, postal_code y FROM cp.\"customer.json\") a,"
            + "(SELECT total_children = 1 as x, postal_code y FROM cp.\"customer.json\") b WHERE a.x IS NOT DISTINCT FROM b.x and a.y = b.y";

    testPlanMatchingPatterns(
        sql,
        new String[] {
          "HashJoin\\(condition\\=\\[AND\\(\\=\\(\\$1, \\$4\\), IS NOT DISTINCT FROM\\(\\$2, \\$5\\)\\)\\], "
