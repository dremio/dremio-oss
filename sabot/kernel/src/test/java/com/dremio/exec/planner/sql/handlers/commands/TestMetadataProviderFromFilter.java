...
public class TestMetadataProviderFromFilter {
  private static Stream<Arguments> testMetadataFilter() {
    return Stream.of(
        ...
        Arguments.of(toFilter("abc.bcd\\_def.ghk:AMP.foo"), toFilter("%MY\\_TABLE\\_foobar"), null),
        Arguments.of(toFilter("abc.bcd\\_def.ghk:AMP.foo"), toFilter(""), null),
        Arguments.of(toFilter(""), toFilter("MY\\_TABLE\\_foobar"), null));
  }

  @ParameterizedTest
  @MethodSource
  public void testMetadataFilter(
      UserProtos.LikeFilter schema, UserProtos.LikeFilter table, NamespaceKey spaceKey)
      throws Exception {
  ...
  private static UserProtos.LikeFilter toFilter(String pattern, String escape) {
    return UserProtos.LikeFilter.newBuilder().setEscape(escape).setPattern(pattern).build();
  }
