...
public class TestPreparedStatementProvider extends BaseTestQuery {
  private static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(TestPreparedStatementProvider.class);
  ...
  public void simple() throws Exception {
    ...
    List<ExpectedColumnResult> expMetadata =
        ImmutableList.of(
            new ExpectedColumnResult(
                "region_id", "BIGINT", true, 20, 0, 0, true, Long.class.getName()),
            new ExpectedColumnResult(
                "sales_city",
                "CHARACTER VARYING",
                true,
            ...
            new ExpectedColumnResult(
                ...
                String.class.getName()),
            new ExpectedColumnResult(
                "sales_district_id", "BIGINT", true, 20, 0, 0, true, Long.class.getName()));
  ...
  public void groupByQuery() throws Exception {
    String query =
        "SELECT sales_city, count(*) as cnt FROM cp.\"region.json\" "
  ...
  public void joinOrderByQuery() throws Exception {
    String query =
        "SELECT l.l_quantity, l.l_shipdate, o.o_custkey FROM cp.\"tpch/lineitem.parquet\" l JOIN cp.\"tpch/orders.parquet\" o "
    ...
    List<ExpectedColumnResult> expMetadata =
        ImmutableList.of(
            new ExpectedColumnResult(
                "l_quantity", "DOUBLE", true, 24, 0, 0, true, Double.class.getName()),
            new ExpectedColumnResult(
                "l_shipdate", "DATE", true, 10, 0, 0, false, Date.class.getName()),
            new ExpectedColumnResult(
                "o_custkey", "INTEGER", true, 11, 0, 0, true, Integer.class.getName()));
  ...
  public void invalidPrepareHandle() throws Exception {
    final ServerPreparedStatementState state =
        ServerPreparedStatementState.newBuilder()
            .setHandle(
                -23) // Handles start from 0 in the sever. This is relying on server implementation
            // detail. May need to find a better way to test it.
            .setSqlQuery(
                "SELECT sales_city, count(*) as cnt FROM cp.\"region.json\" "
    ...
    PreparedStatement preparedStatement =
        PreparedStatement.newBuilder()
            .setServerHandle(
                PreparedStatementHandle.newBuilder().setServerInfo(state.toByteString()).build())
            .build();
  ...
  private static PreparedStatement createPrepareStmt(
      String query, boolean expectFailure, ErrorType errorType) throws Exception {
  ...
  private static class ExpectedColumnResult {
    ...
    boolean isEqualsTo(ResultColumnMetadata result) {
      return result.getCatalogName().equals(InfoSchemaConstants.IS_CATALOG_NAME)
  ...
  private static String toString(ResultColumnMetadata metadata) {
    return "ResultColumnMetadata["
        ...
        + '\''
        + ", nullable="
        + metadata.getIsNullable()
        + ", displaySize="
        + metadata.getDisplaySize()
        + ", precision="
  ...
  private static void verifyMetadata(
      List<ExpectedColumnResult> expMetadata, List<ResultColumnMetadata> actMetadata) {
    ...
    for (ExpectedColumnResult exp : expMetadata) {
      ...
      assertTrue(
          "Failed to find the expected column metadata: " + exp + ". Was: " + toString(act),
          exp.isEqualsTo(act));
    }
  }
}
