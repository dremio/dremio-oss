...
public class TestIcebergMetadataVerifyProcessors extends BaseTestQuery {
  ...
  /**
   * Test metadata verify by calling {@link EntityExplorer#verifyTableMetadata(NamespaceKey, TableMetadataVerifyRequest)}
   * Test: S0 -> Append -> S1
   * Expected ResultCode: APPEND_ONLY
   * Expected snapshot ranges: [(S0, S1)]
   */
  @Test
  public void testCatalogVerifyTableMetadata() throws Exception {
    ...
    Optional<TableMetadataVerifyResult> result = entityExplorer.verifyTableMetadata(
      new NamespaceKey(keyPath),
      new TableMetadataVerifyAppendOnlyRequest(Long.toString(snapshot0.snapshotId()), Long.toString(snapshot1.snapshotId())));
    ...
    assertEquals(Long.toString(snapshot0.snapshotId()), appendOnlyResult.getSnapshotRanges().get(0).getLeft());
    assertEquals(Long.toString(snapshot1.snapshotId()), appendOnlyResult.getSnapshotRanges().get(0).getRight());
  }
   ...
   * Expected snapshot ranges: [(S0, S1)]
   */
  public static void testCatalogVerifyTableMetadata(BufferAllocator allocator, String source) throws Exception {
    try (DmlQueryTestUtils.Table table = createBasicTable(source,2, 1)) {
      // Insert row to increase snapshots
      addRows(table, 1);

      List<QueryDataBatch> queryBatch = testSqlWithResults(String.format("SELECT snapshot_id FROM table(table_snapshot('%s')) order by committed_at", table.fqn));
      ...
      EntityExplorer entityExplorer = CatalogUtil.getSystemCatalogForReflections(getSabotContext().getCatalogService());
      Optional<TableMetadataVerifyResult> result = entityExplorer.verifyTableMetadata(
        new NamespaceKey(parseFullPath(table.fqn)),
  ...
  /**
   * Test metadata verify by calling {@link IcebergMetadataVerifyProcessors#verify(MetadataVerifyRequest, Table)} directly
   * Test: S0 -> Append -> Append -> S1
   * Expected ResultCode: APPEND_ONLY
   * Expected snapshot ranges: [(S0, S1)]
   */
  @Test
  public void testMetadataVerifyOption() throws Exception {
    ...
    MetadataVerifyRequest metadataVerifyRequest = new VerifyAppendOnlyRequest(
      Long.toString(snapshot0.snapshotId()),
      Long.toString(snapshot1.snapshotId()));
    DatasetVerifyAppendOnlyResult result = (DatasetVerifyAppendOnlyResult) IcebergMetadataVerifyProcessors.verify(
      metadataVerifyRequest, test_iceberg_table).get();
    ...
    assertEquals(Long.toString(snapshot0.snapshotId()), result.getSnapshotRanges().get(0).getLeft());
    assertEquals(Long.toString(snapshot1.snapshotId()), result.getSnapshotRanges().get(0).getRight());
  }

  /**
   * Test metadata verify by calling {@link IcebergMetadataVerifyProcessors#isAppendOnlyBetweenSnapshots(Table, Long, Long)} directly
   * Test: Invalid begin snapshot
   * Expected ResultCode: INVALID_BEGIN_SNAPSHOT
   */
  @Test
  public void testInvalidBeginSnapshot() throws Exception {
    ...
    DatasetVerifyAppendOnlyResult result = (DatasetVerifyAppendOnlyResult) IcebergMetadataVerifyProcessors.isAppendOnlyBetweenSnapshots(
      test_iceberg_table, 123L, snapshot1.snapshotId());

    assertEquals(DatasetVerifyAppendOnlyResult.ResultCode.INVALID_BEGIN_SNAPSHOT, result.getResultCode());
   ...
   * Test metadata verify by calling {@link IcebergMetadataVerifyProcessors#isAppendOnlyBetweenSnapshots(Table, Long, Long)} directly
   * Test: Invalid begin snapshot
   * Expected ResultCode: INVALID_END_SNAPSHOT
   */
  @Test
  public void testInvalidEndSnapshot() throws Exception {
    ...
    assertEquals(DatasetVerifyAppendOnlyResult.ResultCode.INVALID_END_SNAPSHOT, result.getResultCode());
    assertEquals(0, result.getSnapshotRanges().size());
  }

  /**
   * Test metadata verify by calling {@link IcebergMetadataVerifyProcessors#isAppendOnlyBetweenSnapshots(Table, Long, Long)} directly
   * Test: Begin snapshot is not ancestor of end snapshot
   * Expected ResultCode: NOT_ANCESTOR
   */
  @Test
  ...
  /**
   * Test metadata verify by calling {@link IcebergMetadataVerifyProcessors#isAppendOnlyBetweenSnapshots(Table, Long, Long)} directly
   * Test: S0 -> Append -> S1 -> Update -> S2
   * Expected ResultCode: NOT_APPEND_ONLY
   */
  @Test
  ...
  /**
   * Test metadata verify by calling {@link IcebergMetadataVerifyProcessors#isAppendOnlyBetweenSnapshots(Table, Long, Long)} directly
   * Test: Begin snapshot is same as end snapshot
   * Expected ResultCode: APPEND_ONLY
   * Expected snapshot ranges: [(S1, S1)]
   */
  @Test
  public void testSameSnapshots() throws Exception {
    ...
    assertEquals(DatasetVerifyAppendOnlyResult.ResultCode.APPEND_ONLY, result.getResultCode());
    assertEquals(1, result.getSnapshotRanges().size());
    assertEquals(Long.toString(snapshot1.snapshotId()), result.getSnapshotRanges().get(0).getLeft());
    assertEquals(Long.toString(snapshot1.snapshotId()), result.getSnapshotRanges().get(0).getRight());
  }

  /**
   * Test metadata verify by calling {@link IcebergMetadataVerifyProcessors#isAppendOnlyBetweenSnapshots(Table, Long, Long)} directly
   * Test: S0 -> Append -> S1 -> Append -> S2 -> Compact -> S3
   * Expected ResultCode: APPEND_ONLY
   * Expected snapshot ranges: [(S0, S2)]
   */
  @Test
  public void testWithOneCompact() throws Exception {
    ...
    assertEquals(1, result.getSnapshotRanges().size());
    assertEquals(Long.toString(snapshot0.snapshotId()), result.getSnapshotRanges().get(0).getLeft());
    assertEquals(Long.toString(snapshot2.snapshotId()), result.getSnapshotRanges().get(0).getRight());
  }

  /**
   * Test metadata verify by calling {@link IcebergMetadataVerifyProcessors#isAppendOnlyBetweenSnapshots(Table, Long, Long)} directly
   * Test: S0 -> Append -> S1 -> Append -> S2 -> Compact -> S3 -> Append -> S4 -> Compact -> S5
   * Expected ResultCode: APPEND_ONLY
   * Expected snapshot ranges: [(S3, S4), (S0, S2)]
   */
  @Test
  public void testWithMultiCompact() throws Exception {
    ...
    assertEquals(Long.toString(snapshot3.snapshotId()), result.getSnapshotRanges().get(0).getLeft());
    assertEquals(Long.toString(snapshot4.snapshotId()), result.getSnapshotRanges().get(0).getRight());
    assertEquals(Long.toString(snapshot0.snapshotId()), result.getSnapshotRanges().get(1).getLeft());
    assertEquals(Long.toString(snapshot2.snapshotId()), result.getSnapshotRanges().get(1).getRight());
  }

  /**
   * Test metadata verify by calling {@link IcebergMetadataVerifyProcessors#isAppendOnlyBetweenSnapshots(Table, Long, Long)} directly
   * Test: S0 -> Append -> S1 -> Append -> S2 -> Compact -> S3 -> Append -> S4 -> Compact -> S5 -> Append -> S6
   * Expected ResultCode: APPEND_ONLY
   * Expected snapshot ranges: [(S5, S6), (S3, S4), (S0, S2)]
   */
  @Test
  public void testWithMultiCompactAndAppend() throws Exception {
    ...
    assertEquals(Long.toString(snapshot5.snapshotId()), result.getSnapshotRanges().get(0).getLeft());
    assertEquals(Long.toString(snapshot6.snapshotId()), result.getSnapshotRanges().get(0).getRight());
    assertEquals(Long.toString(snapshot3.snapshotId()), result.getSnapshotRanges().get(1).getLeft());
    assertEquals(Long.toString(snapshot4.snapshotId()), result.getSnapshotRanges().get(1).getRight());
    assertEquals(Long.toString(snapshot0.snapshotId()), result.getSnapshotRanges().get(2).getLeft());
    assertEquals(Long.toString(snapshot2.snapshotId()), result.getSnapshotRanges().get(2).getRight());
  }

  /**
   * Test metadata verify by calling {@link IcebergMetadataVerifyProcessors#isAppendOnlyBetweenSnapshots(Table, Long, Long)} directly
   * Test: S0 -> Append -> S1 -> Append -> S2 -> Compact -> S3 -> Compact -> S4 -> Append -> S5
   * Expected ResultCode: APPEND_ONLY
   * Expected snapshot ranges: [(S4, S5), (S0, S2)]
   */
  @Test
  public void testWithAdjacentCompacts() throws Exception {
    ...
    assertEquals(DatasetVerifyAppendOnlyResult.ResultCode.APPEND_ONLY, result.getResultCode());
    assertEquals(2, result.getSnapshotRanges().size());
    assertEquals(Long.toString(snapshot4.snapshotId()), result.getSnapshotRanges().get(0).getLeft());
    assertEquals(Long.toString(snapshot5.snapshotId()), result.getSnapshotRanges().get(0).getRight());
    assertEquals(Long.toString(snapshot0.snapshotId()), result.getSnapshotRanges().get(1).getLeft());
    assertEquals(Long.toString(snapshot2.snapshotId()), result.getSnapshotRanges().get(1).getRight());
  }
  ...
  /**
   * Test metadata verify by calling {@link IcebergMetadataVerifyProcessors#isAppendOnlyBetweenSnapshots(Table, Long, Long)} directly
   * Test: -> Compact -> S3 -> Compact -> S4
   * Expected ResultCode: APPEND_ONLY
   * Expected snapshot ranges: [(S3, S3)]
   */
  @Test
  public void testAllAdjacentCompacts() throws Exception {
    ...
    assertEquals(1, result.getSnapshotRanges().size());
    assertEquals(Long.toString(snapshot3.snapshotId()), result.getSnapshotRanges().get(0).getLeft());
    assertEquals(Long.toString(snapshot3.snapshotId()), result.getSnapshotRanges().get(0).getRight());
  }

  /**
   * Test metadata verify by calling {@link IcebergMetadataVerifyProcessors#isAppendOnlyBetweenSnapshots(Table, Long, Long)} directly
   * Test: -> Append -> S2 -> Compact -> S3 -> Compact -> S4
   * Expected ResultCode: APPEND_ONLY
   * Expected snapshot ranges: [(S2, S2)]
   */
  @Test
  public void testAppendAndAllAdjacentCompacts() throws Exception {
    ...
    assertEquals(DatasetVerifyAppendOnlyResult.ResultCode.APPEND_ONLY, result.getResultCode());
    assertEquals(1, result.getSnapshotRanges().size());
    assertEquals(Long.toString(snapshot2.snapshotId()), result.getSnapshotRanges().get(0).getLeft());
    assertEquals(Long.toString(snapshot2.snapshotId()), result.getSnapshotRanges().get(0).getRight());
  }

  /**
   * Test metadata verify by calling {@link IcebergMetadataVerifyProcessors#isAppendOnlyBetweenSnapshots(Table, Long, Long)} directly
   * Test: S0 -> Append -> S1 -> Append -> S2 -> Compact -> S3 -> Update -> S4 -> Compact -> S5 -> Append -> S6
   * Expected ResultCode: NOT_APPEND_ONLY
   */
  @Test
