...
/**
 * PartitionerSenderRootExec test to cover mostly part that deals with multithreaded ability to copy
 * and flush data
 */
public class TestPartitionSender extends PlanTestBase {
  ...
  //    groupByQuery = String.format("SELECT dept_id, count(*) as numEmployees FROM dfs.\"%s\" GROUP
  // BY dept_id", empTableLocation);
  //  }
  //
  //  @AfterClass
  ...
  //    }
  //
  //    final TopNOperator.SimpleRecordBatch incoming = new
  // TopNOperator.SimpleRecordBatch(container, sv, null);
  //
  ...
  //    final Fragment rootFragment = PopUnitTestBase.getRootFragmentFromPlanString(planReader,
  // plan);
  ...
  //   * @throws Exception
  //   */
  //  private void testThreadsHelper(HashToRandomExchange hashToRandomExchange, SabotContext
  // sabotContext, OptionList options,
  //      RecordBatch incoming, FunctionImplementationRegistry registry, PhysicalPlanReader
  // planReader, PlanningSet planningSet, Fragment rootFragment,
  ...
  // PhysicalOperatorUtil.getIndexOrderedEndpoints(Lists.newArrayList(sabotContext.getBits()));
  //
  ...
  //      try {
  //        context = new FragmentContext(sabotContext, planFragment, null, new
  // PassthroughQueryObserver(mockUserClientConnection(null)), registry);
  //        final int majorFragmentId = planFragment.getHandle().getMajorFragmentId();
  //        final HashPartitionSender partSender = new HashPartitionSender(majorFragmentId,
  // hashToRandomExchange, hashToRandomExchange.getExpression(), mfEndPoints);
  ...
  //        final int actualThreads = NODES_COUNT > expectedThreadsCount ? expectedThreadsCount :
  // NODES_COUNT;
  ...
  //          final List<PartitionOutgoingBatch> outBatch = (List<PartitionOutgoingBatch>)
  // part.getOutgoingBatches();
  ...
  //        }
  //        if ( actualThreads == 1 ) {
  //          assertEquals("With single thread parent and child waitNanos should match",
  // partitioners.get(0).getStats().getWaitNanos(),
  ...
  //        for (Partitioner part : partitioners ) {
  //          final List<PartitionOutgoingBatch> outBatches = (List<PartitionOutgoingBatch>)
  // part.getOutgoingBatches();
  ...
  //              assertEquals("Should add metricValue irrespective of exception", 5*actualThreads,
  // metric.getLongValue());
  //            }
  ...
  //      final int actualPartitions = outGoingBatchCount > numberPartitions ? numberPartitions :
  // outGoingBatchCount;
