...
public class TestViewSupport extends TestBaseViewSupport {
  ...
  public void viewWithPartialSchemaIdentifier() throws Exception {
    ...
    try {
      ...
      test("USE cp");
      queryViewHelper(
          String.format("SELECT * FROM %s.\"%s\" ORDER BY \"employeeid\" DESC LIMIT 1", "dfs_test", viewName),
          baselineColumns, baselineValues);
  ...
  public void viewResolvingTablesInWorkspaceSchema() throws Exception {
    ...
    try {
      ...
      // Query the view
      queryViewHelper(
          String.format("SELECT * FROM %s.\"%s\" ORDER BY region_id DESC LIMIT 1", "dfs_test", viewName),
          baselineColumns, baselineValues);

      // Change default schema to "dfs_test" and query by referring to the view using "tmp.viewName"
//      test("USE dfs_test");
//      queryViewHelper(
//          String.format("SELECT * FROM %s.\"%s\" ORDER BY region_id DESC LIMIT 1", "dfs_", viewName),
//          baselineColumns, baselineValues);
