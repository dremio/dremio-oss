...
public class TestViewSupport extends TestBaseViewSupport {
  ...
  public void referToSchemaInsideAndOutsideView() throws Exception {
    ...
    String selectInto =
        "create table monkey as select c_custkey, c_nationkey from cp.\"tpch/customer.parquet\"";
    String createView =
        "create or replace view myMonkeyView as select c_custkey, c_nationkey from monkey";
    String selectInside = "select * from myMonkeyView;";
    String use2 = "use cp;";
    String selectOutside = "select * from dfs_test.myMonkeyView;";
  ...
  /**
   * DRILL-2342 This test is for case where output columns are nullable. Existing tests already
   * cover the case where columns are required.
   */
  @Ignore("DX-2490")
  @Test
  ...
  public void viewWithSelectFieldsInDef_StarInQuery() throws Exception {
    testViewHelper(
        ...
        ImmutableList.of(new Object[] {0L, "None"}, new Object[] {1L, "San Francisco"}));
  }
  ...
  public void viewWithSelectFieldsInDef_SelectFieldsInQuery2() throws Exception {
    testViewHelper(
        ...
        ImmutableList.of(new Object[] {"None"}, new Object[] {"San Francisco"}));
  }
  ...
  public void viewWithSelectFieldsInDef_SelectFieldsInView_SelectFieldsInQuery2() throws Exception {
    testViewHelper(
        ...
        new String[] {"regionid"},
        ImmutableList.of(new Object[] {109L}, new Object[] {108L}));
  }
  ...
  public void viewWithUnionWithSelectFieldsInDef_StarInQuery() throws Exception {
    testViewHelper(
        ...
        "SELECT regionid FROM TEST_SCHEMA.TEST_VIEW_NAME LIMIT 2",
        new String[] {"regionid"},
        ImmutableList.of(new Object[] {110L}, new Object[] {108L}));
  }
  ...
  public void viewWithCompoundIdentifiersInDef() throws Exception {
    final String viewDef =
        "SELECT "
            ...
            + "FROM dfs_root.\"[WORKING_PATH]/src/test/resources/nation\"";
  ...
  public void createViewWhenViewAlreadyExists() throws Exception {
    ...
    try {
      ...
      final String createViewSql =
          String.format("CREATE VIEW %s.\"%s\" AS %s", TEMP_SCHEMA, viewName, viewDef1);
      errorMsgTestHelper(
          createViewSql,
          String.format(
              "A view with given name [%s] already exists in schema [%s]", viewName, TEMP_SCHEMA));

      // Try creating the view with same name in same schema, but with CREATE OR REPLACE VIEW clause
      final String viewDef2 =
          "SELECT sales_state_province FROM cp.\"region.json\" ORDER BY \"region_id\"";
      final String createOrReplaceViewSql =
          String.format("CREATE OR REPLACE VIEW %s.\"%s\" AS %s", TEMP_SCHEMA, viewName, viewDef2);
      ...
      testBuilder()
          ...
          .baselineValues(
              true, String.format("View '%s.%s' replaced successfully", TEMP_SCHEMA, viewName))
          .go();

      // Make sure the new view created returns the data expected.
      queryViewHelper(
          String.format("SELECT * FROM %s.\"%s\" LIMIT 1", TEMP_SCHEMA, viewName),
  ...
  public void createViewWhenATableWithSameNameAlreadyExists() throws Exception {
    ...
    try {
      ...
      // Try to create the view with same name in same schema.
      final String createViewSql =
          String.format("CREATE VIEW %s.\"%s\" AS %s", TEMP_SCHEMA, tableName, tableDef1);
      errorMsgTestHelper(
          createViewSql,
          String.format(
      ...
      final String viewDef2 =
          "SELECT sales_state_province FROM cp.\"region.json\" ORDER BY \"region_id\"";
      errorMsgTestHelper(
          String.format("CREATE OR REPLACE VIEW %s.\"%s\" AS %s", TEMP_SCHEMA, tableName, viewDef2),
          String.format(
              "A non-view table with given name [%s] already exists in schema [%s]",
              tableName, TEMP_SCHEMA));
    } finally {
  ...
  public void infoSchemaWithView() throws Exception {
    ...
    try {
      ...
      testBuilder()
          .sqlQuery(
              String.format(
                  "SELECT * FROM INFORMATION_SCHEMA.\"TABLES\" WHERE TABLE_NAME = '%s'", viewName))
          .unOrdered()
  ...
  public void viewWithPartialSchemaIdentifier() throws Exception {
    ...
    try {
      // Change default schema to just "dfs_test". View is actually created in "dfs_test.tmp"
      // schema.
      test("USE dfs_test");
      ...
      queryViewHelper(
          String.format(
              "SELECT * FROM %s.\"%s\" ORDER BY \"employeeid\" DESC LIMIT 1", "tmp", viewName),
          baselineColumns,
          baselineValues);

      // Change the default schema to "dfs_test.tmp" and query view by referring to it using
      // "viewName"
      test("USE dfs_test");
      queryViewHelper(
      ...
      // Change the default schema to "cp" and query view by referring to it using
      // "dfs_test.tmp.viewName";
      test("USE cp");
      queryViewHelper(
          String.format(
              "SELECT * FROM %s.\"%s\" ORDER BY \"employeeid\" DESC LIMIT 1", "dfs_test", viewName),
          baselineColumns,
          baselineValues);
  ...
  public void viewResolvingTablesInWorkspaceSchema() throws Exception {
    ...
    try {
      ...
      queryViewHelper(
          String.format(
              "SELECT * FROM %s.\"%s\" ORDER BY region_id DESC LIMIT 1", "dfs_test", viewName),
          baselineColumns,
          baselineValues);
      ...
      //      test("USE dfs_test");
      //      queryViewHelper(
      //          String.format("SELECT * FROM %s.\"%s\" ORDER BY region_id DESC LIMIT 1", "dfs_",
      // viewName),
  ...
  public void createViewWithDuplicateColumnsInDef2() throws Exception {
    createViewErrorTestHelper(
        "CREATE VIEW %s.%s AS SELECT region_id, sales_city, sales_city FROM cp.\"region.json\"",
  ...
  private static void createViewErrorTestHelper(final String viewSql, final String expErrorMsg)
      throws Exception {
  ...
  public void showProperMsgWhenDroppingNonExistentView() throws Exception {
    errorMsgTestHelper(
        "DROP VIEW dfs_test.nonExistentView", "Unknown view [dfs_test.nonExistentView].");
  }
  ...
  public void showProperMsgWhenTryingToDropANonViewTable() throws Exception {
    ...
    try {
      test(
          String.format(
              "CREATE TABLE %s.%s AS SELECT c_custkey, c_nationkey from cp.\"tpch/customer.parquet\"",
              TEMP_SCHEMA, testTableName));

      errorMsgTestHelper(
          String.format("DROP VIEW %s.%s", TEMP_SCHEMA, testTableName),
  ...
  public void dropViewIfExistsWhenItIsATable() throws Exception {
    ...
    try {
      ...
      test(
          String.format(
              "CREATE TABLE %s.%s as SELECT region_id, sales_city FROM cp.\"region.json\"",
  ...
  public void testDuplicateViewWithCorrelatedSubQueryFilter() throws Exception {
    ...
    try {
      ...
      queryViewHelper(
          String.format(
              "select count(*) AS cnt FROM (SELECT n_nationkey FROM %s.\"%s\" WHERE n_regionkey IN (0,1,2) UNION ALL "
                  + "SELECT n_nationkey FROM %s.\"%s\" WHERE n_regionkey in (3, 4))",
  ...
  public void testDuplicateViewWithCorrelatedSubQueryNested() throws Exception {
    ...
    try {
      ...
      queryViewHelper(
          String.format(
              "SELECT count(*) AS cnt FROM (SELECT n_nationkey FROM %s.\"%s\" WHERE n_nationkey IN (0,1,2) UNION ALL "
                  + "SELECT n_nationkey FROM %s.\"%s\" WHERE n_nationkey IN (3, 4))",
              TEMP_SCHEMA, duplicateView, TEMP_SCHEMA, duplicateView),
          new String[] {"cnt"},
          ImmutableList.of(new Object[] {5L}));
    } finally {
