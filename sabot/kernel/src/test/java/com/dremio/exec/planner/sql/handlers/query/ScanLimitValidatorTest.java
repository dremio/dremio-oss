...
class ScanLimitValidatorTest {
  ...
  private static final RelDataType INTEGER = JavaTypeFactoryImpl.INSTANCE.createTypeWithNullability(JavaTypeFactoryImpl.INSTANCE.createSqlType(SqlTypeName.INTEGER), true);
  ...
  public ScanLimitValidatorTest() {
    ...
    HepPlanner planner = new HepPlanner(new HepProgramBuilder().build(), context, false, null, new DremioCost.Factory());
    schema = schemaFor(
        table("users",
  ...
  public void shouldDoNothingScanColumnsCountIsNotOverLimit() {
    ...
    RelNode node = builder
        .scan("users")
        .limit(100, 1000)
        .project(rexBuilder.makeLiteral("test me"))
        .build();
  ...
  public void shouldThrowExceptionIfOverLimit() {
    ...
    Assertions.assertThrows(ValidationException.class, () -> ScanLimitValidator.ensureLimit(node, 4));
  }

  @Test
  public void shouldAccountForNestedColumnsWhenCalculatingLimit() {
    RelDataType structType = JavaTypeFactoryImpl.INSTANCE.createTypeWithNullability(JavaTypeFactoryImpl.INSTANCE.createStructType(
        ImmutableList.of(
    ...
    RelOptSchema customSchema = schemaFor(
        table("users",
  ...
  private Tuple<String, RelDataType> table(String name, ImmutableList<String> fieldNames) {
    ImmutableList<RelDataType> types = fieldNames.stream().map(unused -> INTEGER).collect(ImmutableList.toImmutableList());
    return table(name, fieldNames, types);
  }

  private Tuple<String, RelDataType> table(String name, ImmutableList<String> fieldNames, ImmutableList<RelDataType> types) {
  ...
  private RelOptSchema schemaFor(Tuple<String, RelDataType>... tables) {
    ...
    RelOptSchema schema = new RelOptSchema() {
    ...
    for (Tuple<String, RelDataType> table : tables) {
      relTables.put(Collections.singletonList(table.first),
