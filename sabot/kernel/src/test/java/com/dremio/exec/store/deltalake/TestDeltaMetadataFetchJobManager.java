...
public class TestDeltaMetadataFetchJobManager extends BaseTestQuery {
  ...
  public void testSingleBatchReadLatest() {
    DeltaMetadataFetchJobManager manager =
        new DeltaMetadataFetchJobManager(sabotContext, fs, selection, null);
    ...
    List<Long> actualVersions =
        snapshotList.stream().map(x -> x.getVersionId()).collect(Collectors.toList());
  ...
  public void testMultipleBatchesReadLatest() {
    ...
    // Last batch is empty batch as on reading 26 we will figure out that the next file doesn't
    // exist
  ...
  private void testSingleBatchWithoutCheckpoint(TimeTravelOption.TimeTravelRequest travelRequest) {
    DeltaMetadataFetchJobManager manager =
        new DeltaMetadataFetchJobManager(sabotContext, fs, selection, travelRequest);
  ...
  private void testMultipleBatchesWithCheckpoint(TimeTravelOption.TimeTravelRequest travelRequest) {
    ...
    List<Long> expectedVersions =
        Arrays.asList(18L, 17L, 16L, 15L, 14L, 13L, 12L, 11L, 10L, 9L, 8L, 7L);
    List<Long> actualVersions =
        snapshotList.stream().map(x -> x.getVersionId()).collect(Collectors.toList());
  ...
  private void testVeryLargeBatchSize(TimeTravelOption.TimeTravelRequest travelRequest) {
    ...
    List<Long> expectedVersions =
        Arrays.asList(
            22L, 21L, 20L, 19L, 18L, 17L, 16L, 15L, 14L, 13L, 12L, 11L, 10L, 9L, 8L, 7L, 6L, 5L, 4L,
            3L);
    List<Long> actualVersions =
        snapshotList.stream().map(x -> x.getVersionId()).collect(Collectors.toList());
  ...
  public void testEndingWithCheckpointDataset(TimeTravelOption.TimeTravelRequest travelRequest)
      throws Exception {
    ...
    List<Long> expectedVersions =
        manager.isReadLatest() ? Arrays.asList(10L) : Arrays.asList(10L, 9L, 8L);
    List<Long> actualVersions =
        snapshotList.stream().map(x -> x.getVersionId()).collect(Collectors.toList());
  ...
  public void testMetadataFetchJobConcurrencyDefault() throws Exception {
    testMetadataFetchJobConcurrency(TimeTravelOption.newSnapshotIdRequest("22"));
    assertTrue(
        "The Active worker count should be zero after work.",
        DeltaMetadataFetchPool.getPool().getActiveCount() == 0);
    assertTrue(
        "The largest number of threads that have ever simultaneously been in the pool should be > one.",
