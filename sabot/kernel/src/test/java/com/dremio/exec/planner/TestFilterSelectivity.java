...
public class TestFilterSelectivity extends BaseTestQuery {
  ...
  public void testFilterSelectivityOptions() throws Exception {
    ...
    test(
        String.format(
            "alter system set \"planner.filter.min_selectivity_estimate_factor\" = %f", 0.25));
    test(
        String.format(
            "alter system set \"planner.filter.max_selectivity_estimate_factor\" = %f", 0.75));
  }
  ...
  public void testFilterSelectivityOptionsNegative() throws Exception {
    String query1 =
        String.format(
            "alter session set \"planner.filter.min_selectivity_estimate_factor\" = %f", -0.5);
    String errMsg1 =
        "Option planner.filter.min_selectivity_estimate_factor must be between 0.000000 and 1.000000";
  ...
  public void testFilterSelectivityOptionsAbove1() throws Exception {
    String query3 =
        String.format(
            "alter session set \"planner.filter.max_selectivity_estimate_factor\" = %f", 1.5);
    String errMsg3 =
        "Option planner.filter.max_selectivity_estimate_factor must be between 0.000000 and 1.000000";
  ...
  public void testFilterSelectivityOnQuery() throws Exception {
    test(
        String.format(
            "alter session set \"planner.filter.max_selectivity_estimate_factor\" = %f", 1.0));
    test(
        String.format(
            "alter session set \"planner.filter.min_selectivity_estimate_factor\" = %f", 0.9));
    ...
    test(
        String.format(
            "alter session set \"planner.filter.min_selectivity_estimate_factor\" = %f", 0.1));
    final String[] expectedPlan1 = {
      "Filter\\(condition.*\\).*rowcount = 1186.5,.*",
    ...
    final String[] expectedPlan2 = {
      "Filter\\(condition.*\\).*rowcount = 4271.4.*",
      "Scan.*columns=\\[`employee_id`, `department_id`\\].*rowcount = 4746.0.*"
    };
