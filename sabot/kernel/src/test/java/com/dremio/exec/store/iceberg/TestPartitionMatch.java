    ...
    for (String testSchema: SCHEMAS_FOR_TEST) {
      ...
      try (AutoCloseable c = enableIcebergTables()) {
        try {
          ...
          testBuilder()
            .sqlQuery(String.format("select o_orderkey, o_orderdate from %s.%s where o_orderdate='1992-02-21'", testSchema, tableName))
            .unOrdered()
          ...
          testBuilder()
            .sqlQuery(String.format("select o_orderkey, o_orderdate from %s.%s where o_orderdate>'1992-02-20' and o_orderdate<'1992-02-22'", testSchema, tableName))
            .unOrdered()
            .baselineColumns("o_orderkey", "o_orderdate")
            .baselineValues(6, LocalDateTime.parse("1992-02-21T00:00:00.000"))
            .build()
    ...
    for (String testSchema: SCHEMAS_FOR_TEST) {
      ...
      try (AutoCloseable c = enableIcebergTables()) {
        try {
          ...
          testBuilder()
            .sqlQuery(String.format("select count(*) c from %s.%s where DEC9=-123.1234000000", testSchema, tableName))
            .unOrdered()
            .baselineColumns("c")
            .baselineValues(1L)
            .build()
