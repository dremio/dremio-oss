...
public class TestReader3 extends BaseTestQuery {
  ...
  @Rule public final TestRule TIMEOUT = TestTools.getTimeoutRule(50000, TimeUnit.SECONDS);
  ...
  public void createNewFile() throws Exception {
    ...
    test(
        "create table dfs_root.\"/tmp/bah\" as select L_ORDERKEY, L_LINENUMBER, L_COMMENT from dfs_root.opt.data.sfb");
  }
  ...
  public void filtered1() throws Exception {
    test(
        "select L_ORDERKEY, L_LINENUMBER from dfs_root.opt.data.sf1p_head2 where L_ORDERKEY = 3 limit 10");
  }
  ...
  public void allThree() throws Exception {
    ...
    for (int i = 0; i < 10000; i++) {
      testNoResult(
          "select count(*) from dfs_root.tmp.bah where L_COMMENT is not null and L_LINENUMBER is not null and L_ORDERKEY is not null");
    }
  }
  ...
  public void xxx() throws Exception {
    ...
    testNoResult("set planner.disable_exchanges=true");
    test(
        "create table dfs_root.\"/tmp/bah6\" as select case when mod(L_ORDERKEY, 2) = 0 then L_ORDERKEY ELSE NULL END as L_ORDERKEY, L_LINENUMBER, case when mod(L_ORDERKEY, 2) = 0 then L_COMMENT ELSE NULL END as L_COMMENT from dfs_root.opt.data.bah");
  }
  ...
  public void everyOtherNull() throws Exception {
    ...
    for (int i = 0; i < 10000; i++) {
      //      testNoResult("select count(L_ORDERKEY) from dfs_root.tmp.bah4 where L_ORDERKEY/3 =
      // 1");
  ...
  public void testX() throws Exception {
    test(
        "select * from dfs_root.\"/src/dremio/contrib/data/tpch-sample-data/target/tpch/orders.parquet\" where o_orderpriority = 'blue'");
    ...
    //    test("select sum(c_custkey), sum(c_nationkey), sum(c_acctbal), min(c_mktsegment),
    // min(c_comment) from dfs_root.\"/opt/data/tpch10/customer/c0/0_0_0.parquet\"");
  ...
  public void everyOtherNullx() throws Exception {
    testNoResult("set planner.disable_exchanges=true");
    testNoResult(
        "create table dfs_root.\"tmp/chunk/old2\" STORE AS (type => 'text', fieldDelimiter => ',') as select L_COMMENT from dfs_root.opt.data.bah5 OFFSET 23900000 FETCH NEXT 100 ROWS ONLY");
  }
  ...
  public void simpleScan() throws Exception {
    ...
    for (int i = 0; i < 10000; i++) {
      ...
      //      testNoResult("create table dfs_root.\"tmp/oldReaderFile\" store as (type => 'json',
      // prettyPrint => false) as select L_ORDERKEY, L_COMMENT from dfs_root.opt.data.bah5");
    }
  }
  ...
  public void x() throws Exception {
    testNoResult("set planner.disable_exchanges=true");
    testNoResult(
        "create table dfs_root.\"tmp/chunk/gb5\" as select cast(l_linenumber as varchar) as varline, cast(l_linenumber as int) as l_linenumber from dfs_root.opt.data.bah5 limit 10");
  }
  ...
  public void everyOtherNull2() throws Exception {
    ...
    for (int i = 0; i < 10000; i++) {
      //      test("select l_linenumber, l_linenumber+100, l_linenumber+1000, l_linenumber+10000,
      // l_linenumber+100000, count(l_linenumber) from dfs_root.\"tmp/chunk/gb2/01.parquet\" group
      // by l_linenumber, l_linenumber+100, l_linenumber+1000, l_linenumber+10000,
      // l_linenumber+100000");
      //      test("select varline, varline || 'a', varline || 'b', varline || 'c', varline || 'd',
      // varline || 'e', count(l_linenumber), count(l_linenumber*2), count(l_linenumber*3),
      // count(l_linenumber*4) from dfs_root.\"tmp/chunk/gb4\" group by varline, varline || 'a',
      // varline || 'b', varline || 'c', varline || 'd', varline || 'e'");
      //      test("select varline, varline || 'a', count(l_linenumber) from
      // dfs_root.\"tmp/chunk/gb4\" group by varline, varline || 'a'");
      test("select varline, count(l_linenumber) from dfs_root.\"tmp/chunk/gb4\" group by varline");
      //      test("select varline, varline || 'a', varline || 'b', varline || 'c',
      // count(l_linenumber), count(l_linenumber*2), count(l_linenumber*3), count(l_linenumber*4)
      // from dfs_root.\"tmp/chunk/gb4\" group by varline, varline || 'a', varline || 'b', varline
      // || 'c'");
    }
  }
  ...
  public void anotherTest() throws Exception {
    test(
        "select c_date, count(distinct c_integer) from dfs.\"/Users/jnadeau/Downloads/0_0_0.parquet\" group by c_date");
    //    test("select c_date, c_integer from dfs.\"/Users/jnadeau/Downloads/0_0_0.parquet\" where
    // c_date = date '2015-03-06' group by c_date, c_integer");
    //    test("select distinct c_integer from dfs.\"/Users/jnadeau/Downloads/0_0_0.parquet\" where
    // c_date = date '2015-03-06'");
  }
}
