...
public class TestRexFieldAccessUtils {
  ...
  public void setUp() {
    ...
    final PlannerSettings plannerSettings = new PlannerSettings(DremioTest.DEFAULT_SABOT_CONFIG, optionResolver, () -> info);
  ...
  public void testWrapUnwrapNoNestedFields() {
    final FilesystemScanDrel leftScan = new FilesystemScanDrel(cluster, traits, table, pluginId, metadata, projectedColumns, 0, ImmutableList.of(), false, SnapshotDiffContext.NO_SNAPSHOT_DIFF);
    final ProjectRel originalProject = ProjectRel.create(cluster, traits, leftScan,
    ...
    assertEquals("Wrap then Unwrap resulted in an unexpected change", RelOptUtil.toString(originalProject), RelOptUtil.toString(unwrappend));
  }
  ...
  public void testUnwrapWithNestedFields() {
    ...
    final ProjectRel originalProject = ProjectRel.create(cluster, traits, leftScan,
      ImmutableList.of(builder.makeInputRef(leftScan, 0),
    ...
    final RexCall rexCallInner = (RexCall) builder.makeCall(leftScan.getRowType().getField("col3",false,false).getType()
      ,STRUCTURED_WRAPPER,ImmutableList.of(innerMost));
    final RexCall rexCallMiddle = (RexCall) builder.makeCall(leftScan.getRowType().getField("col3",false,false).getType()
      ,STRUCTURED_WRAPPER,ImmutableList.of(rexCallInner));
    final RexCall rexCallOuter = (RexCall) builder.makeCall(leftScan.getRowType().getField("col3",false,false).getType()
      ,STRUCTURED_WRAPPER,ImmutableList.of(rexCallMiddle));
    final RexFieldAccessUtils.StructuredReferenceWrapper unwrapper = new RexFieldAccessUtils.StructuredReferenceWrapper(originalProject.getCluster().getRexBuilder(), false);
