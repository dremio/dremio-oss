...
public class TestEarlyLimit0Optimization extends BaseTestQuery {
  ...
  public void simpleSelect() throws Exception {
    ...
    testBuilder()
        ...
        .baselineValues(
            ...
            1,
            1L,
            formatter.parseLocalDateTime("1961-08-26"),
            formatter.parseLocalDateTime("1994-12-01"),
            80000.0D,
            80000.0F,
            true,
            "Graduate Degree",
  ...
  public void sums() throws Exception {
    ...
    testBuilder().sqlQuery(wrapLimit0(query)).schemaBaseLine(expectedSchema).go();
  ...
  public void stddev() throws Exception {
    final String query =
        "select stddev(student_id),stddev(age),stddev(gpa),stddev(studentnum) from "
            + stddevTypesViewName
            + " where age > 30";
    ...
    testBuilder()
        ...
        .baselineValues(
            14.620274929849652D, 13.017389869877539D, 0.8640723208214439D, 2.9986511701873456E11D)
        .go();
    testBuilder().sqlQuery(wrapLimit0(query)).schemaBaseLine(expectedSchema).go();
  ...
  public void mod() throws Exception {
    final String query =
        "select mod(student_id, 3), mod(age, 2), mod(studentnum, 1235) from "
            + stddevTypesViewName
            + " where student_id=10";
  ...
  public void position() throws Exception {
    final String query =
        "select position('i' in full_name) from " + viewName + " where full_name like '%i%'";
    ...
    testBuilder().sqlQuery(query).ordered().baselineColumns("EXPR$0").baselineValues(5).go();
  ...
  public void charlength() throws Exception {
    final String query =
        "select char_length(full_name) from " + viewName + " where full_name like '%i%'";
    ...
    testBuilder().sqlQuery(query).ordered().baselineColumns("EXPR$0").baselineValues(12).go();
  ...
  public void nullableCount() throws Exception {
    final String query =
        "SELECT "
            + "COUNT(CASE WHEN position_id = 1 THEN NULL ELSE position_id END) AS c FROM "
            + viewName;
    ...
    testBuilder().sqlQuery(query).ordered().baselineColumns("c").baselineValues(0L).go();
  ...
  public void nullableSumAndCount() throws Exception {
    ...
    testBuilder().sqlQuery(query).ordered().baselineColumns("c", "p").baselineValues(1L, null).go();
  ...
  public void castSum() throws Exception {
    ...
    @SuppressWarnings("unchecked")
    final List<Pair<SchemaPath, MajorType>> expectedSchema =
        Lists.newArrayList(Pair.of(SchemaPath.getSimplePath("s"), Types.optional(MinorType.INT)));

    testBuilder().sqlQuery(query).ordered().baselineColumns("s").baselineValues(18422).go();
  ...
  public void sumCast() throws Exception {
    ...
    testBuilder().sqlQuery(query).ordered().baselineColumns("s").baselineValues(18422L).go();
  ...
  public void rank() throws Exception {
    final String query =
        "SELECT RANK() OVER(PARTITION BY employee_id ORDER BY employee_id) AS r FROM " + viewName;
    ...
    testBuilder().sqlQuery(query).ordered().baselineColumns("r").baselineValues(1L).go();
  ...
  public void cast() throws Exception {
    final String query =
        "SELECT CAST(fsalary AS DOUBLE) AS d,"
            + "CAST(employee_id AS BIGINT) AS e FROM "
            + viewName;
  ...
  public void extract() throws Exception {
    ...
    testBuilder().sqlQuery(query).baselineColumns("e").ordered().baselineValues(1994L).go();
  ...
  public void binary() throws Exception {
    final String query =
        "SELECT "
            ...
            + "position_id + position_id AS p FROM "
            + viewName;
  ...
  public void substringTest(final String query) throws Exception {
    ...
    testBuilder().sqlQuery(query).baselineColumns("s").ordered().baselineValues("Sheri").go();
