...
public class AvroTestUtil {
   ...
   * by the Dremio test builder to describe expected results.
   */
  public static class AvroTestRecordWriter implements Closeable {
    ...
    private AvroTestRecordWriter(Schema schema, File file) {
      writer = new DataFileWriter<>(new GenericDatumWriter<GenericData.Record>(schema));
      try {
    ...
    public void put(String key, Object value) {
      ...
      // in the Dremio result set in the test framework
      currentExpectedRecord.put("`" + key + "`", convertAvroValToDremio(value, true));
    }
    ...
    private Object convertAvroValToDremio(Object value, boolean root) {
      if (value instanceof ByteBuffer) {
        ByteBuffer bb = ((ByteBuffer)value);
        byte[] val = new byte[((ByteBuffer)value).remaining()];
        bb.get(val);
        bb.position(0);
        value = val;
        ...
        for (Object o : array) {
          list.add(convertAvroValToDremio(o, false));
        }
        value = list;
        ...
        for (Schema.Field field : rec.getSchema().getFields()) {
          ...
          newRecord.put(field.name(), convertAvroValToDremio(val, false));
        }
        value = newRecord;
      }
      return value;
  ...
  public static AvroTestRecordWriter generateSimplePrimitiveSchema_NoNullValues(int numRecords) throws Exception {
    ...
    final Schema schema = SchemaBuilder.record("AvroRecordReaderTest")
            .namespace("com.dremio.exec.store.avro")
            .fields()
  ...
  /**
   * generates an avro file similar to the following json records:
   * <pre>
   *   { "a_int":0, "b_map":{ "a":1, "b":2 } }
   *   { "a_int":1, "b_map":null }
   *   { "a_int":2, "b_map":{} }
   *   { "a_int":3 }
   * </pre>
   */
  static AvroTestRecordWriter generateNullAndEmptyMaps() throws Exception {

    final File file = File.createTempFile("avro-null-empty-maps", ".avro");
    file.deleteOnExit();
    ...
    final Schema schema = SchemaBuilder.record("AvroRecordReaderTest")
            ...
            .name("b_map").type().optional().map().values(Schema.create(Type.STRING))
            .endRecord();
   ...
   * </pre>
   */
  static AvroTestRecordWriter generateNullAndEmptyLists() throws Exception {
    ...
    final Schema schema = SchemaBuilder.record("AvroRecordReaderTest")
            ...
            .fields()
            .name("a_int").type().intType().noDefault()
            .name("b_list").type().optional().array().items().stringType()
            .endRecord();

    final Schema listSchema = schema.getField("b_list").schema().getTypes().get(1);
    ...
    try {
      // add a non empty list
      {
        ...
        GenericArray<String> array = new GenericData.Array<>(2, listSchema);
        array.add("1");
        array.add("2");
        record.put("b_list", array);
        record.endRecord();
      }
      // add a null list
      {
  ...
  public static String generateLinkedList() throws Exception {
    ...
    final DataFileWriter<GenericRecord> writer = new DataFileWriter<>(new GenericDatumWriter<GenericRecord>(schema));
    writer.create(schema, file);
