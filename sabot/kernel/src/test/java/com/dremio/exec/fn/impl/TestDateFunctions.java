...
public class TestDateFunctions extends PopUnitTestBase {
  ...
  private void testCommon(String[] expectedResults, String physicalPlan, String resourceFile)
      throws Exception {
    try (ClusterCoordinator clusterCoordinator = LocalClusterCoordinator.newRunningCoordinator();
      ...
      try (RecordBatchLoader batchLoader = new RecordBatchLoader(getTestAllocator())) {
        ...
        for (VectorWrapper<?> v : batchLoader) {
          ...
          assertEquals(expectedResults[i++], vv.getObject(0).toString());
        }
      }
  ...
  public void testDateIntervalArithmetic() throws Exception {
    ...
    testCommon(
        expectedResults, "/functions/date/date_interval_arithmetic.json", "/test_simple_date.json");
  }

  @Test
  public void testDateDifferenceArithmetic() throws Exception {

    String[] expectedResults = {"365", "P-366DT-60S"};
    testCommon(
        expectedResults,
        "/functions/date/date_difference_arithmetic.json",
  ...
  public void testAge() throws Exception {
    String[] expectedResults = {"P109M16DT82800S", "P172M27D", "P-172M-27D", "P-39M-18DT-63573S"};
  ...
  public void testIntervalArithmetic() throws Exception {
    ...
    testCommon(
        expectedResults, "/functions/date/interval_arithmetic.json", "/test_simple_date.json");
  }
  ...
  public void testToChar() throws Exception {
    ...
    String[] expectedResults = {
      (new LocalDate(2008, 2, 23)).toString("yyyy-MMM-dd"),
