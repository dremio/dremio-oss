...
public class TestPrefetchingIterator {
  ...
  private void verifyPrefetched(int startIndex, int numPrefetch, List<SplitReaderCreator> creators)
      throws Exception {
    int i = 0;
  ...
  private void testIteratorWithFilterAddedInBetween(boolean fromRowGroupSplits) throws Exception {
    CompositeReaderConfig readerConfig = mock(CompositeReaderConfig.class);
    when(readerConfig.getPartitionNVPairs(
    ...
    SplitReaderCreatorIterator creators =
        createSplitReaderCreator(ctx, fromRowGroupSplits, true, 1, 10, readerConfig);
  ...
  private void testIteratorWithFilterAddedInBetweenPrefetch(
      boolean fromRowGroupSplits, int numPrefetch) throws Exception {
    CompositeReaderConfig readerConfig = mock(CompositeReaderConfig.class);
    when(readerConfig.getPartitionNVPairs(
            any(BufferAllocator.class), any(SplitAndPartitionInfo.class)))
        .thenReturn(getNonMatchingNameValuePairs());
    ...
    OperatorContext ctx = getCtx();
    SplitReaderCreatorIterator creators =
        createSplitReaderCreator(ctx, fromRowGroupSplits, true, numPrefetch, 10, readerConfig);
    PrefetchingIterator it = new PrefetchingIterator(creators);
  ...
  private void testIteratorWithFilterSomeSkipped(boolean fromRowGroupSplits) throws Exception {
    ...
    OperatorContext ctx = getCtx();
    SplitReaderCreatorIterator creators =
        createSplitReaderCreator(
            ctx, fromRowGroupSplits, true, 1, 10, readerConfig, isSelectedSplit, null);
    PrefetchingIterator it = new PrefetchingIterator(creators);
    try (AutoCloseables.RollbackCloseable closer = new AutoCloseables.RollbackCloseable()) {
      RuntimeFilter filter = prepareRuntimeFilter();
      closer.add(filter.getPartitionColumnFilter().getBloomFilter());
  ...
  private void testIteratorWithFilterSomeSkippedPrefetch(
      boolean fromRowGroupSplits, int numPrefetch) throws Exception {
    Predicate<Integer> isSelectedSplit = i -> i == 1 || i == 3 || i == 9;
    CompositeReaderConfig readerConfig = mock(CompositeReaderConfig.class);
    ...
    SplitReaderCreatorIterator creators =
        createSplitReaderCreator(
            ctx, fromRowGroupSplits, true, numPrefetch, 10, readerConfig, isSelectedSplit, null);
  ...
  private void testMultipleFilters(boolean fromRowGroupSplits) throws Exception {
    ...
    SplitReaderCreatorIterator creators =
        createSplitReaderCreator(
            ctx, fromRowGroupSplits, true, 1, 10, readerConfig, isSelectedSplit1, isSelectedSplit2);
  ...
  private List<NameValuePair<?>> getMatchingNameValuePairs() {
    return Collections.singletonList(
        new ConstantColumnPopulators.IntNameValuePair("partitionColumn", 1));
  }
  ...
  private List<NameValuePair<?>> getNonMatchingNameValuePairs() {
    return Collections.singletonList(
        new ConstantColumnPopulators.IntNameValuePair("partitionColumn", 100));
  }
  ...
  private SplitReaderCreatorIterator createSplitReaderCreatorIterator(
      List<SplitReaderCreator> creators) {
  ...
  class SplitReaderCreatorTest extends SplitReaderCreator {
    ...
    SplitReaderCreatorTest(int idx, int numPrefetch) {
      ...
      PartitionProtobuf.NormalizedPartitionInfo partitionInfo =
          PartitionProtobuf.NormalizedPartitionInfo.newBuilder().build();
  ...
  private ParquetSplitReaderCreatorIterator createSplitReaderCreator(
    ...
    return createSplitReaderCreator(
        context, fromRowGroupSplits, prefetch, numPrefetch, numSplits, readerConfig, null, null);
  }
  ...
  private ParquetSplitReaderCreatorIterator createSplitReaderCreator(
      ...
      int numSplits,
      CompositeReaderConfig readerConfig,
      Predicate<Integer> isSelectedSplit1,
      Predicate<Integer> isSelectedSplit2)
      throws Exception {
    ...
    return new ParquetSplitReaderCreatorIterator(
        fragmentExecutionContext, context, config, readerConfig, fromRowGroupSplits);
  }
