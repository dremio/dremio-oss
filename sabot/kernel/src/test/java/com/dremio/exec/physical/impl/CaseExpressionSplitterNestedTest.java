...
public class CaseExpressionSplitterNestedTest extends BaseExpressionSplitterTest {
  ...
  private static final String nestedCaseExpr =
      "(case when (greater_than_or_equal_to(c0, c1)) then ("
          + innerThenCaseExpr
          + ")"
          + "when (greater_than_or_equal_to(c0, "
          + innerWhenCaseExpr
          + ")) then (add(c1, c0)) "
          + "when (less_than_or_equal_to(c0, c3)) then (multiply(c0, c3))  "
          + "else (add(c1, c4)) end)";

  private static final Fixtures.Table nestedCaseInput =
      Fixtures.split(
          th("c0", "c1", "c2", "c3", "c4"),
          5,
  ...
  private static final Fixtures.Table nestedCaseOutput =
      t(th("out"), tr(80), tr(26), tr(400), tr(20), tr(1), tr(40), tr(45), tr(111), tr(500));

  private static final Fixtures.Table nestedCaseSimpleOutput =
      t(th("out"), tr(-10), tr(13), tr(28), tr(28), tr(28), tr(126), tr(126), tr(112), tr(1));

  private static final Fixtures.Table nestedCaseWithConstantsOutput =
      t(th("out"), tr(80), tr(12), tr(11), tr(20), tr(1), tr(13), tr(130), tr(1300), tr(500));

  @Test
  public void testNestedNoSplitsJava() throws Exception {
    GandivaAnnotator annotator = new GandivaAnnotatorCase();
    Split[] expSplits = new Split[] {new Split(false, "out", nestedCaseExpr, 1, 0)};
  ...
  public void testNestedNoSplitsGandiva() throws Exception {
    GandivaAnnotator annotator =
        new GandivaAnnotatorCase(
            ...
            "multiply",
            "divide");
    Split[] expSplits = new Split[] {new Split(true, "out", nestedCaseExpr, 1, 0)};
    splitAndVerifyCase(nestedCaseQuery, nestedCaseInput, nestedCaseOutput, expSplits, annotator);
  }
  ...
  public void testNestedCaseQuerySimple() throws Exception {
    ...
    GandivaAnnotator annotator =
        new GandivaAnnotatorCase("greater_than_or_equal_to", "less_than_or_equal_to", "add");
    Split[] expSplits =
        new Split[] {
    ...
    splitAndVerifyCase(
        nestedCaseQuerySimple, nestedCaseInput, nestedCaseSimpleOutput, expSplits, annotator);
  }
  ...
  public void testNestedMixedSplits() throws Exception {
    final String _xxx0g = "(case when (greater_than_or_equal_to(c0, c1)) then (0i) else (-1i) end)";
    final String _xxx1g =
        "(if (less_than(_xxx0,0i)) then (greater_than_or_equal_to(c2,c1)) "
            + "else (cast((__$internal_null$__) as bit)) end)";
    final String _xxx2j =
        "(if (less_than(_xxx0, 0i)) then "
    ...
    final String _xxx5g =
        "(if (equal(_xxx4, 0i)) then ((case when (greater_than_or_equal_to(c0, c2)) then (0i) "
            + "else (-1i) end)) else (cast((__$INTERNAL_NULL$__) as INT)) end)";
    final String _xxx6j =
        "(if (equal(_xxx4, 0i)) then ((case when (greater_than(_xxx5, -1i)) then (_xxx5) "
            + "when (less_than_or_equal_to(c0, c3)) then (1i) else (-1i) end)) else (cast((__$INTERNAL_NULL$__) as INT)) end)";
    final String _xxx7g =
        "(if (equal(_xxx4, 0i)) then ((case when (greater_than(_xxx6, -1i)) then (_xxx6) "
            + "when (greater_than_or_equal_to(c0, c4)) then (2i) else (3i) end)) "
            + "else (cast((__$INTERNAL_NULL$__) as INT)) end)";
    final String _xxx8j =
        "(if (equal(_xxx4, 0i)) then ((if (equal(_xxx7, 3i)) then "
    ...
    final String _xxx9j =
        "(if (equal(_xxx4, 0i)) then ((case when (equal(_xxx7, 1i)) then (add(c0, c3)) "
            + "else (cast((__$INTERNAL_NULL$__) as INT)) end)) else (cast((__$INTERNAL_NULL$__) as INT)) end) ";
    final String _xxx10j =
        "(case when (equal(_xxx4, 1i)) then (add(c1, c0)) "
            + "when (equal(_xxx4, 3i)) then (add(c1, c4)) else (cast( (__$INTERNAL_NULL$__) as INT)) end)";
    final String _xxx11g =
        "(case when (equal(_xxx4, 0i)) then ((case when (equal(_xxx7, 3i)) then "
    ...
    GandivaAnnotator annotator =
        new GandivaAnnotatorCase("greater_than_or_equal_to", "multiply", "divide");
    Split[] expSplits =
        new Split[] {
  ...
  public void testCorrectnessWithRandomFunctionsInGandiva() throws Exception {
    final String[] fns = {
      "add", "subtract", "divide", "multiply", "less_than_or_equal_to", "greater_than_or_equal_to"
    };
  ...
  public void testCorrectnessForConstantWithRandomFunctionsInGandiva() throws Exception {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      GandivaAnnotator annotator = new GandivaAnnotatorCase(chosenFns.toArray(new String[0]));
      splitAndVerifyCase(
          nestedCaseQueryWithConstants,
          nestedCaseInput,
          nestedCaseWithConstantsOutput,
          null,
          annotator);
    }
  }
