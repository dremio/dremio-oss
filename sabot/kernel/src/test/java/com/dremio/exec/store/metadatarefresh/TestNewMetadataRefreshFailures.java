...
public class TestNewMetadataRefreshFailures extends BaseTestQuery {
  ...
  FileFilter manifestFileFilter =
      pathname -> pathname.getName().toUpperCase().endsWith("avro".toUpperCase());
  ...
  public void testIncrementalRefreshFailureAfterIcebergCommit() throws Exception {
    final String sql =
        "refresh dataset dfs.tmp.metadatarefreshfailures.incrementRefreshAddingPartition";
    ...
    copyFromJar(
        ...
        Paths.get(testRootPath + "incrementRefreshAddingPartition/level2"));

    final String errorAfterIcebergCommit =
        Controls.newBuilder()
            .addException(
                IncrementalMetadataRefreshCommitter.class,
                IncrementalMetadataRefreshCommitter.INJECTOR_AFTER_ICEBERG_COMMIT_ERROR,
    ...
    NamespaceService namespaceService =
        getSabotContext().getNamespaceService(SystemUser.SYSTEM_USERNAME);
    NamespaceKey namespaceKey =
        new NamespaceKey(
            Arrays.asList(
                "dfs", "tmp", "metadatarefreshfailures", "incrementRefreshAddingPartition"));
    ...
    assertNotEquals(
        snapshotAfterFailedCatalogUpdate,
        (long) dataset.getPhysicalDataset().getIcebergMetadata().getSnapshotId());
    expectedSchema =
        new Schema(
            Arrays.asList(
                Types.NestedField.optional(1, "dir0", new Types.StringType()),
    ...
    long modTime =
        new File(
                "/tmp/metadatarefreshfailures/incrementRefreshAddingPartition/level2/float.parquet")
            .lastModified();
    Assert.assertEquals(
        RefreshDatasetTestUtils.getAddedFilePaths(icebergTable).get(0),
        "file:/tmp/metadatarefreshfailures/incrementRefreshAddingPartition/level2/float.parquet?version="
            + modTime);
    ...
    testBuilder()
        .ordered()
        .sqlQuery(
            "select count(*) as cnt from dfs.tmp.metadatarefreshfailures.incrementRefreshAddingPartition")
        .baselineColumns("cnt")
        .baselineValues(4L)
        .go();
  }
  ...
  public void testIncrementalRefreshFailureCleanupAfterIcebergCommit() throws Exception {
    ...
    final String errorAfterIcebergCommit =
        Controls.newBuilder()
            .addException(
                WriterCommitterOperator.class,
                WriterCommitterOperator.INJECTOR_AFTER_NO_MORETO_CONSUME_ERROR,
                UnsupportedOperationException.class)
            .build();
    ControlsInjectionUtil.setControls(client, errorAfterIcebergCommit);
    try {
