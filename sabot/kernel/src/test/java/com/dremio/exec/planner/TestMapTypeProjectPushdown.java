...
public class TestMapTypeProjectPushdown extends PlanTestBase {
  ...
  public void testProjectPastJoinPushDown() throws Exception {
    try (AutoCloseable ignored = withOption(ExecConstants.ENABLE_MAP_DATA_TYPE, true);
      AutoCloseable ignored1 = withOption(PlannerSettings.SPLIT_COMPLEX_FILTER_EXPRESSION, true)) {
      String query = String.format("select attributes['channel_name'] , counters['suspend_because'] " +
        "from %s.\"map_type.parquet\" t0 JOIN %s.\"map_float_type.parquet\" t1 " +
        "ON object_name = retailer_id", path1, path1);
      testPlanMatchingPatterns(query,
        new String[]{
          ".*HashJoin.*0.*2" +
  ...
  public void testProjectWithFilterInJoinConditionPushDown() throws Exception {
    try (AutoCloseable ignored = withOption(ExecConstants.ENABLE_MAP_DATA_TYPE, true);
      AutoCloseable ignored1 = withOption(PlannerSettings.SPLIT_COMPLEX_FILTER_EXPRESSION, true)) {
      String query = String.format("select \"date\", cur_time_stamp " +
        "from %s.\"map_type.parquet\" t0 JOIN %s.\"map_float_type.parquet\" t1 " +
        "ON object_name = retailer_id WHERE attributes['channel_name'] = counters['suspend_because']", path1, path1);
      testPlanMatchingPatterns(query,
        new String[]{
          ".*HashJoin.*AND.*inner" +
            "(?s).*Project.*ITEM.*'value'"+
