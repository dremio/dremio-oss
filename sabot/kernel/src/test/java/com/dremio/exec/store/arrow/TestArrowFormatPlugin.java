...
public class TestArrowFormatPlugin extends PlanTestBase {
  ...
  public static void generateTestData() throws Exception {
    test(
        "CREATE TABLE dfs_test.arrowRegion "
            + "STORE AS (type => 'arrow') AS SELECT * FROM cp.\"region.json\"");

    test(
        "CREATE TABLE dfs_test.lineitem "
            + "STORE AS (type => 'arrow') AS SELECT * FROM cp.\"tpch/lineitem.parquet\"");

    test(
        "CREATE TABLE dfs_test.orders "
            + "STORE AS (type => 'arrow') AS SELECT * FROM cp.\"tpch/orders.parquet\"");

    // Currently DROP TABLE doesn't support table with options, so the above tables are not deleted
    // as part of
    // @AfterClass method. They will be deleted in @AfterClass of superclass as the base temporary
  ...
  public void join() throws Exception {
    ...
    final String joinArrow =
        "SELECT * FROM TABLE(dfs_test.lineitem(type => 'arrow')) l JOIN "
            + "TABLE(dfs_test.orders(type => 'arrow')) o ON l.l_orderkey = o.o_orderkey";

    final String joinParquet =
        "SELECT * FROM cp.\"tpch/lineitem.parquet\" l JOIN "
            + "cp.\"tpch/orders.parquet\" o ON l.l_orderkey = o.o_orderkey";

    testBuilder().unOrdered().sqlQuery(joinArrow).sqlBaselineQuery(joinParquet).go();
  }

  @Test
  public void projectPushDown() throws Exception {
    final String query =
        "SELECT o_orderkey, o_orderpriority FROM TABLE(dfs_test.orders(type => 'arrow')) "
  ...
  public void mapDX3266() throws Exception {
    ...
    try (BufferedWriter bw = new BufferedWriter(new FileWriter(tmpFile))) {
      ...
      bw.write(
          "{\"a\": {\"Tuesday\": {\"close\": \"19:00\",\"open\": \"10:00\"},\"Friday\":  {\"close\": \"19:00\",\"open\": \"10:00\"}}}\n");
      bw.write(
          "{\"a\": {\"Monday\": {\"close\": \"02:00\",\"open\": \"10:00\"},\"Tuesday\":  {\"close\": \"19:00\",\"open\": \"10:00\"}}}\n");
      bw.write(
          "{\"a\": {\"Monday\": {\"close\": \"14:30\",\"open\": \"10:00\"},\"Tuesday\":  {\"close\": \"19:00\",\"open\": \"10:00\"}}}\n");
    }
    test(
        "CREATE TABLE dfs_test.test STORE AS (type => 'arrow') AS SELECT * FROM dfs.\""
  ...
  public void complexProjectPushdown() throws Exception {
    ...
    try (BufferedWriter bw = new BufferedWriter(new FileWriter(tmpFile))) {
      bw.write(
          "{\"a\": {\"Tuesday\": {\"close\": 19,\"open\": 10},\"Friday\":  {\"close\": 19,\"open\": 10}}}");
    }
    test(
        "CREATE TABLE dfs_test.complexPpd STORE AS (type => 'arrow') AS SELECT * FROM dfs.\""
            + tmpFile.getAbsolutePath()
            + "\"");
  ...
  public void complexProjectPushdown2() throws Exception {
    ...
    test(
        "CREATE TABLE dfs_test.complexPpd2 STORE AS (type => 'arrow') AS SELECT * FROM dfs.\""
            + tmpFile.getAbsolutePath()
            + "\"");
