...
public class TestUDFParsing {
  ...
  public void testDropFunction() throws Exception {
    SqlDropFunction sqlDropFunctionResult =
        parse(SqlDropFunction.class, "DROP FUNCTION protect_ssn");
    Assert.assertEquals(
        "function name doesn't match", "protect_ssn", sqlDropFunctionResult.getName().toString());
  ...
  public void testDropFunctionIfExists() throws Exception {
    SqlDropFunction sqlDropFunctionResult =
        parse(SqlDropFunction.class, "DROP FUNCTION IF EXISTS protect_ssn");
    Assert.assertEquals(
        "function name doesn't match", "protect_ssn", sqlDropFunctionResult.getName().toString());
  ...
  private void testCreate(
      ...
      String tabularReturnType)
      throws Exception {
    validateCreate(
        parse(SqlCreateFunction.class, command),
        fName,
        exp,
        fieldList,
        returnType,
  ...
  private <SQL_NODE extends SqlNode> SQL_NODE parse(Class<SQL_NODE> nodeType, String command)
      throws Exception {
  ...
  private void validateCreate(
      SqlCreateFunction sqlCreateFunctionResult,
      String fName,
      String exp,
      String fieldList,
      String scalarReturnType,
      boolean replacePolicy,
      String tabularReturnType) {
    Assert.assertEquals(
        "function name doesn't match", fName, sqlCreateFunctionResult.getName().toString());
    Assert.assertEquals(
        "function body doesn't match", exp, sqlCreateFunctionResult.getExpression().toString());
    Assert.assertEquals(
        "function arguments doesn't match",
        fieldList,
        sqlCreateFunctionResult.getFieldList().toString());
    if (scalarReturnType == null) {
      Assert.assertNull(
          "function return type doesn't match",
          sqlCreateFunctionResult.getReturnType().getScalarReturnType());
    } else {
      Assert.assertEquals(
          "function return type doesn't match",
          scalarReturnType,
          sqlCreateFunctionResult.getReturnType().getScalarReturnType().toString());
    }
    Assert.assertEquals(
        "shouldReplace doesn't match", replacePolicy, sqlCreateFunctionResult.shouldReplace());
  }
}
