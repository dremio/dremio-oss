...
public class RuntimeFilterTestUtils {
    ...
    public OutOfBandMessage newOOB(int sendingMajorFragment, int sendingOperator, int sendingMinorFragment, List<String> partitionCols,
        ...
        if (nonPartitionColFilters.length > 0) {
            for (ValueListFilter vlf : nonPartitionColFilters) {
                ExecProtos.CompositeColumnFilter nonPartitionColFilter = ExecProtos.CompositeColumnFilter.newBuilder()
    ...
    public ValueListFilter prepareNewValueListBooleanFilter(String fieldName, boolean insertNull,
                                                            boolean insertFalse, boolean insertTrue) throws Exception {
