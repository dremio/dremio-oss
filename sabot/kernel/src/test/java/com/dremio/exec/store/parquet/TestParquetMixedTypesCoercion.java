...
public class TestParquetMixedTypesCoercion extends BaseTestQuery {
  ...
  public void testCtasOutputCoercion() throws Exception {
    try (AutoCloseable ac = withOption(ExecConstants.ENABLE_PARQUET_MIXED_TYPES_COERCION, true)) {
      ...
      testBuilder()
        .sqlQuery("select A, B from dfs.\"%s/parquet/mixed_types_coercion/ctas_output\"", testResPath)
        .unOrdered()
        .baselineColumns("A", "B")
        .baselineValues(1, "X")
        .baselineValues(2, OBJECT_MAPPER.writeValueAsString(ImmutableMap.of("B", "X")))
        .baselineValues(3, OBJECT_MAPPER.writeValueAsString(ImmutableList.of("hello", "world", "there")))
        .build()
        .run();
    }
  }
  ...
  public void testSingleFileCoercion() throws Exception {
    try (AutoCloseable ac = withOption(ExecConstants.ENABLE_PARQUET_MIXED_TYPES_COERCION, true)) {
      ...
      String val21 = OBJECT_MAPPER.writeValueAsString(ImmutableMap.of("mixed1", "def", "mixed2", "ghi"));
      ...
      Map<String, Text> val53 = ImmutableMap.of("mixed1", new Text(OBJECT_MAPPER.writeValueAsString("def")),
                                                "mixed2", new Text(OBJECT_MAPPER.writeValueAsString("ghi")));
      String val54 = OBJECT_MAPPER.writeValueAsString(true);
      Map<String, Text> val63 = ImmutableMap.of("mixed1", new Text(OBJECT_MAPPER.writeValueAsString(5.0)),
      ...
      final String testResPath = TestTools.getWorkingPath() + "/src/test/resources";
      testBuilder()
        .sqlQuery("select * from dfs.\"%s/parquet/mixed_types_coercion/single_file\"", testResPath)
        .unOrdered()
