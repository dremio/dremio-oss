...
public class TestNestedLoopJoin extends PlanTestBase {
  ...
  private static final String SINGLE_NLJ =
      "alter session set \"planner.disable_exchanges\" = true; " + NLJ;
  ...
  private static final String testNlJoinExists_1 =
  ...
  private static final String testNlJoinNotIn_1 =
      "select r_regionkey from cp.\"tpch/region.parquet\" "
          + " where r_regionkey not in (select n_regionkey from cp.\"tpch/nation.parquet\" "
          + "                            where n_nationkey < 4)";

  // not-in subquery produces empty set
  private static final String testNlJoinNotIn_2 =
      "select r_regionkey from cp.\"tpch/region.parquet\" "
          + " where r_regionkey not in (select n_regionkey from cp.\"tpch/nation.parquet\" "
          + "                            where 1=0)";

  private static final String testNlJoinInequality_1 =
      "select r_regionkey from cp.\"tpch/region.parquet\" "
          + " where r_regionkey > (select min(n_regionkey) from cp.\"tpch/nation.parquet\" "
          + "                        where n_nationkey < 4)";

  private static final String testNlJoinInequality_2 =
      "select r.r_regionkey, n.n_nationkey from cp.\"tpch/nation.parquet\" n "
          + " inner join cp.\"tpch/region.parquet\" r on n.n_regionkey < r.r_regionkey where n.n_nationkey < 3";

  private static final String testNlJoinInequality_3 =
      "select r_regionkey from cp.\"tpch/region.parquet\" "
          + " where r_regionkey > (select min(n_regionkey) * 2 from cp.\"tpch/nation.parquet\" )";

  private static final String testNlJoinProjectedFields_null =
      "select *, ROW_NUMBER() over() from cp.\"tpch/customer.parquet\" "
  ...
  public void testNlJoinEqualityNonScalar_1_planning() throws Exception {
    String query =
        "select r.r_regionkey from cp.\"tpch/region.parquet\" r inner join cp.\"tpch/nation.parquet\" n"
  ...
  public void testNlJoinEqualityNonScalar_2_planning() throws Exception {
    String query =
        String.format(
            "select n.n_nationkey from cp.\"tpch/nation.parquet\" n, "
  ...
  public void testNLJWithEmptyBatch() throws Exception {
    ...
    String query =
        "select count(*) col from (select a.last_name "
            + "from cp.\"employee.json\" a "
            + "where exists (select n_name from cp.\"tpch/nation.parquet\" b) AND 1 = 0)";

    testBuilder().sqlQuery(query).unOrdered().baselineColumns("col").baselineValues(result).go();

    // Below tests use NLJ in a general case (non-scalar subqueries, followed by filter) with empty
    // batches
    query =
  ...
  public void testDx35886() throws Exception {
    test(
        "SELECT * FROM cp.tpch_json.\"orders.json\" o \n"
