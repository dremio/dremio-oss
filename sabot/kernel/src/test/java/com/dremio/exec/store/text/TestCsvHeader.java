...
public class TestCsvHeader extends BaseTestQuery {
  ...
  public void testCsvColumnsWithHeader() throws Exception {
    // Pre DRILL-951: Qry: select columns[0],columns[2],columns[4] from dfs.\"%s\" where columns[1]
    // = 'Chevy'
    String query =
        String.format("select \"Year\", Model, Price from dfs.\"%s\" where Make = 'Chevy'", root);
  ...
  public void testCsvHeaderShortCircuitReads() throws Exception {
    String query =
        String.format("select \"Year\", Model from dfs.\"%s\" where Make = 'Chevy'", root);
  ...
  public void testCsvHeaderNonExistingColumn() throws Exception {
    String query =
        String.format(
            "select \"Year\", Model, Category from dfs.\"%s\" where Make = 'Chevy'", root);
  ...
  public void testCsvHeaderMismatch() throws Exception {
    ...
    String query =
        String.format(
            "select \"Year\", Model, Category from dfs.\"%s\" where Make = 'Chevy'", ddir);
    testBuilder()
        .sqlQuery(query)
        .unOrdered()
        .baselineColumns("Year", "Model", "Category")
  ...
  public void testCsvHeaderSkipFirstLine() throws Exception {
    ...
    String query =
        String.format(
            "select \"Year\", Model from TABLE(dfs_root.\"%s\"(type => 'TEXT', fieldDelimiter => ',', lineDelimiter => '\n', skipFirstLine => true, extractHeader => true)) where Make = 'Chevy'",
            dfile);
    testBuilder()
        .sqlQuery(query)
        .unOrdered()
  ...
  public void testEmptyFinalColumn() throws Exception {
    ...
    BufferedOutputStream os =
        new BufferedOutputStream(new FileOutputStream(new File(table_dir, "a.csvh")));
    ...
    TestBuilder builder =
        testBuilder().sqlQuery(query).ordered().baselineColumns("field1", "field2");
    for (int i = 0; i < 10000; i++) {
  ...
  public void testCsvWithHeaderEmptyAndDuplicateColumnNames() throws Exception {
    String file =
        FileUtils.getResourceAsFile("/store/text/data/cars-badheaders.csvh").toURI().toString();
  ...
  public void testCsvHeaderDifferentCaseHeadings() throws Exception {
    String dfile =
        FileUtils.getResourceAsFile("/store/text/data/testcasesensitivityheaders")
            .getAbsolutePath();
    String query =
        String.format(
            "select * from TABLE(dfs_root.\"%s\"(type => 'TEXT', fieldDelimiter => ',', lineDelimiter => '\n', extractHeader => true))",
            dfile);
    try {
      testBuilder()
          .sqlQuery(query)
