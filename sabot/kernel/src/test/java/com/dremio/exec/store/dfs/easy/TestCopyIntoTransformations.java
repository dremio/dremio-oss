...
public class TestCopyIntoTransformations {
  ...
  private final Field sampleStringField = new Field("sampleStringField", FieldType.nullable(ArrowType.Utf8.INSTANCE), null);
  private final ExtendedFormatOptions extendedFormatOptions = new ExtendedFormatOptions(false, false, null, null, null, null);

  @Test
  public void testEmptyAsNull() {
    final String emptyString = "";
    extendedFormatOptions.setEmptyAsNull(true);
    final String result = (String) EasyFormatUtils.getValue(sampleStringField, emptyString, this.extendedFormatOptions);
  ...
  public void testNullIf() {
    ...
    String result1 = (String) EasyFormatUtils.getValue(sampleStringField, testString1, this.extendedFormatOptions);
    String result2 = (String) EasyFormatUtils.getValue(sampleStringField, testString2, this.extendedFormatOptions);
    String result3 = (String) EasyFormatUtils.getValue(sampleStringField, testString3, this.extendedFormatOptions);
  ...
  public void testDateTransform() {
    ...
    Field sampleDateField = new Field("sampleDateField", FieldType.nullable(CompleteType.DATE.getType()), null);

    // Check default date format handling.
    int dateTest1DaysFromEpoch = (int) EasyFormatUtils.getValue(sampleDateField, dateTest1, this.extendedFormatOptions);
    ...
    int dateTest2DaysFromEpoch = (int) EasyFormatUtils.getValue(sampleDateField, dateTest2, this.extendedFormatOptions);
  ...
  public void testTimeTransform() {
    ...
    Field sampleTimeField = new Field("sampleTimeField", FieldType.nullable(CompleteType.TIME.getType()), null);

    // Check handling for default time format
    Long timeTest1Result = (Long) (EasyFormatUtils.getValue(sampleTimeField, timeTest1, this.extendedFormatOptions));
    // The above method returns 1000 * (time in milliseconds) whereas we want the time only in milliseconds in this case.
    ...
    Long timeTest2Result = (Long) EasyFormatUtils.getValue(sampleTimeField, timeTest2, this.extendedFormatOptions);
  ...
  public void testTimestampTransform() {
    ...
    Field sampleTimestampField = new Field("sampleTimestampField", FieldType.nullable(CompleteType.TIMESTAMP.getType()), null);

    Long timestampTest1Result = (Long) EasyFormatUtils.getValue(sampleTimestampField, timestampTest1, this.extendedFormatOptions);
    ...
    Long timestampTest2Result = (Long) EasyFormatUtils.getValue(sampleTimestampField, timestampTest2, this.extendedFormatOptions);
  ...
  private void checkDatesAreEqual(int daysFromEpoch, String date, String dateFormat) {
    ...
    DateTime dateFromString = DateTime.parse(date, DateFunctionsUtils.getISOFormatterForFormatString(dateFormat));
    assertThat(dateNew).isEqualByComparingTo(dateFromString);
  }

  private void checkResultCorrespondsToTime(Long timeInMilli, String timeToCompareWith, String timeFormat) {
    DateTime timeFromResult = new DateTime(timeInMilli);
    DateTime timeActual = DateTime.parse(timeToCompareWith, DateFunctionsUtils.getISOFormatterForFormatString(timeFormat));
