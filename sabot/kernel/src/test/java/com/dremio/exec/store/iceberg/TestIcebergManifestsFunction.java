...
public class TestIcebergManifestsFunction extends IcebergMetadataTestTable {
  ...
  public void testTableManifestSchema() throws Exception {
    ...
    expectedSchema.add(
        Pair.of(SchemaPath.getSimplePath("content"), Types.required(TypeProtos.MinorType.INT)));
    expectedSchema.add(
        Pair.of(SchemaPath.getSimplePath("path"), Types.required(TypeProtos.MinorType.VARCHAR)));
    expectedSchema.add(
        Pair.of(SchemaPath.getSimplePath("length"), Types.required(TypeProtos.MinorType.BIGINT)));
    expectedSchema.add(
        Pair.of(
            SchemaPath.getSimplePath("partition_spec_id"),
            Types.required(TypeProtos.MinorType.INT)));
    expectedSchema.add(
        Pair.of(
            SchemaPath.getSimplePath("added_snapshot_id"),
            Types.required(TypeProtos.MinorType.BIGINT)));
    expectedSchema.add(
        Pair.of(
            SchemaPath.getSimplePath("added_data_files_count"),
            Types.required(TypeProtos.MinorType.INT)));
    expectedSchema.add(
        Pair.of(
            SchemaPath.getSimplePath("existing_data_files_count"),
            Types.required(TypeProtos.MinorType.INT)));
    expectedSchema.add(
        Pair.of(
            SchemaPath.getSimplePath("deleted_data_files_count"),
            Types.required(TypeProtos.MinorType.INT)));
    expectedSchema.add(
        Pair.of(
            SchemaPath.getSimplePath("added_delete_files_count"),
            Types.required(TypeProtos.MinorType.INT)));
    expectedSchema.add(
        Pair.of(
            SchemaPath.getSimplePath("existing_delete_files_count"),
            Types.required(TypeProtos.MinorType.INT)));
    expectedSchema.add(
        Pair.of(
            SchemaPath.getSimplePath("deleted_delete_files_count"),
            Types.required(TypeProtos.MinorType.INT)));
    expectedSchema.add(
        Pair.of(
            SchemaPath.getSimplePath("partition_summaries"),
            Types.required(TypeProtos.MinorType.LIST)));
    expectedSchema(
        expectedSchema,
        String.format(
            "SELECT * FROM table(table_manifests('\"%s\".\"%s\"')) limit 1",
  ...
  public void testTableManifests() throws Exception {
    ...
    queryAndMatchResults(
        String.format(
            "SELECT path,length,partition_spec_id,added_snapshot_id FROM table(table_manifests('\"%s\".\"%s\"')) limit 1",
            TEMP_SCHEMA_HADOOP, METADATA_TEST_TABLE_NAME),
        expectedColumns,
        expectedValues);
  }
  ...
  public void testTableManifestsCount() throws Exception {
    queryAndMatchResults(
        String.format(
            "SELECT count(*) as manifest_count FROM table(table_manifests('\"%s\".\"%s\"'))",
            TEMP_SCHEMA_HADOOP, METADATA_TEST_TABLE_NAME),
        new String[] {"manifest_count"},
        new Object[] {(long) getManifests().size()});
  }
