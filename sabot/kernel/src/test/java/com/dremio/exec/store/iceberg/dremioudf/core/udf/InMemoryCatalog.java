 ...
 */
public class InMemoryCatalog extends BaseMetastoreUdfCatalog
  ...
  private final ConcurrentMap<TableIdentifier, String> udfs;
  private FileIO io;
  ...
  public InMemoryCatalog() {
    ...
    this.udfs = Maps.newConcurrentMap();
  }
  ...
  public void renameTable(TableIdentifier from, TableIdentifier to) {
    ...
    synchronized (this) {
      ...
      if (udfs.containsKey(to)) {
        throw new AlreadyExistsException("Cannot rename %s to %s. UDF already exists", from, to);
      }
  ...
  @Override
  public List<TableIdentifier> listUdfs(Namespace namespace) {
    if (!namespaceExists(namespace) && !namespace.isEmpty()) {
      throw new NoSuchNamespaceException(
          "Cannot list views for namespace. Namespace does not exist: %s", namespace);
    }

    return udfs.keySet().stream()
        .filter(v -> namespace.isEmpty() || v.namespace().equals(namespace))
        .sorted(Comparator.comparing(TableIdentifier::toString))
        .collect(Collectors.toList());
  }
  ...
  @Override
  protected InMemoryUdfOperations newUdfOps(TableIdentifier identifier) {
    return new InMemoryUdfOperations(io, identifier);
  }

  @Override
  public boolean dropUdf(TableIdentifier identifier) {
    synchronized (this) {
      return null != udfs.remove(identifier);
    }
  }

  @Override
  public void renameUdf(TableIdentifier from, TableIdentifier to) {
    if (from.equals(to)) {
      return;
    }
    ...
    synchronized (this) {
      ...
      String fromViewLocation = udfs.get(from);
      if (null == fromViewLocation) {
        throw new NoSuchUdfException("Cannot rename %s to %s. UDF does not exist", from, to);
      }
  ...
  private class InMemoryTableOperations extends BaseMetastoreTableOperations {
    ...
    public void doCommit(TableMetadata base, TableMetadata metadata) {
      ...
      synchronized (InMemoryCatalog.this) {
        ...
        if (udfs.containsKey(tableIdentifier)) {
          throw new AlreadyExistsException(
              "UDF with same name already exists: %s", tableIdentifier);
        }
  ...
  private class InMemoryViewOperations extends BaseViewOperations {
    ...
    public void doCommit(ViewMetadata base, ViewMetadata metadata) {
      ...
      synchronized (InMemoryCatalog.this) {
        ...
        if (udfs.containsKey(identifier)) {
  ...
  private class InMemoryUdfOperations extends BaseUdfOperations {
    private final FileIO io;
    private final TableIdentifier identifier;
    private final String fullUdfName;

    InMemoryUdfOperations(FileIO io, TableIdentifier identifier) {
      this.io = io;
      this.identifier = identifier;
      this.fullUdfName = UdfUtil.fullUdfName(catalogName, identifier);
    }

    @Override
    public void doRefresh() {
      String latestLocation = udfs.get(identifier);
      if (latestLocation == null) {
        disableRefresh();
      } else {
        refreshFromMetadataLocation(latestLocation);
    ...
    @Override
    public void doCommit(UdfMetadata base, UdfMetadata metadata) {
      String newLocation = writeNewMetadataIfRequired(metadata);
      String oldLocation = base == null ? null : currentMetadataLocation();
      ...
      synchronized (InMemoryCatalog.this) {
        if (null == base && !namespaceExists(identifier.namespace())) {
          throw new NoSuchNamespaceException(
              "Cannot create UDF %s. Namespace does not exist: %s",
              identifier, identifier.namespace());
        }
        ...
        udfs.compute(
            ...
            (k, existingLocation) -> {
              if (!Objects.equal(existingLocation, oldLocation)) {
                ...
                throw new CommitFailedException(
                    "Cannot commit to UDF %s metadata location from %s to %s "
                        + "because it has been concurrently modified to %s",
                    identifier, oldLocation, newLocation, existingLocation);
              }
