...
public class TestParquetScanTableFunction extends BaseTestParquetScanTableFunction {
  ...
  private BatchSchema FIFTY_ROWGROUPS_SCHEMA = BatchSchema.newBuilder()
  ...
  public void testDataFileSplitWithMultipleRowGroupsAndTrimming() throws Exception {
    ...
    RecordSet input = rs(SystemSchemas.SPLIT_GEN_AND_COL_IDS_SCAN_SCHEMA,
        inputRow(path, 30000, 100000));

    // ids in the file are sequential, with 10 values per rowgroup
    List<Record> ids = Stream.iterate(250L, i -> i + 1L)
        .limit(250)
        .map(RecordSet::r)
        .collect(Collectors.toList());
    RecordSet output = rs(FIFTY_ROWGROUPS_SCHEMA.maskAndReorder(outputColumns), ids.toArray(new Record[0]));

    // validate with TOTAL_HEAP_OBJ_SIZE set to 50, which in the case of this file, would try to trim down to 5
    // rowgroups in memory
  ...
  public void testWorkOnOOBRuntimeFilter() {
    try {
      ...
      ArgumentCaptor<RuntimeFilter> addedFilter = ArgumentCaptor.forClass(com.dremio.exec.store.RuntimeFilter.class);
  ...
  public void testWorkOnOOBRuntimeFilterInvalidFilterSize() {
    ...
    try (ArrowBuf oobMessageBuf = getTestAllocator().buffer(128)) {
      ...
      ParquetScanTableFunction scanOp = new ParquetScanTableFunction(mock(FragmentExecutionContext.class), getMockContext(), getProps(), getTableFunctionConfig());
      scanOp.workOnOOB(msg1);
      Arrays.stream(msg1.getBuffers()).forEach(ArrowBuf::close);
  ...
  private void validate(BatchSchema fileSchema, List<SchemaPath> columns, RecordSet input, RecordSet output)
      throws Exception {
  ...
  private OutOfBandMessage newOOBMessage(ExecProtos.RuntimeFilter filter, ArrowBuf oobMessageBuf, int buildMajorFragment, int buildMinorFragment) {
    int probeScanId = 2;
