...
public class TestExcessiveSplits extends BaseTestOperator {
  ...
  private final FunctionSignature equalFn =
      new FunctionSignature("equal", boolType, Lists.newArrayList(strType, strType));
  private final FunctionSignature isnullFn =
      new FunctionSignature("isnull", boolType, Lists.newArrayList(strType));
  private final FunctionSignature castDecimalFn =
      new FunctionSignature("castDECIMAL", decimalType, Lists.newArrayList(longType));

  private final GandivaPushdownSieveHelper gandivaPushdownSieveHelper =
      new GandivaPushdownSieveHelper();

  private static final String EXPR =
      "case when isnull(c_name) then false else isnull(case when "
          + "(isnull"
          + "(c_name) OR "
  ...
  private static final String EXPR_DECIMAL =
      "case when equal( c_name,  '1+') then castDECIMAL(1l,2l, 0l) else "
    ...
    try {
      ...
      OperatorStats stats =
          runSingle(
              ...
              GenerationDefinition.TpchTable.CUSTOMER_LIMITED,
              6,
              10);
  ...
  public void testMultipleExprsWithOneExcessiveSplits() throws Exception {
    try {
      ...
      Project project =
          new Project(
              PROPS, null, Arrays.asList(n(EXPR, "res"), n("isnull" + "(c_nationkey)", "res1")));
      OperatorStats stats =
          runSingle(
              project,
              ProjectOperator.class,
  ...
  /**
   * Force excessive splits and there by materialize the expression again. The materialization
   * should use original expression and not the one with code gen context.
   *
   * @throws Exception
   */
  @Test
      ...
      testContext
          ...
          .setOption(
              OptionValue.createLong(
                  OptionValue.OptionType.SYSTEM, ExecConstants.MAX_SPLITS_PER_EXPR_KEY, 1));
      testContext
          .getOptions()
          .setOption(
