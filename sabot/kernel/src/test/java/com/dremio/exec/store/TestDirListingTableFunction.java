...
public class TestDirListingTableFunction extends BaseTestTableFunction {
  ...
  private OperatorContext getCtx() {
    ...
    when(operatorContext.getFunctionContext().getContextInformation().getQueryStartTime()).thenReturn(TestDirListingRecordReader.START_TIME);
    when(operatorContext.getOptions().getOption(ExecConstants.DIR_LISTING_EXCLUDE_FUTURE_MOD_TIMES)).thenReturn(true);
  ...
  public void testRecursive() throws Exception {
    when(plugin.createDirListRecordReader(any(), any(), any(), anyBoolean(), any(), any())).thenAnswer(
      i -> new DirListingRecordReader(getCtx(), fs, i.getArgument(2), true,
        null, null,true, false));
    Path inputPath = Path.of("/path1/");
    TestDirListingRecordReader.setupFsListIteratorMock((HadoopFileSystem) fs, inputPath);

    Schema icebergSchema1 = new Schema(
      optional(1000, "dir0", org.apache.iceberg.types.Types.StringType.get())
    );
    Schema icebergSchema2 = new Schema(
      optional(1000, "dir0", org.apache.iceberg.types.Types.StringType.get()),
      optional(1001, "dir1", org.apache.iceberg.types.Types.StringType.get())
    );

    IcebergPartitionData partitionData1 = new IcebergPartitionData(PartitionSpec.unpartitioned().partitionType());
    IcebergPartitionData partitionData2 = new IcebergPartitionData(PartitionSpec.builderFor(icebergSchema1)
      .identity("dir0")
      .build().partitionType());
    partitionData2.set(0, "bar");
    IcebergPartitionData partitionData3 = new IcebergPartitionData(PartitionSpec.builderFor(icebergSchema2)
      .identity("dir0")
      .identity("dir1")
      .build().partitionType());
    ...
    Fixtures.Table input = t(
      th(PATH),
      tr(inputPath.toString())
    );
    Fixtures.Table output = t(
      ...
      tr("/path1/foo.parquet?version=1", 20L, 1L, IcebergSerDe.serializeToByteArray(partitionData1)),
      tr("/path1/bar/file1.parquet?version=31", 70L, 31L, IcebergSerDe.serializeToByteArray(partitionData2)),
      tr("/path1/bar/subBar1/file2.parquet?version=32", 1200L, 32L, IcebergSerDe.serializeToByteArray(partitionData3)),
      tr("/path1/bar/subBar1/file3.parquet?version=312", 1200L, 312L, IcebergSerDe.serializeToByteArray(partitionData3)),
      tr("/path1/bar/subBar1/file4.parquet?version=331", 1400L, 331L, IcebergSerDe.serializeToByteArray(partitionData3))
    );
  ...
  public void testHiveRecursive() throws Exception {
    ...
    IcebergPartitionData partitionData = new IcebergPartitionData(PartitionSpec.builderFor(icebergSchema)
      .identity("integerCol")
      .identity("doubleCol")
      .identity("bitField")
    ...
    when(plugin.createDirListRecordReader(any(), any(), any(), anyBoolean(), any(), any())).thenAnswer(
      i -> new DirListingRecordReader(getCtx(), fs, i.getArgument(2), true,
        tableSchema, partitionValues,false, false));
    ...
    Fixtures.Table output = t(
      ...
      tr("/path2/foo.parquet?version=1", 20L, 1L, IcebergSerDe.serializeToByteArray(partitionData)),
      tr("/path2/bar/file1.parquet?version=31", 70L, 31L, IcebergSerDe.serializeToByteArray(partitionData)),
      tr("/path2/bar/subBar1/file2.parquet?version=32", 1200L, 32L, IcebergSerDe.serializeToByteArray(partitionData)),
      tr("/path2/bar/subBar1/file3.parquet?version=312", 1200L, 312L, IcebergSerDe.serializeToByteArray(partitionData)),
      tr("/path2/bar/subBar1/file4.parquet?version=331", 1400L, 331L, IcebergSerDe.serializeToByteArray(partitionData))
    );
    validateSingle(TABLE_FUNCTION_POP, TableFunctionOperator.class, input, output, 10);
  }
  ...
  public void testInferPartitions() throws Exception {
    ...
    TestDirListingRecordReader.setupFsListIteratorMockWithPartitions((HadoopFileSystem) fs, inputPath);
    ...
    Fixtures.Table output = t(
      ...
      tr("/path3/id=1/data=name/file1.parquet?version=1", 20L, 1L, IcebergSerDe.serializeToByteArray(partitionData)),
      tr("/path3/id=1/data=name/file2.parquet?version=2", 40L, 2L, IcebergSerDe.serializeToByteArray(partitionData)),
      tr("/path3/id=1/data=name/file3.parquet?version=3", 70L, 3L, IcebergSerDe.serializeToByteArray(partitionData)),
      tr("/path3/id=2/data=value/file4.parquet?version=4", 1010L, 4L, IcebergSerDe.serializeToByteArray(partitionData2)),
      tr("/path3/id=2/data=value/file5.parquet?version=5", 1200L, 5L, IcebergSerDe.serializeToByteArray(partitionData2)),
      tr("/path3/id=2/data=value/file6.parquet?version=6", 40L, 6L, IcebergSerDe.serializeToByteArray(partitionData2)),
      tr("/path3/id=2/data=1234/file7.parquet?version=7", 70L, 7L, IcebergSerDe.serializeToByteArray(partitionData3))
    );
    validateSingle(TABLE_FUNCTION_POP, TableFunctionOperator.class, input, output, 10);
  }
}
