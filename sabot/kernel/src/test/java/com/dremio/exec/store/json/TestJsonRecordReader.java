...
public class TestJsonRecordReader extends BaseTestQuery {
  ...
  public void testComplexJsonInput() throws Exception {
    ...
    test(
        "select \"integer\", x['y'] as x1, x['y'] as x2, z[0], z[0]['orange'], z[1]['pink']  from cp.\"jsoninput/input2.json\" limit 10 ");
  ...
  public void testExceptionHandling() throws Exception {
    try {
      ...
      assertEquals(
          "Expected Unsupported Operation Exception.",
          true,
          s.contains("Dremio does not support lists of different types."));
    }
  }
  ...
  public void testJsonWithNulls2() throws Exception {
    final String query =
        "select SUM(1) as \"sum_Number_of_Records_ok\" from cp.\"/jsoninput/twitter_43.json\" having (COUNT(1) > 0)";
    testBuilder()
        .sqlQuery(query)
        .ordered()
  ...
  public void drill_3353() throws Exception {
    try {
      //      testNoResult("alter session set \"store.json.all_text_mode\" = true");
      test(
          "create table dfs_test.tmp.drill_3353 as select a from dfs.\"${WORKING_PATH}/src/test/resources/jsoninput/drill_3353\" where e = true");
      String query = "select t.a.d cnt from dfs_test.tmp.drill_3353 t where t.a.d is not null";
      test(query);
      testBuilder().sqlQuery(query).unOrdered().baselineColumns("cnt").baselineValues(1L).go();
    } finally {
      testNoResult("alter session set \"store.json.all_text_mode\" = false");
    }
  }
  ...
  public void testNestedFilter() throws Exception {
    ...
    testBuilder().sqlQuery(query).unOrdered().sqlBaselineQuery(baselineQuery).go();
  }
  ...
  public void testRefreshOnJsonFileNotFound() throws Exception {
    ...
    try {
      ...
      testBuilder()
          .sqlQuery("select count(*) c from dfs.tmp.json_file_refresh where type = \'donut\'")
          .unOrdered()
          .baselineColumns("c")
          .baselineValues(2L)
          .build()
