...
public class TestIcebergManifests extends BaseTestQuery {
  ...
  public void testManifestCount() throws Exception {
    ...
    int manifestCount =
        getManifestFileCount(
            partitionSpec, partitionValueSize, dataFilesCount, columnName, insertCount);
    Assert.assertTrue(manifestCount < insertCount);
  }

  List<DataFile> getDataFiles(
      PartitionSpec partitionSpec, int partitionValueSize, int dataFilesCount, String columnName) {
  ...
  public int getManifestFileCount(
    ...
    try {
      ...
      table.updateProperties().set(TableProperties.MANIFEST_TARGET_SIZE_BYTES, "20480").commit();
  ...
  ManifestFile getManifestFile(
    ...
    return writeManifest(
        name,
        getDataFiles(partitionSpec, partitionValueSize, dataFilesCount, columnName),
        tableFolder);
  }
