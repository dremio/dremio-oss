...
public class TestVersionedTableExpressionResolver {
  ...
  public static void setup() {
    ...
    final OptionResolver optionResolver = OptionResolverSpecBuilder.build(new OptionResolverSpec()
      .addOption(ExecConstants.ENABLE_ICEBERG_TIME_TRAVEL, true));
    ...
    sqlToRelConverter = new SqlToRelConverter(viewExpander, validator, catalogReader,
      cluster, convertletTable, SqlToRelConverter.Config.DEFAULT);
    resolver = new VersionedTableExpressionResolver(validator, rexBuilder);
  }

  @Test
  public void testResolve() {
    final SqlCall expr = createTimestampAddCall(TimeUnit.DAY, "10",
      createTimestampLiteral(2022, Calendar.JANUARY, 1, 1, 1, 1));
  ...
  public void testMultipleResolvesInNodeList() {
    final SqlCall expr1 = createTimestampAddCall(TimeUnit.DAY, "10",
      createTimestampLiteral(2022, Calendar.JANUARY, 1, 1, 1, 1));
    ...
    final SqlCall expr2 = createTimestampAddCall(TimeUnit.MONTH, "2",
      createTimestampLiteral(2022, Calendar.JANUARY, 1, 0, 0, 0));
    ...
    SqlNodeList nodeList = createNodeList(
  ...
  private SqlCall createTimestampAddCall(TimeUnit timeUnit, String value, SqlTimestampLiteral timestamp) {
