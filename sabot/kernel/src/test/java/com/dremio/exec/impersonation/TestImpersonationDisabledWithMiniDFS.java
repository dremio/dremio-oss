 ...
 * access to a DFS instead of the local filesystem implementation used by default in the rest of the
 * tests. Running this mini cluster is slow and it is best for these tests to only cover necessary
 * cases.
 */
public class TestImpersonationDisabledWithMiniDFS extends BaseTestImpersonation {
  ...
  private static void createTestData() throws Exception {
    ...
    test(
        String.format(
            "CREATE TABLE %s.dfsRegion AS SELECT * FROM cp.\"region.json\"",
            MINIDFS_STORAGE_PLUGIN_NAME));

    test("alter session set \"store.format\"='csv'");
    test(
        String.format(
            "CREATE TABLE %s.dfsRegionCsv AS SELECT * FROM cp.\"region.json\"",
            MINIDFS_STORAGE_PLUGIN_NAME));
  ...
  /**
   * When working on merging the Drill fork of parquet a bug was found that only manifested when run
   * on a cluster. It appears that the local implementation of the Hadoop FileSystem API never fails
   * to provide all of the bytes that are requested in a single read. The API is designed to allow
   * for a subset of the requested bytes be returned, and a client can decide if they want to do
   * processing on teh subset that are available now before requesting the rest.
   *
   * <p>For parquet's block compression of page data, we need all of the bytes. This test is here as
   * a sanitycheck to make sure we don't accidentally introduce an issue where a subset of the bytes
   * are read and would otherwise require testing on a cluster for the full contract of the read
   * method we are using to be exercised.
   */
  @Test
  ...
  public void testSimpleQuery() throws Exception {
    final String query =
        "SELECT sales_city, sales_country FROM dfsRegion ORDER BY region_id DESC LIMIT 2";
  ...
  public void testWithTableOptions() throws Exception {
    test(
        String.format(
            "select * from TABLE(%s.dfsRegionCsv(type => 'TeXT', fieldDelimiter => ',')) ",
            MINIDFS_STORAGE_PLUGIN_NAME));
  }
}
