...
public class TestCTASWithOptions extends PlanTestBase {
  ...
  public void csvWithCustomExtension() throws Exception {
    try {
      test(
          "CREATE TABLE dfs_test.csvWithCustomExtension "
              + "STORE AS (type => 'text', fieldDelimiter => ',', outputExtension => 'myparquet') "
              + "AS SELECT region_id, sales_city FROM cp.\"region.json\" ORDER BY region_id LIMIT 2");

      testBuilder()
          .sqlQuery(
              "SELECT * FROM TABLE(\"dfs_test\".\"csvWithCustomExtension\""
                  + "(type => 'text', fieldDelimiter => ',', extractHeader => true))")
          .unOrdered()
          .baselineColumns("region_id", "sales_city")
          .baselineValues("0", "None")
  ...
  public void csvUnordered() throws Exception {
    try {
      ...
      test(
          "CREATE TABLE dfs_test.testCsvUnordered "
              + "STORE AS (fieldDelimiter => ',', type => 'text') "
              + "AS SELECT region_id, sales_city FROM cp.\"region.json\" ORDER BY region_id LIMIT 2");

      testBuilder()
          .sqlQuery(
              "SELECT * FROM TABLE(\"dfs_test\".\"testCsvUnordered\""
                  + "(type => 'text', fieldDelimiter => ',', extractHeader => true))")
          .unOrdered()
          .baselineColumns("region_id", "sales_city")
          .baselineValues("0", "None")
  ...
  public void csvTabRecordDelimiter() throws Exception {
    try {
      test(
          "CREATE TABLE dfs_test.testCsvTabRecordDelimiter "
  ...
  public void tsv() throws Exception {
    try {
      test(
          "CREATE TABLE dfs_test.testTsv STORE AS (type => 'teXt', fieldDelimiter => '\t') "
              + "AS SELECT region_id, sales_city FROM cp.\"region.json\" ORDER BY region_id LIMIT 2");
  ...
  public void json() throws Exception {
    try {
      test(
          "CREATE TABLE dfs_test.testJson "
              + "STORE AS (type => 'json') "
              + "AS SELECT region_id, sales_city FROM cp.\"region.json\" ORDER BY region_id LIMIT 2");

      testBuilder()
          .sqlQuery("SELECT * FROM " + "TABLE(\"dfs_test\".\"testJson\"(type => 'json'))")
          .unOrdered()
          .baselineColumns("region_id", "sales_city")
  ...
  public void jsonWithCustomExtension() throws Exception {
    try {
      test(
          "CREATE TABLE dfs_test.jsonWithCustomExtension "
              + "STORE AS (type => 'json', outputExtension => 'myjson') "
              + "AS SELECT region_id, sales_city FROM cp.\"region.json\" ORDER BY region_id LIMIT 2");

      testBuilder()
          .sqlQuery(
              "SELECT * FROM " + "TABLE(\"dfs_test\".\"jsonWithCustomExtension\"(type => 'json'))")
          .unOrdered()
          .baselineColumns("region_id", "sales_city")
          .baselineValues(0L, "None")
          .baselineValues(1L, "San Francisco")
  ...
  public void parquet() throws Exception {
    try {
      test(
          "CREATE TABLE dfs_test.testParquet "
              + "STORE AS (type => 'parquet') "
              + "AS SELECT region_id, sales_city FROM cp.\"region.json\" ORDER BY region_id LIMIT 2");

      testBuilder()
          .sqlQuery("SELECT * FROM " + "TABLE(\"dfs_test\".\"testParquet\"(type => 'parquet'))")
          .unOrdered()
          .baselineColumns("region_id", "sales_city")
          .baselineValues(0L, "None")
          .baselineValues(1L, "San Francisco")
  ...
  public void parquetWithCustomExtension() throws Exception {
    try {
      test(
          "CREATE TABLE dfs_test.parquetWithCustomExtension "
  ...
  public void parquetWithPartition() throws Exception {
    try {
      test(
          "CREATE TABLE dfs_test.testParquetWithPartition "
      ...
      testBuilder()
          .sqlQuery(
              "SELECT dir0, region_id, sales_city FROM TABLE(\"dfs_test\".\"testParquetWithPartition\"(type => 'parquet'))")
          .unOrdered()
  ...
  public void negativeCaseUnsupportedType() throws Exception {
    final String query =
        "CREATE TABLE dfs_test.negativeCaseUnsupportedType "
  ...
  public void negativeCaseUnknownOption() throws Exception {
    final String query =
        "CREATE TABLE dfs_test.negativeCaseUnknownOptions "
  ...
  public void csvWithSingleWriter() throws Exception {
    try {
      final String query =
          "CREATE TABLE dfs_test.csvWithSingleWriter "
              + "STORE AS (type => 'text', fieldDelimiter => ',') "
              + "WITH SINGLE WRITER "
      ...
      testBuilder()
          .sqlQuery(
              "SELECT * FROM TABLE(\"dfs_test\".\"csvWithSingleWriter\""
                  + "(type => 'text', fieldDelimiter => ',', extractHeader => true))")
          .unOrdered()
