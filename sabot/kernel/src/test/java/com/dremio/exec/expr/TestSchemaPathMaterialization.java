...
public class TestSchemaPathMaterialization extends BaseTestQuery {
  ...
  public void testSingleProjectionFromMultiLevelRepeatedList() throws Exception {
    final String query =
        "select t.odd[2][0][0] v1 " + " from cp.\"complex/json/repeated_list.json\" t";

    testBuilder().sqlQuery(query).ordered().baselineColumns("v1").baselineValues(5L).go();
  }
  ...
  public void testProjectionFromMultiLevelRepeatedList() throws Exception {
    final String query =
        "select t.odd[0][1][0] v1, t.odd[0][1][0] v2, t.odd[0][2][0] v3, "
  ...
  public void testProjectionMultipleFiles() throws Exception {
    final String query =
        "select t.oooa.oa.oab.oabc[1].rowValue1 rowValue from dfs.\"${WORKING_PATH}/src/test/resources/complex/json/multiple\" t";
