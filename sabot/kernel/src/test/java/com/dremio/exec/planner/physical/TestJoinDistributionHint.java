...
public class TestJoinDistributionHint extends PlanTestBase {
  ...
  /**
   * Baseline test to confirm that the query with no hints behaves in the way we expect.
   * If this fails then subsequent test cases that use hints may not be altering the plan in the way they should.
   * @throws Exception
   */
  @Test
  public void testDefaultBroadcastExchange() throws Exception {
    // When only selecting 1 column from each table, we will default to a broadcast exchange for the join.
    final String query = "SELECT o_orderkey FROM cp.\"tpch/orders.parquet\" o \n" +
  ...
  /**
   * Use BROADCAST hint to force a broadcast exchange on the right side of a join instead of the default hash exchange.
   * @throws Exception
   */
  @Test
  ...
  /**
   * Use BROADCAST hint to force a broadcast exchange on the left side of a join instead of the default hash exchange.
   *
   * The resulting plan will have changed the order of the join to accommodate the hint.
   * @throws Exception
   */
  @Test
  public void testBroadcastHintOnLeft() throws Exception {
    final String query = "SELECT * FROM cp.\"tpch/orders.parquet\" /*+ BROADCAST */ o \n" +
                         "inner JOIN (select * from cp.\"tpch/lineitem.parquet\" limit 10000) l \n" +
                         "ON o.o_orderkey = l.l_orderkey\n";
    final String[] expectedPatterns = new String[]{
  ...
  /**
   * Use BROADCAST hint to force broadcast exchange. Since we are applying it to both sides of the join, the order will
   * depend on our costing (in this case, preserving the order in the query).
   * @throws Exception
   */
  @Test
  ...
  /**
   * Use BROADCAST hint to force the broadcast exchange to be on the opposite side of the join than what the optimizer
   * would have picked by default.
   * @throws Exception
   */
  @Test
  public void testAlterBroadcastedSide() throws Exception {
    ...
    final String query = "SELECT o_orderkey FROM cp.\"tpch/orders.parquet\" /*+ BROADCAST */ o \n" +
                         "inner JOIN (select l_orderkey from cp.\"tpch/lineitem.parquet\" limit 10000) l \n" +
                         "ON o.o_orderkey = l.l_orderkey\n";
    final String[] expectedPatterns = new String[]{
      "HashJoin[\\S ]+RecordType\\(INTEGER l_orderkey[\\S ]+INTEGER o_orderkey",
