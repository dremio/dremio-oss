...
public class TestOrderedPartitionExchange extends PopUnitTestBase {
  ...
  /**
   * Starts two nodes and runs a physical plan with a Mock scan, project, OrderedParititionExchange,
   * Union Exchange, and sort. The final sort is done first on the partition column, and verifies
   * that the partitions are correct, in that all rows in partition 0 should come in the sort order
   * before any row in partition 1, etc. Also verifies that the standard deviation of the size of
   * the partitions is less than one tenth the mean size of the partitions, because we expect all
          ...
          BigIntVector vv1 =
              loader
                  .getValueAccessorById(
                      BigIntVector.class,
                      loader.getValueVectorId(new SchemaPath("col1")).getFieldIds())
                  .getValueVector();
          Float8Vector vv2 =
              loader
                  .getValueAccessorById(
                      Float8Vector.class,
                      loader.getValueVectorId(new SchemaPath("col2")).getFieldIds())
                  .getValueVector();
          IntVector pVector =
              loader
                  .getValueAccessorById(
                      IntVector.class,
                      loader.getValueVectorId(new SchemaPath("partition")).getFieldIds())
                  .getValueVector();
