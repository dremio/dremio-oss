...
public class TestParquetPartitionPruning extends PlanTestBase {
  ...
  public void testPartitionPruningOnTimestamp() throws Exception {
    String sql =
        "select ts from cp.\"parquet/singlets.parquet\" where ts = TIMESTAMP '2008-10-05 05:13:14.000'";
    testBuilder()
        .sqlQuery(sql)
        .unOrdered()
        .baselineColumns("ts")
        .baselineValues(
            JodaDateUtility.formatTimeStampMilli.parseLocalDateTime("2008-10-05 05:13:14.000"))
        .go();
  }
  ...
  public void pruningBasedOnCurrentTimeStamp() throws Exception {
    final String query =
        "SELECT ts FROM cp.\"parquet/singlets.parquet\" WHERE ts > CURRENT_TIMESTAMP";

    testPlanMatchingPatterns(query, new String[] {"Empty"}, "Filter");

    testBuilder().sqlQuery(query).unOrdered().baselineColumns("ts").expectsEmptyResultSet().go();
  }

  @Test // DX-9034
  public void pruningEverythingAcrossUnion() throws Exception {
    String sql =
        "select ts from cp.\"parquet/singlets.parquet\" where ts = TIMESTAMP '1908-10-05 05:13:14.000'"
    ...
    testBuilder().sqlQuery(sql).unOrdered().baselineColumns("ts").expectsEmptyResultSet().go();
  }
  ...
  public void pruningNullPartitionValues() throws Exception {
    ...
    test(
        "CREATE TABLE dfs_test.pruningNullParts HASH PARTITION BY (b) AS "
    ...
    testPlanMatchingPatterns(
        q1, new String[] {"columns=\\[`a`, `b`\\]", "splits=\\[1\\]"}, "Filter");
    testBuilder()
        .sqlQuery(q1)
        .unOrdered()
        .baselineColumns("a", "b")
    ...
    testPlanMatchingPatterns(
        q3, new String[] {"columns=\\[`a`, `b`\\]", "splits=\\[2\\]"}, "Filter");
    testBuilder()
        .sqlQuery(q3)
        .unOrdered()
        .baselineColumns("a", "b")
  ...
  public void pruningDecimalPartitionValues() throws Exception {
    ...
    test(
        "CREATE TABLE dfs_test.decimalPartitions HASH PARTITION BY (b) AS "
    ...
    testBuilder()
        ...
        .baselineColumns("a", "b")
        .baselineValues(
            new BigDecimal("0.023423000000000"), new BigDecimal("34523.456233234000000"))
        .baselineValues(
            new BigDecimal("7564.234230000000000"), new BigDecimal("34523.456233234000000"))
        .go();
    ...
    testBuilder()
        ...
        .baselineValues(
            new BigDecimal("7564.234230000000000"), new BigDecimal("34523.456233234000000"))
        .baselineValues(
            new BigDecimal("9823.634000000000000"), new BigDecimal("7564.234230000000000"))
        .go();
  }
  ...
  public void testPartitionPruningOnDate() throws Exception {
    String q1 =
        "select date_col from dfs.\"" + DATE_PARTITION_TABLE + "\" where date_col = '1996-02-28'";
    ...
    String q2 =
        "select date_col from dfs.\"" + DATE_PARTITION_TABLE + "\" where date_col > '1996-01-29'";
    ...
    String q3 =
        "select date_col from dfs.\"" + DATE_PARTITION_TABLE + "\" where date_col < '1996-03-01'";
