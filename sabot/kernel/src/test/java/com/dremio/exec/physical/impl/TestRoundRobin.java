...
public class TestRoundRobin extends PlanTestBase {
  ...
  private final String useRoundRobinSort =
      "alter session set \"planner.enable_sort_round_robin\" = true";
  private final String useHashSort =
      "alter session set \"planner.enable_sort_round_robin\" = false";

  private final String query =
      "select state, review_count, business_id from dfs.\"${WORKING_PATH}/src/test/resources/yelp_business.json\" order by state, review_count, business_id limit 1";
  ...
  public void done() throws Exception {
    testNoResult(
        "alter session set \"planner.slice_target\" = " + ExecConstants.SLICE_TARGET_DEFAULT);
  }
  ...
  public void testSmallExampleResult() throws Exception {
    testBuilder()
        .sqlQuery(query)
        .unOrdered()
        .baselineColumns("state", "review_count", "business_id")
  ...
  public void testRoundRobinBiggerData() throws Exception {
    ...
    final String queryPmulti =
        "SELECT * FROM dfs_test.\""
            + pmulti
            + "\" ORDER BY o_orderkey, o_custkey desc, o_orderdate";
