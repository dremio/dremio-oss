...
public class TestNewDateFunctions extends BaseTestQuery {
  ...
  private final String dateValues =
      "(values('1900-01-01'), ('3500-01-01'), ('2000-12-31'), ('2005-12-32'), ('2015-02-29'), (cast(null as varchar))) as t(date1)";
  ...
  public void testDateToChar() throws Exception {
    final String dateValues =
        "(values(date '1900-01-01'), (date '3500-01-01'), (date '2000-12-31'), (date '2005-12-30'), (date '2015-02-28'), (cast(null as date))) as t(date1)";
    testBuilder()
    ...
    try {
      ...
      assertTrue(
          "Failed to match exception message",
          ex.getMessage().contains("Invalid date format string 'invalid format'"));
    }
  }
  ...
  public void testIsDate2() throws Exception {
    ...
    testBuilder()
        .sqlQuery(
            "select case when isdate(date1) then cast(date1 as date) else null end res1 from "
                + dateValues)
        .unOrdered()
        .baselineColumns("res1")
  ...
  public void testUnixTimeStampForDateWithPattern() throws Exception {
    DateTimeFormatter formatter =
        DateFunctionsUtils.getISOFormatterForFormatString("YYYY-MM-DD HH:MI:SS.FFF");
    date = formatter.parseLocalDateTime("2009-03-20 11:30:01.0");
    unixTimeStamp = com.dremio.common.util.DateTimes.toMillis(date) / 1000;

    testBuilder()
        .sqlQuery(
            "select unix_timestamp('2009-03-20 11:30:01.0', 'YYYY-MM-DD HH:MI:SS.FFF') from cp.\"employee.json\" limit 1")
        .ordered()
        .baselineColumns("EXPR$0")
        .baselineValues(unixTimeStamp)
        .build()
    ...
    testBuilder()
        .sqlQuery(
            "select unix_timestamp('2009-03-20', 'YYYY-MM-DD') from cp.\"employee.json\" limit 1")
        .ordered()
        .baselineColumns("EXPR$0")
        .baselineValues(unixTimeStamp)
        .build()
  ...
  public void testCurrentDate() throws Exception {
    testBuilder()
        .sqlQuery(
            "select (extract(hour from current_date) = 0) as col from cp.\"employee.json\" limit 1")
        .unOrdered()
        .baselineColumns("col")
        .baselineValues(true)
        .go();
  ...
  public void testLocalTimestamp() throws Exception {
    testBuilder()
        .sqlQuery(
            "select extract(day from localtimestamp) = extract(day from current_date) as col from cp.\"employee.json\" limit 1")
        .unOrdered()
        .baselineColumns("col")
        .baselineValues(true)
        .go();
  ...
  public void testBigIntToDateTimeTimeStamp() throws Exception {
    testBuilder()
        ...
        .unOrdered()
        .baselineColumns(
            "c11", "c12", "c13", "c21", "c22", "c23", "c24", "c25", "c26", "c31", "c32", "c33")
        .baselineValues(
            newDateTime(0), // c11
  ...
  public void testQuarter() throws Exception {
    testBuilder()
        .sqlQuery(
            "select \"quarter\"(d) res1 from (VALUES({d '2017-01-02'}),({d '2000-05-07'}),(CAST(NULL as DATE))) tbl(d)")
        .ordered()
        .baselineColumns("res1")
        .baselineValues(1L)
        .baselineValues(2L)
  ...
  public void testYear() throws Exception {
    testBuilder()
        .sqlQuery(
            "select \"year\"(d) res1 from (VALUES({d '2017-01-02'}),({d '2000-05-07'}),(CAST(NULL as DATE))) tbl(d)")
        .ordered()
        .baselineColumns("res1")
  ...
  public void testCastTimestampToDate() throws Exception {
    String[] tzs = {
      "America/Los_Angeles", "UTC", "Europe/Paris", "Pacific/Auckland", "Australia/Melbourne"
    };
    for (String tz : tzs) {
      DateTimeZone.setDefault(DateTimeZone.forID(tz));
      testBuilder()
          .sqlQuery(
              "SELECT CAST(\"datetime0\" AS DATE) res1 FROM (VALUES({ts '2016-01-02 00:46:36'})) tbl(datetime0)")
          .ordered()
          .baselineColumns("res1")
  ...
  public void testDateMinusDate() throws Exception {
    testBuilder()
        .sqlQuery(
            "SELECT (CAST(\"datetime0\" AS DATE) - CAST({d '2004-01-01'} AS DATE)) res1 FROM (VALUES({ts '2017-01-13 01:02:03'})) tbl(datetime0)")
        .ordered()
        .baselineColumns("res1")
  ...
  public void testDateMinusDateAsInteger() throws Exception {
    testBuilder()
        .sqlQuery(
            "SELECT CAST((CAST(\"datetime0\" AS DATE) - CAST({d '2004-01-01'} AS DATE)) AS INTEGER) res1 FROM (VALUES({ts '2017-01-13 01:02:03'})) tbl(datetime0)")
        .ordered()
        .baselineColumns("res1")
        .baselineValues(4761)
        .go();
  ...
  public void testDateMinusDateAsBigint() throws Exception {
    testBuilder()
        .sqlQuery(
            "SELECT CAST((CAST(\"datetime0\" AS DATE) - CAST({d '2004-01-01'} AS DATE)) AS BIGINT) res1 FROM (VALUES({ts '2017-01-13 01:02:03'})) tbl(datetime0)")
        .ordered()
        .baselineColumns("res1")
  ...
  public void testDateMinusInterval() throws Exception {
    testBuilder()
        .sqlQuery(
            "SELECT ({d '2017-01-13'} - CAST(CAST(a AS VARCHAR) AS INTERVAL DAY)) res1 FROM (VALUES('P1D'),('P2D')) t(a)")
        .ordered()
        .baselineColumns("res1")
  ...
  public void testToDateTypeWithReplaceOptions() throws Exception {
    ...
    try {
      ...
      assertTrue(
          "Failed to match exception message",
          ex.getMessage().contains("Input text cannot be formatted to time"));
    }
  }
  ...
  private static LocalDateTime newDateTime(long instant) {
    // simulating the behavior of {Time/Date/TimeStamp}Vector.getAccessor().getObject() behavior
    // which constructs
