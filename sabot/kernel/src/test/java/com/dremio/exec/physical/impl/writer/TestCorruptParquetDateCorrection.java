...
/**
 * Tests for compatibility reading old parquet files after date corruption issue was fixed in
 * DRILL-4203.
 *
 * <p>Drill was writing non-standard dates into parquet files for all releases before 1.9.0. The
 * values have been read by Drill correctly by Drill, but external tools like Spark reading the
 * files will see corrupted values for all dates that have been written by Drill.
 *
 * <p>This change corrects the behavior of the Drill parquet writer to correctly store dates in the
 * format given in the parquet specification.
 *
 * <p>To maintain compatibility with old files, the parquet reader code has been updated to check
 * for the old format and automatically shift the corrupted values into corrected ones
 * automatically.
 *
 * <p>The test cases included here should ensure that all files produced by historical versions of
 * Drill will continue to return the same values they had in previous releases. For compatibility
 * with external tools, any old files with corrupted dates can be re-written using the CREATE TABLE
 * AS command (as the writer will now only produce the specification-compliant values, even if after
 * reading out of older corrupt files).
 *
 * <p>While the old behavior was a consistent shift into an unlikely range to be used in a modern
 * database (over 10,000 years in the future), these are still valid date values. In the case where
 * these may have been written into files intentionally, and we cannot be certain from the metadata
 * if Drill produced the files, an option is included to turn off the auto-correction. Use of this
 * option is assumed to be extremely unlikely, but it is included for completeness.
 */
public class TestCorruptParquetDateCorrection extends PlanTestBase {
  ...
  public static void initFs() throws Exception {
    ...
    // Move files into temp directory, rewrite the metadata cache file to contain the appropriate
    // absolute
    // path
    copyDirectoryIntoTempSpace(CORRUPTED_PARTITIONED_DATES_1_2_PATH);
    copyDirectoryIntoTempSpace(
        CORRUPTED_PARTITIONED_DATES_1_2_PATH, MIXED_CORRUPTED_AND_CORRECTED_PARTITIONED_FOLDER);
    copyDirectoryIntoTempSpace(
        CORRECTED_PARTITIONED_DATES_1_9_PATH, MIXED_CORRUPTED_AND_CORRECTED_PARTITIONED_FOLDER);
    copyDirectoryIntoTempSpace(
        CORRUPTED_PARTITIONED_DATES_1_4_0_PATH, MIXED_CORRUPTED_AND_CORRECTED_PARTITIONED_FOLDER);
  }

  /**
   * Test reading a directory full of partitioned parquet files with dates, these files have a drill
   * version number of 1.9.0-SNAPSHOT and is.date.correct = true label in their footers, so we can
   * be certain they do not have corruption. The option to disable the correction is passed, but it
   * will not change the result in the case where we are certain correction is NOT needed. For more
   * info see DRILL-4203.
   */
  @Test
  public void testReadPartitionedOnCorrectedDates() throws Exception {
    try {
      ...
      for (String testPath :
          Arrays.asList(
              CORRECTED_PARTITIONED_DATES_1_9_PATH, CORRECTED_PARTITIONED_DATES_1_10_PATH)) {
  ...
  public void testDatePartitionedReadWithCorruption() throws Exception {
    try (AutoCloseable c2 =
      ...
      String sql =
          "select date_col from dfs.\""
              + DATE_PARTITIONED
              + "\" where date_col > TO_DATE('1999-04-08')";
      testPlanMatchingPatterns(
          sql,
          new String[] {"SPLIT_GEN_MANIFEST_SCAN.*rowcount = 21.0"},
          null); // scans all 21 underlying file
    }
  }
  ...
  /**
   * Test reading a directory full of partitioned parquet files with dates, these files have a drill
   * version number of 1.4.0 in their footers, so we can be certain they are corrupt. The option to
   * disable the correction is passed, but it will not change the result in the case where we are
   * certain correction is needed. For more info see DRILL-4203.
   */
  @Test
  ...
  /**
   * To fix some of the corrupted dates fixed as part of DRILL-4203 it requires actually looking at
   * the values stored in the file. A column with date values actually stored must be located to
   * check a value. Just because we find one column where the all values are null does not mean we
   * can safely avoid reading date columns with auto-correction, although null values do not need
   * fixing, other columns may contain actual corrupt date values.
   *
   * <p>This test checks the case where the first columns in the file are all null filled and a
   * later column must be found to identify that the file is corrupt.
   */
  @Test
  public void testReadCorruptDatesWithNullFilledColumns() throws Exception {
    try {
      ...
      testBuilder()
          .sqlQuery(
              "select null_dates_1, null_dates_2, date_col from dfs.\""
  ...
  public void testReadZeroRowGroupEmptyParquetFile() throws Exception {
    try {
      test(String.format("alter session set %s = true", ExecConstants.PARQUET_AUTO_CORRECT_DATES));
      testBuilder()
          .sqlQuery("select * from dfs.\"" + PARQUET_FILE_WITH_ZERO_ROWGROUPS + "\"")
          .unOrdered()
   ...
   * due to DRILL-4203.
   *
   * <p>Tests reading the files with both the vectorized and complex parquet readers.
   *
   * @throws Exception
   */
  @Test
  ...
  /**
   * Input file has a column with dates beyond 5000th year. By default, date correction is disabled.
   * So, those dates should be in output without any conversion
   *
   * @throws Exception
   */
  @Test
  ...
  private static void copyDirectoryIntoTempSpace(String resourcesDir, String destinationSubDir)
      throws IOException {
