...
public class TestAccelParser {
  ...
  public void addAggReflection() throws SqlParseException {
    parse(
        "ALTER TABLE a.b.c CREATE AGGREGATE REFLECTION reflection USING DIMENSIONS (x by day,y) MEASURES (b,c) DISTRIBUTE BY (r,z) PARTITION BY (s,l) LOCALSORT BY (n,x)");
  }

  @Test
  public void addAggReflectionMeasures() throws SqlParseException {
    parse(
        "ALTER TABLE a.b.c CREATE AGGREGATE REFLECTION reflection USING DIMENSIONS (x by day,y) MEASURES (b (COUNT, SUM),c (COUNT, MIN, MAX)) DISTRIBUTE BY (r,z) PARTITION BY (s,l) LOCALSORT BY (n,x)");
  }

  @Test
  public void addAggReflectionEmptyDimensions() throws SqlParseException {
    parse(
        "ALTER TABLE a.b.c CREATE AGGREGATE REFLECTION reflection USING DIMENSIONS () MEASURES (b (COUNT, SUM),c (COUNT, MIN, MAX)) DISTRIBUTE BY (r,z) PARTITION BY (s,l) LOCALSORT BY (n,x)");
    // ignore DIMENSIONS
    parse(
        "ALTER TABLE a.b.c CREATE AGGREGATE REFLECTION reflection USING MEASURES (b (COUNT, SUM),c (COUNT, MIN, MAX)) DISTRIBUTE BY (r,z) PARTITION BY (s,l) LOCALSORT BY (n,x)");
  }

  @Test
  public void addAggReflectionEmptyMeasures() throws SqlParseException {
    parse(
        "ALTER TABLE a.b.c CREATE AGGREGATE REFLECTION reflection USING DIMENSIONS (x by day,y) MEASURES () DISTRIBUTE BY (r,z) PARTITION BY (s,l) LOCALSORT BY (n,x)");
    // ignore MEASURES
    parse(
        "ALTER TABLE a.b.c CREATE AGGREGATE REFLECTION reflection USING DIMENSIONS (x by day,y) DISTRIBUTE BY (r,z) PARTITION BY (s,l) LOCALSORT BY (n,x)");
  }
  ...
  public void addAggReflectionEmptyMeasuresEmptyDimensions() {
    Exception exception1 =
        Assert.assertThrows(
            ...
            () -> {
              parse(
                  "ALTER TABLE a.b.c CREATE AGGREGATE REFLECTION reflection USING DIMENSIONS () MEASURES ()");
            });
    Assert.assertTrue(
        exception1.getMessage().contains("Both Dimensions and Measures cannot be empty."));
    ...
    Assert.assertTrue(
        exception2.getMessage().contains("Both Dimensions and Measures cannot be empty."));
  }

  @Test
  public void addRawReflection() throws SqlParseException {
    parse(
        "ALTER TABLE a.b.c CREATE RAW REFLECTION reflection USING DISPLAY(x,y) DISTRIBUTE BY (r,z) PARTITION BY (s,l) LOCALSORT BY (n,x)");
  }
  ...
  public void createRawWithSpecifier() throws SqlParseException {
    parse(
        "ALTER TABLE a.b.c AT BRANCH main CREATE RAW REFLECTION reflection USING DISPLAY(x,y) DISTRIBUTE BY (r,z) PARTITION BY (s,l) LOCALSORT BY (n,x)");
  }

  @Test
  public void createAggReflectionWithSpecifier() throws SqlParseException {
    parse(
        "ALTER TABLE a.b.c AT TAG footag CREATE AGGREGATE REFLECTION reflection USING DIMENSIONS (x by day,y) MEASURES (b (COUNT, SUM),c (COUNT, MIN, MAX)) DISTRIBUTE BY (r,z) PARTITION BY (s,l) LOCALSORT BY (n,x)");
  }
