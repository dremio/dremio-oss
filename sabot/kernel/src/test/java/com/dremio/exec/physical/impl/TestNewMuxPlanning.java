...
public class TestNewMuxPlanning extends PlanTestBase {
  private static final String sql =
      "select * from lineitem join orders on l_orderkey = o_orderkey order by l_orderkey desc";
  ...
  public static void setup() throws Exception {
    try (AutoCloseable a = withOption(ExecConstants.SLICE_TARGET_OPTION, 1);
      ...
      testNoResult(
          "create table lineitem distribute by (l_orderkey) as select * from cp.tpch.\"lineitem.parquet\"");
      testNoResult(
          "create table orders distribute by (o_orderkey) as select * from cp.tpch.\"orders.parquet\"");
    }
  }
  ...
  public void noMux() throws Exception {
    run(
        PlannerSettings.MUX_BUFFER_THRESHOLD.getDefault().getNumVal(),
        NONE,
        toArray("UnorderedMuxExchange"));
  }
