...
public class TestDeltaLakeTable extends BaseTestQuery {
  ...
  public void testWithLargeDatasetLatest() throws IOException {
    ...
    List<String> actual =
        table.getAllSplits().stream().map(this::getPath).collect(Collectors.toList());
  ...
  private void testWithLargeVersion(TimeTravelOption.TimeTravelRequest travelRequest)
      throws Exception {
  ...
  private void testWithSmallVersion(TimeTravelOption.TimeTravelRequest travelRequest)
      throws Exception {
    DeltaLakeTable table = new DeltaLakeTable(sabotContext, fs, selection, travelRequest);
    DeltaLogSnapshot snap = table.getConsolidatedSnapshot();
  ...
  private void testVersionZero(TimeTravelOption.TimeTravelRequest travelRequest)
      throws IOException {
    DeltaLakeTable table = new DeltaLakeTable(sabotContext, fs, selection, travelRequest);
    DeltaLogSnapshot snap = table.getConsolidatedSnapshot();
  ...
  public void testEndingWithCheckpointDataset() throws Exception {
    ...
    testEndingWithCheckpointDataset(
        selection, TimeTravelOption.newTimestampRequest(1608800728932L));
  }

  private void testEndingWithCheckpointDataset(
      FileSelection selection, TimeTravelOption.TimeTravelRequest travelRequest)
      throws IOException {
    DeltaLakeTable table = new DeltaLakeTable(sabotContext, fs, selection, travelRequest);
    DeltaLogSnapshot snap = table.getConsolidatedSnapshot();
  ...
  public void testEndingWithMultiPartCheckpointDatasetReadLatest() throws IOException {
    ...
    assertEquals(
        snap.getSchema(),
        "{\"type\":\"struct\",\"fields\":[{\"name\":\"intcol\",\"type\":\"integer\",\"nullable\":true,\"metadata\":{}}, {\"name\":\"longcol\",\"type\":\"long\",\"nullable\":true,\"metadata\":{}}, {\"name\":\"stringcol\",\"type\":\"string\",\"nullable\":true,\"metadata\":{}}]}");
  ...
  public void testMultipartCheckpointLastVersion() throws Exception {
    ...
    testMultipartCheckpointLastVersion(
        selection, TimeTravelOption.newTimestampRequest(1684117263315L));
  }

  private void testMultipartCheckpointLastVersion(
      FileSelection selection, TimeTravelOption.TimeTravelRequest travelRequest)
      throws IOException {
    DeltaLakeTable table = new DeltaLakeTable(sabotContext, fs, selection, travelRequest);
    DeltaLogSnapshot snap = table.getConsolidatedSnapshot();
  ...
  public void testMultipartCheckpointSkipToPreviousCheckpoint() throws Exception {
    ...
    testMultipartCheckpointSkipToPreviousCheckpoint(
        selection, TimeTravelOption.newSnapshotIdRequest("10"));
    testMultipartCheckpointSkipToPreviousCheckpoint(
        selection, TimeTravelOption.newTimestampRequest(1684117237899L));
  }

  private void testMultipartCheckpointSkipToPreviousCheckpoint(
      FileSelection selection, TimeTravelOption.TimeTravelRequest travelRequest)
      throws IOException {
    // checkpoint for version 9 is missing a part, expect to skip and use previous checkpoint for
    // version 6
  ...
  public void testMultipartCheckpointSkipToVersion0() throws Exception {
    ...
    testMultipartCheckpointSkipToVersion0(
        selection, TimeTravelOption.newTimestampRequest(1684117164770L));
  }

  private void testMultipartCheckpointSkipToVersion0(
      FileSelection selection, TimeTravelOption.TimeTravelRequest travelRequest)
      throws IOException {
    // checkpoint for version 3 is missing a part, expect to skip and use only json log files
    // starting version 0
