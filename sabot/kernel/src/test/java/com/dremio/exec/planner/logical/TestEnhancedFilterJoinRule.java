...
import static com.dremio.test.dsl.RexDsl.boolInput;
...
public class TestEnhancedFilterJoinRule {
  ...
  @Test
  public void testWithTopFilterWithBooleanCondition() {
    Filter filterRel =
        (Filter)
            relBuilder
                .values(new String[] {"a", "b"}, 1, false)
                .values(new String[] {"x", "y"}, 5, 8)
                .join(JoinRelType.INNER, literal(true))
                .filter(boolInput(1))
                .build();
  ...
  public void testWithTopFilterFilterHasConditionExtractOnlyJoinCondition() {
    testWithTopFilter(
        or(
            and(
                eq(intInput(0), intInput(4)),
                or(eq(intInput(1), literal(10)), eq(intInput(5), literal(20)))),
            and(
                eq(intInput(0), intInput(4)),
                or(eq(intInput(1), literal(30)), eq(intInput(5), literal(40))))),
        literal(true),
        JoinRelType.INNER,
        JoinRelType.INNER,
        "=($0, $4)",
  ...
  public void testWithTopFilterCanPushdownFilterBelowNot() {
    testWithTopFilter(
        or(
            and(
                eq(intInput(0), intInput(4)),
                not(or(eq(intInput(1), literal(10)), eq(intInput(5), literal(20))))),
            and(
                eq(intInput(0), intInput(4)),
                not(or(eq(intInput(2), literal(30)), eq(intInput(4), literal(40)))))),
        literal(true),
        JoinRelType.INNER,
        JoinRelType.INNER,
        "=($0, $4)",
        "OR(<>($1, 10), <>($2, 30))",
        "OR(<>($1, 20), <>($0, 40))",
        "OR(AND(<>($1, 10), <>($5, 20)), AND(<>($2, 30), <>($4, 40)))");
  }
  ...
  public void testWithTopFilterCannotPushdownFilterBelowNot() {
    testWithTopFilter(
        or(
            and(
                eq(intInput(0), intInput(4)),
                not(and(eq(intInput(1), literal(10)), eq(intInput(5), literal(20))))),
            and(
                eq(intInput(0), intInput(4)),
                not(and(eq(intInput(2), literal(30)), eq(intInput(3), literal(40)))))),
        literal(true),
        JoinRelType.INNER,
        JoinRelType.INNER,
        "=($0, $4)",
  ...
  public void testSimplifyRemainingUpperOrHaveCommonNotExact() {
    testWithTopFilter(
        or(
            ...
            and(
                or(
                    and(
                        eq(intInput(4), literal(10)),
                        eq(intInput(6), literal(10)),
                        eq(intInput(2), literal(10))),
                    and(
                        eq(intInput(4), literal(10)),
                        eq(intInput(6), literal(10)),
                        eq(intInput(3), literal(10)))),
                eq(intInput(7), literal(10)))),
        literal(true),
        JoinRelType.INNER,
        JoinRelType.INNER,
        ...
        "OR(AND(=($0, 10), =($1, 10), =($3, 10)), " + "AND(=($0, 10), =($2, 10), =($3, 10)))",
  ...
  /** filter: a = x AND b AND y join: c AND z */
  @Test
  public void testLeftOuterJoinSimplifyToInner() {
    testWithTopFilter(
        and(
            eq(intInput(0), intNullInput(4)),
            eq(intInput(1), literal(10)),
            eq(intNullInput(5), literal(20))),
  ...
  public void testFullOuterJoinSimplifyToInner() {
    testWithTopFilter(
        and(
            eq(intNullInput(0), intNullInput(4)),
            eq(intNullInput(1), literal(10)),
            eq(intNullInput(5), literal(20))),
  ...
  public void testLeftOuterJoinCanPushTopLevelFilterToLeftButNotRight() {
    testWithTopFilter(
        and(
            eq(intInput(0), literal(10)),
            or(eq(intNullInput(4), literal(20)), isNull(intNullInput(4)))),
        literal(true),
        JoinRelType.LEFT,
        JoinRelType.LEFT,
        "true",
  ...
  @Test
  public void testNoInfiniteFilterPushdownDueToSimplification() {
    testWithTopFilter(
        or(
            and(notEq(intNullInput(4), literal(0)), eq(intInput(0), literal(1))),
            and(notEq(intNullInput(4), literal(1)), eq(intInput(0), literal(2)))),
        literal(true),
        JoinRelType.LEFT,
        JoinRelType.LEFT,
        "true",
        "OR(=($0, 1), =($0, 2))",
        "true",
        "OR(AND(<>($4, 0), =($0, 1)), AND(<>($4, 1), =($0, 2)))");
  }

  private void testWithTopFilter(
    ...
    checkPushdown(
        rewrite,
        expectedJoinType,
        expectedJoinConditionString,
        expectedLeftPushdownPredicateString,
        expectedRightPushdownPredicateString,
        expectedRemainingFilterString);
    checkInfiniteLoop(rewrite);
  }

  private void testNoTopFilter(
      RexNode inputJoinCondition,
      JoinRelType joinRelType,
      JoinRelType expectedJoinType,
      String expectedJoinConditionString,
  ...
  private static Pair<String, String> getPushdownPredicatesString(RelNode rewrite) {
    ...
    String pushdownPredicatesStringLeft =
        (joinLeftInput instanceof Filter)
            ? ((Filter) joinLeftInput).getCondition().toString()
            : "true";
    String pushdownPredicatesStringRight =
        (joinRightInput instanceof Filter)
            ? ((Filter) joinRightInput).getCondition().toString()
            : "true";
  ...
  private static String getRemainingFilterString(RelNode rewrite) {
    ...
    if (rewrite instanceof Filter) {
      return ((Filter) EnhancedFilterJoinRule.removeArtifacts(rewrite)).getCondition().toString();
    } else {
      return "true";
    }
  }
