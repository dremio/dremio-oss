...
public class CopyIntoParquetTests extends ITCopyIntoBase {
  ...
  public static void testParquetMixedSchemas(BufferAllocator allocator, String source) throws Exception {
    // two files, one with INT columns, the other one with BIGINT columns, target table with BIGINT columns
    ...
    CopyIntoTests.createCopyIntoSourceFiles(
      ...
      location, ITCopyIntoBase.FileFormat.PARQUET);

    String storageLocation = "\'@" + source + "/" + location.getName() + "\'";
    final String copyIntoQuery = String.format("COPY INTO %s.%s FROM %s FILE_FORMAT 'parquet'",
  ...
  public static void testParquetSingleFile(BufferAllocator allocator, String source) throws Exception {
    String tableName = "cars";
    String fileName = "cars.parquet";
    File location = CopyIntoTests.createTempLocation();
    // parquet file has `INT` type
    CopyIntoTests.createTableAndGenerateSourceFiles(tableName,
      ImmutableList.of(
      ...
      new String[] { fileName }, location, ITCopyIntoBase.FileFormat.PARQUET);

    String storageLocation = "\'@" + source + "/" + location.getName() + "\'";
    final String copyIntoQuery = String.format("COPY INTO %s.%s FROM %s FILES (\'%s\')",
    ...
    new TestBuilder(allocator)
      ...
      .baselineValues(1996L, "Jeep", "Grand Cherokee", "MUST SELL! air, moon roof, loaded", 4799.00)
      .go();
  ...
  public static void testParquetBinaryToStringColumn(BufferAllocator allocator, String source) throws Exception {
  ...
  public static void testParquetMissingColumns(BufferAllocator allocator, String source) throws Exception {
    String tableName = "cars";
    String fileName = "cars.parquet";
    File location = CopyIntoTests.createTempLocation();
    // parquet file has `INT` type
    String schema = "make_year BIGINT, make VARCHAR, model VARCHAR, description VARCHAR, price DOUBLE, missing INT";
    CopyIntoTests.createTableAndGenerateSourceFiles(tableName, ImmutableList.of(
        Pair.of("make_year", "BIGINT"),
        Pair.of("make", "VARCHAR"),
        Pair.of("model", "VARCHAR"),
        Pair.of("description", "VARCHAR"),
    ...
    new TestBuilder(allocator)
      ...
      .baselineValues(1999L, "Chevy", "Venture \"Extended Edition, Very Large\"", null, 5000.00, null)
      .baselineValues(1996L, "Jeep", "Grand Cherokee", "MUST SELL! air, moon roof, loaded", 4799.00, null)
      .go();
  ...
  public static void testParquetExtraColumns(BufferAllocator allocator, String source) throws Exception {
    String tableName = "cars";
    String fileName = "cars.parquet";
    File location = CopyIntoTests.createTempLocation();
    // parquet file has `INT` type
  ...
  public static void testParquetColumnCaseInsensitive(BufferAllocator allocator, String source) throws Exception {
    // two files, one with INT columns, the other one with BIGINT columns, target table with BIGINT columns
    String tableName = "mixed";
    File location = CopyIntoTests.createTempLocation();
  ...
  public static void testParquetNoColumnMatch(BufferAllocator allocator, String source) throws Exception {
    // two files, one with INT columns, the other one with BIGINT columns, target table with BIGINT columns
    ...
    UserExceptionAssert.assertThatThrownBy(() -> test(copyIntoQuery))
        .satisfies(ex -> assertThat(ex).hasMessageContaining(
  ...
  public static void testParquetWithOnErrorContinue(String source) throws Exception {
    ...
    CopyIntoTests.createCopyIntoSourceFiles(
      ...
      location, ITCopyIntoBase.FileFormat.PARQUET);
    String storageLocation = "\'@" + source + "/" + location.getName() + "\'";
    final String copyIntoQuery = String.format("COPY INTO %s.%s FROM %s FILE_FORMAT 'parquet' (ON_ERROR 'continue')",
      CopyIntoTests.TEMP_SCHEMA, tableName, storageLocation);
    UserExceptionAssert.assertThatThrownBy(() -> test(copyIntoQuery))
      .satisfies(ex -> assertThat(ex).hasMessageContaining(
