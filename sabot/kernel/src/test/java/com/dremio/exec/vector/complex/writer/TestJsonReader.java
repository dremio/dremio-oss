...
public class TestJsonReader extends PlanTestBase {
  ...
  public void testFieldSelectionBug() throws Exception {
    try {
      testBuilder()
          .sqlQuery(
              "select t.field_4.inner_3 as col_1, t.field_4 as col_2 from cp.\"store/json/schema_change_int_to_string.json\" t")
          .unOrdered()
  ...
  public void runTestsOnFile(
      String filename, UserBitShared.QueryType queryType, String[] queries, long[] rowCounts)
      throws Exception {
    if (VERBOSE_DEBUG) {
      System.out.println("===================");
  ...
  public void testDrill_1419() throws Exception {
    String[] queries = {
      "select t.trans_id, t.trans_info.prod_id[0],t.trans_info.prod_id[1] from cp.\"/store/json/clicks.json\" t limit 5"
    };
    long[] rowCounts = {5};
  ...
  private void testLeafLimit(String file) throws Exception {
    try (AutoCloseable op1 = withOption(CatalogOptions.METADATA_LEAF_COLUMN_MAX, 4);
      ...
      Assert.assertEquals(
          ColumnCountTooLargeException.class.getCanonicalName(),
          e.getOrCreatePBError(false).getException().getExceptionClass());
    }
  }
  ...
  public void ensureProjectionPushdown() throws Exception {
    // Tests to make sure that we are correctly eliminating schema changing columns.  If completes,
    // means that the projection pushdown was successful.
    test(
  ...
  // The project pushdown rule is correctly adding the projected columns to the scan, however it is
  // not removing
  // the redundant project operator after the scan, this tests runs a physical plan generated from
  // one of the tests to
  ...
  public void testProjectPushdown() throws Exception {
    String[] queries = {
      readResourceAsString("/store/json/project_pushdown_json_physical_plan.json")
    };
    long[] rowCounts = {3};
    ...
    // Scanners are not responsible for projecting non-existent columns (as long as they project one
    // column)
    assertEquals(3, batchLoader.getSchema().getFieldCount());
    testExistentColumns(batchLoader);
  ...
  public void testJsonDirectoryWithEmptyFile() throws Exception {
    String root =
        FileUtils.getResourceAsFile("/store/json/jsonDirectoryWithEmpyFile").toURI().toString();
  ...
  private void testExistentColumns(RecordBatchLoader batchLoader) throws SchemaChangeException {
    ...
    vw =
        batchLoader.getValueAccessorById(
            IntVector.class, //
            batchLoader
                .getValueVectorId(SchemaPath.getCompoundPath("field_3", "inner_1"))
                .getFieldIds() //
            );
    ...
    vw =
        batchLoader.getValueAccessorById(
            IntVector.class, //
            batchLoader
                .getValueVectorId(SchemaPath.getCompoundPath("field_3", "inner_2"))
                .getFieldIds() //
            );
    assertNull(vw.getValueVector().getObject(0));
    ...
    vw =
        batchLoader.getValueAccessorById(
            ListVector.class, //
            batchLoader
                .getValueVectorId(SchemaPath.getCompoundPath("field_4", "inner_1"))
                .getFieldIds() //
            );
  ...
  public void drill_4032() throws Exception {
    ...
    BufferedOutputStream os =
        new BufferedOutputStream(new FileOutputStream(new File(table_dir, "a.json")));
  ...
  public void drill_4479() throws Exception {
    ...
    try {
      ...
      table_dir.mkdir();
      BufferedOutputStream os =
          new BufferedOutputStream(new FileOutputStream(new File(table_dir, "mostlynulls.json")));
      ...
      String query2 =
          "select a, b, c, count(*) as cnt from dfs_test.tmp.drill_4479 t group by a, b, c";
  ...
  public void caseStruct1() throws Exception {
    final String query =
        "SELECT CASE WHEN t.a.b = 1 THEN t.a ELSE null END AS a FROM cp.\"jsoninput/input4.json\" t";
  ...
  public void caseList1() throws Exception {
    final String query =
        "SELECT FLATTEN(CASE WHEN t.z[0].orange = 'yellow' THEN t.rl[0] ELSE t.l END) AS a"
  ...
  public void caseList2() throws Exception {
    final String query =
        "SELECT FLATTEN(CASE WHEN t.z[0].orange = 'yellow' THEN null ELSE t.l END) AS a"
            + " FROM cp.\"jsoninput/input2.json\" t";
