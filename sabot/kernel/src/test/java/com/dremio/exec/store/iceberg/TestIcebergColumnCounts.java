...
public class TestIcebergColumnCounts extends BaseTestQuery {
  ...
  public void tableWithNulls() throws Exception {
    for (Map.Entry<String, IcebergCatalogType> schemaCatalogPair: SCHEMAS_TO_CATALOG_TYPE.entrySet()) {
      ...
      try (AutoCloseable ac = enableIcebergTables()) {
        ...
        IcebergTableWrapper tableWrapper = new IcebergTableWrapper(getSabotContext(), localFs,
          getIcebergModel(tableFolder, schemaCatalogPair.getValue()), tableFolder.toPath().toString());
  ...
  public void testComplex() throws Exception {
    for (Map.Entry<String, IcebergCatalogType> schemaCatalogPair: SCHEMAS_TO_CATALOG_TYPE.entrySet()) {
      ...
      try (AutoCloseable ac = enableIcebergTables()) {
        ...
        testBuilder()
          .sqlQuery(String.format("select count(*) c from %s.%s", schemaCatalogPair.getKey(), tableName))
          .unOrdered()
          .baselineColumns("c")
          .baselineValues(10L)
          .build()
