...
public class TestParquetUtil {
  ...
  public static String setupParquetFiles(String testName, String folderName, String primaryParquet, String workingPath) throws Exception {
    /*
    ...
    try {
      ...
      BaseTestQuery.runSQL("SELECT * FROM dfs.\"" + parquetFiles + "\"");  // to detect schema and auto promote
      ...
      BaseTestQuery.runSQL("alter table dfs.\"" + parquetFiles + "\" refresh metadata force update");  // so it detects second parquet
      BaseTestQuery.setEnableReAttempts(true);
      BaseTestQuery.runSQL("select * from dfs.\"" + parquetFiles + "\""); // need to run select * from pds to get correct schema update. Check DX-25496 for details.
      return parquetFiles;
    } catch (Exception e) {
