...
public class TestMapTypeSyntax extends PlanTestBase {
  ...
  public void testSimpleFilterOnMapWithKeyStringAndValueString() throws Exception {
    try (AutoCloseable ignored = withOption(ExecConstants.ENABLE_MAP_DATA_TYPE, true);
      ...
      String query =
          String.format(
              "SELECT counters FROM  %s.\"map_type.parquet\" WHERE  counters['stats.put.failure'] = 'test'",
              path1);
      testPlanMatchingPatterns(
          query,
          new String[] {
  ...
  public void testSimpleGroupByValueOnMapWithKeyStringAndValueString() throws Exception {
    try (AutoCloseable ignored = withOption(ExecConstants.ENABLE_MAP_DATA_TYPE, true);
      ...
      String query =
          String.format(
              "SELECT counters['stats.put.failure'] FROM  %s.\"map_type.parquet\" GROUP BY  counters['stats.put.failure']",
              path1);
      testPlanMatchingPatterns(
          query,
          new String[] {
  ...
  public void testSimpleProjectOnMapWithKeyStringAndValueFloat() throws Exception {
    try (AutoCloseable ignored = withOption(ExecConstants.ENABLE_MAP_DATA_TYPE, true);
      ...
      String query =
          String.format(
              "SELECT measures['current_stock'] FROM  %s.\"map_float_type.parquet\"", path1);
      testPlanMatchingPatterns(
          query,
          new String[] {
            ".*Project.*ITEM.*value" + "(?s).*Project.*EXPR.*MAP_ENTRY.*'current_stock'",
          });
    }
  }
  ...
  public void testSimpleFilterOnMapWithKeyStringAndValueFloat() throws Exception {
    try (AutoCloseable ignored = withOption(ExecConstants.ENABLE_MAP_DATA_TYPE, true);
      ...
      String query =
          String.format(
              "SELECT measures FROM  %s.\"map_float_type.parquet\" WHERE  measures['current_stock'] = 44",
              path1);
      testPlanMatchingPatterns(
          query,
          new String[] {
  ...
  public void testSimpleGroupByValueOnMapWithKeyStringAndValueFloat() throws Exception {
    try (AutoCloseable ignored = withOption(ExecConstants.ENABLE_MAP_DATA_TYPE, true);
      ...
      String query =
          String.format(
              "SELECT measures['current_stock'] FROM  %s.\"map_float_type.parquet\" GROUP BY  measures['current_stock']",
              path1);
      testPlanMatchingPatterns(
          query,
          new String[] {
  ...
  public void testSimpleOrderByWithKeyStringAndValueFloat() throws Exception {
    try (AutoCloseable ignored = withOption(ExecConstants.ENABLE_MAP_DATA_TYPE, true);
      ...
      String query =
          String.format(
              "SELECT measures['current_stock'] FROM  %s.\"map_float_type.parquet\" ORDER BY  measures['current_stock']",
              path1);
      testPlanMatchingPatterns(
          query,
          new String[] {
  ...
  public void testSimpleExprInSelect() throws Exception {
    try (AutoCloseable ignored = withOption(ExecConstants.ENABLE_MAP_DATA_TYPE, true);
      ...
      String query =
          String.format(
              "SELECT retailer_id, \n"
                  + " 100 + measures['test'] , activity_type FROM %s.\"map_float_type.parquet\"",
              path1);
      testPlanMatchingPatterns(
          query,
          new String[] {
            ".*Project.*EXPR.*100.*ITEM.*'value'" + "(?s).*Project.*EXPR.*MAP_ENTRY.*'test'",
          });
    }
  }
  ...
  public void testSyntaxWithITEMOperator() throws Exception {
    try (AutoCloseable ignored = withOption(ExecConstants.ENABLE_MAP_DATA_TYPE, true);
      ...
      String query =
          String.format(
              "SELECT t.measures['current_stock'] FROM %s.\"map_float_type.parquet\" t ", path1);
      testPlanMatchingPatterns(
          query,
          new String[] {
            ".*Project.*ITEM.*'value'" + "(?s).*Project.*EXPR.*MAP_ENTRY.*'current_stock'"
