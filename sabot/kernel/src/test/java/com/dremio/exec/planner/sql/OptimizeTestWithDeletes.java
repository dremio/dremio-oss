...
public class OptimizeTestWithDeletes extends BaseTestQuery {
  ...
  private static final String SETUP_BASE_PATH = "/tmp/iceberg-test-tables/v2/multi_rowgroup_orders_with_deletes";
  ...
  private static final PartitionSpec PARTITIONED_SPEC = PartitionSpec.builderFor(ICEBERG_SCHEMA)
    .identity("order_year").build();
  ...
  private static final DeleteFile DEL_F1 = unpartitionedDeleteFile("2021/delete-2021-00.parquet", 27919);
  private static final DeleteFile DEL_F2 = unpartitionedDeleteFile("2021/delete-2021-01.parquet", 3841);
  ...
  private static TableInfo setupUnpartitionedV2Table(List<DataFile> unlinkedDataFiles,
                                                                List<DataFile> linkedDataFiles,
                                                                List<DeleteFile> deleteFiles) throws Exception {
    // Setup metadata in a temporary location, so table path doesn't conflict across test-cases
    String metadataPath = Files.createTempDirectory(Paths.get(SETUP_BASE_PATH), "optimizeV2").toAbsolutePath().toString();
    ...
    final String tableFqn = Arrays.stream(tablePathComponents, 2, tablePathComponents.length)
      .collect(Collectors.joining("\".\"", "dfs_hadoop_mutable.\"", "\""));

    String tablePath = "file://" + metadataPath;
    final TableMetadata metadata = TableMetadata.newTableMetadata(ICEBERG_SCHEMA, UNPARTITIONED_SPEC, tablePath,
  ...
  public static TableInfo setupPartitionedV2Table() throws Exception {
    // Setup metadata in a temporary location, so table path doesn't conflict across test-cases
    String metadataPath = Files.createTempDirectory(Paths.get(SETUP_BASE_PATH), "optimizeV2Partitioned").
    ...
    String tablePath = "file://" + metadataPath;
    final TableMetadata metadata = TableMetadata.newTableMetadata(ICEBERG_SCHEMA, PARTITIONED_SPEC, tablePath,
      ImmutableMap.of("format-version", "2"));
    TableOperations ops = new TestHadoopTableOperations(new Path(tablePath), fs.getConf());
  ...
  private static void assertOptimize(BufferAllocator allocator, String table, String options,
                                     long expectedRewrittenDataFiles, long expectedRewrittenDeleteFiles,
                                     long expectedNewDataFiles) throws Exception {
    new TestBuilder(allocator)
      .sqlQuery(String.format("optimize table %s (%s)", table, options))
      .unOrdered()
      .baselineColumns("rewritten_data_files_count", "rewritten_delete_files_count", "new_data_files_count")
      .baselineValues(expectedRewrittenDataFiles, expectedRewrittenDeleteFiles, expectedNewDataFiles)
      .build()
      .run();
  }
  ...
  public static void testV2OptimizeDeleteLinkedFilesOnlyPartitioned(BufferAllocator allocator) throws Exception {
    TableInfo tableInfo = setupPartitionedV2Table();
    assertOptimize(allocator, tableInfo.getTableName(), "MIN_INPUT_FILES=1, MIN_FILE_SIZE_MB=0", 5L, 4L, 2L);
    fs.delete(new Path(tableInfo.getMetadataLocation()), true);
  }

  public static void testV2OptimizeUnpartitioned(BufferAllocator allocator) throws Exception {
    TableInfo tableInfo = setupUnpartitionedV2Table(Collections.EMPTY_LIST, l(DF1, DF2, DF3), l(DEL_F1));
  ...
  public static void testV2OptimizeMinInputFiles(BufferAllocator allocator) throws Exception {
    TableInfo tableInfo = setupUnpartitionedV2Table(Collections.EMPTY_LIST, l(DF1, DF2, DF3, DF4), l(DEL_F1));
    assertOptimize(allocator, tableInfo.getTableName(), "MIN_INPUT_FILES=5", 4L, 1L, 1L);
    fs.delete(new Path(tableInfo.getMetadataLocation()), true);
  }

  public static void testV2OptimizeDeleteLinkedFilesOnlyUnpartitioned(BufferAllocator allocator) throws Exception {
    TableInfo tableInfo = setupUnpartitionedV2Table(l(DF4), l(DF1, DF2, DF3), l(DEL_F1));
    assertOptimize(allocator, tableInfo.getTableName(), "MIN_INPUT_FILES=1, MIN_FILE_SIZE_MB=0", 3L, 1L, 1L);
    fs.delete(new Path(tableInfo.getMetadataLocation()), true);
  }
  ...
  public static void testV2OptimizeMultipleDeleteFiles(BufferAllocator allocator) throws Exception {
    ...
    assertOptimize(allocator, tableInfo.getTableName(), "MIN_INPUT_FILES=1, MIN_FILE_SIZE_MB=0", 1L, 2L, 1L);
    fs.delete(new Path(tableInfo.getMetadataLocation()), true);
  }

  public static void testV2OptimizePartitionEvolution(BufferAllocator allocator) throws Exception {
    TableInfo tableInfo = setupUnpartitionedV2Table(l(DF4), l(DF1, DF2, DF3), l(DEL_F1));
    runSQL(String.format("alter table %s add partition field order_year", tableInfo.getTableName()));
    assertOptimize(allocator, tableInfo.getTableName(), "MIN_INPUT_FILES=1, MIN_FILE_SIZE_MB=0", 4L, 1L, 2L);

    new TestBuilder(allocator)
      .sqlQuery(String.format("SELECT \"partition\" FROM TABLE(TABLE_FILES('%s'))", tableInfo.getTableName()))
      .unOrdered()
  ...
  public static void testV2OptimizeUpdateSequenceNumber(BufferAllocator allocator) throws Exception {
    final String metadataPath = Files.createTempDirectory(Paths.get(SETUP_BASE_PATH), "optimizeV2").toAbsolutePath().toString();
    final String[] tablePathComponents = StringUtils.split(metadataPath, '/');
    final String tableName = tablePathComponents[tablePathComponents.length - 1];
    final String tableFqn = Arrays.stream(tablePathComponents, 2, tablePathComponents.length)
      .collect(Collectors.joining("\".\"", "dfs_hadoop_mutable.\"", "\""));
    ...
    transaction.newRewrite().rewriteFiles(ImmutableSet.of(DF1), ImmutableSet.of(DF1))
    ...
    new TestBuilder(allocator)
      .sqlQuery(String.format("SELECT file_path FROM TABLE(TABLE_FILES('%s')) ORDER BY record_count LIMIT 1", tableFqn))
      .ordered()
  ...
  public static void testV2OptimizeUpdateSequenceNumberWithDeleteLink(BufferAllocator allocator) throws Exception {
    final String metadataPath = Files.createTempDirectory(Paths.get(SETUP_BASE_PATH), "optimizeV2").toAbsolutePath().toString();
    final String[] tablePathComponents = StringUtils.split(metadataPath, '/');
    final String tableName = tablePathComponents[tablePathComponents.length - 1];
    final String tableFqn = Arrays.stream(tablePathComponents, 2, tablePathComponents.length)
    ...
    // delf1 had 300 deletes linked to df1 (1000 rows) and df2 (1000 rows) each, while delf2 has 10 deletes for both.
    // Final count should be 1680 - 990 from df1 [10 rows from delf2 removed] and 690 from df2 [300 rows removed from delf1 and 10 rows from delf2].
    new TestBuilder(allocator)
  ...
  public static void testV2OptimizeForIdentityPartitions(BufferAllocator allocator) throws Exception {
    TableInfo tableInfo = setupPartitionedV2Table();
    new TestBuilder(allocator)
      .sqlQuery(String.format("optimize table %s FOR PARTITIONS order_year=2021 (MIN_INPUT_FILES=1)", tableInfo.getTableName()))
      .unOrdered()
      .baselineColumns("rewritten_data_files_count", "rewritten_delete_files_count", "new_data_files_count")
  ...
  public static void testV2OptimizeForIdentityPartitionsWithExpression(BufferAllocator allocator) throws Exception {
    TableInfo tableInfo = setupPartitionedV2Table();
    new TestBuilder(allocator)
      .sqlQuery(String.format("optimize table %s FOR PARTITIONS order_year%%2=0 (MIN_INPUT_FILES=1, MIN_FILE_SIZE_MB=0)", tableInfo.getTableName()))
      .unOrdered()
      .baselineColumns("rewritten_data_files_count", "rewritten_delete_files_count", "new_data_files_count")
  ...
  public static void testV2OptimizeEmptyPartition(BufferAllocator allocator) throws Exception {
    ...
    new TestBuilder(allocator)
      .sqlQuery(String.format("SELECT * FROM TABLE(TABLE_FILES('%s')) WHERE 'partition'='{order_year=2021}'", tableInfo.getTableName()))
      .unOrdered()
  ...
  private static DataFile partitionedDataFile(String relativePath, long size, int partitionOrderYr) {
    IcebergPartitionData icebergPartitionData = new IcebergPartitionData(PARTITIONED_SPEC.partitionType());
  ...
  private static DeleteFile partitionedDeleteFile(String relativePath, long size, long records, int partitionOrderYr) {
    IcebergPartitionData icebergPartitionData = new IcebergPartitionData(PARTITIONED_SPEC.partitionType());
    icebergPartitionData.set(0, partitionOrderYr);
