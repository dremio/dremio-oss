...
public class TestEqualityDeleteHashTable extends BaseTestEqualityDeleteFilter {
  ...
  private static final BatchSchema SCHEMA_FIXED =
      BatchSchema.newBuilder()
          .addField(Field.nullable("col1", Types.MinorType.INT.getType()))
          .build();
  private static final BatchSchema SCHEMA_VAR =
      BatchSchema.newBuilder()
          .addField(Field.nullable("col1", Types.MinorType.VARCHAR.getType()))
          .build();
  private static final BatchSchema SCHEMA_FIXED_AND_VAR =
      BatchSchema.newBuilder()
          .addField(Field.nullable("col1", Types.MinorType.INT.getType()))
  ...
  public void testSingleFixedField() throws Exception {
    RecordSet build = rs(SCHEMA_FIXED, rb(r(1), r(5), r(2)), rb(r((Integer) null), r(-8), r(10)));
    RecordSet probe =
        rs(
            SCHEMA_FIXED,
  ...
  private void buildAndValidate(
      RecordSet buildRs, RecordSet probeRs, int expectedSize, List<Integer> expectedOrdinalMapping)
      throws Exception {
    ...
    List<Integer> actualOrdinalMapping =
        probeOrdinals.stream()
