...
public class TestValuesRewrite extends PlanTestBase {
  ...
  public void testDecimalValue5() throws Exception {
    String sql =
        "select * from (values " + "(cast (1.23 as decimal(3,2)), cast (4.5 as decimal(2,1))))";
    ...
    for (String testSchema : SCHEMAS_FOR_TEST) {
      ...
      try {
        final String createTableQuery =
            String.format("create table %s.%s (d decimal(20,10))", testSchema, tableName);
        test(createTableQuery);

        Thread.sleep(1001);
        final String insertTableQuery =
            String.format("insert into %s.%s values(1.23)", testSchema, tableName);
  ...
  public void testInsertMultipleValues() throws Exception {
    for (String testSchema : SCHEMAS_FOR_TEST) {
      ...
      try {
        final String createTableQuery =
            String.format("create table %s.%s (col1 int, col2 int)", testSchema, tableName);
        test(createTableQuery);

        Thread.sleep(1001);
        final String insertTableQuery =
            String.format(
                "insert into %s.%s (" + "values" + "(1, 2)," + "(3, null)" + ")",
  ...
  public void testInsertSomeColumnValues() throws Exception {
    for (String testSchema : SCHEMAS_FOR_TEST) {
      ...
      try {
        final String createTableQuery =
            String.format(
                "" + "create table %s.%s " + "(c1 int, c2 varchar, c3 int)", testSchema, tableName);
        test(createTableQuery);

        Thread.sleep(1001);
        String insertTableQuery =
            String.format(
                "insert into %s.%s (values(1))", // c2 and c3 should be null
                testSchema, tableName);
        test(insertTableQuery);
        Thread.sleep(1001);
        ...
        insertTableQuery =
            String.format(
                "insert into %s.%s (values(2,3))", // c3 should be null
                testSchema, tableName);
        test(insertTableQuery);
        Thread.sleep(1001);
        ...
        insertTableQuery =
            String.format(
                "insert into %s.%s (c1, c3) (values(4,5))", // c2 should be null
                testSchema, tableName);
        test(insertTableQuery);
        Thread.sleep(1001);
        ...
        assertThatThrownBy( // user exception should be thrown
                () ->
                    test(
                        String.format(
                            "insert into %s.%s (values(2,'3',1, 2))", testSchema, tableName)))
