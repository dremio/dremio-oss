...
/**
 * This test class verifies the IcebergBaseModel.getPreviousMetadataRoot() Method. This test ensures
 * the metadata location matches the location committed to the Nessie Commit Log during physical
 * dataset promotion. Although a user may have relocated the location of the metadata / manifest
 * files, nessie's commit log only has reference to the original path. Metadata path format varies
 * on the distributed storage source type. This test is designed verify support for all distributed
 * storage config sources.
 */
public class TestIcebergPreviousTableMetadataRoot {
  ...
  final String S3_PATH_ABS =
      "s3://fake.dummy.authentication/usr/A/metadata/fb395f25-4bd7-4858-bbc0-10654b5ebe58/metadata/00003-e8a08c76-dda8-47d9-b5ce-0705a7f39155.metadata.json";
  final Path S3_PATH_EXPECT =
      Path.of("/fake.dummy.authentication/usr/A/metadata/fb395f25-4bd7-4858-bbc0-10654b5ebe58");
  ...
  final Path NAS_PATH_EXPECT =
      Path.of("/Users/firstname.lastname/myNAS/A/metadata/3def2c01-94bc-4089-abb3-12c9488beeee");
  ...
  /**
   * Tests IcebergBaseModel.getPreviousTableMetadataRoot() on a fake S3 distributed storage source
   * config NOTE: test does not actually connect to a S3 source
   */
  @Test
  public void testS3() {
