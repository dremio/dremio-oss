...
public class TestIcebergSnapshotFunction extends IcebergMetadataTestTable {
  ...
  public void testTableSnapshotsCountWithMultipleBatches() throws Exception {
    ...
    try (AutoCloseable closeableForMinSetting = setSystemOptionWithAutoReset(TARGET_BATCH_RECORDS_MIN.getOptionName(), Integer.toString(minBatchSize))) {
      try (AutoCloseable closeableForMaxSetting = setSystemOptionWithAutoReset(TARGET_BATCH_RECORDS_MAX.getOptionName(), Integer.toString(maxBatchSize))) {
        for (int i = 0; i < numberOfNewSnapshots; ++i) {
          // note: table is reset after each test case
          String insertCommand = String.format("insert into %s.%s values (%d, '%s', %f)",
              TEMP_SCHEMA_HADOOP, METADATA_TEST_TABLE_NAME,
              i, "row number " + i, i + 0.5);
          runSQL(insertCommand);
        }
        queryAndMatchResults(String.format("SELECT count(*) as snapshot_count FROM table(table_snapshot('\"%s\".\"%s\"'))", TEMP_SCHEMA_HADOOP, METADATA_TEST_TABLE_NAME),
            new String[]{"snapshot_count"}, new Object[]{(long) numberOfNewSnapshots + 1});
      }
    }
  ...
  public void testJobIdStoredInTableMetadata() throws Exception {
    String appendQuery = String.format("insert into %s.%s values (%d, '%s', %f)",
        TEMP_SCHEMA_HADOOP, METADATA_TEST_TABLE_NAME, 0, "append", 0.1);
    String overwriteQuery = String.format("update %s.%s set c2='%s' where c1=%d",
    ...
    String jobIdPropertyQuery =
      String.format("SELECT operation, t.f.\"key\", t.f.\"value\" FROM (" +
        "SELECT operation, flatten(summary) as f FROM table(table_snapshot('\"%s\".\"%s\"'))" +
        ") t where t.f.\"key\" like '%s' ",
        TEMP_SCHEMA_HADOOP, METADATA_TEST_TABLE_NAME, IcebergBaseCommand.DREMIO_JOB_ID_ICEBERG_PROPERTY);
    ...
    new TestBuilder(allocator)
      ...
      .baselineValues("overwrite", IcebergBaseCommand.DREMIO_JOB_ID_ICEBERG_PROPERTY, overwriteJobId)
      .go();
