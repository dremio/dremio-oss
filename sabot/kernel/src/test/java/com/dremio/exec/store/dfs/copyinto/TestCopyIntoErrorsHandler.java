...
public class TestCopyIntoErrorsHandler {
  ...
  @Mock
  private SystemIcebergTableManager copyFileJobHistoryTableManager;
  @Mock
  ...
  @Captor
  private ArgumentCaptor<Long> numSuccessCaptor;
  @Captor
  private ArgumentCaptor<Long> numErrorsCaptor;
  @Captor
  private ArgumentCaptor<List<String>> filePathsCaptor;
  @Captor
  private ArgumentCaptor<VectorContainer> containerCaptor;
  private BufferAllocator allocator;
  @Before
  public void setup() {
    allocator = allocatorRule.newAllocator("test-copy-into-errors-handler", 0, Long.MAX_VALUE);
    ...
    when(optionManager.getOption(ExecConstants.SYSTEM_ICEBERG_TABLES_WRITE_BATCH_SIZE)).thenReturn(1000L);
  ...
  public void testProcessAndCommit() throws Exception {
    ...
    CopyIntoErrorsHandler errorHandler = new CopyIntoErrorsHandler(context, plugin);
    errorHandler.setCopyJobHistoryTableManager(copyJobHistoryTableManager);
    errorHandler.setCopyFileHistoryTableManager(copyFileJobHistoryTableManager);

    // Create a CopyIntoErrorInfo instance
    CopyIntoErrorInfo errorInfo = new CopyIntoErrorInfo.Builder("queryId", "queryUser", "tableName",
      "storageLocation", "filePath1", new ExtendedFormatOptions(), FileType.JSON.name(), CopyIntoErrorInfo.CopyIntoFileState.PARTIALLY_LOADED)
    ...
    errorHandler.process(errorInfo);

    // Verify the state after processing
    assertThat(errorHandler.getNumberOfRejectedRecords()).isEqualTo(2);
    assertThat(errorHandler.getNumberOfLoadedRecords()).isEqualTo(3);
    assertThat(errorHandler.getErrorInfos().stream().map(CopyIntoErrorInfo::getFilePath).collect(Collectors.toList()))
      .isEqualTo(ImmutableList.of("filePath1"));

    // Create a CopyIntoErrorInfo instance
    errorInfo = new CopyIntoErrorInfo.Builder("queryId", "queryUser", "tableName",
      "storageLocation", "filePath2", new ExtendedFormatOptions(), FileType.JSON.name(), CopyIntoErrorInfo.CopyIntoFileState.PARTIALLY_LOADED)
    ...
    assertThat(errorHandler.getNumberOfRejectedRecords()).isEqualTo(14);
    assertThat(errorHandler.getNumberOfLoadedRecords()).isEqualTo(7);
  ...
  public void testProcessAndCommitReachBatchThreshold() throws Exception {
    ...
    when(optionManager.getOption(ExecConstants.SYSTEM_ICEBERG_TABLES_WRITE_BATCH_SIZE)).thenReturn(0L);
