...
public class TestRuntimeFilter {
  ...
  public void testIsOnSameColumnNonPartitionColumnsNoMatch() throws Exception {
    try (AutoCloseables.RollbackCloseable closer = new AutoCloseables.RollbackCloseable()) {
      ...
      CompositeColumnFilter ccf1 =
          newValListFilterLong("long_field", Lists.newArrayList(1L, 200L, 1500L, 4900L));
      CompositeColumnFilter ccf2 =
          newValListFilterInt("int_field", Lists.newArrayList(1, 900, 1501, 4900));
      CompositeColumnFilter ccf3 =
          newValListFilterInt("int_field", Lists.newArrayList(50, 890, 1550, 4990));
  ...
  public void testIsOnSameColumn() throws Exception {
    try (AutoCloseables.RollbackCloseable closer = new AutoCloseables.RollbackCloseable()) {
      ...
      CompositeColumnFilter ccf4 =
          newValListFilterLong("long_field", Lists.newArrayList(1L, 200L, 1500L, 4900L));
      CompositeColumnFilter ccf5 =
          newValListFilterInt("int_field1", Lists.newArrayList(50, 890, 1550, 4990));
  ...
  private CompositeColumnFilter newValListFilterInt(String col1, List<Integer> vals)
      throws Exception {
    byte blockSize = Integer.BYTES;
    try (ValueListFilterBuilder builder =
            new ValueListFilterBuilder(testAllocator, vals.size(), blockSize, false);
      ...
      return new CompositeColumnFilter.Builder()
          ...
          .setValueList(builder.build())
          .build();
    }
  }

  private CompositeColumnFilter newValListFilterLong(String col1, List<Long> vals)
      throws Exception {
    return newValListFilterLong(col1, vals, Types.MinorType.BIGINT);
  }

  private CompositeColumnFilter newValListFilterLong(
      String col1, List<Long> vals, Types.MinorType type) throws Exception {
    try (ValueListFilterBuilder builder =
            new ValueListFilterBuilder(testAllocator, vals.size(), (byte) 8, false);
