...
public class TestInsertIntoTable extends BaseTestQuery {
  ...
  public void testOutputColumnsForInsertUsingValues() throws Exception {
    try (AutoCloseable c = enableIcebergTables()) {
      testOutputColumnsForInsertUsingValues(
          "output_columns_insert_values_test", TEMP_SCHEMA_HADOOP);
    }
  }
    ...
    try {
      ...
      final String insertQuery =
          String.format(
              ...
              testSchema, newTable, testSchema, newTable);
      String expected =
          isComplexTypeSupport()
              ? "Table schema(listcol1::list<int64>) doesn't match with query schema(listcol2::list<int32>)"
              : "Table schema(listcol1::list<int64>) doesn't match with query schema(listcol1::list<int32>)";
      UserExceptionAssert.assertThatThrownBy(() -> test(insertQuery))
    ...
    try {
      ...
      String expected =
          isComplexTypeSupport()
              ...
              : "schema(structcol1::struct<name::varchar, age::int64>) doesn't match with query schema(structcol1::struct<name::varchar, age::int32>)";
      UserExceptionAssert.assertThatThrownBy(() -> test(insertQuery))
          .hasMessageContaining(expected);
    } finally {
      FileUtils.deleteQuietly(new File(getDfsTestTmpSchemaLocation(), newTable + "_1"));
    ...
    try {
      ...
      String expected =
          isComplexTypeSupport()
              ...
              : "Table schema(structcol1::struct<name::varchar, age::int64>) doesn't match with query schema(structcol1::varchar)";
      UserExceptionAssert.assertThatThrownBy(() -> test(insertQuery))
          .hasMessageContaining(expected);
    } finally {
      FileUtils.deleteQuietly(new File(getDfsTestTmpSchemaLocation(), newTable + "_1"));
