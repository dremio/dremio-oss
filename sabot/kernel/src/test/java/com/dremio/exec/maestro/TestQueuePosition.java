...
public class TestQueuePosition {
  ...
  @Mock QueryContext context;
  @Mock ClusterCoordinator clusterCoordinator;
  @Mock OptionManager optionsManager;
  @Mock MaestroObserver observer;
  ...
  public void testQueue() throws Exception {
    ...
    Mockito.when(context.getQueryContextInfo())
        .thenReturn(CoordExecRPC.QueryContextInformation.getDefaultInstance());
    ...
    Mockito.doAnswer(
        ...
        .when(observer)
        .resourcesScheduled(Mockito.any());
    BasicResourceAllocator ra = new BasicResourceAllocator(() -> clusterCoordinator, null);
    ResourceTracker resourceTracker = new ResourceTracker(context, ra);
    resourceTracker.allocate(plan, observer);
  }

  /**
   * We expect 2 calls. 1. queue has been allocated but we have not been added to the queue 3.
   * queueing is finished and we can go to execution
   */
