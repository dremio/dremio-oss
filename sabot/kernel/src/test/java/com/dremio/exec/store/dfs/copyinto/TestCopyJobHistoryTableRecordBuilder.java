...
public class TestCopyJobHistoryTableRecordBuilder {
  ...
  public void setup() {
    allocator =
        allocatorRule.newAllocator("test-copy-job-history-record-builder", 0, Long.MAX_VALUE);
  }
  ...
  public void testBuildRecord() {
    ...
    CopyIntoFileLoadInfo info = buildInfo();
    long numSuccess = 5L;
    long numErrors = 2L;
    ...
    try (VectorContainer container =
        CopyJobHistoryTableRecordBuilder.buildVector(
            allocator, schemaVersion, info, numSuccess, numErrors)) {

      assertThat(container.getSchema().getFields().size()).isEqualTo(schema.columns().size());

      try (TimeStampMilliVector executedAtVector =
              (TimeStampMilliVector)
                  VectorUtil.getVectorFromSchemaPath(container, schema.findColumnName(1));
          VarCharVector jobIdVector =
              (VarCharVector)
                  VectorUtil.getVectorFromSchemaPath(container, schema.findColumnName(2));
          VarCharVector tableNameVector =
              (VarCharVector)
                  VectorUtil.getVectorFromSchemaPath(container, schema.findColumnName(3));
          BigIntVector recordsLoadedCountVector =
              (BigIntVector)
                  VectorUtil.getVectorFromSchemaPath(container, schema.findColumnName(4));
          BigIntVector recordsRejectedCountVector =
              (BigIntVector)
                  VectorUtil.getVectorFromSchemaPath(container, schema.findColumnName(5));
          VarCharVector copyOptionsVector =
              (VarCharVector)
                  VectorUtil.getVectorFromSchemaPath(container, schema.findColumnName(6));
          VarCharVector userNameVector =
              (VarCharVector)
                  VectorUtil.getVectorFromSchemaPath(container, schema.findColumnName(7));
          BigIntVector snapshotIdVector =
              (BigIntVector)
                  VectorUtil.getVectorFromSchemaPath(container, schema.findColumnName(8));
          VarCharVector storageLocationVector =
              (VarCharVector)
                  VectorUtil.getVectorFromSchemaPath(container, schema.findColumnName(9));
          VarCharVector fileFormatVector =
              (VarCharVector)
                  VectorUtil.getVectorFromSchemaPath(container, schema.findColumnName(10))) {
        ...
        assertThat(copyOptionsVector.get(0))
  ...
  public void testBuildRecordForUnsupportedSchemaVersion() {
    assertThrows(
        ...
        () -> CopyJobHistoryTableRecordBuilder.buildVector(allocator, 2, buildInfo(), 5L, 2L));
  }

  private CopyIntoFileLoadInfo buildInfo() {
