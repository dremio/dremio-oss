...
public abstract class TestAbstractCaseNormalQueries extends PlanTestBase {
  ...
  public void testFixedCaseClause() throws Exception {
    ...
    testBuilder().sqlQuery(sql).ordered().baselineColumns("cnt").baselineValues(10L).go();
  }
  ...
  public void testFixedNestedCase() throws Exception {
    ...
    testBuilder().sqlQuery(sql).ordered().baselineColumns("cnt").baselineValues(12L).go();
  }
  ...
  public void testMixedCaseExpressionInWhen() throws Exception {
    ...
    testBuilder().sqlQuery(sql).unOrdered().baselineColumns("_cnt").baselineValues(4918L).go();
  }

  @Test
  public void testComplexCase() throws Exception {
    final String sql =
        "select case when c.ingredients[2].name = 'Cheese' then 'Scrumptious' "
  ...
  public void testMixedComplexCase() throws Exception {
    final String sql =
        "select case when c.ingredients[2].name = 'Cheese' then "
  ...
  public void testComplexReturn() throws Exception {
    final String sql =
        "select case when c.ingredients[0].name = 'Beef' then c.ingredients "
    ...
    testBuilder()
        ...
        .baselineValues(
            listOf(mapOf("name", "Beef"), mapOf("name", "Lettuce"), mapOf("name", "Cheese")))
        .go();
  }
