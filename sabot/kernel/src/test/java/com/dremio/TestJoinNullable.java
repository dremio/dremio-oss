...
public class TestJoinNullable extends PlanTestBase {
  ...
  public void testHashInnerJoinOnNullableColumns() throws Exception {
    String query =
        String.format(
            "select t1.a1, t1.b1, t2.a2, t2.b2 from dfs_root.\"%s/jsoninput/nullable1.json\" t1, "
                + " dfs_root.\"%s/jsoninput/nullable2.json\" t2 where t1.b1 = t2.b2",
  ...
  public void withNullEqualInWhereConditionNegative() throws Exception {
    final String query =
        "SELECT * FROM "
            ...
            + "WHERE t1.key = t2.key OR (t1.key IS NULL AND t2.key IS NULL)";
    errorMsgTestHelper(query, "This query cannot be planned");
  }
