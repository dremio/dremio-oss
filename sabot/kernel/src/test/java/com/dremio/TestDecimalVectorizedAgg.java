...
/**
 * Tests decimal vectorized aggregations - sum, sum0, min, max, count avg and std-dev.
 *
 * <p>Uses test file that is equivalent of [ {"a":"1", "val" : "123.12345678901234567890"},
 * {"a":"1", "val" : "1.12345678901234567890"}, {"a":"1", "val" : "-11.12345678901234567890"},
 * {"a":"2", "val" : "11.12345678901234567890"}, {"a":"2"}, {"a":"2", "val" :
 * "2.12345678901234567890"}, {"a":"2", "val" : "0.01"}, {"a":"2", "val" : "-2.0"}, {"a":"3"},
 * {"a":"3"}, {"a":"4", "val" : "0.01"}, {"a":"4", "val" : "2.12345678901234567890"}, {"a":"4",
 * "val" : "32.1020"} ]
 */
public class TestDecimalVectorizedAgg extends BaseTestQuery {

  @Test
  public void testDecimalNegative_Parquet() throws Exception {

    final String query =
        "SELECT negative(cast(val as decimal(3,2))-(cast(val as decimal(3,2))-1.5)) FROM cp"
  ...
  public void testDecimalSumAggNonZeroNullValues() throws Exception {
    ...
    final String query =
        "SELECT CASE WHEN t1.val < 0 THEN t1.key ELSE t2.key END, SUM(t1.val + t2.val) FROM "
  ...
  public void testDecimalSumZeroAggNonZeroNullValues() throws Exception {
    ...
    final String query =
        "SELECT CASE WHEN t1.val < 0 THEN t1.key ELSE t2.key END, $SUM0(t1.val +"
            + " t2.val) FROM "
            + "cp.\"parquet/decimals/dec1p0s.parquet\" t1 , cp"
            + ".\"parquet/decimals/dec10p10s.parquet\" t2 GROUP BY 1";
  ...
  public void testDecimalSumAggOverflow_Parquet() {
    final String query =
        "select sum(val) from cp" + ".\"parquet/decimals/overflow.parquet\" group by department";
  ...
  public void testDecimalSum0AggOverflow_Parquet() {
    final String query =
        "select $sum0(val) from cp" + ".\"parquet/decimals/overflow.parquet\" group by department";
  ...
  public void testDecimalAvgScaleUp_Parquet() throws Exception {
    ...
    final String query =
        "select avg(EXPR$0) from cp" + ".\"parquet/decimals/avg-scale-up.parquet\" group by c";
