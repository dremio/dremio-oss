...
public class TestStarQueries extends BaseTestQuery {
  ...
  public void testSelStarCommaSameColumnRepeated() throws Exception {
    testBuilder()
        ...
        .baselineTypes(
            ...
            MinorType.VARCHAR)
        .baselineColumns(
            "n_name", "n_nationkey", "n_name0", "n_regionkey", "n_comment", "n_name00", "n_name1")
        .build()
        .run();
  ...
  public void testSelStarMultipleStarsRegularColumnAsAlias() throws Exception {
    testBuilder()
        ...
        .ordered()
        .csvBaselineFile(
            "testframework/testStarQueries/testSelStarMultipleStarsRegularColumnAsAlias/q1.tsv")
        .baselineTypes(
            MinorType.INT,
            MinorType.VARCHAR,
            MinorType.INT,
    ...
    testBuilder()
        .sqlQuery(
            "select *, n_name as extra, *, n_name as extra from cp.\"tpch/nation.parquet\" limit 2")
        .ordered()
        .csvBaselineFile(
            "testframework/testStarQueries/testSelStarMultipleStarsRegularColumnAsAlias/q2.tsv")
        .baselineTypes(
            MinorType.INT,
            MinorType.VARCHAR,
            MinorType.INT,
  ...
  public void testSelStarWithAdditionalColumnLimit() throws Exception {
    testBuilder()
        ...
        .ordered()
        .csvBaselineFile(
            "testframework/testStarQueries/testSelStarWithAdditionalColumnLimit/q1.tsv")
        .baselineTypes(
            MinorType.INT,
            MinorType.VARCHAR,
            MinorType.INT,
  ...
  public void testSelStarOrderBy() throws Exception {
    testBuilder()
        ...
        .sqlBaselineQuery(
            " select employee_id, full_name,first_name,last_name,position_id,position_title,store_id,"
  ...
  public void testSelStarPlusRegCol() throws Exception {
    testBuilder()
        .unOrdered()
        .sqlQuery(
            "select *, n_nationkey as key2 from cp.\"tpch/nation.parquet\" order by n_name limit 2")
        .sqlBaselineQuery(
            "select n_comment, n_name, n_nationkey, n_regionkey, n_nationkey as key2 from cp.\"tpch/nation.parquet\" order by n_name limit 2")
        .build()
        .run();
  }
  ...
  public void testSelStarJoin() throws Exception {
    testBuilder()
        ...
        .sqlBaselineQuery(
            "select n.n_nationkey, n.n_name,n.n_regionkey,n.n_comment,r.r_regionkey,r.r_name, r.r_comment from cp.\"tpch/nation.parquet\" n, cp.\"tpch/region.parquet\" r where n.n_regionkey = r.r_regionkey order by n.n_name")
        .build()
        .run();
  }
  ...
  public void testSelLeftStarJoin() throws Exception {
    testBuilder()
        .ordered()
        .sqlQuery(
            "select n.* from cp.\"tpch/nation.parquet\" n, cp.\"tpch/region.parquet\" r where n.n_regionkey = r.r_regionkey order by n.n_name")
        .sqlBaselineQuery(
            "select n.n_nationkey, n.n_name, n.n_regionkey, n.n_comment from cp.\"tpch/nation.parquet\" n, cp.\"tpch/region.parquet\" r where n.n_regionkey = r.r_regionkey order by n.n_name")
        .build()
        .run();
  }
  ...
  public void testSelRightStarJoin() throws Exception {
    testBuilder()
        .ordered()
        .sqlQuery(
            "select r.* from cp.\"tpch/nation.parquet\" n, cp.\"tpch/region.parquet\" r where n.n_regionkey = r.r_regionkey order by n.n_name")
        .sqlBaselineQuery(
            "select r.r_regionkey, r.r_name, r.r_comment from cp.\"tpch/nation.parquet\" n, cp.\"tpch/region.parquet\" r where n.n_regionkey = r.r_regionkey order by n.n_name")
        .build()
        .run();
  }
  ...
  public void testSelStarRegColConstJoin() throws Exception {
    testBuilder()
        .ordered()
        .sqlQuery(
            "select *, n.n_nationkey as n_nationkey0, 1 + 2 as constant from cp.\"tpch/nation.parquet\" n, cp.\"tpch/region.parquet\" r where n.n_regionkey = r.r_regionkey order by n.n_name")
        .sqlBaselineQuery(
  ...
  public void testSelStarBothSideJoin() throws Exception {
    testBuilder()
        .unOrdered()
        .sqlQuery(
            "select n.*, r.* from cp.\"tpch/nation.parquet\" n, cp.\"tpch/region.parquet\" r where n.n_regionkey = r.r_regionkey")
        .sqlBaselineQuery(
            "select n.n_nationkey,n.n_name,n.n_regionkey,n.n_comment,r.r_regionkey,r.r_name,r.r_comment from cp.\"tpch/nation.parquet\" n, cp.\"tpch/region.parquet\" r where n.n_regionkey = r.r_regionkey order by n.n_name")
        .build()
        .run();
  ...
  public void testStarView1() throws Exception {
    try {
      ...
      test("use dfs_test");
      test(
          "create view vt1 as select * from cp.\"tpch/region.parquet\" r, cp.\"tpch/nation.parquet\" n where r.r_regionkey = n.n_regionkey");
  ...
  public void testSelStarSubQSchemaTable() throws Exception {
    test(
        "select CATALOG_NAME, CATALOG_DESCRIPTION from (select * from INFORMATION_SCHEMA.CATALOGS);");
  }
  ...
  public void testSelStarJoinSchemaWithSchemaLess() throws Exception {
    String query =
        "select t1.CATALOG_NAME, t1.CATALOG_DESCRIPTION, t2.n_nationkey from "
  ...
  @Test(
      expected =
          UserException
              .class) // Should get "At line 1, column 8: Column 'n_nationkey' is ambiguous"
  @Ignore("this no longer happens")
  public void testSelStarAmbiguousJoin() throws Exception {
    try {
      test(
          "select x.n_nationkey, x.n_name, x.n_regionkey, x.r_name from (select * from cp.\"tpch/nation.parquet\" n, cp.\"tpch/region.parquet\" r where n.n_regionkey = r.r_regionkey) x ");
  ...
  @Test // Select * in SubQuery : regular columns appear in select clause, where, group by, order
  // by.
  public void testSelStarSubQPrefix() throws Exception {
    test(
        "select t.n_nationkey, t.n_name, t.n_regionkey from (select * from cp.\"tpch/nation.parquet\") t where t.n_regionkey > 1 order by t.n_name");

    test(
        "select n.n_regionkey, count(*) as cnt from ( select * from ( select * from cp.\"tpch/nation.parquet\") t where t.n_nationkey < 10 ) n where n.n_nationkey >1 group by n.n_regionkey order by n.n_regionkey ; ");

    test(
        "select t.n_regionkey, count(*) as cnt from (select * from cp.\"tpch/nation.parquet\") t where t.n_nationkey > 1 group by t.n_regionkey order by t.n_regionkey;");
  }
  ...
  public void testSelStarSubQNoPrefix() throws Exception {
    ...
    test(
        "select n_regionkey, count(*) as cnt from ( select * from ( select * from cp.\"tpch/nation.parquet\")  where n_nationkey < 10 ) where n_nationkey >1 group by n_regionkey order by n_regionkey ; ");

    test(
        "select n_regionkey, count(*) as cnt from (select * from cp.\"tpch/nation.parquet\") t where n_nationkey > 1 group by n_regionkey order by n_regionkey;");
  }

  @Test // join two SubQuery, each having select * : regular columns appear in the select , where
  // and on clause, group by, order by.
  ...
  @Test // DRILL-595 : Select * in CTE WithClause : regular columns appear in select clause, where,
  // group by, order by.
  public void testDRILL_595WithClause() throws Exception {
    test(
        " with x as (select * from cp.\"region.json\") \n"
    ...
    test(
        " with x as (select * from cp.\"tpch/nation.parquet\") \n"
  ...
  @Test // DRILL-595 : Join two CTE, each having select * : regular columns appear in the select ,
  // where and on clause, group by, order by.
  public void testDRILL_595WithClauseJoin() throws Exception {
    test(
        "with n as (select * from cp.\"tpch/nation.parquet\"), \n "
  ...
  public void testStarWithOtherExpression() throws Exception {
    ...
    testBuilder()
        ...
        .sqlBaselineQuery(
            "select n_comment, n_name, n_nationkey, n_regionkey, n_nationkey + 5 as myexpr from cp.\"tpch/nation.parquet\" order by n_nationkey limit 3")
        .build()
        .run();
  ...
  public void testStarPartitionFilterOrderBy() throws Exception {
    try {
      ...
      String query =
          String.format(
              "select * from dfs.\"%s/multilevel/parquet\" where dir0=1994 and dir1='Q1' order by dir0 limit 1",
              TEST_RES_PATH);
      org.joda.time.LocalDateTime mydate =
          new org.joda.time.LocalDateTime("1994-01-20T00:00:00.000");
  ...
  public void testCaseSenJoinCTEWithRegTab() throws Exception {
    final String query1 =
        "with a as ( select * from cp.\"tpch/nation.parquet\" ) select * from a, cp.\"tpch/region.parquet\" b where a.N_REGIONKEY = b.R_REGIONKEY";

    int actualRecordCount = testSql(query1);
    int expectedRecordCount = 25;
    assertEquals(
        String.format(
            "Received unexpected number of rows in output for query:\n%s\n expected=%d, received=%s",
            query1, expectedRecordCount, actualRecordCount),
        expectedRecordCount,
        actualRecordCount);
    ...
    assertEquals(
        String.format(
            "Received unexpected number of rows in output for query:\n%s\n expected=%d, received=%s",
            query2, expectedRecordCount, actualRecordCount),
        expectedRecordCount,
        actualRecordCount);
  }
}
