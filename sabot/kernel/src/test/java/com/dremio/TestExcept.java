...
public class TestExcept extends BaseTestQuery {
  private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(TestExcept.class);
  ...
  public void testExcept1ColFromScan() throws Exception {
    String query = "(select n_name as name from cp.\"tpch/nation.parquet\") EXCEPT (select r_name as name from cp.\"tpch/region.parquet\")";
    testBuilder()
      .sqlQuery(query)
      .unOrdered()
  ...
  public void testExceptFromScan() throws Exception {
    String query = "(select n_name as name, n_nationkey as key from cp.\"tpch/nation.parquet\") EXCEPT (select r_name as name, r_regionkey as key from cp.\"tpch/region.parquet\")";
    testBuilder()
      .sqlQuery(query)
      .unOrdered()
