...
public class TestDecimalQueries extends BaseTestQuery {
  ...
  public void testDecimalMixedTypes() throws Exception {
    ...
    final String query =
        "SELECT avg(distinct cast(a as decimal (2,1))) FROM cp"
  ...
  public void testCastDecimalFloat() throws Exception {
    ...
    final String query =
        "select cast(val as float) from cp"
  ...
  public void testJoinFloatDecimal() throws Exception {
    ...
    testBuilder().sqlQuery(query).unOrdered().baselineColumns("EXPR$0").baselineValues(0L).go();
  ...
  public void testDecimalMisc() throws Exception {
    ...
    testBuilder()
        ...
        .baselineValues(
            new BigDecimal("123.12345678901234567890"), new BigDecimal("123.12345678901234567890"))
        .go();
  }
  ...
  public void testDecimalDivideParquetLiteral() throws Exception {
    final String query =
        "select c_bigint / cast(1440.00 as decimal(7,2)) from cp.\"parquet/decimals/j6.parquet\" limit 2";
  ...
  public void testDecimalLiteralPrecisionScale() throws Exception {
    ...
    final String query =
        "SELECT ({fn TIMESTAMPDIFF(SQL_TSI_DAY,{d '1900-01-01'},CAST(\"Calcs\".\"datetime0\" AS DATE))} "
  ...
  public void testDecimalInComplexTypes() throws Exception {
    ...
    final String query2 =
        "select * from (select flatten(places_lived) places from cp.\"decimal/decimal_complex.parquet\") tab where tab.places.place='test'";
    final String query3 =
        "select places_lived[0].decimal_array[0] from cp.\"decimal/decimal_complex.parquet\" where places_lived[0].decimal_array[0]=cast('579.29' as decimal(5,2))";
  ...
  public void testRound() throws Exception {
    final String query =
        "select truncate(cast(val as decimal(10,7)), 2) from cp"
  ...
  public void testLog2() throws Exception {
    ...
    testBuilder().sqlQuery(query).ordered().baselineColumns("log_result").baselineValues(4.0).go();

    query = "select log2(1) as log_result";
    testBuilder().sqlQuery(query).ordered().baselineColumns("log_result").baselineValues(0.0).go();
  }
}
