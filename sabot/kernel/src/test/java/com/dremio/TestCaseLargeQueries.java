...
public class TestCaseLargeQueries extends PlanTestBase {
  ...
  public static void setup() throws Exception {
    ...
    testNoResult("ALTER SESSION SET \"%s\" = false", ExecConstants.GANDIVA_OPTIMIZE.getOptionName());
  }
  ...
  private String projectLargeOrderCase(int level1Size, int level2Size) {
    ...
    for (int i = level1Size; i > 0; i--) {
      ...
      final String whenExpr = (i % 3 == 0) ? "sqrt(o_totalprice) >= " : "castINT(o_totalprice / 100.0) >= ";
      sb.append("WHEN ").append(whenExpr).append(limit)
        .append(" THEN ").append(getNextOrderClause(level2Size, i))
        .append(System.lineSeparator());
    }
  ...
  private String getNextOrderClause(int level2Size, int level1Idx) {
    ...
    for (int i = level2Size; i > 0; i--) {
      final String whenExpr = (i % 10 == 0) ? "(castINT(sqrt(o_totalprice))) > " + i :
        (i % 5 == 0) ? "regexp_matches(o_comment, '.*(quick|haggle|foxes).*') " +
          "OR regexp_matches(LOWER(o_comment), '.*(deposit).*') " +
          "OR regexp_matches(LOWER(o_comment), '.*(ideas).*')"
          : (i % 2 == 0) ? "o_orderstatus IN ('1-URGENT', '2-HIGH')" : "o_totalprice / 1000 > " + i;
      sb.append("WHEN ").append(whenExpr).append(" THEN ").append("lower(concat(o_clerk,'AA")
        .append(level1Idx * i).append("'))")
        .append(System.lineSeparator());
    }
