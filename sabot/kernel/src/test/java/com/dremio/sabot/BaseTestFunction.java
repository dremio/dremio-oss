...
public class BaseTestFunction extends BaseTestOperator {
  ...
  /**
   * Evaluate the given expression using the provided inputs and confirm it results in the expected
   * output. Runs both compiled and interpreted tests.
   *
   * @param expr The expression to evaluate Note that the expression must refer to all the inputs as
   *     c0, c1, c2... cn at least once.
  ...
  /**
   * Evaluate the given expression only in compiled mode. Used for functions that are not supported
   * in interpreted mode for e.g. gandiva only functions.
   */
  ...
  private void testFunctionInner(
      String stringExpression, boolean runInterpretedMode, Object[] fieldsArr) {
    try {
      ...
      Preconditions.checkArgument(
          collector.paths.size() == inputs.size(),
          "Expected number of fields referenced (%s) to equal number of inputs passed (%s).",
          collector.paths.size(),
          inputs.size());
      ...
      for (int i = 0; i < inputs.size(); i++) {
        ...
        Preconditions.checkArgument(
            collector.paths.contains(name), "Expected path (%s) not referenced.", name);
        names[i] = name;
      }

      final Table input =
          Fixtures.t(Fixtures.th(names), Fixtures.tr(inputs.toArray(new Object[inputs.size()])));
      final Table output =
          Fixtures.t(Fixtures.th("out"), Fixtures.tr(fieldsArr[fieldsArr.length - 1]));
      Project p =
          new Project(
              OpProps.prototype(),
              null,
              Arrays.asList(new NamedExpression(expr, new FieldReference("out"))));

      try {
        validateSingle(
            p, ProjectOperator.class, input.toGenerator(getTestAllocator()), output, DEFAULT_BATCH);
      ...
      if (runInterpretedMode) {
        try {
          ...
          throw new RuntimeException(
              "Failure while testing function using code interpretation.", e);
        }
      }
      ...
      throw new RuntimeException(
          String.format(
              "Failure while attempting to evaluate expr [%s] using inputs/outputs of %s.",
              stringExpression, Arrays.toString(fieldsArr)),
          e);
    }
  }

  private void testInterp(Project project, LogicalExpression expr, Table input, Table expected)
      throws Exception {
    ...
    try (OperatorContextImpl context =
      ...
      LogicalExpression materializedExpr =
          ExpressionTreeMaterializer.materializeAndCheckErrors(
              expr, generator.getOutput().getSchema(), testContext.getFunctionLookupContext());
      ...
      InterpreterEvaluator.evaluate(
          generator.getOutput(), context.getFunctionContext(), vector, materializedExpr);
  ...
  public class FieldExpressionCollector extends AbstractExprVisitor<Void, Void, RuntimeException> {
    ...
    @Override
    public Void visitCaseExpression(CaseExpression caseExpression, Void value)
