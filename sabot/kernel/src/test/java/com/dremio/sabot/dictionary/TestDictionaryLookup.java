...
public class TestDictionaryLookup extends BaseTestOperator {
  ...
  public void testDictionaryLookup() throws Throwable {
    ...
    try (final VectorContainer dict1 = new VectorContainer(getTestAllocator());
      ...
      final NullableBigIntVector longVector = dict1.addOrGet(field1);
      longVector.allocateNew();
      longVector.getMutator().setSafe(0, 10L);
      longVector.getMutator().setSafe(1, 20L);
      longVector.getMutator().setSafe(2, 30L);
      longVector.getMutator().setSafe(3, 40L);
      longVector.getMutator().setSafe(4, 50L);
      longVector.getMutator().setValueCount(5);
      ...
      final NullableVarBinaryVector binaryVector = dict2.addOrGet(field2);
      binaryVector.allocateNew();
      binaryVector.getMutator().setSafe(0, "abc".getBytes(UTF8), 0, 3);
      binaryVector.getMutator().setSafe(1, "bcd".getBytes(UTF8), 0, 3);
      binaryVector.getMutator().setSafe(2, "cde".getBytes(UTF8), 0, 3);
      binaryVector.getMutator().setSafe(3, "def".getBytes(UTF8), 0, 3);
      binaryVector.getMutator().setSafe(4, "efg".getBytes(UTF8), 0, 3);
      binaryVector.getMutator().setValueCount(5);
      ...
      final NullableFloat8Vector doubleVector = dict3.addOrGet(field3);
      doubleVector.allocateNew();
      doubleVector.getMutator().setSafe(0, 100.1);
      doubleVector.getMutator().setSafe(1, 200.2);
      doubleVector.getMutator().setSafe(2, 300.3);
      doubleVector.getMutator().setSafe(3, 400.4);
      doubleVector.getMutator().setSafe(4, 500.5);
      doubleVector.getMutator().setValueCount(5);
