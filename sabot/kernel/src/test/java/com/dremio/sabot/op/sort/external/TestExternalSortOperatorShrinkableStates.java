...
public class TestExternalSortOperatorShrinkableStates extends BaseTestOperator {
  ...
  private static final GenerationDefinition.TpchTable TABLE = GenerationDefinition.TpchTable.CUSTOMER;
  private static final double SCALE = 1;


  private ExternalSort getRegionSort(){
    return new ExternalSort(PROPS, null, Collections.singletonList(ordering("c_custkey", RelFieldCollation.Direction.ASCENDING, RelFieldCollation.NullDirection.FIRST)), false);
  }
  ...
  public void checkShrinkableMemory() throws Exception {
    ...
    for (int sortOperatorType = 0; sortOperatorType < 2; sortOperatorType++) {
      ...
      try (AutoCloseable option = with(ExecConstants.EXTERNAL_SORT_ENABLE_SPLAY_SORT, sortOperatorType != 0);
           ExternalSortOperator sortOperator = newOperator(ExternalSortOperator.class, getRegionSort(), 4095);
           TpchGenerator tableGenerator = TpchGenerator.singleGenerator(TABLE, SCALE, getTestAllocator())) {
        ...
        sortOperator.setStates(SingleInputOperator.State.CAN_PRODUCE, ExternalSortOperator.SortState.SPILL_IN_PROGRESS);
        long shrinkableMemory = sortOperator.shrinkableMemory();
        //Shrinkable memory is returned > 0 as operator is allowed to shrink in SPILL_IN_PROGRESS state
        assertTrue(shrinkableMemory > 0L);

        sortOperator.setStates(SingleInputOperator.State.CAN_PRODUCE, ExternalSortOperator.SortState.COPY_FROM_DISK);
        shrinkableMemory = sortOperator.shrinkableMemory();
        //Shrinkable memory is returned as 0 as operator is not allowed to shrink in COPY_FROM_DISK state
        assertEquals (0L , shrinkableMemory);

        sortOperator.setStates(SingleInputOperator.State.CAN_PRODUCE, ExternalSortOperator.SortState.COPY_FROM_MEMORY);
        shrinkableMemory = sortOperator.shrinkableMemory();
        //Shrinkable memory is returned > 0 as operator is allowed to shrink in COPY_FROM_MEMORY state
        assertTrue(shrinkableMemory > 0L);
        ...
        sortOperator.setStates(SingleInputOperator.State.CAN_PRODUCE, ExternalSortOperator.SortState.CONSOLIDATE);
        shrinkableMemory = sortOperator.shrinkableMemory();
  ...
  public void checkShrinkMemoryCommandInVariousStates() throws Exception {
    for (int sortOperatorType = 0; sortOperatorType < 2; sortOperatorType++) {
      try (AutoCloseable option = with(ExecConstants.EXTERNAL_SORT_ENABLE_SPLAY_SORT, sortOperatorType != 0);
        ...
        o.setStates(SingleInputOperator.State.CAN_PRODUCE, ExternalSortOperator.SortState.COPY_FROM_DISK);
        assertEquals(0L, o.shrinkableMemory());
        //Cannot shrink in COPY_FROM_DISK state but function still returns 0 as shrinkable memory is returned 0 in previous state
  ...
  public void testSorterShrinkMemoryFromCopier() throws Exception{
    int total = 10000;
    for(int recordInCopier = 0; recordInCopier <= 10000; recordInCopier += 1000){
  ...
  private void sorterShrinkMemoryFromCopier(int recordInCopier, int recordOnDisk) throws Exception{
    ...
    try (AutoCloseable option = with(ExecConstants.EXTERNAL_SORT_ENABLE_SPLAY_SORT, false)) {
      ...
      try (
        ExternalSortOperator sorter = newOperator(ExternalSortOperator.class, sort, batchSize);
        Generator closeable = generator;) {
        final VectorAccessible output = sorter.setup(generator.getOutput());
        int count = 0;

        // read all 10K record into sorter's memory. 10 batches, each has 1000 records.
  ...
  public void testSorterShrinkMemory() throws Exception{
    ...
    // without 'shrink memory' request, 'small input' will not trigger disk spill but 'large input' will.
    // below for loop will inject 'shrink memory' request at different places for those two scenarios.
  ...
  private void sorterShrinkMemory(int expectedRecordCount, boolean[] pos) throws Exception {
    class Simulator{
      ...
      // twice in a row.
      void InsertShrinkMemoryRequest(ExternalSortOperator sorter, int cur, boolean[] pos) throws Exception {
        if(!pos[cur]){
          return;
        }
    ...
    try (AutoCloseable option = with(ExecConstants.EXTERNAL_SORT_ENABLE_SPLAY_SORT, false)) {
      ...
      try(
        ...
        while(sorter.getState() != State.DONE && (count = generator.next(batchSize)) != 0){
          ...
          sorter.consumeData(count);
          // if spill happens, sorter's state becomes CAN_PRODUCE (plus internal sortState is SPILL_IN_PROGRESS)
          while(sorter.getState() == State.CAN_PRODUCE){
