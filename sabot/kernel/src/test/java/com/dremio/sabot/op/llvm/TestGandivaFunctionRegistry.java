...
public class TestGandivaFunctionRegistry extends ExecTest {
  ...
  public void testGandivaRegistry() throws Exception {
    ...
    GandivaFunctionHolder holder =
        (GandivaFunctionHolder)
            resolver.getBestMatch(fnRegistry.lookupMethods(fnCall.getName()), fnCall);
  ...
  public void testFunctionImplementationRegistry() {
    ...
    GandivaFunctionHolder holder =
        (GandivaFunctionHolder) fnRegistry.findExactFunction(getDecimalSameFn(), true);
    Assert.assertNotNull(holder);

    // test that gandiva functions are not used where they are not intended.
    holder = (GandivaFunctionHolder) fnRegistry.findExactFunction(getDecimalSameFn(), false);
  ...
  private FunctionCall getGandivaOnlyFn() {
    List<LogicalExpression> args =
        Lists.newArrayList(ValueExpressions.getChar("test"), ValueExpressions.getChar("tes"));
