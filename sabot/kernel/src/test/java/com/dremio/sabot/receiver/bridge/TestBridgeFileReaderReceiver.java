...
public class TestBridgeFileReaderReceiver extends BaseTestOperator {
  ...
  private void check(
      Fixtures.Table expected, int batchSize, Generator generator, boolean interleaved)
      throws Exception {
  ...
  private void checkSerialized(
    ...
    try (BridgeFileWriterSenderOperator senderOp =
        newOperator(
      ...
      RawFragmentBatchProvider[] batchBufferProvider = {
        new BatchBufferFromFilesProvider(
            senderOp.getSpillManager().getId(),
            0,
            resourceManager.getGroup("test"),
            getTestAllocator(),
            fileCursorManagerFactory)
      };
      try (BridgeFileReaderReceiverOperator receiverOp =
          newOperator(
              BridgeFileReaderReceiverOperator.class, receiver, batchSize, batchBufferProvider)) {
        int numRecords;
