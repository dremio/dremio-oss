...
import com.dremio.sabot.aggregate.hash.TestSpillingHashAgg;
import com.dremio.sabot.op.spi.SingleInputOperator;

public class TestVectorizedHashAggShrinkableStates extends TestSpillingHashAgg {
  ...
  public void testOutputSpilling() throws Exception {
    ...
    try (AutoCloseable useSpillingAgg = with(VectorizedHashAggOperator.VECTORIZED_HASHAGG_USE_SPILLING_OPERATOR, true);
         CustomHashAggDataGenerator generator = new CustomHashAggDataGenerator(10000, getTestAllocator(), shortLen);
         VectorizedHashAggOperator op = newOperator(VectorizedHashAggOperator.class, agg, 1000)) {
          ...
          while(op.getState() != SingleInputOperator.State.DONE && (count = generator.next(batchSize)) != 0) {
          ...
          while (op.getInternalStateMachine() != VectorizedHashAggOperator.InternalState.OUTPUT_INMEMORY_PARTITIONS) {
            op.outputData();
          }
          ...
          while (op.getInternalStateMachine() == VectorizedHashAggOperator.InternalState.SPILL_NEXT_BATCH) {
            op.outputData();
          }
