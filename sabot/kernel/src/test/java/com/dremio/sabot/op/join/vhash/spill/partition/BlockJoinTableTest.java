...
public class BlockJoinTableTest {
  ...
  @Rule public final AllocatorRule defaultAllocatorRule = AllocatorRule.defaultAllocator();

  @Before
  public void setupBeforeTest() {
    defaultAllocator =
        defaultAllocatorRule.newAllocator("test-block_join_table", 0, Long.MAX_VALUE);
  ...
  public void testInsertingMoreRecordsThanCapacityToNonSpillingHashTable() throws Exception {
    ...
    ExecProtos.FragmentHandle fragmentHandle =
        ExecProtos.FragmentHandle.newBuilder().setMinorFragmentId(1).build();
    ...
    when(mockedHashTable.addSv2(
            ArgumentMatchers.any(),
            anyInt(),
            anyInt(),
            ArgumentMatchers.any(ArrowBuf.class),
            ArgumentMatchers.any(ArrowBuf.class),
            ArgumentMatchers.any(ArrowBuf.class),
            ArgumentMatchers.any(ArrowBuf.class)))
        .thenReturn(3);

    doReturn(mockedHashTableFactory)
        .when(sabotConfig)
        .getInstance(
            anyString(),
            ArgumentMatchers.<Class<HashTableFactory>>any(),
    ...
    try (BlockJoinTable table =
      ...
      final int inserted =
          table.insertPivoted(
              null, 0, 10, tableHash4B, fixedBlockVector, variableBlockVector, output);
  ...
  private OperatorContext mockOpContext(
      ExecProtos.FragmentHandle fragmentHandle, BufferAllocator allocator) {
    ...
    CoordExecRPC.FragmentAssignment assignment =
        CoordExecRPC.FragmentAssignment.newBuilder()
