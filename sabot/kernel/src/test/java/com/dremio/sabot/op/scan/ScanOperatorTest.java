...
public class ScanOperatorTest {
  ...
  public void testWorkOnOOBRuntimeFilter() throws Exception {
    ...
    ScanOperator scanOp =
        new ScanOperator(
            fec, getConfig(), getMockContext(), RecordReaderIterator.from(mockReader), null, null);
    ...
    ArgumentCaptor<com.dremio.exec.store.RuntimeFilter> addedFilter =
    ...
    try (ArrowBuf oobMessageBuf = testAllocator.buffer(128)) {
      ...
      ScanOperator scanOp =
          new ScanOperator(
              ...
              getConfig(),
              getMockContext(),
              RecordReaderIterator.from(mockReader),
              null,
              null);
      scanOp.workOnOOB(msg1);
      Arrays.stream(msg1.getBuffers()).forEach(ArrowBuf::close);
