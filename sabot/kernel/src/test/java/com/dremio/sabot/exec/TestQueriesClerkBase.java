 ...
 *
 * <p>Owns the test allocator, the root test allocator, rules for creation/destruction of the root
 * test allocator, etc.
 */
public class TestQueriesClerkBase {
  protected BufferAllocator
      mockedRootAlloc; // a mock-up of a buffer allocator, useful only for counting children
  ...
  protected PlanFragmentFull getDummyPlan(
      UserBitShared.QueryId queryId, int majorFragmentId, int minorFragmentId) {
    ...
    CoordExecRPC.FragmentPriority priority =
        CoordExecRPC.FragmentPriority.newBuilder()
            .setWorkloadClass(UserBitShared.WorkloadClass.GENERAL)
            .build();

    return new PlanFragmentFull(
        CoordExecRPC.PlanFragmentMajor.newBuilder().setHandle(handle).setPriority(priority).build(),
        CoordExecRPC.PlanFragmentMinor.newBuilder().setMinorFragmentId(minorFragmentId).build());
  }

  protected CoordExecRPC.SchedulingInfo getDummySchedulingInfo() {
    return CoordExecRPC.SchedulingInfo.newBuilder()
        .setQueueId("unused")
        .setWorkloadClass(UserBitShared.WorkloadClass.GENERAL)
        .build();
  }
  ...
  /**
   * Count the number of major fragments running (Note: phases are easy to count. Fragments, on the
   * other hand, are not -- fragments are not tracked within the phase ticket, so counting them is
   * impossible)
   */
  protected void assertLivePhasesCount(QueriesClerk clerk, int expectedValue) {
    ...
    for (QueryTicket c : queryTickets) {
      assertTrue(
          String.format(
              "Fewer refcounts (%d) than active phase tickets (%d)",
  ...
  public final class TestAllocator implements BufferAllocator {
    ...
    public BufferAllocator newChildAllocator(
        String childName, AllocationListener listener, long dummy1, long limit) {
  ...
  /**
   * Used as a callback to the clerk's "build and start query" -- whose only job in this test is to
   * actually get the query ticket
   */
