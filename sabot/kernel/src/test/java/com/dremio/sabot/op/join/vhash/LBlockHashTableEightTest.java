...
public class LBlockHashTableEightTest {
    ...
    public void testPrepareBloomFilter() throws Exception {
        ...
        try (ArrowBuf keyBuf = testAllocator.buffer(9);
            ...
            dataSet.stream().forEach(key -> assertTrue(bloomFilterOptional.get().mightContain(writeKey(keyBuf, key), 9)));
            Set<Long> differentData = generatedData(100);
            long fpCount = differentData.stream()
                    .filter(key -> !dataSet.contains(key))
                    .filter(key -> bloomFilterOptional.get().mightContain(writeKey(keyBuf, key), 9)).count();
