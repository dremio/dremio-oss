...
public class TestRoundRobinSender extends BaseTestOperator {
  ...
  public void checkMemoryLeak() throws Exception {
    ...
    RoundRobinSender sender =
        new RoundRobinSender(
            PROPS,
            getSchema(),
            null,
            1,
    ...
    try (RoundRobinOperator op =
            newOperator(RoundRobinOperator.class, sender, DEFAULT_BATCH, endpointsIndex, provider);
        TpchGenerator g =
            TpchGenerator.singleGenerator(TpchTable.NATION, 0.1, getTestAllocator()); ) {
  ...
  public BatchSchema getSchema() {
    SchemaBuilder builder =
        BatchSchema.newBuilder()
            .addField(
                new Field(
                    "n_nationKey", new FieldType(true, MinorType.BIGINT.getType(), null), null))
            .addField(
                new Field("n_name", new FieldType(true, MinorType.VARCHAR.getType(), null), null))
            .addField(
                new Field(
                    "n_regionKey", new FieldType(true, MinorType.BIGINT.getType(), null), null))
            .addField(
                new Field(
                    "n_comment", new FieldType(true, MinorType.VARCHAR.getType(), null), null));
