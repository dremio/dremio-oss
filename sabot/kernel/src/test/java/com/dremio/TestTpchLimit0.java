...
public class TestTpchLimit0 extends BaseTestQuery {
  ...
  private void testLimitZeroQuery(String query) throws Exception {
    query =
        "ALTER SESSION SET \"planner.slice_target\" = 1; select * from \n("
  ...
  public void tpch15() throws Exception {
    try {
      ...
      // NB: can't use query 15 directly, as it has four distinct parts. testLimitZero(), above,
      // assumes a single query
      test("use dfs_test");
      try {
        test(
