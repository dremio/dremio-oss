...
public class TestSchemaChange extends BaseTestQuery {
  ...
  public void testMultiFilesWithDifferentSchema() throws Exception {
    ...
    try {
      final String query =
          String.format(
              "select * from dfs_root.\"%s/schemachange/multi/\" order by id", TEST_RES_PATH);
      test(query);
    } finally {
  ...
  public void testNewNestedColumn() throws Exception {
    ...
    try {
      final String query =
          String.format(
              "select a from dfs_root.\"%s/schemachange/nested/\" order by id", TEST_RES_PATH);
      testBuilder()
          .sqlQuery(query)
          .ordered()
          .baselineColumns("a")
  ...
  public void keepLearningSchemaAcrossFiles() throws Exception {
    ...
    try {
      final String query =
          String.format(
              "select * from dfs_root.\"%s/schemachange/differentschemas/\"", TEST_RES_PATH);
      try {
        testBuilder()
            .sqlQuery(query)
            .unOrdered()
        ...
        // first attempt may fail as a batch may have been sent to the user, so if it does, this
        // must be the message ..
  ...
  public void testOldAndNewSchemaVisibleOnSchemaChange() throws Exception {
    ...
    try {
      final String query =
          String.format("select * from dfs_root.\"%s/schemachange/schemacontext/\"", TEST_RES_PATH);
      test(query);
    } catch (Exception e) {
      assertTrue(e.getMessage().contains("New schema found. Please reattempt the query."));
  ...
  public void testParquetStructExtraField() throws Exception {
    ...
    String query =
        String.format("select * from dfs_root.\"%s/struct_extra_hdfs_test/\"", TEST_RES_PATH);
    try {
      test(query);
    } catch (Exception e) {
      // first attempt fails
    ...
    query =
        String.format(
            "select col1, col2[0].f1 as f1, col2[0].f2.f_f_1 as ff1, col2[0].f2.f_f_2 as ff2 "
                + "from dfs_root.\"%s/struct_extra_hdfs_test/\"",
            TEST_RES_PATH);
    testBuilder()
        .sqlQuery(query)
        .unOrdered()
    ...
    query =
        String.format(
            "select col1, col2[0].f1 as f1, col2[0].f2.f_f_1 as ff1, col2[0].f2.f_f_3 as ff3 "
                + "from dfs_root.\"%s/struct_extra_hdfs_test/\"",
            TEST_RES_PATH);
    testBuilder()
        .sqlQuery(query)
