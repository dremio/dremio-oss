...
public class GroupingTest extends BaseTestQuery {
  private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(GroupingTest.class);
  ...
  public void testGroupingWithRollup() throws Exception {
    String query = "select n_regionkey, count(*), GROUPING(n_regionkey) from cp.\"tpch/nation.parquet\" group by rollup (n_regionkey);";
  ...
  public void testGroupingWithoutRollup() throws Exception {
    String query = "select n_regionkey, count(*) as cnt , GROUPING(n_regionkey) from cp.\"tpch/nation.parquet\" group by (n_regionkey);";
  ...
  public void testGroupingInFilterWithRollup() throws Exception {
    String query = "select n_regionkey, count(*) from cp.\"tpch/nation.parquet\" group by rollup(n_regionkey) HAVING GROUPING(n_regionkey) = 1";
  ...
  public void testGroupingInFilterWithoutRollup() throws Exception {
    String query = "select n_regionkey, count(*) from cp.\"tpch/nation.parquet\" group by n_regionkey HAVING GROUPING(n_regionkey) = 1";
  ...
  public void testGroupingID1() throws Exception {
    String query = "select n_regionkey, count(*), GROUPING_ID(n_regionkey) as GP_Region from cp.\"tpch/nation.parquet\" group by rollup(n_regionkey);";
