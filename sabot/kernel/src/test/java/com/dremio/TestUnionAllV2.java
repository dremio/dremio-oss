...
public class TestUnionAllV2 extends BaseTestQuery{
  private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(TestUnionAllV2.class);
  ...
  @BeforeClass
  public static void continueIfUnlimitedSplits() {
    assumeTrue(MetadataRefreshUtils.unlimitedSplitsSupportEnabled(getSabotContext().getOptionManager()));
  }
  ...
  public void testFilterPushDownOverUnionAll() throws Exception {
    ...
    // Validate the plan
    final String[] expectedPlan = {"TableFunction.*columns=\\[`n_regionkey`\\]", "TableFunction.*columns=\\[`r_regionkey`\\]"};
  ...
  @Test  // Union-All with Limit0
  public void testUnionAllWithLimit0() throws Exception {
    String query = "select * from (\n" +
      "    select\n" +
      "        count(n_regionkey) as cnt, n_nationkey\n" +
      "    from (\n" +
      "        select\n" +
      ...
      "        select\n" +
      "            n_nationkey, n_regionkey\n" +
      "        from cp.\"tpch/nation.parquet\") tmp group by tmp.n_nationkey) t limit 0";

    final List<Pair<SchemaPath, MajorType>> expectedSchema = Lists.newArrayList();
    expectedSchema.add(Pair.of(SchemaPath.getSimplePath("cnt"), Types.optional(MinorType.BIGINT)));
    expectedSchema.add(Pair.of(SchemaPath.getSimplePath("n_nationkey"), Types.optional(MinorType.INT)));
