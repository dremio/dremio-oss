...
public class DistributionLoader {
  public static <R extends Readable & Closeable> Map<String, Distribution> loadDistribution(
      CharSource input) throws IOException {
    Iterator<String> iterator =
        filter(
            input.readLines().iterator(),
  ...
  private static Distribution loadDistribution(Iterator<String> lines, String name) {
    ...
    while (lines.hasNext()) {
      ...
      List<String> parts =
          ImmutableList.copyOf(Splitter.on('|').trimResults().omitEmptyStrings().split(line));
      checkState(
          parts.size() == 2,
  ...
  private static boolean isEnd(String name, String line) {
    List<String> parts =
        ImmutableList.copyOf(Splitter.on(CharMatcher.whitespace()).omitEmptyStrings().split(line));
    if (parts.get(0).equalsIgnoreCase("END")) {
      checkState(
          parts.size() == 2 && parts.get(1).equalsIgnoreCase(name),
          "Expected end statement be 'END %s', but was '%s'",
          name,
          line);
      return true;
    }
    ...
    while (lines.hasNext()) {
      ...
      List<String> parts =
          ImmutableList.copyOf(
              Splitter.on(CharMatcher.whitespace()).omitEmptyStrings().split(line));
      if (parts.size() != 2) {
        continue;
      }
