...
class RexDeserializer {
  ...
  private RexNode convertSubQuery(PRexSubQuery subQuery) {
    ...
    PRelList list = PRelList.newBuilder()
      ...
      .build();
    final RelNode rel = RelDeserializer.deserialize(
      registry,
  ...
  private RexNode convertFieldAccess(PRexFieldAccess fieldAccess) {
    return rexBuilder.makeFieldAccess(
      convert(fieldAccess.getExpression()),
  ...
  private RexNode convertOver(PRexOver over) {
    ...
    List<RexNode> partitionKeys = window
      .getPartitionKeysList()
      .stream()
      .map(p -> this.convert(p))
      .collect(Collectors.toList());
    List<RexFieldCollation> orderKeys = window
      .getOrderKeysList()
      .stream()
      .map(this::convert)
      .collect(Collectors.toList());
  ...
  private RexWindowBound convert(PRexWindowBound bound) {
    ...
    case UNBOUNDED:
      sqlNode = bound.getUnbounded().getBoundOption() == PBoundOption.FOLLOWING
  ...
  private RexNode convertInputRef(PRexInputRef inputRef, RelDataType rowType) {
    ...
    final RelDataType dataType = rowType == null
  ...
  private static void must(RelDataType actual, SqlTypeName... expected) {
    ...
    throw new IllegalStateException(String.format("Unexpected type. Expected one of types %s but type was actually %s", Arrays.toString(expected), actual));
  }
  ...
  private RexNode convertLiteral(PRexLiteral literal) {
    ...
    case BINARY_VALUE:
      ...
      return rexBuilder.makeBinaryLiteral(new org.apache.calcite.avatica.util.ByteString(literal.getBinaryValue().toByteArray()));
    case BOOLEAN_VALUE:
      must(type, SqlTypeName.BOOLEAN);
      return rexBuilder.makeLiteral(literal.getBooleanValue());
    case DECIMAL_VALUE:
      BigInteger bi = new BigInteger(literal.getDecimalValue().getTwosComplementValue().toByteArray());
    ...
    case LONG_VALUE:
      ...
      case DATE:
        return rexBuilder.makeDateLiteral(DateString.fromDaysSinceEpoch((int) (literal.getLongValue())));
      case TIMESTAMP:
        return rexBuilder.makeTimestampLiteral(TimestampString.fromMillisSinceEpoch(literal.getLongValue()), 3);
      case TIME:
        return rexBuilder.makeTimeLiteral(TimeString.fromMillisOfDay((int) literal.getLongValue()), type.getPrecision());
      default:
        break;
      }
      break;
    case STRING_VALUE:
      ...
      case VARCHAR:
        try {
          ...
          if (!((type.getSqlTypeName() == SqlTypeName.CHAR) || (type.getSqlTypeName() == SqlTypeName.VARCHAR))) {
            throw ce;
          }

          NlsString utf8NlsString = new NlsString(
            literal.getStringValue(),
            "UTF-8", Utf8SqlCollation);
          RelDataType utf8RelDataType = this.types.getFactory().createTypeWithCharsetAndCollation(
            type,
  ...
  private RexNode convertDynamicParam(PRexDynamicParam dynamicParam) {
    return new RexDynamicParam(this.types.fromProto(dynamicParam.getRexVariable().getType()), dynamicParam.getIndex());
  }

  private RexNode convertRangeRef(PRexRangeRef rangeRef) {
    return rexBuilder.makeRangeReference(
      this.types.fromProto(rangeRef.getType()),
      rangeRef.getOffset(),
      false);
  }
