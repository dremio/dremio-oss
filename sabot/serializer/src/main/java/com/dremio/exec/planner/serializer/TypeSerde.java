...
public final class TypeSerde {
  ...
  public PRelDataType toProto(RelDataType type) {
    ...
    final PRelDataType.Builder builder =
        PRelDataType.newBuilder().setNullable(type.isNullable()).setTypeName(name);
    ...
    if (type.getSqlTypeName() == SqlTypeName.STRUCTURED
        || type.getSqlTypeName() == SqlTypeName.ROW) {
      builder
          .setStructKind(toProto(type.getStructKind()))
          .addAllChildren(
              type.getFieldList().stream().map(this::toProto).collect(Collectors.toList()));
    }
  ...
  private RelDataType fromProtoBase(PRelDataType type) {
    ...
    final int precision =
        (name.allowsPrec() && type.getPrecision() != SqlTypeName.DEFAULT_INTERVAL_START_PRECISION)
            ? type.getPrecision()
    ...
    switch (type.getTypeName()) {
      ...
      case VARCHAR:
        final RelDataType newType =
            name.allowsPrec()
                ? factory.createSqlType(name, type.getPrecision())
                : factory.createSqlType(name);
        final Charset charset =
            StringUtils.isEmpty(type.getCharset())
                ? factory.getDefaultCharset()
                : Charset.forName(type.getCharset());
      ...
      case STRUCTURED:
        return factory.createStructType(
            ...
            type.getChildrenList().stream()
                .map(c -> fromProto(c.getType()))
    ...
    throw new UnsupportedOperationException(
        String.format("Unhandled serialization type: %s.", type));
  }
