...
public final class LogicalMatchSerde implements RelNodeSerde<LogicalMatch, PLogicalMatch> {
  ...
  public PLogicalMatch serialize(LogicalMatch node, RelToProto s) {
    PLogicalMatch.Builder builder = PLogicalMatch.newBuilder()
      ...
      .addAllPartitionKeys(node.getPartitionKeys().stream().map(s::toProto).collect(Collectors.toList()))
  ...
  private static Map<String, PRexNode> toProto(ImmutableMap<String, RexNode> map, RelToProto relToProto) {
  ...
  private static Map<String, PLogicalMatch.Strings> toProto(ImmutableMap<String, SortedSet<String>> subsets) {
    Preconditions.checkNotNull(subsets);

    Map<String, PLogicalMatch.Strings> protoSubsets = new HashMap<>();
    for(String key : subsets.keySet()) {
      protoSubsets.put(key, PLogicalMatch.Strings.newBuilder().addAllValues(subsets.get(key).stream().collect(Collectors.toList())).build());
    }

    return protoSubsets;
  }

  private static Map<String, RexNode> fromProto(Map<String, PRexNode> protoMap, RelFromProto relFromProto) {
  ...
  private static Map<String, SortedSet<String>> fromProto(Map<String, PLogicalMatch.Strings> protoSubsets) {
