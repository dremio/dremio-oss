...
/**
 * Contains the list of the supported up-promotions for a table schema between a filesystem source
 * {@link CompleteType} and the destination table {@link CompleteType}.
 */
public class SchemaUpPromotionRules {
  /**
   * Gets the up-promoted {@link CompleteType} from a table {@link CompleteType} and a file {@link
  ...
  private Optional<CompleteType> getResultantTypeForDecimalFileType(
      CompleteType fileType, CompleteType tableType) {
    ...
    if (tableType.isValidDecimal()) {
      ...
      return Optional.of(
          new CompleteType(Decimal.createDecimal(outputPrecision, outputScale, null)));
    }
    return Optional.empty();
  }

  private Optional<CompleteType> getResultantTypeForVarcharFileType(CompleteType tableType) {
    if (tableType.equals(BIT)
        || tableType.equals(INT)
        || tableType.equals(BIGINT)
        || tableType.equals(FLOAT)
        || tableType.equals(DOUBLE)
        || tableType.isValidDecimal()
  ...
  private Optional<CompleteType> handlePrecisionOverflow(
      CompleteType fileType, CompleteType tableType) {
  ...
  private int computeIntegral(CompleteType fileType, CompleteType tableType) {
    return Math.max(
        ...
        tableType.getPrecision() - tableType.getScale());
  }
