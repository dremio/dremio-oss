...
{
  <L_CONTAINS> DQuery(b, fieldMap) <L_RPAREN>
  {
    return SqlContains.getNode(getPos(), fieldMap, b.toString().trim());
  }
}

void DQuery(StringBuilder b, Map<String,String> fieldMap) :
{
}
{
...
}
{
  ...
  (
   Term(b)
   | <L_LPAREN> { b.append(token.image); } DQuery(b, fieldMap) <L_RPAREN> { b.append(token.image + " "); } (<CARAT> { b.append(token.image + " "); } <L_NUMBER> { b.append(token.image + " "); } )?
