...
public final class RootAllocatorFactory {
  ...
  private static final class InternalFactory {
    static {
      ...
      // memory.
      // Try to change Netty's behavior before the default allocator is being created by Netty. This needs to be done once.
  ...
  public static BufferAllocator newRoot(final DremioConfig config) {
    return InternalFactory.newRoot(config.getSabotConfig().getLong(TOP_LEVEL_MAX_ALLOC),
