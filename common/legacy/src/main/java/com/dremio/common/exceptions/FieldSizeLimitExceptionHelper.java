...
public final class FieldSizeLimitExceptionHelper {
  ...
  public static void checkSizeLimit(
      int size, int maxSize, int fieldIndex, org.slf4j.Logger logger) {
    if (size > maxSize) {
  ...
  public static void checkSizeLimit(
      int size, int maxSize, String fieldName, org.slf4j.Logger logger) {
    if (size > maxSize) {
      throw createFieldSizeLimitException(size, maxSize, fieldName, logger);
    }
  }

  public static UserException createListChildrenLimitException(
      String fieldName, long maxSize, long size) {
    return UserException.unsupportedError(new FieldSizeLimitException())
        .message(
            "List field '%s' exceeded the maximum number of elements %d, actual number of elements is %d",
            fieldName, maxSize, size)
        .addContext("limit", maxSize)
        .build();
  }

  public static UserException createFieldSizeLimitException(
      int size, int maxSize, org.slf4j.Logger logger) {
    return UserException.unsupportedError(new FieldSizeLimitException())
        .message(
            "Field exceeds the size limit of %d bytes, actual size is %d bytes.", maxSize, size)
  ...
  public static UserException createFieldSizeLimitException(
    ...
    return UserException.unsupportedError(new FieldSizeLimitException())
        .message(
            "Field with index %d exceeds the size limit of %d bytes, actual size is %d bytes.",
            fieldIndex, maxSize, size)
  ...
  public static UserException createFieldSizeLimitException(
    ...
    return UserException.unsupportedError(new FieldSizeLimitException())
        .message(
            "Field '%s' exceeds the size limit of %d bytes, actual size is %d bytes.",
            fieldName, maxSize, size)
