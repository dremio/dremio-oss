...
public class DremioStringUtils {
  ...
  private static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(DremioStringUtils.class);
  ...
  public static String readable(long bytes) {
    ...
    return String.format(
        "%s%.1f %ciB", (bytes == absBytes ? "" : "-"), absBytes / Math.pow(unit, exp), pre);
  }
  ...
  /**
   * Escapes the characters in a {@code String} according to Java string literal rules.
   *
   * <p>Deals correctly with quotes and control-chars (tab, backslash, cr, ff, etc.) so, for
   * example, a tab becomes the characters {@code '\\'} and {@code 't'}.
   *
   * <p>Example:
   *
   * <pre>
  ...
  /**
   * Return a printable representation of a byte array, escaping the non-printable bytes as '\\xNN'
   * where NN is the hexadecimal representation of such bytes.
   */
   ...
   * For example, the string "-001 18:25:16.766" defines an interval of - 1 day 18 hours 25 minutes
   * 16 seconds and 766 milliseconds
   */
  public static String formatIntervalDay(final Period p) {
    long millis =
        p.getDays() * (long) DateUtility.daysToStandardMillis + JodaDateUtility.millisFromPeriod(p);
