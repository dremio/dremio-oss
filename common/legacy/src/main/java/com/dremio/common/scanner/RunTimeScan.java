...
public class RunTimeScan {
  private static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(RunTimeScan.class);
  ...
  private static ScanResult createFromSavedScanResults() {
    ...
    if (scanResultsFile.exists()) {
      try {
        ...
        logger.warn(
            "Unable to read scan result from {} (proceeding to slow path): {}",
            scanResultsFile.getName(),
            e.toString());
      }
    }
    return null;
  ...
  /**
   * loads prescanned classpath info and scans for extra ones based on configuration. (unless
   * prescan is disabled with {@see ClassPathScanner#IMPLEMENTATIONS_SCAN_CACHE}=falses) If
   * ScanResult was generated at build time and is indicated by com.dremio.savedScanResults, then
