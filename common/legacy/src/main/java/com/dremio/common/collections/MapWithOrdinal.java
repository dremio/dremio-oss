 ...
 * corresponding to a key via {@link
 * com.dremio.common.collections.MapWithOrdinal#getOrdinal(Object)} before attempting to execute a
 * lookup with an ordinal.
 *
 * @param <K> key type
 * @param <V> value type
 */
public class MapWithOrdinal<K, V> implements Map<K, V> {
   ...
   * Map#remove} with ordinal re-cycling. The ordinal corresponding to the given key may be
   * re-assigned to another tuple. It is important that consumer checks the ordinal value via {@link
   * #getOrdinal(Object)} before attempting to look-up by ordinal.
   *
   * <p>{@see java.util.Map#remove}
   */
  @Override
  public V remove(Object key) {
