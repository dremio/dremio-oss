...
public interface FileSystem extends Closeable {
   ...
   *
   * <p>Note that if {@code f} already exists, it will be overwritten
   *
   ...
   * @return an output stream to write file content
   * @throws java.nio.file.FileAlreadyExistsException if file already exists and {@code overwrite}
   ...
   * @throws IOException if an error occurs when accessing file information
   */
  void setPermission(Path p, Set<PosixFilePermission> permissions)
   ...
   *
   * <p>If parent directories do not exist, they are created too.
   *
   * @param f the directory to create
   * @param permissions the directory permission
   * @return {@code true} if directory was successfully created, {@code false} if the directory
   *     already exists
   ...
   * @throws IOException if an error occurs during listing
   */
  DirectoryStream<FileAttributes> list(Path f, Predicate<Path> filter)
      throws FileNotFoundException, IOException;
   ...
   * @throws IOException if an error occurs during listing
   */
  DirectoryStream<FileAttributes> listFiles(Path f, boolean recursive)
      throws FileNotFoundException, IOException;
   ...
   * @throws IOException if an error occurs during listing
   */
  DirectoryStream<FileAttributes> glob(Path pattern, Predicate<Path> filter)
      throws FileNotFoundException, IOException;
   ...
   *
   * <p>If {@code f} designates a directory and {@code recursive} is set to {@code true}, all
   * deletes each directory children recursively.
   *
   ...
   *
   * <p>Only block locations for region designated by {@code start} and {@code len} are returned
   *
   * @param file the file status of the path to get block locations from
   * @param start the start offset of the region
   * @param len the length of the region
   * @return an array of block locations. The array might be empty if {@code len} is 0, or start
   *     exceeds the file size, otherwise it contains at least one element
   * @throws IOException if an error occurs while checking
   */
  Iterable<FileBlockLocation> getFileBlockLocations(FileAttributes file, long start, long len)
      throws IOException;
   ...
   * <p>Only block locations for region designated by {@code start} and {@code len} are returned
   *
   * @param p the file to get block locations from
   * @param start the start offset of the region
   * @param len the length of the region
   * @return an array of block locations. The array might be empty if {@code len} is 0, or start
   *     exceeds the file size, otherwise it contains at least one element
   * @throws IOException if an error occurs while checking
   */
  Iterable<FileBlockLocation> getFileBlockLocations(Path p, long start, long len)
      throws IOException;

  /**
   * Checks if the path designated by {@code path} is accessible for the given {@code mode}.
   *
   ...
   * @throws IOException if an error occurs while checking
   */
  void access(final Path path, final Set<AccessMode> mode)
   ...
   *
   * @param path the default block size filesystem would use to write {@code path}
   * @return the size in bytes, -1 if the filesystem doesn't know/do not provide to directly access
   *     based on blocks
   */
   ...
   */
  AsyncByteReader getAsyncByteReader(FileKey fileKey, Map<String, String> options)
      throws IOException;

  /**
   * Calculation of host affinities for various filesystems are different. For example, HDFS read
   * can be on any of the replicas of the block. So, no ordering is required. In case of cached file
   * system, the read has to hit the node that cached the data. So, ordering of the locations is
   * required.
   *
   * @return
   */
