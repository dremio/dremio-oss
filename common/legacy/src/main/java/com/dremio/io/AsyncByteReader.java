...
public interface AsyncByteReader extends AutoCloseable {
  ...
  /**
   * Checks if the version of the file being read has changed. Note that this should create a
   * singleton instance of the Future to checkVersion. This API is called repeatedly as part of
   * {@link #versionedReadFully} and would cause a performance degradation if the version check is
   * done multiple times.
   *
   ...
   * @return
   */
  default CompletableFuture<Void> versionedReadFully(
      String version, long offset, ByteBuf dst, int dstOffset, int len) {
    return CompletableFuture.allOf(checkVersion(version), readFully(offset, dst, dstOffset, len))
        .whenComplete(
            (v, e) -> {
              if (e != null) {
  ...
  /**
   * Interface to fetch statistics for this async-reader, the actual statistics and values returned
   * in the list are implementation specific.
   */
  class ReaderStat {
    private String name;
  ...
  /**
   * Interface to exchange all properties associated with the file or object for which the async
   * reader is required. The full path, version, and file type should completely qualify this file
   * or object in the underlying layer that implements the async reader.
   */
  interface FileKey {
    ...
    static FileKey of(
        Path path, String version, FileType fileType, List<String> dataset, String pluginUID) {
      ...
      return new FileKey() {
        private final List<String> datasetKey =
            dataset != null ? ImmutableList.copyOf(dataset) : null;
