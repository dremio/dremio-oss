/*
 * Copyright (C) 2007-2009 David Yu dyuproject@gmail.com
 ...
 *
 * <p>This is a fork of https://github.com/protostuff/protostuff/blob/master/protostuff-json/src/
 * main/java/io/protostuff/JsonIOUtil.java that removes implementation of getParserFeatures and
 * getGeneratorFeatures in the custom JSON factory.
 *
 * @author David Yu created Nov 20, 2009
 */
public final class JsonIOUtils {
   ...
   * inBuffer} to use.
   */
  public static UTF8StreamJsonParser newJsonParser(
      InputStream in, byte[] buf, int offset, int limit) throws IOException {
   ...
   * inBuffer} to use.
   */
  static UTF8StreamJsonParser newJsonParser(
      InputStream in,
      byte[] buf,
      int offset,
      int limit,
  ...
  public static UTF8JsonGenerator newJsonGenerator(OutputStream out, byte[] buf) {
    return newJsonGenerator(
        out, buf, 0, false, new IOContext(DEFAULT_JSON_FACTORY._getBufferRecycler(), out, false));
  }
   ...
   * outBuffer} to use.
   */
  static UTF8JsonGenerator newJsonGenerator(
      OutputStream out, byte[] buf, int offset, boolean bufferRecyclable, IOContext context) {
  ...
  /** Merges the {@code message} with the byte array using the given {@code schema}. */
  public static <T> void mergeFrom(
      byte[] data, int offset, int length, T message, Schema<T> schema, boolean numeric)
      throws IOException {
    final IOContext context = new IOContext(DEFAULT_JSON_FACTORY._getBufferRecycler(), data, false);
    final JsonParser parser = newJsonParser(null, data, offset, offset + length, false, context);
    /*
  ...
  public static <T> void mergeFrom(InputStream in, T message, Schema<T> schema, boolean numeric)
    ...
    try (JsonParser parser = newJsonParser(in, context.allocReadIOBuffer(), 0, 0, true, context)) {
      mergeFrom(parser, message, schema, numeric);
    }
  }
   ...
   * Merges the {@code message} from the {@link InputStream} using the given {@code schema}.
   *
   * <p>The {@link LinkedBuffer}'s internal byte array will be used when reading the message.
   */
  public static <T> void mergeFrom(
      InputStream in, T message, Schema<T> schema, boolean numeric, LinkedBuffer buffer)
      throws IOException {
    final IOContext context = new IOContext(DEFAULT_JSON_FACTORY._getBufferRecycler(), in, false);
  ...
  public static <T> byte[] toByteArray(
      T message, Schema<T> schema, boolean numeric, LinkedBuffer buffer) {
    final ByteArrayOutputStream baos = new ByteArrayOutputStream();
    try {
  ...
  public static <T> void writeTo(OutputStream out, T message, Schema<T> schema, boolean numeric)
    ...
    try (JsonGenerator generator =
        newJsonGenerator(out, context.allocWriteEncodingBuffer(), 0, true, context)) {
  ...
  public static <T> void writeTo(
    ...
    try (JsonGenerator generator = newJsonGenerator(out, buffer.buffer, 0, false, context)) {
      writeTo(generator, message, schema, numeric);
    }
  }
  ...
  public static <T> void writeTo(
      JsonGenerator generator, T message, Schema<T> schema, boolean numeric) throws IOException {
  ...
  public static <T> void writeListTo(
      Writer writer, List<T> messages, Schema<T> schema, boolean numeric) throws IOException {
    try (JsonGenerator generator = DEFAULT_JSON_FACTORY.createJsonGenerator(writer)) {
      writeListTo(generator, messages, schema, numeric);
    }
  }
  ...
  public static <T> void writeListTo(
      JsonGenerator generator, List<T> messages, Schema<T> schema, boolean numeric)
      throws IOException {
   ...
   * <p>The {@link LinkedBuffer}'s internal byte array will be used when reading the message.
   */
  public static <T> List<T> parseListFrom(
      InputStream in, Schema<T> schema, boolean numeric, LinkedBuffer buffer) throws IOException {
    final IOContext context = new IOContext(DEFAULT_JSON_FACTORY._getBufferRecycler(), in, false);
    try (JsonParser parser = newJsonParser(in, buffer.buffer, 0, 0, false, context)) {
      return parseListFrom(parser, schema, numeric);
    }
