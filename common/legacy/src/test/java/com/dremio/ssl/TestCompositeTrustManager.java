...
public class TestCompositeTrustManager {
  ...
  public void testFactoryEmptyTrustManager() {
    final CompositeTrustManagerFactory factory =
        CompositeTrustManagerFactory.newBuilder().addTrustStore(Collections.emptyList()).build();
  ...
  public void testCreateByFactory() {
    final CompositeTrustManagerFactory factory =
        CompositeTrustManagerFactory.newBuilder().addDefaultTrustStore().build();
    final TrustManager[] tms = factory.getTrustManagers();
    assertEquals(1, tms.length);
  ...
  public void testAggregatesIssuers() {
    final CompositeTrustManager mgr =
        new CompositeTrustManager(
            ImmutableList.of(new FailingTrustManager(), new AlwaysSuccessTrustManager()));

    assertArrayEquals(
        new X509Certificate[] {
          FailingTrustManager.CERT1,
          FailingTrustManager.CERT2,
          AlwaysSuccessTrustManager.CERT1,
          AlwaysSuccessTrustManager.CERT2
        },
  ...
  public void testEmtpyManagerCheckClientCalled() {
    checkCompositeDelegation(
        (tm -> tm.checkClientTrusted(null, null)), (tm -> tm.checkClientCalled));
  }
  ...
  public void testEmptyManagerCheckServerCalled() {
    checkCompositeDelegation(
        (tm -> tm.checkServerTrusted(null, null)), (tm -> tm.checkServerCalled));
  }
  ...
  /**
   * Creates a CompositeTrustManager composed of the given trustManagers in the same order and
   * verifies that delegation across each is done correctly (or short-circuited). Also verifies if
   * the overall call succeeds or fails depending on if there was at least one passing trust
   * manager.
   */
  private void checkCompositeDelegation(
      TrustFunction trustFunction,
      Function<SingleStateX509ExtendedTrustManager, Boolean> checkDelegated,
    ...
    for (SingleStateX509ExtendedTrustManager curr : trustManagers) {
      ...
      shouldHaveDelegatedToCurrent = prev == null || prev instanceof FailingTrustManager;
