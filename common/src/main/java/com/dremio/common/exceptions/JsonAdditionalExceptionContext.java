...
public abstract class JsonAdditionalExceptionContext implements AdditionalExceptionContext {
  private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(JsonAdditionalExceptionContext.class);
  private static final ObjectMapper contextMapper = new ObjectMapper();

  // all classes that extend AdditionalExceptionContext must register themselves with the contextMapper.
  static {
  ...
  protected static <T extends AdditionalExceptionContext> T fromUserException(Class<T> clazz, UserException ex) {
    try {
      ...
      // If the AdditionalExceptionContext data can't deserialize due to whatever reason, it's fine since we
